 ...
 * @par 
 *   This tool extends functionality of @c ITriggerSelectionTisTos (from which it inherits). While @c ITriggerSelectionTisTos member functions accept
 *   for trigger input only specific Trigger Selection names (which cannot be checked for validity), @c ITriggerTisTos tool knows valid Trigger
 *   Selection names and their hierarchy. Therefore, trigger input can be specified via trigger name pattern which contains wild characters.
 *   This allows user to define trigger structures independently of the particular trigger configuration. 
 *   The Trigger Selections by default are restricted to be among exit points from the Alley system, but this restrictions can be waived.
 *   TIS,TOS classifications can be drilled down entire trigger tree by following input Trigger Selections to the Trigger Selections requested by the caller. 
 *   Auxiliary output functions allow discovering valid Trigger Selection names and their input Trigger Selections, as well as selections matching
 *   decision/TIS/TOS requirements.
 *   @c triggerTisTos output functions belong to @c ITriggerTisTos and allow generic Trigger Input, while @c selectionTisTos output functions inherited
 *   from @c ITriggerSelectionTisTos and require specific Trigger Selection names.
 * @par
 *   User can also obtain a list of LHCbIDs of hits derived from the Offline Input used by the tool to perform matching between the offline and
 *   trigger content, and best TOS matches of trigger objects (tracks, vertices or particles dependening on the type of selection) for specific
 *   Trigger Selection. These output functions are inherited from @c ITriggerSelectionTisTos.  
 * @par
 ...
 *   Trigger Input is defined by Trigger Selection Name pattern passed as @c std::string (or std::vector<std::string>) which may contain
 *   multiple wild characters.
 *   Examples:  "*", "MuonTrigger*", "MuonTrigger*DiMu*", "MuonTriggerSingle"
 * @par
 *   If Trigger Input corresponds to many trigger selection names, the TisTosing output corresponds to an OR between these selections.
 * @par
 *   Trigger Selections are restricted to belong to exit selections from any trigger Alley, unless optional input parameter @c alleyExitsOnly is set to false. 
 * @par
 ...
 * @par
 *   Once the Trigger Input is defined, many calls to the output functions can be made without re-entering the Trigger Input.
 * @par
 *   The Trigger Input can also be passed in functions which retrieve outputs, but this becomes inefficient if many output function calls are
 *   made passing the same Trigger Input (as the Trigger Input is pre-processed and stored in an internal representation).
 * @par
 *   The tool will print a warning message if the passed Trigger Input pattern does not correspond to any trigger selection.
 * @par 
 *   User can obtain names of trigger selections corresponding to the Trigger Input by calling @c triggerSelectionNames() with no arguments
 *   (input selection tree can be explored by calling @c inputTriggerSelectionNames(selectionName) ).
 *   @c triggerSelectionNames takes also optional input parameters for (decision,TIS,TOS) pattern to be matched (0=false, 1=true, 2=anything).
 *   Thus e.g. @c triggerSelectionNames(ITrackTisTos::kAnything,ITrackTisTos:kAnything,ITrackTisTos::kTrueRequired) returns all TOS selections matching
 *   the Trigger Input definition
 * 
 * @par Outputs of @c triggerTisTos 
 *   @c decision, @c TIS, @c TOS - are all of @c bool type. 
 * @par
 *   @c decision is true if any selection corresponding to the Trigger Input was satisfied. This output is independent of the Offline Input!
 * @par
 *   @c TIS = Trigger Independent of Signal ("Signal" = Offline Input) is true if the any selection corresponding to the Trigger Input 
 *   is satisfied with trigger tracks/vertices/particles which are not matched to the Offline Input. Matching criteria are implementation dependent.   
...
public: 
  ...
  /// meaning of 3 optional unsigned int input parameters passed to @c triggerSelectionNames() output function (for decision,tis,tos requirement) 
  ...
  /// shortcut to define Trigger Input via Trigger Name Pattern and calculate decision,tis,tos with respect to previously defined Offline Input
  ...
  /// shortcut to define Trigger Input via Trigger Name Patterns and calculate decision,tis,tos with respect to previously defined Offline Input
   ...
   *  @par
   *       Returned Trigger Selection names do not include their input selection names, even if @c checkInputSelections is set to true.
  ...
  /// shortcut to define Trigger Input and return Trigger Selection names matching decision,tis,tos pattern for previously defined Offline Input 

...
class MassFilterDs(Hlt2ParticleFilter):
    def __init__(self, name, inputs):
        cut = "in_range( %(Ds_Mass_M_MIN)s , M , %(Ds_Mass_M_MAX)s )"
        Hlt2ParticleFilter.__init__(self, linePrefix, name, cut, inputs)

class MassFilterDp(Hlt2ParticleFilter):
    def __init__(self, name, inputs):
        cut = "in_range( %(Dp_Mass_M_MIN)s , M , %(Dp_Mass_M_MAX)s )"
        Hlt2ParticleFilter.__init__(self, linePrefix, name, cut, inputs)

# The class that creates the Hlt2Combiner
from Hlt2Lines.Utilities.Hlt2Combiner import Hlt2Combiner
class D2HHHCombiner(Hlt2Combiner):
    def __init__(self, name):
...
# Combiners
from Inputs import Hlt2LoosePions, Hlt2LooseKaons
D2KPiPi_SS  = D2HHHCombiner('KPiPi_SS')

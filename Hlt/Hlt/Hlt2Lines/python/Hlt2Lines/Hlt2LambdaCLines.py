...
class Hlt2LambdaCLinesConf(HltLinesConfigurableUser) :
   ...
   # define some values so that they are not hard-coded
    __slots__ = {
                  ...
                  ## prescales
                  , 'Prescale'                  : {
                         'Hlt2LambdaC_Lambda0LLPi'    : 1.0
                       , 'Hlt2LambdaC_Lambda0LLK'     : 1.0
                       , 'Hlt2LambdaC_Lambda0DDPi'    : 1.0  
                       , 'Hlt2LambdaC_Lambda0DDK'     : 1.0
                        
                        }
                  , 'HltANNSvcID'  : {
                       'Hlt2LambdaC_Lambda0LLPiDecision'  : 60913
                      ,'Hlt2LambdaC_Lambda0LLKDecision'   : 60914
                      ,'Hlt2LambdaC_Lambda0DDPiDecision'  : 60915  
                      ,'Hlt2LambdaC_Lambda0DDKDecision'   : 60916
    ...
    def __Lambda0LLFilter(self, name, inputContainers) : 
        ...
        incuts =
                 "CHILDCUT('pi+'==ABSID , %(Lambda0LLDaugPionTrackChi2)s)" \
                 "CHILDCUT('p+'==ABSID , %(Lambda0LLDaugProtonTrackChi2)s)" \

                 "& (VFASPF(VCHI2PDOF) < %(Lambda0LLVertexChi2)s)" \

                 "& (PT > %(Lambda0LLPT)s)" \

                 "CHILDCUT('pi+'==ABSID , %(Lambda0LLDaugPionMIPChi2)s)" \
                 "CHILDCUT('p+'==ABSID , %(Lambda0LLDaugProtonMIPChi2)s)" \
    ...
    def __Lambda0DDFilter(self, name, inputContainers) : 
        ...
        incuts =
                 ...
                 "CHILDCUT('pi+'==ABSID , %(Lambda0DDDaugPionTrackChi2)s)" \
                 "CHILDCUT('p+'==ABSID ,  %(Lambda0DDDaugProtonTrackChi2)s)" \

                 "& (VFASPF(VCHI2PDOF) < %(Lambda0DDVertexChi2)s)" \

                 "& (PT > %(Lambda0DDPT)s)" \

                 "CHILDCUT('pi+'==ABSID , %(Lambda0DDDaugPionMIPChi2)s)" \
                 "CHILDCUT('p+'==ABSID ,  %(Lambda0DDDaugProtonMIPChi2)s)" \

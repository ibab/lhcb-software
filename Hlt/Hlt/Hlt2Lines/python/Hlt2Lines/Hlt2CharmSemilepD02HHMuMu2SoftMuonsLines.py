...
class Hlt2CharmSemilepD02HHMuMu2SoftMuonsLinesConf(HltLinesConfigurableUser) :
    __slots__ = {
                 ## HHmumu
                    'TrkPt_HHmumu'                 : 250.0 * MeV
                  , 'TrkP_HHmumu'                  : 2500.0 * MeV
                  , 'TrkPVIPChi2_HHmumu'           : 0.0      # unitless
                  , 'TrkPVIPChi2MAX_HHmumu'           : 8.0      # unitless
                  ...
                  # prescales
                  , 'Prescale'                  : {
                        'Hlt2CharmSemilepD02HHMuMu2SoftMuonsWideMass'    : 0.1
                        }
                  , 'HltANNSvcID'  : {
                          'Hlt2CharmSemilep2HadForD02HHMuMu2SoftMuonsDecision' : 60006
                        , 'Hlt2CharmSemilepD02HHMuMu2SoftMuonsDecision'         : 60007
                        , 'Hlt2CharmSemilepD02HHMuMu2SoftMuonsWideMassDecision' : 60008
                        }
                }
    ...
    def __seqGEC(self) : # {
        ...
        Hlt2CharmKillTooManyInTrkAlg = VoidFilter('Hlt2CharmSemilepD02HHMuMu2SoftMuonsKillTooManyInTrkAlg'
                                                 , Code = filtCode
                                                )
    ...
    def __apply_configuration__(self) :
        ...
        MuonsFiltered = self.__InPartFilter('CharmHHmumu2SoftMuonsInputMu', [ BiKalmanFittedSecondLoopMuons] )
        ...
        # Make HHmumu 
        CharmHHmumuCombine = self.__HHmumuCombine ( name = 'CharmSemilepD02HHMuMu2SoftMuons'
                                                  , inputSeq = [Hlt2Charm2HadForHHmumu, MuonsFiltered]
        ...
        # HHmumu line
        Hlt2CharmHHmumu2SoftMuons = self.__HHmumuFilter ( name = 'CharmSemilepD02HHMuMu2SoftMuons', inputSeq = [CharmHHmumuCombine], extracode = sigMassCut )
        # HHmumu WideMass line - with prescale
        Hlt2CharmHHmumu2SoftMuonsWideMass = self.__HHmumuFilter (name = 'CharmSemilepD02HHMuMu2SoftMuonsWideMass', inputSeq = [CharmHHmumuCombine], extracode = wideMassCut )
        ...
        line = Hlt2Line('CharmSemilep2HadForD02HHMuMu2SoftMuons', prescale = self.prescale
                        , algos = [ PV3D(), Hlt2CharmKillTooManyInTrk, Hlt2Charm2HadForHHmumu]
                        , postscale = self.postscale
                        )
        decName = "Hlt2CharmSemilep2HadForD02HHMuMu2SoftMuonsDecision"
        annSvcID = self._scale(decName,'HltANNSvcID')
        HltANNSvc().Hlt2SelectionID.update( { decName : annSvcID } )
 
        line = Hlt2Line('CharmSemilepD02HHMuMu2SoftMuons', prescale = self.prescale
                        , algos = [ PV3D(), Hlt2CharmKillTooManyInTrk, Hlt2Charm2HadForHHmumu,MuonsFiltered, Hlt2CharmHHmumu2SoftMuons]
                        , postscale = self.postscale
                        )
        decName = "Hlt2CharmSemilepD02HHMuMu2SoftMuonsDecision"
        annSvcID = self._scale(decName,'HltANNSvcID')
        HltANNSvc().Hlt2SelectionID.update( { decName : annSvcID } )

        line = Hlt2Line('CharmSemilepD02HHMuMu2SoftMuonsWideMass', prescale = self.prescale
                        , algos =  [ PV3D(), Hlt2CharmKillTooManyInTrk, Hlt2Charm2HadForHHmumu,MuonsFiltered, Hlt2CharmHHmumu2SoftMuonsWideMass]
                        , postscale = self.postscale
                        )
        decName = "Hlt2CharmSemilepD02HHMuMu2SoftMuonsWideMassDecision"
        annSvcID = self._scale(decName,'HltANNSvcID')
        HltANNSvc().Hlt2SelectionID.update( { decName : annSvcID } )

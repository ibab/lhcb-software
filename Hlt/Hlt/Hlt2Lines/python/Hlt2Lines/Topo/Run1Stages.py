...
class FilterEvent(Hlt2VoidFilter):
    ...
    def __init__(self):
        ...
        ec = "CONTAINS('%s') < %%(RUN1_GEC_MAX)s" % inputs.outputSelection()
        Hlt2VoidFilter.__init__(self, 'Run1TopoEvent', ec, [inputs],
                                shared = True)
...
class FilterParts(Hlt2ParticleFilter):
    ...
    def __init__(self, pid, tag, inputs, gec):
        if pid == 'Kaon':
            pc = ("(PT > %%(RUN1_ALL_PT_MIN)s) & (P > %%(RUN1_ALL_P_MIN)s) "
                  "& (MIPCHI2DV(PRIMARY) > %%(RUN1_ALL_MIPCHI2DV_MIN)s) "
                  "& ((TRCHI2DOF < %%(RUN1_HAD_TRCHI2DOF_MAX)s) "
                  "| ((TRCHI2DOF < %%(RUN1_%s_TRCHI2DOF_MAX)s)" % tag)
            if   tag == 'MU': pc += " & (HASMUON & ISMUON)))"
            elif tag == 'E':  pc += " & (PIDe > %(RUN1_PIDE_MIN)s)))"
            else:             pc += '))'
        else:
        ...
        Hlt2ParticleFilter.__init__(self, 'Run1TopoInput' + pid + tag, pc,
...
class FilterMforN(Hlt2ParticleFilter):
    ...
    def __init__(self, tag, m, n, inputs):
        ...
        if m == 2: pc = ("(M < %f) & (VFASPF(VCHI2)"
                         " < %%(RUN1_V2BODYCHI2_MAX)s)" %
                         (5000 * MeV if n == 4 else 6000 * MeV))
        else:      pc = '(M < %f)' % (6000 * MeV)
        Hlt2ParticleFilter.__init__(self, 'Run1Topo%s%iFor%i' % (tag, m, n),
...
class FilterNforN(Hlt2ParticleFilter):
    ...
    def __init__(self, tag, n, inputs):
        ...
        pc = ("(SUMTREE(PT, %s, 0.0) > %f) & (NINTREE(('KS0' == ID) "
              "| ('Lambda0' == ABSID)) <= %%(RUN1_NV0_%iBody_MAX)s) "
              "& (INTREE(ISBASIC "
              "& (MIPCHI2DV(PRIMARY) > %%(RUN1_ONETRACK_IPCHI2_MIN)s) "
              "& ((PT > %%(RUN1_ONETRACK_PT_MIN)s) | ((HASMUON & ISMUON) "
              "& (PT > %%(RUN1_MU_ONETRACK_PT_MIN)s))))) & (MINTREE(HASTRACK "
              "& %s, TRCHI2DOF) < %%(RUN1_MIN_TRCHI2DOF_MAX)s)"
              % (pids, pt, n, pids))
        from HltTracking.HltPVs import PV3D
        Hlt2ParticleFilter.__init__(self, 'Run1Topo%s%ifor%i' % (tag, n, n),
                                    pc, inputs, shared = True, 
...
class FilterSimple(Hlt2ParticleFilter):
    ...
    def __init__(self, n, inputs, varmap, props):
        ...
        bdttool = Hlt1Tool(type = BBDTSimpleTool, name = 'TrgSimple',
                           Cuts = props['RUN1_SIMPLE_%dBODY_CUTS' % n], 
...
class FilterBDT(Hlt2ParticleFilter):
    ...
    def __init__(self, tag, n, inputs, varmap, props):
        ...
        elif tag == 'E':  pc += (" & INTREE(HASPROTO "
                                 "& (PIDe > %(RUN1_PIDE_MIN)s))")
        from HltTracking.HltPVs import PV3D
...
class CombineN(Hlt2Combiner):
    ...
    def __init__(self, tag, n, inputs):
        ...
        merged = Hlt2MergedStage('Run1Topo%s%iBody' % (tag, n), inputs, 
                                 shared = True)
        Hlt2Combiner.__init__(self, 'Run1Topo%s%iBody' % (tag, n), decays,

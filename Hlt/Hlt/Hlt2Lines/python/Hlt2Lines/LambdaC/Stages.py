...
class LambdaC2KPPiCombiner(Hlt2Combiner):
    def __init__(self, name):
        ...
        p_cut = ("(TRCHI2DOF < %(Trk_TRCHI2DOF_MAX)s )" +
                 ...
                 "& (P > %(P_P_MIN)s)" +
                 "& (MIPCHI2DV(PRIMARY) > %(Trk_MIPCHI2DV_MIN)s)")
        from Hlt2Lines.Utilities.Hlt2TisTosFilter import Hlt2Hlt1TrackAllL0TosTagger as TosFilter
        tosFilter = TosFilter()
        cuts = dict(DaughtersCuts  = {'pi+' : piK_cut, 'K+' : piK_cut, 'p+' : p_cut },
                    CombinationCut = (tosFilter.combinationCut() +
                                       "& (in_range( %(Lc_AM_MIN)s, AM, %(Lc_AM_MAX)s ))" +
        ...
        Hlt2Combiner.__init__(self, linePrefix, 'KPPi', "[Lambda_c+ -> K- p+ pi+]cc", cuts, inputs,
                              dependencies = [TrackGEC(linePrefix + 'TrackGEC'), PV3D('Hlt2'), tosFilter])

...
StatusCode HltMuonTrigger2MuonsNoIP::execute() {
  ...
  verbose() << "selvertices size " << m_selevertices << endmsg;
...
void HltMuonTrigger2MuonsNoIP::makeDiMuonPair(Hlt::VertexContainer& vcon)
{
  ...
  for (Hlt::TrackContainer::const_iterator it = m_inputTracks->begin();
    ...
    for (Hlt::TrackContainer::const_iterator it2 = it+1;
      ...
      verbose()<<"charges " << (*it)->charge() << " " << (*it2)->charge() << endmsg;
...
//===================================================
StatusCode HltMuonTrigger2MuonsNoIP::checkIfSameAncestor(RecVertex* vertexToTest)
{
  //If both muons come from same muon, do not consider this vertex
  ...
  for(  Hlt::VertexContainer::const_iterator L0dimuon=
    ...
    if(m_debug){
      ...
      verbose() << "first muon in PrepareDimuon charge, L0Flag " 
		...
		<< endmsg;
      verbose() << "second muon in PrepareDimuon charge, L0Flag " 
    ...
    for(SmartRefVector< LHCb::Track >::iterator 
      ...
      Track* ancestor=*ance;
      verbose()<<"ancestor of second muon "<<ancestor
	       <<" "<<secondMuon <<" history " 
	       << ancestor->checkHistory(Track::MuonID) <<endreq;

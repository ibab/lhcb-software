...
class HltAlgorithm : public HltBaseAlg, virtual public Hlt::IUnit {
  public:
    ...
    const Hlt::TSelection<T> &retrieveTSelection( const Gaudi::StringKey &key ) {
        ...
        if ( !sel )
            throw GaudiException( "Failed to down_cast Selection", key.str(),
                                  StatusCode::FAILURE );
        return *sel;
    }
  ...
  private:
    ...
    const Hlt::TSelection<T> *registerTESSelection( const Gaudi::StringKey &key ) {
        ...
        if ( sc.isFailure() ) {
            throw GaudiException( "Failed to register TES Selection", key.str(),
                                  StatusCode::FAILURE );
        }
      ...
      public:
        ...
        StatusCode execute() {
            ...
            m_parent.get<typename T::candidate_type::Range>(
                ...
                *m_selection;
            m_selection->setDecision(
                !m_selection->empty() ); // force it processed...

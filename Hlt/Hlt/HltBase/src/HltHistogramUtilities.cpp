...
namespace HistogramUtilities {
  ...
  //antonio
  bool setBinLabels( AIDA::IProfile1D* profile, const std::vector<std::pair<unsigned,std::string> >& labels ) {
    if (profile==0) return false;
    TProfile *prof = Gaudi::Utils::Aida2ROOT::aida2root( profile );  
    if (prof==0) return false;
    return setBinLabels(  prof->GetXaxis(), labels );
  }
  
  bool setBinLabels( AIDA::IProfile1D* profile, const std::vector<std::string>& labels ) {
    std::vector<std::pair<unsigned,std::string> > l;
    for (unsigned i = 0;i<labels.size();++i) {
      l.push_back(std::make_pair( i , labels[i] ) );
    }
    return setBinLabels(profile,l);
  }
};
};

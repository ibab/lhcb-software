...
const md5::digest_type& md5::digest()
{
    if (is_dirty)
    {
        ...
        pack(digest_value, state, sizeof(digest_type::value_type));
        m_digest.reset(digest_value);
...
{
    delete[] m_hex_str_value; m_hex_str_value = 0;
    ...
    for (unsigned int i(0); i < sizeof(value_type); ++i) {
        ...
        m_value[i] = static_cast<uint8_t>(value);
    }
}

const md5::digest_type::hex_str_value_type& md5::digest_type::hex_str_value() const
{
    if (m_hex_str_value == 0) {
        // Note: Have to cast because 'new char[33]' returns 'char*', not 'char(*)[33]'.
        m_hex_str_value = reinterpret_cast<hex_str_value_type*>(new hex_str_value_type);

        for (unsigned int i(0); i < sizeof(value_type); ++i) {
            sprintf(&(*m_hex_str_value)[i*2], "%02x", m_value[i]);
        }

        (*m_hex_str_value)[sizeof(hex_str_value_type) - 1] = '\0';  // Not necessary after sprintf.
    }

    return (*m_hex_str_value);
}

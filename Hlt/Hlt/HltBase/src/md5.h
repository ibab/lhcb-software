...
/*
    Modified G.Raven for LHCb use... (09/11/2007)
...
public:
    ...
    public:
        ...
        : m_hex_str_value(0) { reset(); }

        digest_type(const value_type& a_value)
        : m_hex_str_value(0) { reset(a_value); }

        digest_type(const hex_str_value_type& a_hex_str_value)
        : m_hex_str_value(0) { reset(a_hex_str_value); }

        digest_type(const digest_type& a)
        : m_hex_str_value(0) { reset(a.m_value); }
        ...
        {
            memcpy(m_value, a_value, sizeof(value_type));
            delete[] m_hex_str_value; m_hex_str_value = 0;
        }
    ...
    private:
        value_type m_value;
        mutable hex_str_value_type* m_hex_str_value;
    };

    ...
    : HltBaseAlg( name, pSvcLocator )
{
    ...
    declareProperty( "InputDataContainer",
                     m_DataName = LHCb::HltLumiSummaryLocation::Default );
...
StatusCode LumiHistoMaker::initialize()
{
    ...
    for ( std::vector<std::string>::iterator ivar = m_Variables.begin();
        ...
        if ( counter == LHCb::LumiCounters::Unknown ) {
            ...
            AIDA::IHistogram1D* countHisto =
                initializeHisto( *ivar, -0.5, double( bins ) - 0.5, bins );
            ...
            AIDA::IHistogram1D* digitHisto =
                initializeHisto( *ivar + "_threshold", -0.5, 1.5, 2 );
            m_ThresholdHistos.push_back( digitHisto );

            debug() << "Variable: " << *ivar << " Key: " << counter
                    << " Bins: " << bins << " Threshold: " << m_Thresholds[i]
                    << endmsg;
        }
    }
...
StatusCode LumiHistoMaker::execute()
{
    ...
    for ( unsigned int i = 0; i < m_Histos.size(); ++i ) {
        ...
        if ( msgLevel( MSG::DEBUG ) )
            debug() << "histo:" << m_names[i] << " value " << value << " threshold "
                    << digit << endmsg;
    }

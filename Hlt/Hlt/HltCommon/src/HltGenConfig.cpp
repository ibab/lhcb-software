...
StatusCode HltGenConfig::initialize()
{
    ...
    if ( m_label.empty() ) {
        error() << "You must specify short, descriptive label for a configuration..."
                << endmsg;
        return StatusCode::FAILURE;
    }
    return sc;
...
StatusCode HltGenConfig::generateConfig() const
{
    ...
    ConfigTreeNode topConfig( ConfigTreeNode::digest_type::createInvalid(), depRefs,
                              m_label );
    ...
    ConfigTreeNodeAlias::alias_type topAlias = m_accessSvc->writeConfigTreeNodeAlias(

// $Id: HltGenConfig.cpp,v 1.7 2008-06-30 08:56:53 graven Exp $
// Include files 
#include <algorithm>
...
//TODO: return statuscode, use back inserter...
template <typename COMP, typename I, typename R>
vector<ConfigTreeNode::digest_type> HltGenConfig::getDependencies(I i, I end, R resolver) const {
  vector<ConfigTreeNode::digest_type> depRefs; 
  for (; i!= end; ++i) {
      info() << "Generating config for " << *i << endmsg;
      COMP *comp(0); 
      if (!resolver( *i, comp ).isSuccess()) {
          error() << string("Unable to get ")+System::typeinfoName(typeid(COMP))+*i << endmsg;
          return vector<ConfigTreeNode::digest_type>();
      }
...
StatusCode HltGenConfig::generateConfig() const {
  ...
  typedef StatusCode (ISvcLocator::*fun_t)(const std::string&,IService*&);
  fun_t fun = &ISvcLocator::getService; 
  vector<ConfigTreeNode::digest_type> svcRefs = getDependencies<IService>(m_svcConfig.begin(),
                                                                          m_svcConfig.end(), 
                                                                          bl::bind( fun, serviceLocator(), bl::_1, bl::_2 ) );
  depRefs.insert(depRefs.end(), svcRefs.begin(), svcRefs.end());
  vector<ConfigTreeNode::digest_type> algRefs = getDependencies<IAlgorithm>(m_topConfig.begin(),
                                                                            m_topConfig.end(), 
                                                                            bl::bind( &IAlgManager::getAlgorithm, m_appMgr, bl::_1, bl::_2 ) );
  depRefs.insert(depRefs.end(), algRefs.begin(), algRefs.end());

  IService* appMgr = svc<IService>( "ApplicationMgr" );
  SmartIF<IProperty> propMgr ( IID_IProperty, appMgr );
  std::string AppName("Unknown");    propMgr->getProperty( "AppName", AppName ).ignore();
  std::string AppVersion("Unknown"); propMgr->getProperty( "AppVersion", AppVersion ).ignore();

  std::string comment = "head of ConfigTree generated by HltGenConfig";
  comment += std::string(" AppName: ") + AppName;
  comment += std::string(" AppVersion: ") + AppVersion;
  // TODO: add who ran this, at what time, on which machine?

  ConfigTreeNode topConfig( ConfigTreeNode::digest_type::createInvalid(),depRefs, comment);

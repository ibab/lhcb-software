...
void teacher() 
{
  ...
  // setup NeuroBayes
  //
	const int nvar = 23;                  // number of input variables 
  ...
  //
  teacher->SetIndividualPreproFlag( 0,18); 
  teacher->SetIndividualPreproFlag( 1,18); 
...
//      continue;
    ...
    //with pre-cuts -- MC10
    const double nBGCut =  15720.0   // MC10
    ...
    InputArray[  2 ] =  hPlusDllK;                               hKPlusDllK  ->Fill(InputArray[  2 ]);
    InputArray[  3 ] =  hMinusDllK;                      
    InputArray[  4 ] =  hPlusGhost;
    InputArray[  5 ] =  hMinusGhost;
    InputArray[  6 ] =  hPlusDllMu;                              hKPlusDllMu ->Fill(InputArray[  6 ]);
    InputArray[  7 ] =  hMinusDllMu;                     
    InputArray[  8 ] =  minPt;                                   hKPlusMinPt ->Fill(InputArray[  8 ]);
    InputArray[  9 ] =  maxPt;                                   hKPlusMaxPt ->Fill(InputArray[  9 ]);
    InputArray[ 10 ] =  hPlusP;                                  hKPlusP     ->Fill(InputArray[ 10 ]);
    InputArray[ 11 ] =  hMinusP;                         
    InputArray[ 12 ] =  hPlusDllP;                               hKPlusDllP  ->Fill(InputArray[ 12 ]);
    InputArray[ 13 ] =  hMinusDllP;                      
    InputArray[ 14 ] =  hPlusCaloE;                              hKPlusE     ->Fill(InputArray[ 14 ]);
    InputArray[ 15 ] =  hMinusCaloE;                     
    InputArray[ 16 ] =  chi2B;                                   hBsChi2     ->Fill(InputArray[ 16 ]); 
    InputArray[ 17 ] =  mErrB;                                   hBsMassErr  ->Fill(InputArray[ 17 ]); 
    //InputArray[ 18 ] =  tauErrB;                                 hBsCTauErr  ->Fill(InputArray[ 18 ]); 
    InputArray[ 18 ] =  pB;                                      hBsP        ->Fill(InputArray[ 19 ]);
    InputArray[ 19 ] =  docaB;
    InputArray[ 20 ] =  hPlusTrackChi2;
    InputArray[ 21 ] =  hMinusTrackChi2;
    InputArray[ 22 ] =  TMath::Abs(ntuple.B_s0_CosTheta); 

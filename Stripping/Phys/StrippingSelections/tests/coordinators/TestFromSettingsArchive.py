...
for stream in streams:
  stream.lines=[line for line in stream.lines if "Dijets" in line.name()]

#for stream in streams:
#  for line in stream.lines:
#    if line.RelatedInfoTools: line.RelatedInfoTools = None
#    if line._EnableFlavourTagging == True: line._EnableFlavourTagging = False
#    if line.MDSTFlag == True: line.MDSTFlag = False

#stream.lines=[line for line in stream.lines if "TrackEffDownMuon" not in line.name()]
...
def postConfig():
  ...
  jp.DumpPeriod = 100
  DaVinci().appendToMainSequence( [ jp ] )

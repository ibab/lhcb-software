...
__all__ = ( 'XibcBuilder'               ## LineBuilder class specialization
            ...
            , "makeXibc"	        ## CombineParticles Selection
            ## Neutral Xibc
...
default_config = {
    ...
    #########
    ,	'Protons4Lambda0_PIDp'					: 0
    ,	'LongProtons4Lambda0_MINIPCHI2'	: 2
    , 'Protons4Lambda0_PT'						: '200*MeV'
    ,	'Pions4Lambda0_PIDK'						: +7
    ...
    , 'Lambda0_DownProtonTrackPvalue'	: 0.05 
    , 'Lambda0_DownPionTrackPvalue'  	: 0.05
    , 'Lambda0_minFD' 								: '0.3*mm/GeV'
    #			, 'Lambda0_minFD' 								: '35*mm'
    ,	'Pions4Ximinus_PT'							: '0*MeV'
    ,	'Pions4Ximinus_PIDK'						: 15
    ,	'Pions4Ximinus_TRPCHI2'					: 0.05
    ,	'Pions4Ximinus_MINIPCHI2'				:	0
    , 'Ximinus_MassWindowLarge'				: '120*MeV'
    , 'Ximinus_MassWindowTight'				: '50*MeV'
    , 'Ximinus_APT'										: '400*MeV'
    , 'Ximinus_FlightDistance'				: '0.20*mm/GeV'
    #			, 'Ximinus_FlightDistance'				: '10*mm'
    , 'Ximinus_ENDVERTEXCHI2'					: 20
    , 'Xic0_MassWindowLarge'					: '600*MeV'
    , 'Xic0_MassWindowTight'					: '100*MeV'
    ,	'Pions4Xic0_PT'									: '200*MeV'
    ,	'Pions4Xic0_PIDK'								:	5
    , 'Pions4Xic0_TRPCHI2'						: 0.05
    , 'Xic0_APT'											: '400*MeV'
...
def makeLambdac (localname, _RequiredSelections, config=default_config):
            ...
            _lambdacCP.DaughtersCuts = {
                ...
                "K-"	:	"(TRPCHI2 > %(Kaon4Lambdac_TRPCHI2)s) & (PIDK > %(Kaon4Lambdac_PIDK)s)" % config,
...
def makeLambda0 (localname, additionalRequiredSelections=[], config=default_config):
	...
	_lambdaCommonCut = ("((VFASPF(VCHI2/VDOF)<%(Lambda0_ENDVERTEXCHI2)s) &" + 
                            ...
                            "(BPVVD/P > %(Lambda0_minFD)s))") % config
        
	_lambda = CombineParticles(localname+"lambda2ppi")
	_lambda.DecayDescriptor = "[Lambda0 -> p+ pi-]cc"
	...
	_lambdaDD.DecayDescriptor = "[Lambda0 -> p+ pi-]cc" 
	_lambdaDD.CombinationCut = "ADAMASS('Lambda0') < %(Lambda0_MassWindowLarge)s" % config
	_lambdaDD.DaughtersCuts = {
            "p+" 		: (	_protonCommonCut +
                                        "& ( TRPCHI2 > %(Lambda0_DownProtonTrackPvalue)s )") % config,
            "pi-"	: "TRPCHI2 > %(Lambda0_DownPionTrackPvalue)s" % config
            }
	...
	_lambdaUL.DecayDescriptor = "[Lambda0 -> p+ pi-]cc" 
	_lambdaUL.DaughtersCuts = {"p+" : _protonCommonCut}
	_lambdaUL.CombinationCut = "ADAMASS('Lambda0') <  %(Lambda0_MassWindowLarge)s" % config
...
def makeXi (localname, _RequiredSelections, config=default_config):
	...
	_xi.MotherCut = ("((VFASPF(VCHI2/VDOF) < %(Ximinus_ENDVERTEXCHI2)s) "
                         ...
                         "& (BPVVD/P > %(Ximinus_FlightDistance)s))" 
                         )% config
...
def makeXic (localname, _RequiredSelections, config=default_name):
	...
	_xic.MotherCut = ("((VFASPF(VCHI2/VDOF)< %(Xic0_ENDVERTEXCHI2)s)"+
                          "& (VFASPF(VCHI2/VDOF)< %(Xic0_ENDVERTEXCHI2)s) & " +		
...
def makeXibc0 (localname, _RequiredSelections, config=default_name):
	...
	_xibc.MotherCut = ("(VFASPF(VCHI2/VDOF)< %(Xibc0_ENDVERTEXCHI2)s)" 
                           #										 "(DTF_CUT( CHILD(ADMASS('Xi_c0'),1) < 200*MeV , True , 'Xi-'))"	
                           )% config

...
class StrippingB2HHPi0Conf(LHCbConfigurableUser):
    ...
    def B2HHPi0_Resolved( self ) :
        ...
        return StrippingLine('B2HHPi0_Resolved_line'
                             , prescale = 1
                             , algos = [B2HHPi0_rho, B2HHPi0_Resolved]
                             , postscale = 1
                             )
    ...
    def B2HHPi0_rho_Alg(self):
        from Configurables import CombineParticles
        import GaudiKernel.SystemOfUnits as Units    
	B2HHPi0_rho=CombineParticles("StripB2HHPi0_rho")
	B2HHPi0_rho.InputLocations = ["StdLoosePions"]
	B2HHPi0_rho.DecayDescriptor = "rho(770)0 -> pi+ pi-"
	B2HHPi0_rho.DaughtersCuts = {
            "pi-"  : "(PT>%(PiMinPT)s *MeV) & (P>%(PiMinP)s *MeV) & (TRPCHI2>%(PiMinTrackProb)s) & (MIPCHI2DV(PRIMARY)>%(PiMinIPChi2)s)" % self.getProps()
            ,"pi+" : "(PT>%(PiMinPT)s *MeV) & (P>%(PiMinP)s *MeV) & (TRPCHI2>%(PiMinTrackProb)s) & (MIPCHI2DV(PRIMARY)>%(PiMinIPChi2)s)" % self.getProps()   
            }
	B2HHPi0_rho.CombinationCut = "AALL" % self.getProps()
	B2HHPi0_rho.MotherCut = "(VFASPF(VPCHI2)>%(BMinVtxProb)s) & (BPVVDCHI2>%(BMinVVDChi2)s)" % self.getProps()
        return B2HHPi0_rho
	...
	B2HHPi0M.InputLocations = ["StripB2HHPi0_rho", "StdLooseMergedPi0"]
	B2HHPi0M.DecayDescriptor = "B0 -> rho(770)0 pi0"
	...
	B2HHPi0M.MotherCut = "(PT>%(BMinPT_M)s *MeV) & (BPVIPCHI2()<%(BMaxIPChi2)s) & (BPVDIRA>%(BMinDIRA)s)" % self.getProps()
        return B2HHPi0M
	...
	B2HHPi0R.InputLocations = ["StripB2HHPi0_rho", "StdLooseResolvedPi0"]
	B2HHPi0R.DecayDescriptor = "B0 -> rho(770)0 pi0"
	...
	B2HHPi0R.MotherCut = "(PT>%(BMinPT_R)s *MeV) & (BPVIPCHI2()<%(BMaxIPChi2)s) & (BPVDIRA>%(BMinDIRA)s)" % self.getProps()
        return B2HHPi0R

...
##############################################################
	...
	myResolvedPi0 = StdLooseResolvedPi0
        ...
        #---------------------------------------
        # pi0 selection
        self.selpi0r = makeD02KPiPi0pi0r( name + '_pi0r',
                                          config,
                                          inputSel = [myResolvedPi0]
                                          )
        self.selpi0m = makeD02KPiPi0pi0m( name + '_pi0m',
                                          config,
                                          inputSel = [myMergedPi0]
                                          )        
        #---------------------------------------
        # hh selection
        self.selkstar_forRes = makeD02KPiPi0kstar_forRes( name + '_kstar_forRes',
                                                          config,
                                                          DecayDescriptor = '[K*(892)~0 -> K- pi+]cc',
                                                          inputSel = [myPions,myKaons]
                                                          )
        self.selkstar_forMer = makeD02KPiPi0kstar_forMer( name + '_kstar_forMer',
                                                          config,
                                                          DecayDescriptor = '[K*(892)~0 -> K- pi+]cc',
                                                          inputSel = [myPions,myKaons]
                                                          )
	...
	self.selresolved = makeD02KPiPi0R( name + 'R',
                                           ...
                                           inputSel = [self.selpi0r, self.selkstar_forRes]
                                         )				       
	self.selmerged = makeD02KPiPi0M( name + 'M',
                                         config,
                                         DecayDescriptor = '[D0 -> K*(892)~0 pi0]cc',
                                         inputSel = [self.selpi0m, self.selkstar_forMer]
                                         )
        #---------------------------------------
        # Stripping lines
...
##############################################################
def makeD02KPiPi0pi0r(name,
                      config,
                      inputSel
                      ) :

    _pi0r = "(PT>%(Pi0MinPT_R)s *MeV) & (CHILD(CL,1)>%(ResPi0MinGamCL)s) & (CHILD(CL,2)>%(ResPi0MinGamCL)s)" % locals()['config']
...
##############################################################
def makeD02KPiPi0kstar_forRes(name,
                              config,
                              DecayDescriptor,
                              inputSel
                              ) :
...
##############################################################
def makeD02KPiPi0kstar_forMer(name,
                              config,
                              DecayDescriptor,
                              inputSel
                              ) :

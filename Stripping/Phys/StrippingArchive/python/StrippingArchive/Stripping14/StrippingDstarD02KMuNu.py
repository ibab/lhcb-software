...
class DstarD02KMuNuConf(LineBuilder) :
    ...
    >>> config_params = { 'TRACK_Chi2' :  4., .... etc }
    >>> DstarD02KMuNuConf = DstarD02KMuNuConf('DstarD02KMuNuTest',config_params)
    >>> DstarLines = DstarD02KMuNuConf.lines()
    >>> for line in DstarLines :
...
def makeDstarD02KMuNuWS(name,
    ...
    _combinationCuts = "(AM > %(Dstar_MassMin)s *MeV) "\
    "& (AMAXDOCA('') < %(Dstar_DOCA)s *mm)" % locals()
    _motherCuts = " (VFASPF(VCHI2/VDOF)< %(Dstar_VtxChi2)s ) "\
    "& ( ( M - CHILD(M,1) ) < %(Dstar_DeltaMass)s *MeV )  " % locals()

...
class ConeJetProxyFilter : public FilterDesktop {
  ...
  friend class AlgFactory<ConeJetProxyFilter>;

protected:
  ConeJetProxyFilter(const std::string &name,ISvcLocator *pSvc);
  virtual ~ConeJetProxyFilter(){}
...
public:
  ...
  double conePT(const LHCb::Particle* p,LHCb::Particles *charged,
		LHCb::Particles *neutral, const LHCb::VertexBase *bpv);

private:
  ConeJetProxyFilter();
  ConeJetProxyFilter(const ConeJetProxyFilter&);
  ConeJetProxyFilter& operator=(const ConeJetProxyFilter&);
...
StatusCode ConeJetProxyFilter::filter(const LHCb::Particle::ConstVector& input,
  ...
  if(charged->size() == 0){
    warning() << "Failed to get particles from " << m_chargedLoc << endmsg;
    return StatusCode::FAILURE;
  }
  if(neutral->size() == 0){
    warning() << "Failed to get particles from " << m_neutralLoc << endmsg;
    return StatusCode::FAILURE;
  }
  ...
  for(LHCb::Particle::ConstVector::const_iterator ip = filtered.begin();
    ...
    double pt = conePT(p,charged,neutral,bpv);
...
double ConeJetProxyFilter::conePT(const LHCb::Particle *p,
				  LHCb::Particles *charged,
				  LHCb::Particles *neutral,
				  const LHCb::VertexBase *pv){
  Gaudi::LorentzVector p4cone(0,0,0,0), p4 = p->momentum(); 
  // charged
  //std::cout << "conePT: " << p4.Pt() << std::endl;
  for(LHCb::Particles::const_iterator ip = charged->begin(); ip != charged->end(); ++ip){
    // clean up
    ...
    //    std::cout << use << " " << chi2 << " " << doca << std::endl;
    //if(!use) std::cout << "fail!" << std::endl;
    if(!use) continue;
    Gaudi::LorentzVector p4part = (*ip)->momentum();
    double deltaPhi = fabs(p4.Phi() - p4part.Phi() );
    if(deltaPhi > M_PI) deltaPhi = 2*M_PI - deltaPhi;
    double deltaEta = p4.Eta() - p4part.Eta();
    double dR   = sqrt(deltaPhi*deltaPhi + deltaEta*deltaEta);
    if(dR < m_dR) p4cone += p4part;
  }
  //  std::cout << "charged: " << p4cone.Pt();
  // neutral
  for(LHCb::Particles::const_iterator ip = neutral->begin(); ip != neutral->end(); ++ip){
    Gaudi::LorentzVector p4part = (*ip)->momentum();
    double deltaPhi = fabs(p4.Phi() - p4part.Phi() );
    if(deltaPhi > M_PI) deltaPhi = 2*M_PI - deltaPhi;
    double deltaEta = p4.Eta() - p4part.Eta();
  ...
  //std::cout << " total: " << p4cone.Pt() << std::endl;
  return p4cone.Pt();
}

...
namespace DecayTreeFitter
{
  ...
  double VtkHelixUtils::phidomain(const double phi)
  {
    double rc = phi ;
    if     (phi < -Gaudi::Units::pi)  rc += Gaudi::Units::twopi ;
    else if(phi >  Gaudi::Units::pi)  rc -= Gaudi::Units::twopi ;
    return rc ;
  }
...
//  	 << x0_2 << "," << y0_2 << "," << r_2 << endl ;
	...
	double l1 = (dphi1 + n1*Gaudi::Units::twopi)/omega_1 ;
    ...
    vertex = Gaudi::XYZPoint( 0.5*(x1+x2), 0.5*(y1+y2), 0.5*(z1+z2) ) ;
				  ...
				  double& flt)
  {
    ...
    for(int n = 1-ncirc; n <=1+ncirc ; ++n) {
      double l = (dphi + n*Gaudi::Units::twopi)/omega ;

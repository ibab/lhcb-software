...
bool ZipFile::setupStream( io::filtering_istream& s, const string& name )
    {
...
bool ZipFile::append( const string& name, const stringstream& is )
    {
...
void ZipFile::ZipInfo::print( void ) const
        {
            cout << hex << vmade << ' ' << vneed << ' ' << flag << ' ' << compress
                 ...
                 << ") " << crc << ' ' << dec << data_size << ' ' << file_size << ' '
                 << name_length << ' ' << extra_length << ' ' << comment_length
                 << ' ' << disk << ' ' << hex << iattr << ' ' << eattr << ' '
                 << file_offset << ' ' << " '" << name << "' "
                 << " '" << extra << "' "
                 << " '" << comment << "' " << endl;
        }
...
bool ZipFile::EndRec::read_from( istream& is )
        {
            ...
            for ( er_offset = filesize - streamsize( sizeEndCentDir );
                  er_offset >= max( filesize - streamsize( 0xffff ) -
                                        streamsize( sizeEndCentDir ),
                                    ios::streamoff( 0 ) );
                  er_offset -= 1 ) {

...
namespace ConfigZipFileAccessSvc_details {
    ...
    public:
      ...
      bool append(const string& name, const stringstream& is) {
        ...
        if( uncompressed.size() >  compressed.size()) m_file->write(compressed.c_str(),compressed.size());
        else                                          m_file->write(uncompressed.c_str(),uncompressed.size());
      ...
      template <typename T> bool readObject(T& t, const string& path) {
    ...
    private:
      ...
      struct EndRec {
        ...
        bool write_ecd(ostream &os){
          ...
          if(size > 0xfffffffe && zip64)   put<uint32_t>(os,0xffffffff);
          else                             put<uint32_t>(os,size);
          if(offset > 0xfffffffe && zip64) put<uint32_t>(os,0xffffffff);
          else                             put<uint32_t>(os,offset);
          put<uint16_t>(os,comment_length);

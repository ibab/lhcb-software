        ...
        , m_compressOnWrite( compressOnWrite )
    {
        if ( mode & ios::out ) {
            ...
            m_lock = open( lckname.c_str(), O_RDWR | O_CREAT | O_EXCL,
                           S_IRUSR | S_IWUSR );
            if ( m_lock < 0 ) {
                cerr << "trying to open  " << m_name << " for writing, but lockfile "
                     << lckname << " already exists." << endl;
                cerr << "This (most likely) means that some other process intends "
                        "to write into " << m_name << endl;
                cerr << "Refusing to continue to preserve integrity of " << m_name
                     << "... goodbye" << endl;
                ::abort();
            }

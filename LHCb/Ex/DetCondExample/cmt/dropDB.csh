#!/usr/local/bin/tcsh
# $Id: dropDB.csh,v 1.1 2002-12-03 17:34:20 andreav Exp $
if ( ${?CONDDB_implementation} != 1 ) then
  ...
  source ${DETCONDEXAMPLEROOT}/cmt/cleanupDB_Objy.csh
  exit 0
else if ( "${CONDDB_implementation}" == "CondDBOracle" ) then
  source ${DETCONDEXAMPLEROOT}/cmt/cleanupDB_Oracle.csh
  exit 0
else if ( "${CONDDB_implementation}" == "CondDBMySQL" ) then
  source ${DETCONDEXAMPLEROOT}/cmt/cleanupDB_MySQL.csh
  exit 0
else

...
namespace OTDet
{
  ...
  inline double WalkRelation::fastTanh(const double x)
  {
    //table of tanh
  ...
  inline double vdt_tanh(const double x)
  {
  ...
  double WalkRelation::walk(double l) const
  {
    ...
    //const double tanh = std::tanh(l * overtau);
    //const double tanh = vdt_tanh(l * overtau);
    //const double tanh = fastTanh(l * overtau);
    const double tanh = padeTanh(l * overtau);
    return off + amp * (tanh - 1.0) + l * dptfactor;
  }

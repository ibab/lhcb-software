...
L0Muon::ProcRawErrors::ProcRawErrors()
{
  ...
  bcsu_status= L0Muon:: ErrorHandler(" %01X",0x00F);
  for (int ipu=0; ipu<4; ++ipu){
    pus_bcid[ipu]   =  L0Muon::ErrorHandler("%01X",0x00F);
    pus_status[ipu] =  L0Muon::ErrorHandler("%01X",0x00F);
    add_link[ipu] =  L0Muon::ErrorHandler(" %00X",0x0FF);
    opt_link[ipu] =  L0Muon::ErrorHandler(" %00X",0x0FF);
    par_link[ipu] =  L0Muon::ErrorHandler(" %00X",0x0FF);
    ser_link[ipu] =  L0Muon::ErrorHandler(" %00X",0x03F);
  }

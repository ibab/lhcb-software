...
namespace OTDAQ
{
  ...
  namespace ChannelMap
  {
	...
	return module(modid.station(),modid.layer(),modid.quarter(),modid.module()) ;
      ...
      const Module& module(unsigned int station, unsigned int layer, unsigned int quadrant, unsigned int module) const {
	return m_modules[layout(station,layer,quadrant,module)] ;
      }
      ;
      unsigned int straw(unsigned int station, unsigned int layer, unsigned int quadrant, unsigned int moduleid, unsigned int channelid ) const { 
	return module(station,layer,quadrant,moduleid).straw(channelid) ;
      }
      unsigned int channel(unsigned int station, unsigned int layer, unsigned int quadrant, unsigned int moduleid, unsigned int strawid ) const { 
	return module(station,layer,quadrant,moduleid).channel(strawid) ;
      }
      
      unsigned char layout(unsigned int station, unsigned int layer, unsigned int quadrant, unsigned int module) const {
	...
	return m_modules[m_modulepointers[station-OffsetStations][layer-OffsetLayers][quadrant-OffsetQuadrants][module-OffsetModules]] ;
      }
      
      void setModuleLayout( const LHCb::OTChannelID& modid, unsigned char layout) {
	m_modulepointers[modid.station()-OffsetStations]
	  [modid.layer()-OffsetLayers][modid.quarter()-OffsetQuadrants][modid.module()-OffsetModules] = layout ;
      }

...
namespace Tf
{
  namespace HitCreatorGeom 
  {
    ...
    // Class IndexedVector
    //
    // A container that gives access to elements bot by iterator and
    ...
    public:
      ...
      IndexedVector( IndexedVector& rhs) : m_isowner(true), std::vector<T*>(rhs), m_pointers(rhs.m_pointers) {
	rhs.m_isowner = false ;
      }

      /// Destructor
      ~IndexedVector() { if(m_isowner) std::for_each( std::vector<T*>::begin(), std::vector<T*>::end(), boost::checked_delete<T> ) ; }
    ...
    /////////////////////////////////////////////////////////////
    // Class Layer, Station, Detector
    //
    // Templated helper classes for creating a hierarchy of patreco regions
    /////////////////////////////////////////////////////////////

// $Id: MDFSelector.cpp,v 1.1 2005-12-20 16:33:39 frankb Exp $
//====================================================================
...
//	Package    : MDF
//
//  Description: The LHCb::MDFSelector component is able
...
namespace LHCb  {
  ...
  *  @version 1.0
  */
  class MDFContext : public IEvtSelector::Context {
  public:
    enum StreamType { 
      STREAM_NONE, 
      STREAM_FILE, 
  ...
  public:
    ...
    MDFContext(const MDFSelector* pSelector)
    : m_sel(pSelector), m_type(STREAM_NONE)   {     }
    /// Standard destructor 
    virtual ~MDFContext()          { close();             }
...
/// Receive event and update communication structure
StatusCode LHCb::MDFContext::receiveEvent()  {
...
/// Set connection
StatusCode LHCb::MDFContext::connect()  {
  switch(m_type)  {
    case STREAM_FILE:
    case STREAM_INET:
...
void LHCb::MDFContext::setCriteria(const std::string& crit) {
  Tokenizer tok(true);
  std::string recl;
...
LHCb::MDFSelector::MDFSelector(const std::string& nam, ISvcLocator* svcloc)
...
StatusCode LHCb::MDFSelector::queryInterface(const InterfaceID& riid, void** ppvIf) {
...
StatusCode LHCb::MDFSelector::initialize()    {
...
StatusCode LHCb::MDFSelector::createContext(Context*& refpCtxt) const {
  refpCtxt = new MDFContext(this);
  return StatusCode::SUCCESS;
}

StatusCode LHCb::MDFSelector::next(Context& ctxt) const   {
  MDFContext* pCtxt = dynamic_cast<MDFContext*>(&ctxt);
  if ( pCtxt != 0 )   {
...
StatusCode LHCb::MDFSelector::next(Context& ctxt, int jump) const   {
  if ( jump > 0 ) {
...
StatusCode LHCb::MDFSelector::previous(Context& /* ctxt */) const  {
  MsgStream log(msgSvc(), name());
  log << MSG::FATAL << " EventSelector Iterator, operator -- not supported " << endmsg;
  return StatusCode::FAILURE;
}

StatusCode LHCb::MDFSelector::previous(Context& ctxt, int jump) const   {
  if ( jump > 0 ) {
    for ( int i = 0; i < jump; ++i ) {
...
StatusCode 
LHCb::MDFSelector::createAddress(const Context& ctxt, 
                                 IOpaqueAddress*& pAddr) const    {
  const MDFContext* pctxt = dynamic_cast<const MDFContext*>(&ctxt);
  if ( pctxt ) {
...
StatusCode LHCb::MDFSelector::releaseContext(Context*& ctxt) const  {
  MDFContext* pCtxt = dynamic_cast<MDFContext*>(ctxt);
  if ( pCtxt ) {
    pCtxt->close();
    delete pCtxt;
...
StatusCode 
LHCb::MDFSelector::resetCriteria(const std::string& criteria,
                                 Context& context)  const
{
  MsgStream log(messageService(), name());
  std::string crit = criteria;
  MDFContext* ctxt = dynamic_cast<MDFContext*>(&context);
  if ( ctxt )  {
    ctxt->close();

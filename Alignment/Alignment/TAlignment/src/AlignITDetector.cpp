// $Id: AlignITDetector.cpp,v 1.2 2007-11-06 14:44:59 lnicolas Exp $
// Include files 
...
StatusCode AlignITDetector::initialize ( ) {
  ...
  if ( !tracker ) return Error( "Failed to retrieve IT detector element", StatusCode::FAILURE );
  ...
  if ( m_align == "Stations" ) {
    ...
    m_indexMap = createIndices( boxes.begin( )                                      ,
				boxes.end( )                                        ,
				m_fixed                                             ,
				bind<STChannelID>( &DeITBox::elementID, _1 )        ,
    ...
    return Error( "Unknown elements " + m_align 
		  + ". You can only align Stations, Layers, Boxes or Sectors",
		  StatusCode::FAILURE );

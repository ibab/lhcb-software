// $Id: AlignAlgorithm.cpp,v 1.19 2008-01-31 13:06:39 janos Exp $
// Include files
// from std
// #include <utility>
#include <algorithm>
...
namespace {
 ...
 class Equations {
   ...
   size_t& numHits(unsigned i) { return m_numHits[i] ; }
   size_t numHits(unsigned i) const { return m_numHits[i] ; }
...
StatusCode AlignAlgorithm::execute() {
  ...
  for (TrackIter iTrack = tracks->begin(), iTrackEnd = tracks->end(); iTrack != iTrackEnd; ++iTrack) {
    ...
    if ((*node)->hasMeasurement()) {
      ...
      if (printDebug()) debug() << " id = " << meas.lhcbID() << " -> index = "
...
void AlignAlgorithm::update() {
  ...
  if (solved) {
    ...
    m_dChi2AlignvsIterHisto->fill(m_iteration, derivatives * dChi2dX * derivatives);

...
StatusCode Derivatives::SetGlobal( const Gaudi::XYZVector &slope, const
			...
			const double &gamma ) { // angle of hit wrt to X-axis in LHCb co-ordinate system
  // ============================================
  ...
  if ( m_dofs[0] ) {
    m_derGB[station] = cos( gamma );
    dof_cnt++;
  }

  // shift in co-ordinate perpendicular to measurement:
  if ( m_dofs[1] ) {
    m_derGB[station+dof_cnt*m_n_elements] = sin(gamma);
    dof_cnt++;
  }
  ...
  if ( m_dofs[3] ) {
    ...
    m_derGB[station+dof_cnt*m_n_elements] = -(ty*z+y0)*(tx*cos(gamma)+ty*sin(gamma));
    dof_cnt++;
  }

  // rotation about local perpendicular-axis
  if ( m_dofs[4] ) {
    m_derGB[station+dof_cnt*m_n_elements] = -(tx*z+x0)*(tx*cos(gamma)+ty*sin(gamma));
    dof_cnt++;
  }
  ...
  if ( m_dofs[5] ) {
    m_derGB[station+dof_cnt*m_n_elements] =
      ...
      z*tx*sin(gamma)-z*ty*cos(gamma) + x0*sin(gamma) -y0*cos(gamma);
    dof_cnt++;
  }
...
StatusCode Derivatives::SetLocal( std::vector<double> &m_derLC,
  ...
  debug() << "m_tr_pars: " << m_tr_pars << " " << rank  << " " << z << " " << angle  <<endreq;
  if ( m_tr_pars == 1 )
    m_derLC[0] = cos( angle );
  else if ( m_tr_pars == 2 ) {
    m_derLC[0] = cos( angle );
    m_derLC[1] = z * cos ( angle );
  } else if ( m_tr_pars == 4 ) {
    m_derLC[0] = cos( angle );
    m_derLC[1] = (z + dz) * cos ( angle );
    m_derLC[2] = sin( angle );
    m_derLC[3] = (z + dz) * sin( angle );
  } else if ( m_tr_pars == 5 ) {
    m_derLC[0] = cos( angle );
    m_derLC[1] = z * cos ( angle );
    m_derLC[2] = z * z * cos( angle );
    m_derLC[3] = sin( angle );
    m_derLC[4] = z * sin( angle );
  } else if ( m_tr_pars == 6 ) {
    m_derLC[0] = cos( angle );
    m_derLC[1] = z * cos ( angle );
    m_derLC[2] = z * z * cos( angle );
    m_derLC[4] = sin( angle );
    m_derLC[5] = z * sin( angle );
    m_derLC[6] = z * z * sin( angle );
  }
  debug() << "m_derLC = " << m_derLC << endreq;
  return StatusCode::SUCCESS;
}

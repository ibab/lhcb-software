...
StatusCode BcmDigitization::execute()
{
  ...
  debug()<<"starting the BCM Digitization algorithm "<<endmsg;

  // Check existance of hit location
  if(!exist<LHCb::MCHits>(m_bcmHitsLocation)){
  ...
  int index=0;
  for (i=m_bcmMCHits->begin();i<m_bcmMCHits->end();i++){         
    const LHCb::MCParticle* origparticle=(*i)->mcParticle();
    if(origparticle)  {
      debug()<<"Particle from which the hit originates (PDG code)"<<
  ...
  if(m_bcmMCDigits){
    put( evtSvc(), mcsensors, m_bcmMCDigitLocation );
 	...
 	debug()<<"End of the BCM Digitization"<<endmsg;

  return StatusCode::SUCCESS;
  
}

StatusCode BcmDigitization::finalize()
{

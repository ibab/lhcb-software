...
MuonPhysicalChannelResponse::MuonPhysicalChannelResponse(Rndm::Numbers* flat,Rndm::Numbers* gauss,  
                              Rndm::Numbers* genericPDF, Rndm::Numbers* electronicNoise,double min, double max,
															MuonReadoutCond*  detectorCondition, int readoutType ){
														  p_flat=flat;
															p_gauss=gauss;
...
int MuonPhysicalChannelResponse::extractXTalkX(double distanceFromBoundary){
    ...
    return p_detectorResponse-> singleGapClusterX(randomNumber,distanceFromBoundary,m_readoutIndex);
}
int MuonPhysicalChannelResponse::extractXTalkY(double distanceFromBoundary){
    double randomNumber=(*p_flat)();
    return p_detectorResponse-> singleGapClusterY(randomNumber,distanceFromBoundary,m_readoutIndex);
}

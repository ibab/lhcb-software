...
#include "MuonAlgs/MuonPhysicalChannel.h"
     ...
     for (iter= m_Hits.begin();iter< m_Hits.end();iter++){
          ...
          if((*iter).getMCMuonHitOrigin().getBX()==MuonBXFlag::CURRENT){
			     if(originalTime>0.0&&originalTime<25.0){
				     if((*iter).hitArrivalTime()<0.||(*iter).hitArrivalTime()>25.0){
	...
	;
		...
		void MuonPhysicalChannel::createXTalkChannel(MuonHitTraceBack* iter,unsigned int newChannel,
...
/// odd
        newChannel=indexOfChannel +direction; 
	 	...
	 	void MuonPhysicalChannel::applyXTalk(int phChInX[2], int phChInY[2], std::vector<MuonPhysicalChannel*>&
	     hitDueToXTalk){			 
...
/// here the XTalk probability is evaluated so numberOfXTalkHits modified
            ...
            if(numberOfXTalkHitsY)fireXTalkChannels(1,numberOfXTalkHitsY,iter,
						ny, directiony, phChInY[readoutType],  hitDueToXTalk) ;
			}
		}
	}			
}
  ...
  void MuonPhysicalChannel:: addToPC(MuonPhysicalChannel* pointerOfPCToAdd){
 		std::vector<MuonHitTraceBack>::iterator iter;	
		for (iter= (pointerOfPCToAdd->hitsTraceBack()).begin();iter<(pointerOfPCToAdd->hitsTraceBack()).end();iter++){
       ...
       if((*iter).getMCMuonHitOrigin().getBX()==MuonBXFlag::CURRENT){
         if((*iter).hitArrivalTime()>0&&(*iter).hitArrivalTime()<25.0){
				   if((*iter).getMCMuonHistory().hasTimeJittered()){
					   (*iter).getMCMuonHistory().setHitInByTimeAdjustment((unsigned int)1);
					 }
//alessia correggere aggiungendo una parola che rappresenta il tempo -toflight vero pre time adjustmanet per hit ed usare quella					 
/*					 if(!(*iter).getMCMuonHitOrigin().getXTalkNature()){
			   ...
			   if(originalTime>0.0&&originalTime<25.0){
				   if((*iter).hitArrivalTime()<0.||(*iter).hitArrivalTime()>25.0){
					   (*iter).getMCMuonHistory().setHitOutByTimeAdjustment((unsigned int)1);
					 }
				 }
			 }			 	 
...
// corrected for the BX(negative)
   timeOfStart=-timeOfStart+BX*(-25.0);

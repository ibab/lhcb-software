#ifndef MUONALGS_MUONTEST_CPP
#define MUONALGS_MUONTEST_CPP 1
...
static std::string TESPathOfHitsContainer[]={"Hits","ChamberNoiseHits","FlatSpilloverHits","BackgroundHits"};
...
StatusCode MuonTest::initialize()
{
 ...
 m_histoTiming=histoSvc()->book("/stat/muon/1"," (ms)",50,0.,5000.0);
 NTuplePtr nt(ntupleSvc(), "FILE1/100");
 StatusCode status;
 if ( !nt )  {
    nt = ntupleSvc()->book ("FILE1/100",CLID_ColumnWiseTuple  , "Test digitization");
    if ( nt )    {
...
//      status = nt->addIndexedItem ("notfound ", m_hit, m_notfound)	;		
 ...
 NTuplePtr ntError(ntupleSvc(), "FILE1/101");
  ...
  if ( !ntError )  {
    ntError = ntupleSvc()->book ("FILE1/101",CLID_ColumnWiseTuple  , "Errors in the muon digitization");
    if ( ntError )    {
  ...
  if ( !ntMCDigit )  {
    ntMCDigit = ntupleSvc()->book ("FILE1/102",CLID_ColumnWiseTuple  , "MCMuonDigit");
    if ( ntMCDigit )    {
		  status = ntMCDigit->addItem ("MCDigit",   m_mcdigitnumber ,0, 4000);
      status = ntMCDigit->addIndexedItem ("alive ", m_mcdigitnumber, m_mcmuondigitalive);			
      status = ntMCDigit->addIndexedItem ("region ", m_mcdigitnumber, m_mcmuondigitregion);			
		  status = ntMCDigit->addItem ("MCMuonHit",   m_mcmuonhit ,0, 10000);
      status = ntMCDigit->addIndexedItem ("nature ", m_mcmuonhit, m_mcmuonhitnature);
      status = ntMCDigit->addIndexedItem ("firing ", m_mcmuonhit, m_mcmuonhitalive);
      status = ntMCDigit->addIndexedItem ("chaefficiency ", m_mcmuonhit, m_mcmuonhitefficiency);
  		m_mcdigit = ntMCDigit;
	  }	
  }
...
StatusCode MuonTest::execute()
{
  ...
  m_hitInInput=0;
	...
	MsgStream log(msgSvc(), name());
  log<<MSG::INFO<<"MuonDigitization test starting.... "<<endreq;
 	MuonTileID tile;
  SmartDataPtr<MCMuonDigits> mcdigit(eventSvc(),MCMuonDigitLocation::MCMuonDigit);  	
  MCMuonDigits::const_iterator idigit;
	SmartRefVector<MCMuonHit>::iterator it;	
	for (int nspill=0;nspill<1;nspill++){
  	 for(int station=0;station<5;station++){
        for(int region=0;region<4;region++){
		       for (int container=0;container<4;container++){
		           std::string
			         path="/Event"+ spill[nspill]+"/MC/Muon/M"+stationPath[station]+"/R"+regionPath[region]+"/"+TESPathOfHitsContainer[container];
...
// test that each hit is already been inserted inside on MCMuonDigit
                    bool muon=false;
										int mcode=9999;
										int notfound=0;
										int fired=0;
                    ...
                    locateMuonTileFromXYZ(x,y,z, numberTileOutput, phChTileID);
										if(nspill==0){
...
//													     log<<MSG::INFO<<"ciao" <<madre<<" "<<mcode<<endreq;
													 }
												 }
											}
										}
											    ...
											    if(object->DigitInfo().isAlive()){
													  fired++;
													   ...
													   if(object->DigitInfo().isInDeadTime())dead=dead+1;
														 if(object->DigitInfo().isDeadForChamberInefficiency())dead=dead+10;
														 if(object->DigitInfo().isDeadByTimeJitter())dead=dead+100;
														 if(object->DigitInfo().isDeadByTimeAdjustment())dead=dead+1000;
													}
 									        bool found=false;
	                        SmartRefVector<MCMuonHit>::iterator iterhit= (object)->mcMuonHits().begin();
  		                    for(iterhit=(object)->mcMuonHits().begin(); iterhit<(object)->mcMuonHits().end() ; iterhit++){
											       if((*iterhit).target()==(*i)){
												        found=true ; 
												        break ;
												      }		
											    }
											    if(!found)log<<MSG::INFO<<" missing a MCMuonHit in "<<phChTileID[tile]<<endreq;
 								       }
									     else {
											   notfound=notfound+1;
									       m_hitNoInDigit++;
									       log<<MSG::INFO<<" a missing MCMuonDigit has been found "<<phChTileID[tile]<<endreq;
									     }
							      }
...
//test that all hits inside a digit are correctly placed there
 	...
 	for(idigit=mcdigit->begin();idigit<mcdigit->end();idigit++){
	  std::vector<MCMuonHitHistory>& HitsHistory();
		std::vector<MCMuonHitHistory>::iterator iterHistory=(*idigit)->HitsHistory().begin();
		SmartRefVector<MCMuonHit>::iterator it;
		double x,y,z,dx,dy,dz;
		if(m_mcdigitnumber < m_mcdigitnumber->range().distance()){ 
		  if((*idigit)->DigitInfo().isAlive())m_mcmuondigitalive[m_mcdigitnumber]=1 ;
			m_mcmuondigitregion[m_mcdigitnumber]=(*idigit)->key().station()*4+(*idigit)->key().region();
		   m_mcdigitnumber++;
		}
		it=(*idigit)->mcMuonHits().begin();
		m_pMuonTileXYZ->calcTilePos((*idigit)->key(), x,dx,y,dy,z,dz ) ; 	
		for(iterHistory=	(*idigit)->HitsHistory().begin();iterHistory<(*idigit)->HitsHistory().end();iterHistory++){
      ...
      m_mcmuonhitefficiency[m_mcmuonhit]=  (int) iterHistory->isKilledByChamberInefficiency();
			m_mcmuonhit++;}
		  if((*it)){
        ...
        if(fabs(x-xHiti)>dx+0.1){
				     if(fabs(x-xHito)>dx+0.1){
						   ...
						   log<<MSG::INFO<<"error this hit is outside the digit x"<<endreq;
							 log<<MSG::INFO<<"  "<<x<<" "<<xHiti<<" "<<xHito<<" "<<dx<<endreq; }
			 } 
				 xcheck: 
         if(fabs(y-yHiti)>dy+0.1){
				    if(fabs(y-yHito)>dy+0.1){
						   ...
						   if((y>yHito)&&(y<yHiti)) goto ycheck;							 
							 m_hitWronglyPalcedY++;
						  log<<MSG::INFO<<"error this hit is outside the digit y"<<endreq;  
							log<<MSG::INFO<<"  "<<y<<" "<<yHiti<<" "<<yHito<<" "<<dy<<endreq; }
				 }
			ycheck: 
			int ipop;
  		}
...
// test that the muon are  correctly putted in output
...
StatusCode MuonTest::locateMuonTileFromXYZ(double x,double y,double z,int& numberTileOutput, MuonTileID phChTileID[2]){
...
//					log<<MSG::INFO<<" channels in the chamber  "<< numberOfPCX << " "<<numberOfPCY<<endreq;         
        }
      }
  ...
  //
     for(int mapNumber=0;mapNumber<(int)usefull.getLogMapPerRegion(partition);mapNumber++){
  //
  ...
  //
   ...
   //  
         ...
         MuonLayout layout(layoutX[mapNumber][partition],layoutY[mapNumber][partition]);   
         phChTileID[numberOfTileCreated].setLayout(layout);
         phChTileID[numberOfTileCreated].setStation(station);
         phChTileID[numberOfTileCreated].setReadout(usefull.getLogMapRType(mapNumber,partition));

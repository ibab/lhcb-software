...
//  if (m_subsys->m_inputServicemap.empty()) return;
  m_subsys->Lock();
  if (m_subsys->buffersize == 0)
  {
    m_subsys->unLock();
    return;
  }
  if (m_bsiz < m_subsys->buffersize)
  {
    if (m_buffadd != 0)
    {
      free (m_buffadd);
    }
    m_buffadd = malloc(m_subsys->buffersize);
    m_bsiz = m_subsys->buffersize;
  }
  memcpy(m_buffadd,m_subsys->buffer,m_bsiz);
  m_subsys->unLock();

...
//	      }	 
	 ...
	 IRegistry* pObj;
	     ...
	     log << MSG::INFO << "No Histograms found on the transient store. "  << endreq;
	   }               
	 }	
	 else {
	   log << MSG::INFO << "Unable to go to the transient store. "  << endreq;
	 }
       }
       else {			     
...
//               request all properties
		    ...
		    log << MSG::INFO << "next property name " << propname << "=" << propvalue << endreq;
                    if (propvalue != "") {
		 ...
		 //      strcat(proptmpdata,";");
		       ...
		       propsize+=strlen(proptmp)+1;
                       ...
                       log << MSG::INFO << "obtained empty value for property "<< propname.c_str() << endreq;
		    }
	         }

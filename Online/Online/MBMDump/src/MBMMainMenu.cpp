  ...
  m_partID(0x14d), m_memory(0), m_dispMenu(0)
{
  ...
  setParam(m_name,    1,"%8s",procName(),0,0,0,0,0);
  addCommand(C_PROC,     "Process name :  ^^^^^^^^");
  setParam(&m_partID, 2,"%3X",0x14d,0,0XFFF,0,0,0);
  addCommand(C_PART,     "Partition ID :     0X^^^");
  setParam(m_buffName,3,"%9s",buff_names[0],0,0,buff_names,num_names,0);
  addCommand(C_BUF,      "Buffer name  : ^^^^^^^^^");
  setParam(m_buffType, 4,"%10s",buff_types[0],0,0,buff_types,num_types,1);

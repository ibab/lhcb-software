...
var ClusterSummary = function(c) {
  ...
  sum.className      = 'NodeTable';
  sum.body.className = 'NodeTable';
  sum.body.border    = 'none';

  sum.header = document.createElement('table');
  sum.header.body = document.createElement('tbody');
  ...
  sum.header.body.appendChild(tr);
  sum.header.appendChild(sum.header.body);

  sum.nodes = document.createElement('table');
  sum.nodes.body = document.createElement('tbody');
  sum.nodes.className = 'NodeTable';
  sum.nodes.body.className = 'NodeTable';
  sum.nodes.body.appendChild(NodeSummaryHeader());
  for(var i=0; i<c.nodes.length;++i) {
    sum.nodes.body.appendChild(NodeSummaryBody(c.nodes[i]));
  }
...
var subfarm_body = function()  {
  ...
  //alert('subfarm_body: system:'+sys+' msg:'+msg);

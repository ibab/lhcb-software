...
(function($env){
	...
	var connection;
        $env.debug('connection:'+url+' Method:'+xhr.method);
        if ( /^file\:/.test(url) ) {
            try{
                if ( xhr.method == "PUT" ) {
                    var text =  data || "" ;
	  ...
	  var outstream = null;
          $env.debug('2--connection:'+url+' Method:'+xhr.method+' data:'+data);
	  connection = url.openConnection();
	  ...
	  for (var header in xhr.headers){
	    ...
	    $env.debug('addRequestProperty Header['+header+'] = '+xhr.headers[header]);
	  }
	  ...
	  if(data&&data.length&&data.length>0){
	    if ( xhr.method == "PUT" || xhr.method == "POST" ) {
	      ...
	      connection.setDefaultRequestProperty('content-type','application/xml');
	      var outstream = connection.getOutputStream();
	      var outbuffer = new java.lang.String(data).getBytes('UTF-8');
	      outstream.write(outbuffer, 0, outbuffer.length);
	      //$env.info('3--connection:'+url+' Method:'+xhr.method+' stream:'+
	      //	outstream.toString('UTF-8')+' len:'+outbuffer.length);
	      outstream.flush();
	      outstream.close();
	    }
	  }else{
	    $env.debug('3--connection:'+url+' Method:'+xhr.method+' data:'+data);
	    connection.connect();
	  }
        }
        if(connection){
          $env.debug('4--connection:'+url+' Method:'+xhr.method+' data:'+data);
	  try{
		...
		$env.debug('XHR Header['+headerName+'] = '+headerValue);
	      }
	    }
	  }catch(e)   {
	    $env.error('failed to load response headers',e);
	  ...
	  if ( xhr.readyState < 3 && !(xhr.method == "PUT" || xhr.method == "POST") ) {
	    xhr.readyState = 3;   // LOADING
	    if(responseHandler){
	      $env.info('calling ajax response handler with state LOADING');
	  ...
	  try{
	    ...
	    stream = connection.getErrorStream();
          ...
          $env.info('6--connection:'+url+' Method:'+xhr.method+' data:'+data);
	  while ((length = stream.read(buffer)) != -1) {
	    baos.write(buffer, 0, length);
	    if ( xhr.async && !( xhr.method == "PUT" || xhr.method == "POST" ) )   {
	      xhr.responseText = java.nio.charset.Charset.forName("UTF-8").decode(java.nio.ByteBuffer.wrap(baos.toByteArray())).toString()+"";
	      if(responseHandler){
		var response = responseHandler();
		xhr.onreadystatechange();
		//$env.info('called comet response handler response='+xhr.responseText);
	      }
	    }
	  }
	}
	...
	xhr.responseText = java.nio.charset.Charset.forName("UTF-8").decode(java.nio.ByteBuffer.wrap(baos.toByteArray())).toString()+"";
        if(responseHandler){
	  $env.debug('calling ajax response handler with state DONE');
	  responseHandler();
        }
    };
    ...
    $env.tidy = function(htmlString){
      ...
      var xmlString,
      baos = new java.io.ByteArrayOutputStream(),
      bais = new java.io.ByteArrayInputStream((new java.lang.String(htmlString)).getBytes("UTF8"));
      try{
	if(!tidy){
...
__extend__(HTMLDocument.prototype, {
    ...
    get domain(){
      var url = new java.net.URL(window.location);
      var dom = url.getAuthority().toString()+'';
      print('URL:'+url.toString()+' domain:'+dom);
      if ( dom.indexOf(':') > 0 ) dom = dom.substr(0,dom.indexOf(':'));
      return dom;
...
XMLHttpRequest.prototype = {
	open: function(method, url, async, user, password){ 
		...
		$info('XMLHttpRequest.open("'+method+'","'+url.toString()+'",'+async+')');		
	},
		...
		function makeRequest(){
		  $env.connection(_this, function(){
				...
				_this.__defineGetter__("responseXML", function(){
				    ...
				    try {
				      $info("parsing response text into xml document");
				      responseXML = $domparser.parseFromString(_this.responseText+"");
				      return responseXML;
				    } catch(e) { 
				      $error('response XML does not apear to be well formed xml', e);
				      responseXML = $domparser.parseFromString("<html><head/><body><p> parse error </p></body></html>");
				      return responseXML;
				    }
				  }
      				}else{
				...
				_this.__defineSetter__("responseXML",function(xml){ responseXML = xml; });
			}, data);

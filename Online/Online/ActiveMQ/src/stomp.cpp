...
{
  int socket_family;
  stomp_connection *connection = (stomp_connection*)apr_pcalloc(pool, sizeof(stomp_connection));   
  if( connection == NULL )
    return APR_ENOMEM;
...
#define CHECK_SUCCESS if( rc!=APR_SUCCESS ) { return rc; }
  ...
  apr_status_t rc = apr_sockaddr_info_get(&connection->remote_sa, hostname, APR_UNSPEC, port, 0, pool);
  CHECK_SUCCESS;
...
APR_DECLARE(apr_status_t) stomp_disconnect(stomp_connection **connection_ref)   {
  ...
  apr_status_t rc = apr_socket_shutdown(connection->socket, APR_SHUTDOWN_WRITE);	
  if( rc != APR_SUCCESS )
    result = rc;
...
APR_DECLARE(apr_status_t) stomp_read_line(stomp_connection *connection, char **data, int* length, apr_pool_t *tpool)  {
...
#undef CHECK_SUCCESS
  ...
  *data = (char*)apr_pcalloc(tpool, bytesRead);
  p = *data;
  if( p==NULL ) {
    return APR_ENOMEM;
  }
...
APR_DECLARE(apr_status_t) stomp_read_buffer(stomp_connection *connection, char **data, apr_pool_t *tpool) {
  data_block_list *head, *tail;
  apr_size_t i=0;
  apr_size_t bytesRead=0;
  char *p;
...
#define CHECK_SUCCESS if( rc!=APR_SUCCESS ) { return rc; }
  ...
  if( frame->headers != NULL ) {
    ...
    for (i = apr_hash_first(NULL, frame->headers); i; i = apr_hash_next(i)) {
      apr_hash_this(i, (const void**)&key, NULL, (void**)&value);
    ...
    if(frame->body_length >= 0) {
      ...
      CHECK_SUCCESS;  		  
      length_string = apr_itoa(pool, frame->body_length);
...
APR_DECLARE(apr_status_t) stomp_read(stomp_connection *connection, stomp_frame **frame, apr_pool_t *pool) {
  apr_status_t rc;
  stomp_frame *f = (stomp_frame*)apr_pcalloc(pool, sizeof(stomp_frame));
  if( f == NULL )
    return APR_ENOMEM;
...
#define CHECK_SUCCESS if( rc!=APR_SUCCESS ) { return rc; }   
  // Parse the frame out.
  {
    ...
    // Check for content length
    {
      ...
      if(content_length) {
        char endbuffer[2];
        apr_size_t siz, length = 2;

        siz = f->body_length = atoi(content_length);
        f->body = (char*)apr_pcalloc(pool, f->body_length+1);
        rc = apr_socket_recv(connection->socket, f->body, &siz);
        f->body_length = siz;
        CHECK_SUCCESS;
        // Expect a \n after the end

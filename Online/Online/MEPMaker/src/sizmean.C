...
// void plotmean( TDirectory *target, TList *sourcelist );
...
void plotmean( TList *sourcelist, ostream &outs ) {
...
//    first_source->cd( path );
    ...
    if ( obj->IsA()->InheritsFrom( "TH1" ) ) {
	  ...
	  if (h1->GetBinContent(i) > 0) {
	      maxentry = h1->GetBinCenter(i);
	      break;
	  }
      outs << h1->GetName() << ", " << maxentry << ", " << h1->GetMean() << ", " << 
	  h1->GetRMS() << endl;
...
void plotmean( TList *sourcelist ) 
{
    plotmean( sourcelist, cout);
}

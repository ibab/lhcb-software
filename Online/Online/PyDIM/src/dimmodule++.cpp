 ...
 * **************************************************************************/
 ...
 {    
    ...
    {        
        ...
        if (!this->self) {
            print("No python object found. Can't call to python layer");
            return;
        } else {            
            gstate = PyGILState_Ensure();
            res = PyObject_CallMethod(this->self, "rpcHandler", NULL);
            if (!res) {
...
static PyObject *
DimRpc_rpcHandler (DimRpc_Object * self)
{
 ...
 * **************************************************************************/
 ...
 {    
    ...
    void rpcInfoHandler()
    {
        ...
        if (!this->self) {
            print("No python object found. Can't pass the function call");
            return;
        } else {
            gstate = PyGILState_Ensure();
            res = PyObject_CallMethod(this->self, "rpcInfoHandler", NULL);            
            if (!res) {
                print ("Invalid call to virtual rpcInfoHandler method %p",
                        res);
                PyErr_Print();
            }
            Py_XDECREF(res);
...
DimRpcInfo_init(DimRpcInfo_Object *self, PyObject *args, PyObject *kwds)
{
    ...
    /* It is useless to perform back and forth conversion for default 
     * failure arguments 
     */        
...
static PyObject *
DimRpcInfo_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    /* Allocates a new DimRpcInfo_Object and initialises the cpp_dimRpcInfo to NULL
     */
    DimRpcInfo_Object *self;
    
    self = (DimRpcInfo_Object*)type->tp_alloc(type, 0);   
    if (self != NULL) {
...
static PyObject *
DimRpcInfo_name (DimRpcInfo_Object * self)
{
    PyObject *res=NULL;
...
{    
   ...
   /* Making sure we don't block the other Python threads */ 
    ...
    if (!buff || !buff_size) {
        /* Callback has failed and we provide the error object */
        return self->nolink;
    }
...
initdimcpp(void)
{
     ...
     * Basically this functions will make the necesary initialisations
     * to the _Type objects.
     */
    printf("Initializing C++ DIM interface...  \n");
    if (PyType_Ready(&DimRpc_Type) < 0) {
        printf("Could not initialize type DimRpc\n"); 
        return;
    }
    ...
    /* Magic call to make sure all the data type sizes are respected */          
    //dic_disable_padding();

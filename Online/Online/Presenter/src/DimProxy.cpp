// $Id: DimProxy.cpp,v 1.4 2007-06-19 13:35:14 psomogyi Exp $
...
DimProxy::DimProxy(std::string serviceName = "",
                   int refreshTime = 0,
                   MsgLevel verbosity = SILENT,
                   OnlineHistDB* histoDB = 0):
  DimInfo(serviceName.c_str(), refreshTime, -1),

// Dropt the "xxx/" svc type from dim svc name, to get a histogram ID like
// MonA/CountTime/Efficiency_of_Kaons  
//  OnlineHistogram(serviceName.erase(0, 3).c_str(),
//                  conn, user, page, instance),
...
//  m_H2D(0)
{
 // if (m_verbosity > 0) {    
    ...
    std::cout << "* initialsing HistoID " << serviceName.erase(0, 3).c_str() << std::endl <<

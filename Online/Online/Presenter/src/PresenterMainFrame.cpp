...
{    
  ...
  m_newPageButton = m_toolBar->AddButton(this, m_toolbarElement, 8);
  ...
  m_autoCanvasLayoutButton = m_toolBar->AddButton(this, m_toolbarElement, 0);
  ...
  m_toolbarElement->fTipText = "Delete histo from Canvas";
  m_toolbarElement->fStayDown = false;
  m_toolbarElement->fId = DELETE_HISTO_FROM_CANVAS_COMMAND;
  m_deleteHistoFromCanvasButton = m_toolBar->AddButton(this, m_toolbarElement, 8);
  m_deleteHistoFromCanvasButton->Connect("Clicked()", "PresenterMainFrame",
      this, "deleteHistoFromCanvas()");
  m_deleteHistoFromCanvasButton->SetState(kButtonDisabled);
...
{
  editorFrame->deleteHistoFromCanvas();
  std::cout << "PresenterMainFrame::deleteHistoFromCanvas()" << std::endl;
}
...
void PresenterMainFrame::checkedTreeItemsChildren(TGListTreeItem *node, TGListTree* selected)
{
  while (node) {
    if (node->IsChecked() && node->GetUserData()) {
      selected->AddItem(0, 0, node->GetUserData());
...
void PresenterMainFrame::checkTreeChildrenItems(TGListTreeItem* node, bool check)
{
  if (node && NULL == node->GetFirstChild() && node->GetUserData()) {
...
void PresenterMainFrame::checkTreeChildrenItemsChildren(TGListTreeItem* node, bool check)
{
  while (node) {
...
void PresenterMainFrame::collapseTreeChildrenItems(TGListTree* treeList, TGListTreeItem* node)
{
  if (node && treeList) {
...
void PresenterMainFrame::collapseTreeChildrenItemsChildren(TGListTree* treeList, TGListTreeItem* node)
{
  while (node) {
    if (node && treeList) {
      treeList->CloseItem(node);
...
void PresenterMainFrame::zoomHistogram()
{
  ...
  if (TH1* selectedHisto = getSelectedHisto()) {
    ...
    std::string drawOpts = selectedHisto->GetDrawOption();
    zoomCanvas->cd();
    zoomCanvas->SetTitle(selectedHisto->GetTitle());
    selectedHisto->DrawCopy(drawOpts.c_str());    

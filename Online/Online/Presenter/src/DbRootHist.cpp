...
void DbRootHist::referenceHistogram(ReferenceVisibility visibility) {
  ...
  if ( rootLock && (pres::s_H2D != m_histogramType) &&
    ...
    if ( (0 == m_reference) &&
      ...
      */
      ref = getReference( m_onlineHistogram, 
...
TH1* DbRootHist::getReference( OnlineHistogram* h,
  ...
  if(h) {
    ...
    if ( !f ) {
      if ( 1 != startRun ) {
        fileName = root + "/" + dataType + "_1.root";
    ...
    if(f) {
      if (false == f->IsZombie() ) {        
        std::cout << "GetReference: found file " << fileName << std::endl;
        out = m_analysisLib->findRootHistogram(h, f );
        f->Close();
      }
      delete f;
    }

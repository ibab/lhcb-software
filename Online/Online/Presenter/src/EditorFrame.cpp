...
//void *handle0(void *ptr)
//{
//   EditorFrame *frm = (EditorFrame *) ptr;
//   while (1) {
////      frm->refreshPage();
////      gSystem->Sleep(200);
//      frm->Update();
...
//  t0 = new TThread("t0", handle0, (void*) this);
...
//  m_dimSvcListTree->SetAutoCheckBoxPic(false); ROOT v5.16(?)...
  ...
  m_dimSvcListTree->Connect("Clicked(TGListTreeItem*, Int_t, Int_t, Int_t)",
                            "EditorFrame", this,
                            "clickedDimTreeElement(TGListTreeItem*, Int_t, Int_t, Int_t)");
  ...
  m_dimContextMenu->AddEntry("Add selected to Database", M_AddDimToDB_COMMAND);
  m_dimContextMenu->AddEntry("Add selected to Page", M_AddDimToPage_COMMAND);
  ...
  m_pagesFromHistoDBListTree->Connect("Clicked(TGListTreeItem*, Int_t, Int_t, Int_t)",
                            "EditorFrame", this,
                            "clickedPageTreeElement(TGListTreeItem*, Int_t, Int_t, Int_t)");
...
//                     "highlightSelectedPad(TPad*, TObject*, Int_t)");
  ...
  m_databaseHistogramTreeList->Connect("Clicked(TGListTreeItem*, Int_t, Int_t, Int_t)",
                            "EditorFrame", this,
                            "clickedHistoDBTreeElement(TGListTreeItem*, Int_t, Int_t, Int_t)");   

  m_histoDBContextMenu = new TGPopupMenu(fClient->GetRoot());
  m_histoDBContextMenu->AddEntry("Add selection to Page", M_AddDBHistoToPage_COMMAND);
  m_histoDBContextMenu->AddEntry("Set properties of selection", M_SetHistoPropertiesInDB_COMMAND);
  m_histoDBContextMenu->AddEntry("Delete selected Histogram", M_DeleteDBHisto_COMMAND);
...
//void EditorFrame::runThreads()
//{
//  
//  t0->Run();
//}
//
//void EditorFrame::killThreads()
//{
//  
//  t0->Kill();
//}
...
void EditorFrame::refreshDimSvcListTree()
{
  ...
  if (m_dimSvcListTree && m_dimBrowser) {
    ...
    // Get number of known DIM types from OnlineHistDB
    int knownDimSvcTypeCnt = sizeof(OnlineHistDBEnv_constants::HistTypeName) / 
                             sizeof(OnlineHistDBEnv_constants::HistTypeName[0]);
...
//          m_dimSvcListTree->ToggleItem(node);
          ...
          while(dimType = m_dimBrowser->getNextServerService(dimService,
            ...
            for (int i = 0; i < knownDimSvcTypeCnt; ++i) {
              if (TString(dimService).BeginsWith(OnlineHistDBEnv_constants::HistTypeName[i])) {
...
void EditorFrame::clickedDimTreeElement(TGListTreeItem* node,
...
void EditorFrame::clickedHistoDBTreeElement(TGListTreeItem* node,
                                           MouseButton btn, Int_t x, Int_t y)
{
  // if right button, popup context menu  
   if (node && btn == RIGHT) {
...
void EditorFrame::clickedPageTreeElement(TGListTreeItem* node,
                                        MouseButton btn, Int_t x, Int_t y)
{
  // if right button, popup context menu
  if (node && node->GetText() && btn == RIGHT && node->IsChecked()) {
    ...
    try {
      ...
      if (m_histogramDB) {
        ...
        while(currentNode) {
          ...
          if (OnlineHistDBEnv_constants::HistTypeName[0] == (std::string)(onlineHistogramToSet->hstype()).c_str()) {
            m_drawOption = m_mainFrame->m_bulkHistoOptions.m_1DRootDrawOption;
          } else if (OnlineHistDBEnv_constants::HistTypeName[1] == (std::string)(onlineHistogramToSet->hstype()).c_str()) {
      ...
      try {
        ...
        if (page) {  
          ...
          while (m_onlineHistosOnPageIt != m_onlineHistosOnPage.end()) {
            ...
            TPad* pad = new TPad(dbRootHist->histoRootName()->Data(), TString(""), (double)xlow, (double)ylow, (double)xup, (double)yup);
...
//  yup  [0,1] is the Y position of this point.
  ...
  TPad* pad;
  if(histogram) {
  pad = new TPad(*(histogram->histoRootName()), TString("Title"), xlow, ylow, xup, yup);
  ...
  if(pad ) {
    ...
    if (m_mainFrame->isConnectedToDatabase()) {
        ...
        if (OnlineHistDBEnv_constants::HistTypeName[0] == (std::string)(histogram->hstype()).c_str()) {
          m_drawOption = m_mainFrame->m_bulkHistoOptions.m_1DRootDrawOption;
        } else if (OnlineHistDBEnv_constants::HistTypeName[1] == (std::string)(histogram->hstype()).c_str()) {
          m_drawOption = m_mainFrame->m_bulkHistoOptions.m_2DRootDrawOption;    
        }  
...
void EditorFrame::enablePageRefresh()
{
  ...
  m_DbHistosOnPageIt = m_DbHistosOnPage.begin();
  while (m_DbHistosOnPageIt != m_DbHistosOnPage.end()) {      
    (*m_DbHistosOnPageIt)->enableRefresh();
    ++m_DbHistosOnPageIt;
  }
}
void EditorFrame::disablePageRefresh()
{
  ...
  m_DbHistosOnPageIt = m_DbHistosOnPage.begin();
  while (m_DbHistosOnPageIt != m_DbHistosOnPage.end()) {      
    (*m_DbHistosOnPageIt)->disableRefresh();
    ++m_DbHistosOnPageIt;
  }
}

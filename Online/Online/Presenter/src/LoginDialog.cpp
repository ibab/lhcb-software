...
void LoginDialog::build()
{
  ...
  //m_histogramDB->AddEntry("ORA01", 2);
  m_histogramDB->Select(1);
  m_histogramDB->Resize(168,22);
  AddFrame(m_histogramDB, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));
  m_histogramDB->MoveResize(88,16,168,22);
   
  m_dbUsername = new TGComboBox(this,-1,kHorizontalFrame | kSunkenFrame | kDoubleBorder | kOwnBackground);
  m_dbUsername->AddEntry(gSystem->GetUserInfo()->fUser, 0);
  ...
  AddFrame(m_dbUsername, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));
  m_dbUsername->MoveResize(88,48,168,22);
  m_dbUsername->Connect("ChangedBy(const char *)", "LoginDialog", this, "defaultPw(const char *)");

  m_loginButton = new TGTextButton(this,"&Login");
  m_loginButton->SetTextJustify(36);
  m_loginButton->Resize(90,22);
  AddFrame(m_loginButton, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));
  ...
  AddFrame(m_dbPasswdInput, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));

...
HistogramIdentifier::HistogramIdentifier(const std::string & histogramUrl):
  ...
  m_gauchocommentEric("")
{
  setIdentifiersFromDim(histogramUrl);
}
...
//  $7 histogram name
  ...
  if (!histogramUrlMatchGroup->IsEmpty()) {
    ...
    m_histogramUTGID = (((TObjString *)histogramUrlMatchGroup->At(2))->
      GetString()).Data();      
    m_algorithmName = (((TObjString *)histogramUrlMatchGroup->At(3))->
      GetString()).Data();
    TString setSwitch = ((TObjString *)histogramUrlMatchGroup->At(6))->
      GetString();

    if (s_setSwitch == setSwitch) {
      m_isFromHistogramSet = true;
      m_setName = (((TObjString *)histogramUrlMatchGroup->At(5))->
        GetString()).Data();
      ...
      m_isFromHistogramSet = false;
      m_setName = "";
      m_histogramName = (((TObjString *)histogramUrlMatchGroup->At(4))->
        GetString()).Data();
    }
    m_isPlausible = true;                    
  ...
  TObjArray* histogramUTGIDMatchGroup = s_histogramUTGIDRegexp.

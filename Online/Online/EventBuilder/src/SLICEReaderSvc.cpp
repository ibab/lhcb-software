...
#include "EventBuilder/SLICEReaderSvc.h"
...
StatusCode SLICEReaderSvc::pushEvent(char *event, int size)  {
  ...
  if ( mbm_wait_space(m_BMID) != MBM_NORMAL )  {
    if(m_ReaderState != RUNNING) {
      msgLog << MSG::INFO << WHERE << "End of run : mgm_wait_space cancelled" << endmsg;
      return StatusCode::RECOVERABLE;
    }
    msgLog << MSG::ERROR << WHERE << "mbm_wait_space failed" << endmsg;
    return StatusCode::FAILURE;
  }

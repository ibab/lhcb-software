...
--DBMS_OUTPUT.PUT_LINE ('before select insert ');

execute immediate 'select deviceid,system_name from lhcb_lg_devices   where devicename=:dfrom  and mod(system_name,:sysID)=0 ' into deviceid,node1 using devfrom, systemname;
...
--DBMS_OUTPUT.PUT_LINE ('first select insert ');
execute immediate 'insert into lhcb_pdet_temp(pathid,path_type,node1,node2,node3,node4,node5,node6,node7,node8,node9,node10,node11,pathused,path_weight,system_name) select pathid,path_type,node1,node2,node3,node4,node5,node6,node7,node8,node9,node10,node11,pathused,path_weight,system_name from lhcb_pdetails where (node1-:dev1)*(node2-:dev1)*(node3-:dev1)*(node4-:dev1)*(node5-:dev1)*(node6-:dev1)*(node7-:dev1)*(node8-:dev1)*(node9-:dev1)*(node10-:dev1)*(node11-:dev1)=0 and path_necessary=1 and (mod(system_name,:sysname)=0 or mod(:sysname,system_name)=0)' using deviceid,deviceid,deviceid,deviceid,deviceid,deviceid,deviceid,deviceid,deviceid,deviceid,deviceid,sys_first,sys_first ;
...
--DBMS_OUTPUT.PUT_LINE ('first select insert second');
case nextquery 
when 0 then
...
execute immediate 'insert into lhcb_path_temp select 0,1,p.node1,d.port_nbr,h.linkid,p.node1,f.port_nbr,p.system_name from (select pathid,node1,system_name from lhcb_pdet_temp) p,lhcb_macroscopic_connectivity h,lhcb_port_properties d,lhcb_port_properties f where p.node1=f.deviceid and f.portid=h.portidto and h.portidfrom=d.portid and mod(h.system_name,p.system_name)=0 and h.link_weight=1' ;
...
when 2 then
execute immediate 'insert into lhcb_path_temp select 0,1,p.node1,d.port_nbr,-h.linkid,p.node1,f.port_nbr,p.system_name from (select pathid,node1,system_name from lhcb_pdet_temp) p,lhcb_macroscopic_connectivity h,lhcb_port_properties d,lhcb_port_properties f where p.node1=f.deviceid and f.portid=h.portidfrom and h.portidto=d.portid and mod(h.system_name,p.system_name)=0 and h.link_weight=2 and h.bidirectiona_link_used=1';

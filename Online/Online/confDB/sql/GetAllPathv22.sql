...
execute immediate 'insert into lhcb_pdet_temp select * from lhcb_pdetails where (node1-:dev1)*(node2-:dev1)*(node3-:dev1)*(node4-:dev1)*(node5-:dev1)*(node6-:dev1)*(node7-:dev1)*(node8-:dev1)*(node9-:dev1)*(node10-:dev1)*(node11-:dev1)=0 and path_necessary=1 and (mod(system_name,:sysname)=0 or mod(:sysname,system_name)=0)' ;

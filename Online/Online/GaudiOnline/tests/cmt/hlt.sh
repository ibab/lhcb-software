...
#
$MINITERM EvtProd@${HOST}   -e "export UTGID=${NODENAME}/EvtProd;   exec -a \${UTGID} $gaudi_task -opt=$OPTS/MEPConverter.opts"&
$MINITERM EvtHolder@${HOST} -e "export UTGID=${NODENAME}/EvtHolder; exec -a \${UTGID} $gaudi_task -opt=$OPTS/MEPHolder.opts"&
...
#
$MINITERM EvtCons1@${HOST}  -e "export UTGID=${NODENAME}/EvtCons1;  exec -a \${UTGID} $gaudi_task -opt=$OPTS/NetworkConsumer.opts"&
$MINITERM EvtCons2@${HOST}  -e "export UTGID=${NODENAME}/EvtCons2;  exec -a \${UTGID} $gaudi_task -opt=$OPTS/NetworkConsumer.opts"&
#

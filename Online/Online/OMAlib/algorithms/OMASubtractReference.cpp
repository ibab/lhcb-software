// $Header: /afs/cern.ch/project/cvs/reps/lhcb/Online/OMAlib/algorithms/OMASubtractReference.cpp,v 1.2 2010-06-11 16:28:51 ggiacomo Exp $
#include <TH1F.h>
#include <TH2F.h>
...
OMASubtractReference::OMASubtractReference(OMAlib* Env) : 
  OMAHcreatorAlg("SubtractReference", Env) {
  m_ninput = 1;
  m_histSetFlag = false;
  m_npars = 1;
  ...
  m_needRef = true;
  m_doc = "Subtract reference from histogram to show difference from expected shape. use norm=0 for no normalization, norm=1 for normalization to same area";
  m_doc += ", norm=2 for normalization to same entries";
}

TH1* OMASubtractReference::exec( const std::vector<TH1*> *sources,

...
namespace FrameworkTest  {
  ...
  /** @class ContextFactory Algorithm.h Tests/Algorithm.h
   *
   * @author  M.Frank
   * @version 1.0
   * @date    20/06/2011
  ...
  struct ContextFactory : virtual public Framework::EventContextFactory  {
    /// Standard constructor
    ContextFactory(const Framework::IOMask& mask) : Framework::EventContextFactory(mask)  { }
    /// Default destructor
    virtual ~ContextFactory()  {}
    /// Create a new event context. On error the returned context is invalid (NULL)
    virtual Framework::EventContext* create()  {  return new Framework::EventContext(newId(),dataMask()); }
  };
}

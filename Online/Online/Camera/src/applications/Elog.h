...
#define MAX_ATTACHMENTS  50
  ...
  void setCredential( const std::string& name, 
                      const std::string& pass ) { m_username = name; m_password = pass; }

  void setLogbook( const std::string& name ) { m_logbook = name; }

  int  addAttachment( const std::string& file );
  ...
  int    submit( const std::string& text );

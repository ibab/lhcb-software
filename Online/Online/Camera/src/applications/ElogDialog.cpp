...
ElogDialog::ElogDialog(MessagePresenter* gui, int width, int height) :
...
void ElogDialog::setParameters( std::string& logbook, std::string& username,
                                ...
                                std::string& message, std::string& runNumber,
                                int& isOK ) 
{
  m_logbook  = &logbook;
  m_username = &username;
...
void ElogDialog::build() 
{
  ...
  TGLabel* m_logbookLabel = new TGLabel(elogFrame,"Logbook : ");
  ...
  m_logbookListBox -> AddEntry( "LHCb-Test"       , 3 ) ;
  ...
  m_messageTextEntry->MoveResize( xBeg + xSize, yBeg, xInputSize, 10*40);
  yBeg += 10*40+10;

...
void Worker::run()    {
  ...
  while( m_state == ACTIVE ) {
    ...
    try  {
      if ( m_state == OFFLINE ) {
        ::lib_rtl_output(LIB_RTL_INFO,MSG_SRC_FMT"Shutdown after %d execution cycles.\n",
          "Thread", this, m_wakeup);
        m_state = ENDED;
        unlock();
        return;
      }
    ...
    m_executor = 0;
    m_state = ACTIVE;
    m_master->workerDone(this,executor,context);
...
Status Worker::execute(Executor* executor, EventContext* context) {
  if ( executor && context ) {
    ...
    ::lib_rtl_output(LIB_RTL_ERROR,MSG_SRC_FMT"Submitted. Executor:0x%p Context:0x%p.\n",

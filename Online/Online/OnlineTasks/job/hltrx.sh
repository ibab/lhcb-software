...
$WIDETERM ErrorLogger@${HOST}    -e "export UTGID=${HOST}/ErrLog; ${gaudi_exe2} -opts=$GAUDIONLINEROOT/options/ErrorLogger.opts -main=$GAUDIONLINEROOT/options/Remap.opts " &
#
$MINITERM ErrServ@${HOST} -e "export UTGID=${HOST}/ErrServ ; ${gaudi_exe} -main=$GAUDIONLINEROOT/options/ErrorSrv.opts -opt=$GAUDIONLINEROOT/options/Daemon.opts " &
#
sleep 2
#
#  Monitors:

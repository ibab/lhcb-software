  ...
  : Service( nam, svcloc), m_rootCLID(CLID_NULL), m_ioMgr(0), m_evtCount(0), m_totalEvt(0), m_runNum(0), m_prevRun(0) 
{
  ...
  declareProperty("SaveHistoDir",m_HistoDirectory);
}
...
StatusCode TestEvtSelectorNFiles::setNewHistosName() const {
	...
	//If the directory to save the histograms is not present,create it.
	if (stat(m_HistoDirectory.c_str(), &dir_info) != 0 || !S_ISDIR(dir_info.st_mode)) {
	  if (mkdir(m_HistoDirectory.c_str(),0777) == -1) {
	     info () << "Error creating the directory for the histograms" << endmsg;
	     return StatusCode::FAILURE;
	   }
	}
	...
	StringProperty p2("OutputFile", m_HistoDirectory+m_HistFileName);
...
string TestEvtSelectorNFiles::genRootName(const string& m_run) const{
	...
	st = strftime(buffer,200,"%Y%m%d%H%M%S",timeinfo);
	if (0 != st) {
	    string str(buffer);  
	    new_name = "HltMassMonitorPoller-"+m_run+"-"+str.substr(0,8)+"T"+str.substr(8,std::string::npos) + ".root";
	}
	else {
	    log << MSG::INFO << "Something went wrong during naming the histogram...just name it with the run number." << endmsg; 
	    new_name = "run_"+m_run+".root";
	}
	return new_name;

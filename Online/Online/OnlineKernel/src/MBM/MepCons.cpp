...
namespace {
  ...
  struct Cons : public MEP::Consumer  {
    ...
    : MEP::Consumer(nam, pid)
    {
      ...
      ::printf(" EVENT Type (2=EVENT, 1=MEP): %d\n", evtyp);
    ...
    int eventAction() {
      ...
      if ( e->valid == 0 || e->magic != mep_magic_pattern() || e->refCount<=1 ) {
        ::printf("Found invalid MEP: %p [%d] magic:%08X id=%d refCount:%d valid:%d\n",

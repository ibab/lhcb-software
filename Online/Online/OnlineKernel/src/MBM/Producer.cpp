...
MBM::Producer::Producer(const std::string& buffer_name,
    const std::string& client_name, int partition_id) :
  Client(buffer_name, client_name, partition_id)
{
}

// Initializing constructor
MBM::Producer::Producer(BMID bmid, const std::string& client_name,
    int partition_id) :
...
int MBM::Producer::minAlloc()
{
  if (m_bmid != (BMID) -1)
  {
    ...
    throw std::runtime_error(
        "Failed to access minimal allocation size for MBM buffer:" + m_buffName
            + " [Internal Error]");
  }
  throw std::runtime_error(
      "Failed to access minimal allocation size for MBM buffer:" + m_buffName
...
void MBM::Producer::setNonBlocking(int facility, bool subscribe)
{
  ...
  if (subscribe)
  {
    ...
    if (sc != WT_SUCCESS)
    {
      throw std::runtime_error("Failed to subscribe action:" + m_buffName
          + " [Internal Error]");
    }
  }
}
...
int MBM::Producer::cancel()
{
  if (m_bmid != (BMID) -1)
  {
    ...
    throw std::runtime_error("Failed to cancel request to MBM buffer:"
        + m_buffName + " [Internal Error]");
  }
  throw std::runtime_error("Failed to cancel request to MBM buffer:"
      + m_buffName + " [Buffer not connected]");
}
...
int MBM::Producer::spaceAst()
{
  ...
  if (sc == MBM_NORMAL)
  {
    if (!m_blocking)
    {
      ...
      throw std::runtime_error("Failed to wtc_insert on get space AST:"
          + m_buffName + " [Internal Error]");
    }
    return MBM_NORMAL;
  }
  throw std::runtime_error("Failed mbm_get_space_ast MBM buffer:" + m_buffName
      + " [Internal Error]");
}
...
int MBM::Producer::spaceAction()
{
  ...
  {
    sc = ::mbm_declare_event(m_bmid, e.len, e.type, e.mask, 0, &fadd, &flen,
        m_partID);
    if (sc == MBM_REQ_CANCEL)
    {
      ::lib_rtl_output(
          LIB_RTL_ERROR,
          "Failed to declare event for MBM buffer %s. Request was cancelled.\n",
          m_buffName.c_str());
      return sc;
    }
    else if (sc == MBM_NORMAL)
    {
      ...
      ::lib_rtl_output(LIB_RTL_ERROR,
          "Failed to send space for MBM buffer %s. status = %d %08X.\n",
          m_buffName.c_str(), sc, sc);
      throw std::runtime_error("Failed to send space for MBM buffer:"
          + m_buffName + " [Internal Error]");
    }
    else
    {
      ::lib_rtl_output(LIB_RTL_ERROR,
          "Failed to declare event for MBM buffer %s. status = %d %08X.\n",
          m_buffName.c_str(), sc, sc);
    }
    throw std::runtime_error("Failed to declare event for MBM buffer:"
        + m_buffName + " [Internal Error]");
  }
  throw std::runtime_error("Failed to declare event for MBM buffer:"
      + m_buffName + " [Buffer not connected]");
...
int MBM::Producer::declareEvent()
{
  ...
  if (m_bmid != MBM_INV_DESC)
  {
    ...
    throw std::runtime_error("Failed to declare event to MBM buffer:"
        + m_buffName + " [Internal Error]");
  }
  throw std::runtime_error("Failed to declare event to MBM buffer:"
      + m_buffName + " [Buffer not connected]");
...
int MBM::Producer::declareEventTry()
{
  ...
  if (m_bmid != MBM_INV_DESC)
  {
    sc = ::mbm_declare_event_try(m_bmid, e.len, e.type, e.mask, 0, &fadd,
        &flen, m_partID);
    if (sc == MBM_NORMAL)
    {
      e.data = (int*) fadd;
...
int MBM::Producer::sendSpace()
{
  if (m_bmid != MBM_INV_DESC)
  {
    ...
    throw std::runtime_error("Failed to send space to MBM buffer:" + m_buffName
        + " [Internal Error]");
  }
  throw std::runtime_error("Failed to declare event to MBM buffer:"
      + m_buffName + " [Buffer not connected]");

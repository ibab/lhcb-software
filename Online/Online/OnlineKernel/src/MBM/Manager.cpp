...
int MBM::Manager::mapSections(Manager* m)  {
  ...
  _CHECK( _mbm_map_section(m->ctrl_mod,dsc->ctrl_add) )
  dsc->ctrl = dsc->ctrl_add[0];
  _CHECK( _mbm_map_section(m->event_mod,dsc->event_add) );
  dsc->event = dsc->event_add[0];
  _CHECK( _mbm_map_section(m->user_mod,dsc->user_add) );
  dsc->user = dsc->user_add[0];
  _CHECK( _mbm_map_section(m->bitmap_mod,dsc->bitm_add) );
  dsc->bitmap = dsc->bitm_add[0];
  _CHECK( _mbm_map_section(m->buff_mod,dsc->buff_add) );

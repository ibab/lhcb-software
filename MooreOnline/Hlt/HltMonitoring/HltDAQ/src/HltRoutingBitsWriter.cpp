   ...
   for (iter_t i=m_bits.begin();i!=m_bits.end();++i) {
      ...
      if (!i->second.empty()) {
         ...
         if (i->first<8) {
            ...
            if (m_hltMonSvc) odin_eval.rate = &(m_hltMonSvc->rateCounter(name() + "_" + htitle));
            ...
            if (m_hltMonSvc) l0_eval.rate = &(m_hltMonSvc->rateCounter(name() + "_" + htitle));
            declareInfo(boost::str( boost::format("COUNTER_TO_RATE[%s]")% htitle ),
            ...
            if (m_hltMonSvc) hlt_eval.rate = &(m_hltMonSvc->rateCounter(name() + "_" + htitle));
            declareInfo(boost::str( boost::format("COUNTER_TO_RATE[%s]")% htitle ),
...
StatusCode HltRoutingBitsWriter::initialize() {
   ...
   m_hltMonSvc = svc<IHltMonitorSvc>("HltMonitorSvc", false);
   if (!m_hltMonSvc) {
      info() << "Could not retrieve HltMonitorSvc" << endmsg;
   }
...
StatusCode HltRoutingBitsWriter::execute() {
   ...
   for (unsigned i=0;i<8;++i) {
      ...
      *(odin_eval.counter) += result;
      ...
      if ( result && odin_eval.rate) odin_eval.rate->count(t);
      ...
      if (l0du->valid()) {
         for (unsigned i=8;i<32;++i) {
            ...
            *(l0_eval.counter) += result;
            ...
            if ( result && l0_eval.rate) l0_eval.rate->count( t );
            if ( result ) bits[0] |= (0x01UL << i);
         }
      }
   }
   ...
   for (unsigned j=1;j<3;++j) {
      ...
      for (unsigned i=0;i<32;++i) {
         ...
         *(hlt_eval.counter) += result;
         ...
         if ( result && hlt_eval.rate) hlt_eval.rate->count( t );

...
## @file
#
#  Simple algorithm to get trigger lines for charm particles 
#
...
class TisTosLines(Algo) :
    ...
    def analyse ( self ) :
        ...
        #
        ## collect Tis&Tos trigger lines
        #
        for d in d0 : self.decisions ( d , self.triggers['D0' ] )
        for d in dp : self.decisions ( d , self.triggers['D+' ] )
        for d in ds : self.decisions ( d , self.triggers['Ds+'] )
...
def configure ( datafiles , catalogs = [] ) :
    ...
    ## define the proper even prefilters filters 
    ...
    for t in ( gaudi.tool ( 'TisTosLines.L0TriggerTisTos' ) ,
               gaudi.tool ( 'TisTosLines.TriggerTisTos'   ) ) : 
        
        t . UseParticle2LHCbIDsMap = 2
        t . PropertiesPrint        = True
        
    alg = TisTosLines (
        'TisTosLines'                     ,   ## Algorithm name ,
        RootInTES = rootInTES             ,

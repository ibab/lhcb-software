#!/usr/bin/env python
# =============================================================================
# $Id: muDst_Joel.py,v 1.1 2010-07-13 16:58:13 ibelyaev Exp $ 
# =============================================================================
...
class muDst(Algo) :
    ...
    def analyse ( self ) :
        ...
        ## get jpsis 
        jpsis = self.select('all' , 'J/psi(1S)' == ID )

        for j in jpsis : print j.decay()

        pvs = self.vselect('pv' , PRIMARY )
                
        ## loop over reconstructed j/psi
        for j in jpsis : 

            mass = M ( j ) / GeV 
            self.plot  ( mass , ' mass of J/psi' , 2 , 4 )            
            
            # get the related primary vertex from DVAlgorithm
            pv = self.bestVertex ( j )
            if not pv :
                self.Error('No primary vertex found for J/psi')
                continue


            
        self.setFilterPassed ( not jpsis.empty() )
    ...
    def finalize ( self ) :
        ...
        for key in histos :
            h = histos[key]
            if hasattr ( h , 'dump' ) : print h.dump(50,30)
...
if __name__ == '__main__' :
    ...
    ## configure the job:
    configure (
        '/afs/cern.ch/user/j/jbressie/public/MicroDSTs/joel.mdst'
        )

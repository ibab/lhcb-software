...
StatusCode L0Calo2CaloTool::putClustersOnTES( std::vector<LHCb::CaloCluster*>& clusters ) const
{
  ...
  if ( msgLevel(MSG::DEBUG) )
    debug() << "0: LHCb::CaloClusters at " << m_clusterLocation << " size = " << ( output ? output->size() : -1 )
  ...
  if ( msgLevel(MSG::DEBUG) ) debug() << "1: LHCb::CaloClusters at " << m_clusterLocation << " size = " << ( output ? output->size() : -1 ) << endmsg; ////////////
  ...
  const unsigned int       level  ) const
{
  ...
  if ( msgLevel(MSG::DEBUG) )
     debug() << "1: data at " << m_digitLocation << " does" << ( exist<LHCb::CaloDigits>( m_digitLocation ) ? "" : " NOT") << " exist"
             << " digits->size() = " << ( digits ? digits->size() : -1 ) << endmsg;
  ...
  if ( msgLevel(MSG::VERBOSE) ) verbose() << "2: digits->size() = " << ( digits ? digits->size() : -1 ) << endmsg;

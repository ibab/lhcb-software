  ...
  for (std::map<std::string, unsigned int>::iterator it =
    ...
    for (unsigned int j = 0; j < m_slots.size(); ++j) {
      ...
      if (!m_thetaConfig.empty())
        myAdcsCor.push_back(Gaudi::Utils::Aida2ROOT::aida2root(
            bookProfile1D(nameCor, nameCor, m_minStep - 0.5, m_maxStep + 0.5,
                          m_maxStep - m_minStep + 1)));
      ...
      for (int kk = m_minStep; kk < m_maxStep; kk++) {
        ...
        std::string nameCor =
            "ADCCor/" + u + "/" + station + "/" + quad + "/" + sl;
        perstep.push_back(Gaudi::Utils::Aida2ROOT::aida2root(
            book1D(name, name, -0.5, 1023 + 0.5, 1024)));
        if (!m_thetaConfig.empty())
          perstepCor.push_back(Gaudi::Utils::Aida2ROOT::aida2root(
              book1D(nameCor, nameCor, -200.5, 1223 + 0.5, 1424)));
      }
...
unsigned int HCDelayScan::randomizeADC(unsigned int adc) {
  if (adc < 128.5)
    ...
    return adc + (unsigned int)rand() % 2;
  else if (adc < 511.5)
    return adc + (unsigned int)rand() % 8;
  else
    return adc + (unsigned int)rand() % 16;
}
...
StatusCode HCDelayScan::execute() {
  ...
  if ((int)bxid < m_BxRange[0] || (int)bxid > m_BxRange[1])
    return StatusCode::SUCCESS;
    ...
    for (unsigned int k = 0; k < m_quadrants.size(); ++k) {
      for (int ix = 0 ; ix< m_adcs[0][i][k]->GetNbinsX()+1;++ix){
        ...
        for (unsigned int j = 0 ; j < 3 ; ++j){
          double val =
    m_adcs[j][i][k]->GetBinContent(m_adcs[j][i][k]->GetXaxis()->FindBin(step));
          if (val>adcmax){
            adcmax = val;
            bestslot = j-1;
          }

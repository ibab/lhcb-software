...
void VeloClusterMaker::makeClusters(){
  ...
  for(auto iSens = m_velo->sensorsBegin() ; iSens != m_velo->sensorsEnd() ;
       ++iSens, ++detIndex ){
    ...
    // iterate over hits
    for(auto digIt = range.first; digIt!=range.second; digIt++) {
      ...
      auto currentCluster = makeClusterFromDigit(*currentDigit,currentClusterSTN);
      ...
      if( currentClusterSTN > m_clusterSignalToNoiseCut &&
        ...
        m_clusters->insert(currentCluster.release(),channelID);
      }else{
...
//==========================================================================
std::unique_ptr<LHCb::InternalVeloCluster>
VeloClusterMaker::makeClusterFromDigit(LHCb::VeloDigit& currentDigit,
  ...
  auto currentCluster = std::unique_ptr<LHCb::InternalVeloCluster>(new LHCb::InternalVeloCluster());
...
void VeloClusterMaker::addDigit(LHCb::InternalVeloCluster& currentCluster,
  ...
  if(offset>0){
    ...
    signals.insert(signals.begin(),newpair);
    // update S/N
    currentClusterSTN+=signalToNoise(nearbyDigit);
  }
  currentCluster.setStripSignals(std::move(signals));
}
...
void VeloClusterMaker::unmarkCluster(LHCb::InternalVeloCluster& currentCluster){
  ...
  for (const auto& s : currentCluster.stripSignals()) m_channelUsed[s.first] = false;
}
//=========================================================================
...
//=========================================================================
std::pair<LHCb::VeloDigits::iterator, LHCb::VeloDigits::iterator>
VeloClusterMaker::getVeloDigitsOfSensor(int sensorId) const {
  //
  ...
  return std::equal_range(m_digits->begin(),m_digits->end(),&selObj,
			     VeloEventFunctor::Less_by_sensor<const LHCb::VeloDigit*>());
}
//=========================================================================
double VeloClusterMaker::signalToNoise(const LHCb::VeloDigit& digit) const {

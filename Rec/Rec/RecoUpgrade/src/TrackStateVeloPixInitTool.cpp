#include "TrackStateVeloPixInitTool.h"
#include "STDet/DeSTDetector.h"

//-----------------------------------------------------------------------------
// Implementation file for class : TrackStateVeloPixInitTool
//
// 2009-03-02 : Pavel Krokovny
//-----------------------------------------------------------------------------

DECLARE_TOOL_FACTORY( TrackStateVeloPixInitTool ) ;

TrackStateVeloPixInitTool::TrackStateVeloPixInitTool( const std::string& type,
...
TrackStateVeloPixInitTool::~TrackStateVeloPixInitTool() {}

StatusCode TrackStateVeloPixInitTool::initialize()
{
  StatusCode sc = GaudiTool::initialize();
  m_seedFit = tool<IPatSeedFit>( "PatSeedFitWithVeloPix" );
  m_veloFitter = tool<ITrackFitter>("Tf::PatVeloPixFitLHCbIDs", "FitVeloPix", this) ;
...
StatusCode TrackStateVeloPixInitTool::fit( LHCb::Track& track, bool clearStates ) const
{
  ...
  if(savedType == LHCb::Track::Upstream) {
      ...
      if( sc.isFailure() ) Warning("TrackStateVeloPixInitTool fit TT failed",sc,0).ignore();
  }
  ...
  if(savedType == LHCb::Track::Long ||
    ...
    if( sc.isFailure() ) Warning("TrackStateVeloPixInitTool fit T failed",sc,0).ignore();
    else {
      if( /* savedType == LHCb::Track::Long || */
	 savedType == LHCb::Track::Downstream ) {
...
StatusCode TrackStateVeloPixInitTool::createVeloStates( LHCb::Track& track ) const
{
  // let's just run velo fitter
  return m_veloFitter->fit(track) ;
}

StatusCode TrackStateVeloPixInitTool::createVeloTTStates( LHCb::Track& track) const
{
    return m_veloTTFit->fitVTT(track);
}

StatusCode TrackStateVeloPixInitTool::createTStationStates( LHCb::Track& track ) const
{
...
StatusCode TrackStateVeloPixInitTool::createTTState(LHCb::Track& track ) const
{
...
StatusCode TrackStateVeloPixInitTool::initializeRefStates(LHCb::Track& ,
						   LHCb::ParticleID) const
{
  error() << "TrackStateVeloPixInitTool::initializeRefStates. Code was copied from TrackMasterFitter. Let's leave it there."
	  << endreq ;

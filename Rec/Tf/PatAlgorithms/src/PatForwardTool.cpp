...
// Linker from Event/EventLinker
#include "Linker/AllLinks.h"
#include "Linker/LinkedTo.h"
#include "Linker/LinkedFrom.h"
#include "Linker/LinkedFromKey.h"

#include "/afs/cern.ch/work/m/mstahl/1509/01-ForwardTracking/TMVA/TMVA-v4.2.0/test/weights/BXF_MLP3HL472N.class.C"
...
constexpr struct proj_distance_t {
  ...
  decltype(auto) operator()(const Range& range) const {
    return range.back()->projection() - range.front()->projection() ;
  }
  template <typename Iter>
  decltype(auto) operator()(Iter&& f, Iter&& l) const{
...
public:
  ...
  boost::iterator_range<Iterator> next_range(Iterator current_hit, Predicate spread_predicate)
  {
    using reference = typename std::iterator_traits<Iterator>::reference;

    auto mid = this->next(current_hit);
    //== If not enough hits in the maximum spread, skip
    if ( !spread_predicate(*current_hit,*mid)  ) {
      // first increment first, as we KNOW the current first fails predicate..
      current_hit = std::find_if(++current_hit, mid, [&](reference i) { return spread_predicate(i,*mid);});
    ...
    mid = std::find_if( ++mid, m_last, [&](reference i) { return !spread_predicate(*current_hit,i); } );
    ...
    PatFwdPlaneCounter_<PatFwdPlaneCounter_Policy::OmitSelectedCheck> counter{ current_hit, mid };
    ...
    while ( std::distance(i,mid) != (m_minPlanes-1) && j != m_last ) {
      counter.removeHit(*i);
      ++i;
      j = counter.addHitsWhile( j, m_last, [&](reference k) { return spread_predicate(*i,k); } );
      if ( enough_planes() ) mid = j;
    }
    return { current_hit, mid };
  }
};
...

{
  ...
  //not tunable or fixed to sensible values
  declareProperty( "SecondLoop"             , m_secondLoop              = true                                                 );
  declareProperty( "WriteTuples"            , m_writeMCtuples           = false                                                );
  ...
  //tunable
  declareProperty( "fYCompatible"           , m_fYCompatible            = 50                                                   );//best option: 0
  declareProperty( "YTolOffset"             , m_YTolOffset              = 20                                                   );//best option: 10
  declareProperty( "MaxXCandidateSize"      , m_maxXCandidateSize       = 50                                                   );//best option: 30
  declareProperty( "MaxSpreadX"             , m_maxSpreadX              = 0.6                                                  );//best option: 0.6
  declareProperty( "MaxSpreadSlopeX"        , m_maxSpreadSlopeX         = 0.011                                                );//best option: 0.0045
  declareProperty( "MinPt"                  , m_minPt                   = 80.   * Gaudi::Units::MeV                            );//OK as it is
  declareProperty( "MinMomentum"            , m_minMomentum             = 1.    * Gaudi::Units::GeV                            );//OK as it is
  declareProperty( "MaxChi2X"               , m_maxChi2X                = 40                                                   );//best option: 24
  declareProperty( "MaxChi2Y"               , m_maxChi2Y                = 40                                                   );//best option: 18
  declareProperty( "MaxXSlope"              , m_maxXSlope               = 0.005                                                );//use only with m_withoutBField = true
  declareProperty( "fXPlanes"               , m_fXPlanes                = 1.5                                                  );//OK as it is
  declareProperty( "YCompatibleTol"         , m_YCompatibleTol          = 10.   * Gaudi::Units::mm                             );//OK as it is
  declareProperty( "YCompatibleTolFinal"    , m_YCompatibleTolFinal     = 1.    * Gaudi::Units::mm                             );//OK as it is
  declareProperty( "MaxSpreadY"             , m_maxSpreadY              = 1.5                                                  );//OK as it is
  declareProperty( "MaxSpreadSlopeY"        , m_maxSpreadSlopeY         = 70.                                                  );//OK as it is
  declareProperty( "MaxDeltaY"              , m_maxDeltaY               = 30.                                                  );//OK as it is
  declareProperty( "MaxDeltaYSlope"         , m_maxDeltaYSlope          = 300.                                                 );//OK as it is
  declareProperty( "StereoSpread"           , m_StereoSpread            = 3.0   * Gaudi::Units::mm                             );//OK as it is
  declareProperty( "MLPCut"                 , m_MLPcut                  = 0.0                                                  );//Let's hope...
  declareProperty( "BestResponseCut1"       , m_br1                     = 1.0                                                  );//Let's hope...
  declareProperty( "BestResponseCut2"       , m_br2                     = 1.0                                                  );//Let's hope...
  //unused
  ...
  declareProperty( "ZAfterVelo"             , m_zAfterVelo              = 20                                                   );//<-- not used except for debugging was 1640
  declareProperty( "MinRange"               , m_minRange                = 300.  * Gaudi::Units::mm                             );//<-- not used
...
StatusCode PatForwardTool::initialize ( ) {
  ...
  //std::vector<std::string> varnames{"log(mdx)", "log(sx)", "log(sdy)", "nPlanes", "nYPlanes", "nwHits", "1/sqrt(PT)", "log(chi2)", "pp", "1/sqrt(P)"};
  std::vector<std::string> varnames{"log(mdx)", "log(sx)", "log(spread)", "pp", "nwHits"};
  m_reader = new ReadMLP3HL472N(varnames);
...
StatusCode PatForwardTool::tracksFromTrack( const LHCb::Track& seed,
  ...
  buildXCandidatesList( track, fillXList( track ) );

  int minPlanes = m_minPlanes;  //== Initial value, can be updated later...
  if(m_writeMCtuples)make_tuple(m_candidates,{"BXF",ImHere::BeforeXFit});

  std::vector<PatFwdTrackCandidate> xCandidates, unsortedxCandidates;
  //xCandidates.reserve( m_candidates.size() );

  int minOTX = int( m_fXPlanes * m_minXPlanes );
  auto accept = [=](const PatFwdTrackCandidate& cnd) {
    ...
    return //cnd.chi2PerDoF() < m_maxChi2Track &&
        ( !m_withoutBField || fabs(cnd.slX()-cnd.xSlope(0.)) < m_maxXSlope) &&
  ...
  for( auto icand : m_candidates ) {
    ...
    //"log(mdx)", "log(sx)", "log(spread)", "pp", "nwHits"
    auto mdx_spread = get_mdx_spread(icand.coords());
    MVAvariables.push_back( log(mdx_spread.first) );
    MVAvariables.push_back( log(get_cluster_stddev(icand.coords())) );
    MVAvariables.push_back( log(mdx_spread.second) );
    MVAvariables.push_back( get_cluster_mean(icand.coords()) - icand.xStraight(m_fwdTool->zReference()) );
    MVAvariables.push_back( nT(icand) );
    PatFwdTrackCandidate temp(icand);
    temp.setQuality(m_reader->GetMvaValue(MVAvariables));
    if(temp.quality() < m_MLPcut) continue;
    unsortedxCandidates.push_back(std::move(temp));    
  }

  std::sort(unsortedxCandidates.begin(),unsortedxCandidates.end(),[](PatFwdTrackCandidate lhs, PatFwdTrackCandidate rhs){ return lhs.quality() > rhs.quality(); });
  auto bestResponse = unsortedxCandidates.front().quality();

  for( auto icand : unsortedxCandidates ) {

    auto cand_planes = nbDifferent(icand);
    if( (cand_planes == m_minXPlanes + 1 && icand.quality() < bestResponse - m_br1) || (cand_planes == m_minXPlanes && icand.quality() < bestResponse - m_br2) )continue;
    if( cand_planes >= m_minXPlanes + 2 )cand_planes--;
    //== Fit the candidate, and store them (fitXCandidate also updates the hits)
    while ( m_fwdTool->fitXCandidate( icand, m_maxChi2X, cand_planes ) ) {
      // Copy the whole stuff, keep only used ones and store it.
      PatFwdTrackCandidate temp(icand);
      temp.cleanCoords();//removes the hits that were sorted out by the fit
      ...
      if (!m_writeMCtuples && !accept(temp) ) continue;
      xCandidates.push_back(std::move(temp));
    }
  }
  //if(m_writeMCtuples)make_tuple(xCandidates,{"AXF",ImHere::AfterXFit});

  std::vector<PatFwdTrackCandidate> goodCandidates;
  int maxPlanes = 0;
  for(auto temp : xCandidates){
    //== Now try to get space track from these X track.

    //if(!m_writeMCtuples){
    std::for_each( std::begin( temp.coords() ), std::end( temp.coords() ),[](PatForwardHit* h) { h->setIgnored(false); } );// restore normal flag.
    ...
    if (!m_writeMCtuples && fullCount.nbStereo() < 4) continue;
    if (!m_writeMCtuples && m_maxDeltaY + m_maxDeltaYSlope * qOverP *qOverP < fabs(  m_fwdTool->changeInY( temp ) )) continue;
    ...
    temp.setNbIT( regions.nbIT() );        temp.setNbOT( regions.nbOT() );
    if ( !m_writeMCtuples && !inCenter(temp) ){
      if ( m_minHits > nbT(temp) ) continue;
      if ( temp.nbIT() == 0 && temp.nbOT() < m_minOTHits ) continue;
    }
    maxPlanes = std::max(maxPlanes, fullCount.nbDifferent());
    //== reject if below threshold
    if ( !m_writeMCtuples && !passMomentum( temp, track.sinTrack())) continue;
    ...
    /*//"log(mdx)", "log(sx)", "log(sdy)", "nPlanes", "nYPlanes", "nwHits", "1/sqrt(PT)", "log(chi2)", "pp", "1/sqrt(P)"
       * std::vector<double> MVAvariables;
      float dy_over_toldy = fabs(m_fwdTool->changeInY(temp))/(m_maxDeltaY + pow(computeQOverP(temp),2) * m_maxDeltaYSlope );
      auto momentum = 1.0/fabs(m_fwdTool->qOverP(temp));
      MVAvariables.push_back(log(get_mdx_spread(temp.coords()).first));
      MVAvariables.push_back(log(get_cluster_stddev(temp.coords())));
      MVAvariables.push_back(log(dy_over_toldy));
      MVAvariables.push_back(nbDifferent(temp.coords()));
      MVAvariables.push_back(PatFwdPlaneCounter{temp}.nbStereo());
      MVAvariables.push_back(nT(temp));
      MVAvariables.push_back(1.0/sqrt(temp.sinTrack()*momentum));
      MVAvariables.push_back(log(temp.chi2PerDoF()));
      MVAvariables.push_back(get_cluster_mean  (temp.coords()) - temp.xStraight( m_fwdTool->zReference() ));
      MVAvariables.push_back(1.0/sqrt(momentum));
      auto netresponse = m_reader->GetMvaValue(MVAvariables);
      if(netresponse < m_MLPcut) continue;
  ...
  //if(m_writeMCtuples)make_tuple(goodCandidates,{"ASF",ImHere::AfterStereoFit});
  ...
  if ( goodCandidates.size() > 1  ) {
    ...
    //auto bestQuality = (*(std::max_element(goodCandidates.begin(),goodCandidates.end(),[](const PatFwdTrackCandidate& t, const PatFwdTrackCandidate& f){return t.quality() < f.quality();}))).quality();
    //last = std::remove_if( std::begin(goodCandidates), last,[=](const PatFwdTrackCandidate& t) { return t.quality() < bestQuality - 0.05; }  );

    // remove if sensibly less OT
    auto maxOT = std::accumulate( std::begin(goodCandidates), last, 0,[](int m, const PatFwdTrackCandidate& t) { return std::max(m,nbT(t)); } );
...
PatForwardTool::fillXList ( PatFwdTrackCandidate& track ) const
{
  ...
  auto yCompat = m_YCompatibleTol + m_fYCompatible * fabs(track.slY());
  ...
  for ( const auto& region : x_regions ) {
    ...
    auto tol = ( fabs( yRegion * regionB->sinT() ) + m_YTolOffset);
...
void PatForwardTool::buildXCandidatesList ( PatFwdTrackCandidate& track,
  ...
  m_candidates.clear();
  auto iter = std::begin(full_XHit_range);
  auto ClusterFinder = make_RangeGenerator( m_minXPlanes,std::end(full_XHit_range) );
  auto xExtrap = track.xStraight( m_fwdTool->zReference() );
  auto make_spread_predicate = [=]( const PatFwdHit* hit ) -> MaxSpread { return { allowedXSpread(hit,xExtrap)}; };
  while ( ClusterFinder( iter ) ) {
    auto cluster_spread_predicate = make_spread_predicate(*iter);
    auto cluster = ClusterFinder.next_range( iter, std::cref(cluster_spread_predicate) );//std::cref allows to pass the updated (by calling make_predicate(*iter)) MaxSpread struct to the RangeGenerator
    iter = std::end(cluster);
    if (cluster.empty() || std::accumulate(cluster.begin(),cluster.end(),0,[](int last_it, const PatForwardHit *hit){ return last_it + (int)(hit->isOT() + 2*(!hit->isOT())); }) > m_maxXCandidateSize) continue;
    m_candidates.emplace_back( track.track(), std::begin(cluster), std::end(cluster) );
  }
}
...
bool PatForwardTool::fillStereoList ( PatFwdTrackCandidate& track, double tol ) const {
  ...
  for ( const auto& region : stereo_regions ) {
    ...
    auto xHitMin = xPred - ( fabs( yRegion * regionB->sinT() ) + m_zAfterVelo + tol );
...
//  make tuples
//=========================================================================
StatusCode PatForwardTool::make_tuple (const std::vector<PatFwdTrackCandidate> &cands, const TupInfo &tupleinfo) const {

  bool allOK = true;
  Tuple Candidate_tuple = nTuple(tupleinfo.treename);
  ...
  for(auto cand : cands){
    ...
    if(tupleinfo.WhereAmI == ImHere::AfterXFit){
      std::for_each( std::begin( cand.coords() ), std::end( cand.coords() ),
                     [](PatForwardHit* h) { h->setIgnored(false); } );// restore normal flag.
      cand.setSelectedCoords( );
    }
    ...
    std::vector<bool>  isOTvec;
    std::vector<bool>  matched;
    auto  mdx_spread     = get_mdx_spread    (cand.coords());
    float stddev         = get_cluster_stddev(cand.coords());
    float momentum_proxy = get_cluster_mean  (cand.coords()) - cand.xStraight( m_fwdTool->zReference() );

    PatFwdHits matched_Hits;
    for(auto cand_hit : cand.coords()){
      isOTvec.push_back(cand_hit->isOT());
      if(cand_hit->isOT())matched.push_back(truthmatch_OT(cand_hit));
      else                matched.push_back(truthmatch_ST(cand_hit));
      if(matched.back())matched_Hits.push_back(cand_hit);
    }
    bool  MC_is_OT = false, MC_is_IT = false;
    float MCmdx    = -0.1, MCstddev = -0.1, MCxdist  = -0.1, MCpp = 0.0;
    int   nMCHits  = -1, nMCPlanes = -1, nwMCHits = -1;
    if( matched_Hits.size() >= 0.7*cand.coords().size() ){
      auto MCmdx_spread = get_mdx_spread(matched_Hits);
      MCmdx     = MCmdx_spread.first;
      MCstddev  = (float)get_cluster_stddev(matched_Hits);
      MCxdist   = MCmdx_spread.second;
      nMCHits   = matched_Hits.size();
      nMCPlanes = nbDifferent(matched_Hits);
      MCpp      = get_cluster_mean(matched_Hits) - cand.xStraight( m_fwdTool->zReference() );
      nwMCHits  = std::accumulate(matched_Hits.begin(),matched_Hits.end(),0,[](int last_it, const PatForwardHit *mhit){ return last_it + (int)(mhit->isOT() + 2*(!mhit->isOT())); });
      for(auto MCpart : MCFromSeedVec){
        MCToOT.keyRange(MCpart).empty() ? MC_is_OT |= false : MC_is_OT |= true;
        MCToIT.keyRange(MCpart).empty() ? MC_is_IT |= false : MC_is_IT |= true;
      }
    }

    allOK &= Candidate_tuple -> column( "mdx"        ,(float)mdx_spread.first    );
    allOK &= Candidate_tuple -> column( "sx"         ,stddev                     );
    allOK &= Candidate_tuple -> column( "spread"     ,(float)mdx_spread.second   );
    allOK &= Candidate_tuple -> column( "nPlanes"    ,nbDifferent(cand.coords()) );
    allOK &= Candidate_tuple -> column( "nwHits"     ,nT(cand)                   );
    allOK &= Candidate_tuple -> column( "pp"         ,momentum_proxy             );

    if(tupleinfo.WhereAmI >= ImHere::AfterXFit){
      float momentum = 1.0/fabs(m_fwdTool->qOverP( cand ));
      allOK &= Candidate_tuple -> column( "P"   ,momentum                        );
      allOK &= Candidate_tuple -> column( "PT"  ,(float)cand.sinTrack()*momentum );
      allOK &= Candidate_tuple -> column( "y0"  ,(float)cand.y(0.f)              );
      allOK &= Candidate_tuple -> column( "chi2",(float)cand.chi2PerDoF()        );
      if(tupleinfo.WhereAmI == ImHere::AfterStereoFit){
        float dy_over_toldy = fabs(m_fwdTool->changeInY(cand))/(m_maxDeltaY + pow(computeQOverP(cand),2) * m_maxDeltaYSlope );
        allOK &= Candidate_tuple -> column( "sdy"     ,dy_over_toldy                      );
        allOK &= Candidate_tuple -> column( "nYPlanes",PatFwdPlaneCounter{cand}.nbStereo());
      }
    }

    allOK &= Candidate_tuple -> column( "mdx_MC"     ,MCmdx     );
    allOK &= Candidate_tuple -> column( "sx_MC"      ,MCstddev  );
    allOK &= Candidate_tuple -> column( "spread_MC"  ,MCxdist   );
    allOK &= Candidate_tuple -> column( "nPlanes_MC" ,nMCPlanes );
    allOK &= Candidate_tuple -> column( "pp_MC"      ,MCpp      );
    allOK &= Candidate_tuple -> column( "nHits_MC"   ,nMCHits   );
    allOK &= Candidate_tuple -> column( "nwHits_MC"  ,nwMCHits  );
    allOK &= Candidate_tuple -> column( "isOT_MC"    ,MC_is_OT  );
    allOK &= Candidate_tuple -> column( "isIT_MC"    ,MC_is_IT  );

    allOK &= Candidate_tuple -> farray( "matched",matched.begin()     ,matched.end()     ,"nHits",m_maxXCandidateSize );
    allOK &= Candidate_tuple -> farray( "isOT"   ,isOTvec.begin()     ,isOTvec.end()     ,"nHits",m_maxXCandidateSize );

    allOK &= Candidate_tuple -> write ( );
  }

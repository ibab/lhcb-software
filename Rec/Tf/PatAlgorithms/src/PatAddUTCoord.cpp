...
#include "PatAddUTCoord.h"


//-----------------------------------------------------------------------------
// Implementation file for class : PatAddUTCoord
//
// 2010-02-12 : Michel De Cian, based on Olivier Callot's PatAddUTCoord
//
//-----------------------------------------------------------------------------

DECLARE_TOOL_FACTORY( PatAddUTCoord )

using ROOT::Math::CholeskyDecomp; // -- Maybe copy locally if not running at CERN?
//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
PatAddUTCoord::PatAddUTCoord( const std::string& type,
...
// Destructor
//=============================================================================
PatAddUTCoord::~PatAddUTCoord() {}

//=========================================================================
//
//=========================================================================
StatusCode PatAddUTCoord::initialize ( ) {
...
//=========================================================================
StatusCode PatAddUTCoord::addUTClusters( LHCb::Track& track){
  ...
  for ( PatTTHits::iterator itT = myUTHits.begin(); myUTHits.end() != itT; ++itT ) {
...
//  Return the UT hits
//=========================================================================
StatusCode PatAddUTCoord::returnUTClusters( LHCb::State& state, PatTTHits& utHits, double& finalChi2, double p){
  ...
  PatTTHits goodUT; // -- The container with one "group" of hits
  PatTTHits myUtCoords; // -- The container for the final selection of hits
...
//=========================================================================
void PatAddUTCoord::selectHits(PatTTHits& selected, const LHCb::State& state, const double p){
  ...
  Tf::UTStationHitManager<PatTTHit>::HitRange hits = m_utHitManager->hits();

  // -- Loop over all the UT hits, get all UT hits whose projection is compatible with the track
  for ( PatTTHits::const_iterator itUT = hits.begin(); hits.end() != itUT; ++itUT ) {
    PatTTHit* ut = *itUT;
...
// Calculate Chi2
//=========================================================================
void PatAddUTCoord::calculateChi2(PatTTHits& goodUT, double& chi2, const double& bestChi2,
  ...
  while ( 1.e10 < chi2 && counter < nHits) {
    ...
    for ( PatTTHits::iterator itSel = goodUT.begin(); goodUT.end() != itSel; ++itSel ) {
      PatTTHit* ut = *itSel;
...
// Print out info
//=========================================================================
void PatAddUTCoord::printInfo(const PatTTHits& goodUT, double dist, double chi2, const LHCb::State& state){

  // -- Print some information at the end

  info() << "*** Store this candidate, nbUT = " << goodUT.size() << " chi2 " << chi2 << endmsg;
  for ( PatTTHits::const_iterator itSel = goodUT.begin(); goodUT.end() != itSel; ++itSel ) {
    PatTTHit* ut = *itSel;
    double z     = ut->z();

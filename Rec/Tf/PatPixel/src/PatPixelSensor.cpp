  ...
  m_sensor( sensor ) 
{
  ...
  { if ( 0 > sensor->ladderOfChip(chipNum ) ) break;
    ...
    Gaudi::XYZPoint p0 = sensor->globalXYZ( id0, offsets );     // global X,Y,Z position of chip reference point = pixel(1,1)
    Gaudi::XYZPoint pl = sensor->globalXYZ( idl, offsets );
    Gaudi::XYZPoint ph = sensor->globalXYZ( idh, offsets );

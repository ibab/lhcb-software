...
//-----------------------------------------------------------------------------
// Implementation file for class : PatVeloUTTool
//
...
DECLARE_TOOL_FACTORY( PatVeloUTTool )


//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
PatVeloUTTool::PatVeloUTTool( const std::string& type,
  ...
  , m_PatUTMagnetTool(0)
{
  declareInterface<PatVeloUTTool>(this);
...
//=========================================================================
StatusCode PatVeloUTTool::initialize ( ) {
  ...
  // Cashed once in PatVeloUTTool at initialization. No need to update with small UT movement.
  m_zMidUT    = m_PatUTMagnetTool->zMidUT();
  //  zMidField and distToMomentum isproperly recalculated in PatUTMagnetTool when B field changes
  double zMidField = m_PatUTMagnetTool->zMidField();
  double distToMomentum = m_PatUTMagnetTool->averageDist2mom();
  ...
  for (double dydz = -0.3; dydz < 0.3; dydz+=0.02) {
    double dxdz = 0.0;
    m_PatUTMagnetTool->dxNormFactorsUT(dydz, nfact);
    double dist2mom = m_PatUTMagnetTool->dist2mom(dydz);
  ...
  m_ttHitManager   = tool<Tf::UTStationHitManager <PatTTHit> >("PatUTStationHitManager");
...
//=========================================================================
void PatVeloUTTool::recoVeloUT(LHCb::Track & velotrack, std::vector<LHCb::Track*>& outtracks )
{

  std::vector<PatVUTTrack> vttTracks;
...
//=========================================================================
void PatVeloUTTool::getCandidates( LHCb::Track& veloTrack, std::vector<PatVUTTrack>& vttTracks){
  ...
  for (Tf::UTStationID sta=0; sta<m_ttHitManager->maxStations(); sta++){
    for (Tf::UTLayerID lay=0; lay<m_ttHitManager->maxLayers(); lay++){
      for (Tf::UTRegionID reg=0; reg<m_ttHitManager->maxRegions(); reg++){
        if( 0 == m_ttHitManager->hits(sta,lay,reg).size()) continue;



        const Tf::IUTHitCreator::STRegion* regionB = m_ttHitManager->region(sta,lay,reg);
        ...
        // Loop on hits
        //--------------------------------------------------------------------------
        Tf::UTStationHitManager<PatTTHit>::HitRange range = m_ttHitManager->hits(sta, lay, reg);
...
// Attach clusters to candidate, compute mean dx ...
//=========================================================================
void PatVeloUTTool::saveCandidate(PatTTHits& theClusters,PatVUTTrack& candidate){
  ...
  m_PatUTMagnetTool->dxNormFactorsUT( dyDz,  normFact);
...
//=========================================================================
void PatVeloUTTool::simpleFitTracks(std::vector<PatVUTTrack>& vttTracks) {

  std::vector<PatVUTTrack>::iterator ivttTrB;
...
//
//=========================================================================
void PatVeloUTTool::simpleFit(PatVUTTrack& vtttr) {
  ...
  double bdl      = m_PatUTMagnetTool->bdlIntegral(ySlopeVelo,zOrigin,zVelo);
  double zmid     = m_PatUTMagnetTool->zBdlMiddle(ySlopeVelo,zOrigin,zVelo);
...
//=========================================================================
void PatVeloUTTool::localCleanUp(std::vector<PatVUTTrack>& vttTracks){
  ...
  for(ivttTrB = vttTracks.begin(); ivttTrB != vttTracks.end(); ++ivttTrB){
    ...
    std::vector<PatVUTTrack>::iterator ivttTrE;
...
//=========================================================================
void PatVeloUTTool::selectBestTracks( std::vector<PatVUTTrack>& vttTracks) {
  ...
  for(ivttTrB = vttTracks.begin(); ivttTrB != vttTracks.end(); ++ivttTrB){
    ...
    if((int)pvtt.size() > m_maxSolutionsPerTrack) {
      std::vector<PatVUTTrack*>::iterator iv;
...
//=========================================================================
void PatVeloUTTool::prepareOutputTracks( std::vector<PatVUTTrack>& vttTracks,
                                         std::vector<LHCb::Track*>& outtracks)
{
  ...
  for(ivttTrB = vttTracks.begin(); ivttTrB != vttTracks.end(); ++ivttTrB){
    ...
    outTr->setHistory( LHCb::Track::PatVeloUT );

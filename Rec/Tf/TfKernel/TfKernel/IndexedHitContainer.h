...
class IndexedHitContainer {
    ...
    std::pair<size_t,size_t> offset( unsigned station ) const {
        return { m_s_offsets[station], m_s_offsets[station+1] };
    }
    std::pair<size_t,size_t> offset( unsigned station, unsigned layer ) const {
        auto i = m_s_offsets[station] ;
        return { i + m_l_offsets[station][layer], i + m_l_offsets[station][layer+1] };
    }
    std::pair<size_t,size_t> offset( unsigned station, unsigned layer, unsigned region ) const {
...
public:
    IndexedHitContainer( size_t reserve = 0) 
    {
        ...
        std::fill( std::begin(m_l_offsets[0]),    std::end(m_l_offsets[NStations]),                    0u );
        std::fill( std::begin(m_r_offsets[0][0]), std::end(m_r_offsets[NStations][NLayersPerStation]), 0u );
    }
#endif

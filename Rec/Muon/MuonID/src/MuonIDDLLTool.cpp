// $Id: MuonIDDLLTool.cpp,v 1.2 2005-06-15 06:26:39 pkoppenb Exp $
...
StatusCode MuonIDDLLTool::calcMuonDLL( const MuonID* muonid, double& dll ) {
  ...
  double ProbMu = m_distMuon[0] * (landau_root (dist,m_distMuon[1],m_distMuon[2])) + 
    m_distMuon[3] * (landau_root (dist,m_distMuon[4],m_distMuon[5]));
  double ProbPi = m_distPion[0] * (landau_root (dist,m_distPion[1],m_distPion[2])) + 
  ...
  if ((ProbPi >= small) && (dist < 200.0)){
    dll = ProbMu / ProbPi;   // the main case
    if (dll < small) {   // small ProbMu or/and high ProbPi

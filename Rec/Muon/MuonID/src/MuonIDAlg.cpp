...
// Associate tanh parameters with more generic containers
  ...
  counter("nInAcceptanceOuter");
  counter("nInAcceptanceInner");
  counter("nPreselAndAccept");
  counter("nMomentumPresel");
  counter("nInAcceptanceAfter");
  counter("nPreselAndAcceptAfter");
  counter("nMomentumPreselAfter");
...
StatusCode MuonIDAlg::execute() {
  ...
  for( iTrack = trTracks->begin() ; iTrack != trTracks->end() ; iTrack++){
    ...
    if(!(*iTrack)->checkFlag(LHCb::Track::Clone)  &&
      ...
      ///RVG
      if(msgLevel(MSG::DEBUG)) debug() << "###### For track with p = " << m_MomentumPre << endmsg;
      if(msgLevel(MSG::DEBUG)) debug() << "ProbMu = " << exp(pMuid->MuonLLMu()) << endmsg;
      if(msgLevel(MSG::DEBUG)) debug() << "ProbNonMu = " << exp(pMuid->MuonLLBg()) << endmsg;
     ...
     // Save MuonPIDs only if track is in acceptance, unless DoAllMuonTracks is true
      ...
      if (!m_kalman_foi){
        if(msgLevel(MSG::DEBUG)) debug() << "The number of pMuids is = " << pMuids->size() << endmsg;
...
StatusCode MuonIDAlg::doID(LHCb::MuonPID *pMuid){
  ...
  if (m_kalman_foi){
    ...
    if(pMuid->InAcceptance())  counter("nInAcceptance")++;
    if(pMuid->PreSelMomentum())  counter("nMomentumPresel")++;
    if(pMuid->InAcceptance() && pMuid->PreSelMomentum()) counter("nPreselAndAccept")++;
    ...
    if(pMuid->InAcceptance())  counter("nInAcceptanceAfter")++;
    if(pMuid->PreSelMomentum())  counter("nMomentumPreselAfter")++;
    if(pMuid->InAcceptance() && pMuid->PreSelMomentum()) counter("nPreselAndAcceptAfter")++;
...
StatusCode MuonIDAlg::calcSharedHits( LHCb::MuonPID* muonid, LHCb::MuonPIDs * pMuids ) {
  ...
  double dist1 = m_dist;
  if (msgLevel(MSG::DEBUG) ) debug()  << "    For ORIGINAL track with momentum p = " << (muonid->idTrack())->p() << ". Dist 1 = " << dist1 << ". The track container has " << pMuids->size() << " tracks."<< endmsg;
  ...
  for( iMuon = pMuids->begin(); pMuids->end() != iMuon; ++iMuon ) {
    ...
    if ( compareHits( muonid, *iMuon ) ){
      ...
      if(msgLevel(MSG::DEBUG)) debug() << "---------> Start the computation of Dist 2 <----------" << endmsg;
      sc = calcDist(*iMuon);
      if( sc.isFailure() ) {
      ...
      double dist2 = m_dist;
      if (msgLevel(MSG::DEBUG) ) debug()  << "            For SECONDARY track with momentum p = " << ((*iMuon)->idTrack())->p() << ". Dist 2 = " << dist2 << endmsg;
...
bool MuonIDAlg::compareHits( LHCb::MuonPID* muonid1, LHCb::MuonPID* muonid2 ){
  ...
  for( iCoord1 = mcoord1.begin() ; iCoord1 != mcoord1.end() ; iCoord1++ ){
    for( iCoord2 = mcoord2.begin() ; iCoord2 != mcoord2.end() ; iCoord2++ ){
      if(msgLevel(MSG::DEBUG)) debug() << "compareHits:  The hit1 is = " << (*iCoord1)->key() << ". The hit2 is = " << (*iCoord2)->key() << endmsg;
      if( (*iCoord1)->key() == (*iCoord2)->key() ) {
        if(msgLevel(MSG::DEBUG)) debug() << "The station limit is " << m_iM2 << ".For hits that are the same, the station is = " << (*iCoord1)->key().station() << endmsg;  
...
StatusCode MuonIDAlg::calcDist( LHCb::MuonPID* muonid ){
  ...
  for( iCoord = mcoord.begin() ; iCoord != mcoord.end() ; iCoord++ ){
    ...
    if(mCoordX[station] == 0) {
      ...
      if(msgLevel(MSG::DEBUG)) debug() << "calcDist : In station " << station << endmsg;
      if(msgLevel(MSG::DEBUG)) debug() << "calcDist : x = " << x << ", m_trackX[station] = " << m_trackX[station] << ", dx = " << dx << endmsg;
      if(msgLevel(MSG::DEBUG)) debug() << "calcDist : y = " << y << ", m_trackY[station] = " << m_trackY[station] << ", dy = " << dy << endmsg;
    }else{
      // get best match in X and Y
      if(msgLevel(MSG::DEBUG)) debug() << "(x - m_trackX[station])*(x - m_trackX[station]) = " << (x - m_trackX[station])*(x - m_trackX[station]) << endmsg;
      if(msgLevel(MSG::DEBUG)) debug() << "(y - m_trackY[station])*(y - m_trackY[station]) = " << (y - m_trackY[station])*(y - m_trackY[station]) << endmsg;
      if(msgLevel(MSG::DEBUG)) debug() << "pow((mCoordX[station] - m_trackX[station]),2) = " << pow((mCoordX[station] - m_trackX[station]),2) << endmsg;
      if(msgLevel(MSG::DEBUG)) debug() << "pow((mCoordY[station] - m_trackY[station]),2) = " << pow((mCoordY[station] - m_trackY[station]),2) << endmsg;
	  ...
	  if (last_dx != dx || last_dy != dy)
	    {
          ...
          if(  ( fabs( x - m_trackX[station] ) < foiXDim ) &&
            ...
            if (msgLevel(MSG::VERBOSE) ) {
              ...
              verbose()  << "              hit position X = " << x << ", and Y = " << y << endmsg;
              verbose()  << "              extrapolation X = " << m_trackX[station] << ", and extrapolation Y = " << m_trackY[station] << endmsg;
              verbose()  << "              ratioX = " << fabs( x - m_trackX[station])
                       << " ratioY = " << fabs( y - m_trackY[station]) << " foiXDim = "
                       << foiXDim <<" foiYDim = " << foiYDim <<endmsg;
              verbose()  << "              the foiX is = " << foiX(station, region, m_MomentumPre, dx) << ", the foiY is = " << foiY( station, region, m_MomentumPre, dy) << endmsg;
...
double MuonIDAlg::calc_ProbNonMu(const double& dist0, const double *parNonMu){
  ...
  if(parNonMu[2]>0){
    //RVG test
    if (msgLevel(MSG::DEBUG)) debug() << "parNonMu[2] = " << parNonMu[2] << endmsg;  

...
/**
* compute the muon quality
*/
/*
double DLLMuonTool::muonQuality(LHCb::Track& muTrack){
  ...
  {
    sc.setCode(301);
    return Error("muonQuality:: MUTRACK WITHOUT ANY SEED STATE ON IT",Quality);
  }

  if (muTrack.lhcbIDs().size()<1)
  {
    sc.setCode(302);
    return Error("muonQuality:: NO LHCbIDs ON TRACK!",Quality);
  }
  ...
  {
    sc.setCode(303);
    return Error("WRONG FIT PERFORMED",Quality);
  }
...
*/
/*
StatusCode Chi2MuIDTool::muonCandidate(const LHCb::Track& seed, LHCb::Track& muTrack, const std::vector<LHCb::LHCbID> ids_init){
  StatusCode sc = StatusCode::SUCCESS;
  m_arrivalCuts=true;
  m_applyIsMuon=true;
  ...
  if (m_chi2cut>0.){
    ...
    myquality = muonQuality(muTrack_par);
    if (myquality<0.) {

...
StatusCode TrackKalmanFilter::fit( Track& track ) const
{
  ...
  if(sc.isSuccess()) {
    ...
    for( NodeContainer::reverse_iterator inode(firstMeasurementNode) ; 
      ...
      node->setPredictedStateBackward( state );
      node->setFilteredStateBackward( state );
      LHCb::State revstate = state ;
      revstate.setCovariance( seedCov ) ;
      node->setPredictedStateForward( revstate );
      node->setFilteredStateForward( revstate );
      node->setDeltaChi2Forward(0);
      node->setDeltaChi2Backward(0);
      updateResidual(*node) ;
      prevNode = node ;
    ...
    for( NodeContainer::iterator inode(endMeasurementNode) ; 
      ...
      LHCb::State revstate = state ;
      revstate.setCovariance( seedCov ) ;
      node->setPredictedStateBackward( revstate );
      node->setFilteredStateBackward( revstate );
      node->setDeltaChi2Forward(0);
      node->setDeltaChi2Backward(0);
      updateResidual(*node) ;
    }

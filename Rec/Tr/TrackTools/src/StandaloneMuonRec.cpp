...
StatusCode StandaloneMuonRec::execute() {
  ...
  std::vector<StandaloneMuonTrack>::iterator itMuonTrack;
...
StatusCode StandaloneMuonRec::muonSearch()
{
  ...
  for (unsigned int region=0;region<m_nRegion;region++){    
    ...
    for(itM5=thisRegion->points().begin();itM5<thisRegion->points().end();
      ...
      StandaloneMuonPoint* bestCandidateM2=NULL;
...
{
 std::vector<StandaloneMuonTrack>::iterator itMuonTrackFirst;
 std::vector<StandaloneMuonTrack>::iterator itMuonTrackSecond;
 bool sameM2;
 bool sameM3;
 bool sameM4;

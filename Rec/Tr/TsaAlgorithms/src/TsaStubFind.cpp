// $Id: TsaStubFind.cpp,v 1.3 2007-02-02 16:31:12 cattanem Exp $
...
DECLARE_TOOL_FACTORY( TsaStubFind );


TsaStubFind::TsaStubFind(const std::string& type,
...
TsaStubFind::~TsaStubFind(){
  // destructer
}

StatusCode TsaStubFind::execute(std::vector<SeedHit*> hits[], std::vector<SeedHit*> sHits[], 
  ...
  for ( int stn = 0; stn < 3; ++stn ) {
    ...
    for ( std::vector<SeedHit*>::iterator it1 = hits[lay].begin(); hits[lay].end() != it1; ++it1 ) {
      ...
      for ( std::vector<SeedHit*>::iterator it2 = start; hits[lay+1].end() != it2; ++it2 ) {
        ...
        for ( std::vector<SeedHit*>::iterator it3 = SeedFun::startStereo(sHits[lay],x3,tol); sHits[lay].end() != it3; ++it3 ) {
          SeedHit* hit3 = (*it3);
	  ...
	  SeedHit* firstHit4 = hits[lay].front();;
          ...
          for ( std::vector<SeedHit*>::iterator it4 = SeedFun::startStereo(sHits[lay+1],x4,tol); sHits[lay+1].end() != it4; ++it4 ) {
            SeedHit* hit4 = (*it4);

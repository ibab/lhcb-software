...
//
// 2005-04-07 : Eduardo Rodrigues Figueiredo
//-----------------------------------------------------------------------------
...
VeloPhiMeasurement::VeloPhiMeasurement( VeloCluster& cluster,
  ...
  // Store also the 'position'=measurment,
  //which is the signed distance from strip to the origin.

  std::vector< std::pair<long,double> > sign = cluster->stripSignals();
  ...
  if ( 0 < sum ) {
    ...
    m_sinPhi = sin( phi );
    // MM+
    // m_error  = ( pitch / sum) * sqrt( sum2 / 12 );
...
// Destructor
//=============================================================================
VeloPhiMeasurement::~VeloPhiMeasurement() {}; 

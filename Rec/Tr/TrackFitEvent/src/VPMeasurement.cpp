...
VPMeasurement::VPMeasurement(const VPCluster& cluster,
  ...
  if (m_projection == X) {
    ...
    m_trajectory = std::auto_ptr<Trajectory>(new LineTraj(position, yDir, yRange, true));
  } else {
    setMeasure(info.y);
    setErrMeasure(info.dy);
    Gaudi::XYZVector xDir(1, 0, 0); 
    std::pair<double, double> xRange(-info.dx, info.dx);
    m_trajectory = std::auto_ptr<Trajectory>(new LineTraj(position, xDir, xRange, true));  
  }

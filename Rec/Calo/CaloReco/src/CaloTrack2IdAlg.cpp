// $Id: CaloTrack2IdAlg.cpp,v 1.1 2002-11-17 17:09:26 ibelyaev Exp $
// ============================================================================
  ...
  , m_deltaLL     ( 0 ) 
{
  ...
  declareProperty ( "EfficiencyTable" , m_efficiency     ) ;
  declareProperty ( "RejectionTable"  , m_rejection      ) ;
  // track flags 
...
StatusCode CaloTrack2IdAlg::initialize() 
{
  ...
  // check the efficinecy and rejection addresses 
  if( m_efficiency.empty() )
    { 
      m_efficiency = outputData() + "_Eff" ;
      Warning("'Efficiency' was set automatically to '"+m_efficiency+"'");
    }
  if( m_rejection.empty() )
    { 
      m_rejection = outputData() + "_Rej" ;
      Warning("'Rejection' was set automatically to '"+m_rejection+"'");
    }
  ...
  m_eff     = histoSvc()->book( dir                      , 
                                6                        , 
                                "Efficiency"             , 
                                m_pBins      , 0.0 , 1.0 , 
                                m_vBins      , 0.0 , 1.0 ) ;
  if( 0 == m_eff     )
    { return Error("'Efficiency'           histogram is not booked!"); }
  
  m_rej     = histoSvc()->book( dir                      , 
                                7                        , 
                                "Rejection"              , 
                                m_pBins      , 0.0 , 1.0 , 
                                m_vBins      , 0.0 , 1.0 ) ;
  if( 0 == m_rej     )
    { return Error("'Rejection'            histogram is not booked!"); }
  ...
  { // fill efficiency and rejection histograms 
    for( size_t ip = 0 ; ip < m_pBins ; ++ip ) 
      {
        const double     x  = m_signalN -> binCentreX      ( ip ) ;
        double  s = 0 ;
        double  b = 0 ;
        for( size_t iv = 0 ; iv < m_vBins ; ++iv ) 
          { 
            const double y  = m_signalN -> binCentreY      ( iv  ) ;
            s              += m_signalN -> binHeight   ( ip , iv ) ;
            b              += m_backgrN -> binHeight   ( ip , iv ) ;
            m_eff          -> fill( x , y , s ) ;
            m_rej          -> fill( x , y , b ) ;
          }
      }
...
StatusCode CaloTrack2IdAlg::execute() 
{
  ...
  Output* efficiency = new Output () ;
  sc                 = put( efficiency , m_efficiency ) ;
  if( sc.isFailure() )             { return StatusCode::FAILURE ; } // RETURN
  Output* rejection  = new Output () ;
  sc                 = put( rejection  , m_rejection  ) ;
  if( sc.isFailure() )             { return StatusCode::FAILURE ; } // RETURN
  ...
  for( Range::const_iterator rel = range.begin() ; 
       range.end() != rel ; ++rel ) 
    {
      ...
      const double eff      = 
        m_eff     -> binHeight( m_eff     -> coordToIndexX ( p ) ,
                                m_eff     -> coordToIndexY ( v ) );
      efficiency  -> relate( track , eff     ) ;
      
      const double rej      = 
        m_rej     -> binHeight( m_rej     -> coordToIndexX ( p ) ,
                                m_rej     -> coordToIndexY ( v ) );
      rejection   -> relate( track , rej     ) ;
  ...
  log << MSG::DEBUG 
      ...
      << efficiency -> relations().size() << "/"
      << rejection  -> relations().size() <<        endreq;

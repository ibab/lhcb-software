    ...
    void moveToNextState() {
      ...
      try {
        ...
        if (!finite(totalQuotientLog)) {
          Lester::messHandle().error() << "Please investigate MetropolisSampler as a matter of urgency because the following totalQuotientLog is not finite: " 
                                       << totalQuotientLog  << Lester::endmsg;
        };

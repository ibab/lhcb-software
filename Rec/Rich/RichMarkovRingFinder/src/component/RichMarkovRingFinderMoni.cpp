 ...
 *
 *  $Id: RichMarkovRingFinderMoni.cpp,v 1.25 2008-06-09 15:02:36 shaines Exp $
 *
        ...
        iR2 != rings->end(); ++iR2 )
  {
   LHCb::RichRecRing * ring2 = *iR2;
    ...
    if (centreXYdif < m_sizesepcut){
      info() << "Too close to another ring!" << endmsg;
   ...
   //info() << "Ring radius" << ringRadius<< endmsg;
        ...
        iSeg != richSegments()->end(); ++iSeg )
   {
    ...
    info() << "Segment lies within region around ring centre" << endmsg;
    // Populate map

...
StatusCode Velo::VeloClusterMonitor::initialize() {
  ...
  for ( int i = 0; i < 84; i++ ) {
    if (i<42){
      sprintf( hname, "# VELO clusters sensor %d", i );
      m_hNCluEvt_Sensor[ i ] = book1D( hname, hname , 0., 200., 200) ;
      ...
      sprintf( hname, "# VELO clusters sensor %d", i+22 );
      m_hNCluEvt_Sensor[ i ] = book1D( hname, hname , 0., 200., 200) ;
...
void Velo::VeloClusterMonitor::monitorClusters() {
  ...
  // A vector to store number of clusters per event per sensor
  // ---------------------------------------------------------
  unsigned int nclus_per_sensor[84]={0};
  ...
  for ( itVC = m_clusters -> begin(); itVC != m_clusters -> end(); ++itVC ) {
    ...
    // Plot of number of clusters per event per sensor 
    // -----------------------------------------------
    if (sensorNumber<42)	nclus_per_sensor[sensorNumber]++;
    else 			nclus_per_sensor[sensorNumber-22]++;
  ...
  for ( int i = 0; i < 84; i++ ) {
    m_hNCluEvt_Sensor[i]->fill(nclus_per_sensor[i]);               
  }
...
StatusCode Velo::VeloClusterMonitor::finalize() {
  ...
  for (int i = 0; i < 84; i++){
	...
	m_histCluADC_Sensor[i]->Fit("func");

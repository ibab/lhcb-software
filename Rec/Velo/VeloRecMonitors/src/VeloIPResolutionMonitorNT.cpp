...
StatusCode Velo::VeloIPResolutionMonitorNT::execute() {
  ...
  for ( RecVertices::const_iterator ipv = pvs->begin() ;
    ...
    for( Tracks::const_iterator tr = tracks->begin(); tr != tracks->end(); ++tr ){
      ...
      tuple->column( "PVXerr", std::sqrt( m_pv->covMatrix()(0,0) ) );
      tuple->column( "PVYerr", std::sqrt( m_pv->covMatrix()(1,1) ) );
      tuple->column( "PVZerr", std::sqrt( m_pv->covMatrix()(2,2) ) );
      tuple->column( "PVXNoRefit", (*ipv)->position().x() );
      tuple->column( "PVYNoRefit", (*ipv)->position().y() );
      tuple->column( "PVZNoRefit", (*ipv)->position().z() );
      tuple->column( "PVXerrNoRefit", std::sqrt( (*ipv)->covMatrix()(0,0) ) );
      tuple->column( "PVYerrNoRefit", std::sqrt( (*ipv)->covMatrix()(1,1) ) );
      tuple->column( "PVZerrNoRefit", std::sqrt( (*ipv)->covMatrix()(2,2) ) );
...
void Velo::VeloIPResolutionMonitorNT::distance( const RecVertex* pv, const State & state,
                                                double& dist, double& sigma, int type=0 )
{
  ...
  if( type==0 ){
    dist = std::sqrt( delta.mag2() );
    sigma = std::sqrt( std::pow( cos(theta), 2 )*( covpv(0,0) + covpos(0,0) + covpv(1,1) + covpos(1,1) ) +
                       std::pow( sin(theta), 2 )*( covpv(2,2) + covpos(2,2) ) );
  }
  else if( type==1 ){
    dist = delta.x();
    sigma = std::sqrt( covpv(0,0) + covpos(0,0) );
  }
  else if( type==2 ){
    dist = delta.y();
    sigma = std::sqrt( covpv(1,1) + covpos(1,1) );
  }
  else{
    dist = -999;
    sigma = -999;

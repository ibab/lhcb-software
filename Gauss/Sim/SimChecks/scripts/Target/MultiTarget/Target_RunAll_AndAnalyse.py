...
models=['LHEP','FTFP_BERT','QGSP_BERT']#,'LHEP']#any present in the version of Gauss you are using es. 'FTFP_BERT','LHEP'
...
def createTemplateOptionFile(path,models,particlesTodo,energies,materialsTodo,thicks) :
    ...
    opt_file.write('\n\nfinalPlot: TOTAL INEL EL MULTI ASYM_INEL MULTI_GAMMA PERC_NCH PERC_PLUS PERC_MINUS MULTI_PLUS MULTI_MINUS MULTI_NCH')
    ...
    opt_file.write('\n\n#Set "print" to 1 if you want to print histograms with PDG ID of daughters, 0 if not (all histograms are saved in rootfiles anyway)')
    opt_file.write('\n\nprint: 0')
    ...
    opt_file.write('\n\n#Set the materials in which you want to fire your particles: "Al", "Be", "Cu" are available')
    opt_file.write('\n\nMaterials: ')
...
command = 's/nEvts =.*/nEvts = '+str(nevts)+'/'
...
os.system('TargetAnalysis/TargetAnalysis.out '+path+'/options.txt')

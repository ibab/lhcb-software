                      ...
                      int NumRichColl,  std::vector<int> RichG4CollectionID ) 
{
  ...
  std::vector<int> NumRich1GasSatHit = 
    aRichCounter->NumHitSaturatedPerTrackRich1Gas();
  
  std::vector<int> NumRich1AgelSatHit = 
    aRichCounter->NumHitSaturatedPerTrackRich1Agel();

  std::vector<int> NumRich2GasSatHit = 
  ...
  for(int ihtra=0; ihtra < NumSatTrajRich1Gas; ihtra++ ) {
    ...
    if( nhita > 0) {
      ...
      if(hNumTotHitC4F10Sat) hNumTotHitC4F10Sat->fill(nhita*1.0,1.0);
    }
  ...
  for(int ihtrb=0; ihtrb< NumSatTrajRich1Agel; ihtrb++){
    ...
    if(nhitb >0 ) {
      ...
      if(hNumTotHitAgelSat) hNumTotHitAgelSat->fill(nhitb*1.0,1.0);
    }
  ...
  for(int ihtrc=0; ihtrc < NumSatTrajRich2Gas; ihtrc++ ) {
    ...
    if( nhitc > 0) {
      ...
      if(hNumTotHitCF4Sat) hNumTotHitCF4Sat->fill(nhitc*1.0,1.0);
    }

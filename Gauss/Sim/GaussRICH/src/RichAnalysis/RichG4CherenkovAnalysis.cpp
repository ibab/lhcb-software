...
#include <math.h>
void RichG4CherenkovAnalysis1(const G4Step& aStep, G4double CosThCkv, G4double SinThCkv, G4double PhotProdCkvKE , G4ThreeVector PhotProdPosition, 
 G4double RefInd, G4double BetaInvChPart ) {
  ...
  G4String prePosMaterialName=aPreStepPoint->GetPhysicalVolume()->GetLogicalVolume()->GetMaterial()->GetName(); 
     ...
     SmartDataPtr<IHistogram1D>hCkvZEmissionPtRich1(CurrentHistoSvc,"RICHG4HISTO/25");
     if(hCkvZEmissionPtRich1) hCkvZEmissionPtRich1->fill(PhotProdPosition.z(),1.0);

          G4double Photwavelen=PhotMomCnv/(PhotProdCkvKE*1000000);

     if(pPreVolNameAgel == LogVolAgelNameAnalysis  && pPreVolMatNameAgel == AgelMaterialName &&
         prePos.z()>= AgelZBeginAnalysis &&  prePos.z() <= AgelZEndAnalysis ) {
          SmartDataPtr<IHistogram1D>hCkvAgelRich1(CurrentHistoSvc,"RICHG4HISTO/10");
          if(hCkvAgelRich1) hCkvAgelRich1->fill( ChrAngl ,1.0);
	  //          SmartDataPtr<IHistogram2D>hRefIndAgelRich1(CurrentHistoSvc,"RICHG4HISTO/12");
     ...
     if(pPreVolNameC4F10 == LogVolC4F10NameAnalysis &&  
          ...
          prePos.z() >= C4F10ZBeginAnalysis &&  prePos.z() <=  C4F10ZEndAnalysis ) {
         SmartDataPtr<IHistogram1D>hCkvC4F10Rich1(CurrentHistoSvc,"RICHG4HISTO/20");
          if(hCkvC4F10Rich1)hCkvC4F10Rich1->fill( ChrAngl,1.0);
	  //   SmartDataPtr<IHistogram2D>hRefIndC4F10Rich1(CurrentHistoSvc,"RICHG4HISTO/22");
     ...
     if(pPreVolNameCF4 == LogVolCF4NameAnalysis &&  
          ...
          SmartDataPtr<IHistogram1D>hCkvCF4Rich2(CurrentHistoSvc,"RICHG4HISTO/70");
          if(hCkvCF4Rich2)hCkvCF4Rich2->fill( ChrAngl,1.0);
          SmartDataPtr<IHistogram2D>hRefIndCF4Rich2(CurrentHistoSvc,"RICHG4HISTO/72");
          if(hRefIndCF4Rich2)hRefIndCF4Rich2->fill(Photwavelen,(RefInd-1.0),1.0);
	  ...
	  //Avoid photons which are already dead.
	     ...
	     //now for the case just before incident on the Mirror1 of Rich1.
	      ...
	      // fill a histogram for photon wavelength before mirror1.
	       // the reflection already happened at this point.
              if(PhotOriginZ > C4F10ZBeginAnalysis && PhotOriginZ < C4F10ZEndAnalysis ) {
		  if(PhotCurDir.z() > 0.0 ) {
                  
                  SmartDataPtr<IHistogram1D>hWaveLenBeforeRich1Mirror1(CurrentHistoSvc,"RICHG4HISTO/40");
                  if(hWaveLenBeforeRich1Mirror1)hWaveLenBeforeRich1Mirror1->fill(CurPhotWaveLen,1.0);
	     ...
	     // Now for the case just before incident on Quartz Window.
             ...
             if(cPreLogVolName1 == LogVolRich1MagShNameAnalysis && 
              ...
              if(PhotOriginZ > C4F10ZBeginAnalysis && PhotOriginZ < C4F10ZEndAnalysis ) {
                 if(PhotCurDir.z() > 0.0 ) {
                  SmartDataPtr<IHistogram1D>hWaveLenBeforeRich1GasQW(CurrentHistoSvc,"RICHG4HISTO/50");
                  if(hWaveLenBeforeRich1GasQW)hWaveLenBeforeRich1GasQW->fill(CurPhotWaveLen,1.0);

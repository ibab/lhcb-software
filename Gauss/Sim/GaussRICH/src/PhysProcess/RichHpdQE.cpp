...
RichHpdQE::RichHpdQE():m_curHpdNum(0), m_curRichDetNum(0),
...
{
  double photwa; 
  double photener = getHpdPhotEnergyFromBin(curmbin);
...
double  RichHpdQE::getHpdQEffFromPhotWavelen(double photwavelen) {
  ...
  double photonener = getHpdPhotonEnergyFromWavelen( photwavelen);

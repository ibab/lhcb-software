// $Id: GetMCCkvSegmentsAlg.cpp,v 1.13 2009-07-17 13:46:12 jonrob Exp $
// Include files 
...
//-----------------------------------------------------------------------------
// Implementation file for class : GetMCCkvSegmentsAlg
//
...
//=============================================================================
GetMCCkvSegmentsAlg::GetMCCkvSegmentsAlg( const std::string& name,
...
// Destructor
//=============================================================================
GetMCCkvSegmentsAlg::~GetMCCkvSegmentsAlg() { }

//=============================================================================
// Main execution
//=============================================================================
StatusCode GetMCCkvSegmentsAlg::execute()
{
  ...
  *gigaSvc() >> hitscollections;
  ...
  if ( 0 != hitscollections )
  {
    ...
    typedef std::pair< const CkvG4Hit*, const LHCb::MCRichHit* > HitDataType;
    ...
    for ( int iii = 0; iii < RichG4HitCollectionName()->RichHCSize(); ++iii )
    {
      ...
      if ( 0 == myCollection )
        return Error( "Null CkvG4HitsCollection", StatusCode::SUCCESS );
      ...
      for ( int ihit = 0; ihit < myCollection->entries(); ++ihit )
      {
        ...
        if ( g4hit->GetRadiatorNumber() >= 0 )
        {
          ...
          const int traid = const_cast<CkvG4Hit*>(g4hit)->GetTrackID();
  ...
  {
    info() << "No CkvG4Hits to be converted since no Collections available"
           << endmsg;
  }
...
//  Finalize
//=============================================================================
StatusCode GetMCCkvSegmentsAlg::finalize()
{

// $Id: CherenkovG4HistoFillSet2.cpp,v 1.11 2009-07-03 11:59:49 seaso Exp $
// Include files
...
CherenkovG4HistoFillSet2::CherenkovG4HistoFillSet2(  ) {

}
CherenkovG4HistoFillSet2::~CherenkovG4HistoFillSet2(  ) {
}

void CherenkovG4HistoFillSet2:: FillRichG4HistoSet2( const G4Event* anEvent,
                                                int NumRichColl,  
...
void CherenkovG4HistoFillSet2:: FillRichG4HistoSet2A()
{
...
void CherenkovG4HistoFillSet2:: FillRichG4HistoSet2B( )
{
  ...
  SmartDataPtr<IHistogram1D>hNumTotHitC4F10FullAcceptSat(CurrentHistoSvc,
                                               "RICHG4HISTOSET2/359");
  SmartDataPtr<IHistogram1D>hNumTotSignalHitC4F10FullAcceptSat(CurrentHistoSvc,
                                               "RICHG4HISTOSET2/358");

  SmartDataPtr<IHistogram1D>hNumTotHitCF4FullAcceptSat(CurrentHistoSvc,
                                             "RICHG4HISTOSET2/379");
  SmartDataPtr<IHistogram1D>hNumTotSignalHitCF4FullAcceptSat(CurrentHistoSvc,
                                             "RICHG4HISTOSET2/378");
  ...
  const std::vector<int> & NumRich1GasFullAcceptSatSignalHit =
    aRichCounter->NumSignalHitFullAcceptSatPerTrackR1Gas();
  ...
  int NumSignalSatTrajFullAcceptRich1Gas =  (int) NumRich1GasFullAcceptSatSignalHit.size();
  int NumSignalSatTrajFullAcceptRich2Gas =  (int) NumRich2GasFullAcceptSatSignalHit.size();
  //  G4cout<<" NumSatTrajFullAcceptRich1Gas  NumSatTrajFullAcceptRich2Gas "<<
  //  NumSatTrajFullAcceptRich1Gas <<"  "<<NumSatTrajFullAcceptRich2Gas <<G4endl;
  ...
  for(int ihtras=0; ihtras < NumSignalSatTrajFullAcceptRich1Gas; ++ihtras ) {

    int nhitas =  NumRich1GasFullAcceptSatSignalHit[ihtras];
    if( nhitas > 0) {
      if(hNumTotSignalHitC4F10FullAcceptSat )hNumTotSignalHitC4F10FullAcceptSat ->fill(nhitas,1.0);
    }   
  ...
  for(int ihtrb=0; ihtrb<NumSatTrajFullAccceptRich1Agel ; ++ihtrb){
    ...
    if(nhitb >0 ) {
      ...
      // if(hNumHitVsAngAgelFullAcceptSat) hNumHitVsAngAgelFullAcceptSat
      //      ->fill(trackThetaagel,nhitb);
    }  
  }
  ...
  for(int ihtrcs=0; ihtrcs <  NumSignalSatTrajFullAcceptRich2Gas ; ++ihtrcs ) {

    int nhitcs = NumRich2GasFullAcceptSatSignalHit[ihtrcs];
    if( nhitcs > 0) {
      if(hNumTotSignalHitCF4FullAcceptSat ) hNumTotSignalHitCF4FullAcceptSat->fill(nhitcs,1.0);
    }

// $Id: GeneratorToG4.cpp,v 1.1 2005-10-25 18:03:12 gcorti Exp $
// Include files 
  ...
  , m_gigaSvc      ( 0 )
{
  ...
  declareProperty( "HepMCEventLocation", m_location = HepMCEventLocation::Default );
...
StatusCode GeneratorToG4::execute() {
  ...
  HepMCEvents* hepEvts = get<HepMCEvents> ( m_location );
  ...
  for( HepMCEvents::const_iterator iEvt = hepEvts->begin(); 
        ...
        if( ( (*iPart)->status() == 1 ) ||
          ...
          //             (that is to say a particle gun or machine background)
            ...
            if( (*iPart)->production_vertex()->particles_in_size() == 1 ) {
              ParticleID pidMother( (*(*iPart)->production_vertex()
                                     ->particles_in_const_begin())->pdg_id() );
              ParticleID pid( (*iPart)->pdg_id() );
...
G4PrimaryParticle* GeneratorToG4::makeG4Particle(HepMC::GenParticle* genPart,
  ...
  G4PrimaryParticle*  g4Particle = new G4PrimaryParticle( genPart->pdg_id() , 
  ...
  GiGaPrimaryParticleInformation* gInfo = 
    new GiGaPrimaryParticleInformation( isSignalParticle, genPart->barcode(), theEvent );

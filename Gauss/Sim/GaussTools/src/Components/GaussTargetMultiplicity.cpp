				...
				, m_intVolumeString ( )
{
  // declare own properties
	...
	declareProperty( "InteractionMaterialName" , m_intMaterialName ) ;
...
int GaussTargetMultiplicity::checkIntRegion( const G4Track * dtr ){
  ...
  // Material
	...
	/*
	std::vector<std::string>::iterator it = m_intVolumeName.begin();
	std::vector<std::string>::iterator end = m_intVolumeName.end();
	for (; it < end; ++it )
	debug() << *it << endmsg;
	*/
  ...
  //check if the daugther Physical and Logical volume names and material are those specified by the user via properties
	...
	else if(std::find(m_intMaterialName.begin(), m_intMaterialName.end(), matname) != m_intMaterialName.end()) 
	{
		insider = 1;
	}

  //check if the daughter volume name contains the substring specified
  // by the user via properties

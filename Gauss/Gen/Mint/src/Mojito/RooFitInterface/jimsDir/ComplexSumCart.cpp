...
ClassImp(ComplexSumCart)

complex<Double_t> ComplexSumCart::getSum() const{
...
void ComplexSumCart::addValues(std::map<std::string, Double_t>& valMap) const{
  TIterator* resIter = resonances.createIterator() ;
  AbsComplexBrackets* resPtr;
  complex<Double_t> sum = 0;
  while((resPtr = dynamic_cast<AbsComplexBrackets*>(resIter->Next()))){ 
...
ComplexSumCart::ComplexSumCart(const char* name
		       , const char* title
...
{
  cout << "ComplexSumCart: Got 1 list: " << endl;
  ...
  while((resPtr = (RooAbsArg*) resIter->Next())){
    ...
    if(debug)cout << " ComplexSumCart::ComplexSumCart: adding variable "
		  <<  resPtr->GetName() 
		  << endl;

    if (!dynamic_cast<AbsComplexBrackets*>(resPtr)) {
      cout << "ERROR in ComplexSumCart::ComplexSumCart: variable "
	   <<  resPtr->GetName() 
	   << " is not complex "
	   << endl;
      throw "wrong type";
  ...
  delete resIter;
  cout << "ComplexSumCart: done constructing " << endl;
}

ComplexSumCart::ComplexSumCart(const ComplexSumCart& other
		       , const char* name
		       )
...
{
  /*cout << "ComplexSumCart " << GetName() << " got copied."
       << "\n Sum before copy: " << other.getSum()
       << "\n Sum after copy:  " << this->getSum()
       << endl;*/
}

Double_t ComplexSumCart::evaluate() const{

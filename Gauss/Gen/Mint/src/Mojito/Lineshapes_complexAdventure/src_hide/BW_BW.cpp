...
BW_BW::BW_BW(const BW_BW& other)
  ...
  , _prSq_set(other._prSq_set)
  , _pABSq_set(other._pABSq_set)
  , _Fr_BELLE_set(other._Fr_BELLE_set)
  , _Fr_PDG_BL_set(other._Fr_PDG_BL_set)
  , _GofM_set(other._GofM_set)
...
std::complex<double> BW_BW::Fr_BELLE(){
  ...
  if(! _Fr_BELLE_set){
    ...
    if(twoLPlusOne() == 3){                        // spin = 1
      ...
      if(dbThis)cout << "BW Fr R = " << R << "prSq() = " << prSq() 
		     << " pABSq() " << pABSq()
		     << endl;
      double FrSq = (1. + R_pr_sq)/(1. + R_pAB_sq);
      if(FrSq >=0) _Fr_BELLE = sqrt(FrSq);
      else _Fr_BELLE = std::complex<double>(0, sqrt(-FrSq));
      ...
      double FrSq =  (  9. + 3.*R_pr_sq  + R_pr_4)
...
std::complex<double> BW_BW::Fr_PDG_BL(){
  // (ratio of) Blatt-Weisskopf penetration factors
  ...
  if(! _Fr_PDG_BL_set){
    ...
    if(twoLPlusOne() == 3){                        // spin = 1
      ...
      else _Fr_PDG_BL = complex<double>(0, sqrt(-FrSq));
      _Fr_PDG_BL_set = true;
    }else if(twoLPlusOne() == 5){                   // spin == 2
...
std::complex<double> BW_BW::GofM(){
  bool dbThis=false;
  if(! _GofM_set){
    double densq = prSq();  

    /*
    double saveDensq = twoBody_dgtPsq_in_MumsFrame(mumsPDGMass()
						   , daughterRecoMass(0)
						   , daughterRecoMass(1)
						   );  // debug only
    */
    ...
    std::complex<double> pratio(1,0);
    double ratioSq = numsq/densq;
    
    if(ratioSq >= 0){
      pratio = sqrt(ratioSq);
    }else{
      //pratio = complex<double>(0, sqrt(-ratioSq));
      complex<double>(sqrt(-ratioSq), 0);
    }

    std::complex<double> pratio_to_2Jplus1(1,0);
    ...
    std::complex<double> thisFr = Fr_BELLE();
...
std::complex<double> BW_BW::BreitWigner() {
  ...
  std::complex<double> invBW(mpdg*mpdg - mumsRecoMass2() - i * mpdg * GofM());
...
void BW_BW::resetInternals(){
  _prSq = _pABSq = _mumsRecoMass2 =  _mumsRecoMass = -1;
  _Fr_BELLE = _Fr_PDG_BL =  _GofM = -1;
...
std::complex<double> BW_BW::getVal(){
  ...
  if(startOfDecayChain()){
    ...
    if(dbThis && (abs(returnVal) > 2 || abs(returnVal) < 0.5)){
      cout << " BW_BW for " 
...
std::complex<double> BW_BW::EvtGenValue(){
  ...
  // function is part of BW_BW for convencience, as BW_BW provides all
  ...
  if(dbThis) cout << "EvtGen: fR = " << fR << ", fD = " << fD << endl;

  //  report(INFO,"EvtGen") << "fR = " << fR << " fD = " << fD << endl;
  
  double gammaAB= gammaR*pow(pAB/pR,power)*(mR/mAB)*fR*fR;
  cout << "EvtGen: pAB^2 " << pAB*pAB << ", pR^2 " << pR*pR << endl;
  if(dbThis) cout << "EvtGen: gamma AB = " << gammaAB << endl;

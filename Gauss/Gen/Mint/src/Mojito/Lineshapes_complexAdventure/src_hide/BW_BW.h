...
class BW_BW : public DalitzEventAccess, virtual public ILineshape{
 private:
  mutable double _prSq, _pABSq, _mumsPDGMass, _mumsWidth, 
    _mumsRecoMass2, _mumsRecoMass;
  mutable std::complex<double> _Fr_BELLE, _Fr_PDG_BL, _GofM;
  mutable int _mumsPID;
  mutable bool _mumsPID_set;
  mutable bool _prSq_set, _pABSq_set, _Fr_BELLE_set, _Fr_PDG_BL_set;
  mutable bool _GofM_set;
 ...
 protected:
  ...
  virtual std::complex<double> Fr_PDG_BL();
  virtual std::complex<double> Fr_BELLE();
  virtual double Fr_BELLE_Max();

  virtual std::complex<double> Fr();
  virtual double FrMax();
  virtual std::complex<double> GofM();

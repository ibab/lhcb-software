...
/*
int ScpBinning::mergeAdjacentBoxes(IDalitzEventList* events, ScpBoxSet& boxes, int maxPerBin, int minPerBin){
	//	lessByScpBoxData sorter;
	//	sort(boxes.begin(), boxes.end(), sorter);
	//   double norm = this->normFactor();
	...
	// Create one signle bin across phase space
	...
	do{
		...
		ScpBoxSet mergedBoxes;

		// Check that first splitting requires merging
		...
		for(unsigned int i=0; i < boxes.size(); i++){
			...
			{
				mergedBoxes.push_back(boxSet);
				boxSet.clear();
			}
		}

		for(unsigned int i=0; i < mergedBoxes.size(); i++){
			// Split boxes
			if( mergedBoxes[i].nData() > maxPerBin ){
				needToSplitMore=true;
				newBoxes.add(mergedBoxes[i].split());
			}
			else{
				newBoxes.add(mergedBoxes[i]);
			}
		}
...
int ScpBinning::mergeBoxes(ScpBoxSet& boxes, int minPerBin){
  ...
  for(unsigned int i=0; i < boxes.size(); i++){
      boxSet.add(boxes[i]);
      if(boxSet.weightedData() >= minPerBin /*&& boxSet.weightedMC() >= minPerBin */){
          _boxSets.push_back(boxSet);
          tot = tot + boxSet.weightedData();
          boxSet.clear();
      }
  }
...
ScpBoxSet ScpBinning::splitBoxes(IDalitzEventList* events
  ...
  do{
      ...
      needToSplitMore=false;
      for(unsigned int i=0; i < boxes.size(); i++){
          if( boxes[i].weightedData() > maxPerBin /*|| boxes[i].weightedMC() > maxPerBin*/ ){
              needToSplitMore=true;
...
void ScpBinning::print(std::ostream& os) const{
  ...
  for(unsigned int i=0; i < _boxSets.size(); i++){
      ...
      os << "  dataWeight " <<  _boxSets[i].weightedData();
...
int ScpBinning::Diff_ofBin(unsigned int i) const{
  if(i > _boxSets.size()) return -9999;
  int diff = _boxSets[i].weightedData() - _boxSets[i].weightedMC();
  return diff;
}

double ScpBinning::NormDiff_ofBin(unsigned int i) const{
  if(i > _boxSets.size()) return -9999;
  double Normdiff = _boxSets[i].weightedData() - normFactor()*_boxSets[i].weightedMC();
  return Normdiff;
}
...
double ScpBinning::Err_ofBin(unsigned int i) const{
  ...
  //  if (ndata == 0 || ndataCC == 0) Err = 0.0;
  return Err;
}

double ScpBinning::Entries_ofBin(unsigned int i) const{
  if(i > _boxSets.size()) return -9999;
  int entries = _boxSets[i].nData() + _boxSets[i].nMC();
  return entries;
}
...
double ScpBinning::weightedData() const{
  double sum(0);
  for (int i = 0; i < _boxSets.size(); ++i)
    {
...
double ScpBinning::weightedMC() const
{
  double sum(0);
  for (int i = 0; i < _boxSets.size(); ++i)
    {

...
int ampFit(){
  ...
  for(int j_ex = 0; j_ex < Nexperiments; j_ex++){
    ...
    AmpsPdf amps(&sg, &eventList
		 , integPrecision, integMethod, SgIntegratorFname);
    AmpsPdf ampsBg(&bg, &eventList
		   , integPrecision, integMethod, PeakyBgIntegratorFname);    
    ...
    // make plots:
    if(makePlots){
      DalitzHistoSet fitH = amps.histoSet() * eventList.size(); 
      ...
      datH.drawWithFit(fitH
		       , "datFit_" + anythingToString(j_ex) + "_");
    }

...
void Evtbs2llGammaISRFSRAmp::CalcAmp(EvtParticle *parent,
			                   EvtAmp& amp,
			                   Evtbs2llGammaFF *formFactors,
                               EvtbTosllWilsCoeffNLO *WilsCoeff, 
                               double mu, int Nf, int sr,  
...
//  FILE *mytest;
  ...
  int il1=1, il2=2;  // leptons are the second and thirds daughter particles 
  ...
  lepPlus  = (charge1 > charge2) ? parent->getDaug(il1) : parent->getDaug(il2); // positive charged
  lepMinus = (charge1 < charge2) ? parent->getDaug(il1) : parent->getDaug(il2); // negative charged

  EvtVector4R p = parent->getP4Restframe(); 	 // B-meson momentum in the B-rest frame
  EvtVector4R k = parent->getDaug(iG)->getP4();  // 4-momentum of photon in the B-rest frame
  ...
  EvtVector4R p_minus_p_1 = p-p_1; // transition momentum of the B-meson and antilepton p-p_1
  EvtVector4R p_minus_p_2 = p-p_2; // transition momentum of the B-meson and lepton p-p_2
  ...
  double Egam  = 0.5*M1*(1-hatq2);   // photon energy in the B-meson rest frame
...
//      << std::endl;
  ...
  if (bmesons.contains(parentID)){
    ...
    for(i=0;i<2;i++){ 				
      ...
      // 1
      amp.vertex(i,0,0,CKM_factor*(lvc11*E1+lac11*E2+uniti*lsc11*E3+
...
//        << std::endl;
      ...
      // 2
      amp.vertex(i,0,1,CKM_factor*(lvc12*E1+lac12*E2+uniti*lsc12*E3+
...
//        << std::endl;
      ...
      // 3
      amp.vertex(i,1,0,CKM_factor*(lvc21*E1+lac21*E2+uniti*lsc21*E3+
...
//        << std::endl;
      ...
      // 4
      amp.vertex(i,1,1,CKM_factor*(lvc22*E1+lac22*E2+uniti*lsc22*E3+
...
//
double Evtbs2llGammaISRFSRAmp::CalcMaxProb(EvtId parnum, EvtId photnum,
  ...
  double Mrho   = EvtPDL::getMeanMass(EvtPDL::getId(std::string("rho0")));  // mass of the rho-meson, MeV
...
//   << std::endl;
  ...
  list_of_max_q2_points[4] = pow(M1,2.0)-2.0*M1*Egamma_min; // q^2_max at photon energy cut
...
//  }
  ...
  if(res_swch==0||res_swch==1){
     ...
     for(i_list=0;i_list<=4;i_list++){
        ...
        t_plus = t_plus + sqrt(1.0-4.0*pow(ml,2.0)/s)*(pow(M1,2.0)-s);
        t_plus *=0.5;

        t_minus = pow(M1,2.0)+2.0*pow(ml,2.0)-s;
        t_minus = t_minus - sqrt(1.0-4.0*pow(ml,2.0)/s)*(pow(M1,2.0)-s);
        t_minus *=0.5;

        if(fabs(t_plus-t_minus)<0.000001) t_minus=t_plus;
        ...
        for(ijk=0; ijk<=max_ijk; ijk++){
          ...
          cosBellminus = (pow(ml,2.0)+2.0*Eg*El2-t_for_s)/(2.0*Eg*modl2);

          if((fabs(cosBellminus)>1.0)&&(fabs(cosBellminus)<=1.0001)){
            report(NOTICE,"EvtGen") 
              << "\n Debug in the function EvtbsTollGammaISRFSRAmp::CalcMaxProb(...):"
              << "\n cos(theta) = " << cosBellminus 
              << std::endl;
          ...
          k.set(Eg,  Eg, 0.0, 0.0);
          p2.set(El2, modl2*cosBellminus, -modl2*sqrt(1.0-pow(cosBellminus,2.0)), 0.0);
          p1=p-k-p2;

...
public:
  ...
  bool getIndices(){
    ...
    for(unsigned int i=0; i < pat.size(); i++){
      if(-321 == pat[i]){
	_KMinus_index = i;
      }else if(211 == pat[i] || 777 == pat[i]){ // 777 for debug only
	if(_piPlusFast_index > 0) _piPlusSlow_index = i;
	else _piPlusFast_index = i;
      }else if(-211 == pat[i] || -777 == pat[i]) _piMinus_index =i;// 777 for debug only
    }
...
int coherenceFactor(){
  ...
  NamedParameter<double> precision("Precision", (double) 1.e-3);

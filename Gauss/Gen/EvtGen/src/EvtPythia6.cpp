...
int EvtPythia6::njetsetdecays=0;
EvtDecayBasePtr* EvtPythia6::jetsetdecays=0; 
int EvtPythia6::ntable=0;

int EvtPythia6::ncommand=0;
int EvtPythia6::lcommand=0;
std::string* EvtPythia6::commands=0;
...
std::string EvtPythia6::getName( ){

  return "PYTHIA";     

}

EvtDecayBase* EvtPythia6::clone(){

  return new EvtPythia6;

}


void EvtPythia6::initProbMax(){
...
void EvtPythia6::init(){
  ...
  if (getParentId().isAlias()){
    ...
    report(ERROR,"EvtGen") << "EvtPythia6 finds that you are decaying the"
                           <<std::endl
                           << " aliased particle "
...
std::string EvtPythia6::commandName(){
...
void EvtPythia6::command(std::string cmd){
...
void EvtPythia6::pythiacont(double */*energy*/, int */*ndaugjs*/, int */*kf*/,
...
void EvtPythia6::decay( EvtParticle *p){
  ...
  do{
    ...
    int channel=EvtDecayTable::getInstance()->inChannelList(p->getId(),numstable,
                                             evtnumstable);
  ...
  if (count>9999) {
    report(INFO,"EvtGen") << "Too many loops in EvtPythia6!!!"<<std::endl;
...
void EvtPythia6::fixPolarizations(EvtParticle *p){
...
void EvtPythia6::store(EvtDecayBase* jsdecay){
...
void EvtPythia6::WritePythiaEntryHeader(std::ofstream &outdec, int lundkc,
...
void EvtPythia6::WritePythiaParticle(std::ofstream &outdec,EvtId ipar,
  ...
  for(ijetset=0;ijetset<njetsetdecays;ijetset++){
	...
	if(i<jetsetdecays[ijetset]->getNDaug()){
      ...
      channel=EvtDecayTable::getInstance()->findChannel(EvtPDL::chargeConj(ipar),
...
bool
EvtPythia6::isPythiaSpecial(int ID)
{
  switch(ID)
    {
    case 1:
...
void EvtPythia6::MakePythiaFile(char* fname){  
  EvtId ipar;
  int lundkc;  
...
//      if ( EvtDecayTable::getInstance()->isJetSet( ipar ) ) { replace for new EvtGen by
      if ( EvtDecayTable::getInstance()->decaytable()[ipar.getAlias()].isJetSet() ) {
...
//    if ( EvtDecayTable::getInstance()->isJetSet( ipar ) ) { for new EvtGen:
    if ( EvtDecayTable::getInstance()->decaytable()[ipar.getAlias()].isJetSet() ) {
      //no aliased particles!
...
void EvtPythia6::pythiaInit(int /*dummy*/){
  EvtId  ipar ;
  int    lundkc ;

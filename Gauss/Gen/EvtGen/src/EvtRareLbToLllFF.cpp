                             ...
                             EvtRareLbToLllFF::FormFactors& FF )
{
  ...
  if ( it == FFMap_.end() ) {
    report( ERROR,"EvtGen") << " EvtRareLbToLll does not contain FF for " 
  ...
  { 
    report( ERROR,"EvtGen") << " EvtRareLbToLll expects DIRAC or RARITASWINGER daughter " << std::endl;// should add a warning here 
    return ;
  }

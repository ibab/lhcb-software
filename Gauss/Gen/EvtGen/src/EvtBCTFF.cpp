...
void EvtBCTFF::getscalarff(EvtId, EvtId, double, double,
  ...
  report(ERROR,"EvtGen") << "Not implemented :getscalarff in EvtBCTFF.\n";  
  ::abort();
  ...
  if (idTensor == EvtPDL::getId("chi_c2").getId() ) { // Bc -> chi_c1
    if (whichfit == 3) { // FF from Wang et al 10.1103/PhysRevD.79.114018
      ...
      *bmf = bmf_0*exp(bmf_c1*ratio+bmf_c2*ratio*ratio);
      ...
      report(ERROR,"EvtGen") << "Must choose 0 (a1f = 1) or 3 (Wang).\n";  
      ::abort();
    }
  } else {
    report(ERROR,"EvtGen") << "chi_c1 is the only (pseudo)vector decay implemented in EvtBCTFF.\n";  
    ::abort();    
  }
}
...
void EvtBCTFF::getbaryonff(EvtId, EvtId, double, double,
  ...
  report(ERROR,"EvtGen") << "Not implemented :getbaryonff in EvtBCTFF.\n";  
  ::abort();
...
void EvtBCTFF::getdiracff(EvtId, EvtId, double, double,
  ...
  report(ERROR,"EvtGen") << "Not implemented :getdiracff in EvtBCTFF.\n";
  ::abort();
...
void EvtBCTFF::getraritaff(EvtId, EvtId, double, double,
  ...
  report(ERROR,"EvtGen") << "Not implemented :getraritaff in EvtBCTFF.\n";
  ::abort();

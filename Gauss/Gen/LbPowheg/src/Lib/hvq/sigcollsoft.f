c subroutines hvq_soft, hvq_collfsr, hvq_softcollfsr, hvq_collisrp, hvq_collisrm,
c             hvq_softcollisrp, hvq_softcollisrm 
c are called by hvq_btildereal; they return (1-y) csi^2 R_alr (if kn_emitter
c is >2, i.e. final state) or (1-y^2) csi^2 R (if kn_emitter <=2) in the
c collinear final state                      (hvq_collfsr)
c hvq_soft-collinear final state                 (hvq_softcollfsr)
c collinear initial state in the + direction (hvq_collisrp)
c collinear initial state in the - direction (hvq_collisrm)
c hvq_soft-collinear initial state +             (hvq_softcollisrp)
c hvq_soft-collinear initial state -             (hvq_softcollisrm)
c The hvq_soft limit is taken keeping y (i.e. kn_y) fixed
...
c
c
c Functions: hvq_collbtl and hvq_softbtl are required to compute the damping
c            factor in hvq_bornzerodamp. They are called by sigreal, and
c            sigreal is used by hvq_btilde (hence the btl suffix)

c hvq_softbtl: computes the same quantity as hvq_soft, but does not include 
c          the luminosity
c
c hvq_collbtl: if kn_emitter is in the final state, computes the same quantity as
c          hvq_collfsr, but does not include the luminosity
c          if kn_emitter is 1 or 2  computes the same quantity as
c          hvq_collisrp or hvq_collisrm, without luminosity
c          if kn_emitter is 0 computes the average of the return values
c          of hvq_collisrp and hvq_collisrm, weighted with the factors 
c          (1+y)/2 and (1-y)/2.
c
c 
c
c Functions: hvq_collrad, hvq_softrad  are required to compute the damping
c            factor in hvq_bornzerodamp. They are called by hvq_sigreal_rad,
c            i.e. they are used in the generation of radiation.
c            They compute the same quantities as hvq_collbtl and hvq_softbtl,
...
c            kinematic regions).
      ...
      subroutine hvq_collfsrnopdf(rc)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
...
c for fsr csi is y independent
      ...
      phi=kn_azi
      x=csi*xocsi
      call hvq_buildkperp(em,phi,kperp,kperp2)
      do alr=1,flst_nalr
         if(em.eq.flst_emitter(alr)) then
            call hvq_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
      ...
      subroutine hvq_buildkperp(em,phi,kperp,kperp2)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
...
c     Then rotate it counterclockwise around the em direction
     ...
     #/kn_cmpborn(3,em)
      ...
      call hvq_mrotate(dir,sin(phi),cos(phi),kperp(1))
      kperp(0)=0
      end
      

      subroutine hvq_softcollfsr(rc)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
      ...
      call hvq_collfsrnopdf(rc)
      kn_csi=tmp
      call hvq_pdfcall(1,kn_xb1,pdf1)
      call hvq_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end
      ...
      subroutine hvq_collisrp(rc)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      real * 8 rc(maxalr)
      call hvq_collisr(1,rc)
      end

      subroutine hvq_collisrm(rc)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      real * 8 rc(maxalr)
      call hvq_collisr(2,rc)
      end

      subroutine hvq_softcollisrp(rc)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
      ...
      subroutine hvq_softcollisrm(rc)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
      ...
      call hvq_pdfcall(1,x1,pdf1)
      call hvq_pdfcall(2,x2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end

      subroutine hvq_collisrnopdf(i,rc)
      implicit none
      integer i
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
...
c Construct kperp
      ...
      do alr=1,flst_nalr
         if(flst_emitter(alr).eq.kn_emitter) then
            call hvq_collisralr(alr,i,csi,kperp,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
...
c softvec(0:3): real * 8, 4-vector of hvq_soft gluon normalized to softvec(0)=1
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
...
c find boost velocity
      ...
      y=kn_y
      do alr=1,flst_nalr
         call hvq_softalr(alr,em,y,r0(alr))
      enddo
      call hvq_pdfcall(1,kn_xb1,pdf1)
      call hvq_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
      ...
      subroutine hvq_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,res)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
...
c provide multiplicity of emitter in underlyng Born
      ...
      subroutine hvq_collisralr(alr,i,csi,kperp,res)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
...
c     The remaining csi=1-x factor has been applied earlier
      ...
      subroutine hvq_softalr(alr,em,y,res)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
               ...
               if(abs(flst_born(j,iub)).le.6) then
                  do k=j+1,nlegborn
                     if(abs(flst_born(k,iub)).le.6) then
                        result=result+
     #hvq_dotp(kn_cmpborn(0,j),kn_cmpborn(0,k))
     #/(hvq_dotp(kn_cmpborn(0,j),kn_softvec)*
     #hvq_dotp(kn_cmpborn(0,k),kn_softvec)) * br_bornjk(j,k,iub)
                     endif
                  enddo
               endif
            enddo
...
c     symmetric, multiply by 2
     ...
     #kn_cmpborn(2,j)**2-kn_cmpborn(3,j)**2
                  result=result-pjsq/hvq_dotp(kn_cmpborn(0,j),
     #kn_softvec)**2*br_born(iub)*cf
               endif
            enddo
c     having chosen a hvq_soft four momentum of energy 1, supply
...
c     Multiply hvq_soft result by (hvq_soft limit Sij appropriate factors)
      if(res.ne.0) then
         sumdijinv=0
...
c cross section used in hvq_btilde
      subroutine hvq_collbtl(rc)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
         ...
         do alr=1,flst_nalr
            if(flst_emitter(alr).eq.em) then
               if(em.ne.2) then
                  ...
                  call hvq_collisralr(alr,i,csi,kperp,r1)
               endif
               if(em.ne.1) then
                  i=2
                  csi=kn_csi*kn_csimaxm/kn_csimax
                  call hvq_collisralr(alr,i,csi,kperp,r2)
               endif
               if(em.eq.0) then
...
c     Construct kperp
            ...
            if(em.ne.2) then
               csi=kn_csi*kn_csimaxp/kn_csimax
               call hvq_collisralr(alr,1,csi,kperp,r1)
            endif
            if(em.ne.1) then
               csi=kn_csi*kn_csimaxm/kn_csimax
               call hvq_collisralr(alr,2,csi,kperp,r2)
            endif
            if(em.eq.0) then
               rc(alr)=(r1*(1+kn_y)+r2*(1-kn_y))/2
            elseif(em.eq.1) then

      subroutine hvq_btildereal(xrad,resreal,www)
      implicit none
     ...
     # rrrp,rrrps,rrrm,rrrms,remnant,out0,out1
      integer j,iuborn
      logical hvq_valid_emitter
      external hvq_valid_emitter
      do j=1,flst_nborn
         resreal(j)=0
      enddo
      do kn_emitter=0,nlegborn
c output values for hvq_analysis_driver
         out0=0
         out1=0
c check that emitter is valid
         if(hvq_valid_emitter(kn_emitter)) then
            if(kn_emitter.gt.2) then
c     final state radiation
               call hvq_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call hvq_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call hvq_collfsr(rc)
c     hvq_soft subtraction
                  call hvq_soft(r0s)
                  call hvq_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call hvq_collisrp(rp)
                     call hvq_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call hvq_collisrm(rm)
                     call hvq_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call hvq_analysis_driver(out0,0)
            if(out1.ne.0d0) call hvq_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine hvq_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
      include 'src/Lib/hvq/pwhg_dbg.h'
      external hvq_sigreal_btl,hvq_soft,hvq_collfsr,hvq_softcollfsr, hvq_collisrp,
     $     hvq_softcollisrp,hvq_collisrm,hvq_softcollisrm
      call hvq_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  hvq_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call hvq_checksoft(hvq_sigreal_btl,hvq_soft,' hvq_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call hvq_checkcoll(hvq_sigreal_btl,hvq_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
c            call hvq_randomsave
            if(kn_emitter.ne.2) call hvq_checkcoll(hvq_sigreal_btl,hvq_collisrp,1
     $           ,' coll-plus',iun)
c            call hvq_randomrestore
            if(kn_emitter.ne.1) call hvq_checkcoll(hvq_sigreal_btl,hvq_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  hvq_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call hvq_checksoft(hvq_collfsr,hvq_softcollfsr,' hvq_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  hvq_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call hvq_checksoft(hvq_collisrp,hvq_softcollisrp,
     $           ' hvq_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call hvq_checksoft(hvq_collisrm,hvq_softcollisrm,
     $           ' hvq_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-hvq_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call hvq_checkcoll(hvq_soft,hvq_softcollfsr,1,' coll-hvq_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-hvq_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call hvq_checkcoll(hvq_soft,hvq_softcollisrp,1
     $           ,' coll-plus-hvq_soft',iun)
            if(kn_emitter.ne.1) call hvq_checkcoll(hvq_soft,hvq_softcollisrm,-1
     $           ,' coll-minus-hvq_soft',iun)
         enddo
      endif
      call hvq_randomrestore
      end

      subroutine hvq_checkborn(iun)
...
c are consistent with total Born
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine hvq_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/hvq/pwhg_dbg.h'
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 hvq_random,hvq_dotp
      external hvq_random,hvq_dotp
      logical hvq_valid_emitter,iszero,isnonzero,isequal
      external hvq_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=hvq_random()
      enddo
      ...
      call hvq_allborn
      call hvq_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*hvq_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=hvq_random()
      enddo
c Check hvq_soft limits
      if(hvq_valid_emitter(kn_emitter)) then
         write(iun,*) ' hvq_random Born variables ====> ',xborn
          write(iun,*) ' hvq_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call hvq_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call hvq_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check hvq_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
            ...
            if(isnonzero.and..not.iszero) then
               write(iun,*) ' emitter ',kn_emitter, ', hvq_process ',
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine hvq_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/hvq/pwhg_dbg.h'
      include 'src/Lib/hvq/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' hvq_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
...
c               if(r0(alr,jexp).ne.0) isnonzero=.true.
 ...
 111        continue
     ...
     #', hvq_process ',(flst_alr(j,alr),j=1,nlegreal),', ',label,':'
               do alrp=alr+1,flst_nalr
                  isequal=.true.
                  do jexp=1,nexp
     ...
     #r0c(alr,jexp).ne.r0c(alrp,jexp)) isequal=.false.
                  ...
                  if(isequal) then
                     write(iun,*) ' emitter ',kn_emitter,
     #', hvq_process ',(flst_alr(j,alrp),j=1,nlegreal),', ',label,':'
                     ident(alrp)=.true.
                  endif
               enddo
...
c     Added this 'if' to be sure that no division by zero occurs
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine hvq_sigreal_rad(sig)
      implicit none
      real * 8 sig
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      ...
      if(ini) then
         ...
         if(flg_smartsig) then
            call hvq_randomsave
c     generate "nmomset" hvq_random real-phase space configurations
            call hvq_fillmomenta(nlegreal,nmomset,kn_masses,preal)
            do alr=1,flst_nalr
               do j=1,nmomset
                  call hvq_realgr(
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call hvq_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      do alr=1,flst_nalr
         r0(alr)=0
      enddo
      call hvq_pdfcall(1,kn_x1,pdf1)
      call hvq_pdfcall(2,kn_x2,pdf2)
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call hvq_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call hvq_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine hvq_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call hvq_sigreal_btl0(r0,0)
      end

c Real cross section, required by hvq_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine hvq_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
...
c     generate "nmomset" hvq_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call hvq_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c First mark as being computed
               markused(alr)=1
               call hvq_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' hvq_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: hvq_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine hvq_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=hvq_random()
            enddo
         enddo
         preal(3,nlegreal,j)=hvq_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine hvq_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from hvq_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine hvq_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call hvq_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'hvq_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine hvq_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_math.h'
      ...
      integer rflav(nlegreal)
      real * 8 amp2 
      logical hvq_pwhg_isfinite
      external hvq_pwhg_isfinite
      call hvq_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.hvq_pwhg_isfinite(amp2)) amp2=0d0

      ...
      integer i1,n1,n2
      call hvq_init_flsttag
      flg_debug=.false.
      if(hvq_powheginput("#flg_debug").eq.1) flg_debug=.true.
c whether to output negative weights or not
      flg_withnegweights=.false.
      if(hvq_powheginput("#withnegweights").eq.1)flg_withnegweights=.true.
c See if we have weighted events
      flg_weightedev=.false.
      if(hvq_powheginput("#bornsuppfact").gt.0) flg_weightedev=.true.
      if(hvq_powheginput("#ptsupp").gt.0) then
...
c     amplitudes, for Born, real and virtual contributions
      flg_smartsig=.true.
      if(hvq_powheginput("#smartsig").eq.0) flg_smartsig=.false.
c     if true also counterterm are output in NLO tests (default)
      flg_withsubtr=.true.
      if(hvq_powheginput("#withsubtr").eq.0) flg_withsubtr=.false.
...
c     is required
      flg_withdamp=.false.
      if((hvq_powheginput("#withdamp").eq.1).or.
     $  (hvq_powheginput("#hfact").gt.0d0))   flg_withdamp=.true.
c     If set do only the Born term
      flg_bornonly=.false.
      if (hvq_powheginput("#bornonly").eq.1) flg_bornonly=.true.
c     initialize hvq_random number sequence
      i1=hvq_powheginput('#iseed')
      if (i1.lt.0) i1=0
      n1=hvq_powheginput('#rand1')
      if (n1.lt.0) n1=0
      n2=hvq_powheginput('#rand2')
      if (n2.lt.0) n2=0
c select which upper bounding function form
      rad_iupperisr=hvq_powheginput("#iupperisr")
      if(rad_iupperisr.lt.0) rad_iupperisr=1
      rad_iupperfsr=hvq_powheginput("#iupperfsr")
      if(rad_iupperfsr.lt.0) rad_iupperfsr=2
c info on pdf for each event
      flg_pdfreweight=.false.
      if(hvq_powheginput("#pdfreweight").eq.1)flg_pdfreweight=.true.
c
      call hvq_setrandom(i1,n1,n2)
c     assign a default id for the hvq_process at hand
c     if the user want to assign different id's
c     to each subprocess, he/she can reassign lprup(1)
c     inside the user-defined subroutine hvq_init_processes
      lprup(1)=10001

      subroutine hvq_genericpdf(ndns,ih,xmu2,x,fx)
c Interface to hvq_MLMPDF package.
      implicit none
      integer ndns,ih
      ...
      sx=x
      sxmu2=xmu2
      call hvq_MLMPDF(ndns,ih,sxmu2,sx,sfx,5)
      do j=-5,5
         fx(j)=sfx(j)
      enddo
      fx(1)=sfx(2)
...
c link errors. It is never invoked in the present case.
      subroutine hvq_getq2min()
      end

      subroutine hvq_genericpdfpar(ndns,ih,xlam,hvq_scheme,iorder,iret)
      implicit none
      integer ndns,ih
      real * 8 xlam
      character * 2 hvq_scheme
      integer iret,iorder
      call hvq_PDFPAR(ndns,ih,xlam,hvq_scheme,iret)
c not yet implemented
      iorder=-1
      end

      function hvq_whichpdfpk()

...
c...writes initialization information to a les houches events file on unit nlf. 
      subroutine hvq_lhefwritehdr(nlf)
      implicit none
     ...
     #     version
      ...
      call hvq_wrtpowheginput(nlf)
      write(nlf,'(a)') 'End of powheg.input content'
      call hvq_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) 'hvq_random number generator initialized with: ',
     # iran,' ',n1ran,' ',n2ran
      write(nlf,'(a)') '-->'
      write(nlf,'(a)') '<init>'
 ...
 120  format(1p,3(1x,e12.5),1x,i6)
      ...
      subroutine hvq_printleshouches
c useful for debugging
      call hvq_lhefwritev(6)
      end

c...lhefeader(nlf)
c...writes event information to a les houches events file on unit nlf. 
      subroutine hvq_lhefwritev(nlf)
      implicit none
      integer nlf
      include 'src/Lib/hvq/LesHouches.h'
      include 'src/Lib/hvq/pwhg_flg.h'
     ...
     & vtimup(i),spinup(i)
 200  continue
      if(flg_pdfreweight) call hvq_lhefwritepdfrw(nlf)
      if(flg_debug) call hvq_lhefwritextra(nlf)
      write(nlf,'(a)')'</event>'      
...
c...writes trailer to a les houches events file on unit nlf. 
      subroutine hvq_lhefwritetrailer(nlf)
      implicit none
      integer nlf,iran,n1ran,n2ran
      write(nlf,'(a)') '</LesHouchesEvents>'
c     save last hvq_random number
      call hvq_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) '#hvq_random number generator exit values: ',
     # iran,' ',n1ran,' ',n2ran
      end

      subroutine hvq_lhefwritextra(nlf)
      implicit none
      include 'src/Lib/hvq/LesHouches.h'
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      ...
      call hvq_getcurrentrandom(iran,n1ran,n2ran)
      write(nlf,*) '# ', iran,' ',n1ran,' ',n2ran,
     #     "    previous event's hvq_random seeds "
      write(nlf,'(a)') '# End extra-info-previous-event'
      end

      subroutine hvq_lhefwritepdfrw(nlf)
      implicit none
      integer nlf
      integer id1,id2
      real * 8 x1,x2,xf1,xf2,xmufact
      call hvq_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)

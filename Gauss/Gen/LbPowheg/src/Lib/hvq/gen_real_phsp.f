...
c hvq_gen_real_phsp_fsr: provides the mapping for the final state
...
c hvq_gen_real_phsp_isr: mapping for the initial state radiation
      subroutine hvq_gen_real_phsp_fsr(xrad,
...
c remember: no csimax in the jacobian factor, we are integrating in csitilde 
     ...
     #                *(1-kn_csi/2*q0/kn_cmpborn(0,kn_emitter))
      ...
      subroutine hvq_gen_real_phsp_fsr_rad0
c     Same as hvq_gen_real_phsp_fsr_rad, but for given kn_csitilde
c     instead of kn_csi.
...
c hvq_gen_real_phsp_fsr_rad: provides the mapping for the final state
...
c the underlying Born jacobian is the phase space volume
      subroutine hvq_gen_real_phsp_fsr_rad
      implicit none
      include 'src/Lib/hvq/pwhg_math.h'
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
...
c remember: no csimax factor, we are integrating in csitilde 
      call hvq_barradmap(nlegborn-2,kn_emitter-2,q0,kn_cmpborn(0,3),
     #    kn_csi,kn_y,kn_azi,kn_preal(0,3),kn_jacreal)
      beta=(kn_xb1-kn_xb2)/(kn_xb1+kn_xb2)
      call hvq_mboost(nlegreal-2,vec,beta,kn_preal(0,3),kn_preal(0,3))
      do i=0,3
      ...
      kn_x2=kn_xb2
      kn_sreal=kn_sborn
      call hvq_checkmomzero(nlegreal,kn_preal)
      call hvq_compcmkin
      call hvq_compdij
      call hvq_setsoftvecfsr
      call hvq_compdijsoft
      end
...
c jac         : jacobian factor on phirad
      subroutine hvq_barradmap(n,j,q0,barredk,csi,y,phi,xk,jac)
      implicit none
c parameters
      include 'src/Lib/hvq/pwhg_math.h'
      integer n,j
...
c Rotate k_n+1 around vec of an amount psi
      call hvq_mrotate(vec,sqrt(abs(1-cpsi**2)),cpsi,xk(1,n+1))
c Rotate k_j around vec of an amount psi1 in opposite direction
      call hvq_mrotate(vec,-sqrt(1-cpsi1**2),cpsi1,xk(1,j))
...
c Rotate k_j and k_n+1 around this vector of an amount phi
      call hvq_mrotate(vec,sin(phi),cos(phi),xk(1,n+1))
      call hvq_mrotate(vec,sin(phi),cos(phi),xk(1,j))
...
c     beta in the k direction (same as barred k_j)
      do i=1,3
         vec(i)=barredk(i,j)/ubkj
      enddo
      call hvq_mboost(j-1,vec,beta,barredk(0,1),xk(0,1))
      if(n-j.gt.0) call hvq_mboost(n-j,vec,beta,barredk(0,j+1),xk(0,j+1))
      k2=2*ukj*uknp1*(1-y)
...
c ISR:
      ...
      subroutine hvq_gen_real_phsp_isr(xrad,
...
c      call hvq_checkmomzero(nlegreal,kn_preal)
      end

      subroutine hvq_compcsimax
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
      real * 8 y,xb1,xb2
...
c     Same as hvq_gen_real_phsp_isr_rad, but for given kn_csitilde
c     instead of kn_csi.
      subroutine hvq_gen_real_phsp_isr_rad0
      implicit none
      include 'src/Lib/hvq/pwhg_math.h'
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      ...
      subroutine hvq_gen_real_phsp_isr_rad
      implicit none
      include 'src/Lib/hvq/pwhg_math.h'
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
...
c also when hvq_gen_real_phsp_isr_rad is called directly
c (i.e. not through hvq_gen_real_phsp_isr_rad0)
      call hvq_compcsimax
      y=kn_y
      xb1=kn_xb1
      xb2=kn_xb2
...
c longitudinal boost of underlying Born to zero rapidity frame
     ...
     #          /(kn_pborn(0,1)+kn_pborn(0,2))
      ...
      do i=1,3
         vecl(i)=vecl(i)/betal
      enddo
      call hvq_mboost(nlegborn-2,vecl,-betal,kn_pborn(0,3),kn_preal(0,3))
...
c      write(*,*) ' k+1: ',(kn_preal(mu,nlegreal),mu=0,3)
      call hvq_mboost(nlegborn-2,vect,-betat,kn_preal(0,3),kn_preal(0,3))
c      call printtot(nlegborn,kn_preal(0,1))
c longitudinal boost in opposite direction
      call hvq_mboost(nlegborn-2,vecl,betal,kn_preal(0,3),kn_preal(0,3))
c      call printtot(nlegreal,kn_preal(0,1))
      kn_jacreal=kn_sreal/(4*pi)**3*csi/(1-csi)
      call hvq_compcmkin
      call hvq_compdij
      call hvq_setsoftvecisr
      call hvq_compdijsoft
      end


      subroutine hvq_compcmkin
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
      ...
      call hvq_mboost(nlegreal,vecl,betal,kn_preal,kn_cmpreal)
      end

      subroutine hvq_compdij
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
      ...
      integer j,k
      real * 8 y
      real * 8 hvq_crossp,hvq_dotp
      external hvq_crossp,hvq_dotp
      do j=flst_lightpart,nlegreal
         y=1-hvq_dotp(kn_cmpreal(0,1),kn_cmpreal(0,j))
     ...
     # *2*(1+y))**par_diexp
      ...
      do j=flst_lightpart,nlegreal
         do k=j+1,nlegreal
            kn_dijterm(j,k)=(2*hvq_dotp(kn_cmpreal(0,k),kn_cmpreal(0,j))*
...
c     4        (kn_cmpreal(3,k)+kn_cmpreal(3,j))**2))**par_dijexp
      ...
      if(par_diexp.ne.par_dijexp) then
         write(*,*)
     1   ' hvq_compdijsoft: if you have different par_diexp and par_dijexp'
         write(*,*) ' you better fix the hvq_soft subroutine too'
         stop
      endif
      y=1-hvq_dotp(kn_cmpborn(0,1),kn_softvec(0))
     ...
     #*2*(1+y))**par_diexp
      do k=flst_lightpart,nlegreal-1
         kn_dijterm_soft(k)=
     1   (2*hvq_dotp(kn_cmpborn(0,k),kn_softvec(0))*
     ...
     3     / kn_cmpborn(0,k)**2)**par_dijexp
      ...
      function hvq_crossp(a,b)
      implicit none
      real * 8 hvq_crossp,a(3),b(3)
      hvq_crossp=sqrt((a(1)*b(2)-a(2)*b(1))**2
     #           +(a(2)*b(3)-a(3)*b(2))**2
     #           +(a(3)*b(1)-a(1)*b(3))**2)
      end


      subroutine hvq_setsoftvecfsr
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'
      ...
      em=kn_emitter
      y=kn_y
c set hvq_soft vector parallel to the emitter
      do j=0,3
...
c Set up a unit vector orthogonal to p_em and to the z axis
      ...
      call hvq_mrotate(dir,sqrt(1-y**2),y,kn_softvec(1))
      do j=1,3
         dir(j)=kn_cmpborn(j,em)/kn_cmpborn(0,em)
      enddo
c Rotate kn_softvec around dir of an amount azi
      call hvq_mrotate(dir,sin(kn_azi),cos(kn_azi),kn_softvec(1))
      end

      subroutine hvq_setsoftvecisr
      implicit none
      include 'src/Lib/hvq/nlegborn.h'
      include 'src/Lib/hvq/pwhg_flst.h'
      include 'src/Lib/hvq/pwhg_kn.h'

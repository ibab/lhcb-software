      subroutine wj_genericpdf(ndns,ih,xmu2,x,fx)
c Interface to wj_MLMPDF package.
      implicit none
      integer ndns,ih
      ...
      sx=x
      sxmu2=xmu2
      call wj_MLMPDF(ndns,ih,sxmu2,sx,sfx,5)
      do j=-5,5
         fx(j)=sfx(j)
      enddo
      fx(1)=sfx(2)
...
c link errors. It is never invoked in the present case.
      subroutine wj_getq2min()
      end

      subroutine wj_genericpdfpar(ndns,ih,xlam,wj_scheme,iorder,iret)
      implicit none
      integer ndns,ih
      real * 8 xlam
      character * 2 wj_scheme
      integer iret,iorder
      call wj_PDFPAR(ndns,ih,xlam,wj_scheme,iret)
c not yet implemented
      iorder=-1
      end

      function wj_whichpdfpk()

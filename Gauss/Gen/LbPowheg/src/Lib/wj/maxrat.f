      subroutine wj_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 wj_powheginput,wj_random
      external wj_powheginput,wj_random      
      call wj_zerohistnorms
      loaded=.false.
      nubound=wj_powheginput('nubound')
      rad_nynorms=wj_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=wj_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=wj_powheginput('#xupbound')
      ...
      call wj_newunit(iun)
      if(wj_powheginput('use-old-ubound').eq.1) then
         call wj_newunit(iun)
...
c      call maxratident
      ...
      common/wj_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical wj_refuse_pdf
      real * 8 wj_pwhg_pt2,wj_pwhg_upperb_rad
      external wj_pwhg_pt2,wj_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         call wj_set_rad_scales(ptsq)
         if(.not.wj_refuse_pdf()) then
            ...
            if(born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' wj_inc_norms: Warning, born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' wj_inc_norms: x1,x2', kn_xb1,kn_xb2
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' wj_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' wj_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/born/wj_pwhg_upperb_rad()
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
     ...
     1              efficiencies(ireg,iborn)/wj_powheginput('nubound')/
...
c     1           wj_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine wj_zerohistnorms
      implicit none
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'
      include 'src/Lib/wj/pwhg_rad.h'
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call wj_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call wj_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine wj_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/wj/pwhg_math.h'
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'
...
c
      ...
      subroutine wj_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 wj_btilde,wj_powheginput
      external wj_btilde,wj_powheginput
...
c
     ...
     1       ' Born configurations according to the Born wj_cross section'
c set up the grids
         do j=1,ndimbornint
            do k=0,50
               xgrid(k,j)=0
...
c will not be used;
         call wj_resettotals
         call wj_mint(wj_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=wj_powheginput('ncall2')
         itmx2=wj_powheginput('itmx2')
         call wj_mint(wj_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call wj_gen(wj_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn't used the ifold array may need reset to 1 here.
      call wj_gen(wj_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

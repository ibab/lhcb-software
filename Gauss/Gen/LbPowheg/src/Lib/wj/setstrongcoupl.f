      subroutine wj_setscalesbtilde
      implicit none
      include 'src/Lib/wj/pwhg_math.h'
      include 'src/Lib/wj/pwhg_st.h'
      real * 8 wj_pwhg_alphas
      external wj_pwhg_alphas
      real * 8 muf,mur
      call wj_set_fac_ren_scales(muf,mur)
      st_mufact2= muf**2*st_facfact**2
      st_muren2 = mur**2*st_renfact**2
      st_alpha  = wj_pwhg_alphas(st_muren2,st_lambda5MSB,st_nlight)
      end

      subroutine wj_set_rad_scales(ptsq)
      implicit none
      real * 8 ptsq
      include 'src/Lib/wj/pwhg_math.h'
...
c     heavy quarks involved one should use their wj_masses as minimum value
c     of factorization wj_scale, as we make later on. This works if ptmin
c     is greater or equal to the mass of heavy quark 
        elseif( wj_whichpdfpk().eq.'mlm') then    
c ad hoc value here (wj_MLMPDF does not provide this)
           q2min=2d0
        else
...
c     should always cancel out in the ratio R/B. If the radiation wj_scale
...
c     To prevent this, we use a wj_scale higher than the heavy-flavour
...
c     mufact (50 is an arbitrary choice).
      ...
      st_muren2=ptsq
      st_alpha = wj_pwhg_alphas(st_muren2,st_lambda5MSB,-1)
     ...
     #   (1+st_alpha/(2*pi)*((67d0/18-pi**2/6)*ca-5d0/9*nf))
      ...
      include 'src/Lib/wj/pwhg_flst.h'
      include 'src/Lib/wj/pwhg_rad.h'
      real * 8 b0,mu0sq,as,wj_pwhg_alphas
      external wj_pwhg_alphas
      b0=(33-2*5)/(12*pi)
      mu0sq=(2*st_lambda5MSB)**2
c running value of alpha at initial wj_scale (see notes: running_coupling)
      as=wj_pwhg_alphas(mu0sq,st_lambda5MSB,-1)
c for better NLL accuracy (FNO2006, (4.32) and corresponding references)
      ...
      function wj_pwhg_alphas0(q2,xlam,inf)
      implicit none
      real * 8 wj_pwhg_alphas0,q2,xlam
      integer inf
      real * 8 pi
      parameter (pi=3.141592653589793d0)
      real * 8 b0
      b0=(33-2*inf)/(12*pi)
      wj_pwhg_alphas0=1/(b0*log(q2/xlam**2))
      end
      
C----------------------------------------------------------------------------
C-------------------------------------------------------------------
C------- ALPHA QCD -------------------------------------
c Program to calculate wj_alfa strong with nf flavours,
c as a function of lambda with 5 flavors.
c The value of wj_alfa is matched at the thresholds q = mq.
c When invoked with nf < 0 it chooses nf as the number of
c flavors with mass less then q.
c
      function wj_pwhg_alphas(q2,xlam,inf)
      implicit none
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'
      include 'src/Lib/wj/pwhg_rad.h'
      real * 8 wj_pwhg_alphas,q2,xlam
      integer inf
      real * 8 pi
      parameter (pi=3.141592653589793d0)
...
c      data ini/.true./
c      save ini
c      wj_pwhg_alphas = 30d0
c      if (ini) then
c         write(*,*) '****************************************'
c         write(*,*) '****************************************'
c         write(*,*) '      RETURN alpha_s = ',wj_pwhg_alphas
c         write(*,*) '****************************************'
c         write(*,*) '****************************************'
c         ini = .false.         
c      endif           
c      return
     ...
     #        - 1/( 1/(b3 * xlc) - xllc*bp3/(b3 * xlc)**2 ) + c45
      ...
      if    ( nf .eq. 5 ) then
        wj_pwhg_alphas = 1/(b5 * xlq) -  bp5/(b5 * xlq)**2 * xllq
      elseif( nf .eq. 4 ) then
        wj_pwhg_alphas =
     ...
     #    1/( 1/(1/(b3 * xlq) - bp3/(b3 * xlq)**2 * xllq) + c35 )
        ...
        print *,'error in wj_alfa: unimplemented # of light flavours',nf
        call exit(1)
      endif
      return
      end

...
c         res(j)=kn_jacborn/(2*kn_sborn)/flst_nborn
      ...
      subroutine wj_sigborn_rad(born)
      implicit none
      real * 8 born
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'
     ...
     #         bmunu(0:3,0:3,nlegborn)
      call wj_pdfcall(1,kn_xb1,pdf1)
      call wj_pdfcall(2,kn_xb2,pdf2)
      call wj_setborn0(kn_cmpborn,flst_born(1,rad_ubornidx),born,
     #        bornjk,bmunu)
      born=born *
     #  pdf1(flst_born(1,rad_ubornidx))*pdf2(flst_born(2,rad_ubornidx))
      ...
      if(ini) then
         ...
         if(flg_smartsig) then
            call wj_randomsave
            call wj_fillmomenta(nlegborn,nmomset,kn_masses,pborn)
            do iborn=1,flst_nborn
               do j=1,nmomset
                  call wj_setborn0(pborn(0,1,j),flst_born(1,iborn),
     1                 born(j,iborn),bornjk(1,1,j,iborn),
     2                 bmunu(0,0,1,j,iborn))
               enddo
               call wj_compare_vecsb(nmomset,iborn,born,bornjk,bmunu,
     1              ibornpr,cprop,iret)
      ...
      do iborn=1,flst_nborn
         if(equivto(iborn).lt.0) then
            call wj_setborn0(kn_cmpborn,flst_born(1,iborn),br_born(iborn),
     ...
     #                   *equivcoef(iborn)
      ...
      subroutine wj_compare_vecsb(nmomset,iborn,born,bornjk,bmunu,
     1     ibornpr,cprop,iret)
      implicit none
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'
...
c some of these are zero, be careful
 ...
 10      continue
      ...
      subroutine wj_setborn0(p,bflav,born,bornjk,bmunu)
c provide the flux factor to the user Born routine
      ...
      call wj_setborn(p,bflav,born,bornjk,bmunu)
c     check if born, bornjk and bmunu are finite
      if (.not.wj_pwhg_isfinite(born)) born=0d0
      do j=1,nlegs
         do mu=0,3
            do nu=0,3
               if (.not.wj_pwhg_isfinite(bmunu(mu,nu,j))) 
     $               bmunu(mu,nu,j)=0d0
            enddo
         enddo
      enddo
      do j=1,nlegs
         do k=1,nlegs
            if (.not.wj_pwhg_isfinite(bornjk(j,k))) bornjk(j,k)=0d0
         enddo
      enddo
c

...
C Call wj_PWHGBOOK(N,'TITLE',DEL,XMIN,XMAX) for each histogram to be booked.
...
C Example:
C      call wj_PWHGBOOK(2,'pt distribution',1.,10,70)
...
C
C FILLING:
C When it's time, call wj_PWHGFILL(N,X,Y); this will add Y (real*4) to the bin 
C in which X (real*4) happens to be, within histogram N. 
C
C PLAYING AROUND:
C At the end of the day you may want to sum, divide, cancel, etc.etc.
C various histograms (bin by bin). Then you call wj_PWHGOPERA(I,'O',J,K,X,Y). 
...
C M  : statistical wj_analysis; if I contains the weights (let's say WGT),
...
C
C FINAL ACCOUNTING:
C Now we can finalize our histograms; wj_PWHGFINAL(N) will calculate the integral
C of the histogram N, the mean value of the X variable and its RMS.
C If we now want to renormalize the hist's, we can call wj_PWHGNORM(N,X), which
C will normalize the integral to X  -- CAUTION: do not call wj_PWHGNORM before
C wj_PWHGFINAL, it will blow up.
C
C OUTPUT:
C To get a .dat file containing the values of the histograms, together with
C some information (like integral, mean values, etc.etc.) call wj_PWHGPRINT(N),
C for each hist N that you want in the .dat file. Before the call to wj_PWHGPRINT
...
C wj_PWHGTOP(N,M,'X','Y','wj_scale'). The points of the plot will be taken from histogram
C N, the error bars from histogram M. 'wj_scale', character*(*), determines
C the wj_scale for y, logarithmic or linear (wj_scale=LOG,LIN).
...
C itself. Before calling wj_PWHGTOP,
C     OPEN(UNIT=99,NAME='NAME.TOP',STATUS='NEW')
C--------------------------------------------------------------------------
C
C  COMMON/wj_histo/  Histogram N
C                           
...
C   HAVG(N),      Average value of x, weighted over the x range of the wj_histo
...
C
      ...
      SUBROUTINE wj_pwhgINIHIST
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      integer i
      DO 1, I=1,NMB             
   1  BOOK(I)=' NO'
      DO 2 I=1,10
   2  BOOK2(I)=' NO'
      call wj_pwhgresetmultitop
      END  


      subroutine wj_pwhggettitle(N,TIT)
      implicit none
      character *(*) TIT
      integer n
      include 'src/Lib/wj/pwhg_book.h'
      ...
      SUBROUTINE wj_PWHGBOOK(N,TIT,DEL,XMIN,XMAX)
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      integer n,i
      ...
      DATA ONEP/1.00001d0/
      IF(N.GT.NMB) THEN
	 CALL wj_pwhgwarn('wj_PWHGBOOK')
         WRITE(*,*) 'wj_PWHGBOOK: no more then ',NMB,' histograms'
         STOP
      ENDIF
      NHIST = MAX(N,NHIST)
...
C
      ...
      SUBROUTINE wj_PWHGINTEG(NIN,NOUT,IDIR,IPOW)
...
C of a given distribution. Overscores and underscores are included.
      ...
      I = (IDIR + 3)/2
      IF(NOUT.NE.NIN) THEN
      	CALL wj_PWHGBOOK(NOUT,TITLE(NIN)//C(I), 
     &                HDEL(NIN),HMIN(NIN),HMAX(NIN))
      ENDIF
      IF(IDIR.EQ.1) THEN
         HIST(NOUT,1) = wj_PWHGSUMPOW(HIST(NIN,1),USCORE(NIN),IPOW)
         IHIS(NOUT,1) = IHIS(NIN,1) + IUSCORE(NIN)
         XHIS(NOUT,1) = XHIS(NIN,1) + HDEL(NIN)/2
         DO L=2,M                      
            HIST(NOUT,L) = wj_PWHGSUMPOW(HIST(NIN,L),HIST(NOUT,L-1),IPOW)
            IHIS(NOUT,L) = IHIS(NIN,L) + IHIS(NOUT,L-1) 
            XHIS(NOUT,L) = XHIS(NIN,L) + HDEL(NIN)/2
         ENDDO
         OSCORE(NOUT) = wj_PWHGSUMPOW(OSCORE(NIN),HIST(NIN,M),IPOW)
         IOSCORE(NOUT) = IOSCORE(NIN) + IHIS(NIN,M)
      ELSEIF(IDIR.EQ.-1) THEN
         HIST(NOUT,M) = wj_PWHGSUMPOW(HIST(NIN,M),OSCORE(NIN),IPOW)
         ...
         DO L=M-1,1,-1                        
            ...
            XHIS(NOUT,L) = XHIS(NIN,L) - HDEL(NIN)/2
         ENDDO
         USCORE(NOUT) = wj_PWHGSUMPOW(USCORE(NIN),HIST(NIN,1),IPOW)
         IUSCORE(NOUT) = IUSCORE(NIN)+IHIS(NIN,1)
      ELSE                                 
         CALL wj_pwhgwarn('wj_PWHGINTEG')
         WRITE(*,*) 'Wrong idir in wj_PWHGINTEG: OPERATION NOT PERFORMED'
         STOP
      ENDIF
      END

      FUNCTION wj_PWHGSUMPOW(X,Y,IPOW)
      implicit none
      real * 8 x,y,wj_PWHGSUMPOW
      integer ipow
      IF(IPOW.EQ.1) THEN
         wj_PWHGSUMPOW = X + Y
      ELSEIF(IPOW.EQ.2) THEN
         wj_PWHGSUMPOW = SQRT(X**2+Y**2)
      ELSEIF(IPOW.EQ.0) THEN
         CALL wj_pwhgwarn('wj_PWHGSUMPOW')
         WRITE(*,*)'Error: IPOW=0 not allowed in wj_PWHGSUMPOW'
         STOP
      ELSE
         wj_PWHGSUMPOW = (X**IPOW+Y**IPOW)**(1./IPOW)
      ENDIF
      END

      SUBROUTINE wj_PWHGOPERA(I,OPER,J,K,X,Y)
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      integer i,j,k,l
...
c the average value of I to K and will put in J the quadratic dispersion.
   ...
   1  HIST(N,I)=0
      ...
      SUBROUTINE wj_PWHGPUTTAG(J,NAME)
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      integer j
c Per marcare un istogramma
      ...
      BOOK(J) = NAME
      RETURN
      ENTRY wj_pwhgGETTAG(J,TAG)
      TAG = BOOK(J)
      END

      SUBROUTINE wj_PWHGFINAL(N)
      implicit none
      integer n
...
c      CALL TIME(CTIME)
   ...
   20 FORMAT(' ENTRIES=',I5,4X,'UNDERSCORE=',I5,4x,'OVERSCORE=',I5,//)
      ...
      SUBROUTINE wj_PWHGTOP(N,M,BTIT,LTIT,wj_scale)
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      integer  imon,iday,iyear,ini,n,m,j,itime(3)
      CHARACTER CTIME*8
      CHARACTER*(*) LTIT,BTIT,wj_scale
      save imon,iday,iyear,ctime,ini
      DATA INI/0/
      data ctime/'        '/
      IF(INI.EQ.0) THEN
...
c      CALL TIME(CTIME)
      ...
      ENDIF
      IF(BOOK(N).NE.'YES') RETURN
      WRITE(99,100) TITLE(N),BTIT,LTIT,wj_scale,HMIN(N),HMAX(N)
  100 FORMAT( /1x,                               
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE wj_PWHGMNRTOP(N,M,BTIT,LTIT,wj_scale)
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      integer imon,iday,iyear,ini,n,m,j,itime(3)
      CHARACTER CTIME*8
...
c         CALL TIME(CTIME)
      ...
      ENDIF
      CALL wj_PWHGFINAL(N)
      WRITE(99,100) TITLE(N)(1:20),BTIT,LTIT,wj_scale,HMIN(N),HMAX(N)
  100 FORMAT( /1x,                               
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE wj_PWHGBOOK2(N,TIT,DELX,XMIN,XMAX,DELY,YMIN,YMAX)
...
C-- setup y-boundaries
   ...
   3  CONTINUE
      ...
      SUBROUTINE wj_PWHGFILL2(N,X,Y,WGT)
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/wj/pwhg_book.h'
C-- find x-coordinate
...
C-- find y-coordinate
     ...
     *                                             THEN        
      ...
      SUBROUTINE wj_PWHGOPERA2(I,OPER,J,K,X,Y)
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/wj/pwhg_book.h'
      CHARACTER OPER*1
     ...
     &'*'.OR.OPER.EQ.'/'.OR.OPER.EQ.'M')) GO TO 10
      DO wj_L1=1,NBIN2(1,I)
      DO wj_L2=1,NBIN2(2,I)
      IF(OPER.EQ.'+') THEN
      HIST2(K,wj_L1,wj_L2)=X*HIST2(I,wj_L1,wj_L2) + Y*HIST2(J,wj_L1,wj_L2)
      ELSEIF(OPER.EQ.'-') THEN
      HIST2(K,wj_L1,wj_L2)=X*HIST2(I,wj_L1,wj_L2) - Y*HIST2(J,wj_L1,wj_L2)
      ELSEIF(OPER.EQ.'*') THEN
      HIST2(K,wj_L1,wj_L2)=X*HIST2(I,wj_L1,wj_L2) * Y*HIST2(J,wj_L1,wj_L2)
      ELSEIF(OPER.EQ.'/') THEN
        IF(Y.EQ.0..OR.HIST2(J,wj_L1,wj_L2).EQ.0.) THEN
          HIST2(K,wj_L1,wj_L2)=0.
          ELSE
          HIST2(K,wj_L1,wj_L2)=X*HIST2(I,wj_L1,wj_L2) / (Y*HIST2(J,wj_L1,wj_L2))
        ENDIF
      ELSEIF(OPER.EQ.'F') THEN
      HIST2(K,wj_L1,wj_L2)=X*HIST2(I,wj_L1,wj_L2)
      ELSEIF(OPER.EQ.'R') THEN
        IF(HIST2(I,wj_L1,wj_L2).GT.0.) THEN
        HIST2(K,wj_L1,wj_L2)=X*SQRT(HIST2(I,wj_L1,wj_L2))
        ELSE
        HIST2(K,wj_L1,wj_L2)=0.
        ENDIF
      ELSEIF(OPER.EQ.'S') THEN
      HIST2(K,wj_L1,wj_L2)=X*HIST2(I,wj_L1,wj_L2)**2
      ELSEIF(OPER.EQ.'L') THEN
        IF(HIST2(I,wj_L1,wj_L2).EQ.0..OR.J.EQ.0.) THEN
             HIST2(K,wj_L1,wj_L2)=0.
             ELSE
             HIST2(K,wj_L1,wj_L2)=X*LOG10(Y*HIST2(I,wj_L1,wj_L2))
        ENDIF
      ELSE
      WRITE(98,5) OPER
   5  FORMAT(' ****** OPERATION ="',A1,'" UNKNOWN ********'/)
...
C-- projection on the y-axis
  ...
  10  BOOK2(N)=' NO'
      ...
      SUBROUTINE wj_PWHGPROHIS(N,M,IAX)
...
c$$$      include 'src/Lib/wj/pwhg_book.h'
c$$$      CHARACTER CTIME*8
...
c$$$      DO ND=1,NDOTS
c$$$      X=XHIS2(N,I)+(2.*wj_random()-1.)*HDEL2(1,N)
c$$$      Y=YHIS2(N,J)+(2.*wj_random()-1.)*HDEL2(2,N)
c$$$      WRITE(99,*) X,Y
c$$$      ENDDO
c$$$      NTOT=NTOT+NDOTS
c$$$  3   CONTINUE       
...
c$$$      END
      ...
      SUBROUTINE wj_PWHGMULTITOP(NH,NE,N,M,BTIT,LTIT,SCA,INI)
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      CHARACTER CTIME*8,wj_scale*3
...
20    CONTINUE
	...
	XMX=MAX(XMX,ABS(X))
      ...
      wj_scale=SCA
50    IF(wj_scale.EQ.'LIN') THEN
	IF(FMN.GE.0.)	FMIN=0.
	IF(FMN.LT.0.)	FMIN=FMN*1.3
	IF(FMX.GT.0.)	FMAX=FMX*1.3
	IF(FMX.LT.0.)	FMAX=0.
      ELSEIF(wj_scale.EQ.'LOG') THEN 
        IF(FMN.LE.0.) THEN
	     	wj_scale='LIN'
		GO TO 50
	ENDIF
...
106   FORMAT(2X,'SET ORD X Y ',/,2(F8.4,1X),/,2(F8.4,1X),/,
     ...
     *       2X,'JOIN TEXT')                                    
      WRITE(99,107) BTIT,LTIT,wj_scale,HMIN(NH),HMAX(NH),FMIN,FMAX
107   FORMAT(                                           
     &' TITLE BOTTOM ','"',A,'"',/1X,
     &' TITLE LEFT ','"',A,'"',/1X,
     &' SET wj_scale Y ',A,/1X,
...
C
      ENTRY wj_PWHGTFILL(NH,NE,N,M,BTIT,LTIT,SCA)
      IP=IP+1                             
      IF(IP.GT.4) IP=1
...
202   FORMAT('   NEW PLOT',/,/)
203   RETURN     
      entry wj_pwhgresetmultitop
      iframe=0
      END                      

      SUBROUTINE wj_PWHGNEWPLOT
      WRITE(99,202) 
202   FORMAT('   NEW PLOT',/,/)
      CALL wj_PWHGMULTITOP(iDUM,iDUM,iDUM,iDUM,' ',' ','REF',0)
      END


c  added by SA and ER to use with MC@NLO
      SUBROUTINE wj_PWHGCOPY(NIN,NOUT)
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      integer nin,nout,i
...
C This may happen if wj_PWHGFINAL is called prior to wj_PWHGCOPY
      IF(BOOK(NIN)(1:1).EQ.'R'.AND.HMIN(NIN).EQ.HMAX(NIN)) RETURN
      IF(BOOK(NOUT)(1:1).NE.'Y'.AND.BOOK(NOUT)(1:1).NE.'R') 
     &  CALL wj_PWHGBOOK(NOUT,TITLE(NIN)(2:),HDEL(NIN),HMIN(NIN),HMAX(NIN))
      ...
      subroutine wj_pwhgbookup(n,string,wj_scale,del,xl,xu)
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      real * 8 del,xl,xu
      integer n
      character * (*) string
      character * 3 wj_scale
...
c     A call to wj_pwhgaccumup causes
c     the histogram n to be summed to the n+nmh, its square is
c     summed into n+nmh2. and n is zeroed.
c
c     When the subroutine wj_pwhgaddout is called, the statistical
c     wj_analysis of the n+nmh and n+nmh2 content is performed, the
c     average (with respect to the number of calls to wj_pwhgaccumup)
c     is added to the n+nmh3, and the error is summed in quadrature
c     to the n+nmh4; n+nmh and n+nmh2 are zeroed.
c
c     If instead wj_pwhgsetout is called, the statistical
c     wj_analysis of the n+nmh and n+nmh2 content is performed, the
c     average (with respect to the number of calls to wj_pwhgaccumup)
...
c.
      ...
      call wj_PWHGBOOK(n,  string,del,xl,xu)
      call wj_PWHGBOOK(n+nmh,'tmp ',del,xl,xu)
      call wj_PWHGBOOK(n+nmh2,'tmp square',del,xl,xu)
c this is the histogram that will be output
      call wj_PWHGBOOK(n+nmh3,string,del,xl,xu)
c the wj_scale for plotting (log or lin) is encoded in the title
      call wj_PWHGBOOK(n+nmh4,wj_scale//'error ',del,xl,xu)
      call wj_PWHGPUTTAG(n,'YST')
      return
      end

      

      subroutine wj_pwhgaccumup
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      integer j
      character * 3 tag
...
c     statistical wj_analysis, then empty the original histogram j.
c
      do j=1,nmh
         call wj_pwhgGETTAG(j,tag)
         if(tag.eq.'YST') then
            do l=1,nbin(j)
...
c
      do j=1,nmb
c     finish statistical wj_analysis
         call wj_pwhgGETTAG(j,tag)
         if(tag.eq.'YST') then
...
c     for the mean value is the number of calls to wj_pwhgaccumup, which in
...
c     histogram, as filled by pwhgaccumup.
     ...
     1             + xxx**2*(abs(xsumsq-xsum**2*xxx)))
      ...
      subroutine wj_pwhgtopout
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      character * 50 title0,wj_scale
      character * 3 tag
      integer i,ini
      ini=0
      do i=1,nmb
         call wj_pwhgGETTAG(i,tag)
         if(tag.eq.'YST') then
            call wj_PWHGFINAL(i+nmh3)
            call wj_PWHGFINAL(i+nmh4)
            call wj_pwhggettitle(i,title0)
            call wj_pwhggettitle(i+nmh4,wj_scale)
            call wj_PWHGMULTITOP(i+nmh3,i+nmh4,2,3,title0,' ',wj_scale(2:4)
     $           ,ini)
         endif
      enddo
      end            
      ...
      subroutine wj_pwhgrescale(wj_scale)
C********************************************************************
C The following function rescale the content of the histogram by the
C wj_scale factor.  IMPORTANT: must be called AFTER the statistical
C wj_analysis has been performed and before histograms are written to the
C TOPDRAWER file. For example : 
C    call wj_pwhgsetout ( or wj_pwhgaddout)
C    call wj_pwhgrescale(wj_scale)
C    call wj_pwhgtopout 
C**********************************************************************
      implicit none
      include 'src/Lib/wj/pwhg_book.h'
      real * 8 wj_scale,dummy_real
      integer i,dummy_int
      character * 3 tag
      do i=1,nmb
         call wj_pwhgGETTAG(i,tag)
         if(tag.eq.'YST') then
C wj_PWHGOPERA(I,'F',J,K,X,Y) multiplies hist I by the factor X, and puts
C the result in hist K;
            call wj_PWHGOPERA(i+nmh3,'F',dummy_int,i+nmh3,wj_scale,dummy_real)
C i+nmh3 contains the values 
            call wj_PWHGOPERA(i+nmh4,'F',dummy_int,i+nmh4,wj_scale,dummy_real)

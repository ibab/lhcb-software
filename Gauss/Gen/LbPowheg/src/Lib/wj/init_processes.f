...
c    ID of vector boson produced
      idvecbos=wj_powheginput('idvecbos')
c   decay products of the vector boson
      tmp=wj_powheginput('vdecaymode')
...
c     Check that an inequivalent configuration is generated
                     if(wj_flavequiv(nlegreal,flst_real(1,j),ii(1)))
     $                    goto 11
                  enddo
 ...
 11               continue
      ...
      return
 998  write(*,*) 'wj_init_processes: increase maxprocborn'
 999  write(*,*) 'wj_init_processes: increase maxprocreal'
      stop
      end

      ...
      integer iret,iun
      real * 8 suppfact
      real * 8 wj_random,wj_powheginput
      external wj_random,wj_powheginput
      integer mcalls,icalls
      real * 8 wj_pwhg_pt2,wj_pt2max_regular
      external wj_pwhg_pt2,wj_pt2max_regular
      integer i
      if(idwtup.eq.3) then
         xwgtup=1
...
c     store current wj_random seeds. To be used to restart at problematic events
      call wj_savecurrentrandom
      if(wj_random().gt.rad_totrm/rad_totgen) then
c     generate underlying Born kinematics
         call wj_gen_btilde(mcalls,icalls)
c     generate underlying Born flavour
         call wj_gen_uborn_idx
c
         if(wj_powheginput('#wj_testsuda').eq.1) then
            call wj_testsuda
         endif
c generate radiation
         call wj_gen_radiation
c add a wj_random azimuthal rotation around beam axis
         call wj_add_azimuth
c --- set up les houches interface
         rad_pt2max=wj_pwhg_pt2()
         call wj_gen_leshouches
c if negative weight, flip the sign of xwgtup
         if(rad_btilde_sign(rad_ubornidx).eq.-1) then
            xwgtup=-xwgtup
         endif
c rad_type=1 for wj_btilde events (used only for debugging purposes)
         rad_type=1
...
c generate remnant n+1 body wj_cross section
         call wj_gen_sigremnant
c pick a configuration according to its wj_cross section
...
c iret=2: reg contribution (real graphs without singular regions)
         call wj_gen_remnant(iret)
c         if (wj_pwhg_pt2().lt.rad_ptsqmin) then
c            write(*,*) '****************************************'
c            write(*,*) 'WARNING in wj_gen_remnant'
c            write(*,*) 'wj_pwhg_pt2 < rad_ptsqmin ',
c     #           wj_pwhg_pt2(),' < ',rad_ptsqmin
c            write(*,*) (flst_alr(i,rad_realalr),i=1,nlegreal)
c            write(*,*) 'To generate this event, use the following seeds'
c            call wj_printcurrentrandom
c            write(*,*) '****************************************'
c         endif
...
c     set st_muren2 equal to pt2 for scalup value
            rad_pt2max=max(rad_ptsqmin,wj_pwhg_pt2())
            call wj_set_rad_scales(rad_pt2max)
            call wj_gen_leshouches
            call wj_born_suppression(suppfact)
            if(suppfact.eq.0) then
               write(*,*) ' 0 suppression factor in event generation'
...
c     set st_muren2 for scalup value for regular contributions
            rad_pt2max=max(rad_ptsqmin,wj_pt2max_regular())
            call wj_set_rad_scales(rad_pt2max)
            call wj_gen_leshouches_reg
c rad_type=3 for regular contributions
      ...
      real * 8 tmax
      common/wj_ctmax/tmax
      integer kinreg,firstreg,lastreg
      logical ini
      data ini/.true./
      real * 8 wj_pwhg_pt2,wj_powheginput
      external wj_pwhg_pt2,wj_powheginput
      save ini,firstreg,lastreg
      if(ini) then
         firstreg=wj_powheginput("#radregion")
...
c     initial state radiation
               call wj_gen_rad_isr(t)
            else
c     final state radiation
               call wj_gen_rad_fsr(t)
            endif
            if(t.gt.tmax) then
               tmax=t
               kinreg=rad_kinreg
...
c Generate a Born like event
         ...
         t=tmax
         if(rad_kinreg.eq.1) then
            call wj_gen_real_phsp_isr_rad
         else
            call wj_gen_real_phsp_fsr_rad
         endif
         call wj_set_rad_scales(t)
         call wj_sigreal_rad(sig)
         call wj_gen_real_idx
      endif
      end



      function wj_pwhg_pt2()
      implicit none
      real * 8 wj_pwhg_pt2
      include 'src/Lib/wj/pwhg_math.h'
      include 'src/Lib/wj/nlegborn.h'
      ...
      include 'src/Lib/wj/pwhg_rad.h'
      if(rad_kinreg.eq.1) then
         wj_pwhg_pt2=(kn_sreal/4)*(1-kn_y**2)*kn_csi**2
      else
         wj_pwhg_pt2=(kn_sreal/2)*(1-kn_y)*kn_csi**2
      endif
      end



      function wj_pwhg_upperb_rad()
      implicit none
      real * 8 wj_pwhg_upperb_rad
      include 'src/Lib/wj/pwhg_math.h'
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'
      include 'src/Lib/wj/pwhg_kn.h'
      ...
      if(rad_kinreg.eq.1) then
         if(rad_iupperisr.eq.1) then
            wj_pwhg_upperb_rad = st_alpha/((1-x)*(1-y**2))
c Possible alternatives:
c rad_iupper=2   wj_pwhg_upperb_rad = st_alpha/(x*(1-x)*(1-y**2))
c 
c rad_iupper=3:  wj_pwhg_upperb_rad = st_alpha/(x**2*(1-x)*(1-y**2))
         else
...
c for now use the same
         if(rad_iupperfsr.eq.1) then
            wj_pwhg_upperb_rad = st_alpha/(csi*(1-y))
         elseif(rad_iupperfsr.eq.2) then
            wj_pwhg_upperb_rad = st_alpha/(csi**2*(1-y)*(1-csi/2*(1-y))**2)
     2          *csi
...
c We use it to find its zero in pt2.
      ...
      include 'src/Lib/wj/pwhg_rad.h'
      include 'src/Lib/wj/pwhg_math.h'
      real * 8 wj_pt2solve,pt2
c i set by wj_DZERO: 1 for first call, 2 for subsequent calls, 3 for last call
c before a normal exit; not used here
      integer i
      real * 8 xlr,q2,xlam2c,kt2max,cunorm
      integer nlc
      common/wj_cpt2solve/xlr,q2,kt2max,xlam2c,cunorm,nlc
...
c see Notes/upperbounding-isr.pdf
     ...
     #           + xlr
               ...
               wj_pt2solve=cunorm*pi/b0*(log(2d0)
...
c final state radiation
     ...
     2   /(p*xm)/2.d+0) + xlr
      ...
      subroutine wj_gen_rad_isr(t)
c Generates hard radiation kinematics according to
c appendix D in ZZ paper.
c
c  common/wj_cptmin/ptminsq: minimum pt^2 accepted
c 
      implicit none
      include 'src/Lib/wj/pwhg_math.h'
      include 'src/Lib/wj/nlegborn.h'
      ...
      integer nlc
      common/wj_cpt2solve/xlr,q2,kt2max,xlam2c,unorm,nlc
      real * 8 xmin,rv,xp,xm,chi,tk,uk,ubound,ufct,
     #   sborn,value,err,tmp1,tmp2,tmp,rvalue,born,sig
      common/wj_cdfxmin/xmin
      real * 8 tmax
      common/wj_ctmax/tmax
      real * 8 wj_random,wj_pt2solve,dfxmin,wj_pwhg_alphas0,wj_pwhg_upperb_rad
      external wj_random,wj_pt2solve,dfxmin,wj_pwhg_alphas0,wj_pwhg_upperb_rad
...
c see section 4 in ZZ paper, last paragraph
      ...
      xlr=0
 1    continue
      xlr=xlr+log(wj_random())
c CERNLIB voodoo:
      call wj_KERSET('C205. ',0,0,101)
c solve for zero of wj_pt2solve
c wj_DZERO(xmin,xmax,x,err,eps,maxcalls,function)
...
c eps: required accuracy
      call wj_DZERO(rad_ptsqmin,kt2max,t,err,1d-8,1000000,wj_pt2solve)
c error conditions
      if(t.eq.0.and.err.lt.0d0
     # .and.err.gt.rad_ptsqmin-kt2max) then
         write(*,*) 'wj_DZERO fails'
...
c to set xmuren2:
      call wj_set_rad_scales(t)
      tmp2=st_alpha / wj_pwhg_alphas0(t,rad_lamll,nlc)
      tmp=tmp1*tmp2
      if(tmp.gt.1) then
...
c using       d sqrt(xm-x) /sqrt(xp-xm) (rad_iupper=1) or d sqrt(xm-x) /(xmin sqrt(xp-xm)) (rad_iupper=2) as upper bound using hit and miss
 2    chi=sqrt(xm-xmin)*wj_random()
      x=xm-chi**2
      if(rad_iupperisr.eq.1) then
         if(wj_random().gt.sqrt(xp-xm)/sqrt(xp-x)) goto 2
      elseif(rad_iupperisr.eq.2) then
         if(wj_random().gt.(xmin*sqrt(xp-xm))/(x*sqrt(xp-x))) goto 2
      endif
c get y (abs to avoid tiny negative values)
      y=sqrt(abs(1-4*x/(1-x)**2*t/sborn))
      if(wj_random().gt.0.5d0) y=-y
...
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call wj_uboundfct(ufct,1-x,y)
      if(wj_random().gt.ufct) goto 1
c Veto from upper bound to real value. Count how many vetoes,
c since these may be expensive.
      call wj_sigborn_rad(born)
      if(born.lt.0) then
         born=0
      endif
      if(born.eq.0) then
c bizarre situation that may arise when the wj_scale gets so low
c that some pdf vanish (typically heavy flavour pdf's)
      ...
      kn_y=y
      kn_csi=1-x
      kn_azi=2*pi*wj_random()
      ubound=born*wj_pwhg_upperb_rad()*unorm*ufct
      call wj_gen_real_phsp_isr_rad
      call wj_sigreal_rad(sig)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      rvalue=wj_random()*ubound
      if(rvalue.gt.value) then
         call wj_increasecnt('vetoed radiation')
         goto 1
      endif
      end


      subroutine wj_gen_rad_fsr(t)
...
c 
     ...
     #   s,value,err,tmp,rvalue,born,sig
      ...
      real * 8 tmax
      common/wj_ctmax/tmax
      real * 8 wj_random,wj_pt2solve,wj_pwhg_alphas0,wj_pwhg_upperb_rad
      external wj_random,wj_pt2solve,wj_pwhg_alphas0,wj_pwhg_upperb_rad
      unorm=rad_norms(rad_kinreg,rad_ubornidx)
c kn_sborn=kn_sreal:
      s=kn_sborn
...
c vetoes:
      ...
      call wj_set_rad_scales(t)
      if(rad_iupperfsr.eq.1) then
         tmp=st_alpha / wj_pwhg_alphas0(t,rad_lamll,nlc)
      elseif(rad_iupperfsr.eq.2) then
         tmp=st_alpha
      elseif(rad_iupperfsr.eq.3) then
         tmp=st_alpha
...
c get y
         y=1-2*t/(s*csi**2)
         if(wj_random().gt.(csi-t/s)) goto 1
      else
         write(*,*) ' wj_gen_rad_fsr:  rad_iupper=',rad_iupperfsr,
     1        ' invalid'
      endif
c
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call wj_uboundfct(ufct,csi,y)
      if(wj_random().gt.ufct) goto 1
c Veto from upper bound to real value. Count how many vetoes,
c since these may be expensive.
...
c that some pdf vanish (typically heavy flavour pdf's)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      real * 8 wj_random
      external wj_random
      azi=2d0*pi*wj_random()
      sazi = sin(azi)
      cazi = cos(azi)
      if (kn_csi.eq.0d0) then
         do ileg=1, nlegborn
            call wj_mrotate(dir,sazi,cazi,kn_pborn(1,ileg))
         enddo
      else
         do ileg=1, nlegreal
            call wj_mrotate(dir,sazi,cazi,kn_preal(1,ileg))
         enddo
      endif
      end

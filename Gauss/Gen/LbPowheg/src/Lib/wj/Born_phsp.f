...
c velocity of W in partonic CM
      ...
      vec(2)=0
      vec(3)=cth
      call wj_mboost(2,vec,beta,kn_cmpborn(0,3),kn_cmpborn(0,3))
...
c now boost everything along 3
      ...
      vec(2)=0
      vec(3)=1
      call wj_mboost(nlegborn-2,vec,beta,kn_cmpborn(0,3),kn_pborn(0,3))
      do mu=0,3
         kn_pborn(mu,1)=kn_xb1*kn_beams(mu,1)
         kn_pborn(mu,2)=kn_xb2*kn_beams(mu,2)
      enddo
c      call wj_checkmomzero(nlegborn,kn_pborn)
c      call wj_checkmass(2,kn_pborn(0,3))
...
c minimal final state mass 
      ...
      subroutine wj_born_suppression(fact)
      implicit none
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'
      include 'src/Lib/wj/pwhg_kn.h'
      logical ini
      data ini/.true./
      real * 8 fact,pt2,pt2supp,wj_powheginput,pt
      save ini,pt2supp,pt    
c CAVEAT!!!  process dependent subroutine
      if (ini) then
         pt = wj_powheginput("#ptsupp")
         if(pt.gt.0) then
            ...
            pt = wj_powheginput("#bornsuppfact")
         endif
         ini = .false.
         pt2supp = pt**2
      endif
      ...
      subroutine wj_set_fac_ren_scales(muf,mur)
      implicit none
      include 'src/Lib/wj/PhysPars.h'
      include 'src/Lib/wj/nlegborn.h'
      include 'src/Lib/wj/pwhg_flst.h'

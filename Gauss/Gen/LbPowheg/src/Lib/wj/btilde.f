      function wj_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 wj_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call wj_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call wj_btildevirt(resvirt)
            call wj_btildecoll(xrad,rescoll,www)
            call wj_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         wj_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            wj_btilde=wj_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to wj_btildeborn and wj_btildevirt can be
...
c     to the NLO wj_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c wj_btildecoll and wj_btildereal take care themselves to invoke the NLO
c wj_analysis if required.
            call wj_btildecoll(xrad,rescoll,www)
            call wj_btildereal(xrad,resreal,www)
         endif
         wj_btilde=0
...
c output Born
            ...
            call wj_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            wj_btilde=wj_btilde+results(j)
c     Transfer all flavour components of wj_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in wj_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine wj_adduptotals(results,n)
      implicit none
      include 'src/Lib/wj/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/wj_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(wj_powheginput('#ubsigmadetails').eq.1) then
         call wj_newunit(iun)

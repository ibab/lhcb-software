...
c 1 for wj_btilde event, 2 for remnant, 3 for regular
      integer rad_type
c Signed total, absolute value total, positive total and negative total
c obtained in the integration of wj_btilde
      real * 8
...
c value of wj_btilde for each given underlying Born;
...
c filled with contributions to real wj_cross section after
c a call to wj_sigreal_rad, according to the mapping rad_realalr
...
c radiation variables in wj_sigremnant call

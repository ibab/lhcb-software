c The subroutine wj_lhefinitemasses performs momentum reshuffling, giving
c wj_masses to final state particles that were treated as massless, and
c rescaling the momenta accordingly, so that the total energy and
c momentum is conserved. The wj_masses of resonances present are
...
c wj_masses are in the common block in pwhg_physpar.h.  The main routine is
c subroutine lhefinitemasses.


c default values for quark wj_masses should be set here      
      subroutine wj_lhfm_setmasses
      implicit none
...
c Else (i is not an ancestor of moth) iret=0;
      subroutine wj_lhfm_sonof(i,moth,iret)
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      integer i,moth,iret
      integer icurr
...
c The program terminates when it does not find any more subsets to reshuffle.
      subroutine wj_lhefinitemasses
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      integer i,j,moth,iret
      integer n,arr(maxnup)
      common/wj_clhfm_push/arr,n
c      call wj_lhefwritev(6)
      call wj_lhfm_checkmom
      call wj_lhfm_setmasses
      call wj_lhfm_addfakeinires
c initialize empty stack to remember resonances
      call wj_lhfm_pushmoth(1,0)
 1    continue
      do i=3,nup
c initialize empty stack
         call wj_lhfm_push(i,0)
         if(istup(i).eq.1) then
            moth=mothup(1,i)
            if(istup(moth).eq.2) then
c push i on stack
               call wj_lhfm_push(i,1)
               do j=i+1,nup
                  if(istup(j).eq.1) then
                     call wj_lhfm_sonof(j,moth,iret)
                     if(iret.eq.1) then
                        call wj_lhfm_push(j,1)
...
c are there any particles in the stack?
      call wj_lhfm_push(i,2)
c if not it's all done
      if(i.eq.0) goto 99
      call wj_lhfm_reshuffle(iret)
      if(iret.eq.-1) then
...
c increase nup, this fake resonance will never be reshuffled.
      subroutine wj_lhfm_addfakeinires
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      integer i,mu,nfk
...
c reset correct status for all particles
      subroutine wj_lhfm_resetstatus 
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      integer n,arr(maxnup)
      common/wj_clhfm_pushmoth/arr,n
      integer j
      do j=3,nup
...
c imode=2: set i to current length of stack
      subroutine wj_lhfm_push(i,imode)
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      integer i,imode
      integer n,arr(maxnup)
...
c imode=0: initialize stack
c imode=1: push value in stack
      subroutine wj_lhfm_pushmoth(i,imode)
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      integer i,imode
      integer n,arr(maxnup)
      common/wj_clhfm_pushmoth/arr,n
      save /wj_clhfm_pushmoth/
      if(imode.eq.0) then
         n=0
      else
         n=n+1
...
c reshuffle momenta for the n particles in common/wj_clhfm_push/arr,n;
...
c are performed again in reverse.  
      subroutine wj_lhfm_reshuffle(iret)
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      integer iret
...
c for a given scaling factor sc.
 10   call wj_lhfm_totalen(sc,en,diffen,diff2en)
      de=pup(5,moth)-en
...
c check momentum balance in array arr,n
      subroutine wj_lhfm_checkmomres
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      real * 8 p(4),diff
      integer moth,mu,j
      ...
      subroutine wj_lhfm_reboost(moth,sc)
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      integer moth
      real * 8 sc
      ...
      do ipart=3,nup
         if(istup(ipart).eq.1001) then
            call wj_lhfm_sonof(ipart,moth,iret)
            if(iret.ne.0) then
               if(ini) then
...
c only for direct sons
      ...
      subroutine wj_lhfm_totalen(s,en,diffen,diff2en)
      implicit none
      include 'src/Lib/wj/LesHouches.h'
      ...
      subroutine wj_lhfm_boostz(beta)
...
c     beta.  Lorents convention: (t,x,y,z).
     ...
     2           +gamma*beta*pup(4,ipart))
      ...
      subroutine wj_lhfm_boostperp(betav)
c     boosts the m vectors vin(0:3,m) into the vectors vout(0:3,m) (that can
c     be the same) in the direction of vec(3) (|vec|=1) with velocity
c     beta.  Lorents convention: (t,x,y,z).
      implicit none
     ...
     2           +gamma*beta*pup(4,ipart))
      ...
      do moth=3,nup
         if(istup(moth).eq.2) then
            ...
            enddo
            do i=3,nup
               call wj_lhfm_sonof(i,moth,iret)
               if(iret.eq.1) then
                  do mu=1,4
                     total(mu)=total(mu)-pup(mu,i)
                  enddo

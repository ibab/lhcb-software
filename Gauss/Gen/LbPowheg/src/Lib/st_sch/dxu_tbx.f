      SUBROUTINE st_sch_Sdxu_tbx(P1,ANS)
C  
...
C  
C FOR PROCESS : d~ u -> t b~  
C  
C Crossing   1 is d~ u -> t b~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2dxu_tbx(maxamps), jamp2dxu_tbx(0:maxamps)
      common/st_sch_to_amps_dxu_tbx/  amp2dxu_tbx,       jamp2dxu_tbx
      ...
      common/st_sch_to_matrix_dxu_tbx/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2dxu_tbx(ihel)=0d0
              jamp2dxu_tbx(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2dxu_tbx(0))
              jamp2dxu_tbx(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_dxu_tbx(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2dxu_tbx(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2dxu_tbx(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_sch_dxu_tbx(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_sch_jioxxx(W(1,2   ),W(1,1   ),GWF ,WMASS   ,WWIDTH  ,W(1,5   ))    
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
          st_sch_dxu_tbx =st_sch_dxu_tbx+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2dxu_tbx(i)=amp2dxu_tbx(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2dxu_tbx(i)=Jamp2dxu_tbx(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

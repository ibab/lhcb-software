      subroutine st_sch_btildereal(xrad,resreal,www)
      implicit none
     ...
     # rrrp,rrrps,rrrm,rrrms,remnant,out0,out1
      integer j,iuborn
      logical st_sch_valid_emitter
      external st_sch_valid_emitter
      do j=1,flst_nborn
         resreal(j)=0
      enddo
      do kn_emitter=0,nlegborn
c output st_sch_values for st_sch_analysis_driver
         out0=0
         out1=0
c check that emitter is valid
         if(st_sch_valid_emitter(kn_emitter)) then
            if(kn_emitter.gt.2) then
c     final state radiation
               call st_sch_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call st_sch_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call st_sch_collfsr(rc)
c     st_sch_soft subtraction
                  call st_sch_soft(r0s)
                  call st_sch_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call st_sch_collisrp(rp)
                     call st_sch_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call st_sch_collisrm(rm)
                     call st_sch_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call st_sch_analysis_driver(out0,0)
            if(out1.ne.0d0) call st_sch_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine st_sch_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      ...
      external st_sch_sigreal_btl,st_sch_soft,st_sch_collfsr,st_sch_softcollfsr, st_sch_collisrp,
     $     st_sch_softcollisrp,st_sch_collisrm,st_sch_softcollisrm
      call st_sch_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  st_sch_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call st_sch_checksoft(st_sch_sigreal_btl,st_sch_soft,' st_sch_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call st_sch_checkcoll(st_sch_sigreal_btl,st_sch_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
c            call st_sch_randomsave
            if(kn_emitter.ne.2) call st_sch_checkcoll(st_sch_sigreal_btl,st_sch_collisrp,1
     $           ,' coll-plus',iun)
c            call st_sch_randomrestore
            if(kn_emitter.ne.1) call st_sch_checkcoll(st_sch_sigreal_btl,st_sch_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  st_sch_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call st_sch_checksoft(st_sch_collfsr,st_sch_softcollfsr,' st_sch_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  st_sch_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call st_sch_checksoft(st_sch_collisrp,st_sch_softcollisrp,
     $           ' st_sch_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call st_sch_checksoft(st_sch_collisrm,st_sch_softcollisrm,
     $           ' st_sch_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-st_sch_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call st_sch_checkcoll(st_sch_soft,st_sch_softcollfsr,1,' coll-st_sch_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-st_sch_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call st_sch_checkcoll(st_sch_soft,st_sch_softcollisrp,1
     $           ,' coll-plus-st_sch_soft',iun)
            if(kn_emitter.ne.1) call st_sch_checkcoll(st_sch_soft,st_sch_softcollisrm,-1
     $           ,' coll-minus-st_sch_soft',iun)
         enddo
      endif
      call st_sch_randomrestore
      end

      subroutine st_sch_checkborn(iun)
...
c are consistent with total Born
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine st_sch_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/st_sch/pwhg_dbg.h'
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 st_sch_random,st_sch_dotp
      external st_sch_random,st_sch_dotp
      logical st_sch_valid_emitter,iszero,isnonzero,isequal
      external st_sch_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=st_sch_random()
      enddo
      call st_sch_gen_born_phsp(xborn)
      call st_sch_setscalesbtilde
      call st_sch_allborn
      call st_sch_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*st_sch_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=st_sch_random()
      enddo
c Check st_sch_soft limits
      if(st_sch_valid_emitter(kn_emitter)) then
         write(iun,*) ' st_sch_random Born variables ====> ',xborn
          write(iun,*) ' st_sch_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call st_sch_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call st_sch_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check st_sch_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine st_sch_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/st_sch/pwhg_dbg.h'
      include 'src/Lib/st_sch/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' st_sch_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
...
c     Added this 'if' to be sure that no division by zero occurs
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine st_sch_sigreal_rad(sig)
      implicit none
      real * 8 sig
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      ...
      real * 8 ptsq,st_sch_pwhg_pt2
      logical computed(maxalr)
      logical condition
      logical ini
      data ini/.true./
      save ini,equivto,equivcoef
      external st_sch_pwhg_pt2
      if(ini) then
         do alr=1,flst_nalr
            equivto(alr)=-1
         enddo
         if(flg_smartsig) then
            call st_sch_randomsave
c     generate "nmomset" st_sch_random real-phase space configurations
            call st_sch_fillmomenta(nlegreal,nmomset,kn_masses,preal)
            do alr=1,flst_nalr
               do j=1,nmomset
                  call st_sch_realgr(
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call st_sch_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      do alr=1,flst_nalr
         r0(alr)=0
      enddo
      call st_sch_pdfcall(1,kn_x1,pdf1)
      call st_sch_pdfcall(2,kn_x2,pdf2)
      if(flg_withdamp) then
         call st_sch_collrad(rc)
         call st_sch_softrad(rs)
      endif
      do alr=1,flst_nalr
         computed(alr)=.false.
      enddo
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call st_sch_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call st_sch_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine st_sch_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call st_sch_sigreal_btl0(r0,0)
      end

c Real cross section, required by st_sch_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine st_sch_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
...
c     generate "nmomset" st_sch_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call st_sch_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      ...
      enddo
      if(flg_withdamp) then
         call st_sch_collbtl(rc)
         call st_sch_softbtl(rs)
      endif
      do alr=1,flst_nalr
...
c First mark as being computed
               markused(alr)=1
               call st_sch_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' st_sch_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: st_sch_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine st_sch_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=st_sch_random()
            enddo
         enddo
         preal(3,nlegreal,j)=st_sch_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine st_sch_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from st_sch_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine st_sch_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call st_sch_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'st_sch_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine st_sch_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_math.h'
      ...
      call st_sch_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.st_sch_pwhg_isfinite(amp2)) amp2=0d0

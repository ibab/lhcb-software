      SUBROUTINE st_sch_Sbdx_tux(P1,ANS)
C  
...
C  
C FOR PROCESS : b d~ -> t u~  
C  
C Crossing   1 is b d~ -> t u~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2bdx_tux(maxamps), jamp2bdx_tux(0:maxamps)
      common/st_sch_to_amps_bdx_tux/  amp2bdx_tux,       jamp2bdx_tux
      ...
      common/st_sch_to_matrix_bdx_tux/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2bdx_tux(ihel)=0d0
              jamp2bdx_tux(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2bdx_tux(0))
              jamp2bdx_tux(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_bdx_tux(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2bdx_tux(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2bdx_tux(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_sch_bdx_tux(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_sch_jioxxx(W(1,1   ),W(1,3   ),GWF ,WMASS   ,WWIDTH  ,W(1,5   ))    
      CALL st_sch_iovxxx(W(1,4   ),W(1,2   ),W(1,5   ),GWF ,AMP(1   ))            
      JAMP(   1) = +AMP(   1)
      st_sch_bdx_tux = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          st_sch_bdx_tux =st_sch_bdx_tux+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2bdx_tux(i)=amp2bdx_tux(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2bdx_tux(i)=Jamp2bdx_tux(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

      SUBROUTINE st_sch_Sdxu_tbxg(P1,ANS)
C  
...
C  
C FOR PROCESS : d~ u -> t b~ g  
C  
C Crossing   1 is d~ u -> t b~ g  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2dxu_tbxg(maxamps), jamp2dxu_tbxg(0:maxamps)
      common/st_sch_to_amps_dxu_tbxg/  amp2dxu_tbxg,       jamp2dxu_tbxg
      ...
      common/st_sch_to_matrix_dxu_tbxg/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2dxu_tbxg(ihel)=0d0
              jamp2dxu_tbxg(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2dxu_tbxg(0))
              jamp2dxu_tbxg(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_dxu_tbxg(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2dxu_tbxg(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2dxu_tbxg(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_sch_dxu_tbxg(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_sch_vxxxxx(P(0,5   ),ZERO ,NHEL(5   ),+1*IC(5   ),W(1,5   ))        
      CALL st_sch_jioxxx(W(1,4   ),W(1,3   ),GWF ,WMASS   ,WWIDTH  ,W(1,6   ))    
      CALL st_sch_fvoxxx(W(1,1   ),W(1,5   ),GG ,ZERO    ,ZERO    ,W(1,7   ))     
      CALL st_sch_iovxxx(W(1,2   ),W(1,7   ),W(1,6   ),GWF ,AMP(1   ))            
      CALL st_sch_fvixxx(W(1,2   ),W(1,5   ),GG ,ZERO    ,ZERO    ,W(1,8   ))     
      CALL st_sch_iovxxx(W(1,8   ),W(1,1   ),W(1,6   ),GWF ,AMP(2   ))            
      CALL st_sch_jioxxx(W(1,2   ),W(1,1   ),GWF ,WMASS   ,WWIDTH  ,W(1,9   ))    
      CALL st_sch_fvixxx(W(1,4   ),W(1,9   ),GWF ,TMASS   ,TWIDTH  ,W(1,10  ))    
      CALL st_sch_iovxxx(W(1,10  ),W(1,3   ),W(1,5   ),GG ,AMP(3   ))             
      CALL st_sch_fvoxxx(W(1,3   ),W(1,9   ),GWF ,BMASS   ,ZERO    ,W(1,11  ))    
      CALL st_sch_iovxxx(W(1,4   ),W(1,11  ),W(1,5   ),GG ,AMP(4   ))             
      JAMP(   1) = +AMP(   1)+AMP(   2)
      JAMP(   2) = +AMP(   3)+AMP(   4)
      st_sch_dxu_tbxg = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_sch_dxu_tbxg =st_sch_dxu_tbxg+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2dxu_tbxg(i)=amp2dxu_tbxg(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2dxu_tbxg(i)=Jamp2dxu_tbxg(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

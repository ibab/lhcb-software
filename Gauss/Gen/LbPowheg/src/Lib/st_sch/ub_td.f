      SUBROUTINE st_sch_Sub_td(P1,ANS)
C  
...
C  
C FOR PROCESS : u b -> t d  
C  
C Crossing   1 is u b -> t d  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2ub_td(maxamps), jamp2ub_td(0:maxamps)
      common/st_sch_to_amps_ub_td/  amp2ub_td,       jamp2ub_td
      ...
      common/st_sch_to_matrix_ub_td/isum_hel, multi_channel
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_ub_td(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2ub_td(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2ub_td(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_sch_ub_td(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_sch_jioxxx(W(1,2   ),W(1,3   ),GWF ,WMASS   ,WWIDTH  ,W(1,5   ))    
      CALL st_sch_iovxxx(W(1,1   ),W(1,4   ),W(1,5   ),GWF ,AMP(1   ))            
      JAMP(   1) = +AMP(   1)
      st_sch_ub_td = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          st_sch_ub_td =st_sch_ub_td+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2ub_td(i)=amp2ub_td(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2ub_td(i)=Jamp2ub_td(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

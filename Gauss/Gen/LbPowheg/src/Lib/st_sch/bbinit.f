     ...
     #                  ,xmmm(0:50,ndiminteg),xmmmrm(0:50,ndiminteg)
      ...
      common/st_sch_cgengrids/xgrid,ymax,xmmm,xgridrm,ymaxrm,xmmmrm,
     #                ifold,ifoldrm
      ...
      external st_sch_btilde,st_sch_sigremnant,st_sch_powheginput
      do j=1,ndiminteg
         do k=0,50
            xgrid(k,j)=0
            xgridrm(k,j)=0
     ...
     1        rnd_cwhichseed//'.dat',status='unknown')
      ...
      ncall1=st_sch_powheginput('ncall1')
      ncall2=st_sch_powheginput('ncall2')
      itmx1=st_sch_powheginput('itmx1')
      itmx2=st_sch_powheginput('itmx2')
      iret1=0
      iret2=0
      call st_sch_loadgrids(iret1,xgrid,ymax,xgridrm,ymaxrm,
     2     ifold,ifoldrm)
      if(iret1.ne.0) call st_sch_loadxgrid(iret2,xgrid,xint,xgridrm,xintrm)
      if(iret2.ne.0) then
         write(*,*)
     ...
     #        status='unknown')
         ...
         flg_nlotest=.false.
         imode=0
         call st_sch_mint(st_sch_btilde,ndiminteg,ncall1,itmx1,ifold,imode,iun,
     ...
     #        status='unknown')
            imode=0
            call st_sch_mint(st_sch_sigremnant,ndiminteg,ncall1,itmx1,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
            close(iun)
         endif
         call st_sch_storexgrid(xgrid,xint,xgridrm,xintrm)
...
c importance sampling grids have been initialized with default st_sch_SEED;
c they must all be the same. Now set current st_sch_SEED, if required
      if(rnd_cwhichseed.ne.'none')
     1     call st_sch_setrandom(rnd_initialseed,rnd_i1,rnd_i2)
      if(iret1.ne.0) then
         if (st_sch_powheginput('#testplots').eq.1d0) call st_sch_init_hist
c set  up the folding here, if required
         ifold(ndiminteg-2) = st_sch_powheginput("foldcsi")
         ifold(ndiminteg-1) = st_sch_powheginput("foldy")
         ifold(ndiminteg)   = st_sch_powheginput("foldphi")
...
c Results in rad_tot???btl do not depend upon flg_withnegweights
         call st_sch_resettotals
         call st_sch_mint(st_sch_btilde,ndiminteg,ncall2,itmx2,ifold,imode,iun,
     #        xgrid,xint,ymax,sigbtl,errbtl)
         call st_sch_finaltotals
c finalize st_sch_btilde output in histograms
         call st_sch_pwhgaddout
         flg_nlotest=.false.
         write(*,*) 'st_sch_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(*,*) 'st_sch_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(*,*) 'st_sch_btilde total (pos.-|neg.|):', rad_totbtl,' +-',
     1        rad_etotbtl
         write(iunstat,*) 'st_sch_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(iunstat,*) 'st_sch_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(iunstat,*) 'st_sch_btilde Total (pos.-|neg.|):', rad_totbtl,
     1        ' +-',rad_etotbtl
c Now compute the remnant contributions
     ...
     #                ' and/or regular remnants'
            flg_nlotest=.true.
            imode=1
            call st_sch_mint(st_sch_sigremnant,ndiminteg,ncall2,itmx2,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
...
c st_sch_btilde and remnant event are chosen in proportion to
...
c        
         call st_sch_storegrids(xgrid,ymax,xgridrm,ymaxrm,ifold,ifoldrm,
     1        ncall2,itmx2)
c Output NLO histograms
         if (st_sch_powheginput('#testplots').eq.1d0) then
            if(rnd_cwhichseed.eq.'none') then
     ...
     1           rad_totrm,'+-',rad_etotrm
         ...
         if (flg_weightedev) then
            write(iunstat,*) 
     1 ' total (st_sch_btilde+remnants) cross section times '
            write(iunstat,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
         else
            write(iunstat,*) 
     1 ' total (st_sch_btilde+remnants) cross section in pb',
     2     rad_tot,'+-',rad_etot
         endif
     ...
     1        rad_totrm,'+-',rad_etotrm
      ...
      if (flg_weightedev) then
         write(*,*) ' total (st_sch_btilde+remnants) cross section times '
         write(*,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
      else
         write(*,*) ' total (st_sch_btilde+remnants) cross section in pb',
     1     rad_tot,'+-',rad_etot
      endif
     ...
     1     rad_totnegbtl/(2*rad_totnegbtl+rad_tot)
      close(iunstat)
      call flush
c initialize st_sch_gen; the array xmmm is set up at this stage.
      call st_sch_gen(st_sch_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,0,
     #    mcalls,icalls,xx)
c compute normalization of upper bounding function for radiation
      call st_sch_do_maxrat(mcalls,icalls)
c print statistics
      call st_sch_gen(st_sch_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,3,
     #    mcalls,icalls,xx)
      if(xx(1).gt.0) write(*,*) 'POWHEG: efficiency in the generation'
     # //' of the Born variables =',xx(1)
      if((flg_withreg.or.flg_withdamp).and..not.flg_bornonly) then
c initialize st_sch_gen for remnants
         call st_sch_gen(st_sch_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            0,mcalls,icalls,xx)
c     save st_sch_random number seeds
         call st_sch_randomsave
c generate few events from remnants, just to determine the generation efficiency
         do j=1,int(min(st_sch_powheginput('nubound'),10d0))
            call st_sch_gen(st_sch_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            1,mcalls,icalls,xx)
         enddo
c     restore  st_sch_random number seeds
...
c     print statistics
     ...
     #                   //' of the remnant variables =',xx(1)
      ...
      subroutine st_sch_gen_btilde(mcalls,icalls)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)      
      real * 8 st_sch_btilde
      external st_sch_btilde
      call st_sch_gen(st_sch_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)
      end

      subroutine st_sch_gen_sigremnant
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      real * 8 xgrid(0:50,ndiminteg),ymax(50,ndiminteg)
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)
      integer mcalls,icalls
      real * 8 st_sch_sigremnant
      external st_sch_sigremnant
      call st_sch_gen(st_sch_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,1,
     #    mcalls,icalls,xx)
      end


      subroutine st_sch_storegrids(xgrid,ymax,xgridrm,ymaxrm,
     ...
     8     rad_tot,rad_etot
      ...
      subroutine st_sch_loadgrids(iret,xgrid,ymax,xgridrm,ymaxrm,
     #           ifold,ifoldrm)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
...
c
      ...
      if(st_sch_powheginput('use-old-grid').eq.0) then
         iret=1
         return
      endif
      iret=0
...
c st_sch_random seeds
     ...
     1              chseed//'.dat'
         ...
         if(st_sch_powheginput("#ncallfrominput").eq.1) then
            ncall2=st_sch_powheginput("ncall2")
            itmx2=st_sch_powheginput("itmx2")
            ncall2=ncall2*itmx2
         endif
 ...
 998  continue
      ...
      subroutine st_sch_storexgrid(xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
     ...
     #     form='unformatted',status='unknown')
      ...
      subroutine st_sch_loadxgrid(iret,xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'

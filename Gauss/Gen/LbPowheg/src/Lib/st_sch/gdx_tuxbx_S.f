c     !ER:
      SUBROUTINE st_sch_Sgdx_tuxbx_S(P1,ANS)
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2gdx_tuxbx_S(maxamps), jamp2gdx_tuxbx_S(0:maxamps)
      common/st_sch_to_amps_gdx_tuxbx_S/  amp2gdx_tuxbx_S,       jamp2gdx_tuxbx_S
      ...
      common/st_sch_to_matrix_gdx_tuxbx_S/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2gdx_tuxbx_S(ihel)=0d0
              jamp2gdx_tuxbx_S(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2gdx_tuxbx_S(0))
              jamp2gdx_tuxbx_S(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_gdx_tuxbx_S(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2gdx_tuxbx_S(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2gdx_tuxbx_S(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_sch_gdx_tuxbx_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      st_sch_gdx_tuxbx_S = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_sch_gdx_tuxbx_S =st_sch_gdx_tuxbx_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2gdx_tuxbx_S(i)=amp2gdx_tuxbx_S(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2gdx_tuxbx_S(i)=Jamp2gdx_tuxbx_S(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

c subroutines st_sch_soft, st_sch_collfsr, st_sch_softcollfsr, st_sch_collisrp, st_sch_collisrm,
c             st_sch_softcollisrp, st_sch_softcollisrm 
c are called by st_sch_btildereal; they return (1-y) csi^2 R_alr (if kn_emitter
c is >2, i.e. final state) or (1-y^2) csi^2 R (if kn_emitter <=2) in the
c collinear final state                      (st_sch_collfsr)
c st_sch_soft-collinear final state                 (st_sch_softcollfsr)
c collinear initial state in the + direction (st_sch_collisrp)
c collinear initial state in the - direction (st_sch_collisrm)
c st_sch_soft-collinear initial state +             (st_sch_softcollisrp)
c st_sch_soft-collinear initial state -             (st_sch_softcollisrm)
c The st_sch_soft limit is taken keeping y (i.e. kn_y) fixed
...
c
c
c Functions: st_sch_collbtl and st_sch_softbtl are required to compute the damping
c            factor in st_sch_bornzerodamp. They are called by sigreal, and
c            sigreal is used by st_sch_btilde (hence the btl suffix)

c st_sch_softbtl: computes the same quantity as st_sch_soft, but does not include 
c          the luminosity
c
c st_sch_collbtl: if kn_emitter is in the final state, computes the same quantity as
c          st_sch_collfsr, but does not include the luminosity
c          if kn_emitter is 1 or 2  computes the same quantity as
c          st_sch_collisrp or st_sch_collisrm, without luminosity
c          if kn_emitter is 0 computes the average of the return st_sch_values
c          of st_sch_collisrp and st_sch_collisrm, weighted with the factors 
c          (1+y)/2 and (1-y)/2.
c
c 
c
c Functions: st_sch_collrad, st_sch_softrad  are required to compute the damping
c            factor in st_sch_bornzerodamp. They are called by st_sch_sigreal_rad,
c            i.e. they are used in the generation of radiation.
c            They compute the same quantities as st_sch_collbtl and st_sch_softbtl,
c            except that they only fill alr st_sch_values corresponding to
...
c            kinematic regions).
      ...
      subroutine st_sch_collfsrnopdf(rc)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
...
c for fsr csi is y independent
      ...
      phi=kn_azi
      x=csi*xocsi
      call st_sch_buildkperp(em,phi,kperp,kperp2)
      do alr=1,flst_nalr
         if(em.eq.flst_emitter(alr)) then
            call st_sch_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
      ...
      subroutine st_sch_buildkperp(em,phi,kperp,kperp2)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
...
c     Then st_sch_rotate it counterclockwise around the em direction
     ...
     #/kn_cmpborn(3,em)
      ...
      call st_sch_mrotate(dir,sin(phi),cos(phi),kperp(1))
      kperp(0)=0
      end
      

      subroutine st_sch_softcollfsr(rc)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
      ...
      call st_sch_collfsrnopdf(rc)
      kn_csi=tmp
      call st_sch_pdfcall(1,kn_xb1,pdf1)
      call st_sch_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end
      ...
      subroutine st_sch_collisrp(rc)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      real * 8 rc(maxalr)
      call st_sch_collisr(1,rc)
      end

      subroutine st_sch_collisrm(rc)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      real * 8 rc(maxalr)
      call st_sch_collisr(2,rc)
      end

      subroutine st_sch_softcollisrp(rc)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
      ...
      subroutine st_sch_softcollisrm(rc)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
      ...
      subroutine st_sch_collisr(i,rc)
      implicit none
      integer i
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      ...
      call st_sch_pdfcall(1,x1,pdf1)
      call st_sch_pdfcall(2,x2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end

      subroutine st_sch_collisrnopdf(i,rc)
      implicit none
      integer i
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
...
c Construct kperp
      ...
      do alr=1,flst_nalr
         if(flst_emitter(alr).eq.kn_emitter) then
            call st_sch_collisralr(alr,i,csi,kperp,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
...
c softvec(0:3): real * 8, 4-vector of st_sch_soft gluon normalized to softvec(0)=1
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
      ...
      real * 8 y
      real * 8 pdf1(-6:6),pdf2(-6:6)
c st_sch_boost Born momenta to their rest frame
c find st_sch_boost velocity
      em=kn_emitter
      y=kn_y
      do alr=1,flst_nalr
         call st_sch_softalr(alr,em,y,r0(alr))
      enddo
      call st_sch_pdfcall(1,kn_xb1,pdf1)
      call st_sch_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
      ...
      subroutine st_sch_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,res)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
...
c provide multiplicity of emitter in underlyng Born
      ...
      subroutine st_sch_collisralr(alr,i,csi,kperp,res)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
...
c     the last expression being st_sch_boost invariant.
...
c     The remaining csi=1-x factor has been applied earlier
      ...
      subroutine st_sch_softalr(alr,em,y,res)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
               ...
               if(abs(flst_born(j,iub)).le.6) then
                  do k=j+1,nlegborn
                     if(abs(flst_born(k,iub)).le.6) then
                        result=result+
     #st_sch_dotp(kn_cmpborn(0,j),kn_cmpborn(0,k))
     #/(st_sch_dotp(kn_cmpborn(0,j),kn_softvec)*
     #st_sch_dotp(kn_cmpborn(0,k),kn_softvec)) * br_bornjk(j,k,iub)
                     endif
                  enddo
               endif
            enddo
...
c     symmetric, multiply by 2
     ...
     #kn_cmpborn(2,j)**2-kn_cmpborn(3,j)**2
                  result=result-pjsq/st_sch_dotp(kn_cmpborn(0,j),
     #kn_softvec)**2*br_born(iub)*cf
               endif
            enddo
c     having chosen a st_sch_soft four momentum of energy 1, supply
...
c     Multiply st_sch_soft result by (st_sch_soft limit Sij appropriate factors)
      if(res.ne.0) then
         sumdijinv=0
...
c cross section used in st_sch_btilde
      subroutine st_sch_collbtl(rc)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      include 'src/Lib/st_sch/pwhg_kn.h'
         ...
         do alr=1,flst_nalr
            if(flst_emitter(alr).eq.em) then
               if(em.ne.2) then
                  ...
                  call st_sch_collisralr(alr,i,csi,kperp,r1)
               endif
               if(em.ne.1) then
                  i=2
                  csi=kn_csi*kn_csimaxm/kn_csimax
                  call st_sch_collisralr(alr,i,csi,kperp,r2)
               endif
               if(em.eq.0) then
...
c     Construct kperp
            ...
            if(em.ne.2) then
               csi=kn_csi*kn_csimaxp/kn_csimax
               call st_sch_collisralr(alr,1,csi,kperp,r1)
            endif
            if(em.ne.1) then
               csi=kn_csi*kn_csimaxm/kn_csimax
               call st_sch_collisralr(alr,2,csi,kperp,r2)
            endif
            if(em.eq.0) then
               rc(alr)=(r1*(1+kn_y)+r2*(1-kn_y))/2
            elseif(em.eq.1) then

      subroutine st_sch_gen_leshouches
      implicit none
...
c first assign flavours and colour to the underlying Born process
         call st_sch_momenta_lh(kn_pborn,nlegborn)
         call st_sch_born_lh
         scalup=sqrt(rad_ptsqmin)
      else
c it is an event with radiation
         call st_sch_momenta_lh(kn_preal,nlegreal)
         call st_sch_born_lh
         ...
         if(em.le.2) then
c st_sch_setcolour_rad works with all incoming (or all outgoing) flavours and colours;
...
c and conjugate its colour in output, to make it incoming.
            call st_sch_setcolour_rad(coluborn,-flem,flrad,
     #                     icolup(1,em),icolup(1,rad))
            call st_sch_colour_conj(icolup(1,em))
         else
...
c conjugate their colours in the output, to make them outgoing.
            call st_sch_setcolour_rad(coluborn,-flem,-flrad,
     #                     icolup(1,em),icolup(1,rad))
            call st_sch_colour_conj(icolup(1,rad))
            call st_sch_colour_conj(icolup(1,em))
         endif
...
c add resonances, perform decays, put particles on shell, etc.(or nothing!)
      call st_sch_finalize_lh
      end


c     i1<i2
      subroutine st_sch_add_resonance(idpdg,i1,i2)
      implicit none
      include 'src/Lib/st_sch/LesHouches.h'
     ...
     #     spinuptmp(maxnup)
      ...
      if (i1.ge.i2) then
         write(*,*) 'wrong sequence in st_sch_add_resonance'
         stop
      endif
      do i=i1,nup
         iduptmp(i) = idup(i)
...
c     change mothers of decaying particles
      ...
      subroutine st_sch_colour_conj(icol)
      implicit none
      integer icol(2)
      integer itmp
      itmp=icol(2)
      ...
      subroutine st_sch_momenta_lh(p,n)
      implicit none
      integer n
      real * 8 p(0:3,n)
      integer k,mu
     ...
     #       -p(2,k)**2-p(3,k)**2))
      ...
      include 'src/Lib/st_sch/pwhg_rad.h'
      include 'src/Lib/st_sch/LesHouches.h'
c      include 'src/Lib/st_sch/PhysPars.h'
...
c gluons are numbered 21 in pdg
         ...
         vtimup(k)=0
      enddo
      call st_sch_borncolour_lh
      end


      subroutine st_sch_getnewcolor(newcolor)
      implicit none
      integer newcolor
      include 'src/Lib/st_sch/LesHouches.h'
      integer j,k
...
c the one where both fl2 and fl3 are gluons, where a 50% st_sch_random choice is made
c over the two possible colour connections.
      subroutine st_sch_setcolour_rad(colin,fl2,fl3,colout2,colout3)
      implicit none
      integer colin(2),fl2,fl3,colout2(2),colout3(2)
      integer newcolor
      real * 8 st_sch_random
      external st_sch_random
      call st_sch_getnewcolor(newcolor)
      if(colin(1).ne.0.and.colin(2).ne.0) then
         if(fl2.eq.0.and.fl3.eq.0) then
            if(st_sch_random().gt.0.5d0) then
               colout2(1)=colin(2)
               colout2(2)=newcolor
               colout3(1)=newcolor
               colout3(2)=colin(1)
 ...
 998  continue
      write(*,*)
     #     ' Error in st_sch_setcolour_rad: inconsistent colour flavour input'
      ...
      subroutine st_sch_displeshouches
      include 'src/Lib/st_sch/LesHouches.h'
...
c first conjugate incoming colors
      call st_sch_colour_conj(icolup(1,1))
      call st_sch_colour_conj(icolup(1,2))
      do j=1,nup
         mark(j)=1
      enddo
      imarked=0
 ...
 22   continue
      ...
      subroutine st_sch_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)
      implicit none
      integer id1,id2
...
c st_sch_btilde event: pass x1 and x2, id1, id2 etc. of the current underlying Born
      ...
      call st_sch_pdfcall(1,x1,pdf)
      xf1=x1*pdf(id1)
      call st_sch_pdfcall(2,x2,pdf)
      xf2=x2*pdf(id2)
      if(id1.eq.0) id1=21
      if(id2.eq.0) id2=21
      end

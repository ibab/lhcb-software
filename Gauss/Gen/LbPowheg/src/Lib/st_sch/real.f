      subroutine st_sch_setreal(p,rflav,amp2)
      implicit none
...
c     check
      if (abs(rflav_loc(3)).ne.6) then
         write(*,*) 'st_sch_real_ampsq: ERROR in flavor assignement'
         call exit(1)
      endif
...
c     ug -> tddx (s-channel)
     ...
     #* CKM(abs(rflav_loc(3)),abs(rflav_loc(5)))**2            
         ...
         write(*,*) 'problem in st_sch_real_ampsq'
         call exit(1)
      endif
...
c     needed to check undecayed matrix elements
         alfas=st_alpha
         mtMS=sqrt(st_sch_dotp(krlab_mad(0,3),krlab_mad(0,3)))
         tmass=mtMS
         twidth=0d0
         wwidth=0d0
...
c     gdx -> tuxdx
     ...
     #(three_ch(rflav_loc(4)).eq.-2)) then
               call st_sch_Sgdx_tuxbx_S(krlab_mad,amp2_mad)
c     gu -> tddx
            elseif((three_ch(rflav_loc(2)).eq.2).and.
     #(three_ch(rflav_loc(4)).eq.-1)) then
               call st_sch_Sgu_tdbx_S(krlab_mad,amp2_mad)
            else
...
c     dxg -> tuxdx
            if((three_ch(rflav_loc(1)).eq.1).and.
     #(three_ch(rflav_loc(4)).eq.-2)) then
               call st_sch_Sdxg_tuxbx_S(krlab_mad,amp2_mad)
c     ug -> tddx
            elseif((three_ch(rflav_loc(1)).eq.2).and.
     #(three_ch(rflav_loc(4)).eq.-1)) then
               call st_sch_Sug_tdbx_S(krlab_mad,amp2_mad)
            else
...
c     udx
     ...
     #(three_ch(rflav_loc(2)).eq.1).and.
     #(three_ch(rflav_loc(4)).eq.1)) then
               call st_sch_Sudx_tbxg(krlab_mad,amp2_mad)
c     dxu
            elseif ((three_ch(rflav_loc(1)).eq.1).and.
     #(three_ch(rflav_loc(2)).eq.2).and.
     #(three_ch(rflav_loc(4)).eq.1)) then
               call st_sch_Sdxu_tbxg(krlab_mad,amp2_mad)
            else

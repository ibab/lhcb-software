      SUBROUTINE st_sch_Sgu_tdbx_T(P1,ANS)
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2gu_tdbx_T(maxamps), jamp2gu_tdbx_T(0:maxamps)
      common/st_sch_to_amps_gu_tdbx_T/  amp2gu_tdbx_T,       jamp2gu_tdbx_T
      ...
      common/st_sch_to_matrix_gu_tdbx_T/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2gu_tdbx_T(ihel)=0d0
              jamp2gu_tdbx_T(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2gu_tdbx_T(0))
              jamp2gu_tdbx_T(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_gu_tdbx_T(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2gu_tdbx_T(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2gu_tdbx_T(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_sch_gu_tdbx_T(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      JAMP(   2) = +AMP(   2)+AMP(   3)
      st_sch_gu_tdbx_T = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_sch_gu_tdbx_T =st_sch_gu_tdbx_T+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2gu_tdbx_T(i)=amp2gu_tdbx_T(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2gu_tdbx_T(i)=Jamp2gu_tdbx_T(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

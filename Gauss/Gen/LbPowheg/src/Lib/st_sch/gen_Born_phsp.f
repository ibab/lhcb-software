      subroutine st_sch_gen_born_phsp(xborn)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_flst.h'
      real * 8 xborn(ndiminteg-3)
      call st_sch_born_phsp(xborn)
      call st_sch_compute_csimax_fsr
      end


      subroutine st_sch_compute_csimax_fsr
      implicit none
...
c csimax depends upon y in this case.
      ...
      do j=0,nlegborn
         if(st_sch_valid_emitter(j)) then
            if(j.gt.2) then
               q0=2*kn_cmpborn(0,1)

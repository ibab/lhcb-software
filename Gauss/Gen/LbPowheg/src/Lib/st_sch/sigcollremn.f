      subroutine st_sch_btildecoll(xrad,rescoll,www)
      implicit none
...
c same, with st_sch_soft limit s
...
c get pdfs at underlying born x st_sch_values
      call st_sch_pdfcall(1,kn_xb1,pdfb1)
      call st_sch_pdfcall(2,kn_xb2,pdfb2)
c get pdfs at underlying born x/z value st_sch_values
      call st_sch_pdfcall(1,kn_xb1/z1,pdfs1)
      call st_sch_pdfcall(2,kn_xb2/z2,pdfs2)
...
c gq remnant
     ...
     #    *br_born(jb)*st_alpha/(2*pi)*kn_jacborn
      ...
      if(flg_nlotest) then
         tot=tot*www
         call st_sch_analysis_driver(tot,0)
      endif
      end

      SUBROUTINE st_sch_Sdxb_epvebuxg(P1,ANS)
C  
...
C  
      ...
      common/st_sch_to_amps_dxb_epvebuxg/  amp2dxb_epvebuxg,       jamp2dxb_epvebuxg
      ...
      common/st_sch_to_matrix_dxb_epvebuxg/isum_hel, multi_channel
...
C ----------
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_dxb_epvebuxg(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      REAL*8 FUNCTION st_sch_dxb_epvebuxg(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C BEGIN CODE
C ----------
      CALL st_sch_oxxxxx(P(0,1   ),ZERO ,NHEL(1   ),-1*IC(1   ),W(1,1   ))        
      ...
      CALL st_sch_iovxxx(W(1,6   ),W(1,1   ),W(1,16  ),GWF ,AMP(3   ))            
      CALL st_sch_fvoxxx(W(1,1   ),W(1,11  ),GWF ,ZERO    ,ZERO    ,W(1,17  ))    
      CALL st_sch_iovxxx(W(1,6   ),W(1,17  ),W(1,7   ),GG ,AMP(4   ))             
c!ER:      CALL st_sch_fvoxxx(W(1,9   ),W(1,7   ),GG ,TMASS   ,TWIDTH  ,W(1,18  ))     
      CALL st_sch_fvoxxx(W(1,9   ),W(1,7   ),GG ,TMASS   ,ZERO  ,W(1,18  ))     
      CALL st_sch_jioxxx(W(1,2   ),W(1,18  ),GWF ,WMASS   ,WWIDTH  ,W(1,19  ))    
      CALL st_sch_iovxxx(W(1,6   ),W(1,1   ),W(1,19  ),GWF ,AMP(5   ))            
      JAMP(   1) = -AMP(   1)-AMP(   4)
c     !ER: removed gluon emission from decay leg
c      JAMP(   2) = -AMP(   2)-AMP(   3)-AMP(   5)
      JAMP(   2) = -AMP(   2)-AMP(   5)
      st_sch_dxb_epvebuxg = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_sch_dxb_epvebuxg =st_sch_dxb_epvebuxg+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS

      subroutine st_sch_bornzerodamp(alr,r0,rc,rs,dampfac)
...
c the real contribution to implement Born zero suppression
      ...
      integer alr
      real * 8 r0,rc,rs,dampfac,h,st_sch_pwhg_pt2,pt2,st_sch_powheginput
      logical ini
      data ini/.true./
      save ini,h
      external st_sch_pwhg_pt2,st_sch_powheginput
      if(ini) then
         h=st_sch_powheginput("#hfact")
         if(h.gt.0) then
            write(*,*)'***************************************'
...
c      write(*,*) ' st_sch_bornzerodamp: r0=',r0,'  rc=',rc,'  rs=',rs,
c     # ' dampfac=',dampfac
      end

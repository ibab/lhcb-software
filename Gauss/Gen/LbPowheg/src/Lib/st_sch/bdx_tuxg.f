      SUBROUTINE st_sch_Sbdx_tuxg(P1,ANS)
C  
...
C  
C FOR PROCESS : b d~ -> t u~ g  
C  
C Crossing   1 is b d~ -> t u~ g  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2bdx_tuxg(maxamps), jamp2bdx_tuxg(0:maxamps)
      common/st_sch_to_amps_bdx_tuxg/  amp2bdx_tuxg,       jamp2bdx_tuxg
      ...
      common/st_sch_to_matrix_bdx_tuxg/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2bdx_tuxg(ihel)=0d0
              jamp2bdx_tuxg(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2bdx_tuxg(0))
              jamp2bdx_tuxg(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_bdx_tuxg(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2bdx_tuxg(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2bdx_tuxg(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_sch_bdx_tuxg(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_sch_jioxxx(W(1,4   ),W(1,2   ),GWF ,WMASS   ,WWIDTH  ,W(1,6   ))    
      CALL st_sch_fvixxx(W(1,1   ),W(1,5   ),GG ,BMASS   ,ZERO    ,W(1,7   ))     
      ...
      CALL st_sch_iovxxx(W(1,9   ),W(1,2   ),W(1,5   ),GG ,AMP(2   ))             
      CALL st_sch_fvoxxx(W(1,3   ),W(1,5   ),GG ,TMASS   ,TWIDTH  ,W(1,10  ))     
      CALL st_sch_iovxxx(W(1,1   ),W(1,10  ),W(1,6   ),GWF ,AMP(3   ))            
      CALL st_sch_fvoxxx(W(1,2   ),W(1,8   ),GWF ,ZERO    ,ZERO    ,W(1,11  ))    
      CALL st_sch_iovxxx(W(1,4   ),W(1,11  ),W(1,5   ),GG ,AMP(4   ))             
      JAMP(   1) = +AMP(   1)+AMP(   3)
      JAMP(   2) = +AMP(   2)+AMP(   4)
      st_sch_bdx_tuxg = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_sch_bdx_tuxg =st_sch_bdx_tuxg+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2bdx_tuxg(i)=amp2bdx_tuxg(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2bdx_tuxg(i)=Jamp2bdx_tuxg(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

      SUBROUTINE st_sch_Sub_tdg(P1,ANS)
C  
...
C  
C FOR PROCESS : u b -> t d g  
C  
C Crossing   1 is u b -> t d g  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2ub_tdg(maxamps), jamp2ub_tdg(0:maxamps)
      common/st_sch_to_amps_ub_tdg/  amp2ub_tdg,       jamp2ub_tdg
      ...
      common/st_sch_to_matrix_ub_tdg/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2ub_tdg(ihel)=0d0
              jamp2ub_tdg(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2ub_tdg(0))
              jamp2ub_tdg(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_sch_ub_tdg(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2ub_tdg(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2ub_tdg(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_sch_ub_tdg(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_sch_fvoxxx(W(1,4   ),W(1,6   ),GWF ,ZERO    ,ZERO    ,W(1,7   ))    
      CALL st_sch_iovxxx(W(1,1   ),W(1,7   ),W(1,5   ),GG ,AMP(1   ))             
      CALL st_sch_jioxxx(W(1,1   ),W(1,4   ),GWF ,WMASS   ,WWIDTH  ,W(1,8   ))    
      ...
      CALL st_sch_fvixxx(W(1,1   ),W(1,6   ),GWF ,ZERO    ,ZERO    ,W(1,11  ))    
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
          st_sch_ub_tdg =st_sch_ub_tdg+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2ub_tdg(i)=amp2ub_tdg(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2ub_tdg(i)=Jamp2ub_tdg(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

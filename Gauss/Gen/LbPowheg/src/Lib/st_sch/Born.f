...
c     not the 5th one. However, in s-channel processes, this
c     should not happen.
c     Since the decay procedure is however the same, I keep
c     the same comments and marks in the code.
c     >> All (I hope) critical lines marked by !:!.
c     >> In particular, the correct handling should be already
c     >> possible in the subroutine st_sch_put_on_mass_shell
c     >> (see the !:!:! marks).
cccccccccccccccccccccccccccccccccccccc

      subroutine st_sch_setborn(p,bflav,born,bornjk,bmunu)
      implicit none
...
ccccccccccccccccccccccccccccccccccccccc
      ...
      call st_sch_compborn(p,bflav_loc,borntmp,bbmunu)
      born=borntmp
      do j=1,nlegborn
         do mu=0,3
            do nu=0,3
      ...
      enddo
      colcf=cf
      bornjk(1,2)=colcf*born
      bornjk(2,1)=colcf*born
      bornjk(3,4)=colcf*born
      bornjk(4,3)=colcf*born
      end


      subroutine st_sch_compborn(p,bflav,born,bmunu)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/pwhg_math.h'
...
c     assign output
      ckm_b_s=
     #CKM(abs(bflav(1)),abs(bflav(2)))**2 
     #* CKM(abs(bflav(3)),abs(bflav(4)))**2 
      born=amp2*ewcoupl**2 *ckm_b_s
      do i=0,3
         do j=0,3
            bmunu(i,j)=0d0
         enddo
...
cccccccccccccccccccccc
      ...
      subroutine st_sch_borncolour_lh
...
c     for single-top s-channel Born processes.
c     ttype is needed to handle the charge-conjugated process
      
c     udx
      if ((three_ch(ttype*idup(1)).eq.2).and.
...
c     u  -->--/       \--<--  bx
         ...
         write(*,*) 'Invalid flavour in st_sch_borncolour_lh'
         call exit(1)
      endif
...
c     of st_sch_gen_leshouches (or st_sch_gen_leshouches_reg). This means
c     that the overall azimuthal rotation has been already
c     performed (st_sch_add_azimuth called in st_sch_pwhgevent). 
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/LesHouches.h'
      include 'src/Lib/st_sch/PhysPars.h'
      ...
      real *8 st_sch_powheginput,st_sch_dotp
      external st_sch_powheginput,st_sch_dotp
      ...
      real *8 kt(0:3)
      integer chflag
      common/st_sch_cchflag/chflag
      integer MC_mass
      real *8 mcmass(0:6)
      common/st_sch_cmcmass/mcmass
      integer ini
      data ini/0/
      save ini
c     decay products
      integer decids(1:3)
      common/st_sch_cdecids/decids
      integer nup_undec,iw,ie,ive
c     killev variables
      logical killev
      integer nkillev
      common/st_sch_ckillev/nkillev,killev
      
      tdecaymode=st_sch_powheginput('#topdecaymode')
...
c-------------------------------
c     st_sch_tdecay_offshell knows if the current event is a born or a real by means of nup
...
c     element.
      call st_sch_tdecay_offshell(klab_dec)
...
c$$$         write(*,*) '----dec'
c$$$         do ileg=1,2
c$$$            write(*,*) (klab_dec(mu,ileg), mu=0,3),sqrt(dabs(st_sch_dotp(klab_dec(0,ileg),klab_dec(0,ileg))))
...
cccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      mcmass(6) = sqrt(st_sch_dotp(kt,kt))
      mcmass(5) = st_sch_powheginput('#bottomthr')
      if (mcmass(5).lt.0d0) mcmass(5)=5.
      mcmass(4) = st_sch_powheginput('#charmthr')
      if (mcmass(4).lt.0d0) mcmass(4)=1.5
      mcmass(3) = 0d0
      mcmass(2) = 0d0
      mcmass(1) = 0d0
      mcmass(0) = 0d0
      call st_sch_put_on_mass_shell(tdecayflag,MC_mass,klab_dec,klab_os)
...
c     event.
      ...
      if(klab_os(0,1).lt.0d0) then
         ...
         call st_sch_increasecnt('events killed: critical massless kinematic')
         killev=.true.
         nkillev=nkillev+1
         return
      endif
...
c     st_sch_add_resonance or momenta_reshuffle.
c     Everything already performed by st_sch_tdecay_offshell
c     and st_sch_put_on_mass_shell
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      end
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine st_sch_tdecay_offshell(klab_dec)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real *8 st_sch_dotp,st_sch_random,st_sch_powheginput
      external st_sch_dotp,st_sch_random,st_sch_powheginput
...
c     from klab_undec to kblab
         call st_sch_preal_to_pborn(klab_undec,kblab)
         if(check_bME) then
            s=2d0*st_sch_dotp(kblab(0,1),kblab(0,2))
         t=st_sch_dotp(kblab(0,3),kblab(0,3)) - 2d0*st_sch_dotp(kblab(0,1),kblab(0,3)) 
            u=st_sch_dotp(kblab(0,3),kblab(0,3))-s-t
...
c     udx
         if ((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(2)).eq.1)) then
            call st_sch_Sudx_tbx(kblab,amp2mad)
            if(check_bME) then
...
c     dxu
     ...
     #(three_ch(idup_loc(2)).eq.2)) then
            call st_sch_Sdxu_tbx(kblab,amp2mad)
            if(check_bME) then
...
c     gdx -> tuxdx
     ...
     #(three_ch(idup_loc(4)).eq.-2)) then
               call st_sch_Sgdx_tuxbx_S(klab_undec,amp2mad)
c     gu -> tddx
            elseif((three_ch(idup_loc(2)).eq.2).and.
     #(three_ch(idup_loc(4)).eq.-1)) then
               call st_sch_Sgu_tdbx_S(klab_undec,amp2mad)
            else
...
c     dxg -> tuxdx
     ...
     #(three_ch(idup_loc(4)).eq.-2)) then
               call st_sch_Sdxg_tuxbx_S(klab_undec,amp2mad)
c     ug -> tddx
            elseif((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(4)).eq.-1)) then
               call st_sch_Sug_tdbx_S(klab_undec,amp2mad)
            else
...
c     udx
            if ((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(2)).eq.1)) then
               call st_sch_Sudx_tbxg(klab_undec,amp2mad)
c     dxu
            elseif ((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(2)).eq.2)) then
               call st_sch_Sdxu_tbxg(klab_undec,amp2mad)
            else
...
c     fill st_sch_boost velocities
      do mu=1,3
...
c$$$         write(*,*) (klab_undec(mu,ileg), mu=0,3),sqrt(dabs(st_sch_dotp(klab_undec(0,ileg),klab_undec(0,ileg))))
c$$$      enddo

c     st_sch_boost momenta in partonic cm frame
      do ileg=1,nlegreal
         call st_sch_boost(beta_lab_to_cm,klab_undec(0,ileg),kcm_undec(0,ileg))
      enddo

c$$$      write(*,*) 'Undecayed momenta in CM frame'
c$$$      do ileg=1,nlegreal
c$$$         write(*,*) (kcm_undec(mu,ileg), mu=0,3),sqrt(dabs(st_sch_dotp(kcm_undec(0,ileg),kcm_undec(0,ileg))))
c$$$      enddo

c     st_sch_boost 3rd and 'non-FKS-parton' momentum in their CM frame
...
c     wwidth and twidth are fixed st_sch_values (they don't change on an
...
c     generate top virtuality (offshellness)
      m678_2=virt2(topmass_pow,topwidth_pow,st_sch_random())
...
c     generation of st_sch_random decay phase space.
...
c     upon m67.
      m67_2=virt2(wmass_pow,wwidth_pow,st_sch_random())
...
c     extreme (large) st_sch_values of m67 seem to cause problems to herwig; avoid them!
...
c         write(*,*) 'extreme value for m67: ',sqrt(m67_2),' regenerate'
      ...
      m68_2=m68min_2 +st_sch_random()*(m68max_2-m68min_2)
...
c     st_sch_boost the new 3rd and recth momenta back in the CM frame,
...
c     3rec system (i.e. the 3 momentum of the radiated parton)
     ...
     $        /E3rec_cm  !:!
      enddo
      do ileg=3,4 !:!
       call st_sch_boost(beta_3reccm_to_cm,k3reccm_dec(0,ileg),kcm_dec(0,ileg))
      enddo
...
c     Jacobian: sqrt(tau) * (m3rec/E3rec)  in CM frame
     ...
     $2d0*st_sch_dotp(k3reccm_undec(0,3),k3reccm_undec(0,4))) !:!
...
c     It can be argued that other choices are more appropriate.
      ...
      do ileg=1,2
         ...
         if(ileg.eq.1) then
            call st_sch_pdfcall(1,xpdf(1),pdf1_und)
            call st_sch_pdfcall(1,xpdf_dec(1),pdf1_dec)
            if(pdf1_und(flav(1)).ne.0) then
               cfac=cfac 
     $              *pdf1_dec(flav(1))/pdf1_und(flav(1))
            endif
         elseif(ileg.eq.2) then
            call st_sch_pdfcall(2,xpdf(2),pdf2_und)
            call st_sch_pdfcall(2,xpdf_dec(2),pdf2_dec)
...
c     hit-and-miss rule, as in POWHEG-hvq
     ...
     $           cfac/cfacbound,' new PS&LUM upper bound is = ',cfac
      ...
      if(cfacbound*st_sch_random().gt.cfac) then
...
c     Fill the array klab_dec, and exit from this routine
      ...
      if(tdecaymode.eq.0) then
         do ileg=1,nlegreal
            call st_sch_boost(beta_cm_to_lab,kcm_undec_off(0,ileg),klab_dec(0,ileg))
         enddo
...
c     6-> e, 7->ve, 8->b
      call st_sch_build_decay_mom(kcm_undec_off(0,3),m67_2,m68_2,m6,m7,m8,
     $     k3cm_dec)
...
c     check mom.cons. and invariant masses
      if(nup.eq.nlegborn) then
c$$$  call st_sch_check_kinematics(kbcm_mad,nlegborn+2)
         do mu=0,3
            diff=0d0
            do ileg=1,2
      ...
      elseif(nup.eq.nlegreal) then
c$$$  call st_sch_check_kinematics(krcm_mad,nlegreal+2)
         do mu=0,3
            diff=0d0
            do ileg=1,2
...
c     been already set by the call to st_sch_my_setpara before the
...
c     udx
         if ((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(2)).eq.1)) then
            call st_sch_Sudx_epvebbx(kbcm_mad,amp2mad)
c     dxu
         elseif ((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(2)).eq.2)) then
            call st_sch_Sdxu_epvebbx(kbcm_mad,amp2mad)   
         else
...
c     gdx -> tuxdx
            if((three_ch(idup_loc(2)).eq.1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call st_sch_Sgdx_epvebuxbx_S(krcm_mad,amp2mad)
c     gu -> tddx
            elseif((three_ch(idup_loc(2)).eq.2).and.
     #(three_ch(idup_loc(4)).eq.-1)) then
               call st_sch_Sgu_epvebdbx_S(krcm_mad,amp2mad)
            else
...
c     dxg -> tuxdx
            if((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call st_sch_Sdxg_epvebuxbx_S(krcm_mad,amp2mad)
c     ug -> tddx
            elseif((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(4)).eq.-1)) then
               call st_sch_Sug_epvebdbx_S(krcm_mad,amp2mad)
            else
...
c     udx
            if ((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(2)).eq.1)) then
               call st_sch_Sudx_epvebbxg(krcm_mad,amp2mad)
c     dxu
            elseif ((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(2)).eq.2)) then
               call st_sch_Sdxu_epvebbxg(krcm_mad,amp2mad)
            else
...
c     Amplitude squared for (top decay * w decay)
     ...
     #(wdecamp(m67_2)/sthw2_pow)
      ...
      if(ini.eq.0) then
         nubound=st_sch_powheginput('nubound')
...
c     is larger (3000 is an empyrical value)
 ...
 3       continue
      ...
      if(rat.gt.boundnorm) then
         call  st_sch_increasecnt('topdecay upper bound violations')
         if(verbose) then
     ...
     $           kn_csitilde,kn_y
      ...
      if(boundnorm*st_sch_random().gt.rat) then
         call st_sch_increasecnt('topdecay vetoed configurations')
         goto 1
      endif
...
c     If we are here, kinematics has been accepted: we can save the momenta
      ...
      enddo
      do ileg=nlegreal+1,nlegreal+3
         call st_sch_boost(beta_cm_to_lab,k3cm_dec(0,ileg),klab_dec(0,ileg))
      enddo

c$$$      call st_sch_check_kinematics(klab_dec,nlegreal+3)
      end


      subroutine st_sch_pickwdecay(iw1,mdecw1,iw2,mdecw2,totbr)
...
c     save
      ...
      if(ini.eq.0.or.iw1.eq.-1000) then
         ini=1
c     on first run look for decay mode in st_sch_powheginput
         imode=st_sch_powheginput('topdecaymode')
         if(imode.le.0) then
            write(*,*) 'Invalid value for tdecaymode, in st_sch_pickwdecay'
            call exit(1)
         endif
...
c     load from input card the branching t->(b l vl) (only one lepton flavour)
         ebr=st_sch_powheginput('tdec/elbranching')
...
c     assigned here. The 3 light quarks are assumed massless.
         mass(11)=st_sch_powheginput('#tdec/emass')
         if(mass(11).lt.0) mass(11)=0.000511
         mass(13)=st_sch_powheginput('#tdec/mumass')
         if(mass(13).lt.0) mass(13)=0.1056
         mass(15)=st_sch_powheginput('#tdec/taumass')
         ...
         mass(2)=0
         mass(3)=0
c         mass(5)=st_sch_powheginput('tdec/bmass')
         mass(5)=st_sch_powheginput('#bottomthr')
         if (mass(5).lt.0d0) mass(5)=5.
c         mass(4)=st_sch_powheginput('tdec/cmass')
         mass(4)=st_sch_powheginput('#charmthr')
...
c     now we have j decay mode
      if(j.gt.5) then
         write(*,*) 'Error in st_sch_pickwdecay, j',r,totprbs
         call exit(1)
      endif
...
c     !: This has not been implemented yet
      do j=1,2
         if(abs(iwa(j)).eq.1) then
            if(st_sch_random().lt.sin2cabibbo) then
               iwa(j)=sign(3,iwa(j))
            endif
      ...
      subroutine st_sch_build_decay_mom(xktop,m2eve,m2eb,me,mve,mb,xk3_dec)
...
c     st_sch_boost top in its rest frame
      call st_sch_boost(beta_to_trest,ktop,ktop_trest)
      m2t=ktop_trest(0)**2

c     check
      if(check) then
         if(dabs(st_sch_dotp(ktop,ktop)-m2t).gt.tiny) then
            write(*,*) 'Error in st_sch_build_decay_mom'
            stop
         endif
      endif
...
c     generate st_sch_random directed b momentum in t rest frame
      call st_sch_rn3vec(k3_trest(1,3),pb_trest)
      k3_trest(0,3)=eb_trest

c     st_sch_boost from w rest frame to t rest frame
...
c$$$      call st_sch_rn3vec(k2_wrest(1,1),pe_wrest)
c$$$      k2_wrest(0,1)=ee_wrest
ccccccccccccccccccccccccccc
ccccccccccccccccccccccccccccccccccccccccc
c     new version: relative orientations of 3 momenta are completely fixed by
c     chosen energies in t rest frame (apart from a st_sch_random azimuth in w rest frame)
      call st_sch_theta4vec(k2_wrest(0,1),ee_wrest,pe_wrest,ee_trest,
     #beta_wr_to_tr)
cccccccccccccccccccccccccccccccccccccccccc
...
c     st_sch_boost W decay products with W velocity in t rest frame
c     (its momentum is opposite to the b momentum)
      do ileg=1,2
         call st_sch_boost(beta_wr_to_tr,k2_wrest(0,ileg),
     #k3_trest(0,ileg))
      enddo
     ...
     #(dabs(k3_trest(0,2)-eve_trest).gt.tiny)) then
         ...
         if(dabs(m2eve-2d0*st_sch_dotp(k3_trest(0,1),k3_trest(0,2))).gt.tiny)
     #then
            write(*,*) 'error in build_dec_mom: m2eve'
            stop
         endif
         if(dabs(m2eb-2d0*st_sch_dotp(k3_trest(0,1),k3_trest(0,3))).gt.tiny)
     #then
...
c     st_sch_boost them back in original frame (all decay products along top velocity)
      do ileg=1,3
         call st_sch_boost(beta_to_trest_inv,k3_trest(0,ileg),
     #xk3_dec(0,ileg))
      enddo
      ...
      subroutine st_sch_put_on_mass_shell(tdecayflag,MC_mass,xklab,xklab_os)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
      include 'src/Lib/st_sch/LesHouches.h'
      include 'src/Lib/st_sch/PhysPars.h'
...
c     local
     ...
     #beta_wr_to_tr(3)
      ...
      integer mu,ileg
c     external
      real *8 st_sch_dotp,st_sch_norm,st_sch_random
      external st_sch_dotp,st_sch_norm,st_sch_random
...
c     event left massless
      if(MC_mass.eq.0) then
         ...
         enddo
         if(tdecayflag) then
            call st_sch_pickwdecay(iw1,mdecw1,iw2,mdecw2,dummy)
c     e
            decids(1)=iw1
c     ve
            decids(2)=iw2
c     b
      ...
      elseif(nup.eq.nlegborn) then
         ...
         write(*,*) 'NUP ERROR in st_sch_put_on_mass_shell '
      endif
...
ccccccccccccccccccccccccccccccccccc
c     st_sch_boost lab momenta in cm frame
ccccccccccccccccccccccccccccccccccc
      betacm(1)=0d0
      betacm(2)=0d0
      ...
      enddo
      do ileg=1,nlegreal
         call st_sch_boost(betacm_inv,klab(0,ileg),kcm(0,ileg))
      enddo
...
c     in their original cm frame. Then st_sch_boost back to the lab frame.
...
c$$$c     st_sch_boost back in the lab frame
c$$$         do ileg=1,2
c$$$            call st_sch_boost(betacm,kcm(0,ileg),klab(0,ileg))
c$$$         enddo
c$$$c--   
c$$$c     check that partons are on shell and that ycm and shat are
c$$$c     unchanged
c$$$         if(check) then
c$$$            if(dabs(st_sch_dotp(klab(0,1),klab(0,1))-m(1)**2).gt.tiny) then
c$$$               write(*,*) 'Error in put_on_mshell: check on m(1)'
c$$$               stop
c$$$            endif
c$$$            if(dabs(st_sch_dotp(klab(0,2),klab(0,2))-m(2)**2).gt.tiny) then
...
c     momenta and put them on mass shell.
      ...
      if(nup.eq.4) then
         if(st_sch_norm(kcm(0,4)).lt.(m(4)*(m(4)/2+m(3))/(m(3)+m(4)))) then
            xklab_os(0,1)=-100d0
            return
         endif
      elseif(nup.eq.5) then
         ...
         enddo
         m34=sqrt(st_sch_dotp(k34cm,k34cm))
         if(st_sch_norm(kcm(0,5)).lt.((2d0*m34*m(5)+m(5)**2)/2d0/(m34+m(5))))
     #then
            xklab_os(0,1)=-100d0
            return
         endif
...
c     be changed.
      if(delta.lt.delta_thr) then
         ...
         call st_sch_increasecnt('events where delta < delta_thr ')
...
c     st_sch_boost back in the lab frame (with the old st_sch_boost vector)
         do ileg=1,2
            call st_sch_boost(betacm,kcm(0,ileg),klab(0,ileg))
            if(verbose) write(*,*)
...
c     3-momentum of k3 and k4 in their cm frame, with masses included
     ...
     #m34**2-2d0*(m(3)**2+m(4)**2)+(m(3)**2-m(4)**2)**2/m34**2)
      ...
      do mu=1,3
         dir(mu)=0d0
      enddo
c     st_sch_boost from cm frame to k3+k4 rest frame (and inverse) 
      do mu=1,3
         beta_cm_to_34(mu)=-k34cm(mu)/k34cm(0)
         beta_34_to_cm(mu)=-beta_cm_to_34(mu)
      enddo
c     st_sch_boost k3, k4 and k34 from cm to rest frame of (k3+k4)
      do ileg=3,4
         call st_sch_boost(beta_cm_to_34,kcm(0,ileg),kleg_34(0,ileg))
      enddo
      call st_sch_boost(beta_cm_to_34,k34cm(0),ktmp(0))
      if(check) then
     ...
     #(dabs(ktmp(2)).gt.tiny).or.(dabs(ktmp(3)).gt.tiny)) then
         ...
         if(dabs(st_sch_dotp(k34cm,k34cm)-ktmp(0)**2).gt.tiny) then
      ...
      do mu=1,3
         dir(mu)=0.5d0* (kleg_34(mu,3)/st_sch_norm(kleg_34(0,3)) - 
     #kleg_34(mu,4)/st_sch_norm(kleg_34(0,4)))
      enddo
      ...
      do ileg=3,4
         call st_sch_boost(beta_34_to_cm,kleg_34(0,ileg),kcm(0,ileg))
         if(dabs(st_sch_dotp(kcm(0,ileg),kcm(0,ileg))-m(ileg)**2).gt.tiny) then
            write(*,*) 'Error in put_on_mshell: #5', ileg
            write(*,*) m(ileg)**2,st_sch_dotp(kcm(0,ileg),kcm(0,ileg)),
     #st_sch_dotp(kleg_34(0,ileg),kleg_34(0,ileg))
            stop
         endif
      enddo
...
!      if(nup.eq.4) then
     ...
     $           2d0*st_sch_dotp(xklab_os(0,1),xklab_os(0,2)),
     $           175**2+2d0*st_sch_dotp(xklab_os(0,3),xklab_os(0,4))
            do ileg=1,2
...
c     st_sch_boost decay products in the old top rest frame, i.e. in the t rest frame where
c     decay products are massless.
         do ileg=nlegreal+1,nlegreal+3
            call st_sch_boost(beta_cm_to_tr_old,kcm(0,ileg),k3tr(0,ileg))
         enddo
c     now k3tr(mu,6)+k3tr(mu,7)+k3tr(mu,8)=(mt,0,0,0)
         ...
         mt=sqrt(st_sch_dotp(kcm(0,3),kcm(0,3)))
         m67=sqrt(2d0*st_sch_dotp(kcm(0,6),kcm(0,7)))
         Eprime=(mt**2+m(8)**2-m67**2)/2d0/mt
         kprime=sqrt(dabs(Eprime**2-m(8)**2))
         k67_mod=st_sch_norm(k3tr(0,8))
         do mu=1,3
         ...
         do ileg=nlegreal+1,nlegreal+2
            call st_sch_boost(beta_tr_to_wr,k3tr(0,ileg),k2wr(0,ileg))
         enddo
         do mu=1,3
            dir(mu)=0.5d0* (k2wr(mu,6)/st_sch_norm(k2wr(0,6)) - 
     #k2wr(mu,7)/st_sch_norm(k2wr(0,7)))
         enddo
         tmp=sqrt(dir(1)**2+dir(2)**2+dir(3)**2)
         do mu=1,3
            dir(mu)=dir(mu)/tmp
     ...
     #m67**2-2d0*(m(6)**2+m(7)**2)+(m(6)**2-m(7)**2)**2/m67**2)
         ...
         do ileg=nlegreal+1,nlegreal+2
            call st_sch_boost(beta_wr_to_tr,k2wr(0,ileg),k3tr(0,ileg))
         enddo

         do ileg=nlegreal+1,nlegreal+3
            call st_sch_boost(beta_tr_to_cm,k3tr(0,ileg),kcm(0,ileg))
         enddo
      ...
      subroutine st_sch_rn3vec(vec,r)
c     !: originally taken from POWHEG-hvq      
c Generates a 3d vector in unit sphere
      implicit none
      real * 8 vec(3),r,r02,st_sch_norm
      integer j
c generate in unit cube -1<x,y,z<1
 1    call st_sch_RM48(vec,3)
      do j=1,3
         vec(j)=1-2*vec(j)
      enddo
c generate in unit sphere by hit and miss
      ...
      if(r02.gt.1) goto 1
c normalize
      st_sch_norm=r/sqrt(r02)
      do j=1,3
         vec(j)=vec(j)*st_sch_norm
      enddo
      end


      subroutine st_sch_theta4vec(vec,en,st_sch_norm,en_prime,beta)
c     given the st_sch_boost velocity beta, energy and spatial st_sch_norm in
c     the boosted frame (en,st_sch_norm) and energy in the initial frame (en_prime),
...
c     vec azimuth wrt beta direction is cast randomly.
      implicit none
      include 'src/Lib/st_sch/pwhg_math.h'
      real *8 vec(0:3),en,st_sch_norm,en_prime,beta(3)
c     local
...
c     external
      ...
      ctheta=(en_prime/gamma_b-en)/beta_mod/st_sch_norm

      phi=st_sch_random()*2d0*pi
      vec(3)=st_sch_norm*ctheta
      vec(1)=st_sch_norm*sqrt(1d0-ctheta**2)*dcos(phi)
      vec(2)=st_sch_norm*sqrt(1d0-ctheta**2)*dsin(phi)
      vec(0)=en

      thn=dacos(beta(3)/beta_mod)
      phin=datan2(beta(2)/beta_mod,beta(1)/beta_mod)

      call st_sch_rotate(vec,thn,phin,vec_tmp)
      do mu=0,3
         vec(mu)=vec_tmp(mu)
      enddo    
      ...
      subroutine st_sch_preal_to_pborn(p,pprime)
      implicit none
      include 'src/Lib/st_sch/nlegborn.h'
...
c     local
      ...
      subroutine st_sch_rotate(p,th,phi,newp)
...
c     and an angle phi with the x axis
      ...
      subroutine st_sch_check_kinematics(p,nleg)
...
c     external
      ...
      do j=1,nleg
         write(*,*) '\tpart ',j,': ',st_sch_dotp(p(0,j),p(0,j))
      enddo
     ...
     #     pyin-pyout,pzin-pzout 
      ...
      function st_sch_norm(vec)
c     returns spatial st_sch_norm of a 4 vector
      implicit none
      real *8 st_sch_norm
      real *8 vec(0:3)
      st_sch_norm=sqrt(vec(1)**2+vec(2)**2+vec(3)**2)
      end

...
c###################################################### 
      function gg_h_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 gg_h_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
     ...
     #     resreal(maxprocborn),rescoll(maxprocborn)
      ...
      www=www0*hc2*gg_h_finitemtcorr()
      do j=1,ndiminteg-3
         xborn(j)=xx(j)
      enddo
      do j=1,3
...
c set the mt dependency in Born cross section

         call gg_h_setbornmassdep

c evaluate Born cross section with the chosen top gg_h_mass

         call gg_h_allborn
c     sets xscaled, y, phi in kinematics common block
         call gg_h_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call gg_h_btildevirt(resvirt)
            call gg_h_btildecoll(xrad,rescoll,www)
            call gg_h_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         gg_h_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            gg_h_btilde=gg_h_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to gg_h_btildeborn and gg_h_btildevirt can be
...
c     to the NLO gg_h_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c gg_h_btildecoll and gg_h_btildereal take care themselves to invoke the NLO
c gg_h_analysis if required.
            call gg_h_btildecoll(xrad,rescoll,www)
            call gg_h_btildereal(xrad,resreal,www)
         endif
         gg_h_btilde=0
...
c output Born
            ...
            call gg_h_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
               ...
               call gg_h_analysis_driver(totvirt,0)
            endif
c closing call to end a sequence of correlated events in the
c gg_h_analysis routines.
            call gg_h_pwhgaccumup
         endif
c     closing call: accumulate values with correct signs
         gg_h_btilde=0
         call gg_h_adduptotals(results,flst_nborn)
         do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            gg_h_btilde=gg_h_btilde+results(j)
c     Transfer all flavour components of gg_h_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in gg_h_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine gg_h_adduptotals(results,n)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/gg_h_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(gg_h_powheginput('#ubsigmadetails').eq.1) then

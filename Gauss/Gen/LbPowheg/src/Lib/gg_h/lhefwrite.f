...
c...writes initialization information to a les houches events file on unit nlf. 
      subroutine gg_h_lhefwritehdr(nlf)
      implicit none
     ...
     #     version
      ...
      call gg_h_wrtpowheginput(nlf)
      write(nlf,'(a)') 'End of powheg.input content'
      call gg_h_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) 'gg_h_random number generator initialized with: ',
     # iran,' ',n1ran,' ',n2ran
      write(nlf,'(a)') '-->'
      write(nlf,'(a)') '<init>'
 ...
 120  format(1p,3(1x,e12.5),1x,i6)
      ...
      subroutine gg_h_printleshouches
c useful for debugging
      call gg_h_lhefwritev(6)
      end

c...lhefeader(nlf)
c...writes event information to a les houches events file on unit nlf. 
      subroutine gg_h_lhefwritev(nlf)
      implicit none
      integer nlf
      include 'src/Lib/gg_h/LesHouches.h'
      include 'src/Lib/gg_h/pwhg_flg.h'
     ...
     & vtimup(i),spinup(i)
 200  continue
      if(flg_pdfreweight) call gg_h_lhefwritepdfrw(nlf)
      if(flg_debug) call gg_h_lhefwritextra(nlf)
      write(nlf,'(a)')'</event>'      
...
c...writes trailer to a les houches events file on unit nlf. 
      subroutine gg_h_lhefwritetrailer(nlf)
      implicit none
      integer nlf,iran,n1ran,n2ran
      write(nlf,'(a)') '</LesHouchesEvents>'
c     save last gg_h_random number
      call gg_h_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) '#gg_h_random number generator exit values: ',
     # iran,' ',n1ran,' ',n2ran
      end

      subroutine gg_h_lhefwritextra(nlf)
      implicit none
      include 'src/Lib/gg_h/LesHouches.h'
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      ...
      call gg_h_getcurrentrandom(iran,n1ran,n2ran)
      write(nlf,*) '# ', iran,' ',n1ran,' ',n2ran,
     #     "    previous event's gg_h_random seeds "
      write(nlf,'(a)') '# End extra-info-previous-event'
      end

      subroutine gg_h_lhefwritepdfrw(nlf)
      implicit none
      integer nlf
      integer id1,id2
      real * 8 x1,x2,xf1,xf2,xmufact
      call gg_h_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)

      ...
      integer i1,n1,n2
      call gg_h_init_flsttag
      flg_debug=.false.
      if(gg_h_powheginput("#flg_debug").eq.1) flg_debug=.true.
c whether to output negative weights or not
      flg_withnegweights=.false.
      if(gg_h_powheginput("#withnegweights").eq.1)flg_withnegweights=.true.
c See if we have weighted events
      flg_weightedev=.false.
      if(gg_h_powheginput("#bornsuppfact").gt.0) flg_weightedev=.true.
      if(gg_h_powheginput("#ptsupp").gt.0) then
...
c     amplitudes, for Born, real and virtual contributions
      flg_smartsig=.true.
      if(gg_h_powheginput("#smartsig").eq.0) flg_smartsig=.false.
c     if true also counterterm are output in NLO tests (default)
      flg_withsubtr=.true.
      if(gg_h_powheginput("#withsubtr").eq.0) flg_withsubtr=.false.
...
c     is required
      flg_withdamp=.false.
      if((gg_h_powheginput("#withdamp").eq.1).or.
     $  (gg_h_powheginput("#hfact").gt.0d0))   flg_withdamp=.true.
c     If set do only the Born term
      flg_bornonly=.false.
      if (gg_h_powheginput("#bornonly").eq.1) flg_bornonly=.true.
c     initialize gg_h_random number sequence
      i1=gg_h_powheginput('#iseed')
      if (i1.lt.0) i1=0
      n1=gg_h_powheginput('#rand1')
      if (n1.lt.0) n1=0
      n2=gg_h_powheginput('#rand2')
      if (n2.lt.0) n2=0
c select which upper bounding function form
      rad_iupperisr=gg_h_powheginput("#iupperisr")
      if(rad_iupperisr.lt.0) rad_iupperisr=1
      rad_iupperfsr=gg_h_powheginput("#iupperfsr")
      if(rad_iupperfsr.lt.0) rad_iupperfsr=2
c info on pdf for each event
      flg_pdfreweight=.false.
      if(gg_h_powheginput("#pdfreweight").eq.1)flg_pdfreweight=.true.
c
      call gg_h_setrandom(i1,n1,n2)
...
c     inside the user-defined subroutine gg_h_init_processes
      lprup(1)=10001

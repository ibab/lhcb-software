      subroutine gg_h_bornzerodamp(alr,r0,rc,rs,dampfac)
...
c the real contribution to implement Born zero suppression
      ...
      integer alr
      real * 8 r0,rc,rs,dampfac,h,gg_h_pwhg_pt2,pt2,gg_h_powheginput
      logical ini
      data ini/.true./
      save ini,h
      external gg_h_pwhg_pt2,gg_h_powheginput
      if(ini) then
         h=gg_h_powheginput("#hfact")
         if(h.gt.0) then
            write(*,*)'***************************************'
...
c      write(*,*) ' gg_h_bornzerodamp: r0=',r0,'  rc=',rc,'  rs=',rs,
c     # ' dampfac=',dampfac
      end

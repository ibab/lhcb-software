...
*
*
c# 1 "/usr/local/home/video/cernlib/2005/include/gg_h_gen/pilot.h" 1
...
c# 40 "/usr/local/home/video/cernlib/2005/include/gg_h_gen/pilot.h"

c# 57 "/usr/local/home/video/cernlib/2005/include/gg_h_gen/pilot.h"
...
c# 10 "dilog64.F" 2
      ...
      FUNCTION gg_h_DDILOG(X)
c# 1 "/usr/local/home/video/cernlib/2005/include/gg_h_gen/imp64.inc" 1
*
...
*
* $Id: gg_h_DZERO.F,v 1.1.1.1 1996/02/15 17:49:07 mclareni Exp $
*
* $Log: gg_h_DZERO.F,v $
...
*
      ...
      SUBROUTINE gg_h_DZERO(A,B,X0,R,EPS,MXF,F)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
*
...
*
    ...
    4 R=EE
      FF=F(X0,3)
      RETURN
    5 CALL gg_h_KERMTR('C205.1',LGFILE,MFLAG,RFLAG)
      IF(MFLAG) THEN
       IF(LGFILE .EQ. 0) WRITE(*,100)
       IF(LGFILE .NE. 0) WRITE(LGFILE,100)
      END IF
      IF(.NOT.RFLAG) CALL gg_h_ABEND
      R=-2*(XB-XA)
      X0=0
      RETURN
    6 CALL gg_h_KERMTR('C205.2',LGFILE,MFLAG,RFLAG)
      IF(MFLAG) THEN
  ...
  100 FORMAT(1X,'***** CERN C205 gg_h_DZERO ... F(A) AND F(B)',
     1          ' HAVE THE SAME SIGN')
  101 FORMAT(1X,'***** CERN C205 gg_h_DZERO ... TOO MANY FUNCTION CALLS')
      END
...
c# 1 "gg_h_KERSET.F"
*
* $Id: gg_h_KERSET.F,v 1.1.1.1 1996/02/15 17:48:35 mclareni Exp $
*
* $Log: gg_h_KERSET.F,v $
...
c# 10 "gg_h_KERSET.F" 2
          SUBROUTINE gg_h_KERSET(ERCODE,LGFILE,LIMITM,LIMITR)
                    PARAMETER(KOUNTE  =  27)
  ...
  14         CONTINUE
          RETURN
          ENTRY gg_h_KERMTR(ERCODE,LOG,MFLAG,RFLAG)
          LOG  =  LOGF
          DO 20     I  =  1, KOUNTE
...
1000      FORMAT(' KERNLIB LIBRARY ERROR. ' /
     +           ' ERROR CODE ',A6,' NOT RECOGNIZED BY gg_h_KERMTR',
...
c# 1 "gg_h_RM48.F"
*
* $Id: gg_h_RM48.F,v 1.2 1996/12/12 16:32:06 cernlib Exp $
*
* $Log: gg_h_RM48.F,v $
...
c# 13 "gg_h_RM48.F" 2
      SUBROUTINE gg_h_RM48(RVEC,LENV)
C     Double-precision version of
C Universal gg_h_random number generator proposed by Marsaglia and Zaman
...
C   Input and output entry points: gg_h_RM48IN, RM48UT.
C!!! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C!!!  Calling sequences for gg_h_RM48:                                    ++
C!!!      CALL gg_h_RM48 (RVEC, LEN)     returns a vector RVEC of LEN     ++
C!!!                   64-bit gg_h_random floating point numbers between  ++
C!!!                   zero and one.                                 ++
C!!!      CALL gg_h_RM48IN(I1,N1,N2)   initializes the generator from one ++
...
C!!!                    output by gg_h_RM48UT)                            ++ 
C!!!      CALL gg_h_RM48UT(I1,N1,N2)   outputs the value of the original  ++
C!!!                  gg_h_SEED and the two number counts, to be used     ++
...
C for 32-bit machines, use IMPLICIT DOUBLE PRECISION
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION RVEC(*)
      COMMON/gg_h_R48ST1/U(97),C,I97,J97
...
C
C        Default initialization. User has called gg_h_RM48 without RM48IN.
      IJKL = 54217137
      NTOT = 0
      NTOT2 = 0
      KALLED = 0
      GO TO 1
C
      ENTRY      gg_h_RM48IN(IJKLIN, NTOTIN,NTOT2N)
C         Initializing routine for gg_h_RM48, may be called before
C         generating pseudorandom numbers with gg_h_RM48.   The input
...
C          always come here to initialize
    1 CONTINUE
      ...
      K = MOD(KL/169, 178) + 1
      L = MOD(KL, 169)
      WRITE(6,'(A,I10,2X,2I10)') ' gg_h_RM48 INITIALIZED:',IJKL,NTOT,NTOT2
...
C            (NTOT2*MODCNS + NTOT) gg_h_random numbers
      DO 45 LOOP2= 1, NTOT2+1
      NOW = MODCNS
      IF (LOOP2 .EQ. NTOT2+1)  NOW=NTOT
      IF (NOW .GT. 0)  THEN
      WRITE(6,'(A,I15)') ' gg_h_RM48IN SKIPPING OVER ',NOW
          DO 40 IDUM = 1, NTOT
          UNI = U(I97)-U(J97)
...
C
C          Normal entry to generate LENV gg_h_random numbers
   50 CONTINUE
      DO 100 IVEC= 1, LENV
      UNI = U(I97)-U(J97)
      IF (UNI .LT. ZERO)  UNI=UNI+ONE
...
C           Entry to output current status
      ENTRY gg_h_RM48UT(IJKLUT,NTOTUT,NTOT2T)
      IJKLUT = IJKL
      NTOTUT = NTOT
      NTOT2T = NTOT2
      RETURN
...
c# 1 "gg_h_ABEND.F"
*
* $Id: gg_h_ABEND.F,v 1.1.1.1 1996/02/15 17:50:37 mclareni Exp $
*
* $Log: gg_h_ABEND.F,v $
...
c# 10 "gg_h_ABEND.F" 2
      ...
      SUBROUTINE gg_h_ABEND
C
C CERN PROGLIB# Z035    gg_h_ABEND           .VERSION KERNFOR  4.31  911111
C ORIG.  8/02/88  JZ
C
...
c# 1 "gg_h_LENOCC.F"
*
* $Id: gg_h_LENOCC.F,v 1.1.1.1 1996/02/15 17:49:49 mclareni Exp $
*
* $Log: gg_h_LENOCC.F,v $
...
c# 10 "gg_h_LENOCC.F" 2
      FUNCTION gg_h_LENOCC (CHV)
C
C CERN PROGLIB# M507    gg_h_LENOCC          .VERSION KERNFOR  4.21  890323
...
c# 1 "gg_h_MTLSET.F"
*
* $Id: gg_h_MTLSET.F,v 1.1.1.1 1996/04/01 15:02:53 mclareni Exp $
*
* $Log: gg_h_MTLSET.F,v $
...
c# 10 "gg_h_MTLSET.F" 2
      SUBROUTINE gg_h_MTLSET(ERC,NLG,MXM,MXR)
...
c# 175 "gg_h_MTLSET.F"
   ...
   14 CONTINUE
      ...
      ENTRY gg_h_MTLMTR(ERC,MLG,LMF,LRF)
   ...
   21 LMF=KNTM(I) .GE. 1
      ...
      ENDIF
      RETURN
  100 FORMAT(7X,'***** CERN N002 gg_h_MTLSET ... ERROR N002: ',
     1'ERROR CODE ',A6,' NOT RECOGNIZED BY ERROR MONITOR. RUN ABORTED.')
  101 FORMAT(7X,'***** CERN N002 gg_h_MTLSET ... ERROR NOO2.1: ',
...
c# 1 "gg_h_MTLPRT.F"
*
* $Id: gg_h_MTLPRT.F,v 1.1.1.1 1996/04/01 15:02:52 mclareni Exp $
*
* $Log: gg_h_MTLPRT.F,v $
...
c# 10 "gg_h_MTLPRT.F" 2
      SUBROUTINE gg_h_MTLPRT(NAME,ERC,TEXT)
      CHARACTER*(*) NAME,ERC,TEXT
      LOGICAL LMF,LRF

      IF(ERC(5:6).NE.'.0') THEN
        CALL gg_h_MTLMTR(ERC,MLG,LMF,LRF)
      ELSE
        LMF=.TRUE.
        LRF=.FALSE.
      ENDIF
...
c# 1 "gg_h_PERMU.F"
*
* $Id: gg_h_PERMU.F,v 1.1.1.1 1996/04/01 15:02:57 mclareni Exp $
*
* $Log: gg_h_PERMU.F,v $
...
c# 10 "gg_h_PERMU.F" 2
      SUBROUTINE gg_h_PERMU(IA,N)
C
      CHARACTER*(*) NAME
      PARAMETER(NAME='gg_h_PERMUT')
C
      DIMENSION IA(*)
      PARAMETER (IFD = 12)
   ...
   17  IB=IA(K-1)
      ...
      ENTRY gg_h_PERMUT(NRP,N,IA)

      IF(N .LE. 0) RETURN
      IF(N .GT. IFD) THEN
       WRITE(ERRTXT,101) N
       CALL gg_h_MTLPRT(NAME,'V202.1',ERRTXT)
      ELSEIF(NRP .GT. IFCT(N)) THEN
       IA(1)=0
       CALL gg_h_MTLPRT(NAME,'V202.2',
   ...
   22  CONTINUE
      ...
      ENTRY gg_h_COMBI(IA,N,J)

      IF(N .LE. 0 .OR. J .LE. 0) RETURN
      IF(J .GT. N) THEN
       WRITE(ERRTXT,103) J,N
       CALL gg_h_MTLPRT(NAME,'V202.3',ERRTXT)
...
c# 1 "gg_h_SORTZV.F"
*
* $Id: gg_h_SORTZV.F,v 1.1.1.1 1996/02/15 17:49:50 mclareni Exp $
*
* $Log: gg_h_SORTZV.F,v $
...
c# 10 "gg_h_SORTZV.F" 2
      SUBROUTINE gg_h_SORTZV (A,INDEX,N1,MODE,NWAY,NSORT)
C
C CERN PROGLIB# M101    gg_h_SORTZV          .VERSION KERNFOR  3.15  820113
C ORIG. 02/10/75
C
...
C
    2 IF (N.EQ.1)            RETURN
      IF (MODE)    10,20,30
   10 CALL gg_h_SORTTI (A,INDEX,N)
      GO TO 40
C
   20 CALL gg_h_SORTTC(A,INDEX,N)
      GO TO 40
C
   30 CALL gg_h_SORTTF (A,INDEX,N)
C
   ...
   50 RETURN
      END
*     ========================================
      SUBROUTINE gg_h_SORTTF (A,INDEX,N1)
C
      DIMENSION A(N1),INDEX(N1)
C
      N = N1
   ...
   12 INDEX (1) = I3
      ...
      END
*     ========================================
      SUBROUTINE gg_h_SORTTI (A,INDEX,N1)
C
      INTEGER A,AI
      DIMENSION A(N1),INDEX(N1)
C
   ...
   12 INDEX (1) = I3
      ...
      END
*     ========================================
      SUBROUTINE gg_h_SORTTC (A,INDEX,N1)
C
      INTEGER A,AI
      DIMENSION A(N1),INDEX(N1)
C
    ...
    1 I2 = I3/2
      IF (I2) 3,3,2
    2 I22 = INDEX(I2)
      IF(gg_h_ICMPCH(AI,A(I22)))3,3,21
   21 INDEX (I3) = I22
      I3 = I2
      GO TO 1
    3 INDEX (I3) = I33
    ...
    6 I2 = I1 + I1
      ...
      IF (I2-N) 7,9,11
    7 I222 = INDEX (I2+1)
      IF (gg_h_ICMPCH(A(I22),A(I222))) 8,9,9
    8 I2 = I2+1
      I22 = I222
    9 IF (gg_h_ICMPCH(AI,A(I22))) 10,11,11
   10 INDEX(I1) = I22
      I1 = I2
      GO TO 6
   11 INDEX (I1) = I3
   ...
   12 INDEX (1) = I3
      ...
      END
*     ========================================
      FUNCTION gg_h_ICMPCH(IC1,IC2)
C     FUNCTION TO COMPARE TWO 4 CHARACTER EBCDIC STRINGS - IC1,IC2
C     gg_h_ICMPCH=-1 IF HEX VALUE OF IC1 IS LESS THAN IC2
C     gg_h_ICMPCH=0  IF HEX VALUES OF IC1 AND IC2 ARE THE SAME
C     gg_h_ICMPCH=+1 IF HEX VALUES OF IC1 IS GREATER THAN IC2
      I1=IC1
      I2=IC2
...
c **********  THIS IS NOT A CERN ROUTINE!!  ***************
      logical function gg_h_pwhg_isfinite(x)
      implicit none
      real * 8 x
c According to ieee standards, a NaN is the only real not
c satisfying x.eq.x.
      if (.not.(x.eq.x)) then
         gg_h_pwhg_isfinite = .false.
         call gg_h_increasecnt('NaN exception')
         return
      endif
c Put constraint to avoid denormals
      if(x.gt.1.or.x.lt.-1) then
         if (1/x.eq.0) then
            gg_h_pwhg_isfinite = .false.
            call gg_h_increasecnt('Inf exception')
            return
         endif
      endif
      gg_h_pwhg_isfinite = .true.
      end


      subroutine gg_h_increasecnt(string)
      implicit none
      character *(*) string
      integer maxnum
      ...
      integer ncounters
      common/gg_h_ccounters/keywords,counters,ncounters
      integer ini,j
      data ini/0/
      save ini
      if(ini.eq.0) then
...
c not found
      if(ncounters.eq.maxnum) then
         write(*,*) 'ERROR: gg_h_increasecnt too many counters requested'
         stop
      endif
      ...
      subroutine gg_h_resetcnt(string)
      implicit none
      character *(*) string
      integer maxnum
      parameter (maxnum=100)
...
c not found
      ...
      subroutine gg_h_incrcntrs(string,k)
      implicit none
      integer k
      character *(*) string
      character * 20 line
      ...
      line(17:)=' '
      write(line(17:),'(i4)') k
      call gg_h_increasecnt(line)
      end

      subroutine gg_h_incrcntrs2(string,j,k)
      implicit none
      integer j,k
      character *(*) string
      character * 20 line

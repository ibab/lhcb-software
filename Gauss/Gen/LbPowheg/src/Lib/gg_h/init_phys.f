      ...
      include 'src/Lib/gg_h/pwhg_physpar.h'
      character * 5 scheme
      character * 3 gg_h_whichpdfpk
      real * 8 gg_h_powheginput
      integer iorder,iret,iun,j
      external gg_h_whichpdfpk,gg_h_powheginput
...
c End initialization of common block defaults.
      pdf_ih1=gg_h_powheginput('ih1')
      pdf_ih2=gg_h_powheginput('ih2')
      if(gg_h_whichpdfpk().eq.'lha') then
         pdf_ndns1=gg_h_powheginput('lhans1')
         pdf_ndns2=gg_h_powheginput('lhans2')
      elseif(gg_h_whichpdfpk().eq.'mlm') then
         pdf_ndns1=gg_h_powheginput('ndns1')
         pdf_ndns2=gg_h_powheginput('ndns2')
      else
         write(*,*) ' unimplemented pdf package',gg_h_whichpdfpk()
         stop
      endif
      if(pdf_ndns1.ne.pdf_ndns2) then
         st_lambda5MSB=gg_h_powheginput('QCDLambda5')
      else
         call gg_h_genericpdfpar(pdf_ndns1,pdf_ih1,st_lambda5MSB,
     1                      scheme,iorder,iret)
      ...
      kn_beams(0,1)=gg_h_powheginput('ebeam1')
      kn_beams(0,2)=gg_h_powheginput('ebeam2')
      kn_beams(1,1)=0
      kn_beams(1,2)=0
      kn_beams(2,1)=0
      kn_beams(2,2)=0
...
c generation cut: see gg_h_gen_born_phsp.f
      kn_ktmin=gg_h_powheginput("#bornktmin")
...
c thresholds 
      rad_ptsqmin=gg_h_powheginput('#ptsqmin')
      if(rad_ptsqmin.lt.0) rad_ptsqmin=0.8d0
      rad_charmthr2=gg_h_powheginput('#charmthr')
      if(rad_charmthr2.lt.0) rad_charmthr2=1.5d0
      rad_charmthr2=rad_charmthr2**2
      rad_bottomthr2=gg_h_powheginput('#bottomthr')
      if(rad_bottomthr2.lt.0) rad_bottomthr2=5d0
      rad_bottomthr2=rad_bottomthr2**2
c scale factors
      st_renfact=gg_h_powheginput('#renscfact')
      st_facfact=gg_h_powheginput('#facscfact')
...
c initialize Lambda values for radiation
      call gg_h_init_rad_lambda
c
      call gg_h_init_processes

      call gg_h_init_couplings

c initialize number of singular regions
      rad_nkinreg=1+(nlegborn-flst_lightpart+1)
      call gg_h_genflavreglist


      dbg_softtest=.true.
      dbg_colltest=.true.
      if(flg_withdamp) then
         write(*,*) ' no gg_h_soft tests if withdamp is set'
         dbg_softtest=.false.
      endif
      if(flg_bornonly) then
         write(*,*) ' no gg_h_soft and coll. tests if bornonly is set'
         dbg_softtest=.false.
         dbg_colltest=.false.
      endif

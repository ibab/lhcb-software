      subroutine gg_h_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 gg_h_powheginput,gg_h_random
      external gg_h_powheginput,gg_h_random      
      call gg_h_zerohistnorms
      loaded=.false.
      nubound=gg_h_powheginput('nubound')
      rad_nynorms=gg_h_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=gg_h_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=gg_h_powheginput('#xupbound')
      ...
      if(gg_h_powheginput('use-old-ubound').eq.1) then
         call gg_h_newunit(iun)
...
c      call maxratident
      ...
      common/gg_h_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical gg_h_refuse_pdf
      real * 8 gg_h_pwhg_pt2,gg_h_pwhg_upperb_rad
      external gg_h_pwhg_pt2,gg_h_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         call gg_h_set_rad_scales(ptsq)
         if(.not.gg_h_refuse_pdf()) then
            ...
            if(born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' gg_h_inc_norms: Warning, born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' gg_h_inc_norms: x1,x2', kn_xb1,kn_xb2
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' gg_h_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' gg_h_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/born/gg_h_pwhg_upperb_rad()
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
      ...
      subroutine gg_h_printhistnorms
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      include 'src/Lib/gg_h/pwhg_rad.h'
     ...
     1              efficiencies(ireg,iborn)/gg_h_powheginput('nubound')/
...
c     1           gg_h_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine gg_h_zerohistnorms
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      include 'src/Lib/gg_h/pwhg_rad.h'
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call gg_h_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call gg_h_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine gg_h_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/gg_h/pwhg_math.h'
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
...
c
      ...
      subroutine gg_h_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 gg_h_btilde,gg_h_powheginput
      external gg_h_btilde,gg_h_powheginput
...
c will not be used;
         ...
         call gg_h_mint(gg_h_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=gg_h_powheginput('ncall2')
         itmx2=gg_h_powheginput('itmx2')
         call gg_h_mint(gg_h_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call gg_h_gen(gg_h_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn't used the ifold array may need reset to 1 here.
      call gg_h_gen(gg_h_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

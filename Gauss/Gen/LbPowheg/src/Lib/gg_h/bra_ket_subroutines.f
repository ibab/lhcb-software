      function gg_h_dotp(p1,p2)
      implicit none
      real * 8 gg_h_dotp,p1(0:3),p2(0:3)
      gg_h_dotp = (p1(0)*p2(0) - p1(3)*p2(3)) - p1(1)*p2(1) - p1(2)*p2(2)
      end

      function gg_h_cdotp(p1,cp2)
      implicit none      
      complex * 16 gg_h_cdotp,cp2(0:3)
      real * 8 p1(0:3)
      gg_h_cdotp = (p1(0)*cp2(0) - p1(3)*cp2(3))
     #      - p1(1)*cp2(1) - p1(2)*cp2(2)
      end
      
      function gg_h_ccdotp(cp1,cp2)
      implicit none      
      complex * 16 gg_h_ccdotp,cp1(0:3),cp2(0:3)
      gg_h_ccdotp = (cp1(0)*cp2(0) - cp1(3)*cp2(3))
...
C
C
C*********** polarisation vector (gg_h_POLVEC) ************
C*********** entry gg_h_HELVEC ****************************
C
...
C   bug in gg_h_HELVEC for KT=0 corrected Nov.4, 1993
C
C   INPUT:
C
C            kbar      real*8  array(0:4)
C                        physical four momentum for boson and gg_h_mass
C                        kbar(0:4) = [Ebar, kbarx, kbary, kbarz, gg_h_mass]
C
C            sigmak      integer
...
C                        -1,0,1) when entry gg_h_HELVEC is used
C
C
C   OUTPUT:
...
C                  must be used!
C
      SUBROUTINE gg_h_POLVEC(KBAR,LAMBDA,EPSCAR)
C
C   arguments:
C
      real*8  KBAR(0:4)
...
C
      ...
      ELSE IF (LAMBDA .EQ. 3) THEN
            IF (KBAR(4) .EQ. 0) THEN
C
C                  gg_h_mass = 0 not allowed for longitudinal polarization
C
                  WRITE(*,*) 'gg_h_mass = 0 for Lambda = 3 in gg_h_POLVEC'
                  EPSCAR(1) = 0.0d0
                  EPSCAR(2) = 0.0d0
                  EPSCAR(3) = 0.0d0
                  RETURN
...
C
            WRITE (*,*) 'Invalid Lambda in gg_h_POLVEC: Lambda = ',LAMBDA
                  EPSCAR(1) = 0.0d0
                  EPSCAR(2) = 0.0d0
                  EPSCAR(3) = 0.0d0
            END IF
      RETURN
C
      ENTRY gg_h_HELVEC(KBAR,SIGMAK,LAMBDA,EPSHEL)
C
C   code:
C
C
...
C                  gg_h_mass = 0 not allowed for longitudinal polarization
C
                  WRITE(*,*) 'gg_h_mass = 0 for Lambda = 0 in gg_h_HELVEC'
                  EPSHEL(1) = 0.0d0
                  EPSHEL(2) = 0.0d0
                  EPSHEL(3) = 0.0d0
                  RETURN
...
C                  Unrecognised value of Lambda
C
            WRITE (*,*) 'Invalid Lambda in gg_h_HELVEC: Lambda = ',LAMBDA
                  EPSHEL(1) = 0.0d0
                  EPSHEL(2) = 0.0d0
                  EPSHEL(3) = 0.0d0
            END IF
...
c gives gg_h_ket(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine gg_h_ket(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
...
c gives gg_h_ket(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine gg_h_ket_limit(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
      complex * 16 psi(1:2,-1:1)
...
c p = physical momentum  p[0] = E > 0
c ferm_type = fermion (+1) or antifermion (-1)
c gives gg_h_bra(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine gg_h_bra(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
      complex * 16 psi(1:2,-1:1)
...
c                           ( ris(1))
c gives p_slash |gg_h_ket,hel> = (       )
c                           ( ris(2))
      subroutine gg_h_slash_ket(p,gg_h_ket,hel,ris)
      implicit none
      complex * 16 p(0:3)
      complex * 16 gg_h_ket(1:2)
      integer hel
      complex * 16 ris(1:2)
      ...
      ris(1) = pslash(1,1)*gg_h_ket(1)+pslash(1,2)*gg_h_ket(2)
      ris(2) = pslash(2,1)*gg_h_ket(1)+pslash(2,2)*gg_h_ket(2)
      end


c gives <gg_h_bra,hel | p_slash = ( ris(1), ris(2) )      

      subroutine gg_h_slash_bra(p,gg_h_bra,hel,ris)
      implicit none
      complex * 16 p(0:3)
      complex * 16 gg_h_bra(1:2)
      integer hel
      complex * 16 ris(1:2)
      complex * 16 pslash(1:2,1:2)
      complex * 16 I
      ...
      pslash(2,1) = -hel*(p(1) + I*p(2))
      pslash(2,2) = p(0) + hel*p(3)
      ris(1) = pslash(1,1)*gg_h_bra(1)+pslash(2,1)*gg_h_bra(2)
      ris(2) = pslash(1,2)*gg_h_bra(1)+pslash(2,2)*gg_h_bra(2)
      end


c <gg_h_bra, helbra| slash_p_1 slash_p_2 ... slash_p_n |gg_h_ket, helket>
c where list_mom = [p_1, p_2,...., p_n]
c helket = -(-1)^n*helbra, otherwise the result is zero!!!
      subroutine gg_h_bra_slash_ket(gg_h_bra,gg_h_ket,helket,list_mom,n,ris)
      implicit none
      complex * 16 gg_h_bra(2,-1:1),gg_h_ket(2,-1:1)
      integer helket
      integer n
...
c      dimension list_mom(0:3,n)
      ...
      helbra = -(-1)**n*helket
      hel = helket
      b(1) = gg_h_bra(1,helbra)
      b(2) = gg_h_bra(2,helbra)
      k(1) = gg_h_ket(1,helket)
      k(2) = gg_h_ket(2,helket)
      
      do ii=n,1,-1
         call gg_h_slash_ket(list_mom(0,ii),k,hel,new_ket)
         k(1) = new_ket(1)
         k(2) = new_ket(2)
         hel = -hel
      enddo
...
c <gg_h_bra, helbra| slash_curr_vertex_1, slash_p1, slash_curr_vertex_2|gg_h_ket, helket>
...
c is emitted 
      subroutine gg_h_bra_slash_ket_gluon
     #     (gg_h_bra,gg_h_ket,helket,pbra,pket,pg,eps,list_mom,ris)
      implicit none
      complex * 16 gg_h_bra(2,-1:1),gg_h_ket(2,-1:1)
      integer helket
...
c      parameter (I=(0,1))
      ...
      integer mu
      complex * 16 gg_h_ccdotp
c emissione dalla gamba esterna gg_h_bra
      do mu=0,3
      ...
      enddo
      call gg_h_bra_slash_ket(gg_h_bra,gg_h_ket,helket,local_list,5,ris1)

      ris1 = ris1 / gg_h_ccdotp(local_list(0,2),local_list(0,2))/
     #     gg_h_ccdotp(local_list(0,4),local_list(0,4))
...
c emissione dal propagatore interno
      do mu=0,3
         ...
         local_list(mu,5) = list_mom(mu,3)
      enddo
      call gg_h_bra_slash_ket(gg_h_bra,gg_h_ket,helket,local_list,5,ris2)
      ris2 = ris2 / gg_h_ccdotp(local_list(0,2),local_list(0,2)) /
     #     gg_h_ccdotp(local_list(0,4),local_list(0,4))


c emissione dalla gamba esterna gg_h_ket
      do mu=0,3
         local_list(mu,1) = list_mom(mu,1)
         local_list(mu,2) = list_mom(mu,2)-pg(mu)
      ...
      enddo
      call gg_h_bra_slash_ket(gg_h_bra,gg_h_ket,helket,local_list,5,ris3)
      ris3 = ris3 / gg_h_ccdotp(local_list(0,2),local_list(0,2))/
     #     gg_h_ccdotp(local_list(0,4),local_list(0,4))
...
C NO TRIPLE GLUON VERTEX HERE!!!
      subroutine gg_h_bra_slash_ket_g1_g2
     #     (gg_h_bra,gg_h_ket,helket,pbra,pket,pg1,eps1,pg2,eps2,jcurr,ris)
      implicit none
      complex * 16 gg_h_bra(2,-1:1),gg_h_ket(2,-1:1)
      integer helket
...
c emission  <gg_h_bra| 1,2,j |gg_h_ket>
      do mu=0,3
...
c emission <gg_h_bra|1,j,2|gg_h_ket> 
      do mu=0,3
         local_list(mu,1) = eps1(mu)
         local_list(mu,2) = pg1(mu)+pbra(mu)
...
c emission <gg_h_bra|j,1,2|gg_h_ket>
...
c where list_mom = [p_1, p_2,...., p_n]
c helket = -(-1)^n*helbra, otherwise the result is zero!!!
      subroutine gg_h_bra_slash_ket1(gg_h_bra,gg_h_ket,helbra,list_mom,n,ris)
      implicit none
      complex * 16 gg_h_bra(2,-1:1),gg_h_ket(2,-1:1)
      integer helket
      integer n
...
c      dimension list_mom(0:3,n)
      ...
      do ii=1,n
         call gg_h_slash_bra(list_mom(0,ii),b,hel,new_bra)
         b(1) = new_bra(1)
         b(2) = new_bra(2)
         hel = -hel
      enddo
...
c the helicity of the gg_h_bra and the gg_h_ket are the same, otherwise this is zero!
c sig^mu = <gg_h_bra, helbra| gamma^mu |gg_h_ket, helket>
      subroutine gg_h_bra_gamma_ket(gg_h_bra,gg_h_ket,hel,sig)
      implicit none
      complex * 16 gg_h_bra(2,-1:1),gg_h_ket(2,-1:1)
      complex * 16 b(2),k(2)
      integer hel
...
c the helicity of the gg_h_bra and the gg_h_ket are the same, otherwise this is zero!
c sig^mu = 1/gg_h_dotp(pbra+pglu,pbra+pglu) * 
c   <gg_h_bra, pbra, hel| eps_slash, (pbra+pglu)_slash, gamma^mu |gg_h_ket, pket, hel>+
c          1/gg_h_dotp(pket-pglu,pket-pglu) * 
c   <gg_h_bra, pbra, hel| gamma^mu, (pket-pglu)_slash, eps_slash |gg_h_ket, pket, hel>

      subroutine gg_h_bra_gamma_ket_gluon(gg_h_bra,gg_h_ket,hel,pbra,pket,pglu,eps,sig)
      implicit none
      complex * 16 gg_h_bra(2,-1:1),gg_h_ket(2,-1:1)
      integer hel
      ...
      parameter (I=(0,1))
      complex*16 list_mom(0:3,3)
      real * 8 gamma(0:3),prop1,prop2,gg_h_dotp
      integer mu,nu
      ...
      do mu=0,3
         ...
         enddo 
         call gg_h_bra_slash_ket(gg_h_bra,gg_h_ket,hel,list_mom,3,J1(mu))
      enddo
      ...
      do mu=0,3
         ...
         enddo 
         call gg_h_bra_slash_ket(gg_h_bra,gg_h_ket,hel,list_mom,3,J2(mu))
      enddo


      prop1 = 2*gg_h_dotp(pglu,pbra)
      prop2 = -2*gg_h_dotp(pglu,pket)
...
c with a single vertex inserted
      subroutine gg_h_bra_gamma_ket_3gv(gg_h_bra,gg_h_ket,hel,pbra,pket,p1,eps1,
     #                  p2,eps2,sig)
      implicit none
      complex * 16 gg_h_bra(2,-1:1),gg_h_ket(2,-1:1)
      integer hel
     ...
     #            eps1(0:3),eps2(0:3)
      ...
      coeff1=2d0*gg_h_dotp(p1,eps2)+gg_h_dotp(p2,eps2)
      coeff2=-2d0*gg_h_dotp(p2,eps1)-gg_h_dotp(p1,eps1)
      coeff3=gg_h_dotp(eps1,eps2)
     ...
     #             coeff3*(p2(mu)-p1(mu))
      ...
      prop1 = 2*gg_h_dotp(p1,p2)*(gg_h_dotp(ptemp,ptemp)+2*gg_h_dotp(pbra,ptemp))
      prop2 = 2*gg_h_dotp(p1,p2)*(gg_h_dotp(ptemp,ptemp)-2*gg_h_dotp(ptemp,pket))
...
c all the fermionic line  (NO COLOR STRUCTURE!!)
c the helicity of the gg_h_bra and the gg_h_ket are the same, otherwise this is zero!
c sig^mu = 1/gg_h_dotp(pbra+pglu,pbra+pcurr) * 
c   <gg_h_bra, pbra, hel| j_slash, (pbra+pcurr)_slash, gamma^mu |gg_h_ket, pket, hel>+
c          1/gg_h_dotp(pket-pcurr,pket-pcurr) * 
c   <gg_h_bra, pbra, hel| gamma^mu, (pket-pcurr)_slash, j_slash |gg_h_ket, pket, hel>

      subroutine gg_h_bra_gamma_ket_curr(gg_h_bra,gg_h_ket,hel,pbra,
     # pket,pcurr,jcurr,sig)
      ...
      prop1 = gg_h_dotp(pcurr,pcurr)*(gg_h_dotp(pcurr,pcurr)+2*gg_h_dotp(pcurr,pbra))
      prop2 = gg_h_dotp(pcurr,pcurr)*(gg_h_dotp(pcurr,pcurr)-2*gg_h_dotp(pcurr,pket))
      ...
      subroutine gg_h_exchange_momenta(p1,p2)
      implicit none
...
c return a gg_h_random number between 0 and 1
      function gg_h_rand_num()
      implicit none
      real * 8 gg_h_rand_num
      real * 8 gg_h_random2
      integer num,num2
      COMMON/gg_h_SEED/num,num2
c set gg_h_random seeds
      data NUM/12345/,NUM2/67890/
      gg_h_rand_num = gg_h_random2(num,num2)
      end
      
      subroutine gg_h_exchange_mom(p,i,j,dim,pnew)
      implicit none
...
C-----------------------------------------------------------------------
      FUNCTION gg_h_random2(iseed1,iseed2)
C     MAIN gg_h_random NUMBER GENERATOR
C     USES METHOD OF l'Ecuyer, (VIA F.JAMES, COMP PHYS COMM 60(1990)329)
C-----------------------------------------------------------------------
      IMPLICIT NONE
      DOUBLE PRECISION gg_h_random2
      ...
      gg_h_random2=DBLE(IZ)/2147483589
c      gg_h_random2=DBLE(IZ)*4.656613001013252D-10

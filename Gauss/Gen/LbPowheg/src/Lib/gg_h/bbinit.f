     ...
     #                  ,xmmm(0:50,ndiminteg),xmmmrm(0:50,ndiminteg)
      ...
      common/gg_h_cgengrids/xgrid,ymax,xmmm,xgridrm,ymaxrm,xmmmrm,
     #                ifold,ifoldrm
      character * 20 pwgprefix
      integer lprefix
      common/gg_h_cpwgprefix/pwgprefix,lprefix
      integer j,k,mcalls,icalls,imode,iunstat
      real * 8 gg_h_powheginput
      external gg_h_btilde,gg_h_sigremnant,gg_h_powheginput
      do j=1,ndiminteg
         do k=0,50
            xgrid(k,j)=0
            xgridrm(k,j)=0
     ...
     1        rnd_cwhichseed//'.dat',status='unknown')
      ...
      ncall1=gg_h_powheginput('ncall1')
      ncall2=gg_h_powheginput('ncall2')
      itmx1=gg_h_powheginput('itmx1')
      itmx2=gg_h_powheginput('itmx2')
      iret1=0
      iret2=0
      call gg_h_loadgrids(iret1,xgrid,ymax,xgridrm,ymaxrm,
     2     ifold,ifoldrm)
      if(iret1.ne.0) call gg_h_loadxgrid(iret2,xgrid,xint,xgridrm,xintrm)
      if(iret2.ne.0) then
         write(*,*)
     ...
     #        status='unknown')
         ...
         flg_nlotest=.false.
         imode=0
         call gg_h_mint(gg_h_btilde,ndiminteg,ncall1,itmx1,ifold,imode,iun,
     ...
     #        status='unknown')
            imode=0
            call gg_h_mint(gg_h_sigremnant,ndiminteg,ncall1,itmx1,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
            close(iun)
         endif
         call gg_h_storexgrid(xgrid,xint,xgridrm,xintrm)
...
c importance sampling grids have been initialized with default gg_h_SEED;
c they must all be the same. Now set current gg_h_SEED, if required
      if(rnd_cwhichseed.ne.'none')
     1     call gg_h_setrandom(rnd_initialseed,rnd_i1,rnd_i2)
      if(iret1.ne.0) then
         if (gg_h_powheginput('#testplots').eq.1d0) call gg_h_init_hist
c set  up the folding here, if required
         ifold(ndiminteg-2) = gg_h_powheginput("foldcsi")
         ifold(ndiminteg-1) = gg_h_powheginput("foldy")
         ifold(ndiminteg)   = gg_h_powheginput("foldphi")
...
c Results in rad_tot???btl do not depend upon flg_withnegweights
         call gg_h_resettotals
         call gg_h_mint(gg_h_btilde,ndiminteg,ncall2,itmx2,ifold,imode,iun,
     #        xgrid,xint,ymax,sigbtl,errbtl)
         call gg_h_finaltotals
c finalize gg_h_btilde output in histograms
         call gg_h_pwhgaddout
         flg_nlotest=.false.
         write(*,*) 'gg_h_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(*,*) 'gg_h_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(*,*) 'gg_h_btilde total (pos.-|neg.|):', rad_totbtl,' +-',
     1        rad_etotbtl
         write(iunstat,*) 'gg_h_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(iunstat,*) 'gg_h_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(iunstat,*) 'gg_h_btilde Total (pos.-|neg.|):', rad_totbtl,
     1        ' +-',rad_etotbtl
c Now compute the remnant contributions
     ...
     #                ' and/or regular remnants'
            flg_nlotest=.true.
            imode=1
            call gg_h_mint(gg_h_sigremnant,ndiminteg,ncall2,itmx2,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
...
c gg_h_btilde and remnant event are chosen in proportion to
...
c        
         call gg_h_storegrids(xgrid,ymax,xgridrm,ymaxrm,ifold,ifoldrm,
     1        ncall2,itmx2)
c Output NLO histograms
         if (gg_h_powheginput('#testplots').eq.1d0) then
            if(rnd_cwhichseed.eq.'none') then
     ...
     1           rad_totrm,'+-',rad_etotrm
         ...
         if (flg_weightedev) then
            write(iunstat,*) 
     1 ' total (gg_h_btilde+remnants) cross section times '
            write(iunstat,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
         else
            write(iunstat,*) 
     1 ' total (gg_h_btilde+remnants) cross section in pb',
     2     rad_tot,'+-',rad_etot
         endif
     ...
     1        rad_totrm,'+-',rad_etotrm
      ...
      if (flg_weightedev) then
         write(*,*) ' total (gg_h_btilde+remnants) cross section times '
         write(*,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
      else
         write(*,*) ' total (gg_h_btilde+remnants) cross section in pb',
     1     rad_tot,'+-',rad_etot
      endif
     ...
     1     rad_totnegbtl/(2*rad_totnegbtl+rad_tot)
      close(iunstat)
      call flush
c initialize gg_h_gen; the array xmmm is set up at this stage.
      call gg_h_gen(gg_h_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,0,
     #    mcalls,icalls,xx)
c compute normalization of upper bounding function for radiation
      call gg_h_do_maxrat(mcalls,icalls)
c print statistics
      call gg_h_gen(gg_h_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,3,
     #    mcalls,icalls,xx)
      if(xx(1).gt.0) write(*,*) 'POWHEG: efficiency in the generation'
     # //' of the Born variables =',xx(1)
      if((flg_withreg.or.flg_withdamp).and..not.flg_bornonly) then
c initialize gg_h_gen for remnants
         call gg_h_gen(gg_h_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            0,mcalls,icalls,xx)
c     save gg_h_random number seeds
         call gg_h_randomsave
c generate few events from remnants, just to determine the generation efficiency
         do j=1,int(min(gg_h_powheginput('nubound'),10d0))
            call gg_h_gen(gg_h_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            1,mcalls,icalls,xx)
         enddo
c     restore  gg_h_random number seeds
...
c     print statistics
     ...
     #                   //' of the remnant variables =',xx(1)
      ...
      subroutine gg_h_gen_btilde(mcalls,icalls)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)      
      real * 8 gg_h_btilde
      external gg_h_btilde
      call gg_h_gen(gg_h_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)
      end

      subroutine gg_h_gen_sigremnant
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      real * 8 xgrid(0:50,ndiminteg),ymax(50,ndiminteg)
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)
      integer mcalls,icalls
      real * 8 gg_h_sigremnant
      external gg_h_sigremnant
      call gg_h_gen(gg_h_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,1,
     #    mcalls,icalls,xx)
      end


      subroutine gg_h_storegrids(xgrid,ymax,xgridrm,ymaxrm,
     #                ifold,ifoldrm,ncall2,itmx2)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      include 'src/Lib/gg_h/pwhg_kn.h'
      include 'src/Lib/gg_h/pwhg_pdf.h'
      include 'src/Lib/gg_h/pwhg_rad.h'
      include 'src/Lib/gg_h/pwhg_rnd.h'
      real * 8 xgrid(0:50,ndiminteg),ymax(50,ndiminteg)
     #        ,xgridrm(0:50,ndiminteg),ymaxrm(50,ndiminteg)
     ...
     8     rad_tot,rad_etot
      ...
      subroutine gg_h_loadgrids(iret,xgrid,ymax,xgridrm,ymaxrm,
     #           ifold,ifoldrm)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
...
c
      ...
      if(gg_h_powheginput('use-old-grid').eq.0) then
         iret=1
         return
      endif
      iret=0
...
c gg_h_random seeds
     ...
     1              chseed//'.dat'
         ...
         if(gg_h_powheginput("#ncallfrominput").eq.1) then
            ncall2=gg_h_powheginput("ncall2")
            itmx2=gg_h_powheginput("itmx2")
            ncall2=ncall2*itmx2
         endif
 ...
 998  continue
      ...
      subroutine gg_h_storexgrid(xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      include 'src/Lib/gg_h/pwhg_kn.h'
     ...
     #     form='unformatted',status='unknown')
      ...
      subroutine gg_h_loadxgrid(iret,xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      include 'src/Lib/gg_h/pwhg_kn.h'

      subroutine gg_h_btildereal(xrad,resreal,www)
      implicit none
      ...
      include 'src/Lib/gg_h/pwhg_par.h'
...
c output values for gg_h_analysis_driver
         out0=0
         out1=0
...
c     final state radiation
               call gg_h_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call gg_h_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call gg_h_collfsr(rc)
c     gg_h_soft subtraction
                  call gg_h_soft(r0s)
                  call gg_h_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call gg_h_collisrp(rp)
                     call gg_h_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call gg_h_collisrm(rm)
                     call gg_h_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call gg_h_analysis_driver(out0,0)
            if(out1.ne.0d0) call gg_h_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine gg_h_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      include 'src/Lib/gg_h/pwhg_kn.h'
      include 'src/Lib/gg_h/pwhg_dbg.h'
      external gg_h_sigreal_btl,gg_h_soft,gg_h_collfsr,gg_h_softcollfsr, gg_h_collisrp,
     $     gg_h_softcollisrp,gg_h_collisrm,gg_h_softcollisrm
      call gg_h_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  gg_h_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call gg_h_checksoft(gg_h_sigreal_btl,gg_h_soft,' gg_h_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call gg_h_checkcoll(gg_h_sigreal_btl,gg_h_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
c            call gg_h_randomsave
            if(kn_emitter.ne.2) call gg_h_checkcoll(gg_h_sigreal_btl,gg_h_collisrp,1
     $           ,' coll-plus',iun)
c            call gg_h_randomrestore
            if(kn_emitter.ne.1) call gg_h_checkcoll(gg_h_sigreal_btl,gg_h_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  gg_h_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call gg_h_checksoft(gg_h_collfsr,gg_h_softcollfsr,' gg_h_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  gg_h_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call gg_h_checksoft(gg_h_collisrp,gg_h_softcollisrp,
     $           ' gg_h_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call gg_h_checksoft(gg_h_collisrm,gg_h_softcollisrm,
     $           ' gg_h_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-gg_h_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call gg_h_checkcoll(gg_h_soft,gg_h_softcollfsr,1,' coll-gg_h_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-gg_h_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call gg_h_checkcoll(gg_h_soft,gg_h_softcollisrp,1
     $           ,' coll-plus-gg_h_soft',iun)
            if(kn_emitter.ne.1) call gg_h_checkcoll(gg_h_soft,gg_h_softcollisrm,-1
     $           ,' coll-minus-gg_h_soft',iun)
         enddo
      endif
      call gg_h_randomrestore
      end

      subroutine gg_h_checkborn(iun)
...
c are consistent with total Born
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine gg_h_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/gg_h/pwhg_dbg.h'
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 gg_h_random,gg_h_dotp
      external gg_h_random,gg_h_dotp
      logical gg_h_valid_emitter,iszero,isnonzero,isequal
      external gg_h_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=gg_h_random()
      enddo
      ...
      call gg_h_allborn
      call gg_h_checkborn(iun)
c      write(iun,*)' gg_h_mass',sqrt(2*gg_h_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=gg_h_random()
      enddo
c Check gg_h_soft limits
      if(gg_h_valid_emitter(kn_emitter)) then
         write(iun,*) ' gg_h_random Born variables ====> ',xborn
          write(iun,*) ' gg_h_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call gg_h_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call gg_h_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check gg_h_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine gg_h_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/gg_h/pwhg_dbg.h'
      include 'src/Lib/gg_h/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' gg_h_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
...
c     Added this 'if' to be sure that no division by zero occurs
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine gg_h_sigreal_rad(sig)
      implicit none
      real * 8 sig
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
...
c     generate "nmomset" gg_h_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call gg_h_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      ...
      if(flg_withdamp) then
         call gg_h_collrad(rc)
         call gg_h_softrad(rs)
      endif
      do alr=1,flst_nalr
         computed(alr)=.false.
      enddo
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call gg_h_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call gg_h_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine gg_h_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call gg_h_sigreal_btl0(r0,0)
      end

c Real cross section, required by gg_h_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine gg_h_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      include 'src/Lib/gg_h/pwhg_kn.h'
...
c     generate "nmomset" gg_h_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call gg_h_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      ...
      enddo
      if(flg_withdamp) then
         call gg_h_collbtl(rc)
         call gg_h_softbtl(rs)
      endif
      do alr=1,flst_nalr
...
c First mark as being computed
               markused(alr)=1
               call gg_h_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' gg_h_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: gg_h_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine gg_h_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=gg_h_random()
            enddo
         enddo
         preal(3,nlegreal,j)=gg_h_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine gg_h_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from gg_h_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine gg_h_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call gg_h_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'gg_h_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine gg_h_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/gg_h/nlegborn.h'
      include 'src/Lib/gg_h/pwhg_flst.h'
      include 'src/Lib/gg_h/pwhg_math.h'
      ...
      integer rflav(nlegreal)
      real * 8 amp2 
      logical gg_h_pwhg_isfinite
      external gg_h_pwhg_isfinite
      call gg_h_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.gg_h_pwhg_isfinite(amp2)) amp2=0d0

      subroutine zz_btildereal(xrad,resreal,www)
      implicit none
...
c output values for zz_analysis_driver
         out0=0
         out1=0
c check that emitter is valid
         if(zz_valid_emitter(kn_emitter)) then
            if(kn_emitter.gt.2) then
c     final state radiation
               call zz_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call zz_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call zz_collfsr(rc)
c     zz_soft subtraction
                  call zz_soft(r0s)
                  call zz_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call zz_collisrp(rp)
                     call zz_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call zz_collisrm(rm)
                     call zz_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call zz_analysis_driver(out0,0)
            if(out1.ne.0d0) call zz_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine zz_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      ...
      external zz_sigreal_btl,zz_soft,zz_collfsr,zz_softcollfsr, zz_collisrp,
     $     zz_softcollisrp,zz_collisrm,zz_softcollisrm
      call zz_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  zz_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call zz_checksoft(zz_sigreal_btl,zz_soft,' zz_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call zz_checkcoll(zz_sigreal_btl,zz_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
c            call zz_randomsave
            if(kn_emitter.ne.2) call zz_checkcoll(zz_sigreal_btl,zz_collisrp,1
     $           ,' coll-plus',iun)
c            call zz_randomrestore
            if(kn_emitter.ne.1) call zz_checkcoll(zz_sigreal_btl,zz_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  zz_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call zz_checksoft(zz_collfsr,zz_softcollfsr,' zz_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  zz_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call zz_checksoft(zz_collisrp,zz_softcollisrp,
     $           ' zz_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call zz_checksoft(zz_collisrm,zz_softcollisrm,
     $           ' zz_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-zz_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call zz_checkcoll(zz_soft,zz_softcollfsr,1,' coll-zz_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-zz_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call zz_checkcoll(zz_soft,zz_softcollisrp,1
     $           ,' coll-plus-zz_soft',iun)
            if(kn_emitter.ne.1) call zz_checkcoll(zz_soft,zz_softcollisrm,-1
     $           ,' coll-minus-zz_soft',iun)
         enddo
      endif
      call zz_randomrestore
      end

      subroutine zz_checkborn(iun)
...
c are consistent with total Born
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine zz_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/zz/pwhg_dbg.h'
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 zz_random,zz_dotp
      external zz_random,zz_dotp
      logical zz_valid_emitter,iszero,isnonzero,isequal
      external zz_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=zz_random()
      enddo
      ...
      call zz_allborn
      call zz_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*zz_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=zz_random()
      enddo
c Check zz_soft limits
      if(zz_valid_emitter(kn_emitter)) then
         write(iun,*) ' zz_random Born variables ====> ',xborn
          write(iun,*) ' zz_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call zz_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call zz_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check zz_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine zz_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/zz/pwhg_dbg.h'
      include 'src/Lib/zz/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' zz_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      if(ini) then
         ...
         if(flg_smartsig) then
            call zz_randomsave
c     generate "nmomset" zz_random real-phase space configurations
            call zz_fillmomenta(nlegreal,nmomset,kn_masses,preal)
            do alr=1,flst_nalr
               do j=1,nmomset
                  call zz_realgr(
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call zz_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      do alr=1,flst_nalr
         r0(alr)=0
      enddo
      call zz_pdfcall(1,kn_x1,pdf1)
      call zz_pdfcall(2,kn_x2,pdf2)
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call zz_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call zz_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine zz_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call zz_sigreal_btl0(r0,0)
      end

c Real cross section, required by zz_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine zz_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
...
c     generate "nmomset" zz_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call zz_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c First mark as being computed
               markused(alr)=1
               call zz_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' zz_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: zz_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine zz_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=zz_random()
            enddo
         enddo
         preal(3,nlegreal,j)=zz_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine zz_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from zz_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine zz_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call zz_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'zz_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine zz_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_math.h'
      ...
      call zz_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.zz_pwhg_isfinite(amp2)) amp2=0d0

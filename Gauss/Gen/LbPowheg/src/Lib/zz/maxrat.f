      subroutine zz_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 zz_powheginput,zz_random
      external zz_powheginput,zz_random      
      call zz_zerohistnorms
      loaded=.false.
      nubound=zz_powheginput('nubound')
      rad_nynorms=zz_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=zz_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=zz_powheginput('#xupbound')
      ...
      call zz_newunit(iun)
      if(zz_powheginput('use-old-ubound').eq.1) then
         call zz_newunit(iun)
...
c      call maxratident
      ...
      common/zz_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical zz_refuse_pdf
      real * 8 zz_pwhg_pt2,zz_pwhg_upperb_rad
      external zz_pwhg_pt2,zz_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         call zz_set_rad_scales(ptsq)
         if(.not.zz_refuse_pdf()) then
            ...
            if(born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' zz_inc_norms: Warning, born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' zz_inc_norms: x1,x2', kn_xb1,kn_xb2
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' zz_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' zz_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/born/zz_pwhg_upperb_rad()
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
     ...
     1              efficiencies(ireg,iborn)/zz_powheginput('nubound')/
...
c     1           zz_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine zz_zerohistnorms
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_rad.h'
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call zz_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call zz_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine zz_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/zz/pwhg_math.h'
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
...
c
      ...
      subroutine zz_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 zz_btilde,zz_powheginput
      external zz_btilde,zz_powheginput
...
c will not be used;
         call zz_resettotals
         call zz_mint(zz_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=zz_powheginput('ncall2')
         itmx2=zz_powheginput('itmx2')
         call zz_mint(zz_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call zz_gen(zz_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn'zz_t used the ifold array may need reset to 1 here.
      call zz_gen(zz_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

c subroutines zz_soft, zz_collfsr, zz_softcollfsr, zz_collisrp, zz_collisrm,
c             zz_softcollisrp, zz_softcollisrm 
c are called by zz_btildereal; they return (1-y) csi^2 R_alr (if kn_emitter
c is >2, i.e. final state) or (1-y^2) csi^2 R (if kn_emitter <=2) in the
c collinear final state                      (zz_collfsr)
c zz_soft-collinear final state                 (zz_softcollfsr)
c collinear initial state in the + direction (zz_collisrp)
c collinear initial state in the - direction (zz_collisrm)
c zz_soft-collinear initial state +             (zz_softcollisrp)
c zz_soft-collinear initial state -             (zz_softcollisrm)
c The zz_soft limit is taken keeping y (i.e. kn_y) fixed
...
c
c
c Functions: zz_collbtl and zz_softbtl are required to compute the damping
c            factor in zz_bornzerodamp. They are called by sigreal, and
c            sigreal is used by zz_btilde (hence the btl suffix)

c zz_softbtl: computes the same quantity as zz_soft, but does not include 
c          the luminosity
c
c zz_collbtl: if kn_emitter is in the final state, computes the same quantity as
c          zz_collfsr, but does not include the luminosity
c          if kn_emitter is 1 or 2  computes the same quantity as
c          zz_collisrp or zz_collisrm, without luminosity
c          if kn_emitter is 0 computes the average of the return values
c          of zz_collisrp and zz_collisrm, weighted with the factors 
c          (1+y)/2 and (1-y)/2.
c
c 
c
c Functions: zz_collrad, zz_softrad  are required to compute the damping
c            factor in zz_bornzerodamp. They are called by zz_sigreal_rad,
c            i.e. they are used in the generation of radiation.
c            They compute the same quantities as zz_collbtl and zz_softbtl,
...
c for fsr csi is y independent
      ...
      phi=kn_azi
      x=csi*xocsi
      call zz_buildkperp(em,phi,kperp,kperp2)
      do alr=1,flst_nalr
         if(em.eq.flst_emitter(alr)) then
            call zz_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
      ...
      subroutine zz_buildkperp(em,phi,kperp,kperp2)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
...
c     Then rotate it counterclockwise around the em direction
     ...
     #/kn_cmpborn(3,em)
      ...
      call zz_mrotate(dir,sin(phi),cos(phi),kperp(1))
      kperp(0)=0
      end
      

      subroutine zz_softcollfsr(rc)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
      ...
      call zz_collfsrnopdf(rc)
      kn_csi=tmp
      call zz_pdfcall(1,kn_xb1,pdf1)
      call zz_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end
      ...
      subroutine zz_collisrp(rc)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      real * 8 rc(maxalr)
      call zz_collisr(1,rc)
      end

      subroutine zz_collisrm(rc)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      real * 8 rc(maxalr)
      call zz_collisr(2,rc)
      end

      subroutine zz_softcollisrp(rc)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
      ...
      call zz_pdfcall(1,x1,pdf1)
      call zz_pdfcall(2,x2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end

      subroutine zz_collisrnopdf(i,rc)
      implicit none
      integer i
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
...
c Construct kperp
      ...
      do alr=1,flst_nalr
         if(flst_emitter(alr).eq.kn_emitter) then
            call zz_collisralr(alr,i,csi,kperp,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
...
c softvec(0:3): real * 8, 4-vector of zz_soft gluon normalized to softvec(0)=1
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
      ...
      real * 8 y
      real * 8 pdf1(-6:6),pdf2(-6:6)
c zz_boost Born momenta to their rest frame
c find zz_boost velocity
      em=kn_emitter
      y=kn_y
      do alr=1,flst_nalr
         call zz_softalr(alr,em,y,r0(alr))
      enddo
      call zz_pdfcall(1,kn_xb1,pdf1)
      call zz_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
      ...
      subroutine zz_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,res)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
...
c provide multiplicity of emitter in underlyng Born
      ...
      subroutine zz_collisralr(alr,i,csi,kperp,res)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
...
c     the last expression being zz_boost invariant.
...
c     The remaining csi=1-x factor has been applied earlier
      ...
      subroutine zz_softalr(alr,em,y,res)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
               ...
               if(abs(flst_born(j,iub)).le.6) then
                  do k=j+1,nlegborn
                     if(abs(flst_born(k,iub)).le.6) then
                        result=result+
     #zz_dotp(kn_cmpborn(0,j),kn_cmpborn(0,k))
     #/(zz_dotp(kn_cmpborn(0,j),kn_softvec)*
     #zz_dotp(kn_cmpborn(0,k),kn_softvec)) * br_bornjk(j,k,iub)
                     endif
                  enddo
               endif
            enddo
...
c     symmetric, multiply by 2
     ...
     #kn_cmpborn(2,j)**2-kn_cmpborn(3,j)**2
                  result=result-pjsq/zz_dotp(kn_cmpborn(0,j),
     #kn_softvec)**2*br_born(iub)*cf
               endif
            enddo
c     having chosen a zz_soft four momentum of energy 1, supply
...
c     Multiply zz_soft result by (zz_soft limit Sij appropriate factors)
      if(res.ne.0) then
         sumdijinv=0
...
c cross section used in zz_btilde
      subroutine zz_collbtl(rc)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
         ...
         do alr=1,flst_nalr
            if(flst_emitter(alr).eq.em) then
               if(em.ne.2) then
                  ...
                  call zz_collisralr(alr,i,csi,kperp,r1)
               endif
               if(em.ne.1) then
                  i=2
                  csi=kn_csi*kn_csimaxm/kn_csimax
                  call zz_collisralr(alr,i,csi,kperp,r2)
               endif
               if(em.eq.0) then
...
c     Construct kperp
            ...
            if(em.ne.2) then
               csi=kn_csi*kn_csimaxp/kn_csimax
               call zz_collisralr(alr,1,csi,kperp,r1)
            endif
            if(em.ne.1) then
               csi=kn_csi*kn_csimaxm/kn_csimax
               call zz_collisralr(alr,2,csi,kperp,r2)
            endif
            if(em.eq.0) then
               rc(alr)=(r1*(1+kn_y)+r2*(1-kn_y))/2
            elseif(em.eq.1) then

      ...
      integer i1,n1,n2
      call zz_init_flsttag
      flg_debug=.false.
      if(zz_powheginput("#flg_debug").eq.1) flg_debug=.true.
c whether to output negative weights or not
      flg_withnegweights=.false.
      if(zz_powheginput("#withnegweights").eq.1)flg_withnegweights=.true.
c See if we have weighted events
      flg_weightedev=.false.
      if(zz_powheginput("#bornsuppfact").gt.0) flg_weightedev=.true.
      if(zz_powheginput("#ptsupp").gt.0) then
...
c     amplitudes, for Born, real and virtual contributions
      flg_smartsig=.true.
      if(zz_powheginput("#smartsig").eq.0) flg_smartsig=.false.
c     if true also counterterm are output in NLO tests (default)
      flg_withsubtr=.true.
      if(zz_powheginput("#withsubtr").eq.0) flg_withsubtr=.false.
...
c     is required
      flg_withdamp=.false.
      if((zz_powheginput("#withdamp").eq.1).or.
     $  (zz_powheginput("#hfact").gt.0d0))   flg_withdamp=.true.
c     If set do only the Born term
      flg_bornonly=.false.
      if (zz_powheginput("#bornonly").eq.1) flg_bornonly=.true.
c     initialize zz_random number sequence
      i1=zz_powheginput('#iseed')
      if (i1.lt.0) i1=0
      n1=zz_powheginput('#rand1')
      if (n1.lt.0) n1=0
      n2=zz_powheginput('#rand2')
      if (n2.lt.0) n2=0
c select which upper bounding function form
      rad_iupperisr=zz_powheginput("#iupperisr")
      if(rad_iupperisr.lt.0) rad_iupperisr=1
      rad_iupperfsr=zz_powheginput("#iupperfsr")
      if(rad_iupperfsr.lt.0) rad_iupperfsr=2
c info on pdf for each event
      flg_pdfreweight=.false.
      if(zz_powheginput("#pdfreweight").eq.1)flg_pdfreweight=.true.
c
      call zz_setrandom(i1,n1,n2)
...
c     inside the user-defined subroutine zz_init_processes
      lprup(1)=10001

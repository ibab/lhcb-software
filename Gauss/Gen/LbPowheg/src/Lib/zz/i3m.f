      double complex function zz_I3m(s1,s2,s3)
C     This is the function zz_I3m, a massless triangle with all three external 
...
!      include 'src/Lib/zz/constants.f'
      ...
      double precision zz_I3m1a,flag
      double complex zz_I3m1b
...
c---case all negative
             flag=0d0
             zz_I3m=zz_I3m1b(smax,smid,smin,rtdel3,flag)
         elseif (smin .gt. 0) then
c---case all positive
             flag=0d0
             zz_I3m=-zz_I3m1b(-smin,-smid,-smax,rtdel3,flag)
...
c---case two positive and one negative
      ...
      rtdel3=sqrt(-del3)
         if (smax .lt. 0) then
c---case all negative
             zz_I3m=+dcmplx(zz_I3m1a(+s1,+s2,+s3,rtdel3))
         elseif (smin .gt. 0) then
c---case all positive
             zz_I3m=-dcmplx(zz_I3m1a(-s1,-s2,-s3,rtdel3))  
          endif
      ...
      double precision function zz_I3m1a(s1,s2,s3,rtmdel)
      implicit none
...
!      include 'src/Lib/zz/constants.f'
      ...
      double precision two,pisq,one
      parameter (two=2d0,pisq=pi**2,one=1d0)
      double precision s1,s2,s3,d1,d2,d3,rtmdel,arg1,arg2,arg3,zz_DCLAUS
      ...
      zz_I3m1a=two/rtmdel*(zz_DCLAUS(arg1)+zz_DCLAUS(arg2)+zz_DCLAUS(arg3))

      end

      
      double complex function zz_I3m1b(s1,s2,s3,rtdel,flag)
      implicit none
...
!      include 'src/Lib/zz/constants.f'
      ...
      double precision s1,s2,s3,d3,temp,zz_DDILOG,zz_xlog,ylog,rat
      ...
      write(6,*) 'problems with call of zz_I3m1b'
      write(6,*) 'argdlx',argdlx
      write(6,*) 'argdly',argdly
      stop
      endif

      zz_xlog=log(abs(argx))
      ylog=log(abs(argy))
      temp=zz_xlog*ylog+pisq/3d0+(ylog-zz_xlog)*log((one+argy)/(one+argx))
     & +two*(zz_DDILOG(argdlx)+zz_DDILOG(argdly))
      zz_I3m1b=Dcmplx(temp-abs(flag)*pisq)+impi*Dcmplx(flag*(zz_xlog+ylog))
      zz_I3m1b=-zz_I3m1b/Dcmplx(rtdel)
      end

      subroutine zz_setborn(p,bflav,born,bornjk,bmunu)
      implicit none
      ...
      call zz_compborn(p,bflav,born,bmunu,bornjk)

      end


      subroutine zz_compborn(pin,bflav,born,bmunu,bornjk)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_math.h'
...
c idup(3)+idup(4) is 0 for Z, 1 for W+, -1 for W-
      if(idup(3)+idup(4).eq.0) then
         call zz_add_resonance(23,3,4)
      else
         call zz_add_resonance(24*(idup(3)+idup(4)),3,4)
      endif
c Careful now! 5 6 have been bumped to 6 7 by the
c resonance inserted above!
      if(idup(6)+idup(7).eq.0) then
         call zz_add_resonance(23,6,7)
      else
         call zz_add_resonance(24*(idup(6)+idup(7)),6,7)
      endif
...
c So we should now exchange 5 and 8
      ...
      if(i1.ne.0.and.zz_random().gt.0.5d0) then

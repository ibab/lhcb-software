      ...
      logical debug,ini,zz_pwhg_isfinite
      data ini/.true./
      data debug/.false./
      ...
      if (debug)  write(*,*) 'Entering zz_born_phsp: ndiminteg', ndiminteg
      if(ini) then
         do k=1,nlegborn
            kn_masses(k)=0
         enddo
         kn_masses(nlegreal)=0
         mllmin=zz_powheginput("#mllmin")
...
c zz_breitw, if zerowidth is true, does the right thing
      call zz_breitw(z,smin,smax,ph_zmass,ph_zwidth,s,wt)
...
C     total incoming momentum 
      ...
      call zz_phi1_2(xborn(3),xborn(4),p12,p34,p56,m34,m56,wt)
      xjac=xjac*wt
      call zz_phi3m0(xborn(7),xborn(8),p34,p3,p4,wt)
      xjac=xjac*wt
      call zz_phi3m0(xborn(5),xborn(6),p56,p5,p6,wt)
      xjac=xjac*wt
...
c     now zz_boost everything BACK along z-axis 
      kn_xb1 = xx(1)
      kn_xb2 = xx(2)
      ...
      vec(2)=0
      vec(3)=-1
      call zz_mboost(nlegborn-2,vec,beta,kn_pborn(:,3:),kn_cmpborn(:,3:))
      do mu=0,3
...
c zz_analysis level, but in this way we avoid confusing the user
     ...
     1  ) then
      ...
      if(.not.zz_pwhg_isfinite(kn_jacborn)) kn_jacborn=0
      ...
      subroutine zz_born_suppression(fact)
      implicit none
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      include 'src/Lib/zz/pwhg_kn.h'
      logical ini
      data ini/.true./
      real * 8 fact,pt2supp,zz_powheginput,pt
      save ini,pt2supp,pt     
      if (ini) then
         pt = zz_powheginput("#ptsupp")         
         ini = .false.
         pt2supp = pt**2
      endif
      if (pt.lt.0) then
...
c      endif
      ...
      subroutine zz_set_fac_ren_scales(muf,mur)
      implicit none
      include 'src/Lib/zz/PhysPars.h'
      include 'src/Lib/zz/nlegborn.h'
      include 'src/Lib/zz/pwhg_flst.h'
      ...
      save fixedscale,ini
      if (ini) then
         if(zz_powheginput("#fixedscale").lt.0) then
            write(*,*) '*************************************'

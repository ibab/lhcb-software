      subroutine vbf_wp_wp_pwhgevent
      implicit none
      ...
      integer iret,iun
      real * 8 suppfact
      real * 8 vbf_wp_wp_random,vbf_wp_wp_powheginput
      external vbf_wp_wp_random,vbf_wp_wp_powheginput
      integer mcalls,icalls
      real * 8 vbf_wp_wp_pwhg_pt2,vbf_wp_wp_pt2max_regular
      external vbf_wp_wp_pwhg_pt2,vbf_wp_wp_pt2max_regular
      integer i
      if(idwtup.eq.3) then
         xwgtup=1
...
c     store current vbf_wp_wp_random seeds. To be used to restart at problematic events
      call vbf_wp_wp_savecurrentrandom
      if(vbf_wp_wp_random().gt.rad_totrm/rad_totgen) then
c     generate underlying vbf_wp_wp_born kinematics
         call vbf_wp_wp_gen_btilde(mcalls,icalls)
c     generate underlying vbf_wp_wp_born flavour
         call vbf_wp_wp_gen_uborn_idx
c
         if(vbf_wp_wp_powheginput('#vbf_wp_wp_testsuda').eq.1) then
            call vbf_wp_wp_testsuda
         endif
c generate radiation
         call vbf_wp_wp_gen_radiation
c add a vbf_wp_wp_random azimuthal rotation around beam axis
         call vbf_wp_wp_add_azimuth
c --- set up les houches interface
         rad_pt2max=vbf_wp_wp_pwhg_pt2()
         call vbf_wp_wp_gen_leshouches
c if negative weight, flip the sign of xwgtup
         if(rad_btilde_sign(rad_ubornidx).eq.-1) then
            xwgtup=-xwgtup
         endif
c rad_type=1 for vbf_wp_wp_btilde events (used only for debugging purposes)
         rad_type=1
...
c iret=2: reg contribution (real graphs without singular regions)
         call vbf_wp_wp_gen_remnant(iret)
c         if (vbf_wp_wp_pwhg_pt2().lt.rad_ptsqmin) then
c            write(*,*) '****************************************'
c            write(*,*) 'WARNING in vbf_wp_wp_gen_remnant'
c            write(*,*) 'vbf_wp_wp_pwhg_pt2 < rad_ptsqmin ',
c     #           vbf_wp_wp_pwhg_pt2(),' < ',rad_ptsqmin
c            write(*,*) (flst_alr(i,rad_realalr),i=1,nlegreal)
c            write(*,*) 'To generate this event, use the following seeds'
c            call vbf_wp_wp_printcurrentrandom
c            write(*,*) '****************************************'
c         endif
...
c     set st_muren2 equal to pt2 for scalup value
            rad_pt2max=max(rad_ptsqmin,vbf_wp_wp_pwhg_pt2())
            call vbf_wp_wp_set_rad_scales(rad_pt2max)
            call vbf_wp_wp_gen_leshouches
            call vbf_wp_wp_born_suppression(suppfact)
            if(suppfact.eq.0) then
               write(*,*) ' 0 suppression factor in event generation'
...
c     set st_muren2 for scalup value for regular contributions
            rad_pt2max=max(rad_ptsqmin,vbf_wp_wp_pt2max_regular())
            call vbf_wp_wp_set_rad_scales(rad_pt2max)
            call vbf_wp_wp_gen_leshouches_reg
c rad_type=3 for regular contributions
      ...
      subroutine vbf_wp_wp_gen_radiation
      implicit none
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      ...
      real * 8 tmax
      common/vbf_wp_wp_ctmax/tmax
      integer kinreg,firstreg,lastreg
      logical ini
      data ini/.true./
      real * 8 vbf_wp_wp_pwhg_pt2,vbf_wp_wp_powheginput
      external vbf_wp_wp_pwhg_pt2,vbf_wp_wp_powheginput
...
c     initial state radiation
               call vbf_wp_wp_gen_rad_isr(t)
            else
c     final state radiation
               call vbf_wp_wp_gen_rad_fsr(t)
            endif
            if(t.gt.tmax) then
               tmax=t
               kinreg=rad_kinreg
...
c Set up radiation kinematics
      if(tmax.eq.0) then
c Generate a vbf_wp_wp_born like event
         kn_csi=0
         rad_kinreg=0
         return
      else         
         ...
         t=tmax
         if(rad_kinreg.eq.1) then
            call vbf_wp_wp_gen_real_phsp_isr_rad
         else
            call vbf_wp_wp_gen_real_phsp_fsr_rad
         endif
         call vbf_wp_wp_set_rad_scales(t)
         call vbf_wp_wp_sigreal_rad(sig)
         call vbf_wp_wp_gen_real_idx
      endif
      end



      function vbf_wp_wp_pwhg_pt2()
      implicit none
      real * 8 vbf_wp_wp_pwhg_pt2
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_rad.h'
      if(rad_kinreg.eq.1) then
         vbf_wp_wp_pwhg_pt2=(kn_sreal/4)*(1-kn_y**2)*kn_csi**2
      else
         vbf_wp_wp_pwhg_pt2=(kn_sreal/2)*(1-kn_y)*kn_csi**2
      endif
      end



      function vbf_wp_wp_pwhg_upperb_rad()
      implicit none
      real * 8 vbf_wp_wp_pwhg_upperb_rad
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      ...
      if(rad_kinreg.eq.1) then
         if(rad_iupperisr.eq.1) then
            vbf_wp_wp_pwhg_upperb_rad = st_alpha/((1-x)*(1-y**2))
c Possible alternatives:
c rad_iupper=2   vbf_wp_wp_pwhg_upperb_rad = st_alpha/(x*(1-x)*(1-y**2))
c 
c rad_iupper=3:  vbf_wp_wp_pwhg_upperb_rad = st_alpha/(x**2*(1-x)*(1-y**2))
         else
...
c for now use the same
         if(rad_iupperfsr.eq.1) then
            vbf_wp_wp_pwhg_upperb_rad = st_alpha/(csi*(1-y))
         elseif(rad_iupperfsr.eq.2) then
            vbf_wp_wp_pwhg_upperb_rad = st_alpha/(csi**2*(1-y)*(1-csi/2*(1-y))**2)
     2          *csi
     ...
     1            'alternative not implemented'
      ...
      function vbf_wp_wp_pt2solve(pt2,i)
...
c We use it to find its zero in pt2.
      ...
      include 'src/Lib/vbf_wp_wp/pwhg_rad.h'
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      real * 8 vbf_wp_wp_pt2solve,pt2
c i set by vbf_wp_wp_DZERO: 1 for first call, 2 for subsequent calls, 3 for last call
c before a normal exit; not used here
      integer i
      real * 8 xlr,q2,xlam2c,kt2max,cunorm
      integer nlc
      common/vbf_wp_wp_cpt2solve/xlr,q2,kt2max,xlam2c,cunorm,nlc
      real * 8 vbf_wp_wp_B0,sborn,xm,p
      sborn=kn_sborn
      vbf_wp_wp_B0=(11*CA-4*TF*nlc)/(12*pi)
      if(rad_kinreg.eq.1) then
         if(rad_iupperisr.eq.1) then
c see Notes/upperbounding-isr.pdf
            if(pt2.lt.sborn) then
               if(sborn.lt.kt2max) then
                  vbf_wp_wp_pt2solve=cunorm*pi/vbf_wp_wp_B0*(
     ...
     #           + xlr
               ...
               vbf_wp_wp_pt2solve=cunorm*pi/vbf_wp_wp_B0*(log(2d0)
...
c         vbf_wp_wp_pt2solve=cunorm*pi/vbf_wp_wp_B0/2
...
c final state radiation
     ...
     #           + xlr
         elseif(rad_iupperfsr.eq.2) then
            xm=kn_csimax
            p=sqrt(pt2/sborn)
            vbf_wp_wp_pt2solve=cunorm*2*pi*2*(
     ...
     2   /(p*xm)/2.d+0) + xlr
      ...
      subroutine vbf_wp_wp_gen_rad_isr(t)
c Generates hard radiation kinematics according to
c appendix D in ZZ paper.
c
c  common/vbf_wp_wp_cptmin/ptminsq: minimum pt^2 accepted
c 
      implicit none
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      ...
      integer nlc
      common/vbf_wp_wp_cpt2solve/xlr,q2,kt2max,xlam2c,unorm,nlc
      real * 8 xmin,rv,xp,xm,chi,tk,uk,ubound,ufct,
     #   sborn,value,err,tmp1,tmp2,tmp,rvalue,vbf_wp_wp_born,sig
      common/vbf_wp_wp_cdfxmin/xmin
      real * 8 tmax
      common/vbf_wp_wp_ctmax/tmax
      real * 8 vbf_wp_wp_random,vbf_wp_wp_pt2solve,dfxmin,vbf_wp_wp_pwhg_alphas0,vbf_wp_wp_pwhg_upperb_rad
      external vbf_wp_wp_random,vbf_wp_wp_pt2solve,dfxmin,vbf_wp_wp_pwhg_alphas0,vbf_wp_wp_pwhg_upperb_rad
...
c see section 4 in ZZ paper, last paragraph
      ...
      xlr=0
 1    continue
      xlr=xlr+log(vbf_wp_wp_random())
c CERNLIB voodoo:
      call vbf_wp_wp_KERSET('C205. ',0,0,101)
c solve for zero of vbf_wp_wp_pt2solve
c vbf_wp_wp_DZERO(xmin,xmax,x,err,eps,maxcalls,function)
...
c eps: required accuracy
      call vbf_wp_wp_DZERO(rad_ptsqmin,kt2max,t,err,1d-8,1000000,vbf_wp_wp_pt2solve)
c error conditions
      if(t.eq.0.and.err.lt.0d0
     # .and.err.gt.rad_ptsqmin-kt2max) then
         write(*,*) 'vbf_wp_wp_DZERO fails'
...
c radiation in highest bid loop): generate a vbf_wp_wp_born event
         t=-1
         kn_csi=0
         return
      endif
...
c to set xmuren2:
      call vbf_wp_wp_set_rad_scales(t)
      tmp2=st_alpha / vbf_wp_wp_pwhg_alphas0(t,rad_lamll,nlc)
      tmp=tmp1*tmp2
      if(tmp.gt.1) then
...
c using       d sqrt(xm-x) /sqrt(xp-xm) (rad_iupper=1) or d sqrt(xm-x) /(xmin sqrt(xp-xm)) (rad_iupper=2) as upper bound using hit and miss
 2    chi=sqrt(xm-xmin)*vbf_wp_wp_random()
      x=xm-chi**2
      if(rad_iupperisr.eq.1) then
         if(vbf_wp_wp_random().gt.sqrt(xp-xm)/sqrt(xp-x)) goto 2
      elseif(rad_iupperisr.eq.2) then
         if(vbf_wp_wp_random().gt.(xmin*sqrt(xp-xm))/(x*sqrt(xp-x))) goto 2
      endif
c get y (abs to avoid tiny negative values)
      y=sqrt(abs(1-4*x/(1-x)**2*t/sborn))
      if(vbf_wp_wp_random().gt.0.5d0) y=-y
...
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call vbf_wp_wp_uboundfct(ufct,1-x,y)
      if(vbf_wp_wp_random().gt.ufct) goto 1
...
c since these may be expensive.
      ...
      if(vbf_wp_wp_born.lt.0) then
         vbf_wp_wp_born=0
      endif
      if(vbf_wp_wp_born.eq.0) then
...
c that some pdf vanish (typically heavy flavour pdf's)
      ...
      ubound=vbf_wp_wp_born*vbf_wp_wp_pwhg_upperb_rad()*unorm*ufct
      call vbf_wp_wp_gen_real_phsp_isr_rad
      call vbf_wp_wp_sigreal_rad(sig)
      value=sig*kn_jacreal
      if(value.gt.ubound) then
         call vbf_wp_wp_increasecnt(
     # 'upper bound failures in generation of radiation')
      endif
      rvalue=vbf_wp_wp_random()*ubound
      if(rvalue.gt.value) then
         call vbf_wp_wp_increasecnt('vetoed radiation')
         goto 1
      endif
      end


      subroutine vbf_wp_wp_gen_rad_fsr(t)
...
c 
     ...
     #   s,value,err,tmp,rvalue,vbf_wp_wp_born,sig
      common/vbf_wp_wp_cdfxmin/xmin
      real * 8 tmax
      common/vbf_wp_wp_ctmax/tmax
      real * 8 vbf_wp_wp_random,vbf_wp_wp_pt2solve,vbf_wp_wp_pwhg_alphas0,vbf_wp_wp_pwhg_upperb_rad
      external vbf_wp_wp_random,vbf_wp_wp_pt2solve,vbf_wp_wp_pwhg_alphas0,vbf_wp_wp_pwhg_upperb_rad
      unorm=rad_norms(rad_kinreg,rad_ubornidx)
c kn_sborn=kn_sreal:
      s=kn_sborn
...
c vetoes:
      ...
      call vbf_wp_wp_set_rad_scales(t)
      if(rad_iupperfsr.eq.1) then
         tmp=st_alpha / vbf_wp_wp_pwhg_alphas0(t,rad_lamll,nlc)
      elseif(rad_iupperfsr.eq.2) then
         tmp=st_alpha
      elseif(rad_iupperfsr.eq.3) then
         tmp=st_alpha
...
c extra csi dependent factor
         if(vbf_wp_wp_random().gt.csi) goto 1
c get y
         y=1-2*t/(s*csi**2)
c At this point a csi-y pair is generated according to the
c distribution upper(). It is automatically within range.
...
c get y
         y=1-2*t/(s*csi**2)
         if(vbf_wp_wp_random().gt.(csi-t/s)) goto 1
      else
         write(*,*) ' vbf_wp_wp_gen_rad_fsr:  rad_iupper=',rad_iupperfsr,
     1        ' invalid'
      endif
c
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call vbf_wp_wp_uboundfct(ufct,csi,y)
      if(vbf_wp_wp_random().gt.ufct) goto 1
c Veto from upper bound to real value. Count how many vetoes,
c since these may be expensive.
...
c that some pdf vanish (typically heavy flavour pdf's)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      subroutine vbf_wp_wp_add_azimuth
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      ...
      azi=2d0*pi*vbf_wp_wp_random()
      sazi = sin(azi)
      cazi = cos(azi)
      if (kn_csi.eq.0d0) then
         do ileg=1, nlegborn
            call vbf_wp_wp_mrotate(dir,sazi,cazi,kn_pborn(1,ileg))
         enddo
      else
         do ileg=1, nlegreal
            call vbf_wp_wp_mrotate(dir,sazi,cazi,kn_preal(1,ileg))
         enddo
      endif
      end

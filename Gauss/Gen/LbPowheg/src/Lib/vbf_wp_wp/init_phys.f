      subroutine vbf_wp_wp_init_phys
      implicit none
      ...
      external vbf_wp_wp_whichpdfpk,vbf_wp_wp_powheginput
...
c End initialization of common block defaults.
      pdf_ih1=vbf_wp_wp_powheginput('ih1')
      pdf_ih2=vbf_wp_wp_powheginput('ih2')
      if(vbf_wp_wp_whichpdfpk().eq.'lha') then
         pdf_ndns1=vbf_wp_wp_powheginput('lhans1')
         pdf_ndns2=vbf_wp_wp_powheginput('lhans2')
      elseif(vbf_wp_wp_whichpdfpk().eq.'mlm') then
         pdf_ndns1=vbf_wp_wp_powheginput('ndns1')
         pdf_ndns2=vbf_wp_wp_powheginput('ndns2')
      else
         write(*,*) ' unimplemented pdf package',vbf_wp_wp_whichpdfpk()
         stop
      endif
      if(pdf_ndns1.ne.pdf_ndns2) then
         st_lambda5MSB=vbf_wp_wp_powheginput('QCDLambda5')
      else
         call vbf_wp_wp_genericpdfpar(pdf_ndns1,pdf_ih1,st_lambda5MSB,
     1                      scheme,iorder,iret)
      ...
      kn_beams(0,1)=vbf_wp_wp_powheginput('ebeam1')
      kn_beams(0,2)=vbf_wp_wp_powheginput('ebeam2')
      kn_beams(1,1)=0
      kn_beams(1,2)=0
      kn_beams(2,1)=0
      kn_beams(2,2)=0
...
c generation cut: see vbf_wp_wp_gen_born_phsp.f
      kn_ktmin=vbf_wp_wp_powheginput("#bornktmin")
...
c thresholds 
      rad_ptsqmin=vbf_wp_wp_powheginput('#ptsqmin')
      if(rad_ptsqmin.lt.0) rad_ptsqmin=0.8d0
      rad_charmthr2=vbf_wp_wp_powheginput('#charmthr')
      if(rad_charmthr2.lt.0) rad_charmthr2=1.5d0
      rad_charmthr2=rad_charmthr2**2
      rad_bottomthr2=vbf_wp_wp_powheginput('#bottomthr')
      if(rad_bottomthr2.lt.0) rad_bottomthr2=5d0
      rad_bottomthr2=rad_bottomthr2**2
c scale factors
      st_renfact=vbf_wp_wp_powheginput('#renscfact')
      st_facfact=vbf_wp_wp_powheginput('#facscfact')
...
c initialize Lambda values for radiation
      call vbf_wp_wp_init_rad_lambda
c
      call vbf_wp_wp_init_processes

      call vbf_wp_wp_init_couplings

c initialize number of singular regions
      rad_nkinreg=1+(nlegborn-flst_lightpart+1)
      call vbf_wp_wp_genflavreglist


      dbg_softtest=.true.
      dbg_colltest=.true.
      if(flg_withdamp) then
         write(*,*) ' no vbf_wp_wp_soft tests if withdamp is set'
         dbg_softtest=.false.
      endif
      if(flg_bornonly) then
         write(*,*) ' no vbf_wp_wp_soft and coll. tests if bornonly is set'
         dbg_softtest=.false.
         dbg_colltest=.false.
      endif

      function vbf_wp_wp_dotp(p1,p2)
      implicit none
      real * 8 vbf_wp_wp_dotp,p1(0:3),p2(0:3)
      vbf_wp_wp_dotp = (p1(0)*p2(0) - p1(3)*p2(3)) - p1(1)*p2(1) - p1(2)*p2(2)
      end

      function vbf_wp_wp_cdotp(p1,cp2)
      implicit none      
      complex * 16 vbf_wp_wp_cdotp,cp2(0:3)
      real * 8 p1(0:3)
      vbf_wp_wp_cdotp = (p1(0)*cp2(0) - p1(3)*cp2(3))
     #      - p1(1)*cp2(1) - p1(2)*cp2(2)
      end
      
      function vbf_wp_wp_ccdotp(cp1,cp2)
      implicit none      
      complex * 16 vbf_wp_wp_ccdotp,cp1(0:3),cp2(0:3)
      vbf_wp_wp_ccdotp = (cp1(0)*cp2(0) - cp1(3)*cp2(3))
...
C
C
C*********** polarisation vector (vbf_wp_wp_POLVEC) ************
C*********** entry vbf_wp_wp_HELVEC ****************************
C
...
C   bug in vbf_wp_wp_HELVEC for KT=0 corrected Nov.4, 1993
C
C   INPUT:
C
...
C                        -1,0,1) when entry vbf_wp_wp_HELVEC is used
C
C
C   OUTPUT:
...
C                  must be used!
C
      SUBROUTINE vbf_wp_wp_POLVEC(KBAR,LAMBDA,EPSCAR)
C
C   arguments:
C
      real*8  KBAR(0:4)
...
C
                  WRITE(*,*) 'Mass = 0 for Lambda = 3 in vbf_wp_wp_POLVEC'
                  EPSCAR(1) = 0.0d0
                  EPSCAR(2) = 0.0d0
                  EPSCAR(3) = 0.0d0
                  RETURN
...
C
            WRITE (*,*) 'Invalid Lambda in vbf_wp_wp_POLVEC: Lambda = ',LAMBDA
                  EPSCAR(1) = 0.0d0
                  EPSCAR(2) = 0.0d0
                  EPSCAR(3) = 0.0d0
            END IF
      RETURN
C
      ENTRY vbf_wp_wp_HELVEC(KBAR,SIGMAK,LAMBDA,EPSHEL)
C
C   code:
C
C
...
C                  Mass = 0 not allowed for longitudinal polarization
C
                  WRITE(*,*) 'Mass = 0 for Lambda = 0 in vbf_wp_wp_HELVEC'
                  EPSHEL(1) = 0.0d0
                  EPSHEL(2) = 0.0d0
                  EPSHEL(3) = 0.0d0
                  RETURN
...
C                  Unrecognised value of Lambda
C
            WRITE (*,*) 'Invalid Lambda in vbf_wp_wp_HELVEC: Lambda = ',LAMBDA
                  EPSHEL(1) = 0.0d0
                  EPSHEL(2) = 0.0d0
                  EPSHEL(3) = 0.0d0
            END IF
...
c gives vbf_wp_wp_ket(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine vbf_wp_wp_ket(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
...
c gives vbf_wp_wp_ket(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine vbf_wp_wp_ket_limit(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
      complex * 16 psi(1:2,-1:1)
...
c p = physical momentum  p[0] = E > 0
c ferm_type = fermion (+1) or antifermion (-1)
c gives vbf_wp_wp_bra(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine vbf_wp_wp_bra(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
      complex * 16 psi(1:2,-1:1)
...
c                           ( ris(1))
c gives p_slash |vbf_wp_wp_ket,hel> = (       )
c                           ( ris(2))
      subroutine vbf_wp_wp_slash_ket(p,vbf_wp_wp_ket,hel,ris)
      implicit none
      complex * 16 p(0:3)
      complex * 16 vbf_wp_wp_ket(1:2)
      integer hel
      complex * 16 ris(1:2)
      ...
      ris(1) = pslash(1,1)*vbf_wp_wp_ket(1)+pslash(1,2)*vbf_wp_wp_ket(2)
      ris(2) = pslash(2,1)*vbf_wp_wp_ket(1)+pslash(2,2)*vbf_wp_wp_ket(2)
      end


c gives <vbf_wp_wp_bra,hel | p_slash = ( ris(1), ris(2) )      

      subroutine vbf_wp_wp_slash_bra(p,vbf_wp_wp_bra,hel,ris)
      implicit none
      complex * 16 p(0:3)
      complex * 16 vbf_wp_wp_bra(1:2)
      integer hel
      complex * 16 ris(1:2)
      complex * 16 pslash(1:2,1:2)
      complex * 16 I
      ...
      pslash(2,1) = -hel*(p(1) + I*p(2))
      pslash(2,2) = p(0) + hel*p(3)
      ris(1) = pslash(1,1)*vbf_wp_wp_bra(1)+pslash(2,1)*vbf_wp_wp_bra(2)
      ris(2) = pslash(1,2)*vbf_wp_wp_bra(1)+pslash(2,2)*vbf_wp_wp_bra(2)
      end


c <vbf_wp_wp_bra, helbra| slash_p_1 slash_p_2 ... slash_p_n |vbf_wp_wp_ket, helket>
c where list_mom = [p_1, p_2,...., p_n]
c helket = -(-1)^n*helbra, otherwise the result is zero!!!
      subroutine vbf_wp_wp_bra_slash_ket(vbf_wp_wp_bra,vbf_wp_wp_ket,helket,list_mom,n,ris)
      implicit none
...
c      dimension list_mom(0:3,n)
      ...
      helbra = -(-1)**n*helket
      hel = helket
      b(1) = vbf_wp_wp_bra(1,helbra)
      b(2) = vbf_wp_wp_bra(2,helbra)
      k(1) = vbf_wp_wp_ket(1,helket)
      k(2) = vbf_wp_wp_ket(2,helket)
      
      do ii=n,1,-1
         call vbf_wp_wp_slash_ket(list_mom(0,ii),k,hel,new_ket)
         k(1) = new_ket(1)
         k(2) = new_ket(2)
         hel = -hel
      enddo
...
c <vbf_wp_wp_bra, helbra| slash_curr_vertex_1, slash_p1, slash_curr_vertex_2|vbf_wp_wp_ket, helket>
...
c is emitted 
      subroutine vbf_wp_wp_bra_slash_ket_gluon
     #     (vbf_wp_wp_bra,vbf_wp_wp_ket,helket,pbra,pket,pg,eps,list_mom,ris)
      implicit none
      complex * 16 vbf_wp_wp_bra(2,-1:1),vbf_wp_wp_ket(2,-1:1)
      integer helket
...
c      parameter (I=(0,1))
      ...
      integer mu
      complex * 16 vbf_wp_wp_ccdotp
c emissione dalla gamba esterna vbf_wp_wp_bra
      do mu=0,3
      ...
      enddo
      call vbf_wp_wp_bra_slash_ket(vbf_wp_wp_bra,vbf_wp_wp_ket,helket,local_list,5,ris1)

      ris1 = ris1 / vbf_wp_wp_ccdotp(local_list(0,2),local_list(0,2))/
     #     vbf_wp_wp_ccdotp(local_list(0,4),local_list(0,4))
...
c emissione dal propagatore interno
      do mu=0,3
         ...
         local_list(mu,5) = list_mom(mu,3)
      enddo
      call vbf_wp_wp_bra_slash_ket(vbf_wp_wp_bra,vbf_wp_wp_ket,helket,local_list,5,ris2)
      ris2 = ris2 / vbf_wp_wp_ccdotp(local_list(0,2),local_list(0,2)) /
     #     vbf_wp_wp_ccdotp(local_list(0,4),local_list(0,4))


c emissione dalla gamba esterna vbf_wp_wp_ket
      do mu=0,3
         local_list(mu,1) = list_mom(mu,1)
         local_list(mu,2) = list_mom(mu,2)-pg(mu)
      ...
      enddo
      call vbf_wp_wp_bra_slash_ket(vbf_wp_wp_bra,vbf_wp_wp_ket,helket,local_list,5,ris3)
      ris3 = ris3 / vbf_wp_wp_ccdotp(local_list(0,2),local_list(0,2))/
     #     vbf_wp_wp_ccdotp(local_list(0,4),local_list(0,4))
...
C NO TRIPLE GLUON VERTEX HERE!!!
      subroutine vbf_wp_wp_bra_slash_ket_g1_g2
     #     (vbf_wp_wp_bra,vbf_wp_wp_ket,helket,pbra,pket,pg1,eps1,pg2,eps2,jcurr,ris)
      implicit none
      complex * 16 vbf_wp_wp_bra(2,-1:1),vbf_wp_wp_ket(2,-1:1)
      integer helket
...
c emission  <vbf_wp_wp_bra| 1,2,j |vbf_wp_wp_ket>
      do mu=0,3
...
c emission <vbf_wp_wp_bra|1,j,2|vbf_wp_wp_ket> 
      do mu=0,3
         local_list(mu,1) = eps1(mu)
         local_list(mu,2) = pg1(mu)+pbra(mu)
...
c emission <vbf_wp_wp_bra|j,1,2|vbf_wp_wp_ket>
...
c where list_mom = [p_1, p_2,...., p_n]
c helket = -(-1)^n*helbra, otherwise the result is zero!!!
      subroutine vbf_wp_wp_bra_slash_ket1(vbf_wp_wp_bra,vbf_wp_wp_ket,helbra,list_mom,n,ris)
      implicit none
      complex * 16 vbf_wp_wp_bra(2,-1:1),vbf_wp_wp_ket(2,-1:1)
      integer helket
      integer n
...
c      dimension list_mom(0:3,n)
      ...
      do ii=1,n
         call vbf_wp_wp_slash_bra(list_mom(0,ii),b,hel,new_bra)
         b(1) = new_bra(1)
         b(2) = new_bra(2)
         hel = -hel
      enddo
...
c the helicity of the vbf_wp_wp_bra and the vbf_wp_wp_ket are the same, otherwise this is zero!
c sig^mu = <vbf_wp_wp_bra, helbra| gamma^mu |vbf_wp_wp_ket, helket>
      subroutine vbf_wp_wp_bra_gamma_ket(vbf_wp_wp_bra,vbf_wp_wp_ket,hel,sig)
      implicit none
      complex * 16 vbf_wp_wp_bra(2,-1:1),vbf_wp_wp_ket(2,-1:1)
      complex * 16 b(2),k(2)
      integer hel
      ...
      b(1) = vbf_wp_wp_bra(1,hel)
      b(2) = vbf_wp_wp_bra(2,hel)
      k(1) = vbf_wp_wp_ket(1,hel)
      k(2) = vbf_wp_wp_ket(2,hel)
...
c here we have just all the vbf_wp_wp_indexes up or down ==> the minus sign on the
...
c the helicity of the vbf_wp_wp_bra and the vbf_wp_wp_ket are the same, otherwise this is zero!
c sig^mu = 1/vbf_wp_wp_dotp(pbra+pglu,pbra+pglu) * 
c   <vbf_wp_wp_bra, pbra, hel| eps_slash, (pbra+pglu)_slash, gamma^mu |vbf_wp_wp_ket, pket, hel>+
c          1/vbf_wp_wp_dotp(pket-pglu,pket-pglu) * 
c   <vbf_wp_wp_bra, pbra, hel| gamma^mu, (pket-pglu)_slash, eps_slash |vbf_wp_wp_ket, pket, hel>

      subroutine vbf_wp_wp_bra_gamma_ket_gluon(vbf_wp_wp_bra,vbf_wp_wp_ket,hel,pbra,pket,pglu,eps,sig)
      implicit none
      complex * 16 vbf_wp_wp_bra(2,-1:1),vbf_wp_wp_ket(2,-1:1)
      integer hel
      ...
      parameter (I=(0,1))
      complex*16 list_mom(0:3,3)
      real * 8 gamma(0:3),prop1,prop2,vbf_wp_wp_dotp
      integer mu,nu
      ...
      do mu=0,3
         ...
         enddo 
         call vbf_wp_wp_bra_slash_ket(vbf_wp_wp_bra,vbf_wp_wp_ket,hel,list_mom,3,J1(mu))
      enddo
      ...
      do mu=0,3
         ...
         enddo 
         call vbf_wp_wp_bra_slash_ket(vbf_wp_wp_bra,vbf_wp_wp_ket,hel,list_mom,3,J2(mu))
      enddo


      prop1 = 2*vbf_wp_wp_dotp(pglu,pbra)
      prop2 = -2*vbf_wp_wp_dotp(pglu,pket)
...
c with a single vertex inserted
      subroutine vbf_wp_wp_bra_gamma_ket_3gv(vbf_wp_wp_bra,vbf_wp_wp_ket,hel,pbra,pket,p1,eps1,
     #                  p2,eps2,sig)
      implicit none
      complex * 16 vbf_wp_wp_bra(2,-1:1),vbf_wp_wp_ket(2,-1:1)
      integer hel
     ...
     #            eps1(0:3),eps2(0:3)
      ...
      coeff1=2d0*vbf_wp_wp_dotp(p1,eps2)+vbf_wp_wp_dotp(p2,eps2)
      coeff2=-2d0*vbf_wp_wp_dotp(p2,eps1)-vbf_wp_wp_dotp(p1,eps1)
      coeff3=vbf_wp_wp_dotp(eps1,eps2)
     ...
     #             coeff3*(p2(mu)-p1(mu))
      ...
      prop1 = 2*vbf_wp_wp_dotp(p1,p2)*(vbf_wp_wp_dotp(ptemp,ptemp)+2*vbf_wp_wp_dotp(pbra,ptemp))
      prop2 = 2*vbf_wp_wp_dotp(p1,p2)*(vbf_wp_wp_dotp(ptemp,ptemp)-2*vbf_wp_wp_dotp(ptemp,pket))
...
c all the fermionic line  (NO COLOR STRUCTURE!!)
c the helicity of the vbf_wp_wp_bra and the vbf_wp_wp_ket are the same, otherwise this is zero!
c sig^mu = 1/vbf_wp_wp_dotp(pbra+pglu,pbra+pcurr) * 
c   <vbf_wp_wp_bra, pbra, hel| j_slash, (pbra+pcurr)_slash, gamma^mu |vbf_wp_wp_ket, pket, hel>+
c          1/vbf_wp_wp_dotp(pket-pcurr,pket-pcurr) * 
c   <vbf_wp_wp_bra, pbra, hel| gamma^mu, (pket-pcurr)_slash, j_slash |vbf_wp_wp_ket, pket, hel>

      subroutine vbf_wp_wp_bra_gamma_ket_curr(vbf_wp_wp_bra,vbf_wp_wp_ket,hel,pbra,
     # pket,pcurr,jcurr,sig)
      ...
      prop1 = vbf_wp_wp_dotp(pcurr,pcurr)*(vbf_wp_wp_dotp(pcurr,pcurr)+2*vbf_wp_wp_dotp(pcurr,pbra))
      prop2 = vbf_wp_wp_dotp(pcurr,pcurr)*(vbf_wp_wp_dotp(pcurr,pcurr)-2*vbf_wp_wp_dotp(pcurr,pket))
      ...
      subroutine vbf_wp_wp_exchange_momenta(p1,p2)
      implicit none
...
c return a vbf_wp_wp_random number between 0 and 1
      function vbf_wp_wp_rand_num()
      implicit none
      real * 8 vbf_wp_wp_rand_num
      real * 8 vbf_wp_wp_random2
      integer num,num2
      COMMON/vbf_wp_wp_SEED/num,num2
c set vbf_wp_wp_random seeds
      data NUM/12345/,NUM2/67890/
      vbf_wp_wp_rand_num = vbf_wp_wp_random2(num,num2)
      end
      
      subroutine vbf_wp_wp_exchange_mom(p,i,j,dim,pnew)
      implicit none
...
C-----------------------------------------------------------------------
      FUNCTION vbf_wp_wp_random2(iseed1,iseed2)
C     MAIN vbf_wp_wp_random NUMBER GENERATOR
C     USES METHOD OF l'Ecuyer, (VIA F.JAMES, COMP PHYS COMM 60(1990)329)
C-----------------------------------------------------------------------
      IMPLICIT NONE
      DOUBLE PRECISION vbf_wp_wp_random2
      ...
      vbf_wp_wp_random2=DBLE(IZ)/2147483589
c      vbf_wp_wp_random2=DBLE(IZ)*4.656613001013252D-10

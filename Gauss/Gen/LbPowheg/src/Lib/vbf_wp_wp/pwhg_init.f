      ...
      integer i1,n1,n2
      call vbf_wp_wp_init_flsttag
      flg_debug=.false.
      if(vbf_wp_wp_powheginput("#flg_debug").eq.1) flg_debug=.true.
c whether to output negative weights or not
      flg_withnegweights=.false.
      if(vbf_wp_wp_powheginput("#withnegweights").eq.1)flg_withnegweights=.true.
c See if we have weighted events
      flg_weightedev=.false.
      if(vbf_wp_wp_powheginput("#bornsuppfact").gt.0) flg_weightedev=.true.
      if(vbf_wp_wp_powheginput("#ptsupp").gt.0) then
...
c     amplitudes, for vbf_wp_wp_born, real and virtual contributions
      flg_smartsig=.true.
      if(vbf_wp_wp_powheginput("#smartsig").eq.0) flg_smartsig=.false.
c     if true also counterterm are output in NLO tests (default)
      flg_withsubtr=.true.
      if(vbf_wp_wp_powheginput("#withsubtr").eq.0) flg_withsubtr=.false.
c     if true use vbf_wp_wp_born zero damping factor and include remnants
c     set flg_withdamp to true also if the damping of real radiation
c     is required
      flg_withdamp=.false.
      if((vbf_wp_wp_powheginput("#withdamp").eq.1).or.
     $  (vbf_wp_wp_powheginput("#hfact").gt.0d0))   flg_withdamp=.true.
c     If set do only the vbf_wp_wp_born term
      flg_bornonly=.false.
      if (vbf_wp_wp_powheginput("#bornonly").eq.1) flg_bornonly=.true.
c     initialize vbf_wp_wp_random number sequence
      i1=vbf_wp_wp_powheginput('#iseed')
      if (i1.lt.0) i1=0
      n1=vbf_wp_wp_powheginput('#rand1')
      if (n1.lt.0) n1=0
      n2=vbf_wp_wp_powheginput('#rand2')
      if (n2.lt.0) n2=0
c select which upper bounding function form
      rad_iupperisr=vbf_wp_wp_powheginput("#iupperisr")
      if(rad_iupperisr.lt.0) rad_iupperisr=1
      rad_iupperfsr=vbf_wp_wp_powheginput("#iupperfsr")
      if(rad_iupperfsr.lt.0) rad_iupperfsr=2
c info on pdf for each event
      flg_pdfreweight=.false.
      if(vbf_wp_wp_powheginput("#pdfreweight").eq.1)flg_pdfreweight=.true.
c
      call vbf_wp_wp_setrandom(i1,n1,n2)
...
c     inside the user-defined subroutine vbf_wp_wp_init_processes
      lprup(1)=10001
c     initialize physical parameters
      call vbf_wp_wp_init_phys
...
c now the cross section is available
      ...
      subroutine vbf_wp_wp_init_flsttag
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      integer j,l
      do j=1,maxprocreal

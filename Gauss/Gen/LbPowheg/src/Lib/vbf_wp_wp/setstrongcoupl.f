      subroutine vbf_wp_wp_setscalesbtilde
      implicit none
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/pwhg_st.h'
      real * 8 vbf_wp_wp_pwhg_alphas
      external vbf_wp_wp_pwhg_alphas
      real * 8 muf,mur
      call vbf_wp_wp_set_fac_ren_scales(muf,mur)
      st_mufact2= muf**2*st_facfact**2
      st_muren2 = mur**2*st_renfact**2
      st_alpha  = vbf_wp_wp_pwhg_alphas(st_muren2,st_lambda5MSB,st_nlight)
      end

      subroutine vbf_wp_wp_set_rad_scales(ptsq)
      implicit none
      real * 8 ptsq
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
...
c     SAER FIX
      ...
      external vbf_wp_wp_whichpdfpk
      integer ini,mem
      data ini/0/
      save ini,q2min
      
      if (ini.eq.0) then
         if( vbf_wp_wp_whichpdfpk().eq.'lha') then    
           mem = 0
           call vbf_wp_wp_getq2min(mem,q2min)
...
c     is greater or equal to the mass of heavy quark 
        elseif( vbf_wp_wp_whichpdfpk().eq.'mlm') then    
c ad hoc value here (vbf_wp_wp_MLMPDF does not provide this)
           q2min=2d0
        else
           write(*,*) ' unimplemented pdf package',vbf_wp_wp_whichpdfpk()
           stop
        endif 
        ini=1
      endif
...
c     In case of final-state radiation, vbf_wp_wp_born and real PDF's
...
c     mufact (50 is an arbitrary choice).
      ...
      st_muren2=ptsq
      st_alpha = vbf_wp_wp_pwhg_alphas(st_muren2,st_lambda5MSB,-1)
     ...
     #   (1+st_alpha/(2*pi)*((67d0/18-pi**2/6)*ca-5d0/9*nf))
      ...
      subroutine vbf_wp_wp_init_rad_lambda
      implicit none
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/pwhg_st.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_rad.h'
      real * 8 vbf_wp_wp_B0,mu0sq,as,vbf_wp_wp_pwhg_alphas
      external vbf_wp_wp_pwhg_alphas
      vbf_wp_wp_B0=(33-2*5)/(12*pi)
      mu0sq=(2*st_lambda5MSB)**2
c running value of alpha at initial scale (see notes: running_coupling)
      as=vbf_wp_wp_pwhg_alphas(mu0sq,st_lambda5MSB,-1)
c for better NLL accuracy (FNO2006, (4.32) and corresponding references)
      as=as*(1+as/(2*pi)*((67d0/18-pi**2/6)*ca-5d0/9*3))
      rad_lamll=sqrt(exp(-1/(vbf_wp_wp_B0*as))*mu0sq)
      end

      function vbf_wp_wp_pwhg_alphas0(q2,xlam,inf)
      implicit none
      real * 8 vbf_wp_wp_pwhg_alphas0,q2,xlam
      integer inf
      real * 8 pi
      parameter (pi=3.141592653589793d0)
      real * 8 vbf_wp_wp_B0
      vbf_wp_wp_B0=(33-2*inf)/(12*pi)
      vbf_wp_wp_pwhg_alphas0=1/(vbf_wp_wp_B0*log(q2/xlam**2))
      end
      
C----------------------------------------------------------------------------
C-------------------------------------------------------------------
C------- ALPHA QCD -------------------------------------
c Program to calculate vbf_wp_wp_alfa strong with nf flavours,
c as a function of lambda with 5 flavors.
c The value of vbf_wp_wp_alfa is matched at the thresholds q = mq.
c When invoked with nf < 0 it chooses nf as the number of
c flavors with mass less then q.
c
      function vbf_wp_wp_pwhg_alphas(q2,xlam,inf)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_rad.h'
      real * 8 vbf_wp_wp_pwhg_alphas,q2,xlam
      integer inf
      real * 8 pi
      parameter (pi=3.141592653589793d0)
...
c      data ini/.true./
c      save ini
c      vbf_wp_wp_pwhg_alphas = 30d0
c      if (ini) then
c         write(*,*) '****************************************'
c         write(*,*) '****************************************'
c         write(*,*) '      RETURN alpha_s = ',vbf_wp_wp_pwhg_alphas
c         write(*,*) '****************************************'
c         write(*,*) '****************************************'
c         ini = .false.         
c      endif           
c      return
     ...
     #        - 1/( 1/(b3 * xlc) - xllc*bp3/(b3 * xlc)**2 ) + c45
      ...
      if    ( nf .eq. 5 ) then
        vbf_wp_wp_pwhg_alphas = 1/(b5 * xlq) -  bp5/(b5 * xlq)**2 * xllq
      elseif( nf .eq. 4 ) then
        vbf_wp_wp_pwhg_alphas =
     #    1/( 1/(1/(b4 * xlq) - bp4/(b4 * xlq)**2 * xllq) + c45 )
      elseif( nf .eq. 3 ) then
        vbf_wp_wp_pwhg_alphas =
     #    1/( 1/(1/(b3 * xlq) - bp3/(b3 * xlq)**2 * xllq) + c35 )
      else
        print *,'error in vbf_wp_wp_alfa: unimplemented # of light flavours',nf
        call exit(1)
      endif
      return
      end

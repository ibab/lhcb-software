c
      subroutine vbf_wp_wp_qqwppqqj(pbar,sign,qbar,gsign,bos,ucds)
      implicit none
c
...
C
C  vbf_wp_wp_qqwppqqj calculates the matrix elements**2 for electroweak
...
c     odd psi is a vbf_wp_wp_ket, if even psi is a bra.
...
c
        call vbf_wp_wp_curr6(-1,psi(1,-1,2),p(0,2),psi(1,-1,1),p(0,1),
     #						jqq(0,-1,1,is1,0))      
        call vbf_wp_wp_curr6(-1,psi(1,-1,4),p(0,4),psi(1,-1,3),p(0,3),
     #						jqq(0,-1,2,is3,0))
c
...
c  Get the gluon polarization vector and the gluon emission spinors
      do l = 1,2	! 2 gluon polarizations
         call vbf_wp_wp_POLVEC(qbar,l,eps(0,l))  ! get gluon pol.vectors
	 	 
         isig = -1	! fermion helicity 
 
c     NOTES for bras and kets: .true. if psi is a 2-spinor of the chi
c     form as output by vbf_wp_wp_PSI0M, .false. otherwise.  the last entry is
c     the sum of the two momenta (p plus q) and effectively the
c     momentum of the new spinor.
            
            call vbf_wp_wp_KET2R(psi(1,isig,1),.true.,p(0,1),isig,q,eps(0,l),
     $           braketg(1,isig,1,l),pq(0,1))      ! |q,1>_l,isig1
            call vbf_wp_wp_BRA2R(psi(1,isig,2),.true.,p(0,2),isig,q,eps(0,l),
     $           braketg(1,isig,2,l),pq(0,2))      ! <2,q|_l,isig2
            call vbf_wp_wp_KET2R(psi(1,isig,3),.true.,p(0,3),isig,q,eps(0,l),
     $           braketg(1,isig,3,l),pq(0,3))      ! |q,3>_l,isig3
            call vbf_wp_wp_BRA2R(psi(1,isig,4),.true.,p(0,4),isig,q,eps(0,l),
...
c	gluon from upper line:
      do l = 1, 2 ! gluon polarizations
         call vbf_wp_wp_curr6(-1,psi(1,-1,2),p(0,2),braketg(1,-1,1,l),pq(0,1),jh1)
c                                            =  <2|vertex|q,1>_l,isig1
         call vbf_wp_wp_curr6(-1,braketg(1,-1,2,l),pq(0,2),psi(1,-1,1),p(0,1),jh2)
...
c	gluon from lower line:
         call vbf_wp_wp_curr6(-1,psi(1,-1,4),p(0,4),braketg(1,-1,3,l),pq(0,3),jh1)
c                                           =   <4|gam.mu|q,3>_l,isig3
         call vbf_wp_wp_curr6(-1,braketg(1,-1,4,l),pq(0,4),psi(1,-1,3),p(0,3),jh2)
...
c
      ...
      mww = vbf_wp_wp_contract_Tjj(wppwpp(0,0,j),jqq(0,-1,1,is1,l),
     #      					jqq(0,-1,2,is3,0))
      ...
      mww = vbf_wp_wp_contract_Tjj(wppwpp(0,0,j),jqq(0,-1,1,is1,0),
     #						jqq(0,-1,2,is3,l))
      mat(j,l,1) = mww*clr(3,3,-1)**2
...
c 
	...
	! only fermions with isig = -1 couple to W (-> no loop for isig)
	 ...
	 call vbf_wp_wp_KET2C(psi(1,-1,i),.true.,p(0,i),-1,qe,we,
     1  	    psiwe(1,i),fqe(0,i))
	 call vbf_wp_wp_KET2C(psi(1,-1,i),.true.,p(0,i),-1,qu,wu,
     1  	    psiwu(1,i),fqu(0,i))
	 call vbf_wp_wp_BRA2C(psi(1,-1,i+1),.true.,p(0,i+1),-1,qe,we,
     1  	    psiwe(1,i+1),fqe(0,i+1))
	 call vbf_wp_wp_BRA2C(psi(1,-1,i+1),.true.,p(0,i+1),-1,qu,wu,
     1  	    psiwu(1,i+1),fqu(0,i+1))
     
	  
	 call vbf_wp_wp_curr6(-1,psi(1,-1,i+1),p(0,i+1),
     1  	       psiwe(1,i),fqe(0,i), jwe(0,-1,is,i,l)   )
	 call vbf_wp_wp_curr6(-1,psiwe(1,i+1),fqe(0,i+1),
     1  	       psi(1,-1,i),p(0,i), jwe(0,-1,is,i+1,l) )
	 call vbf_wp_wp_curr6(-1,psi(1,-1,i+1),p(0,i+1),
     1  	       psiwu(1,i),fqu(0,i), jwu(0,-1,is,i,l)   )
	 call vbf_wp_wp_curr6(-1,psiwu(1,i+1),fqu(0,i+1),
...
c attach W->ev to f1 or f2 / f3 or f4:
 ...
 ! gluon radiation from fermion i / i+1
 	 do l = 1,2
            	call vbf_wp_wp_KET2C(braketg(1,-1,i,l),.false.,pq(0,i),
     $                     -1,qe,we,braketgwe(1,-1,i,l),pgwe(0,i))
            	call vbf_wp_wp_BRA2C(braketg(1,-1,i+1,l),.false.,pq(0,i+1),
     $                 -1,qe,we,braketgwe(1,-1,i+1,l),pgwe(0,i+1))
		
		call vbf_wp_wp_KET2R(psiwe(1,i),.false.,fqe(0,i),-1,
     $	    		    q,eps(0,l),braketweg(1,-1,i,l),pewg(0,i))      
            	call vbf_wp_wp_BRA2R(psiwe(1,i+1),.false.,fqe(0,i+1),-1,
     $	    		q,eps(0,l),braketweg(1,-1,i+1,l),pewg(0,i+1))      


   	    call vbf_wp_wp_curr6(-1,psi(1,-1,i+1),p(0,i+1),
     $	    			braketgwe(1,-1,i,l),pgwe(0,i),jwgi) 		
      	    call vbf_wp_wp_curr6(-1,braketgwe(1,-1,i+1,l),pgwe(0,i+1),
     $	    			psi(1,-1,i),p(0,i),jwgii)		       
     
      	    call vbf_wp_wp_curr6(-1,psi(1,-1,i+1),p(0,i+1),
     $			        braketweg(1,-1,i,l),pewg(0,i),jgwi)
      	    call vbf_wp_wp_curr6(-1,braketweg(1,-1,i+1,l),pewg(0,i+1),
     $			        psi(1,-1,i),p(0,i),jgwii)
 
  ! gluon radiation from fermion i+1 / i
     	    call vbf_wp_wp_curr6(-1,braketg(1,-1,i+1,l),pq(0,i+1),
...
c attach W->muvm to f1 or f2 / f3 or f4:
 ...
 ! gluon radiation from fermion i / i+1
     ...
     $                 -1,qu,wu,braketgwu(1,-1,i+1,l),pgwu(0,i+1))
            	...
            	call vbf_wp_wp_KET2R(psiwu(1,i),.false.,fqu(0,i),-1,
     $	    		q,eps(0,l),braketwug(1,-1,i,l),puwg(0,i))      
            	call vbf_wp_wp_BRA2R(psiwu(1,i+1),.false.,fqu(0,i+1),-1,
     $	    		q,eps(0,l),braketwug(1,-1,i+1,l),puwg(0,i+1))      

   	    call vbf_wp_wp_curr6(-1,psi(1,-1,i+1),p(0,i+1),
     $	    			braketgwu(1,-1,i,l),pgwu(0,i),jwgi)	
      	    call vbf_wp_wp_curr6(-1,braketgwu(1,-1,i+1,l),pgwu(0,i+1),
     $	    			psi(1,-1,i),p(0,i),jwgii)		       
     
      	    call vbf_wp_wp_curr6(-1,psi(1,-1,i+1),p(0,i+1),
     $			        braketwug(1,-1,i,l),puwg(0,i),jgwi)
      	    call vbf_wp_wp_curr6(-1,braketwug(1,-1,i+1,l),puwg(0,i+1),
     $			        psi(1,-1,i),p(0,i),jgwii)
...
c
c  for the q^mu*q^nu/M_V^2 terms in the vbf_wp_wp_gauge boson propagators 
c  we need zeu/zue:
c
      l = 0
	
      jj21u(l) = vbf_wp_wp_dotcc(wu,jqq(0,-1,1,is1,l))
      jj43e(l) = vbf_wp_wp_dotcc(we,jqq(0,-1,2,is3,l))
     
      jj21e(l) = vbf_wp_wp_dotcc(we,jqq(0,-1,1,is1,l))
      jj43u(l) = vbf_wp_wp_dotcc(wu,jqq(0,-1,2,is3,l))
...
c We from upper line, Wu from lower line:
	do ide = 1,2 ! fermion id for external We coupling
	  do idu = 3,4 ! fermion id for external Wu coupling
	     j = 2
	     mb(ide,idu,j,l) = 
     &            -vbf_wp_wp_dotcc(jwe(0,-1,is1,ide,l),jwu(0,-1,is3,idu,0))
	     j = 3
	     mb(ide,idu,j,l) = 
     &           -vbf_wp_wp_dotcc(jwe(0,-1,is1,ide,0),jwu(0,-1,is3,idu,l))
	  enddo !idu
	enddo !ide
...
c Wu from upper line, We from lower line:
	do ide = 3,4 ! fermion id for external We coupling
	  do idu = 1,2 ! fermion id for external Wu coupling
	     j = 2
	     mb(ide,idu,j,l) = 
     &           -vbf_wp_wp_dotcc(jwu(0,-1,is1,idu,l),jwe(0,-1,is3,ide,0))
	     j = 3
	     mb(ide,idu,j,l) =
     &            -vbf_wp_wp_dotcc(jwu(0,-1,is1,idu,0),jwe(0,-1,is3,ide,l))
	  enddo !idu
	enddo !ide
...
c lk=2 and kk=1 is for external boson from lower line
      ...
      ! l = 0:
       ...
       is = is3
        ! VW-> e ve:	
        call vbf_wp_wp_contract_T1j(awen(0,0,lk,1+lk),jqq(0,-1,kk,is,0),
     $       				    epsawe(0,lk,1+lk,0)) !for AW
        call vbf_wp_wp_contract_T1j(zwen(0,0,lk,1+lk),jqq(0,-1,kk,is,0),
     $       				    epszwe(0,lk,1+lk,0)) !for ZW
        ! VW-> mu vm:
        call vbf_wp_wp_contract_T1j(awmu(0,0,lk,1+lk),jqq(0,-1,kk,is,0),
     $       				    epsawu(0,lk,1+lk,0)) !for AW
        call vbf_wp_wp_contract_T1j(zwmu(0,0,lk,1+lk),jqq(0,-1,kk,is,0),
     $       				    epszwu(0,lk,1+lk,0)) !for ZW
     
        qepszwe(lk,1+lk,0) = 
     &           -vbf_wp_wp_dotrc(quu(0,1+lk),epszwe(0,lk,1+lk,0))*zm2i(2)
        qepszwu(lk,1+lk,0) =
     &            -vbf_wp_wp_dotrc(qee(0,1+lk),epszwu(0,lk,1+lk,0))*zm2i(2)
     
        do l = 1,2
           call vbf_wp_wp_contract_T1j(awen(0,0,lk,4-lk),jqq(0,-1,kk,is,l),
     $  				     epsawe(0,lk,4-lk,l)) 
           call vbf_wp_wp_contract_T1j(zwen(0,0,lk,4-lk),jqq(0,-1,kk,is,l),
     $  				     epszwe(0,lk,4-lk,l)) 
           call vbf_wp_wp_contract_T1j(awmu(0,0,lk,4-lk),jqq(0,-1,kk,is,l),
     $  				     epsawu(0,lk,4-lk,l)) 
           call vbf_wp_wp_contract_T1j(zwmu(0,0,lk,4-lk),jqq(0,-1,kk,is,l),
     $  				     epszwu(0,lk,4-lk,l)) 
         	 
	   qepszwe(lk,4-lk,l) =
     &            -vbf_wp_wp_dotrc(quu(0,4-lk),epszwe(0,lk,4-lk,l))*zm2i(2)
	   qepszwu(lk,4-lk,l) =
     &            -vbf_wp_wp_dotrc(qee(0,4-lk),epszwu(0,lk,4-lk,l))*zm2i(2)
	...
	lk = 2
        ...
        is = is1
	 ! VW-> e ve: 	
	 call vbf_wp_wp_contract_T1j(waen(0,0,lk,1+lk),jqq(0,-1,kk,is,0),
     $       				    epswae(0,lk,1+lk,0)) !for WA
	 call vbf_wp_wp_contract_T1j(wzen(0,0,lk,1+lk),jqq(0,-1,kk,is,0),
     $        				    epswze(0,lk,1+lk,0)) !for WZ
	 ! VW-> mu vm:
	 call vbf_wp_wp_contract_T1j(wamu(0,0,lk,1+lk),jqq(0,-1,kk,is,0),
     $       				    epswau(0,lk,1+lk,0)) !for WA
	 call vbf_wp_wp_contract_T1j(wzmu(0,0,lk,1+lk),jqq(0,-1,kk,is,0),
     $       				    epswzu(0,lk,1+lk,0)) !for WZ
	
         qepszwe(lk,1+lk,0) = 
     &           -vbf_wp_wp_dotrc(qee(0,1+lk),epswze(0,lk,1+lk,0))*zm2i(2)
         qepszwu(lk,1+lk,0) =
     &            -vbf_wp_wp_dotrc(quu(0,1+lk),epswzu(0,lk,1+lk,0))*zm2i(2)
    
        do l = 1,2
           call vbf_wp_wp_contract_T1j(waen(0,0,lk,4-lk),jqq(0,-1,kk,is,l),
     $  		        	    epswae(0,lk,4-lk,l)) 
           call vbf_wp_wp_contract_T1j(wzen(0,0,lk,4-lk),jqq(0,-1,kk,is,l),
     $  		        	    epswze(0,lk,4-lk,l)) 
           call vbf_wp_wp_contract_T1j(wamu(0,0,lk,4-lk),jqq(0,-1,kk,is,l),
     $  		        	    epswau(0,lk,4-lk,l)) 
           call vbf_wp_wp_contract_T1j(wzmu(0,0,lk,4-lk),jqq(0,-1,kk,is,l),
     $  				    epswzu(0,lk,4-lk,l))
         	 
	   qepszwe(lk,4-lk,l) = 
     &           -vbf_wp_wp_dotrc(qee(0,4-lk),epswze(0,lk,4-lk,l))*zm2i(2)
	   qepszwu(lk,4-lk,l) =
     &            -vbf_wp_wp_dotrc(quu(0,4-lk),epswzu(0,lk,4-lk,l))*zm2i(2)
...
c vbf_wp_wp_gauge boson from upper quark line:	   
	  ...
	  if (jlog2) then
	  j = 2
      	  m2we(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is1,1,l),epszwu(0,lk,j,0))    
	  m1we(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is1,2,l),epszwu(0,lk,j,0))	    
	  w2we(j,l) = qepszwu(lk,j,0)*jj21e(l)    
	 
	  m2ze(j,l) = 
     &          (m2we(j,l)-w2we(j,l))*clr(ifl(1),3,-1)*clr(ifl(2),2,-1)	    
	  m2ae(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is1,1,l),epsawu(0,lk,j,0))*
     &	  	 	    clr(ifl(1),3,-1)*clr(ifl(2),1,-1)	  
	  m2e(j,l)  = m2ze(j,l)+m2ae(j,l)
	      
	  m1ze(j,l) =
     &          (m1we(j,l)+w2we(j,l))*clr(ifl(2),3,-1)*clr(ifl(1),2,-1)	  
	  m1ae(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is1,2,l),epsawu(0,lk,j,0))*
     &	  	 	    clr(ifl(2),3,-1)*clr(ifl(1),1,-1)	  
	  m1e(j,l) = m1ze(j,l)+m1ae(j,l)
	   	  
	  
      	  m2wu(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is1,1,l),epszwe(0,lk,j,0))    
	  m1wu(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is1,2,l),epszwe(0,lk,j,0))	    
	  w2wu(j,l) = qepszwe(lk,j,0)*jj21u(l)    
	 
	  m2zu(j,l) =
     &          (m2wu(j,l)-w2wu(j,l))*clr(ifl(1),3,-1)*clr(ifl(2),2,-1)	    
	  m2au(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is1,1,l),epsawe(0,lk,j,0))*
     &	  	 	    clr(ifl(1),3,-1)*clr(ifl(2),1,-1)	  
	  m2u(j,l)  = m2zu(j,l)+m2au(j,l)
	      
	  m1zu(j,l) =
     &          (m1wu(j,l)+w2wu(j,l))*clr(ifl(2),3,-1)*clr(ifl(1),2,-1)	  
	  m1au(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is1,2,l),epsawe(0,lk,j,0))*
     &	  	 	    clr(ifl(2),3,-1)*clr(ifl(1),1,-1)	  
     ...
     &            -(m1e(j,l)+m1u(j,l)+m2e(j,l)+m2u(j,l))*clr(3,3,-1)
	  ...
	  if (jlog3) then
	  j = 3
      	  m2we(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is1,1,0),epszwu(0,lk,j,l))    
	  m1we(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is1,2,0),epszwu(0,lk,j,l))	    
	  w2we(j,l) = qepszwu(lk,j,l)*jj21e(0)    
	 
	  m2ze(j,l) = 
     &           (m2we(j,l)-w2we(j,l))*clr(ifl(1),3,-1)*clr(ifl(2),2,-1)
    
	  m2ae(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is1,1,0),epsawu(0,lk,j,l))*
     &	  	 	    clr(ifl(1),3,-1)*clr(ifl(2),1,-1)	  
	  m2e(j,l)  = m2ze(j,l)+m2ae(j,l)
     ...
     &           (m1we(j,l)+w2we(j,l))*clr(ifl(2),3,-1)*clr(ifl(1),2,-1)
	  ...
	  m1ae(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is1,2,0),epsawu(0,lk,j,l))*
     &	  	 	    clr(ifl(2),3,-1)*clr(ifl(1),1,-1)	  
	  m1e(j,l) = m1ze(j,l)+m1ae(j,l)
	  	   
	   
      	  m2wu(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is1,1,0),epszwe(0,lk,j,l))    
	  m1wu(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is1,2,0),epszwe(0,lk,j,l))	    
	  w2wu(j,l) = qepszwe(lk,j,l)*jj21u(0)    
	 
	  m2zu(j,l) = 
     &           (m2wu(j,l)-w2wu(j,l))*clr(ifl(1),3,-1)*clr(ifl(2),2,-1)
    
	  m2au(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is1,1,0),epsawe(0,lk,j,l))*
     &	  	 	    clr(ifl(1),3,-1)*clr(ifl(2),1,-1)	  
	  m2u(j,l)  = m2zu(j,l)+m2au(j,l)
	      
	  m1zu(j,l) =
     &           (m1wu(j,l)+w2wu(j,l))*clr(ifl(2),3,-1)*clr(ifl(1),2,-1)	  
	  m1au(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is1,2,0),epsawe(0,lk,j,l))*
     &	  	 	    clr(ifl(2),3,-1)*clr(ifl(1),1,-1)	  
	  m1u(j,l) = m1zu(j,l)+m1au(j,l)
...
c vbf_wp_wp_gauge boson from lower quark line:	   
	  ...
	  if (jlog2) then
	  j = 2
      	  m4we(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is3,3,0),epswzu(0,lk,j,l))    
	  m3we(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is3,4,0),epswzu(0,lk,j,l))	    
	  w4we(j,l) = -qepszwu(lk,j,l)*jj43e(0)    
	 
	  m4ze(j,l) = 
     &          (m4we(j,l)-w4we(j,l))*clr(ifl(3),3,-1)*clr(ifl(4),2,-1)    
	  m4ae(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is3,3,0),epswau(0,lk,j,l))*
     &	  	 	    clr(ifl(3),3,-1)*clr(ifl(4),1,-1)	  
	  m4e(j,l)  = m4ze(j,l)+m4ae(j,l)
	      
	  m3ze(j,l) =
     &          (m3we(j,l)+w4we(j,l))*clr(ifl(4),3,-1)*clr(ifl(3),2,-1)  
	  m3ae(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is3,4,0),epswau(0,lk,j,l))*
     &	  	    clr(ifl(4),3,-1)*clr(ifl(3),1,-1)	  
	  m3e(j,l) = m3ze(j,l)+m3ae(j,l)
	  
	 	  
      	  m4wu(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is3,3,0),epswze(0,lk,j,l))    
	  m3wu(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is3,4,0),epswze(0,lk,j,l))	    
	  w4wu(j,l) = -qepszwe(lk,j,l)*jj43u(0)    
	 
	  m4zu(j,l) = 
     &         (m4wu(j,l)-w4wu(j,l))*clr(ifl(3),3,-1)*clr(ifl(4),2,-1)    
	  m4au(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is3,3,0),epswae(0,lk,j,l))*
     &	  	 	    clr(ifl(3),3,-1)*clr(ifl(4),1,-1)	  
	  m4u(j,l)  = m4zu(j,l)+m4au(j,l)
	      
	  m3zu(j,l) = 
     &         (m3wu(j,l)+w4wu(j,l))*clr(ifl(4),3,-1)*clr(ifl(3),2,-1)  
	  m3au(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is3,4,0),epswae(0,lk,j,l))*
     &	  	 	    clr(ifl(4),3,-1)*clr(ifl(3),1,-1)	  
     ...
     &            -(m3e(j,l)+m3u(j,l)+m4e(j,l)+m4u(j,l))*clr(3,3,-1)
	  ...
	  if (jlog3) then
	  j = 3
      	  m4we(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is3,3,l),epswzu(0,lk,j,0))    
	  m3we(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is3,4,l),epswzu(0,lk,j,0))	    
	  w4we(j,l) = -qepszwu(lk,j,0)*jj43e(l)    
	 
	  m4ze(j,l) = 
     &         (m4we(j,l)-w4we(j,l))*clr(ifl(3),3,-1)*clr(ifl(4),2,-1)    
	  m4ae(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is3,3,l),epswau(0,lk,j,0))*
     &	  	 	    clr(ifl(3),3,-1)*clr(ifl(4),1,-1)	  
	  m4e(j,l)  = m4ze(j,l)+m4ae(j,l)
	      
	  m3ze(j,l) = 
     &         (m3we(j,l)+w4we(j,l))*clr(ifl(4),3,-1)*clr(ifl(3),2,-1)  
	  m3ae(j,l) = vbf_wp_wp_dotcc(jwe(0,-1,is3,4,l),epswau(0,lk,j,0))*
     &	  	 	    clr(ifl(4),3,-1)*clr(ifl(3),1,-1)	  
	  m3e(j,l) = m3ze(j,l)+m3ae(j,l)
	  
	   
      	  m4wu(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is3,3,l),epswze(0,lk,j,0))    
	  m3wu(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is3,4,l),epswze(0,lk,j,0))	    
	  w4wu(j,l) = -qepszwe(lk,j,0)*jj43u(l)    
	 
	  m4zu(j,l) =
     &          (m4wu(j,l)-w4wu(j,l))*clr(ifl(3),3,-1)*clr(ifl(4),2,-1)    
	  m4au(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is3,3,l),epswae(0,lk,j,0))*
     &	  	 	    clr(ifl(3),3,-1)*clr(ifl(4),1,-1)	  
	  m4u(j,l)  = m4zu(j,l)+m4au(j,l)
	      
	  m3zu(j,l) = 
     &          (m3wu(j,l)+w4wu(j,l))*clr(ifl(4),3,-1)*clr(ifl(3),2,-1)  
	  m3au(j,l) = vbf_wp_wp_dotcc(jwu(0,-1,is3,4,l),epswae(0,lk,j,0))*
     &	  	 	    clr(ifl(4),3,-1)*clr(ifl(3),1,-1)	  
	  m3u(j,l) = m3zu(j,l)+m3au(j,l)	   
...
c  eps1=W,eps2=j_lower,eps3=W'
      j = 2
      m5ue(j,l) =
     &  vbf_wp_wp_S1C(braketgwu(1,-1,2,l),jqq(0,-1,2,is3,0),.true.,-1,psiwe(1,1))+
     $  vbf_wp_wp_S1C(braketwug(1,-1,2,l),jqq(0,-1,2,is3,0),.true.,-1,psiwe(1,1))+
     $  vbf_wp_wp_S1C(psiwu(1,2),jqq(0,-1,2,is3,0),.true.,-1,braketgwe(1,-1,1,l))+
     $  vbf_wp_wp_S1C(psiwu(1,2),jqq(0,-1,2,is3,0),.true.,-1,braketweg(1,-1,1,l))
         
      m5eu(j,l) = 
     &  vbf_wp_wp_S1C(braketgwe(1,-1,2,l),jqq(0,-1,2,is3,0),.true.,-1,psiwu(1,1))+
     $  vbf_wp_wp_S1C(braketweg(1,-1,2,l),jqq(0,-1,2,is3,0),.true.,-1,psiwu(1,1))+
     $  vbf_wp_wp_S1C(psiwe(1,2),jqq(0,-1,2,is3,0),.true.,-1,braketgwu(1,-1,1,l))+
     $  vbf_wp_wp_S1C(psiwe(1,2),jqq(0,-1,2,is3,0),.true.,-1,braketwug(1,-1,1,l)) 
...
c  eps1=W,eps2=j_upper,eps3=W'
      ...
      m5ue(j,l) = vbf_wp_wp_S1C(psiwu(1,4),jqq(0,-1,1,is1,l),.true.,-1,psiwe(1,3))
      m5eu(j,l) = vbf_wp_wp_S1C(psiwe(1,4),jqq(0,-1,1,is1,l),.true.,-1,psiwu(1,3))
...
c  eps1=v,eps2=j_lower,eps3=v'
      j = 3
      m5ue(j,l) = vbf_wp_wp_S1C(psiwu(1,2),jqq(0,-1,2,is3,l),.true.,-1,psiwe(1,1))
      m5eu(j,l) = vbf_wp_wp_S1C(psiwe(1,2),jqq(0,-1,2,is3,l),.true.,-1,psiwu(1,1))
...
c  eps1=v,eps2=j_upper,eps3=v'
      m5ue(j,l) = 
     &  vbf_wp_wp_S1C(braketgwu(1,-1,4,l),jqq(0,-1,1,is1,0),.true.,-1,psiwe(1,3))+
     $  vbf_wp_wp_S1C(braketwug(1,-1,4,l),jqq(0,-1,1,is1,0),.true.,-1,psiwe(1,3))+
     $  vbf_wp_wp_S1C(psiwu(1,4),jqq(0,-1,1,is1,0),.true.,-1,braketgwe(1,-1,3,l))+
     $  vbf_wp_wp_S1C(psiwu(1,4),jqq(0,-1,1,is1,0),.true.,-1,braketweg(1,-1,3,l))
 
      m5eu(j,l) = 
     &  vbf_wp_wp_S1C(braketgwe(1,-1,4,l),jqq(0,-1,1,is1,0),.true.,-1,psiwu(1,3))+
     $  vbf_wp_wp_S1C(braketweg(1,-1,4,l),jqq(0,-1,1,is1,0),.true.,-1,psiwu(1,3))+
     $  vbf_wp_wp_S1C(psiwe(1,4),jqq(0,-1,1,is1,0),.true.,-1,braketgwu(1,-1,3,l))+
     $  vbf_wp_wp_S1C(psiwe(1,4),jqq(0,-1,1,is1,0),.true.,-1,braketwug(1,-1,3,l))

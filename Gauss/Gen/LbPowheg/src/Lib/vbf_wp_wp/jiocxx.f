...
c ----------------------------------------------------------------------
c
      subroutine vbf_wp_wp_jiocxx(fic,foc,gc,vmass,vwidth , jioc)
c
...
c vbf_wp_wp_gauge for a massless vector and in unitary vbf_wp_wp_gauge for a massive vector.
c                                                                       
c input:                                                                
...
c
      implicit none
      double complex fic(6),foc(6),gc(2),jioc(6),vbf_wp_wp_C0,c1,c2,c3,cs,d
...
c         d = -rOne/dcmplx( q2-vm2, max(vwidth*q2/vmass,rZero) )
         ...
         if ( gc(2).ne.cZero ) then
            vbf_wp_wp_C0=  gc(2)*( foc(3)*fic(1)+foc(4)*fic(2))
     ...
     &          +gc(1)*( foc(1)*fic(3)-foc(2)*fic(4))
            ...
            d = d*gc(1)
            vbf_wp_wp_C0 =  foc(1)*fic(3)+foc(2)*fic(4)

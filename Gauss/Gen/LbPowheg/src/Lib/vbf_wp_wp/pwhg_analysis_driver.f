      subroutine vbf_wp_wp_analysis_driver(dsig0,ikin)
      implicit none
      real * 8 dsig0
      integer ikin
      integer jpart,mu
      ...
      include 'src/Lib/vbf_wp_wp/hepevt.h'
      real * 8 vbf_wp_wp_powheginput
      
      if (vbf_wp_wp_powheginput('#testplots').ne.1d0) return
      if(ikin.eq.0) then
         nhep=nlegborn
         do jpart=1,nhep
            do mu=1,3
...
c     We can then use the flst_born(jpart,1) of the FIRST vbf_wp_wp_born
      do jpart=3,flst_lightpart-1
         idhep(jpart)=flst_born(jpart,1)
      enddo
c     call vbf_wp_wp_analysis routine
      call vbf_wp_wp_analysis(dsig0)
      end

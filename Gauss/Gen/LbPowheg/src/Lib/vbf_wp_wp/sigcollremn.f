      subroutine vbf_wp_wp_btildecoll(xrad,rescoll,www)
      implicit none
      ...
      real * 8 un
      parameter (un=1d0)
c pdfb: vbf_wp_wp_born pdf's, pdfs: pdfs with scaled x->x/z
...
c same, with vbf_wp_wp_soft limit s
...
c get pdfs at underlying vbf_wp_wp_born x values
      call vbf_wp_wp_pdfcall(1,kn_xb1,pdfb1)
      call vbf_wp_wp_pdfcall(2,kn_xb2,pdfb2)
c get pdfs at underlying vbf_wp_wp_born x/z value values
      call vbf_wp_wp_pdfcall(1,kn_xb1/z1,pdfs1)
      call vbf_wp_wp_pdfcall(2,kn_xb2/z2,pdfs2)
c Compute the singlet
      ...
      enddo
c loop over all vbf_wp_wp_born configurations
      tot=0
      do jb=1,flst_nborn
         fl1=flst_born(1,jb)
         fl2=flst_born(2,jb)
...
c gq remnant
     ...
     #    *br_born(jb)*st_alpha/(2*pi)*kn_jacborn
      ...
      if(flg_nlotest) then
         tot=tot*www
         call vbf_wp_wp_analysis_driver(tot,0)
      endif
      end

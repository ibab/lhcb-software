      subroutine vbf_wp_wp_sigvirtual(virt_arr)
      implicit none
      ...
      if(ini) then
         ...
         if(flg_smartsig) then
            ...
            do iborn=1,flst_nborn
               do j=1,nmomset
                  call vbf_wp_wp_setvirtual(pborn(0,1,j),flst_born(1,iborn),
     #                 virtual(j,iborn))
c     check if virtual(j,iborn) is finite
                  if (.not.vbf_wp_wp_pwhg_isfinite(virtual(j,iborn))) 
     #                 virtual(j,iborn)=0d0
               enddo
               call vbf_wp_wp_compare_vecsv(nmomset,iborn,virtual,ibornpr,
     #              cprop,iret)
               if(iret.eq.0) then
      ...
      do iborn=1,flst_nborn
         if(equivto(iborn).lt.0) then
            call vbf_wp_wp_setvirtual(kn_cmpborn,flst_born(1,iborn),
     #           virt_arr(iborn))
c     check if virt_arr(iborn) is finite
                  if (.not.vbf_wp_wp_pwhg_isfinite(virt_arr(iborn))) 
     #                 virt_arr(iborn)=0d0
      ...
      subroutine vbf_wp_wp_compare_vecsv(nmomset,iborn,res,ibornpr,cprop,iret)
      implicit none
      real * 8 ep
      parameter (ep=1d-12)

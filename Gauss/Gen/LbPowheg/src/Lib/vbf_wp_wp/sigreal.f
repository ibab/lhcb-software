      subroutine vbf_wp_wp_btildereal(xrad,resreal,www)
      implicit none
...
c output values for vbf_wp_wp_analysis_driver
         out0=0
         out1=0
c check that emitter is valid
         if(vbf_wp_wp_valid_emitter(kn_emitter)) then
            if(kn_emitter.gt.2) then
c     final state radiation
               call vbf_wp_wp_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call vbf_wp_wp_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call vbf_wp_wp_collfsr(rc)
c     vbf_wp_wp_soft subtraction
                  call vbf_wp_wp_soft(r0s)
                  call vbf_wp_wp_softcollfsr(rcs)
...
c     zero rm (rp).
               call vbf_wp_wp_gen_real_phsp_isr
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call vbf_wp_wp_collisrp(rp)
                     call vbf_wp_wp_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call vbf_wp_wp_collisrm(rm)
                     call vbf_wp_wp_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call vbf_wp_wp_analysis_driver(out0,0)
            if(out1.ne.0d0) call vbf_wp_wp_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine vbf_wp_wp_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_dbg.h'
      external vbf_wp_wp_sigreal_btl,vbf_wp_wp_soft,vbf_wp_wp_collfsr,vbf_wp_wp_softcollfsr, vbf_wp_wp_collisrp,
     $     vbf_wp_wp_softcollisrp,vbf_wp_wp_collisrm,vbf_wp_wp_softcollisrm
      call vbf_wp_wp_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  vbf_wp_wp_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call vbf_wp_wp_checksoft(vbf_wp_wp_sigreal_btl,vbf_wp_wp_soft,' vbf_wp_wp_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call vbf_wp_wp_checkcoll(vbf_wp_wp_sigreal_btl,vbf_wp_wp_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
c            call vbf_wp_wp_randomsave
            if(kn_emitter.ne.2) call vbf_wp_wp_checkcoll(vbf_wp_wp_sigreal_btl,vbf_wp_wp_collisrp,1
     $           ,' coll-plus',iun)
c            call vbf_wp_wp_randomrestore
            if(kn_emitter.ne.1) call vbf_wp_wp_checkcoll(vbf_wp_wp_sigreal_btl,vbf_wp_wp_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  vbf_wp_wp_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call vbf_wp_wp_checksoft(vbf_wp_wp_collfsr,vbf_wp_wp_softcollfsr,' vbf_wp_wp_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  vbf_wp_wp_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call vbf_wp_wp_checksoft(vbf_wp_wp_collisrp,vbf_wp_wp_softcollisrp,
     $           ' vbf_wp_wp_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call vbf_wp_wp_checksoft(vbf_wp_wp_collisrm,vbf_wp_wp_softcollisrm,
     $           ' vbf_wp_wp_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-vbf_wp_wp_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call vbf_wp_wp_checkcoll(vbf_wp_wp_soft,vbf_wp_wp_softcollfsr,1,' coll-vbf_wp_wp_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-vbf_wp_wp_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call vbf_wp_wp_checkcoll(vbf_wp_wp_soft,vbf_wp_wp_softcollisrp,1
     $           ,' coll-plus-vbf_wp_wp_soft',iun)
            if(kn_emitter.ne.1) call vbf_wp_wp_checkcoll(vbf_wp_wp_soft,vbf_wp_wp_softcollisrm,-1
     $           ,' coll-minus-vbf_wp_wp_soft',iun)
         enddo
      endif
      call vbf_wp_wp_randomrestore
      end

      subroutine vbf_wp_wp_checkborn(iun)
c Check if vbf_wp_wp_born, colour correlated vbf_wp_wp_born and spin correlated vbf_wp_wp_born
c are consistent with total vbf_wp_wp_born
      implicit none
      integer iun
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_br.h'
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine vbf_wp_wp_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/vbf_wp_wp/pwhg_dbg.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 vbf_wp_wp_random,vbf_wp_wp_dotp
      external vbf_wp_wp_random,vbf_wp_wp_dotp
      logical vbf_wp_wp_valid_emitter,iszero,isnonzero,isequal
      external vbf_wp_wp_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=vbf_wp_wp_random()
      enddo
      call vbf_wp_wp_gen_born_phsp(xborn)
      call vbf_wp_wp_setscalesbtilde
      call vbf_wp_wp_allborn
      call vbf_wp_wp_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*vbf_wp_wp_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=vbf_wp_wp_random()
      enddo
c Check vbf_wp_wp_soft limits
      if(vbf_wp_wp_valid_emitter(kn_emitter)) then
         write(iun,*) ' vbf_wp_wp_random vbf_wp_wp_born variables ====> ',xborn
          write(iun,*) ' vbf_wp_wp_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call vbf_wp_wp_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call vbf_wp_wp_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check vbf_wp_wp_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine vbf_wp_wp_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/vbf_wp_wp/pwhg_dbg.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' vbf_wp_wp_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
...
c     Added this 'if' to be sure that no division by zero occurs
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine vbf_wp_wp_sigreal_rad(sig)
      implicit none
      real * 8 sig
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      ...
      real * 8 ptsq,vbf_wp_wp_pwhg_pt2
      logical computed(maxalr)
      logical condition
      logical ini
      data ini/.true./
      save ini,equivto,equivcoef
      external vbf_wp_wp_pwhg_pt2
      if(ini) then
         do alr=1,flst_nalr
            equivto(alr)=-1
         enddo
         if(flg_smartsig) then
            call vbf_wp_wp_randomsave
c     generate "nmomset" vbf_wp_wp_random real-phase space configurations
            call vbf_wp_wp_fillmomenta(nlegreal,nmomset,kn_masses,preal)
            do alr=1,flst_nalr
               do j=1,nmomset
                  call vbf_wp_wp_realgr(
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call vbf_wp_wp_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      do alr=1,flst_nalr
         r0(alr)=0
      enddo
      call vbf_wp_wp_pdfcall(1,kn_x1,pdf1)
      call vbf_wp_wp_pdfcall(2,kn_x2,pdf2)
      if(flg_withdamp) then
         call vbf_wp_wp_collrad(rc)
         call vbf_wp_wp_softrad(rs)
      endif
      do alr=1,flst_nalr
         computed(alr)=.false.
      enddo
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call vbf_wp_wp_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply vbf_wp_wp_born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call vbf_wp_wp_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine vbf_wp_wp_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call vbf_wp_wp_sigreal_btl0(r0,0)
      end

c Real cross section, required by vbf_wp_wp_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine vbf_wp_wp_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
...
c     generate "nmomset" vbf_wp_wp_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call vbf_wp_wp_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      ...
      enddo
      if(flg_withdamp) then
         call vbf_wp_wp_collbtl(rc)
         call vbf_wp_wp_softbtl(rs)
      endif
      do alr=1,flst_nalr
...
c First mark as being computed
               markused(alr)=1
               call vbf_wp_wp_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply vbf_wp_wp_born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' vbf_wp_wp_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: vbf_wp_wp_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine vbf_wp_wp_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=vbf_wp_wp_random()
            enddo
         enddo
         preal(3,nlegreal,j)=vbf_wp_wp_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine vbf_wp_wp_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from vbf_wp_wp_btilde,
c imode=1 when called for radiation. In the latter
c case, graphs that do not have the same underlying vbf_wp_wp_born
c are not considered.
 ...
 10      continue
      ...
      subroutine vbf_wp_wp_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call vbf_wp_wp_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'vbf_wp_wp_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine vbf_wp_wp_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      ...
      integer rflav(nlegreal)
      real * 8 amp2 
      logical vbf_wp_wp_pwhg_isfinite
      external vbf_wp_wp_pwhg_isfinite
      call vbf_wp_wp_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.vbf_wp_wp_pwhg_isfinite(amp2)) amp2=0d0

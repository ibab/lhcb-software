      subroutine vbf_wp_wp_btildeborn(res)
      implicit none
...
c         res(j)=kn_jacborn/(2*kn_sborn)/flst_nborn
      ...
      subroutine vbf_wp_wp_sigborn_rad(vbf_wp_wp_born)
      implicit none
      real * 8 vbf_wp_wp_born
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_br.h'
     ...
     #         bmunu(0:3,0:3,nlegborn)
      call vbf_wp_wp_pdfcall(1,kn_xb1,pdf1)
      call vbf_wp_wp_pdfcall(2,kn_xb2,pdf2)
      call vbf_wp_wp_setborn0(kn_cmpborn,flst_born(1,rad_ubornidx),vbf_wp_wp_born,
     #        bornjk,bmunu)
      vbf_wp_wp_born=vbf_wp_wp_born *
     #  pdf1(flst_born(1,rad_ubornidx))*pdf2(flst_born(2,rad_ubornidx))
      end

     

      subroutine vbf_wp_wp_allborn
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      ...
      real * 8 vbf_wp_wp_born(nmomset,maxprocborn)
      ...
      if(ini) then
         ...
         if(flg_smartsig) then
            ...
            call vbf_wp_wp_fillmomenta(nlegborn,nmomset,kn_masses,pborn)
            do iborn=1,flst_nborn
               do j=1,nmomset
                  call vbf_wp_wp_setborn0(pborn(0,1,j),flst_born(1,iborn),
     1                 vbf_wp_wp_born(j,iborn),bornjk(1,1,j,iborn),
     2                 bmunu(0,0,1,j,iborn))
               enddo
               call vbf_wp_wp_compare_vecsb(nmomset,iborn,vbf_wp_wp_born,bornjk,bmunu,
     1              ibornpr,cprop,iret)
      ...
      do iborn=1,flst_nborn
         if(equivto(iborn).lt.0) then
            call vbf_wp_wp_setborn0(kn_cmpborn,flst_born(1,iborn),br_born(iborn),
     ...
     #                   *equivcoef(iborn)
      ...
      subroutine vbf_wp_wp_compare_vecsb(nmomset,iborn,vbf_wp_wp_born,bornjk,bmunu,
     1     ibornpr,cprop,iret)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      ...
      real * 8 vbf_wp_wp_born(nmomset,iborn),cprop,rat,resi,resj
      real * 8 bornjk(nlegborn,nlegborn,nmomset,maxprocborn)
      real * 8 bmunu(0:3,0:3,nlegborn,nmomset,maxprocborn)
      do jborn=1,iborn-1
         rat=vbf_wp_wp_born(1,iborn)/vbf_wp_wp_born(1,jborn)
         do k=1,nmomset
            resi=vbf_wp_wp_born(k,iborn)
            resj=vbf_wp_wp_born(k,jborn)
...
c some of these are zero, be careful
 ...
 10      continue
      ...
      subroutine vbf_wp_wp_setborn0(p,bflav,vbf_wp_wp_born,bornjk,bmunu)
c provide the flux factor to the user vbf_wp_wp_born routine
      implicit none
      ...
      real * 8 p(0:3,nlegs)
      integer bflav(nlegs)
      real * 8 vbf_wp_wp_born,bornjk(nlegs,nlegs),bmunu(0:3,0:3,nlegs)
      integer j,k,mu,nu
      logical vbf_wp_wp_pwhg_isfinite
      external vbf_wp_wp_pwhg_isfinite
      call vbf_wp_wp_setborn(p,bflav,vbf_wp_wp_born,bornjk,bmunu)
c     check if vbf_wp_wp_born, bornjk and bmunu are finite
      if (.not.vbf_wp_wp_pwhg_isfinite(vbf_wp_wp_born)) vbf_wp_wp_born=0d0
      do j=1,nlegs
         do mu=0,3
            do nu=0,3
               if (.not.vbf_wp_wp_pwhg_isfinite(bmunu(mu,nu,j))) 
     $               bmunu(mu,nu,j)=0d0
            enddo
         enddo
      enddo
      do j=1,nlegs
         do k=1,nlegs
            if (.not.vbf_wp_wp_pwhg_isfinite(bornjk(j,k))) bornjk(j,k)=0d0
         enddo
      enddo
c
      vbf_wp_wp_born=vbf_wp_wp_born/(2*kn_sborn)
      do j=1,nlegs
         do mu=0,3
            do nu=0,3

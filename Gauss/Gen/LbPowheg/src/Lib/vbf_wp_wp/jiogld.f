...
c ----------------------------------------------------------------------
c
      subroutine vbf_wp_wp_jiogld(fi,fo,gc,vmass,vwidth,idecay , jio)
c
...
c propagator is given in feynman vbf_wp_wp_gauge for a massless vector and in 
c unitary vbf_wp_wp_gauge for a massive vector. The h.c. of the NLSP decay is
...
c
      implicit none
      double complex  fi(6), fo(6), gc(2), jio(6), vbf_wp_wp_C0, c1, c2, c3, cs
...
c
      if ( idecay.eq.1 ) then
         ...
         write(6,*) 'error in idecay of vbf_wp_wp_jiogld'
         stop
      end if
...
c         d = rOne/dcmplx( q2-vm2, max(vwidth*q2/vmass,rZero) )
     ...
     &            +(fo(3)*p23m + fo(4)*p14m)*fi(2)
            ...
            vbf_wp_wp_C0 = dum*pG(1) - ( fo(3)*fi(1) + fo(4)*fi(2) )*pdotpG
            c1 = dum*pG(2) + ( fo(4)*fi(1) + fo(3)*fi(2) )*pdotpG
            c2 = dum*pG(3) + ( fo(4)*fi(1) - fo(3)*fi(2) )*pdotpG*ci
            c3 = dum*pG(4) + ( fo(3)*fi(1) - fo(4)*fi(2) )*pdotpG
         end if

         cs = (q(0)*vbf_wp_wp_C0 - q(1)*c1 - q(2)*c2 - q(3)*c3) / vm2

         jio(1) = (vbf_wp_wp_C0-cs*q(0))*d
         jio(2) = (c1-cs*q(1))*d
         jio(3) = (c2-cs*q(2))*d
         jio(4) = (c3-cs*q(3))*d

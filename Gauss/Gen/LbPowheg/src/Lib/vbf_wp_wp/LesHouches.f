      subroutine vbf_wp_wp_gen_leshouches
      implicit none
...
c it is a vbf_wp_wp_born event
c first assign flavours and colour to the underlying vbf_wp_wp_born process
         call vbf_wp_wp_momenta_lh(kn_pborn,nlegborn)
         call vbf_wp_wp_born_lh
         scalup=sqrt(rad_ptsqmin)
      else
c it is an event with radiation
         call vbf_wp_wp_momenta_lh(kn_preal,nlegreal)
         call vbf_wp_wp_born_lh
         ...
         if(em.le.2) then
c vbf_wp_wp_setcolour_rad works with all incoming (or all outgoing) flavours and colours;
...
c and conjugate its colour in output, to make it incoming.
            call vbf_wp_wp_setcolour_rad(coluborn,-flem,flrad,
     #                     icolup(1,em),icolup(1,rad))
            call vbf_wp_wp_colour_conj(icolup(1,em))
         else
...
c conjugate their colours in the output, to make them outgoing.
            call vbf_wp_wp_setcolour_rad(coluborn,-flem,-flrad,
     #                     icolup(1,em),icolup(1,rad))
            call vbf_wp_wp_colour_conj(icolup(1,rad))
            call vbf_wp_wp_colour_conj(icolup(1,em))
         endif
...
c add resonances, perform decays, put particles on shell, etc.(or nothing!)
      call vbf_wp_wp_finalize_lh
      end


c     i1<i2
      subroutine vbf_wp_wp_add_resonance(idpdg,i1,i2)
      implicit none
      include 'src/Lib/vbf_wp_wp/LesHouches.h'
     ...
     #     spinuptmp(maxnup)
      ...
      if (i1.ge.i2) then
         write(*,*) 'wrong sequence in vbf_wp_wp_add_resonance'
         stop
      endif
      do i=i1,nup
         iduptmp(i) = idup(i)
...
c     change mothers of decaying particles
      ...
      subroutine vbf_wp_wp_colour_conj(icol)
      implicit none
      integer icol(2)
      integer itmp
      itmp=icol(2)
      ...
      subroutine vbf_wp_wp_momenta_lh(p,n)
      implicit none
      integer n
      real * 8 p(0:3,n)
      integer k,mu
     ...
     #       -p(2,k)**2-p(3,k)**2))
      ...
      include 'src/Lib/vbf_wp_wp/pwhg_rad.h'
      include 'src/Lib/vbf_wp_wp/LesHouches.h'
c      include 'src/Lib/vbf_wp_wp/PhysPars.h'
...
c gluons are numbered 21 in pdg
         ...
         vtimup(k)=0
      enddo
      call vbf_wp_wp_borncolour_lh
      end


      subroutine vbf_wp_wp_getnewcolor(newcolor)
      implicit none
      integer newcolor
      include 'src/Lib/vbf_wp_wp/LesHouches.h'
      integer j,k
...
c the one where both fl2 and fl3 are gluons, where a 50% vbf_wp_wp_random choice is made
c over the two possible colour connections.
      subroutine vbf_wp_wp_setcolour_rad(colin,fl2,fl3,colout2,colout3)
      implicit none
      integer colin(2),fl2,fl3,colout2(2),colout3(2)
      integer newcolor
      real * 8 vbf_wp_wp_random
      external vbf_wp_wp_random
      call vbf_wp_wp_getnewcolor(newcolor)
      if(colin(1).ne.0.and.colin(2).ne.0) then
         if(fl2.eq.0.and.fl3.eq.0) then
            if(vbf_wp_wp_random().gt.0.5d0) then
               colout2(1)=colin(2)
               colout2(2)=newcolor
               colout3(1)=newcolor
               colout3(2)=colin(1)
 ...
 998  continue
      write(*,*)
     #     ' Error in vbf_wp_wp_setcolour_rad: inconsistent colour flavour input'
      ...
      subroutine vbf_wp_wp_displeshouches
      include 'src/Lib/vbf_wp_wp/LesHouches.h'
...
c first conjugate incoming colors
      call vbf_wp_wp_colour_conj(icolup(1,1))
      call vbf_wp_wp_colour_conj(icolup(1,2))
      do j=1,nup
         mark(j)=1
      enddo
      imarked=0
 ...
 22   continue
      ...
      subroutine vbf_wp_wp_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)
      implicit none
      integer id1,id2
...
c vbf_wp_wp_btilde event: pass x1 and x2, id1, id2 etc. of the current underlying vbf_wp_wp_born
      ...
      call vbf_wp_wp_pdfcall(1,x1,pdf)
      xf1=x1*pdf(id1)
      call vbf_wp_wp_pdfcall(2,x2,pdf)
      xf2=x2*pdf(id2)
      if(id1.eq.0) id1=21
      if(id2.eq.0) id2=21
      end

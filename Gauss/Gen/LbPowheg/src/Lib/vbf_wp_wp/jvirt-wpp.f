      subroutine  vbf_wp_wp_J_virtual_tri_box_pent45(psi,p,nq1,nq2, sign, musq,
...
**********************************************************************
  ...
  ! compute vbf_wp_wp_born current for pentagon line:
    
         if (num_ret_current.eq.1) then ! mu,jal,jbe
	
            call vbf_wp_wp_BRA2C(psi(1,-1,nq2),.true.,p(0,nq2),-1,qb,
     #           jbe,psib(1,nq2),fb(0,nq2))
            call vbf_wp_wp_BRA2C(psib(1,nq2),.false.,fb(0,nq2),-1,qa,
     #           jal,psiba(1,nq2),fba(0,nq2))
            call vbf_wp_wp_curr6(-1,psiba(1,nq2),fba(0,nq2),psi(1,-1,nq1),p(0,nq1),
     #           jborn)	
         ...
         elseif (num_ret_current.eq.2) then ! jal,mu,jbe
            ...
            call vbf_wp_wp_BRA2C(psi(1,-1,nq2),.true.,p(0,nq2),-1,qb,
     #           jbe,psib(1,nq2),fb(0,nq2))
            call vbf_wp_wp_KET2C(psi(1,-1,nq1),.true.,p(0,nq1),-1,qa,
     #           jal,psia(1,nq1),fa(0,nq1))
            call vbf_wp_wp_curr6(-1,psib(1,nq2),fb(0,nq2),psia(1,nq1),fa(0,nq1),
     #           jborn)	
         ...
         elseif (num_ret_current.eq.3) then ! jal,jbe,mu
            ...
            call vbf_wp_wp_KET2C(psi(1,-1,nq1),.true.,p(0,nq1),-1,qa,
     #           jal,psia(1,nq1),fa(0,nq1))
            call vbf_wp_wp_KET2C(psia(1,nq1),.false.,fa(0,nq1),-1,qb,
     #           jbe,psiba(1,nq1),fba(0,nq1))
            call vbf_wp_wp_curr6(-1,psi(1,-1,nq2),p(0,nq2),psiba(1,nq1),fba(0,nq1),
     #           jborn)	
      ...
      !  compute pentagon line including vbf_wp_wp_born contribution:
            
         call vbf_wp_wp_J_virtual_tri_box_pent1(psi,p,nq1,nq2, sign, musq,
     1        jal, jbe, num_ret_current, jret_nlo )

	! subtract vbf_wp_wp_born from nlo current:
...
c
	...
	!	kb = 2 ... 	qa -> qak(mu,kb=2) = qa + qb
         ...
         enddo 
         do kb = 1,2	
            qak(4,kb) = qak(0,kb)**2-qak(1,kb)**2-
     #           qak(2,kb)**2-qak(3,kb)**2
            qbk(4,kb) = qbk(0,kb)**2-qbk(1,kb)**2-
     #           qbk(2,kb)**2-qbk(3,kb)**2
         enddo
...
c-------------------------------------------
	...
	! case kb
         ...
         do mu = 0,3
            do kb = 1,2
               cak(mu,kb)  = dcmplx(qa(mu)) 
               !complex momenta for contraction in 'vbf_wp_wp_ket'
...
c		call vbf_wp_wp_KET2C(psi(1,-1,i),.true.,p(0,i),-1,kboson(0:4),
c     #			wpol(0:3),res(1:2),mom-res(0:4))

c		call vbf_wp_wp_curr6(-1,psibar(2,-1:1),pbar(0:3),
...
c first graph (k=3): order = jal, jbe, open index (LO)
         ...
         if (num_ret_current.eq.3) then ! jal,jbe,jexch
            ...
            call vbf_wp_wp_KET2C(psi(1,-1,nq1),.true.,p(0,nq1),-1,qak(0,1),
     #           jal(0),psiak(1,1,nq1),fqak(0,1,nq1))
     
            call vbf_wp_wp_KET2C(psi(1,-1,nq1),.true.,p(0,nq1),-1,qak(0,2),
     #           jal(0),psiak(1,2,nq1),fqak(0,2,nq1))
     
            call vbf_wp_wp_KET2C(psi(1,-1,nq1),.true.,p(0,nq1),-1,qak(0,2),
     #           jbe(0),psibk(1,2,nq1),fqbk(0,2,nq1))
    
            call vbf_wp_wp_KET2C(psi(1,-1,nq1),.true.,p(0,nq1),-1,qbk(0,2),
     #           cbk(0,2),psipbk(1,2,nq1),fpbk(0,2,nq1))     
     
		
            call vbf_wp_wp_curr6(-1,psi(1,-1,nq2),p(0,nq2),
...
c           call vbf_wp_wp_boxlinec(isigmax, psi1(2,-1:1),psi2,k1(0:3),k2(0:3),
c     1                    ld0calc, jext(0:5), pos.of open index ivmu = 1:2, 
c     2                    jborn(0:5,-1:1), jvirt(0:5,-1:1) )
 
           call vbf_wp_wp_boxlinec(-1,psi(1,-1,nq1),psi(1,-1,nq2),p(0,nq1),
     #           p(0,nq2),.true.,jak(0,1),2,j0ak(0,-1,1),jvirt1(0,-1))

           call vbf_wp_wp_boxlinec(-1,psi(1,-1,nq1),psi(1,-1,nq2),p(0,nq1),
     #           p(0,nq2),.true.,jak(0,2),2,j0ak(0,-1,2),jvirt2(0,-1))
     
      	   call vbf_wp_wp_boxlinec(-1,psi(1,-1,nq1),psi(1,-1,nq2),p(0,nq1),
     #           p(0,nq2),.true.,jbk(0,2),2,j0bk(0,-1,2),jvirt3(0,-1))
    
       	   call vbf_wp_wp_boxlinec(-1,psi(1,-1,nq1),psi(1,-1,nq2),p(0,nq1),
     #           p(0,nq2),.true.,cbk(0,2),2,j0bb(0,-1,2),jvirt4(0,-1))
...
c second graph (k=2): order = jal, open index, jbe (LO)
     ...
     #           jal(0),psiak(1,1,nq1),fqak(0,1,nq1))
            ...
            call vbf_wp_wp_BRA2C(psi(1,-1,nq2),.true.,p(0,nq2),-1,qbk(0,1),
     #           jbe(0),psibk(1,1,nq2),fqbk(0,1,nq2))

	! the third contriution gives zero analytically 
        ! -> don't compute numerically:    
	!  call vbf_wp_wp_KET2C(psi(1,-1,nq1),.true.,p(0,nq1),-1,qak(0,1),
c     #			cak(0,1),psipak(1,1,nq1),fpak(0,1,nq1))     
     ...
     1           psiak(1,1,nq1),fqak(0,1,nq1),j0ak(0,-1,1)  )
            ...
            call vbf_wp_wp_curr6(-1,psibk(1,1,nq2),fqbk(0,1,nq2),
     1           psi(1,-1,nq1),p(0,nq1),j0bk(0,-1,1)   )

c            call vbf_wp_wp_curr6(-1,psi(1,-1,nq2),p(0,nq2),
...
c          call vbf_wp_wp_boxlinec(-1,psi(1,-1,nq1),psi(1,-1,nq2),p(0,nq1),
c     #	   	p(0,nq2),.true.,cak(0,1),2,j0aa(0,-1,1),jvirt3(0,-1))   
...
c third graph (k=1): order = open index, jal, jbe (LO)
         ...
         if (num_ret_current.eq.1) then ! jexch,jal,jbe
            ...
            call vbf_wp_wp_BRA2C(psi(1,-1,nq2),.true.,p(0,nq2),-1,qak(0,2),
     #           jal(0),psiak(1,2,nq2),fqak(0,2,nq2))
         
            call vbf_wp_wp_BRA2C(psi(1,-1,nq2),.true.,p(0,nq2),-1,qbk(0,2),
     ...
     #           cak(0,2),psipak(1,2,nq2),fpak(0,2,nq2))     
            ...
            call vbf_wp_wp_curr6(-1,psiak(1,2,nq2),fqak(0,2,nq2),
     1           psi(1,-1,nq1),p(0,nq1),j0ak(0,-1,2)   )
		
            call vbf_wp_wp_curr6(-1,psibk(1,2,nq2),fqbk(0,2,nq2),
     1           psi(1,-1,nq1),p(0,nq1),j0bk(0,-1,2)   )

            call vbf_wp_wp_curr6(-1,psibk(1,1,nq2),fqbk(0,1,nq2),
     1           psi(1,-1,nq1),p(0,nq1),j0bk(0,-1,1)   )

            call vbf_wp_wp_curr6(-1,psipak(1,2,nq2),fpak(0,2,nq2),
...
c ---------------------------------------------------------------------
         ...
         enddo
  		! ... add (-1) here to have same sign
                !     as for 'genuine' pentagon 

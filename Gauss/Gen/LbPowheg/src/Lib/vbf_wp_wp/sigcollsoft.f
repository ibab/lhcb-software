c subroutines vbf_wp_wp_soft, vbf_wp_wp_collfsr, vbf_wp_wp_softcollfsr, vbf_wp_wp_collisrp, vbf_wp_wp_collisrm,
c             vbf_wp_wp_softcollisrp, vbf_wp_wp_softcollisrm 
c are called by vbf_wp_wp_btildereal; they return (1-y) csi^2 R_alr (if kn_emitter
c is >2, i.e. final state) or (1-y^2) csi^2 R (if kn_emitter <=2) in the
c collinear final state                      (vbf_wp_wp_collfsr)
c vbf_wp_wp_soft-collinear final state                 (vbf_wp_wp_softcollfsr)
c collinear initial state in the + direction (vbf_wp_wp_collisrp)
c collinear initial state in the - direction (vbf_wp_wp_collisrm)
c vbf_wp_wp_soft-collinear initial state +             (vbf_wp_wp_softcollisrp)
c vbf_wp_wp_soft-collinear initial state -             (vbf_wp_wp_softcollisrm)
c The vbf_wp_wp_soft limit is taken keeping y (i.e. kn_y) fixed
c The collinear limit is taken keeping csitilde (kn_csitilde) fixed
c
c The vbf_wp_wp_born and real phase space must have been set before calling them.
...
c
c
c Functions: vbf_wp_wp_collbtl and vbf_wp_wp_softbtl are required to compute the damping
c            factor in vbf_wp_wp_bornzerodamp. They are called by sigreal, and
c            sigreal is used by vbf_wp_wp_btilde (hence the btl suffix)

c vbf_wp_wp_softbtl: computes the same quantity as vbf_wp_wp_soft, but does not include 
c          the luminosity
c
c vbf_wp_wp_collbtl: if kn_emitter is in the final state, computes the same quantity as
c          vbf_wp_wp_collfsr, but does not include the luminosity
c          if kn_emitter is 1 or 2  computes the same quantity as
c          vbf_wp_wp_collisrp or vbf_wp_wp_collisrm, without luminosity
c          if kn_emitter is 0 computes the average of the return values
c          of vbf_wp_wp_collisrp and vbf_wp_wp_collisrm, weighted with the factors 
c          (1+y)/2 and (1-y)/2.
c
c 
c
c Functions: vbf_wp_wp_collrad, vbf_wp_wp_softrad  are required to compute the damping
c            factor in vbf_wp_wp_bornzerodamp. They are called by vbf_wp_wp_sigreal_rad,
c            i.e. they are used in the generation of radiation.
c            They compute the same quantities as vbf_wp_wp_collbtl and vbf_wp_wp_softbtl,
c            except that they only fill alr values corresponding to
c            the current radiation underlying vbf_wp_wp_born (i.e. the alr list
...
c            kinematic regions).
      ...
      subroutine vbf_wp_wp_collfsr(rc)
      implicit none
      ...
      integer alr
      call vbf_wp_wp_collfsrnopdf(rc)
      ...
      subroutine vbf_wp_wp_collfsrnopdf(rc)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
...
c for fsr csi is y independent
      ...
      phi=kn_azi
      x=csi*xocsi
      call vbf_wp_wp_buildkperp(em,phi,kperp,kperp2)
      do alr=1,flst_nalr
         if(em.eq.flst_emitter(alr)) then
            call vbf_wp_wp_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
      ...
      subroutine vbf_wp_wp_buildkperp(em,phi,kperp,kperp2)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
...
c     Then rotate it counterclockwise around the em direction
     ...
     #/kn_cmpborn(3,em)
      ...
      call vbf_wp_wp_mrotate(dir,sin(phi),cos(phi),kperp(1))
      kperp(0)=0
      end
      

      subroutine vbf_wp_wp_softcollfsr(rc)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      ...
      call vbf_wp_wp_collfsrnopdf(rc)
      kn_csi=tmp
      call vbf_wp_wp_pdfcall(1,kn_xb1,pdf1)
      call vbf_wp_wp_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end
      ...
      subroutine vbf_wp_wp_collisrp(rc)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      real * 8 rc(maxalr)
      call vbf_wp_wp_collisr(1,rc)
      end

      subroutine vbf_wp_wp_collisrm(rc)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      real * 8 rc(maxalr)
      call vbf_wp_wp_collisr(2,rc)
      end

      subroutine vbf_wp_wp_softcollisrp(rc)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      ...
      tmp=kn_csip
      kn_csip=0
      call vbf_wp_wp_collisrnopdf(1,rc)
      kn_csip=tmp
      call vbf_wp_wp_pdfcall(1,kn_xb1,pdf1)
      call vbf_wp_wp_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
      ...
      subroutine vbf_wp_wp_softcollisrm(rc)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      ...
      tmp=kn_csim
      kn_csim=0
      call vbf_wp_wp_collisrnopdf(2,rc)
      kn_csim=tmp
      call vbf_wp_wp_pdfcall(1,kn_xb1,pdf1)
      call vbf_wp_wp_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
      ...
      subroutine vbf_wp_wp_collisr(i,rc)
      implicit none
      integer i
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      ...
      integer alr
      call vbf_wp_wp_collisrnopdf(i,rc)
      if(i.eq.1) then
         x1=kn_xb1/(1-kn_csip)
         x2=kn_xb2
      elseif(i.eq.2) then
      ...
      call vbf_wp_wp_pdfcall(1,x1,pdf1)
      call vbf_wp_wp_pdfcall(2,x2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end

      subroutine vbf_wp_wp_collisrnopdf(i,rc)
      implicit none
      integer i
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
...
c Construct kperp
      ...
      do alr=1,flst_nalr
         if(flst_emitter(alr).eq.kn_emitter) then
            call vbf_wp_wp_collisralr(alr,i,csi,kperp,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
      ...
      subroutine vbf_wp_wp_soft(r0)
c blegs:        integer, number of legs of vbf_wp_wp_born
...
c softvec(0:3): real * 8, 4-vector of vbf_wp_wp_soft gluon normalized to softvec(0)=1
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      ...
      real * 8 y
      real * 8 pdf1(-6:6),pdf2(-6:6)
c vbf_wp_wp_boost vbf_wp_wp_born momenta to their rest frame
c find vbf_wp_wp_boost velocity
      em=kn_emitter
      y=kn_y
      do alr=1,flst_nalr
         call vbf_wp_wp_softalr(alr,em,y,r0(alr))
      enddo
      call vbf_wp_wp_pdfcall(1,kn_xb1,pdf1)
      call vbf_wp_wp_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
      ...
      subroutine vbf_wp_wp_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,res)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
...
c     above; the other here.
     ...
     #*(4*pi*st_alpha)
c provide multiplicity of emitter in underlyng vbf_wp_wp_born
      res=res*flst_ubmult(alr)
      end


      subroutine vbf_wp_wp_collisralr(alr,i,csi,kperp,res)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
...
c     the last expression being vbf_wp_wp_boost invariant.
...
c     The remaining csi=1-x factor has been applied earlier
      ...
      subroutine vbf_wp_wp_softalr(alr,em,y,res)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
               ...
               if(abs(flst_born(j,iub)).le.6) then
                  do k=j+1,nlegborn
                     if(abs(flst_born(k,iub)).le.6) then
                        result=result+
     #vbf_wp_wp_dotp(kn_cmpborn(0,j),kn_cmpborn(0,k))
     #/(vbf_wp_wp_dotp(kn_cmpborn(0,j),kn_softvec)*
     #vbf_wp_wp_dotp(kn_cmpborn(0,k),kn_softvec)) * br_bornjk(j,k,iub)
                     endif
                  enddo
               endif
            enddo
c     the previous sum should run over all vbf_wp_wp_indexes. Since br_bornjk is
c     symmetric, multiply by 2
     ...
     #kn_cmpborn(2,j)**2-kn_cmpborn(3,j)**2
                  result=result-pjsq/vbf_wp_wp_dotp(kn_cmpborn(0,j),
     #kn_softvec)**2*br_born(iub)*cf
               endif
            enddo
c     having chosen a vbf_wp_wp_soft four momentum of energy 1, supply
...
c     Multiply vbf_wp_wp_soft result by (vbf_wp_wp_soft limit Sij appropriate factors)
      if(res.ne.0) then
         sumdijinv=0
     ...
     #+1/kn_dijterm_soft(flst_allreg(1,j,alr))
      ...
      subroutine vbf_wp_wp_softbtl(r0)
c blegs:        integer, number of legs of vbf_wp_wp_born
c bflav(nlegs): integer, flavours of the incoming partons, according to PDG conventions,
c               MODIFIED TO HAVE 0 FOR GLUONS (instead of 21)
c p(0:3,nleg):  real * 8, momenta, 0 is time component
...
c cross section used in vbf_wp_wp_btilde
      subroutine vbf_wp_wp_collbtl(rc)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
         ...
         do alr=1,flst_nalr
            if(flst_emitter(alr).eq.em) then
               if(em.ne.2) then
                  ...
                  call vbf_wp_wp_collisralr(alr,i,csi,kperp,r1)
               endif
               if(em.ne.1) then
                  i=2
                  csi=kn_csi*kn_csimaxm/kn_csimax
                  call vbf_wp_wp_collisralr(alr,i,csi,kperp,r2)
               endif
               if(em.eq.0) then
...
c in the real radiation cross section
      subroutine vbf_wp_wp_collrad(rc)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
...
c     Construct kperp
            ...
            if(em.ne.2) then
               csi=kn_csi*kn_csimaxp/kn_csimax
               call vbf_wp_wp_collisralr(alr,1,csi,kperp,r1)
            endif
            if(em.ne.1) then
               csi=kn_csi*kn_csimaxm/kn_csimax
               call vbf_wp_wp_collisralr(alr,2,csi,kperp,r2)
            endif
            if(em.eq.0) then
               rc(alr)=(r1*(1+kn_y)+r2*(1-kn_y))/2
            elseif(em.eq.1) then
      ...
      subroutine vbf_wp_wp_softrad(r0)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'

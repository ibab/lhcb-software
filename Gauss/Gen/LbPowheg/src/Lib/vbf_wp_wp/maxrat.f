      subroutine vbf_wp_wp_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 vbf_wp_wp_powheginput,vbf_wp_wp_random
      external vbf_wp_wp_powheginput,vbf_wp_wp_random      
      call vbf_wp_wp_zerohistnorms
      loaded=.false.
      nubound=vbf_wp_wp_powheginput('nubound')
      rad_nynorms=vbf_wp_wp_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=vbf_wp_wp_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=vbf_wp_wp_powheginput('#xupbound')
      ...
      if(vbf_wp_wp_powheginput('use-old-ubound').eq.1) then
         call vbf_wp_wp_newunit(iun)
...
c Compute and store normalization grid
 111  continue
     ...
     # ' bounding function for radiation'
      ...
      enddo
      do j=1,nubound
         call vbf_wp_wp_gen_born(mcalls,icalls)
         call vbf_wp_wp_gen_uborn_idx
         kn_csitilde=vbf_wp_wp_random()
         kn_y=2*vbf_wp_wp_random()-1
...
c     final state radiation
     ...
     1        //rnd_cwhichseed//'.dat',status='unknown')
      ...
      do j=1,flst_nborn
         write(iun,fmt=*,iostat=ios) j,'   vbf_wp_wp_born index'
         do iy=1,rad_nynorms
...
c      call maxratident
      ...
      subroutine vbf_wp_wp_inc_norms
      implicit none
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_rad.h'
      real * 8 ptsq,vbf_wp_wp_born,sig,xnorm
      real * 8 histnorms(nlegborn-1,maxprocborn,100)
      real * 8 efficiencies(nlegborn-1,maxprocborn)
      common/vbf_wp_wp_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical vbf_wp_wp_refuse_pdf
      real * 8 vbf_wp_wp_pwhg_pt2,vbf_wp_wp_pwhg_upperb_rad
      external vbf_wp_wp_pwhg_pt2,vbf_wp_wp_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         ...
         if(.not.vbf_wp_wp_refuse_pdf()) then
            ...
            sig=sig*kn_jacreal
            if(vbf_wp_wp_born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' vbf_wp_wp_inc_norms: Warning, vbf_wp_wp_born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' vbf_wp_wp_inc_norms: x1,x2', kn_xb1,kn_xb2
               write(*,*) ' muf= ',sqrt(st_mufact2)
               write(*,*) ' Flavour: ', (flst_born(ind,rad_ubornidx),ind
     $              =1,5) 
               
               write(*,*) ' vbf_wp_wp_born w/o pdf: ',br_born(rad_ubornidx)
     $              ,'  jac: ',kn_jacborn 
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' vbf_wp_wp_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' vbf_wp_wp_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/vbf_wp_wp_born/vbf_wp_wp_pwhg_upperb_rad()
            if (debug) then
...
c
     ...
     #    histnorms(rad_kinreg,rad_ubornidx,ind)+1
      ...
      subroutine vbf_wp_wp_printevent
      implicit none
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
     ...
     # (flst_born(j,rad_ubornidx),j=1,nlegborn)
      ...
      enddo
      write(*,*) ' pt', sqrt(vbf_wp_wp_pwhg_pt2())
      write(*,*) ' x1,x2 = ', kn_x1, kn_x2
      write(*,*) ' underlying vbf_wp_wp_born x1,x2', kn_xb1, kn_xb2
      write(*,*) ' y, csi:', kn_y,kn_csi
c      write(*,*) ' vbf_wp_wp_born pt', sqrt(kn_born_pt2)
c      write(*,*) ' cosine decay angle', kn_cthdec
      write(*,*) ' vbf_wp_wp_born cm kinematics'
      do k=1,nlegborn
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
      ...
      subroutine vbf_wp_wp_printhistnorms
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_rad.h'
      ...
      do ireg=1,rad_nkinreg
         do iborn=1,flst_nborn
            ...
            write(iun,*)'title top "region=',ireg,', vbf_wp_wp_born=',iborn,'"'
            do j=1,100
               x1=exp(j-50d0)
               x2=exp(j+1-50d0)
               xm=(x1+x2)/2
     ...
     1              efficiencies(ireg,iborn)/vbf_wp_wp_powheginput('nubound')/
...
c            write(iun,*)'( efficiencies: region=',ireg,', vbf_wp_wp_born=',iborn 
c            write(iun,*) '(', efficiencies(ireg,iborn)/
c     1           vbf_wp_wp_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine vbf_wp_wp_zerohistnorms
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_rad.h'
...
c maxrat
      ...
      integer k,j,fl
      suppfc(x)=(1-x)*x
c in FKS, for final state radiation, pdf always cancel in real/vbf_wp_wp_born
      if(rad_kinreg.ne.1) then
         vbf_wp_wp_refuse_pdf=.false.
         return
      endif
      if(st_mufact2.lt.2.or.kn_x1.gt.0.9.or.kn_x2.gt.0.9) then
         vbf_wp_wp_refuse_pdf=.true.
         return
      endif
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call vbf_wp_wp_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call vbf_wp_wp_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine vbf_wp_wp_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/vbf_wp_wp/pwhg_math.h'
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
...
c
      ...
      subroutine vbf_wp_wp_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 vbf_wp_wp_btilde,vbf_wp_wp_powheginput
      external vbf_wp_wp_btilde,vbf_wp_wp_powheginput
...
c
     ...
     1       ' vbf_wp_wp_born configurations according to the vbf_wp_wp_born cross section'
c set up the grids
         do j=1,ndimbornint
            do k=0,50
               xgrid(k,j)=0
...
c will not be used;
         ...
         call vbf_wp_wp_mint(vbf_wp_wp_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=vbf_wp_wp_powheginput('ncall2')
         itmx2=vbf_wp_wp_powheginput('itmx2')
         call vbf_wp_wp_mint(vbf_wp_wp_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call vbf_wp_wp_gen(vbf_wp_wp_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn't used the ifold array may need reset to 1 here.
      call vbf_wp_wp_gen(vbf_wp_wp_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

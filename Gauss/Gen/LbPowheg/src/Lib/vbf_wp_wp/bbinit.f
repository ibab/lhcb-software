     ...
     #                  ,xmmm(0:50,ndiminteg),xmmmrm(0:50,ndiminteg)
      ...
      common/vbf_wp_wp_cgengrids/xgrid,ymax,xmmm,xgridrm,ymaxrm,xmmmrm,
     #                ifold,ifoldrm
      ...
      external vbf_wp_wp_btilde,vbf_wp_wp_sigremnant,vbf_wp_wp_powheginput
      do j=1,ndiminteg
         do k=0,50
            xgrid(k,j)=0
            xgridrm(k,j)=0
         ...
         ifoldrm(j)=1
      enddo
      call vbf_wp_wp_newunit(iunstat)
     ...
     1        rnd_cwhichseed//'.dat',status='unknown')
      ...
      ncall1=vbf_wp_wp_powheginput('ncall1')
      ncall2=vbf_wp_wp_powheginput('ncall2')
      itmx1=vbf_wp_wp_powheginput('itmx1')
      itmx2=vbf_wp_wp_powheginput('itmx2')
      iret1=0
      iret2=0
      call vbf_wp_wp_loadgrids(iret1,xgrid,ymax,xgridrm,ymaxrm,
     2     ifold,ifoldrm)
      if(iret1.ne.0) call vbf_wp_wp_loadxgrid(iret2,xgrid,xint,xgridrm,xintrm)
      if(iret2.ne.0) then
         write(*,*)
     ...
     #        status='unknown')
         ...
         flg_nlotest=.false.
         imode=0
         call vbf_wp_wp_mint(vbf_wp_wp_btilde,ndiminteg,ncall1,itmx1,ifold,imode,iun,
     ...
     #        status='unknown')
            imode=0
            call vbf_wp_wp_mint(vbf_wp_wp_sigremnant,ndiminteg,ncall1,itmx1,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
            close(iun)
         endif
         call vbf_wp_wp_storexgrid(xgrid,xint,xgridrm,xintrm)
...
c importance sampling grids have been initialized with default vbf_wp_wp_SEED;
c they must all be the same. Now set current vbf_wp_wp_SEED, if required
      if(rnd_cwhichseed.ne.'none')
     1     call vbf_wp_wp_setrandom(rnd_initialseed,rnd_i1,rnd_i2)
      if(iret1.ne.0) then
         if (vbf_wp_wp_powheginput('#testplots').eq.1d0) call vbf_wp_wp_init_hist
c set  up the folding here, if required
         ifold(ndiminteg-2) = vbf_wp_wp_powheginput("foldcsi")
         ifold(ndiminteg-1) = vbf_wp_wp_powheginput("foldy")
         ifold(ndiminteg)   = vbf_wp_wp_powheginput("foldphi")
...
c Results in rad_tot???btl do not depend upon flg_withnegweights
         call vbf_wp_wp_resettotals
         call vbf_wp_wp_mint(vbf_wp_wp_btilde,ndiminteg,ncall2,itmx2,ifold,imode,iun,
     #        xgrid,xint,ymax,sigbtl,errbtl)
         call vbf_wp_wp_finaltotals
c finalize vbf_wp_wp_btilde output in histograms
         call vbf_wp_wp_pwhgaddout
         flg_nlotest=.false.
         write(*,*) 'vbf_wp_wp_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(*,*) 'vbf_wp_wp_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(*,*) 'vbf_wp_wp_btilde total (pos.-|neg.|):', rad_totbtl,' +-',
     1        rad_etotbtl
         write(iunstat,*) 'vbf_wp_wp_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(iunstat,*) 'vbf_wp_wp_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(iunstat,*) 'vbf_wp_wp_btilde Total (pos.-|neg.|):', rad_totbtl,
     1        ' +-',rad_etotbtl
c Now compute the remnant contributions
     ...
     #                ' and/or regular remnants'
            flg_nlotest=.true.
            imode=1
            call vbf_wp_wp_mint(vbf_wp_wp_sigremnant,ndiminteg,ncall2,itmx2,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
...
c vbf_wp_wp_btilde and remnant event are chosen in proportion to
...
c        
         call vbf_wp_wp_storegrids(xgrid,ymax,xgridrm,ymaxrm,ifold,ifoldrm,
     1        ncall2,itmx2)
c Output NLO histograms
         if (vbf_wp_wp_powheginput('#testplots').eq.1d0) then
            if(rnd_cwhichseed.eq.'none') then
     ...
     1           rad_totrm,'+-',rad_etotrm
         ...
         if (flg_weightedev) then
            write(iunstat,*) 
     1 ' total (vbf_wp_wp_btilde+remnants) cross section times '
            write(iunstat,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
         else
            write(iunstat,*) 
     1 ' total (vbf_wp_wp_btilde+remnants) cross section in pb',
     2     rad_tot,'+-',rad_etot
         endif
     ...
     1        rad_totrm,'+-',rad_etotrm
      ...
      if (flg_weightedev) then
         write(*,*) ' total (vbf_wp_wp_btilde+remnants) cross section times '
         write(*,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
      else
         write(*,*) ' total (vbf_wp_wp_btilde+remnants) cross section in pb',
     1     rad_tot,'+-',rad_etot
      endif
     ...
     1     rad_totnegbtl/(2*rad_totnegbtl+rad_tot)
      close(iunstat)
      call flush
c initialize vbf_wp_wp_gen; the array xmmm is set up at this stage.
      call vbf_wp_wp_gen(vbf_wp_wp_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,0,
     #    mcalls,icalls,xx)
c compute normalization of upper bounding function for radiation
      call vbf_wp_wp_do_maxrat(mcalls,icalls)
c print statistics
      call vbf_wp_wp_gen(vbf_wp_wp_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,3,
     #    mcalls,icalls,xx)
      if(xx(1).gt.0) write(*,*) 'POWHEG: efficiency in the generation'
     # //' of the vbf_wp_wp_born variables =',xx(1)
      if((flg_withreg.or.flg_withdamp).and..not.flg_bornonly) then
c initialize vbf_wp_wp_gen for remnants
         call vbf_wp_wp_gen(vbf_wp_wp_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            0,mcalls,icalls,xx)
c     save vbf_wp_wp_random number seeds
         call vbf_wp_wp_randomsave
c generate few events from remnants, just to determine the generation efficiency
         do j=1,int(min(vbf_wp_wp_powheginput('nubound'),10d0))
            call vbf_wp_wp_gen(vbf_wp_wp_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            1,mcalls,icalls,xx)
         enddo
c     restore  vbf_wp_wp_random number seeds
...
c     print statistics
     ...
     #                   //' of the remnant variables =',xx(1)
      ...
      subroutine vbf_wp_wp_gen_btilde(mcalls,icalls)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)      
      real * 8 vbf_wp_wp_btilde
      external vbf_wp_wp_btilde
      call vbf_wp_wp_gen(vbf_wp_wp_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)
      end

      subroutine vbf_wp_wp_gen_sigremnant
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      real * 8 xgrid(0:50,ndiminteg),ymax(50,ndiminteg)
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)
      integer mcalls,icalls
      real * 8 vbf_wp_wp_sigremnant
      external vbf_wp_wp_sigremnant
      call vbf_wp_wp_gen(vbf_wp_wp_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,1,
     #    mcalls,icalls,xx)
      end


      subroutine vbf_wp_wp_storegrids(xgrid,ymax,xgridrm,ymaxrm,
     ...
     8     rad_tot,rad_etot
      ...
      subroutine vbf_wp_wp_loadgrids(iret,xgrid,ymax,xgridrm,ymaxrm,
     #           ifold,ifoldrm)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
...
c
      ...
      if(vbf_wp_wp_powheginput('use-old-grid').eq.0) then
         iret=1
         return
      endif
      iret=0
...
c vbf_wp_wp_random seeds
     ...
     1              chseed//'.dat'
         ...
         if(vbf_wp_wp_powheginput("#ncallfrominput").eq.1) then
            ncall2=vbf_wp_wp_powheginput("ncall2")
            itmx2=vbf_wp_wp_powheginput("itmx2")
            ncall2=ncall2*itmx2
         endif
 ...
 998  continue
      ...
      subroutine vbf_wp_wp_storexgrid(xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'
     ...
     #     form='unformatted',status='unknown')
      ...
      subroutine vbf_wp_wp_loadxgrid(iret,xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/vbf_wp_wp/nlegborn.h'
      include 'src/Lib/vbf_wp_wp/pwhg_flst.h'
      include 'src/Lib/vbf_wp_wp/pwhg_kn.h'

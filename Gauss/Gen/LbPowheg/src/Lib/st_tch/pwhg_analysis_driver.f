      subroutine st_tch_analysis_driver(dsig0,ikin)
      implicit none
      real * 8 dsig0
      integer ikin
      integer jpart,mu
      ...
      include 'src/Lib/st_tch/hepevt.h'
      real * 8 st_tch_powheginput
      
      if (st_tch_powheginput('#testplots').ne.1d0) return
      if(ikin.eq.0) then
         nhep=nlegborn
         do jpart=1,nhep
            do mu=1,3
...
c     We can then use the flst_born(jpart,1) of the FIRST Born
      ...
      enddo
c     call st_tch_analysis routine
      call st_tch_analysis(dsig0)
      end

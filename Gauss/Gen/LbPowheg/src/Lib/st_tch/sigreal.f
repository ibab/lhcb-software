      subroutine st_tch_btildereal(xrad,resreal,www)
      implicit none
      ...
      include 'src/Lib/st_tch/pwhg_par.h'
...
c output st_tch_values for st_tch_analysis_driver
         out0=0
         out1=0
...
c     final state radiation
               call st_tch_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call st_tch_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call st_tch_collfsr(rc)
c     st_tch_soft subtraction
                  call st_tch_soft(r0s)
                  call st_tch_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call st_tch_collisrp(rp)
                     call st_tch_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call st_tch_collisrm(rm)
                     call st_tch_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call st_tch_analysis_driver(out0,0)
            if(out1.ne.0d0) call st_tch_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine st_tch_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      include 'src/Lib/st_tch/pwhg_kn.h'
      include 'src/Lib/st_tch/pwhg_dbg.h'
      external st_tch_sigreal_btl,st_tch_soft,st_tch_collfsr,st_tch_softcollfsr, st_tch_collisrp,
     $     st_tch_softcollisrp,st_tch_collisrm,st_tch_softcollisrm
      call st_tch_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  st_tch_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call st_tch_checksoft(st_tch_sigreal_btl,st_tch_soft,' st_tch_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call st_tch_checkcoll(st_tch_sigreal_btl,st_tch_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
c            call st_tch_randomsave
            if(kn_emitter.ne.2) call st_tch_checkcoll(st_tch_sigreal_btl,st_tch_collisrp,1
     $           ,' coll-plus',iun)
c            call st_tch_randomrestore
            if(kn_emitter.ne.1) call st_tch_checkcoll(st_tch_sigreal_btl,st_tch_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  st_tch_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call st_tch_checksoft(st_tch_collfsr,st_tch_softcollfsr,' st_tch_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  st_tch_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call st_tch_checksoft(st_tch_collisrp,st_tch_softcollisrp,
     $           ' st_tch_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call st_tch_checksoft(st_tch_collisrm,st_tch_softcollisrm,
     $           ' st_tch_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-st_tch_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call st_tch_checkcoll(st_tch_soft,st_tch_softcollfsr,1,' coll-st_tch_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-st_tch_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call st_tch_checkcoll(st_tch_soft,st_tch_softcollisrp,1
     $           ,' coll-plus-st_tch_soft',iun)
            if(kn_emitter.ne.1) call st_tch_checkcoll(st_tch_soft,st_tch_softcollisrm,-1
     $           ,' coll-minus-st_tch_soft',iun)
         enddo
      endif
      call st_tch_randomrestore
      end

      subroutine st_tch_checkborn(iun)
...
c are consistent with total Born
      ...
      include 'src/Lib/st_tch/pwhg_flst.h'
      include 'src/Lib/st_tch/pwhg_br.h'
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine st_tch_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/st_tch/pwhg_dbg.h'
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 st_tch_random,st_tch_dotp
      external st_tch_random,st_tch_dotp
      logical st_tch_valid_emitter,iszero,isnonzero,isequal
      external st_tch_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=st_tch_random()
      enddo
      call st_tch_gen_born_phsp(xborn)
      call st_tch_setscalesbtilde
      call st_tch_allborn
      call st_tch_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*st_tch_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=st_tch_random()
      enddo
c Check st_tch_soft limits
      if(st_tch_valid_emitter(kn_emitter)) then
         write(iun,*) ' st_tch_random Born variables ====> ',xborn
          write(iun,*) ' st_tch_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call st_tch_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call st_tch_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check st_tch_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine st_tch_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/st_tch/pwhg_dbg.h'
      include 'src/Lib/st_tch/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' st_tch_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
...
c     Added this 'if' to be sure that no division by zero occurs
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine st_tch_sigreal_rad(sig)
      implicit none
      real * 8 sig
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      ...
      real * 8 ptsq,st_tch_pwhg_pt2
      logical computed(maxalr)
      logical condition
      logical ini
      data ini/.true./
      save ini,equivto,equivcoef
      external st_tch_pwhg_pt2
      if(ini) then
         do alr=1,flst_nalr
            equivto(alr)=-1
         enddo
...
c     generate "nmomset" st_tch_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call st_tch_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      ...
      if(flg_withdamp) then
         call st_tch_collrad(rc)
         call st_tch_softrad(rs)
      endif
      do alr=1,flst_nalr
         computed(alr)=.false.
      enddo
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call st_tch_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call st_tch_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine st_tch_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call st_tch_sigreal_btl0(r0,0)
      end

c Real cross section, required by st_tch_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine st_tch_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      include 'src/Lib/st_tch/pwhg_kn.h'
...
c     generate "nmomset" st_tch_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call st_tch_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      ...
      enddo
      if(flg_withdamp) then
         call st_tch_collbtl(rc)
         call st_tch_softbtl(rs)
      endif
      do alr=1,flst_nalr
...
c First mark as being computed
               markused(alr)=1
               call st_tch_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' st_tch_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: st_tch_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine st_tch_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=st_tch_random()
            enddo
         enddo
         preal(3,nlegreal,j)=st_tch_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine st_tch_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from st_tch_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine st_tch_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call st_tch_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'st_tch_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine st_tch_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      include 'src/Lib/st_tch/pwhg_math.h'
      ...
      integer rflav(nlegreal)
      real * 8 amp2 
      logical st_tch_pwhg_isfinite
      external st_tch_pwhg_isfinite
      call st_tch_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.st_tch_pwhg_isfinite(amp2)) amp2=0d0

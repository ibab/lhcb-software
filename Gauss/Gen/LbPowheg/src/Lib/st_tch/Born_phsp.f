      subroutine st_tch_born_phsp(xborn)
      implicit none
...
c     born phase space: variables
      if(numgamma(1).gt.0d0) then
         print*, 'Error in st_tch_born_phsp: top Breit-Wigner not implemented'
         call exit(1)
      else
         m2top=topmass_pow**2
      endif
...
c now st_tch_boost everything along 3
      beta=(kn_xb1-kn_xb2)/(kn_xb1+kn_xb2)
      vec(1)=0
      vec(2)=0
      vec(3)=1
      call st_tch_mboost(nlegborn-2,vec,beta,kn_cmpborn(0,3),kn_pborn(0,3))
      do mu=0,3
         kn_pborn(mu,1)=kn_xb1*kn_beams(mu,1)
         kn_pborn(mu,2)=kn_xb2*kn_beams(mu,2)
      enddo
      call st_tch_checkmomzero(nlegborn,kn_pborn)
c      call st_tch_checkmass(2,kn_pborn(0,3))

      kn_minmass=topmass_pow

      end

      subroutine st_tch_born_suppression(fact)
      implicit none
      logical ini
      data ini/.true./
      real * 8 fact
      ...
      subroutine st_tch_set_fac_ren_scales(muf,mur)
      implicit none
      include 'src/Lib/st_tch/PhysPars.h'
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_kn.h'

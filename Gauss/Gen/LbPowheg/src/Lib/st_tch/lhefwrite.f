...
c...writes initialization information to a les houches events file on unit nlf. 
      subroutine st_tch_lhefwritehdr(nlf)
      implicit none
      ...
      integer nlf
      real * 8 version
      common/st_tch_cversion/version
      data version/1.0/
     ...
     #     version
      ...
      call st_tch_wrtpowheginput(nlf)
      write(nlf,'(a)') 'End of powheg.input content'
      call st_tch_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) 'st_tch_random number generator initialized with: ',
     # iran,' ',n1ran,' ',n2ran
      write(nlf,'(a)') '-->'
      write(nlf,'(a)') '<init>'
 ...
 120  format(1p,3(1x,e12.5),1x,i6)
      ...
      subroutine st_tch_printleshouches
c useful for debugging
      call st_tch_lhefwritev(6)
      end

c...lhefeader(nlf)
c...writes event information to a les houches events file on unit nlf. 
      subroutine st_tch_lhefwritev(nlf)
      implicit none
      integer nlf
      include 'src/Lib/st_tch/LesHouches.h'
      include 'src/Lib/st_tch/pwhg_flg.h'
     ...
     & vtimup(i),spinup(i)
 200  continue
      if(flg_pdfreweight) call st_tch_lhefwritepdfrw(nlf)
      if(flg_debug) call st_tch_lhefwritextra(nlf)
      write(nlf,'(a)')'</event>'      
...
c...writes trailer to a les houches events file on unit nlf. 
      subroutine st_tch_lhefwritetrailer(nlf)
      implicit none
      integer nlf,iran,n1ran,n2ran
      write(nlf,'(a)') '</LesHouchesEvents>'
c     save last st_tch_random number
      call st_tch_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) '#st_tch_random number generator exit st_tch_values: ',
     # iran,' ',n1ran,' ',n2ran
      end

      subroutine st_tch_lhefwritextra(nlf)
      implicit none
      include 'src/Lib/st_tch/LesHouches.h'
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      ...
      call st_tch_getcurrentrandom(iran,n1ran,n2ran)
      write(nlf,*) '# ', iran,' ',n1ran,' ',n2ran,
     #     "    previous event's st_tch_random seeds "
      write(nlf,'(a)') '# End extra-info-previous-event'
      end

      subroutine st_tch_lhefwritepdfrw(nlf)
      implicit none
      integer nlf
      integer id1,id2
      real * 8 x1,x2,xf1,xf2,xmufact
      call st_tch_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)

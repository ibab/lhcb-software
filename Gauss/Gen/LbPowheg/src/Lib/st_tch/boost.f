      subroutine st_tch_boost(beta,pin,pout)
      implicit none
...
c beta value
      ...
      b = sqrt(bsq)
      if (b**2.ge.1d0) then
         write(*,*) 'ERROR: st_tch_boost vector has st_tch_norm bigger than 1'
         write(*,*) 'RETURN 0'
         pout(0) = 0d0
         pout(1) = 0d0
         pout(2) = 0d0
...
c     To avoid problems with MC@NLO when using the same st_tch_analysis subroutine
      subroutine st_tch_boost_pow(beta,pin,pout)
      implicit none
      real * 8 beta(1:3),pin(0:3),pout(0:3)

      call st_tch_boost(beta,pin,pout)
      end

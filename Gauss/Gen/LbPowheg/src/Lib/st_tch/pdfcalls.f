      subroutine st_tch_pdfcall(ih,x,pdf)
      implicit none
      integer ih
      real * 8 x,pdf(-6:6)
      ...
      if(ih.eq.1) then
         call st_tch_genericpdf0(pdf_ndns1,pdf_ih1,st_mufact2,x,pdf)
      elseif(ih.eq.2) then
         call st_tch_genericpdf0(pdf_ndns2,pdf_ih2,st_mufact2,x,pdf)
      else
         write(*,*) ' st_tch_pdfcall: invalid call, ih=',ih
         stop
      endif
      end


c Front end to st_tch_genericpdf; it stores the arguments and return st_tch_values of
c the nrec most recent calls to st_tch_genericpdf. When invoked it looks in the
...
c when st_tch_CTEQ6 pdf are used.
      subroutine st_tch_genericpdf0(ns,ih,xmu2,x,fx)
      implicit none
      integer ns,ih
...
c set to impossible st_tch_values to begin with
      data ox/nrec*-1d0/
      data irec/0/
      integer j,k
      ...
      if(ini) then
         charmthr2=st_tch_powheginput('#charmthrpdf')
         bottomthr2=st_tch_powheginput('#bottomthrpdf')
      ...
      oxmu2(irec)=xmu2
      ox(irec)=x
      call st_tch_genericpdf(ns,ih,xmu2,x,ofx(-6,irec))

      SUBROUTINE st_tch_Sudx_epvebbxg(P1,ANS)
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2udx_epvebbxg(maxamps), jamp2udx_epvebbxg(0:maxamps)
      common/st_tch_to_amps_udx_epvebbxg/  amp2udx_epvebbxg,       jamp2udx_epvebbxg
      ...
      common/st_tch_to_matrix_udx_epvebbxg/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2udx_epvebbxg(ihel)=0d0
              jamp2udx_epvebbxg(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2udx_epvebbxg(0))
              jamp2udx_epvebbxg(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_tch_udx_epvebbxg(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2udx_epvebbxg(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2udx_epvebbxg(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_tch_udx_epvebbxg(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_tch_jioxxx(W(1,10  ),W(1,2   ),GWF ,WMASS   ,WWIDTH  ,W(1,11  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,9   ),W(1,11  ),GWF ,AMP(1   ))            
      CALL st_tch_fvoxxx(W(1,2   ),W(1,7   ),GG ,ZERO    ,ZERO    ,W(1,12  ))     
      CALL st_tch_jioxxx(W(1,1   ),W(1,12  ),GWF ,WMASS   ,WWIDTH  ,W(1,13  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,9   ),W(1,13  ),GWF ,AMP(2   ))            
      CALL st_tch_jioxxx(W(1,1   ),W(1,2   ),GWF ,WMASS   ,WWIDTH  ,W(1,14  ))    
...
c      JAMP(   2) = -AMP(   3)-AMP(   4)-AMP(   5)
      JAMP(   2) = -AMP(   4)-AMP(   5)
      st_tch_udx_epvebbxg = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_tch_udx_epvebbxg =st_tch_udx_epvebbxg+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2udx_epvebbxg(i)=amp2udx_epvebbxg(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2udx_epvebbxg(i)=Jamp2udx_epvebbxg(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

      SUBROUTINE st_tch_Sub_epvebdg(P1,ANS)
C  
...
C  
C FOR PROCESS : u b -> e+ ve b d g  
C  
C Crossing   1 is u b -> e+ ve b d g  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2ub_epvebdg(maxamps), jamp2ub_epvebdg(0:maxamps)
      common/st_tch_to_amps_ub_epvebdg/  amp2ub_epvebdg,       jamp2ub_epvebdg
      ...
      common/st_tch_to_matrix_ub_epvebdg/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2ub_epvebdg(ihel)=0d0
              jamp2ub_epvebdg(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2ub_epvebdg(0))
              jamp2ub_epvebdg(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_tch_ub_epvebdg(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2ub_epvebdg(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2ub_epvebdg(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_tch_ub_epvebdg(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_tch_oxxxxx(P(0,6   ),ZERO ,NHEL(6   ),+1*IC(6   ),W(1,6   ))        
      ...
      CALL st_tch_iovxxx(W(1,1   ),W(1,6   ),W(1,16  ),GWF ,AMP(3   ))            
      CALL st_tch_fvixxx(W(1,1   ),W(1,11  ),GWF ,ZERO    ,ZERO    ,W(1,17  ))    
      CALL st_tch_iovxxx(W(1,17  ),W(1,6   ),W(1,7   ),GG ,AMP(4   ))             
!ER:      CALL st_tch_fvoxxx(W(1,9   ),W(1,7   ),GG ,TMASS   ,TWIDTH  ,W(1,18  ))     
      ...
      CALL st_tch_iovxxx(W(1,1   ),W(1,6   ),W(1,19  ),GWF ,AMP(5   ))            
      JAMP(   1) = +AMP(   1)+AMP(   4)
c     !ER: removed gluon emission from decay leg
c      JAMP(   2) = +AMP(   2)+AMP(   3)+AMP(   5)
      JAMP(   2) = +AMP(   2)+AMP(   5)
      st_tch_ub_epvebdg = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_tch_ub_epvebdg =st_tch_ub_epvebdg+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2ub_epvebdg(i)=amp2ub_epvebdg(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2ub_epvebdg(i)=Jamp2ub_epvebdg(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

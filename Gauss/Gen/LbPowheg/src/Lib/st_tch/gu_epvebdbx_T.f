      SUBROUTINE st_tch_Sgu_epvebdbx_T(P1,ANS)
C  
...
C  
C FOR PROCESS : g u -> e+ ve b d b~  
C  
C Crossing   1 is g u -> e+ ve b d b~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2gu_epvebdbx_T(maxamps), jamp2gu_epvebdbx_T(0:maxamps)
      common/st_tch_to_amps_gu_epvebdbx_T/  amp2gu_epvebdbx_T,       jamp2gu_epvebdbx_T
      ...
      common/st_tch_to_matrix_gu_epvebdbx_T/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2gu_epvebdbx_T(ihel)=0d0
              jamp2gu_epvebdbx_T(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2gu_epvebdbx_T(0))
              jamp2gu_epvebdbx_T(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_tch_gu_epvebdbx_T(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2gu_epvebdbx_T(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2gu_epvebdbx_T(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_tch_gu_epvebdbx_T(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C BEGIN CODE
C ----------
      CALL st_tch_vxxxxx(P(0,1   ),ZERO ,NHEL(1   ),-1*IC(1   ),W(1,1   ))        
      ...
      CALL st_tch_jioxxx(W(1,7   ),W(1,9   ),GWF ,WMASS   ,WWIDTH  ,W(1,10  ))    
      CALL st_tch_fvixxx(W(1,2   ),W(1,10  ),GWF ,ZERO    ,ZERO    ,W(1,11  ))    
      CALL st_tch_iovxxx(W(1,11  ),W(1,6   ),W(1,1   ),GG ,AMP(1   ))             
      CALL st_tch_fvixxx(W(1,7   ),W(1,1   ),GG ,BMASS   ,ZERO    ,W(1,12  ))     
      CALL st_tch_jioxxx(W(1,12  ),W(1,9   ),GWF ,WMASS   ,WWIDTH  ,W(1,13  ))    
      CALL st_tch_iovxxx(W(1,2   ),W(1,6   ),W(1,13  ),GWF ,AMP(2   ))            
!ER:      CALL st_tch_fvoxxx(W(1,9   ),W(1,1   ),GG ,TMASS   ,TWIDTH  ,W(1,14  ))     
      CALL st_tch_fvoxxx(W(1,9   ),W(1,1   ),GG ,TMASS   ,ZERO  ,W(1,14  ))     
      CALL st_tch_jioxxx(W(1,7   ),W(1,14  ),GWF ,WMASS   ,WWIDTH  ,W(1,15  ))    
      CALL st_tch_iovxxx(W(1,2   ),W(1,6   ),W(1,15  ),GWF ,AMP(3   ))            
      CALL st_tch_fvixxx(W(1,2   ),W(1,1   ),GG ,ZERO    ,ZERO    ,W(1,16  ))     
      CALL st_tch_iovxxx(W(1,16  ),W(1,6   ),W(1,10  ),GWF ,AMP(4   ))            
      JAMP(   1) = (0d0,0d0)!ER:+AMP(   1)+AMP(   4)
      JAMP(   2) = +AMP(   2)+AMP(   3)
      st_tch_gu_epvebdbx_T = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_tch_gu_epvebdbx_T =st_tch_gu_epvebdbx_T+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2gu_epvebdbx_T(i)=amp2gu_epvebdbx_T(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2gu_epvebdbx_T(i)=Jamp2gu_epvebdbx_T(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

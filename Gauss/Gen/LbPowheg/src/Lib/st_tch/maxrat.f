      subroutine st_tch_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 st_tch_powheginput,st_tch_random
      external st_tch_powheginput,st_tch_random      
      call st_tch_zerohistnorms
      loaded=.false.
      nubound=st_tch_powheginput('nubound')
      rad_nynorms=st_tch_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=st_tch_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=st_tch_powheginput('#xupbound')
      ...
      if(st_tch_powheginput('use-old-ubound').eq.1) then
         call st_tch_newunit(iun)
...
c Compute and store normalization grid
 111  continue
     ...
     # ' bounding function for radiation'
      ...
      enddo
      do j=1,nubound
         call st_tch_gen_born(mcalls,icalls)
         call st_tch_gen_uborn_idx
         kn_csitilde=st_tch_random()
         kn_y=2*st_tch_random()-1
         kn_azi=2*pi*st_tch_random()
         do rad_kinreg=1,rad_nkinreg
            if(rad_kinreg.eq.1.and.rad_kinreg_on(1)) then
c     initial state radiation
               call st_tch_gen_real_phsp_isr_rad0
               call st_tch_inc_norms
            elseif(rad_kinreg_on(rad_kinreg)) then
c     final state radiation
               call st_tch_gen_real_phsp_fsr_rad0
               call st_tch_inc_norms
            endif
         enddo    
      enddo
...
c      call maxratident
      ...
      common/st_tch_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical st_tch_refuse_pdf
      real * 8 st_tch_pwhg_pt2,st_tch_pwhg_upperb_rad
      external st_tch_pwhg_pt2,st_tch_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         call st_tch_set_rad_scales(ptsq)
         if(.not.st_tch_refuse_pdf()) then
            ...
            if(born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' st_tch_inc_norms: Warning, born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' st_tch_inc_norms: x1,x2', kn_xb1,kn_xb2
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' st_tch_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' st_tch_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/born/st_tch_pwhg_upperb_rad()
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
      ...
      subroutine st_tch_printhistnorms
      implicit none
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      include 'src/Lib/st_tch/pwhg_rad.h'
     ...
     1              efficiencies(ireg,iborn)/st_tch_powheginput('nubound')/
...
c     1           st_tch_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine st_tch_zerohistnorms
      implicit none
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      include 'src/Lib/st_tch/pwhg_rad.h'
      ...
      function st_tch_refuse_pdf()
c Some pdf have (inconsistently) vanishing quark density at x st_tch_values
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call st_tch_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call st_tch_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine st_tch_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/st_tch/pwhg_math.h'
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
...
c
      ...
      subroutine st_tch_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 st_tch_btilde,st_tch_powheginput
      external st_tch_btilde,st_tch_powheginput
...
c will not be used;
         ...
         call st_tch_mint(st_tch_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=st_tch_powheginput('ncall2')
         itmx2=st_tch_powheginput('itmx2')
         call st_tch_mint(st_tch_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call st_tch_gen(st_tch_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn't used the ifold array may need reset to 1 here.
      call st_tch_gen(st_tch_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

      ...
      real * 8 st_tch_pwhg_pt2,st_tch_pt2max_regular
      external st_tch_pwhg_pt2,st_tch_pt2max_regular
      integer i
      if(idwtup.eq.3) then
         xwgtup=1
...
c     store current st_tch_random seeds. To be used to restart at problematic events
      call st_tch_savecurrentrandom
      if(st_tch_random().gt.rad_totrm/rad_totgen) then
c     generate underlying Born kinematics
         call st_tch_gen_btilde(mcalls,icalls)
c     generate underlying Born flavour
         call st_tch_gen_uborn_idx
c
         if(st_tch_powheginput('#st_tch_testsuda').eq.1) then
            call st_tch_testsuda
         endif
c generate radiation
         call st_tch_gen_radiation
c add a st_tch_random azimuthal rotation around beam axis
         call st_tch_add_azimuth
c --- set up les houches interface
         rad_pt2max=st_tch_pwhg_pt2()
         call st_tch_gen_leshouches
c if negative weight, flip the sign of xwgtup
         if(rad_btilde_sign(rad_ubornidx).eq.-1) then
            xwgtup=-xwgtup
         endif
c rad_type=1 for st_tch_btilde events (used only for debugging purposes)
         rad_type=1
...
c iret=2: reg contribution (real graphs without singular regions)
         call st_tch_gen_remnant(iret)
c         if (st_tch_pwhg_pt2().lt.rad_ptsqmin) then
c            write(*,*) '****************************************'
c            write(*,*) 'WARNING in st_tch_gen_remnant'
c            write(*,*) 'st_tch_pwhg_pt2 < rad_ptsqmin ',
c     #           st_tch_pwhg_pt2(),' < ',rad_ptsqmin
c            write(*,*) (flst_alr(i,rad_realalr),i=1,nlegreal)
c            write(*,*) 'To generate this event, use the following seeds'
c            call st_tch_printcurrentrandom
c            write(*,*) '****************************************'
c         endif
...
c     set st_muren2 equal to pt2 for scalup value
            rad_pt2max=max(rad_ptsqmin,st_tch_pwhg_pt2())
            call st_tch_set_rad_scales(rad_pt2max)
            call st_tch_gen_leshouches
            call st_tch_born_suppression(suppfact)
            if(suppfact.eq.0) then
               write(*,*) ' 0 suppression factor in event generation'
...
c     set st_muren2 for scalup value for regular contributions
            rad_pt2max=max(rad_ptsqmin,st_tch_pt2max_regular())
            call st_tch_set_rad_scales(rad_pt2max)
            call st_tch_gen_leshouches_reg
c rad_type=3 for regular contributions
      ...
      subroutine st_tch_gen_radiation
      implicit none
      include 'src/Lib/st_tch/pwhg_math.h'
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      ...
      real * 8 tmax
      common/st_tch_ctmax/tmax
      integer kinreg,firstreg,lastreg
      logical ini
      data ini/.true./
      real * 8 st_tch_pwhg_pt2,st_tch_powheginput
      external st_tch_pwhg_pt2,st_tch_powheginput
      save ini,firstreg,lastreg
      if(ini) then
         firstreg=st_tch_powheginput("#radregion")
...
c     initial state radiation
               call st_tch_gen_rad_isr(t)
            else
c     final state radiation
               call st_tch_gen_rad_fsr(t)
            endif
            if(t.gt.tmax) then
               tmax=t
               kinreg=rad_kinreg
...
c Generate a Born like event
         ...
         t=tmax
         if(rad_kinreg.eq.1) then
            call st_tch_gen_real_phsp_isr_rad
         else
            call st_tch_gen_real_phsp_fsr_rad
         endif
         call st_tch_set_rad_scales(t)
         call st_tch_sigreal_rad(sig)
         call st_tch_gen_real_idx
      endif
      end



      function st_tch_pwhg_pt2()
      implicit none
      real * 8 st_tch_pwhg_pt2
      include 'src/Lib/st_tch/pwhg_math.h'
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      include 'src/Lib/st_tch/pwhg_kn.h'
      include 'src/Lib/st_tch/pwhg_rad.h'
      if(rad_kinreg.eq.1) then
         st_tch_pwhg_pt2=(kn_sreal/4)*(1-kn_y**2)*kn_csi**2
      else
         st_tch_pwhg_pt2=(kn_sreal/2)*(1-kn_y)*kn_csi**2
      endif
      end



      function st_tch_pwhg_upperb_rad()
      implicit none
      real * 8 st_tch_pwhg_upperb_rad
      include 'src/Lib/st_tch/pwhg_math.h'
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      include 'src/Lib/st_tch/pwhg_kn.h'
      ...
      if(rad_kinreg.eq.1) then
         if(rad_iupperisr.eq.1) then
            st_tch_pwhg_upperb_rad = st_alpha/((1-x)*(1-y**2))
c Possible alternatives:
c rad_iupper=2   st_tch_pwhg_upperb_rad = st_alpha/(x*(1-x)*(1-y**2))
c 
c rad_iupper=3:  st_tch_pwhg_upperb_rad = st_alpha/(x**2*(1-x)*(1-y**2))
         else
...
c for now use the same
         if(rad_iupperfsr.eq.1) then
            st_tch_pwhg_upperb_rad = st_alpha/(csi*(1-y))
         elseif(rad_iupperfsr.eq.2) then
            st_tch_pwhg_upperb_rad = st_alpha/(csi**2*(1-y)*(1-csi/2*(1-y))**2)
     2          *csi
     ...
     1            'alternative not implemented'
      ...
      function st_tch_pt2solve(pt2,i)
...
c We use it to find its zero in pt2.
      ...
      include 'src/Lib/st_tch/pwhg_rad.h'
      include 'src/Lib/st_tch/pwhg_math.h'
      real * 8 st_tch_pt2solve,pt2
c i set by st_tch_DZERO: 1 for first call, 2 for subsequent calls, 3 for last call
c before a normal exit; not used here
      integer i
      real * 8 xlr,q2,xlam2c,kt2max,cunorm
      integer nlc
      common/st_tch_cpt2solve/xlr,q2,kt2max,xlam2c,cunorm,nlc
...
c see Notes/upperbounding-isr.pdf
     ...
     #           + xlr
               ...
               st_tch_pt2solve=cunorm*pi/b0*(log(2d0)
...
c         st_tch_pt2solve=cunorm*pi/b0/2
...
c final state radiation
     ...
     2   /(p*xm)/2.d+0) + xlr
      ...
      subroutine st_tch_gen_rad_isr(t)
c Generates hard radiation kinematics according to
c appendix D in ZZ paper.
c
c  common/st_tch_cptmin/ptminsq: minimum pt^2 accepted
c 
      implicit none
      include 'src/Lib/st_tch/pwhg_math.h'
      include 'src/Lib/st_tch/nlegborn.h'
      ...
      integer nlc
      common/st_tch_cpt2solve/xlr,q2,kt2max,xlam2c,unorm,nlc
      real * 8 xmin,rv,xp,xm,chi,tk,uk,ubound,ufct,
     #   sborn,value,err,tmp1,tmp2,tmp,rvalue,born,sig
      common/st_tch_cdfxmin/xmin
      real * 8 tmax
      common/st_tch_ctmax/tmax
      real * 8 st_tch_random,st_tch_pt2solve,dfxmin,st_tch_pwhg_alphas0,st_tch_pwhg_upperb_rad
      external st_tch_random,st_tch_pt2solve,dfxmin,st_tch_pwhg_alphas0,st_tch_pwhg_upperb_rad
...
c see section 4 in ZZ paper, last paragraph
      ...
      xlr=0
 1    continue
      xlr=xlr+log(st_tch_random())
c CERNLIB voodoo:
      call st_tch_KERSET('C205. ',0,0,101)
c solve for zero of st_tch_pt2solve
c st_tch_DZERO(xmin,xmax,x,err,eps,maxcalls,function)
...
c eps: required accuracy
      call st_tch_DZERO(rad_ptsqmin,kt2max,t,err,1d-8,1000000,st_tch_pt2solve)
c error conditions
      if(t.eq.0.and.err.lt.0d0
     # .and.err.gt.rad_ptsqmin-kt2max) then
         write(*,*) 'st_tch_DZERO fails'
...
c to set xmuren2:
      call st_tch_set_rad_scales(t)
      tmp2=st_alpha / st_tch_pwhg_alphas0(t,rad_lamll,nlc)
      tmp=tmp1*tmp2
      if(tmp.gt.1) then
...
c using       d sqrt(xm-x) /sqrt(xp-xm) (rad_iupper=1) or d sqrt(xm-x) /(xmin sqrt(xp-xm)) (rad_iupper=2) as upper bound using hit and miss
 2    chi=sqrt(xm-xmin)*st_tch_random()
      x=xm-chi**2
      if(rad_iupperisr.eq.1) then
         if(st_tch_random().gt.sqrt(xp-xm)/sqrt(xp-x)) goto 2
      elseif(rad_iupperisr.eq.2) then
         if(st_tch_random().gt.(xmin*sqrt(xp-xm))/(x*sqrt(xp-x))) goto 2
      endif
c get y (abs to avoid tiny negative st_tch_values)
      y=sqrt(abs(1-4*x/(1-x)**2*t/sborn))
      if(st_tch_random().gt.0.5d0) y=-y
...
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call st_tch_uboundfct(ufct,1-x,y)
      if(st_tch_random().gt.ufct) goto 1
c Veto from upper bound to real value. Count how many vetoes,
c since these may be expensive.
      call st_tch_sigborn_rad(born)
      if(born.lt.0) then
         born=0
      endif
      if(born.eq.0) then
...
c that some pdf vanish (typically heavy flavour pdf's)
      ...
      kn_y=y
      kn_csi=1-x
      kn_azi=2*pi*st_tch_random()
      ubound=born*st_tch_pwhg_upperb_rad()*unorm*ufct
      call st_tch_gen_real_phsp_isr_rad
      call st_tch_sigreal_rad(sig)
      value=sig*kn_jacreal
      if(value.gt.ubound) then
         call st_tch_increasecnt(
     # 'upper bound failures in generation of radiation')
      endif
      rvalue=st_tch_random()*ubound
      if(rvalue.gt.value) then
         call st_tch_increasecnt('vetoed radiation')
         goto 1
      endif
      end


      subroutine st_tch_gen_rad_fsr(t)
...
c 
     ...
     #   s,value,err,tmp,rvalue,born,sig
      ...
      real * 8 tmax
      common/st_tch_ctmax/tmax
      real * 8 st_tch_random,st_tch_pt2solve,st_tch_pwhg_alphas0,st_tch_pwhg_upperb_rad
      external st_tch_random,st_tch_pt2solve,st_tch_pwhg_alphas0,st_tch_pwhg_upperb_rad
      unorm=rad_norms(rad_kinreg,rad_ubornidx)
c kn_sborn=kn_sreal:
      s=kn_sborn
...
c vetoes:
      ...
      call st_tch_set_rad_scales(t)
      if(rad_iupperfsr.eq.1) then
         tmp=st_alpha / st_tch_pwhg_alphas0(t,rad_lamll,nlc)
      elseif(rad_iupperfsr.eq.2) then
         tmp=st_alpha
      elseif(rad_iupperfsr.eq.3) then
         tmp=st_alpha
...
c extra csi dependent factor
         if(st_tch_random().gt.csi) goto 1
c get y
         y=1-2*t/(s*csi**2)
c At this point a csi-y pair is generated according to the
c distribution upper(). It is automatically within range.
...
c get y
         y=1-2*t/(s*csi**2)
         if(st_tch_random().gt.(csi-t/s)) goto 1
      else
         write(*,*) ' st_tch_gen_rad_fsr:  rad_iupper=',rad_iupperfsr,
     1        ' invalid'
      endif
c
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call st_tch_uboundfct(ufct,csi,y)
      if(st_tch_random().gt.ufct) goto 1
c Veto from upper bound to real value. Count how many vetoes,
c since these may be expensive.
...
c that some pdf vanish (typically heavy flavour pdf's)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      azi=2d0*pi*st_tch_random()
      sazi = sin(azi)
      cazi = cos(azi)
      if (kn_csi.eq.0d0) then
         do ileg=1, nlegborn
            call st_tch_mrotate(dir,sazi,cazi,kn_pborn(1,ileg))
         enddo
      else
         do ileg=1, nlegreal
            call st_tch_mrotate(dir,sazi,cazi,kn_preal(1,ileg))
         enddo
      endif
      end

      subroutine st_tch_bornzerodamp(alr,r0,rc,rs,dampfac)
...
c the real contribution to implement Born zero suppression
      ...
      integer alr
      real * 8 r0,rc,rs,dampfac,h,st_tch_pwhg_pt2,pt2,st_tch_powheginput
      logical ini
      data ini/.true./
      save ini,h
      external st_tch_pwhg_pt2,st_tch_powheginput
      if(ini) then
         h=st_tch_powheginput("#hfact")
         if(h.gt.0) then
            write(*,*)'***************************************'
...
c      write(*,*) ' st_tch_bornzerodamp: r0=',r0,'  rc=',rc,'  rs=',rs,
c     # ' dampfac=',dampfac
      end

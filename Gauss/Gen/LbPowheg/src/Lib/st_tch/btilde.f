      function st_tch_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 st_tch_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call st_tch_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call st_tch_btildevirt(resvirt)
            call st_tch_btildecoll(xrad,rescoll,www)
            call st_tch_btildereal(xrad,resreal,www)
         endif
c     accumulate st_tch_values
         st_tch_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            st_tch_btilde=st_tch_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to st_tch_btildeborn and st_tch_btildevirt can be
...
c     to the NLO st_tch_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c st_tch_btildecoll and st_tch_btildereal take care themselves to invoke the NLO
c st_tch_analysis if required.
            call st_tch_btildecoll(xrad,rescoll,www)
            call st_tch_btildereal(xrad,resreal,www)
         endif
         st_tch_btilde=0
...
c     accumulate st_tch_values in results
...
c output Born
            ...
            call st_tch_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
               ...
               call st_tch_analysis_driver(totvirt,0)
            endif
c closing call to end a sequence of correlated events in the
c st_tch_analysis routines.
            call st_tch_pwhgaccumup
         endif
c     closing call: accumulate st_tch_values with correct signs
         st_tch_btilde=0
         call st_tch_adduptotals(results,flst_nborn)
         do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            st_tch_btilde=st_tch_btilde+results(j)
c     Transfer all flavour components of st_tch_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in st_tch_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine st_tch_adduptotals(results,n)
      implicit none
      include 'src/Lib/st_tch/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/st_tch_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(st_tch_powheginput('#ubsigmadetails').eq.1) then

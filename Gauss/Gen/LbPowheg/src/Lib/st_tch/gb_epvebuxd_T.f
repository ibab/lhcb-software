      SUBROUTINE st_tch_Sgb_epvebuxd_T(P1,ANS)
C  
...
C  
C FOR PROCESS : g b -> e+ ve b u~ d  
C  
C Crossing   1 is g b -> e+ ve b u~ d  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2gb_epvebuxd_T(maxamps), jamp2gb_epvebuxd_T(0:maxamps)
      common/st_tch_to_amps_gb_epvebuxd_T/  amp2gb_epvebuxd_T,       jamp2gb_epvebuxd_T
      ...
      common/st_tch_to_matrix_gb_epvebuxd_T/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2gb_epvebuxd_T(ihel)=0d0
              jamp2gb_epvebuxd_T(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2gb_epvebuxd_T(0))
              jamp2gb_epvebuxd_T(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_tch_gb_epvebuxd_T(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2gb_epvebuxd_T(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2gb_epvebuxd_T(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_tch_gb_epvebuxd_T(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_tch_oxxxxx(P(0,7   ),ZERO ,NHEL(7   ),+1*IC(7   ),W(1,7   ))        
      CALL st_tch_jioxxx(W(1,3   ),W(1,4   ),GWF ,WMASS   ,WWIDTH  ,W(1,8   ))    
      CALL st_tch_fvoxxx(W(1,5   ),W(1,8   ),GWF ,TMASS   ,TWIDTH  ,W(1,9   ))    
      CALL st_tch_fvoxxx(W(1,7   ),W(1,1   ),GG ,ZERO    ,ZERO    ,W(1,10  ))     
      CALL st_tch_jioxxx(W(1,2   ),W(1,9   ),GWF ,WMASS   ,WWIDTH  ,W(1,11  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,10  ),W(1,11  ),GWF ,AMP(1   ))            
      CALL st_tch_fvoxxx(W(1,7   ),W(1,11  ),GWF ,ZERO    ,ZERO    ,W(1,12  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,12  ),W(1,1   ),GG ,AMP(2   ))             
      CALL st_tch_fvoxxx(W(1,9   ),W(1,1   ),GG ,TMASS   ,TWIDTH  ,W(1,13  ))     
      CALL st_tch_jioxxx(W(1,2   ),W(1,13  ),GWF ,WMASS   ,WWIDTH  ,W(1,14  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,7   ),W(1,14  ),GWF ,AMP(3   ))            
      CALL st_tch_fvixxx(W(1,2   ),W(1,1   ),GG ,BMASS   ,ZERO    ,W(1,15  ))     
      CALL st_tch_jioxxx(W(1,15  ),W(1,9   ),GWF ,WMASS   ,WWIDTH  ,W(1,16  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,7   ),W(1,16  ),GWF ,AMP(4   ))            
      JAMP(   1) = +AMP(   1)+AMP(   2)
      JAMP(   2) = (0d0,0d0)!ER:+AMP(   3)+AMP(   4)
      st_tch_gb_epvebuxd_T = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_tch_gb_epvebuxd_T =st_tch_gb_epvebuxd_T+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2gb_epvebuxd_T(i)=amp2gb_epvebuxd_T(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2gb_epvebuxd_T(i)=Jamp2gb_epvebuxd_T(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

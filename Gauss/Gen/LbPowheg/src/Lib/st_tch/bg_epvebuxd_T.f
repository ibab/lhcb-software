      SUBROUTINE st_tch_Sbg_epvebuxd_T(P1,ANS)
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2bg_epvebuxd_T(maxamps), jamp2bg_epvebuxd_T(0:maxamps)
      common/st_tch_to_amps_bg_epvebuxd_T/  amp2bg_epvebuxd_T,       jamp2bg_epvebuxd_T
      ...
      common/st_tch_to_matrix_bg_epvebuxd_T/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2bg_epvebuxd_T(ihel)=0d0
              jamp2bg_epvebuxd_T(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2bg_epvebuxd_T(0))
              jamp2bg_epvebuxd_T(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_tch_bg_epvebuxd_T(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2bg_epvebuxd_T(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2bg_epvebuxd_T(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_tch_bg_epvebuxd_T(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_tch_fvoxxx(W(1,9   ),W(1,2   ),GG ,TMASS   ,TWIDTH  ,W(1,10  ))     
      CALL st_tch_jioxxx(W(1,1   ),W(1,10  ),GWF ,WMASS   ,WWIDTH  ,W(1,11  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,7   ),W(1,11  ),GWF ,AMP(1   ))            
      CALL st_tch_fvoxxx(W(1,7   ),W(1,2   ),GG ,ZERO    ,ZERO    ,W(1,12  ))     
      CALL st_tch_jioxxx(W(1,1   ),W(1,9   ),GWF ,WMASS   ,WWIDTH  ,W(1,13  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,12  ),W(1,13  ),GWF ,AMP(2   ))            
      CALL st_tch_fvoxxx(W(1,7   ),W(1,13  ),GWF ,ZERO    ,ZERO    ,W(1,14  ))    
      CALL st_tch_iovxxx(W(1,6   ),W(1,14  ),W(1,2   ),GG ,AMP(3   ))             
      CALL st_tch_fvixxx(W(1,1   ),W(1,2   ),GG ,BMASS   ,ZERO    ,W(1,15  ))     
      ...
      JAMP(   2) = +AMP(   2)+AMP(   3)
      st_tch_bg_epvebuxd_T = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_tch_bg_epvebuxd_T =st_tch_bg_epvebuxd_T+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2bg_epvebuxd_T(i)=amp2bg_epvebuxd_T(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2bg_epvebuxd_T(i)=Jamp2bg_epvebuxd_T(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

...
C Call st_tch_PWHGBOOK(N,'TITLE',DEL,XMIN,XMAX) for each histogram to be booked.
...
C Example:
C      call st_tch_PWHGBOOK(2,'pt distribution',1.,10,70)
...
C
C FILLING:
C When it's time, call st_tch_PWHGFILL(N,X,Y); this will add Y (real*4) to the bin 
C in which X (real*4) happens to be, within histogram N. 
C
C PLAYING AROUND:
C At the end of the day you may want to sum, divide, cancel, etc.etc.
C various histograms (bin by bin). Then you call st_tch_PWHGOPERA(I,'O',J,K,X,Y). 
C The 1-character string O can take the following st_tch_values:
...
C M  : statistical st_tch_analysis; if I contains the weights (let's say WGT),
...
C      Fill I with the st_tch_values of
...
C
C FINAL ACCOUNTING:
C Now we can finalize our histograms; st_tch_PWHGFINAL(N) will calculate the integral
C of the histogram N, the mean value of the X variable and its RMS.
C If we now want to renormalize the hist's, we can call st_tch_PWHGNORM(N,X), which
C will normalize the integral to X  -- CAUTION: do not call st_tch_PWHGNORM before
C st_tch_PWHGFINAL, it will blow up.
C
C OUTPUT:
C To get a .dat file containing the st_tch_values of the histograms, together with
C some information (like integral, mean st_tch_values, etc.etc.) call st_tch_PWHGPRINT(N),
C for each hist N that you want in the .dat file. Before the call to st_tch_PWHGPRINT
C you want to open unit 98 and give it a name:
C     OPEN(UNIT=98,NAME='NAME.DAT',STATUS='NEW')
C If you want a topdrawer file with a plot of the hist st_tch_values, call 
C st_tch_PWHGTOP(N,M,'X','Y','SCALE'). The points of the plot will be taken from histogram
...
C itself. Before calling st_tch_PWHGTOP,
C     OPEN(UNIT=99,NAME='NAME.TOP',STATUS='NEW')
C--------------------------------------------------------------------------
C
C  COMMON/st_tch_histo/  Histogram N
C                           
...
C   HAVG(N),      Average value of x, weighted over the x range of the st_tch_histo
...
C
      ...
      SUBROUTINE st_tch_pwhgINIHIST
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      integer i
      DO 1, I=1,NMB             
   1  BOOK(I)=' NO'
      DO 2 I=1,10
   2  BOOK2(I)=' NO'
      call st_tch_pwhgresetmultitop
      END  


      subroutine st_tch_pwhggettitle(N,TIT)
      implicit none
      character *(*) TIT
      integer n
      include 'src/Lib/st_tch/pwhg_book.h'
      ...
      SUBROUTINE st_tch_PWHGBOOK(N,TIT,DEL,XMIN,XMAX)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      integer n,i
      ...
      DATA ONEP/1.00001d0/
      IF(N.GT.NMB) THEN
	 CALL st_tch_pwhgwarn('st_tch_PWHGBOOK')
         WRITE(*,*) 'st_tch_PWHGBOOK: no more then ',NMB,' histograms'
         STOP
      ENDIF
      NHIST = MAX(N,NHIST)
...
1     HDEL(N) = DEL
	...
	GO TO 1
     ...
     #'Histogram ', TIT, ' Change of upper limit:',xmax,'-->',HMAX(N)
      ...
      SUBROUTINE st_tch_PWHGFILL(N,X,Y)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      real * 8 x,y,xi
      integer n,i
...
C
      ...
      SUBROUTINE st_tch_PWHGINTEG(NIN,NOUT,IDIR,IPOW)
...
C of a given distribution. Overscores and underscores are included.
      ...
      I = (IDIR + 3)/2
      IF(NOUT.NE.NIN) THEN
      	CALL st_tch_PWHGBOOK(NOUT,TITLE(NIN)//C(I), 
     &                HDEL(NIN),HMIN(NIN),HMAX(NIN))
      ENDIF
      IF(IDIR.EQ.1) THEN
         HIST(NOUT,1) = st_tch_PWHGSUMPOW(HIST(NIN,1),USCORE(NIN),IPOW)
         IHIS(NOUT,1) = IHIS(NIN,1) + IUSCORE(NIN)
         XHIS(NOUT,1) = XHIS(NIN,1) + HDEL(NIN)/2
         DO L=2,M                      
            HIST(NOUT,L) = st_tch_PWHGSUMPOW(HIST(NIN,L),HIST(NOUT,L-1),IPOW)
            IHIS(NOUT,L) = IHIS(NIN,L) + IHIS(NOUT,L-1) 
            XHIS(NOUT,L) = XHIS(NIN,L) + HDEL(NIN)/2
         ENDDO
         OSCORE(NOUT) = st_tch_PWHGSUMPOW(OSCORE(NIN),HIST(NIN,M),IPOW)
         IOSCORE(NOUT) = IOSCORE(NIN) + IHIS(NIN,M)
      ELSEIF(IDIR.EQ.-1) THEN
         HIST(NOUT,M) = st_tch_PWHGSUMPOW(HIST(NIN,M),OSCORE(NIN),IPOW)
         ...
         DO L=M-1,1,-1                        
            ...
            XHIS(NOUT,L) = XHIS(NIN,L) - HDEL(NIN)/2
         ENDDO
         USCORE(NOUT) = st_tch_PWHGSUMPOW(USCORE(NIN),HIST(NIN,1),IPOW)
         IUSCORE(NOUT) = IUSCORE(NIN)+IHIS(NIN,1)
      ELSE                                 
         CALL st_tch_pwhgwarn('st_tch_PWHGINTEG')
         WRITE(*,*) 'Wrong idir in st_tch_PWHGINTEG: OPERATION NOT PERFORMED'
         STOP
      ENDIF
      END

      FUNCTION st_tch_PWHGSUMPOW(X,Y,IPOW)
      implicit none
      real * 8 x,y,st_tch_PWHGSUMPOW
      integer ipow
      IF(IPOW.EQ.1) THEN
         st_tch_PWHGSUMPOW = X + Y
      ELSEIF(IPOW.EQ.2) THEN
         st_tch_PWHGSUMPOW = SQRT(X**2+Y**2)
      ELSEIF(IPOW.EQ.0) THEN
         CALL st_tch_pwhgwarn('st_tch_PWHGSUMPOW')
         WRITE(*,*)'Error: IPOW=0 not allowed in st_tch_PWHGSUMPOW'
         STOP
      ELSE
         st_tch_PWHGSUMPOW = (X**IPOW+Y**IPOW)**(1./IPOW)
      ENDIF
      END

      SUBROUTINE st_tch_PWHGOPERA(I,OPER,J,K,X,Y)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      integer i,j,k,l
     ...
     &    '*'.OR.OPER.EQ.'/'.OR.OPER.EQ.'M'.OR.OPER.EQ.'A')) THEN
	  CALL st_tch_pwhgwarn('st_tch_PWHGOPERA')
          WRITE(20,*) I,J                               
...
c the average value of I to K and will put in J the quadratic dispersion.
   ...
   1  HIST(N,I)=0
      ...
      SUBROUTINE st_tch_PWHGPUTTAG(J,NAME)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      integer j
c Per marcare un istogramma
      ...
      BOOK(J) = NAME
      RETURN
      ENTRY st_tch_pwhgGETTAG(J,TAG)
      TAG = BOOK(J)
      END

      SUBROUTINE st_tch_PWHGFINAL(N)
      implicit none
      integer n
 ...
 10   CONTINUE
      ...
      SUBROUTINE st_tch_PWHGNORM(N,X)
      implicit none
      integer n,i
      real * 8 x,y
      include 'src/Lib/st_tch/pwhg_book.h'
      ...
      IF(HINT(N).EQ.0.) THEN
	CALL st_tch_pwhgwarn('st_tch_PWHGNORM')
...
c      CALL TIME(CTIME)
   ...
   20 FORMAT(' ENTRIES=',I5,4X,'UNDERSCORE=',I5,4x,'OVERSCORE=',I5,//)
      ...
      SUBROUTINE st_tch_PWHGTOP(N,M,BTIT,LTIT,SCALE)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
...
c      CALL TIME(CTIME)
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE st_tch_PWHGMNRTOP(N,M,BTIT,LTIT,SCALE)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      integer imon,iday,iyear,ini,n,m,j,itime(3)
      CHARACTER CTIME*8
...
c         CALL TIME(CTIME)
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE st_tch_PWHGBOOK2(N,TIT,DELX,XMIN,XMAX,DELY,YMIN,YMAX)
...
C-- setup y-boundaries
   ...
   3  CONTINUE
      ...
      SUBROUTINE st_tch_PWHGFILL2(N,X,Y,WGT)
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/st_tch/pwhg_book.h'
C-- find x-coordinate
...
C-- find y-coordinate
     ...
     *                                             THEN        
      ...
      SUBROUTINE st_tch_PWHGOPERA2(I,OPER,J,K,X,Y)
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/st_tch/pwhg_book.h'
      CHARACTER OPER*1
  ...
  20  FORMAT(' ****** INCOMPATIBLE OPERATION HIST2 ',I2,' &',I2,
     &                                                   '*******'/)
      ...
      SUBROUTINE st_tch_PWHGFINAL2(N)
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/st_tch/pwhg_book.h'
...
C-- projection on the y-axis
  ...
  10  BOOK2(N)=' NO'
      ...
      SUBROUTINE st_tch_PWHGPROHIS(N,M,IAX)
...
C   put the contents in histogram M, after automatically booking it.
      ...
      SUBROUTINE st_tch_PWHGNORM2(N,X)    
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/st_tch/pwhg_book.h'
      IF(BOOK2(N).NE.'YES')RETURN
      DO 1, I=1,NBIN2(1,N)
    ...
    1 HIST2(N,I,J)=HIST2(N,I,J)/HINT2(N)*X
      ...
      SUBROUTINE st_tch_PWHGPRINT2(N)
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/st_tch/pwhg_book.h'
...
c$$$      DO ND=1,NDOTS
c$$$      X=XHIS2(N,I)+(2.*st_tch_random()-1.)*HDEL2(1,N)
c$$$      Y=YHIS2(N,J)+(2.*st_tch_random()-1.)*HDEL2(2,N)
c$$$      WRITE(99,*) X,Y
c$$$      ENDDO
c$$$      NTOT=NTOT+NDOTS
c$$$  3   CONTINUE       
...
c$$$      END
      ...
      SUBROUTINE st_tch_PWHGMULTITOP(NH,NE,N,M,BTIT,LTIT,SCA,INI)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
...
C
      ENTRY st_tch_PWHGTFILL(NH,NE,N,M,BTIT,LTIT,SCA)
      IP=IP+1                             
      IF(IP.GT.4) IP=1
...
202   FORMAT('   NEW PLOT',/,/)
203   RETURN     
      entry st_tch_pwhgresetmultitop
      iframe=0
      END                      

      SUBROUTINE st_tch_PWHGNEWPLOT
      WRITE(99,202) 
202   FORMAT('   NEW PLOT',/,/)
      CALL st_tch_PWHGMULTITOP(iDUM,iDUM,iDUM,iDUM,' ',' ','REF',0)
      END


c  added by SA and ER to use with MC@NLO
      SUBROUTINE st_tch_PWHGCOPY(NIN,NOUT)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      integer nin,nout,i
...
C This may happen if st_tch_PWHGFINAL is called prior to st_tch_PWHGCOPY
      IF(BOOK(NIN)(1:1).EQ.'R'.AND.HMIN(NIN).EQ.HMAX(NIN)) RETURN
      IF(BOOK(NOUT)(1:1).NE.'Y'.AND.BOOK(NOUT)(1:1).NE.'R') 
     &  CALL st_tch_PWHGBOOK(NOUT,TITLE(NIN)(2:),HDEL(NIN),HMIN(NIN),HMAX(NIN))
      ...
      subroutine st_tch_pwhgwarn(str)
      character *(*) str
      write(*,*) '********** WARNING **********'
      write(*,*) '*********  ',str,'  *********'
      end
      ...
      subroutine st_tch_pwhgbookup(n,string,scale,del,xl,xu)
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      real * 8 del,xl,xu
      integer n
...
c     A call to st_tch_pwhgaccumup causes
c     the histogram n to be summed to the n+nmh, its square is
c     summed into n+nmh2. and n is zeroed.
c
c     When the subroutine st_tch_pwhgaddout is called, the statistical
c     st_tch_analysis of the n+nmh and n+nmh2 content is performed, the
c     average (with respect to the number of calls to st_tch_pwhgaccumup)
c     is added to the n+nmh3, and the error is summed in quadrature
c     to the n+nmh4; n+nmh and n+nmh2 are zeroed.
c
c     If instead st_tch_pwhgsetout is called, the statistical
c     st_tch_analysis of the n+nmh and n+nmh2 content is performed, the
c     average (with respect to the number of calls to st_tch_pwhgaccumup)
...
c.
      ...
      call st_tch_PWHGBOOK(n,  string,del,xl,xu)
      call st_tch_PWHGBOOK(n+nmh,'tmp ',del,xl,xu)
      call st_tch_PWHGBOOK(n+nmh2,'tmp square',del,xl,xu)
c this is the histogram that will be output
      call st_tch_PWHGBOOK(n+nmh3,string,del,xl,xu)
c the scale for plotting (log or lin) is encoded in the title
      call st_tch_PWHGBOOK(n+nmh4,scale//'error ',del,xl,xu)
      call st_tch_PWHGPUTTAG(n,'YST')
      return
      end

      

      subroutine st_tch_pwhgaccumup
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      integer j
      character * 3 tag
...
c
c     Accumulates st_tch_values (j+nmh) and squared st_tch_values (j+nmh2) for
c     statistical st_tch_analysis, then empty the original histogram j.
c
      do j=1,nmh
         call st_tch_pwhgGETTAG(j,tag)
         if(tag.eq.'YST') then
            do l=1,nbin(j)
...
c
      do j=1,nmb
c     finish statistical st_tch_analysis
         call st_tch_pwhgGETTAG(j,tag)
         if(tag.eq.'YST') then
...
c     for the mean value is the number of calls to st_tch_pwhgaccumup, which in
...
c     histogram, as filled by pwhgaccumup.
     ...
     1             + xxx**2*(abs(xsumsq-xsum**2*xxx)))
      ...
      ini=0
      do i=1,nmb
         call st_tch_pwhgGETTAG(i,tag)
         if(tag.eq.'YST') then
            call st_tch_PWHGFINAL(i+nmh3)
            call st_tch_PWHGFINAL(i+nmh4)
            call st_tch_pwhggettitle(i,title0)
            call st_tch_pwhggettitle(i+nmh4,scale)
            call st_tch_PWHGMULTITOP(i+nmh3,i+nmh4,2,3,title0,' ',scale(2:4)
     $           ,ini)
         endif
      enddo
      end            
      ...
      subroutine st_tch_pwhgrescale(scale)
C********************************************************************
...
C st_tch_analysis has been performed and before histograms are written to the
C TOPDRAWER file. For example : 
C    call st_tch_pwhgsetout ( or st_tch_pwhgaddout)
C    call st_tch_pwhgrescale(scale)
C    call st_tch_pwhgtopout 
C**********************************************************************
      implicit none
      include 'src/Lib/st_tch/pwhg_book.h'
      ...
      do i=1,nmb
         call st_tch_pwhgGETTAG(i,tag)
         if(tag.eq.'YST') then
C st_tch_PWHGOPERA(I,'F',J,K,X,Y) multiplies hist I by the factor X, and puts
C the result in hist K;
            call st_tch_PWHGOPERA(i+nmh3,'F',dummy_int,i+nmh3,scale,dummy_real)
C i+nmh3 contains the st_tch_values 
            call st_tch_PWHGOPERA(i+nmh4,'F',dummy_int,i+nmh4,scale,dummy_real)

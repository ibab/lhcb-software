      SUBROUTINE st_tch_Sgu_epvebdbx_S(P1,ANS)
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2gu_epvebdbx_S(maxamps), jamp2gu_epvebdbx_S(0:maxamps)
      common/st_tch_to_amps_gu_epvebdbx_S/  amp2gu_epvebdbx_S,       jamp2gu_epvebdbx_S
      ...
      common/st_tch_to_matrix_gu_epvebdbx_S/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2gu_epvebdbx_S(ihel)=0d0
              jamp2gu_epvebdbx_S(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2gu_epvebdbx_S(0))
              jamp2gu_epvebdbx_S(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_tch_gu_epvebdbx_S(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2gu_epvebdbx_S(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2gu_epvebdbx_S(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_tch_gu_epvebdbx_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_tch_fvoxxx(W(1,9   ),W(1,1   ),GG ,TMASS   ,TWIDTH  ,W(1,14  ))     
      ...
      JAMP(   1) = +AMP(   1)+AMP(   4)
      JAMP(   2) = (0d0,0d0)!ER:+AMP(   2)+AMP(   3)
      st_tch_gu_epvebdbx_S = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_tch_gu_epvebdbx_S =st_tch_gu_epvebdbx_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2gu_epvebdbx_S(i)=amp2gu_epvebdbx_S(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2gu_epvebdbx_S(i)=Jamp2gu_epvebdbx_S(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

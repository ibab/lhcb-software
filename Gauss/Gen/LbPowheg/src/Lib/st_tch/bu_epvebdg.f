      SUBROUTINE st_tch_Sbu_epvebdg(P1,ANS)
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2bu_epvebdg(maxamps), jamp2bu_epvebdg(0:maxamps)
      common/st_tch_to_amps_bu_epvebdg/  amp2bu_epvebdg,       jamp2bu_epvebdg
      ...
      common/st_tch_to_matrix_bu_epvebdg/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2bu_epvebdg(ihel)=0d0
              jamp2bu_epvebdg(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2bu_epvebdg(0))
              jamp2bu_epvebdg(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_tch_bu_epvebdg(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2bu_epvebdg(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2bu_epvebdg(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_tch_bu_epvebdg(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL st_tch_ixxxxx(P(0,2   ),ZERO ,NHEL(2   ),+1*IC(2   ),W(1,2   ))        
      ...
      CALL st_tch_iovxxx(W(1,2   ),W(1,6   ),W(1,11  ),GWF ,AMP(1   ))            
      CALL st_tch_fvixxx(W(1,2   ),W(1,7   ),GG ,ZERO    ,ZERO    ,W(1,12  ))     
      CALL st_tch_jioxxx(W(1,1   ),W(1,9   ),GWF ,WMASS   ,WWIDTH  ,W(1,13  ))    
      CALL st_tch_iovxxx(W(1,12  ),W(1,6   ),W(1,13  ),GWF ,AMP(2   ))            
      ...
      CALL st_tch_iovxxx(W(1,2   ),W(1,6   ),W(1,16  ),GWF ,AMP(3   ))            
      CALL st_tch_fvixxx(W(1,2   ),W(1,13  ),GWF ,ZERO    ,ZERO    ,W(1,17  ))    
...
!ER:      CALL st_tch_fvoxxx(W(1,9   ),W(1,7   ),GG ,TMASS   ,TWIDTH  ,W(1,18  ))     
      ...
      CALL st_tch_iovxxx(W(1,2   ),W(1,6   ),W(1,19  ),GWF ,AMP(5   ))            
c     !ER: removed gluon emission from decay leg
c      JAMP(   1) = -AMP(   1)-AMP(   3)-AMP(   5)
      JAMP(   1) = -AMP(   1)-AMP(   5)
      JAMP(   2) = -AMP(   2)-AMP(   4)
      st_tch_bu_epvebdg = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_tch_bu_epvebdg =st_tch_bu_epvebdg+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2bu_epvebdg(i)=amp2bu_epvebdg(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2bu_epvebdg(i)=Jamp2bu_epvebdg(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

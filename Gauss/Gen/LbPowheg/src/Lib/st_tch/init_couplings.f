      subroutine st_tch_init_couplings
      implicit none
...
c$$$      include 'src/Lib/st_tch/pwhg_par.h'
      logical verbose
      parameter(verbose=.true.)
      integer aemrun
      real *8 st_tch_powheginput
      external st_tch_powheginput
      real *8 st_tch_alfaem,st_tch_pwhg_alphas,rdummy,totbr
      external st_tch_alfaem,st_tch_pwhg_alphas
      integer i,j,idummy
      real *8 alphaem_inv
      common/st_tch_calphaem_inv/alphaem_inv
      integer decayCKM
...
c     branching ratio (used only in LH event file)
      if(st_tch_powheginput('topdecaymode').eq.0) then
         write(*,*) 'No decay products will be generated'
         totbr=1d0
      else
         write(*,*) 'Top decay products will be generated'
         call st_tch_pickwdecay(-1000,rdummy,idummy,rdummy,totbr)
      endif
      rad_branching=totbr

c     top mass
      topmass_pow=st_tch_powheginput('topmass')
...
c     W mass
      wmass_pow=st_tch_powheginput('#wmass')
      if(wmass_pow.lt.0) wmass_pow=80.4d0

c     sthw2
      sthw2_pow=st_tch_powheginput('#sthw2')
      if(sthw2_pow.lt.0) sthw2_pow=0.23113d0

c     alphaem
c     typical st_tch_values for alphaem:
...
c     definition of alphaem_pow value, according to aemrun
      if(aemrun.eq.0) then
         alphaem_inv=st_tch_powheginput('#alphaem_inv')
         if(alphaem_inv.lt.0) alphaem_inv=127.011989
         alphaem_pow=1d0/alphaem_inv
         zmass_pow=91.188d0     !Not relevant in POWHEG; needed only by st_tch_set_madgraph_parameters
      elseif(aemrun.eq.1) then
         write(*,*) 'Invalid option for aemrun: program stops'
         call exit(1)
c$$$c     alphaem_pow is evaluated at the top mass value using the st_tch_alfaem function.
c$$$c     In this case zmass is needed by the st_tch_alfaem function to set a reference
c$$$c     point for the running of alphaem. 
c$$$c     zmass needed also by set_madgraph_parameters.
c$$$c     This reference value is read and used by the function st_tch_alfaem itself
c$$$c     that will assume st_tch_alfaem(zmass)=1/alphaem_inv.
c$$$         alphaem_inv=127.934
c$$$         zmass_pow=92d0
c$$$         alphaem_pow=st_tch_alfaem(topmass_pow**2)
      else
...
c     CKM matrix entries
      CKM_pow(1,1)= st_tch_powheginput('#CKM_Vud')
      CKM_pow(1,2)= st_tch_powheginput('#CKM_Vus') 
      CKM_pow(1,3)= st_tch_powheginput('#CKM_Vub') 
      CKM_pow(2,1)= st_tch_powheginput('#CKM_Vcd') 
      CKM_pow(2,2)= st_tch_powheginput('#CKM_Vcs') 
      CKM_pow(2,3)= st_tch_powheginput('#CKM_Vcb') 
      CKM_pow(3,1)= st_tch_powheginput('#CKM_Vtd') 
      CKM_pow(3,2)= st_tch_powheginput('#CKM_Vts') 
      CKM_pow(3,3)= st_tch_powheginput('#CKM_Vtb') 
...
c      decayCKM=st_tch_powheginput('#decayCKM')
      decayCKM=0
      if(decayCKM.eq.1) then
         write(*,*) '*************** WARNING ****************'
         write(*,*) 'Using special st_tch_values for CKM matrix '
         write(*,*) '        in the decay process'
         write(*,*) '  THIS IS NOT A STANDARD OPTION'
         write(*,*) 'BE SURE YOU KNOW WHAT YOU ARE DOING'
         write(*,*) '***********************************'
         dCKM_pow(1,1)= st_tch_powheginput('dCKM_Vud')
         dCKM_pow(1,2)= st_tch_powheginput('dCKM_Vus') 
         dCKM_pow(1,3)= st_tch_powheginput('dCKM_Vub') 
         dCKM_pow(2,1)= st_tch_powheginput('dCKM_Vcd') 
         dCKM_pow(2,2)= st_tch_powheginput('dCKM_Vcs') 
         dCKM_pow(2,3)= st_tch_powheginput('dCKM_Vcb') 
         dCKM_pow(3,1)= st_tch_powheginput('dCKM_Vtd') 
         dCKM_pow(3,2)= st_tch_powheginput('dCKM_Vts') 
         dCKM_pow(3,3)= st_tch_powheginput('dCKM_Vtb') 
...
c     W width (only for decay)
      wwidth_pow=st_tch_powheginput('#wwidth')
      if(wwidth_pow.lt.0) wwidth_pow=2.141d0

c     top width (only for decay)
      topwidth_pow=st_tch_powheginput('#topwidth')
...
c     setting madgraph parameters (needed for madgraph subroutines)
      call st_tch_set_madgraph_parameters
     ...
     $,st_tch_pwhg_alphas(91.2d0**2,st_lambda5MSB,st_nlight)
         write(*,'(1X,A,f7.3,A,f15.7)') 'alpha_s(',topmass_pow,')'
     $,st_tch_pwhg_alphas(topmass_pow**2,st_lambda5MSB,st_nlight)
         write(*,*)
...
c logs of sqrt(q2) at m_e=0.000511,m_mu=0.1056,m_tau=1.777      
      ...
      if(q2.lt.exp(2.*xlogmu(1))) then
         st_tch_alfaem = 1.d0/ooa(1)	 
      elseif(q2.lt.exp(2.*xlogmu(2))) then
         xlq = log(q2)/2.d0
         st_tch_alfaem = 1.d0/ooa(2)
      elseif(q2.lt.exp(2.*xlogmu(3))) then
         xlq = log(q2)/2.d0
         st_tch_alfaem = 1.d0/ooa(3)
      elseif(q2.lt.5.**2) then
         b = 3 + 2*nc*(1d0/3d0)**2 + 2*nc*(2d0/3d0)**2
         xlq = log(q2) - 2.*xlogmu(3)
         st_tch_alfaem = 1d0/ooa(3)/(1.d0 - 1.d0/3.d0/pi/ooa(3)*b*xlq)
      else
         b = 3 + 3*nc*(1d0/3d0)**2 + 2*nc*(2d0/3d0)**2
         xlq = log(q2/zm**2)
         st_tch_alfaem = 1d0/ooaz/(1.d0 - 1.d0/3.d0/pi/ooaz*b*xlq)
      endif
      return
      end
...
c     setting of MADGRAPH inputs
      subroutine st_tch_set_madgraph_parameters
      include 'src/Lib/st_tch/PhysPars.h'
      include 'src/Lib/st_tch/pwhg_math.h'
...
ccccccccccccccccccccccccccccccc
      ...
      write(*,*) 'POWHEG: st_tch_set_madgraph_parameters called'
...
c     for wm (see st_tch_my_setpara subroutine).
      www=zmass*sqrt(1-sthw2_pow)
      gfermi=pi*zmass**2*alpha/sqrt(2.)
      gfermi=gfermi/(zmass**2*www**2 - www**4)

c     setting of other remaining couplings is done by st_tch_my_setpara on
...
ccccccccccccccccccccccccccccccccccccccccccccc
c     see subroutine st_tch_my_setpara

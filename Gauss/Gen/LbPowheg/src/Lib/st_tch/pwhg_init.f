      ...
      integer i1,n1,n2
      call st_tch_init_flsttag
      flg_debug=.false.
      if(st_tch_powheginput("#flg_debug").eq.1) flg_debug=.true.
c whether to output negative weights or not
      flg_withnegweights=.false.
      if(st_tch_powheginput("#withnegweights").eq.1)flg_withnegweights=.true.
c See if we have weighted events
      flg_weightedev=.false.
      if(st_tch_powheginput("#bornsuppfact").gt.0) flg_weightedev=.true.
      if(st_tch_powheginput("#ptsupp").gt.0) then
         write(*,*) ' ******** WARNING: ptsupp is deprecated'
         write(*,*) ' ******** Replace it with bornsuppfact'
         call flush
         call exit(-1)
      endif
c     Set to true to remember and use identical st_tch_values of the computed 
c     amplitudes, for Born, real and virtual contributions
      flg_smartsig=.true.
      if(st_tch_powheginput("#smartsig").eq.0) flg_smartsig=.false.
c     if true also counterterm are output in NLO tests (default)
      flg_withsubtr=.true.
      if(st_tch_powheginput("#withsubtr").eq.0) flg_withsubtr=.false.
...
c     is required
      flg_withdamp=.false.
      if((st_tch_powheginput("#withdamp").eq.1).or.
     $  (st_tch_powheginput("#hfact").gt.0d0))   flg_withdamp=.true.
c     If set do only the Born term
      flg_bornonly=.false.
      if (st_tch_powheginput("#bornonly").eq.1) flg_bornonly=.true.
c     initialize st_tch_random number sequence
      i1=st_tch_powheginput('#iseed')
      if (i1.lt.0) i1=0
      n1=st_tch_powheginput('#rand1')
      if (n1.lt.0) n1=0
      n2=st_tch_powheginput('#rand2')
      if (n2.lt.0) n2=0
c select which upper bounding function form
      rad_iupperisr=st_tch_powheginput("#iupperisr")
      if(rad_iupperisr.lt.0) rad_iupperisr=1
      rad_iupperfsr=st_tch_powheginput("#iupperfsr")
      if(rad_iupperfsr.lt.0) rad_iupperfsr=2
c info on pdf for each event
      flg_pdfreweight=.false.
      if(st_tch_powheginput("#pdfreweight").eq.1)flg_pdfreweight=.true.
c
      call st_tch_setrandom(i1,n1,n2)
...
c     inside the user-defined subroutine st_tch_init_processes
      lprup(1)=10001
c     initialize physical parameters
      call st_tch_init_phys
...
c now the cross section is available
      ...
      subroutine st_tch_init_flsttag
      include 'src/Lib/st_tch/nlegborn.h'
      include 'src/Lib/st_tch/pwhg_flst.h'
      integer j,l
      do j=1,maxprocreal

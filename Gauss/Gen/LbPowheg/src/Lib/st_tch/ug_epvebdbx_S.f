      SUBROUTINE st_tch_Sug_epvebdbx_S(P1,ANS)
C  
...
C GLOBAL VARIABLES
C  
      Double Precision amp2ug_epvebdbx_S(maxamps), jamp2ug_epvebdbx_S(0:maxamps)
      common/st_tch_to_amps_ug_epvebdbx_S/  amp2ug_epvebdbx_S,       jamp2ug_epvebdbx_S
      ...
      common/st_tch_to_matrix_ug_epvebdbx_S/isum_hel, multi_channel
...
C ----------
      ...
      IF (.false.) THEN
          DO IHEL=1,NGRAPHS
              amp2ug_epvebdbx_S(ihel)=0d0
              jamp2ug_epvebdbx_S(ihel)=0d0
          ENDDO
          DO IHEL=1,int(jamp2ug_epvebdbx_S(0))
              jamp2ug_epvebdbx_S(ihel)=0d0
          ENDDO
      ENDIF
      ANS(IPROC) = 0D0
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=st_tch_ug_epvebdbx_S(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     $             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      IF (.false.) THEN
          ...
          DO IHEL=1,MAPCONFIG(0)
              XTOT=XTOT+amp2ug_epvebdbx_S(MAPCONFIG(IHEL))
          ENDDO
          IF (XTOT.NE.0D0) THEN
              ANS(IPROC)=ANS(IPROC)*amp2ug_epvebdbx_S(MAPCONFIG(ICONFIG))/XTOT
          ELSE
              ANS(IPROC)=0D0
          ENDIF
      ENDIF
      ...
      REAL*8 FUNCTION st_tch_ug_epvebdbx_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      JAMP(   2) = +AMP(   3)+AMP(   4)
      st_tch_ug_epvebdbx_S = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          st_tch_ug_epvebdbx_S =st_tch_ug_epvebdbx_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2ug_epvebdbx_S(i)=amp2ug_epvebdbx_S(i)+amp(i)*dconjg(amp(i))
      Enddo
      Do I = 1, NCOLOR
          Jamp2ug_epvebdbx_S(i)=Jamp2ug_epvebdbx_S(i)+Jamp(i)*dconjg(Jamp(i))
      Enddo

...
c...writes initialization information to a les houches events file on unit nlf. 
      subroutine ww_lhefwritehdr(nlf)
      implicit none
     ...
     #     version
      ...
      call ww_wrtpowheginput(nlf)
      write(nlf,'(a)') 'End of powheg.input content'
      call ww_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) 'ww_random number generator initialized with: ',
     # iran,' ',n1ran,' ',n2ran
      write(nlf,'(a)') '-->'
      write(nlf,'(a)') '<init>'
 ...
 120  format(1p,3(1x,e12.5),1x,i6)
      ...
      subroutine ww_printleshouches
c useful for debugging
      call ww_lhefwritev(6)
      end

c...lhefeader(nlf)
c...writes event information to a les houches events file on unit nlf. 
      subroutine ww_lhefwritev(nlf)
      implicit none
      integer nlf
      include 'src/Lib/ww/LesHouches.h'
      include 'src/Lib/ww/pwhg_flg.h'
     ...
     & vtimup(i),spinup(i)
 200  continue
      if(flg_pdfreweight) call ww_lhefwritepdfrw(nlf)
      if(flg_debug) call ww_lhefwritextra(nlf)
      write(nlf,'(a)')'</event>'      
...
c...writes trailer to a les houches events file on unit nlf. 
      subroutine ww_lhefwritetrailer(nlf)
      implicit none
      integer nlf,iran,n1ran,n2ran
      write(nlf,'(a)') '</LesHouchesEvents>'
c     save last ww_random number
      call ww_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) '#ww_random number generator exit values: ',
     # iran,' ',n1ran,' ',n2ran
      end

      subroutine ww_lhefwritextra(nlf)
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
      ...
      call ww_getcurrentrandom(iran,n1ran,n2ran)
      write(nlf,*) '# ', iran,' ',n1ran,' ',n2ran,
     #     "    previous event's ww_random seeds "
      write(nlf,'(a)') '# End extra-info-previous-event'
      end

      subroutine ww_lhefwritepdfrw(nlf)
      implicit none
      integer nlf
      integer id1,id2
      real * 8 x1,x2,xf1,xf2,xmufact
      call ww_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)

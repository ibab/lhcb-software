      subroutine ww_btildereal(xrad,resreal,www)
      implicit none
...
c output values for ww_analysis_driver
         out0=0
         out1=0
c check that emitter is valid
         if(ww_valid_emitter(kn_emitter)) then
            if(kn_emitter.gt.2) then
c     final state radiation
               call ww_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call ww_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call ww_collfsr(rc)
c     ww_soft subtraction
                  call ww_soft(r0s)
                  call ww_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call ww_collisrp(rp)
                     call ww_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call ww_collisrm(rm)
                     call ww_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call ww_analysis_driver(out0,0)
            if(out1.ne.0d0) call ww_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine ww_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
      include 'src/Lib/ww/pwhg_kn.h'
      include 'src/Lib/ww/pwhg_dbg.h'
      external ww_sigreal_btl,ww_soft,ww_collfsr,ww_softcollfsr, ww_collisrp,
     $     ww_softcollisrp,ww_collisrm,ww_softcollisrm
      call ww_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  ww_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call ww_checksoft(ww_sigreal_btl,ww_soft,' ww_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call ww_checkcoll(ww_sigreal_btl,ww_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
c            call ww_randomsave
            if(kn_emitter.ne.2) call ww_checkcoll(ww_sigreal_btl,ww_collisrp,1
     $           ,' coll-plus',iun)
c            call ww_randomrestore
            if(kn_emitter.ne.1) call ww_checkcoll(ww_sigreal_btl,ww_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  ww_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call ww_checksoft(ww_collfsr,ww_softcollfsr,' ww_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  ww_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call ww_checksoft(ww_collisrp,ww_softcollisrp,
     $           ' ww_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call ww_checksoft(ww_collisrm,ww_softcollisrm,
     $           ' ww_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-ww_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call ww_checkcoll(ww_soft,ww_softcollfsr,1,' coll-ww_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-ww_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call ww_checkcoll(ww_soft,ww_softcollisrp,1
     $           ,' coll-plus-ww_soft',iun)
            if(kn_emitter.ne.1) call ww_checkcoll(ww_soft,ww_softcollisrm,-1
     $           ,' coll-minus-ww_soft',iun)
         enddo
      endif
      call ww_randomrestore
      end

      subroutine ww_checkborn(iun)
...
c are consistent with total Born
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine ww_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/ww/pwhg_dbg.h'
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 ww_random,ww_dotp
      external ww_random,ww_dotp
      logical ww_valid_emitter,iszero,isnonzero,isequal
      external ww_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=ww_random()
      enddo
      call ww_gen_born_phsp(xborn)
      call ww_setscalesbtilde
      call ww_allborn
      call ww_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*ww_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=ww_random()
      enddo
c Check ww_soft limits
      if(ww_valid_emitter(kn_emitter)) then
         write(iun,*) ' ww_random Born variables ====> ',xborn
          write(iun,*) ' ww_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call ww_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call ww_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check ww_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine ww_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/ww/pwhg_dbg.h'
      include 'src/Lib/ww/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' ww_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      if(ini) then
         ...
         if(flg_smartsig) then
            call ww_randomsave
c     generate "nmomset" ww_random real-phase space configurations
            call ww_fillmomenta(nlegreal,nmomset,kn_masses,preal)
            do alr=1,flst_nalr
               do j=1,nmomset
                  call ww_realgr(
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call ww_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      do alr=1,flst_nalr
         r0(alr)=0
      enddo
      call ww_pdfcall(1,kn_x1,pdf1)
      call ww_pdfcall(2,kn_x2,pdf2)
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call ww_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call ww_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine ww_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call ww_sigreal_btl0(r0,0)
      end

c Real cross section, required by ww_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine ww_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
      include 'src/Lib/ww/pwhg_kn.h'
...
c     generate "nmomset" ww_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call ww_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c First mark as being computed
               markused(alr)=1
               call ww_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' ww_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: ww_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine ww_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=ww_random()
            enddo
         enddo
         preal(3,nlegreal,j)=ww_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine ww_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from ww_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine ww_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call ww_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'ww_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine ww_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
      include 'src/Lib/ww/pwhg_math.h'
      ...
      call ww_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.ww_pwhg_isfinite(amp2)) amp2=0d0

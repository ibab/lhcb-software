c The subroutine ww_lhefinitemasses performs momentum reshuffling, giving
...
c Else (i is not an ancestor of moth) iret=0;
      subroutine ww_lhfm_sonof(i,moth,iret)
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer i,moth,iret
      integer icurr
...
c The program terminates when it does not find any more subsets to reshuffle.
      subroutine ww_lhefinitemasses
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer i,j,moth,iret
      integer n,arr(maxnup)
      common/ww_clhfm_push/arr,n
c      call ww_lhefwritev(6)
      call ww_lhfm_checkmom
      call ww_lhfm_setmasses
      call ww_lhfm_addfakeinires
c initialize empty stack to remember resonances
      call ww_lhfm_pushmoth(1,0)
 1    continue
      do i=3,nup
c initialize empty stack
         call ww_lhfm_push(i,0)
         if(istup(i).eq.1) then
            moth=mothup(1,i)
            if(istup(moth).eq.2) then
c push i on stack
               call ww_lhfm_push(i,1)
               do j=i+1,nup
                  if(istup(j).eq.1) then
                     call ww_lhfm_sonof(j,moth,iret)
                     if(iret.eq.1) then
                        call ww_lhfm_push(j,1)
...
c are there any particles in the stack?
      call ww_lhfm_push(i,2)
c if not it's all done
      if(i.eq.0) goto 99
      call ww_lhfm_reshuffle(iret)
      if(iret.eq.-1) then
...
c increase nup, this fake resonance will never be reshuffled.
      subroutine ww_lhfm_addfakeinires
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer i,mu,nfk
...
c reset correct status for all particles
      subroutine ww_lhfm_resetstatus 
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer n,arr(maxnup)
      common/ww_clhfm_pushmoth/arr,n
      integer j
      do j=3,nup
...
c imode=2: set i to current length of stack
      subroutine ww_lhfm_push(i,imode)
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer i,imode
      integer n,arr(maxnup)
...
c imode=0: initialize stack
c imode=1: push value in stack
      subroutine ww_lhfm_pushmoth(i,imode)
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer i,imode
      integer n,arr(maxnup)
      common/ww_clhfm_pushmoth/arr,n
      save /ww_clhfm_pushmoth/
      if(imode.eq.0) then
         n=0
      else
         n=n+1
...
c reshuffle momenta for the n particles in common/ww_clhfm_push/arr,n;
...
c are performed again in reverse.  
      subroutine ww_lhfm_reshuffle(iret)
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer iret
...
c ww_boost all momenta in arr (and its sons)
c to the resonance rest frame
      call ww_lhfm_boostz(-betaz)
      beta(1)=-pup(1,moth)/pup(4,moth)
      beta(2)=-pup(2,moth)/pup(4,moth)
      call ww_lhfm_boostperp(beta)
c rescale momenta
...
c for a given scaling factor sc.
 10   call ww_lhfm_totalen(sc,en,diffen,diff2en)
      de=pup(5,moth)-en
...
c     find appropriate ww_boost for its sons;
         call ww_lhfm_reboost(ipart,sc)
      enddo
c      call checkmomres
      beta(1)=-beta(1)
      beta(2)=-beta(2)
...
c check momentum balance in array arr,n
      subroutine ww_lhfm_checkmomres
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      real * 8 p(4),diff
      integer moth,mu,j
      ...
      subroutine ww_lhfm_reboost(moth,sc)
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer moth
      real * 8 sc
      ...
      do ipart=3,nup
         if(istup(ipart).eq.1001) then
            call ww_lhfm_sonof(ipart,moth,iret)
            if(iret.ne.0) then
               if(ini) then
...
c only for direct sons
      ...
      subroutine ww_lhfm_totalen(s,en,diffen,diff2en)
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      ...
      subroutine ww_lhfm_boostz(beta)
...
c     beta.  Lorents convention: (ww_t,x,y,z).
      implicit none
      include 'src/Lib/ww/LesHouches.h'
      integer moth,iret
     ...
     2           +gamma*beta*pup(4,ipart))
      ...
      subroutine ww_lhfm_boostperp(betav)
c     boosts the m vectors vin(0:3,m) into the vectors vout(0:3,m) (that can
c     be the same) in the direction of vec(3) (|vec|=1) with velocity
c     beta.  Lorents convention: (ww_t,x,y,z).
      implicit none
     ...
     2           +gamma*beta*pup(4,ipart))
      ...
      do moth=3,nup
         if(istup(moth).eq.2) then
            ...
            enddo
            do i=3,nup
               call ww_lhfm_sonof(i,moth,iret)
               if(iret.eq.1) then
                  do mu=1,4
                     total(mu)=total(mu)-pup(mu,i)
                  enddo

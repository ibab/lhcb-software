      subroutine ww_setborn(p,bflav,born,bornjk,bmunu)
      implicit none
      ...
      call ww_compborn(p,bflav,born,bmunu,bornjk)
      end


      subroutine ww_compborn(pin,bflav,born,bmunu,bornjk)
      implicit none
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
...
c     vector boson id and decay
      ...
      common/ww_cvecbos/idvecbos,vdecaymode
...
c idup(3)+idup(4) is 0 for Z, 1 for W+, -1 for W-
      if(idup(3)+idup(4).eq.0) then
         call ww_add_resonance(23,3,4)
      else
         call ww_add_resonance(24*(idup(3)+idup(4)),3,4)
      endif
c Careful now! 5 6 have been bumped to 6 7 by the
c resonance inserted above!
      if(idup(6)+idup(7).eq.0) then
         call ww_add_resonance(23,6,7)
      else
         call ww_add_resonance(24*(idup(6)+idup(7)),6,7)
      endif
...
c So we should now exchange 5 and 8
      ...
      if(i1.ne.0.and.ww_random().gt.0.5d0) then

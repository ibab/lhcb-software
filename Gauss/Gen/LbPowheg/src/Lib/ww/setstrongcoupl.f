      subroutine ww_setscalesbtilde
      implicit none
      include 'src/Lib/ww/pwhg_math.h'
      include 'src/Lib/ww/pwhg_st.h'
      real * 8 ww_pwhg_alphas
      external ww_pwhg_alphas
      real * 8 muf,mur
      call ww_set_fac_ren_scales(muf,mur)
      st_mufact2= muf**2*st_facfact**2
      st_muren2 = mur**2*st_renfact**2
      st_alpha  = ww_pwhg_alphas(st_muren2,st_lambda5MSB,st_nlight)
      end

      subroutine ww_set_rad_scales(ptsq)
      implicit none
      real * 8 ptsq
      include 'src/Lib/ww/pwhg_math.h'
...
c     SAER FIX
      real * 8 q2min
      character * 3 ww_whichpdfpk
      external ww_whichpdfpk
      integer ini,mem
      data ini/0/
      save ini,q2min
      
      if (ini.eq.0) then
         if( ww_whichpdfpk().eq.'lha') then    
           mem = 0
           call ww_getq2min(mem,q2min)
...
c     is greater or equal to the mass of heavy quark 
        elseif( ww_whichpdfpk().eq.'mlm') then    
c ad hoc value here (ww_MLMPDF does not provide this)
           q2min=2d0
        else
           write(*,*) ' unimplemented pdf package',ww_whichpdfpk()
           stop
        endif 
        ini=1
      endif
...
c     mufact (50 is an arbitrary choice).
      ...
      st_muren2=ptsq
      st_alpha = ww_pwhg_alphas(st_muren2,st_lambda5MSB,-1)
...
c for better NLL accuracy (FNO2006, (4.32) and corresponding references)
      ...
      function ww_pwhg_alphas0(q2,xlam,inf)
      implicit none
      real * 8 ww_pwhg_alphas0,q2,xlam
      integer inf
      real * 8 pi
      parameter (pi=3.141592653589793d0)
      real * 8 b0
      b0=(33-2*inf)/(12*pi)
      ww_pwhg_alphas0=1/(b0*log(q2/xlam**2))
      end
      
C----------------------------------------------------------------------------
C-------------------------------------------------------------------
C------- ALPHA QCD -------------------------------------
c Program to calculate ww_alfa strong with nf flavours,
c as a function of lambda with 5 flavors.
c The value of ww_alfa is matched at the thresholds q = mq.
c When invoked with nf < 0 it chooses nf as the number of
c flavors with mass less then q.
c
      function ww_pwhg_alphas(q2,xlam,inf)
      implicit none
      include 'src/Lib/ww/nlegborn.h'
      include 'src/Lib/ww/pwhg_flst.h'
      include 'src/Lib/ww/pwhg_rad.h'
      real * 8 ww_pwhg_alphas,q2,xlam
      integer inf
      real * 8 pi
      parameter (pi=3.141592653589793d0)
...
c      data ini/.true./
c      save ini
c      ww_pwhg_alphas = 30d0
c      if (ini) then
c         write(*,*) '****************************************'
c         write(*,*) '****************************************'
c         write(*,*) '      RETURN alpha_s = ',ww_pwhg_alphas
c         write(*,*) '****************************************'
c         write(*,*) '****************************************'
c         ini = .false.         
c      endif           
c      return
     ...
     #        - 1/( 1/(b3 * xlc) - xllc*bp3/(b3 * xlc)**2 ) + c45
      ...
      if    ( nf .eq. 5 ) then
        ww_pwhg_alphas = 1/(b5 * xlq) -  bp5/(b5 * xlq)**2 * xllq
      elseif( nf .eq. 4 ) then
        ww_pwhg_alphas =
     ...
     #    1/( 1/(1/(b3 * xlq) - bp3/(b3 * xlq)**2 * xllq) + c35 )
        ...
        print *,'error in ww_alfa: unimplemented # of light flavours',nf
        call exit(1)
      endif
      return
      end

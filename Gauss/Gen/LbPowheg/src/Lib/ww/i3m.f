...
C     This is the function ww_I3m, a massless triangle with all three external 
...
c     SLAC-PUB-5809
      ...
      ww_I3m1a=two/rtmdel*(ww_DCLAUS(arg1)+ww_DCLAUS(arg2)+ww_DCLAUS(arg3))
...
C  %%CITATION = HEP-PH 9402223;%%
      ...
      include 'src/Lib/ww/constants.h' 
      double precision s1,s2,s3,d3,temp,ww_DDILOG,ww_xlog,ylog,rat
      ...
      ww_xlog=log(abs(argx))
      ylog=log(abs(argy))
      temp=ww_xlog*ylog+pisq/3d0+(ylog-ww_xlog)*log((one+argy)/(one+argx))
     & +two*(ww_DDILOG(argdlx)+ww_DDILOG(argdly))
      ww_I3m1b=Dcmplx(temp-abs(flag)*pisq)+impi*Dcmplx(flag*(ww_xlog+ylog))

      ...
      external ww_whichpdfpk,ww_powheginput
...
c End initialization of common block defaults.
      pdf_ih1=ww_powheginput('ih1')
      pdf_ih2=ww_powheginput('ih2')
      if(ww_whichpdfpk().eq.'lha') then
         pdf_ndns1=ww_powheginput('lhans1')
         pdf_ndns2=ww_powheginput('lhans2')
      elseif(ww_whichpdfpk().eq.'mlm') then
         pdf_ndns1=ww_powheginput('ndns1')
         pdf_ndns2=ww_powheginput('ndns2')
      else
         write(*,*) ' unimplemented pdf package',ww_whichpdfpk()
         stop
      endif
      if(pdf_ndns1.ne.pdf_ndns2) then
         st_lambda5MSB=ww_powheginput('QCDLambda5')
      else
         call ww_genericpdfpar(pdf_ndns1,pdf_ih1,st_lambda5MSB,
     1                      scheme,iorder,iret)
      ...
      kn_beams(0,1)=ww_powheginput('ebeam1')
      kn_beams(0,2)=ww_powheginput('ebeam2')
      kn_beams(1,1)=0
      kn_beams(1,2)=0
      kn_beams(2,1)=0
      kn_beams(2,2)=0
...
c generation cut: see ww_gen_born_phsp.f
      kn_ktmin=ww_powheginput("#bornktmin")
...
c thresholds 
      rad_ptsqmin=ww_powheginput('#ptsqmin')
      if(rad_ptsqmin.lt.0) rad_ptsqmin=0.8d0
      rad_charmthr2=ww_powheginput('#charmthr')
      if(rad_charmthr2.lt.0) rad_charmthr2=1.5d0
      rad_charmthr2=rad_charmthr2**2
      rad_bottomthr2=ww_powheginput('#bottomthr')
      if(rad_bottomthr2.lt.0) rad_bottomthr2=5d0
      rad_bottomthr2=rad_bottomthr2**2
c scale factors
      st_renfact=ww_powheginput('#renscfact')
      st_facfact=ww_powheginput('#facscfact')
...
c initialize Lambda values for radiation
      call ww_init_rad_lambda
c
      call ww_init_processes

      call ww_init_couplings

c initialize number of singular regions
      rad_nkinreg=1+(nlegborn-flst_lightpart+1)
      call ww_genflavreglist


      dbg_softtest=.true.
      dbg_colltest=.true.
      if(flg_withdamp) then
         write(*,*) ' no ww_soft tests if withdamp is set'
         dbg_softtest=.false.
      endif
      if(flg_bornonly) then
         write(*,*) ' no ww_soft and coll. tests if bornonly is set'
         dbg_softtest=.false.
         dbg_colltest=.false.
      endif

      function ww_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 ww_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call ww_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call ww_btildevirt(resvirt)
            call ww_btildecoll(xrad,rescoll,www)
            call ww_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         ww_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            ww_btilde=ww_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to ww_btildeborn and ww_btildevirt can be
...
c     to the NLO ww_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c ww_btildecoll and ww_btildereal take care themselves to invoke the NLO
c ww_analysis if required.
            call ww_btildecoll(xrad,rescoll,www)
            call ww_btildereal(xrad,resreal,www)
         endif
         ww_btilde=0
...
c output Born
            ...
            call ww_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            ww_btilde=ww_btilde+results(j)
c     Transfer all flavour components of ww_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in ww_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine ww_adduptotals(results,n)
      implicit none
      include 'src/Lib/ww/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/ww_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(ww_powheginput('#ubsigmadetails').eq.1) then
         call ww_newunit(iun)

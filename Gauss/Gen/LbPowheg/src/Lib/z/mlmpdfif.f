      subroutine z_genericpdf(ndns,ih,xmu2,x,fx)
c Interface to z_MLMPDF package.
      implicit none
      integer ndns,ih
      ...
      sx=x
      sxmu2=xmu2
      call z_MLMPDF(ndns,ih,sxmu2,sx,sfx,5)
      do j=-5,5
         fx(j)=sfx(j)
      enddo
      fx(1)=sfx(2)
...
c link errors. It is never invoked in the present case.
      subroutine z_getq2min()
      end

      subroutine z_genericpdfpar(ndns,ih,xlam,scheme,iorder,iret)
      implicit none
      integer ndns,ih
      real * 8 xlam
      character * 2 scheme
      integer iret,iorder
      call z_PDFPAR(ndns,ih,xlam,scheme,iret)
c not yet implemented
      iorder=-1
      end

      function z_whichpdfpk()

      ...
      integer iret,iun
      real * 8 suppfact
      real * 8 z_random,z_powheginput
      external z_random,z_powheginput
      integer mcalls,icalls
      real * 8 z_pwhg_pt2,z_pt2max_regular
      external z_pwhg_pt2,z_pt2max_regular
      integer i
      if(idwtup.eq.3) then
         xwgtup=1
...
c     store current z_random seeds. To be used to restart at problematic events
      call z_savecurrentrandom
      if(z_random().gt.rad_totrm/rad_totgen) then
c     generate underlying Born kinematics
         call z_gen_btilde(mcalls,icalls)
c     generate underlying Born flavour
         call z_gen_uborn_idx
c
         if(z_powheginput('#z_testsuda').eq.1) then
            call z_testsuda
         endif
c generate radiation
         call z_gen_radiation
c add a z_random azimuthal rotation around beam axis
         call z_add_azimuth
c --- set up les houches interface
         rad_pt2max=z_pwhg_pt2()
         call z_gen_leshouches
c if negative weight, flip the sign of xwgtup
         if(rad_btilde_sign(rad_ubornidx).eq.-1) then
            xwgtup=-xwgtup
         endif
c rad_type=1 for z_btilde events (used only for debugging purposes)
         rad_type=1
         if(flg_weightedev) then
            call z_born_suppression(suppfact)
...
c iret=2: reg contribution (real graphs without singular regions)
         call z_gen_remnant(iret)
c         if (z_pwhg_pt2().lt.rad_ptsqmin) then
c            write(*,*) '****************************************'
c            write(*,*) 'WARNING in z_gen_remnant'
c            write(*,*) 'z_pwhg_pt2 < rad_ptsqmin ',
c     #           z_pwhg_pt2(),' < ',rad_ptsqmin
c            write(*,*) (flst_alr(i,rad_realalr),i=1,nlegreal)
c            write(*,*) 'To generate this event, use the following seeds'
c            call z_printcurrentrandom
c            write(*,*) '****************************************'
c         endif
...
c     set st_muren2 equal to pt2 for scalup value
            rad_pt2max=max(rad_ptsqmin,z_pwhg_pt2())
            call z_set_rad_scales(rad_pt2max)
            call z_gen_leshouches
            call z_born_suppression(suppfact)
            if(suppfact.eq.0) then
               write(*,*) ' 0 suppression factor in event generation'
...
c     set st_muren2 for scalup value for regular contributions
            rad_pt2max=max(rad_ptsqmin,z_pt2max_regular())
            call z_set_rad_scales(rad_pt2max)
            call z_gen_leshouches_reg
c rad_type=3 for regular contributions
      ...
      logical ini
      data ini/.true./
      real * 8 z_pwhg_pt2,z_powheginput
      external z_pwhg_pt2,z_powheginput
      save ini,firstreg,lastreg
      if(ini) then
         firstreg=z_powheginput("#radregion")
...
c     initial state radiation
               call z_gen_rad_isr(t)
            else
c     final state radiation
               call z_gen_rad_fsr(t)
            endif
            if(t.gt.tmax) then
               tmax=t
               kinreg=rad_kinreg
...
c Generate a Born like event
         ...
         t=tmax
         if(rad_kinreg.eq.1) then
            call z_gen_real_phsp_isr_rad
         else
            call z_gen_real_phsp_fsr_rad
         endif
         call z_set_rad_scales(t)
         call z_sigreal_rad(sig)
         call z_gen_real_idx
      endif
      end
      ...
      include 'src/Lib/z/pwhg_rad.h'
      if(rad_kinreg.eq.1) then
         z_pwhg_pt2=(kn_sreal/4)*(1-kn_y**2)*kn_csi**2
      else
         z_pwhg_pt2=(kn_sreal/2)*(1-kn_y)*kn_csi**2
      endif
      end



      function z_pwhg_upperb_rad()
      implicit none
      real * 8 z_pwhg_upperb_rad
      include 'src/Lib/z/pwhg_math.h'
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      include 'src/Lib/z/pwhg_kn.h'
      ...
      if(rad_kinreg.eq.1) then
         if(rad_iupperisr.eq.1) then
            z_pwhg_upperb_rad = st_alpha/((1-x)*(1-y**2))
c Possible alternatives:
c rad_iupper=2   z_pwhg_upperb_rad = st_alpha/(x*(1-x)*(1-y**2))
c 
c rad_iupper=3:  z_pwhg_upperb_rad = st_alpha/(x**2*(1-x)*(1-y**2))
         else
...
c for now use the same
         if(rad_iupperfsr.eq.1) then
            z_pwhg_upperb_rad = st_alpha/(csi*(1-y))
         elseif(rad_iupperfsr.eq.2) then
            z_pwhg_upperb_rad = st_alpha/(csi**2*(1-y)*(1-csi/2*(1-y))**2)
     2          *csi
...
c We use it to find its zero in pt2.
      ...
      include 'src/Lib/z/pwhg_rad.h'
      include 'src/Lib/z/pwhg_math.h'
      real * 8 z_pt2solve,pt2
c i set by z_DZERO: 1 for first call, 2 for subsequent calls, 3 for last call
c before a normal exit; not used here
      integer i
      real * 8 xlr,q2,xlam2c,kt2max,cunorm
      integer nlc
      common/z_cpt2solve/xlr,q2,kt2max,xlam2c,cunorm,nlc
...
c see Notes/upperbounding-isr.pdf
     ...
     #           + xlr
               ...
               z_pt2solve=cunorm*pi/b0*(log(2d0)
...
c 
      ...
      integer nlc
      common/z_cpt2solve/xlr,q2,kt2max,xlam2c,unorm,nlc
      real * 8 xmin,rv,xp,xm,chi,tk,uk,ubound,ufct,
     #   sborn,value,err,tmp1,tmp2,tmp,rvalue,born,sig
      common/z_cdfxmin/xmin
      real * 8 tmax
      common/z_ctmax/tmax
      real * 8 z_random,z_pt2solve,dfxmin,z_pwhg_alphas0,z_pwhg_upperb_rad
      external z_random,z_pt2solve,dfxmin,z_pwhg_alphas0,z_pwhg_upperb_rad
...
c see section 4 in ZZ paper, last paragraph
      ...
      xlr=0
 1    continue
      xlr=xlr+log(z_random())
c CERNLIB voodoo:
      call z_KERSET('C205. ',0,0,101)
c solve for zero of z_pt2solve
c z_DZERO(xmin,xmax,x,err,eps,maxcalls,function)
...
c eps: required accuracy
      call z_DZERO(rad_ptsqmin,kt2max,t,err,1d-8,1000000,z_pt2solve)
c error conditions
...
c to set xmuren2:
      call z_set_rad_scales(t)
      tmp2=st_alpha / z_pwhg_alphas0(t,rad_lamll,nlc)
      tmp=tmp1*tmp2
      if(tmp.gt.1) then
...
c using       d sqrt(xm-x) /sqrt(xp-xm) (rad_iupper=1) or d sqrt(xm-x) /(xmin sqrt(xp-xm)) (rad_iupper=2) as upper bound using hit and miss
 2    chi=sqrt(xm-xmin)*z_random()
      x=xm-chi**2
      if(rad_iupperisr.eq.1) then
         if(z_random().gt.sqrt(xp-xm)/sqrt(xp-x)) goto 2
      elseif(rad_iupperisr.eq.2) then
         if(z_random().gt.(xmin*sqrt(xp-xm))/(x*sqrt(xp-x))) goto 2
      endif
c get y (abs to avoid tiny negative values)
      y=sqrt(abs(1-4*x/(1-x)**2*t/sborn))
      if(z_random().gt.0.5d0) y=-y
...
c that some pdf vanish (typically heavy flavour pdf's)
      ...
      kn_y=y
      kn_csi=1-x
      kn_azi=2*pi*z_random()
      ubound=born*z_pwhg_upperb_rad()*unorm*ufct
      call z_gen_real_phsp_isr_rad
      call z_sigreal_rad(sig)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      rvalue=z_random()*ubound
      if(rvalue.gt.value) then
         call z_increasecnt('vetoed radiation')
         goto 1
      endif
      end


      subroutine z_gen_rad_fsr(t)
...
c 
     ...
     #   s,value,err,tmp,rvalue,born,sig
      ...
      real * 8 tmax
      common/z_ctmax/tmax
      real * 8 z_random,z_pt2solve,z_pwhg_alphas0,z_pwhg_upperb_rad
      external z_random,z_pt2solve,z_pwhg_alphas0,z_pwhg_upperb_rad
      unorm=rad_norms(rad_kinreg,rad_ubornidx)
c kn_sborn=kn_sreal:
      s=kn_sborn
...
c vetoes:
      ...
      call z_set_rad_scales(t)
      if(rad_iupperfsr.eq.1) then
         tmp=st_alpha / z_pwhg_alphas0(t,rad_lamll,nlc)
      elseif(rad_iupperfsr.eq.2) then
         tmp=st_alpha
      elseif(rad_iupperfsr.eq.3) then
         tmp=st_alpha
...
c get y
         y=1-2*t/(s*csi**2)
         if(z_random().gt.(csi-t/s)) goto 1
      else
         write(*,*) ' z_gen_rad_fsr:  rad_iupper=',rad_iupperfsr,
     1        ' invalid'
      endif
c
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call z_uboundfct(ufct,csi,y)
      if(z_random().gt.ufct) goto 1
c Veto from upper bound to real value. Count how many vetoes,
c since these may be expensive.
...
c that some pdf vanish (typically heavy flavour pdf's)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      if (kn_csi.eq.0d0) then
         do ileg=1, nlegborn
            call z_mrotate(dir,sazi,cazi,kn_pborn(1,ileg))
         enddo
      else
         do ileg=1, nlegreal
            call z_mrotate(dir,sazi,cazi,kn_preal(1,ileg))
         enddo
      endif
      end

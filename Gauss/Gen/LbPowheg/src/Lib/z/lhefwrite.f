...
c...writes initialization information to a les houches events file on unit nlf. 
     ...
     #     version
      ...
      call z_wrtpowheginput(nlf)
      write(nlf,'(a)') 'End of powheg.input content'
      call z_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) 'z_random number generator initialized with: ',
     # iran,' ',n1ran,' ',n2ran
      write(nlf,'(a)') '-->'
      write(nlf,'(a)') '<init>'
...
c...writes event information to a les houches events file on unit nlf. 
     ...
     & vtimup(i),spinup(i)
 200  continue
      if(flg_pdfreweight) call z_lhefwritepdfrw(nlf)
      if(flg_debug) call z_lhefwritextra(nlf)
      write(nlf,'(a)')'</event>'      
...
c...writes trailer to a les houches events file on unit nlf. 
      subroutine z_lhefwritetrailer(nlf)
      implicit none
      integer nlf,iran,n1ran,n2ran
      write(nlf,'(a)') '</LesHouchesEvents>'
c     save last z_random number
      call z_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) '#z_random number generator exit values: ',
     # iran,' ',n1ran,' ',n2ran
      end

      subroutine z_lhefwritextra(nlf)
      implicit none
      include 'src/Lib/z/LesHouches.h'
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      ...
      call z_getcurrentrandom(iran,n1ran,n2ran)
      write(nlf,*) '# ', iran,' ',n1ran,' ',n2ran,
     #     "    previous event's z_random seeds "
      write(nlf,'(a)') '# End extra-info-previous-event'
      end

      subroutine z_lhefwritepdfrw(nlf)
      implicit none
      integer nlf
      integer id1,id2
      real * 8 x1,x2,xf1,xf2,xmufact
      call z_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)

      subroutine z_btildevirt(resvirt)
      implicit none
     ...
     #     loglog
      ...
      logical ini
      data ini/.true./
      real * 8 z_DDILOG,z_dotp
      external z_DDILOG,z_dotp
      save c,gamma,gammap,ini
      real * 8 tiny
      parameter (tiny=1d-14)
      real * 8 ll
      real * 8 z_Intm_ep,z_Int0m_0,z_Int0m_ep,z_Intmm_0,z_Intmm_ep
      external z_Intm_ep,z_Int0m_0,z_Int0m_ep,z_Intmm_0,z_Intmm_ep
...
c get pdfs
      call z_pdfcall(1,kn_xb1,pdfb1)
      call z_pdfcall(2,kn_xb2,pdfb2)      

      call z_sigvirtual(virt_arr)
...
c     loop on final-state massive partons
     ...
     #              (ll-0.5*z_Intm_ep(kn_cmpborn(0,leg)))               
            endif
         enddo
         Q=Q*br_born(jb)
         I=0
...
c     massless-massless case
            if (kn_masses(legi).eq.0.and.kn_masses(legj).eq.0) then
               kij=z_dotp(kn_cmpborn(0,legi),kn_cmpborn(0,legj))
               ...
               I=I+(1d0/2*ll**2
     #              +ll*log(kij/(2*eij))
     #              -z_DDILOG(kij/(2*eij))+1d0/2*log(kij/(2*eij))**2
     #              -loglog)*
...
c     massless-massive case
     ...
     #            +0.5*z_Int0m_0(kn_cmpborn(0,legi),kn_cmpborn(0,legj))*ll
     #            -0.5*z_Int0m_ep(kn_cmpborn(0,legi),kn_cmpborn(0,legj)))*
     #              br_bornjk(legi,legj,jb)
            endif
c     massive-massless case
            if (kn_masses(legi).gt.0.and.kn_masses(legj).eq.0) then
               I = I + (0.5*(0.5*ll**2 - pi**2/6)
     #            +0.5*z_Int0m_0(kn_cmpborn(0,legj),kn_cmpborn(0,legi))*ll
     #            -0.5*z_Int0m_ep(kn_cmpborn(0,legj),kn_cmpborn(0,legi)))*
     #              br_bornjk(legj,legi,jb)
            endif
c     massive-massive case
            if (kn_masses(legi).gt.0.and.kn_masses(legj).gt.0) then
               I = I + (0.5*ll*
     #            z_Intmm_0(kn_cmpborn(0,legi),kn_cmpborn(0,legj))
     #            -0.5*z_Intmm_ep(kn_cmpborn(0,legi),kn_cmpborn(0,legj)))
     #            *br_bornjk(legi,legj,jb)
...
c we only summed over j>i, multiply by 2
     ...
     #       *pdfb1(fl1)*pdfb2(fl2)*kn_jacborn
         tot=tot+resvirt(jb)
      enddo
      if (.not.z_pwhg_isfinite(tot)) then
         do jb=1,flst_nborn
            resvirt(jb)=0d0
         enddo
      endif
...
c I_ep(k,m)
      function z_Int0m_ep(k,m)
c               /                                           [                  ]
c               |           d phi                           [   k.m       k0   ]
c z_Int0m_ep = -2 | d cos th  ----- log(sin th sin phi)  l0^2 [ ------- - ------ ]
c               |             pi                            [ k.l m.l   k.l l0 ]
c               /                                           [                  ]
c the range in phi is 0<phi<pi.
      implicit none
      real * 8 z_Int0m_ep, k(0:3),m(0:3)
      ...
      do j=0,3
         ...
         mh(j)=m(j)/m(0)
      enddo
      b=sqrt(1-z_dotp(mh,mh))
      khmh=z_dotp(kh,mh)
      z_Int0m_ep=-2*(log((1-b)/(1+b))**2/4+log(khmh/(1+b))*log(khmh/(1-b))
     # +z_DDILOG(1-khmh/(1+b))+z_DDILOG(1-khmh/(1-b)))
      end
...
c            /                       [                  ]
c the range in phi is 0<phi<pi.
      function z_Int0m_0(k,m)
      implicit none
      real * 8 z_Int0m_0, k(0:3),m(0:3)
      ...
      do j=0,3
         ...
         mh(j)=m(j)/m(0)
      enddo
      z_Int0m_0=log( z_dotp(kh,mh)**2/z_dotp(mh,mh) )
      end



      function z_Intm_ep(p)
c               /                                              2
c               |           d phi                             p  
c  z_Intm_ep = -2 | d cos th  ----- log(sin th sin phi) l0^2  -------
c               |             pi                            (p.l)^2
c               /
c the range in phi is 0<phi<pi.
      implicit none
      real * 8 p(0:3),z_Intm_ep
      real * 8 beta2,beta
      integer j
      beta2=0
      do j=1,3
      ...
      enddo
      beta=sqrt(beta2)
      z_Intm_ep=2*log((1+beta)/(1-beta))/beta
      end


      function z_Intmm_ep(p1,p2)
c               / 
c               |           d phi                             p1.p2  
c z_Intmm_ep= -2 | d cos th  ----- log(sin th sin phi) k0^2 ----------
c               |             pi                           p1.k  p2.k
c               /
c p1^2>0, p2^2>0.
c The range in phi is 0<phi<pi.
      implicit none
      real * 8 p1(0:3),p2(0:3), softintmm3,z_Intmm_ep
     ...
     # -2*z_DDILOG(2*zm/(zp-zm)*(zp-z)/(zm+z))
     # -2*z_DDILOG(-2*zp/(zp-zm)*(zm+z)/(zp-z))
      do j=1,3
         b1(j)=p1(j)/p1(0)
         b2(j)=p2(j)/p2(0)
      enddo
...
c z1 over b (same as before
     ...
     2 +2*z_DDILOG(2*zmob/(zp-zm)*(zp-z1)/(zmob+z1ob))
     3 +2*z_DDILOG(-2*zp/(zp-zm)*(zm+z1)/(zp-z1)) )
     4  *(1-b1b2)/sqrt(a-b)
      end

      function z_Intmm_0(p1,p2)
c            / 
c            |           d phi        p1.p2  
c  z_Intmm_0 = | d cos th  ----- k0^2 ----------
c            |             pi       p1.k  p2.k
c            /
c the range in phi is 0<phi<pi.
      implicit none
      real * 8 p1(0:3),p2(0:3), z_Intmm_0
      ...
      z_Intmm_0=log((1+beta)/(1-beta))/beta
      end

c Function to be integrated with z_mint, used to generate
c non-singular contributions using standard z_mint-z_gen method
...
c damping
      function z_sigremnant(xx,ww,ifirst)
      implicit none
      ...
      real * 8 z_sigremnant,xx(ndiminteg),ww
      integer ifirst
      real * 8 xrad(3)
     ...
     #         jac_over_csi_s,jac_over_csi_coll
      ...
      if(ifirst.eq.2) then
         z_sigremnant=rad_reg_tot+rad_damp_rem_tot
         if(flg_nlotest) call z_pwhgaccumup
         return
      endif
      do j=1,ndiminteg-3
         xborn(j)=xx(j)
...
c the following is needed to compute z_soft and collinear limits
      call z_allborn
      call z_born_suppression(suppfact)
      if(flg_withreg.or.(flg_withdamp.and.(
     # z_valid_emitter(0).or.z_valid_emitter(1).or.z_valid_emitter(2)))) then
         call z_gen_real_phsp_isr(xrad,
     #    jac_over_csi,jac_over_csi_p,jac_over_csi_m,jac_over_csi_s)
         xjac=jac_over_csi*kn_csi*kn_csimax*kn_jacborn*suppfact*ww*hc2
      endif
      if(flg_withreg) then
         call z_sigreal_reg(xjac,rad_reg_tot,rad_reg_arr)
         if(flg_nlotest) then
            call z_analysis_driver(rad_reg_tot/suppfact,1)
         endif
      else
         rad_reg_tot=0
      endif
...
c     No need to generate phase space; it is already available
                  call z_sigreal_damp_rem(xjac,ttt,rad_damp_rem_arr)
                  if(flg_nlotest) then
                     call z_analysis_driver(ttt/suppfact,1)
                  endif
                  rad_damp_rem_tot=rad_damp_rem_tot+ttt
               else
                  call z_gen_real_phsp_fsr(xrad,
     ...
     #                *kn_jacborn*suppfact*ww*hc2
      ...
      subroutine z_sigreal_reg(xjac,sig,r0)
...
c     generate "nmomset" z_random real-phase space configurations
     ...
     1                 flst_regular(1,lreg),preal(0,1,j),res(j,lreg))
               enddo
               call z_compare_vecs_reg(nmomset,lreg,res,lregpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(lreg)=lregpr
                  equivcoef(lreg)=1
...
c ----------------
         if(equivto(lreg).lt.0) then
            call z_realgr(flst_regular(1,lreg),kn_cmpreal,r0(lreg))
         else
     ...
     #      pdf1(flst_regular(1,lreg))*pdf2(flst_regular(2,lreg))
      ...
      subroutine z_compare_vecs_reg(nmomset,lreg,res,lregpr,cprop,iret)
      implicit none
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      real * 8 ep
 ...
 10      continue
      ...
      subroutine z_sigreal_damp_rem(xjac,sig,r1)
      implicit none
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      include 'src/Lib/z/pwhg_kn.h'

      subroutine z_btildereal(xrad,resreal,www)
      implicit none
...
c output values for z_analysis_driver
         out0=0
         out1=0
c check that emitter is valid
         if(z_valid_emitter(kn_emitter)) then
            if(kn_emitter.gt.2) then
c     final state radiation
               call z_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call z_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call z_collfsr(rc)
c     z_soft subtraction
                  call z_soft(r0s)
                  call z_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call z_collisrp(rp)
                     call z_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call z_collisrm(rm)
                     call z_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call z_analysis_driver(out0,0)
            if(out1.ne.0d0) call z_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine z_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      ...
      external z_sigreal_btl,z_soft,z_collfsr,z_softcollfsr, z_collisrp,
     $     z_softcollisrp,z_collisrm,z_softcollisrm
      call z_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  z_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call z_checksoft(z_sigreal_btl,z_soft,' z_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call z_checkcoll(z_sigreal_btl,z_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         do kn_emitter=0,2
c            call z_randomsave
            if(kn_emitter.ne.2) call z_checkcoll(z_sigreal_btl,z_collisrp,1
     $           ,' coll-plus',iun)
c            call z_randomrestore
            if(kn_emitter.ne.1) call z_checkcoll(z_sigreal_btl,z_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  z_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call z_checksoft(z_collfsr,z_softcollfsr,' z_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  z_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call z_checksoft(z_collisrp,z_softcollisrp,
     $           ' z_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call z_checksoft(z_collisrm,z_softcollisrm,
     $           ' z_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-z_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call z_checkcoll(z_soft,z_softcollfsr,1,' coll-z_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-z_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call z_checkcoll(z_soft,z_softcollisrp,1
     $           ,' coll-plus-z_soft',iun)
            if(kn_emitter.ne.1) call z_checkcoll(z_soft,z_softcollisrm,-1
     $           ,' coll-minus-z_soft',iun)
         enddo
      endif
      call z_randomrestore
      end

      subroutine z_checkborn(iun)
...
c are consistent with total Born
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine z_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/z/pwhg_dbg.h'
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 z_random,z_dotp
      external z_random,z_dotp
      logical z_valid_emitter,iszero,isnonzero,isequal
      external z_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=z_random()
      enddo
      call z_gen_born_phsp(xborn)
      call z_setscalesbtilde
      call z_allborn
      call z_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*z_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=z_random()
      enddo
c Check z_soft limits
      if(z_valid_emitter(kn_emitter)) then
         write(iun,*) ' z_random Born variables ====> ',xborn
          write(iun,*) ' z_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call z_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call z_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check z_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine z_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/z/pwhg_dbg.h'
      include 'src/Lib/z/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' z_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      if(ini) then
         ...
         if(flg_smartsig) then
            call z_randomsave
c     generate "nmomset" z_random real-phase space configurations
            call z_fillmomenta(nlegreal,nmomset,kn_masses,preal)
            do alr=1,flst_nalr
               do j=1,nmomset
                  call z_realgr(
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call z_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      do alr=1,flst_nalr
         r0(alr)=0
      enddo
      call z_pdfcall(1,kn_x1,pdf1)
      call z_pdfcall(2,kn_x2,pdf2)
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call z_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call z_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine z_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call z_sigreal_btl0(r0,0)
      end

c Real cross section, required by z_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine z_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      include 'src/Lib/z/pwhg_kn.h'
...
c     generate "nmomset" z_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call z_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c First mark as being computed
               markused(alr)=1
               call z_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' z_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: z_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine z_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=z_random()
            enddo
         enddo
         preal(3,nlegreal,j)=z_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine z_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from z_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine z_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call z_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'z_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine z_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/z/nlegborn.h'
      include 'src/Lib/z/pwhg_flst.h'
      include 'src/Lib/z/pwhg_math.h'
      ...
      integer rflav(nlegreal)
      real * 8 amp2 
      logical z_pwhg_isfinite
      external z_pwhg_isfinite
      call z_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.z_pwhg_isfinite(amp2)) amp2=0d0

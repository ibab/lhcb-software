      subroutine z_wrtpowheginput(nlf)
      implicit none
      integer nlf
      character * 100 line
     ...
     #     status='old',iostat=ios)
      ...
      if(ios.ne.0) then
         write(*,*) ' cannot open z_powheginput.dat'
         stop
      endif
 1    continue
      read(unit=33,fmt='(a)',iostat=ios,end=999) line
      if(ios.ne.0) then
         write(*,*) ' cannot read z_powheginput.dat'
         stop
      endif
      k=100
 2    if(k.gt.0) then
 ...
 999  end
      ...
      function z_powheginput(stringa)
      implicit none
         ...
         do l=1,1000000
            ...
            if(line.ne.' ') then
               if(numvalues.eq.maxnum) then
                  write(*,*) ' too many entries in z_powheginput.dat'
                  call exit(-1)
               endif
...
c skip blanks
 12            if(line(1:1).eq.' ') then
               ...
               if(ios.ne.0) then
                  write(*,*) ' z_powheginput error: cannot parse '
                  write(*,'(a)') line0
                  stop
               endif
            endif
 ...
 10      continue
      ...
      do j=1,numvalues
         if(string.eq.keywords(j)) then
            z_powheginput=values(j)
            if(.not.used(j)) then
               used(j)=.true.
               write(*,*) ' z_powheginput keyword ',keywords(j),
     1                    ' set to ',values(j)
            endif
            return
         endif
      enddo
      if(imode.eq.1) then
         write(*,*) ' z_powheginput: keyword ',string,' not found'
         call exit(-1)
      endif
c Not found; assign value -1d6; store the token anyhow
      if(numvalues.eq.maxnum) then
         write(*,*) ' too many entries in z_powheginput.dat'
         write(*,*) ' increase maxnum in z_powheginput.f'
         call exit(-1)
      endif
      numvalues=numvalues+1
...
c      integer j
c      real * 8 z_powheginput,res
c      external z_powheginput
c      string='QMASS'
c      res= z_powheginput(string)
c      write(*,*) string,res
c      end

      function z_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 z_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call z_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call z_btildevirt(resvirt)
            call z_btildecoll(xrad,rescoll,www)
            call z_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         z_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            z_btilde=z_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to z_btildeborn and z_btildevirt can be
...
c     to the NLO z_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c z_btildecoll and z_btildereal take care themselves to invoke the NLO
c z_analysis if required.
            call z_btildecoll(xrad,rescoll,www)
            call z_btildereal(xrad,resreal,www)
         endif
         z_btilde=0
...
c output Born
            ...
            call z_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            z_btilde=z_btilde+results(j)
c     Transfer all flavour components of z_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in z_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine z_adduptotals(results,n)
      implicit none
      include 'src/Lib/z/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/z_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(z_powheginput('#ubsigmadetails').eq.1) then
         call z_newunit(iun)

      subroutine vbf_h_setscalesbtilde
      implicit none
      include 'src/Lib/vbf_h/pwhg_math.h'
      include 'src/Lib/vbf_h/pwhg_st.h'
      real * 8 vbf_h_pwhg_alphas
      external vbf_h_pwhg_alphas
      real * 8 muf,mur
      call vbf_h_set_fac_ren_scales(muf,mur)
      st_mufact2= muf**2*st_facfact**2
      st_muren2 = mur**2*st_renfact**2
      st_alpha  = vbf_h_pwhg_alphas(st_muren2,st_lambda5MSB,st_nlight)
      end

      subroutine vbf_h_set_rad_scales(ptsq)
      implicit none
      real * 8 ptsq
      include 'src/Lib/vbf_h/pwhg_math.h'
...
c     is greater or equal to the vbf_h_mass of heavy quark 
        elseif( vbf_h_whichpdfpk().eq.'mlm') then    
c ad hoc value here (vbf_h_MLMPDF does not provide this)
           q2min=2d0
        else
...
c     mufact (50 is an arbitrary choice).
      ...
      st_muren2=ptsq
      st_alpha = vbf_h_pwhg_alphas(st_muren2,st_lambda5MSB,-1)
     ...
     #   (1+st_alpha/(2*pi)*((67d0/18-pi**2/6)*ca-5d0/9*nf))
      ...
      subroutine vbf_h_init_rad_lambda
      implicit none
      include 'src/Lib/vbf_h/pwhg_math.h'
      include 'src/Lib/vbf_h/pwhg_st.h'
      include 'src/Lib/vbf_h/nlegborn.h'
...
c for better NLL accuracy (FNO2006, (4.32) and corresponding references)
      ...
      function vbf_h_pwhg_alphas0(q2,xlam,inf)
      implicit none
      real * 8 vbf_h_pwhg_alphas0,q2,xlam
      integer inf
      real * 8 pi
      parameter (pi=3.141592653589793d0)
      real * 8 b0
      b0=(33-2*inf)/(12*pi)
      vbf_h_pwhg_alphas0=1/(b0*log(q2/xlam**2))
      end
      
C----------------------------------------------------------------------------
C-------------------------------------------------------------------
C------- ALPHA QCD -------------------------------------
c Program to calculate vbf_h_alfa strong with nf flavours,
c as a function of lambda with 5 flavors.
c The value of vbf_h_alfa is matched at the thresholds q = mq.
c When invoked with nf < 0 it chooses nf as the number of
c flavors with vbf_h_mass less then q.
c
      function vbf_h_pwhg_alphas(q2,xlam,inf)
      implicit none
      include 'src/Lib/vbf_h/nlegborn.h'
      include 'src/Lib/vbf_h/pwhg_flst.h'
      include 'src/Lib/vbf_h/pwhg_rad.h'
      real * 8 vbf_h_pwhg_alphas,q2,xlam
      integer inf
      real * 8 pi
      parameter (pi=3.141592653589793d0)
...
c      data ini/.true./
c      save ini
c      vbf_h_pwhg_alphas = 30d0
c      if (ini) then
c         write(*,*) '****************************************'
c         write(*,*) '****************************************'
c         write(*,*) '      RETURN alpha_s = ',vbf_h_pwhg_alphas
c         write(*,*) '****************************************'
c         write(*,*) '****************************************'
c         ini = .false.         
c      endif           
c      return
     ...
     #        - 1/( 1/(b3 * xlc) - xllc*bp3/(b3 * xlc)**2 ) + c45
      ...
      if    ( nf .eq. 5 ) then
        vbf_h_pwhg_alphas = 1/(b5 * xlq) -  bp5/(b5 * xlq)**2 * xllq
      elseif( nf .eq. 4 ) then
        vbf_h_pwhg_alphas =
     ...
     #    1/( 1/(1/(b3 * xlq) - bp3/(b3 * xlq)**2 * xllq) + c35 )
        ...
        print *,'error in vbf_h_alfa: unimplemented # of light flavours',nf
        call exit(1)
      endif
      return
      end

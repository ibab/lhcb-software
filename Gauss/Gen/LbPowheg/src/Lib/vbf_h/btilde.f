      function vbf_h_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 vbf_h_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call vbf_h_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call vbf_h_btildevirt(resvirt)
            call vbf_h_btildecoll(xrad,rescoll,www)
            call vbf_h_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         vbf_h_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            vbf_h_btilde=vbf_h_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to vbf_h_btildeborn and vbf_h_btildevirt can be
...
c     to the NLO vbf_h_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c vbf_h_btildecoll and vbf_h_btildereal take care themselves to invoke the NLO
c vbf_h_analysis if required.
            call vbf_h_btildecoll(xrad,rescoll,www)
            call vbf_h_btildereal(xrad,resreal,www)
         endif
         vbf_h_btilde=0
...
c output Born
            ...
            call vbf_h_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
               ...
               call vbf_h_analysis_driver(totvirt,0)
            endif
c closing call to end a sequence of correlated events in the
c vbf_h_analysis routines.
            call vbf_h_pwhgaccumup
         endif
c     closing call: accumulate values with correct signs
         vbf_h_btilde=0
         call vbf_h_adduptotals(results,flst_nborn)
         do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            vbf_h_btilde=vbf_h_btilde+results(j)
c     Transfer all flavour components of vbf_h_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in vbf_h_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine vbf_h_adduptotals(results,n)
      implicit none
      include 'src/Lib/vbf_h/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/vbf_h_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(vbf_h_powheginput('#ubsigmadetails').eq.1) then

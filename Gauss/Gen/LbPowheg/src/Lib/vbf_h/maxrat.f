      subroutine vbf_h_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 vbf_h_powheginput,vbf_h_random
      external vbf_h_powheginput,vbf_h_random      
      call vbf_h_zerohistnorms
      loaded=.false.
      nubound=vbf_h_powheginput('nubound')
      rad_nynorms=vbf_h_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=vbf_h_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=vbf_h_powheginput('#xupbound')
      ...
      if(vbf_h_powheginput('use-old-ubound').eq.1) then
         call vbf_h_newunit(iun)
...
c Compute and store normalization grid
 111  continue
     ...
     # ' bounding function for radiation'
      ...
      enddo
      do j=1,nubound
         call vbf_h_gen_born(mcalls,icalls)
         call vbf_h_gen_uborn_idx
         kn_csitilde=vbf_h_random()
         kn_y=2*vbf_h_random()-1
...
c      call maxratident
      ...
      common/vbf_h_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical vbf_h_refuse_pdf
      real * 8 vbf_h_pwhg_pt2,vbf_h_pwhg_upperb_rad
      external vbf_h_pwhg_pt2,vbf_h_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         ...
         if(.not.vbf_h_refuse_pdf()) then
            ...
            if(born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' vbf_h_inc_norms: Warning, born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' vbf_h_inc_norms: x1,x2', kn_xb1,kn_xb2
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' vbf_h_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' vbf_h_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/born/vbf_h_pwhg_upperb_rad()
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
      ...
      subroutine vbf_h_printhistnorms
      implicit none
      include 'src/Lib/vbf_h/nlegborn.h'
      include 'src/Lib/vbf_h/pwhg_flst.h'
      include 'src/Lib/vbf_h/pwhg_rad.h'
     ...
     1              efficiencies(ireg,iborn)/vbf_h_powheginput('nubound')/
...
c     1           vbf_h_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine vbf_h_zerohistnorms
      implicit none
      include 'src/Lib/vbf_h/nlegborn.h'
      include 'src/Lib/vbf_h/pwhg_flst.h'
      include 'src/Lib/vbf_h/pwhg_rad.h'
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call vbf_h_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call vbf_h_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine vbf_h_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/vbf_h/pwhg_math.h'
      include 'src/Lib/vbf_h/nlegborn.h'
      include 'src/Lib/vbf_h/pwhg_flst.h'
...
c
      ...
      subroutine vbf_h_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/vbf_h/nlegborn.h'
      include 'src/Lib/vbf_h/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 vbf_h_btilde,vbf_h_powheginput
      external vbf_h_btilde,vbf_h_powheginput
...
c will not be used;
         ...
         call vbf_h_mint(vbf_h_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=vbf_h_powheginput('ncall2')
         itmx2=vbf_h_powheginput('itmx2')
         call vbf_h_mint(vbf_h_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call vbf_h_gen(vbf_h_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn't used the ifold array may need reset to 1 here.
      call vbf_h_gen(vbf_h_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

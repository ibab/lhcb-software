      subroutine zj_pdfcall(ih,x,pdf)
      implicit none
      integer ih
      real * 8 x,pdf(-6:6)
      ...
      if(ih.eq.1) then
         call zj_genericpdf0(pdf_ndns1,pdf_ih1,st_mufact2,x,pdf)
      elseif(ih.eq.2) then
         call zj_genericpdf0(pdf_ndns2,pdf_ih2,st_mufact2,x,pdf)
      else
         write(*,*) ' zj_pdfcall: invalid call, ih=',ih
         stop
      endif
      end


c Front end to zj_genericpdf; it stores the arguments and return values of
c the nrec most recent calls to zj_genericpdf. When invoked it looks in the
...
c when zj_CTEQ6 pdf are used.
      subroutine zj_genericpdf0(ns,ih,xmu2,x,fx)
      implicit none
      integer ns,ih
...
c set to impossible values to begin with
      ...
      if(ini) then
         charmthr2=zj_powheginput('#charmthrpdf')
         bottomthr2=zj_powheginput('#bottomthrpdf')
      ...
      oxmu2(irec)=xmu2
      ox(irec)=x
      call zj_genericpdf(ns,ih,xmu2,x,ofx(-6,irec))

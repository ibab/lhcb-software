...
c...writes initialization information to a les houches events file on unit nlf. 
      subroutine zj_lhefwritehdr(nlf)
      implicit none
     ...
     #     version
      ...
      call zj_wrtpowheginput(nlf)
      write(nlf,'(a)') 'End of powheg.input content'
      call zj_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) 'zj_random number generator initialized with: ',
     # iran,' ',n1ran,' ',n2ran
      write(nlf,'(a)') '-->'
      write(nlf,'(a)') '<init>'
 ...
 120  format(1p,3(1x,e12.5),1x,i6)
      ...
      subroutine zj_printleshouches
c useful for debugging
      call zj_lhefwritev(6)
      end

c...lhefeader(nlf)
c...writes event information to a les houches events file on unit nlf. 
      subroutine zj_lhefwritev(nlf)
      implicit none
      integer nlf
      include 'src/Lib/zj/LesHouches.h'
      include 'src/Lib/zj/pwhg_flg.h'
     ...
     & vtimup(i),spinup(i)
 200  continue
      if(flg_pdfreweight) call zj_lhefwritepdfrw(nlf)
      if(flg_debug) call zj_lhefwritextra(nlf)
      write(nlf,'(a)')'</event>'      
...
c...writes trailer to a les houches events file on unit nlf. 
      subroutine zj_lhefwritetrailer(nlf)
      implicit none
      integer nlf,iran,n1ran,n2ran
      write(nlf,'(a)') '</LesHouchesEvents>'
c     save last zj_random number
      call zj_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) '#zj_random number generator exit values: ',
     # iran,' ',n1ran,' ',n2ran
      end

      subroutine zj_lhefwritextra(nlf)
      implicit none
      include 'src/Lib/zj/LesHouches.h'
      include 'src/Lib/zj/nlegborn.h'
      include 'src/Lib/zj/pwhg_flst.h'
      ...
      call zj_getcurrentrandom(iran,n1ran,n2ran)
      write(nlf,*) '# ', iran,' ',n1ran,' ',n2ran,
     #     "    previous event's zj_random seeds "
      write(nlf,'(a)') '# End extra-info-previous-event'
      end

      subroutine zj_lhefwritepdfrw(nlf)
      implicit none
      integer nlf
      integer id1,id2
      real * 8 x1,x2,xf1,xf2,xmufact
      call zj_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)

      function zj_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 zj_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call zj_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call zj_btildevirt(resvirt)
            call zj_btildecoll(xrad,rescoll,www)
            call zj_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         zj_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            zj_btilde=zj_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to zj_btildeborn and zj_btildevirt can be
...
c     to the NLO zj_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c zj_btildecoll and zj_btildereal take care themselves to invoke the NLO
c zj_analysis if required.
            call zj_btildecoll(xrad,rescoll,www)
            call zj_btildereal(xrad,resreal,www)
         endif
         zj_btilde=0
...
c output Born
            ...
            call zj_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            zj_btilde=zj_btilde+results(j)
c     Transfer all flavour components of zj_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in zj_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine zj_adduptotals(results,n)
      implicit none
      include 'src/Lib/zj/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/zj_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(zj_powheginput('#ubsigmadetails').eq.1) then
         call zj_newunit(iun)

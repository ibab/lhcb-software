      subroutine zj_bornzerodamp(alr,r0,rc,rs,dampfac)
...
c the real contribution to implement Born zero suppression
      ...
      integer alr
      real * 8 r0,rc,rs,dampfac,h,zj_pwhg_pt2,pt2,zj_powheginput
      logical ini
      data ini/.true./
      save ini,h
      external zj_pwhg_pt2,zj_powheginput
      if(ini) then
         h=zj_powheginput("#hfact")
         if(h.gt.0) then
            write(*,*)'***************************************'
...
c      write(*,*) ' zj_bornzerodamp: r0=',r0,'  rc=',rc,'  rs=',rs,
c     # ' dampfac=',dampfac
      end

...
C Call zj_PWHGBOOK(N,'TITLE',DEL,XMIN,XMAX) for each histogram to be booked.
...
C Example:
C      call zj_PWHGBOOK(2,'pt distribution',1.,10,70)
...
C
C FILLING:
C When it's time, call zj_PWHGFILL(N,X,Y); this will add Y (real*4) to the bin 
C in which X (real*4) happens to be, within histogram N. 
C
C PLAYING AROUND:
C At the end of the day you may want to sum, divide, cancel, etc.etc.
C various histograms (bin by bin). Then you call zj_PWHGOPERA(I,'O',J,K,X,Y). 
...
C M  : statistical zj_analysis; if I contains the weights (let's say WGT),
...
C
C FINAL ACCOUNTING:
C Now we can finalize our histograms; zj_PWHGFINAL(N) will calculate the integral
C of the histogram N, the mean value of the X variable and its RMS.
C If we now want to renormalize the hist's, we can call zj_PWHGNORM(N,X), which
C will normalize the integral to X  -- CAUTION: do not call zj_PWHGNORM before
C zj_PWHGFINAL, it will blow up.
C
C OUTPUT:
C To get a .dat file containing the values of the histograms, together with
C some information (like integral, mean values, etc.etc.) call zj_PWHGPRINT(N),
C for each hist N that you want in the .dat file. Before the call to zj_PWHGPRINT
...
C zj_PWHGTOP(N,M,'X','Y','zj_scale'). The points of the plot will be taken from histogram
C N, the error bars from histogram M. 'zj_scale', character*(*), determines
C the zj_scale for y, logarithmic or linear (zj_scale=LOG,LIN).
...
C itself. Before calling zj_PWHGTOP,
C     OPEN(UNIT=99,NAME='NAME.TOP',STATUS='NEW')
C--------------------------------------------------------------------------
C
C  COMMON/zj_histo/  Histogram N
C                           
...
C   HAVG(N),      Average value of x, weighted over the x range of the zj_histo
...
C
      ...
      SUBROUTINE zj_pwhgINIHIST
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      integer i
      DO 1, I=1,NMB             
   1  BOOK(I)=' NO'
      DO 2 I=1,10
   2  BOOK2(I)=' NO'
      call zj_pwhgresetmultitop
      END  


      subroutine zj_pwhggettitle(N,TIT)
      implicit none
      character *(*) TIT
      integer n
      include 'src/Lib/zj/pwhg_book.h'
      ...
      SUBROUTINE zj_PWHGBOOK(N,TIT,DEL,XMIN,XMAX)
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      integer n,i
      ...
      DATA ONEP/1.00001d0/
      IF(N.GT.NMB) THEN
	 CALL zj_pwhgwarn('zj_PWHGBOOK')
         WRITE(*,*) 'zj_PWHGBOOK: no more then ',NMB,' histograms'
         STOP
      ENDIF
      NHIST = MAX(N,NHIST)
...
C
      ...
      SUBROUTINE zj_PWHGINTEG(NIN,NOUT,IDIR,IPOW)
...
C of a given distribution. Overscores and underscores are included.
      ...
      I = (IDIR + 3)/2
      IF(NOUT.NE.NIN) THEN
      	CALL zj_PWHGBOOK(NOUT,TITLE(NIN)//C(I), 
     &                HDEL(NIN),HMIN(NIN),HMAX(NIN))
      ENDIF
      IF(IDIR.EQ.1) THEN
         HIST(NOUT,1) = zj_PWHGSUMPOW(HIST(NIN,1),USCORE(NIN),IPOW)
         IHIS(NOUT,1) = IHIS(NIN,1) + IUSCORE(NIN)
         XHIS(NOUT,1) = XHIS(NIN,1) + HDEL(NIN)/2
         DO L=2,M                      
            HIST(NOUT,L) = zj_PWHGSUMPOW(HIST(NIN,L),HIST(NOUT,L-1),IPOW)
            IHIS(NOUT,L) = IHIS(NIN,L) + IHIS(NOUT,L-1) 
            XHIS(NOUT,L) = XHIS(NIN,L) + HDEL(NIN)/2
         ENDDO
         OSCORE(NOUT) = zj_PWHGSUMPOW(OSCORE(NIN),HIST(NIN,M),IPOW)
         IOSCORE(NOUT) = IOSCORE(NIN) + IHIS(NIN,M)
      ELSEIF(IDIR.EQ.-1) THEN
         HIST(NOUT,M) = zj_PWHGSUMPOW(HIST(NIN,M),OSCORE(NIN),IPOW)
         ...
         DO L=M-1,1,-1                        
            ...
            XHIS(NOUT,L) = XHIS(NIN,L) - HDEL(NIN)/2
         ENDDO
         USCORE(NOUT) = zj_PWHGSUMPOW(USCORE(NIN),HIST(NIN,1),IPOW)
         IUSCORE(NOUT) = IUSCORE(NIN)+IHIS(NIN,1)
      ELSE                                 
         CALL zj_pwhgwarn('zj_PWHGINTEG')
         WRITE(*,*) 'Wrong idir in zj_PWHGINTEG: OPERATION NOT PERFORMED'
         STOP
      ENDIF
      END

      FUNCTION zj_PWHGSUMPOW(X,Y,IPOW)
      implicit none
      real * 8 x,y,zj_PWHGSUMPOW
      integer ipow
      IF(IPOW.EQ.1) THEN
         zj_PWHGSUMPOW = X + Y
      ELSEIF(IPOW.EQ.2) THEN
         zj_PWHGSUMPOW = SQRT(X**2+Y**2)
      ELSEIF(IPOW.EQ.0) THEN
         CALL zj_pwhgwarn('zj_PWHGSUMPOW')
         WRITE(*,*)'Error: IPOW=0 not allowed in zj_PWHGSUMPOW'
         STOP
      ELSE
         zj_PWHGSUMPOW = (X**IPOW+Y**IPOW)**(1./IPOW)
      ENDIF
      END

      SUBROUTINE zj_PWHGOPERA(I,OPER,J,K,X,Y)
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      integer i,j,k,l
...
c the average value of I to K and will put in J the quadratic dispersion.
   ...
   1  HIST(N,I)=0
      ...
      SUBROUTINE zj_PWHGPUTTAG(J,NAME)
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      integer j
c Per marcare un istogramma
      ...
      BOOK(J) = NAME
      RETURN
      ENTRY zj_pwhgGETTAG(J,TAG)
      TAG = BOOK(J)
      END

      SUBROUTINE zj_PWHGFINAL(N)
      implicit none
      integer n
...
c      CALL TIME(CTIME)
   ...
   20 FORMAT(' ENTRIES=',I5,4X,'UNDERSCORE=',I5,4x,'OVERSCORE=',I5,//)
      ...
      SUBROUTINE zj_PWHGTOP(N,M,BTIT,LTIT,zj_scale)
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      integer  imon,iday,iyear,ini,n,m,j,itime(3)
      CHARACTER CTIME*8
      CHARACTER*(*) LTIT,BTIT,zj_scale
      save imon,iday,iyear,ctime,ini
      DATA INI/0/
      data ctime/'        '/
      IF(INI.EQ.0) THEN
...
c      CALL TIME(CTIME)
      ...
      ENDIF
      IF(BOOK(N).NE.'YES') RETURN
      WRITE(99,100) TITLE(N),BTIT,LTIT,zj_scale,HMIN(N),HMAX(N)
  100 FORMAT( /1x,                               
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE zj_PWHGMNRTOP(N,M,BTIT,LTIT,zj_scale)
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      integer imon,iday,iyear,ini,n,m,j,itime(3)
      CHARACTER CTIME*8
...
c         CALL TIME(CTIME)
      ...
      ENDIF
      CALL zj_PWHGFINAL(N)
      WRITE(99,100) TITLE(N)(1:20),BTIT,LTIT,zj_scale,HMIN(N),HMAX(N)
  100 FORMAT( /1x,                               
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE zj_PWHGBOOK2(N,TIT,DELX,XMIN,XMAX,DELY,YMIN,YMAX)
...
C-- setup y-boundaries
   ...
   3  CONTINUE
      ...
      SUBROUTINE zj_PWHGFILL2(N,X,Y,WGT)
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/zj/pwhg_book.h'
C-- find x-coordinate
...
C-- find y-coordinate
     ...
     *                                             THEN        
      ...
      SUBROUTINE zj_PWHGOPERA2(I,OPER,J,K,X,Y)
      implicit real * 8 (a-h,o-z)
      include 'src/Lib/zj/pwhg_book.h'
      CHARACTER OPER*1
     ...
     &'*'.OR.OPER.EQ.'/'.OR.OPER.EQ.'M')) GO TO 10
      DO zj_L1=1,NBIN2(1,I)
      DO zj_L2=1,NBIN2(2,I)
      IF(OPER.EQ.'+') THEN
      HIST2(K,zj_L1,zj_L2)=X*HIST2(I,zj_L1,zj_L2) + Y*HIST2(J,zj_L1,zj_L2)
      ELSEIF(OPER.EQ.'-') THEN
      HIST2(K,zj_L1,zj_L2)=X*HIST2(I,zj_L1,zj_L2) - Y*HIST2(J,zj_L1,zj_L2)
      ELSEIF(OPER.EQ.'*') THEN
      HIST2(K,zj_L1,zj_L2)=X*HIST2(I,zj_L1,zj_L2) * Y*HIST2(J,zj_L1,zj_L2)
      ELSEIF(OPER.EQ.'/') THEN
        IF(Y.EQ.0..OR.HIST2(J,zj_L1,zj_L2).EQ.0.) THEN
          HIST2(K,zj_L1,zj_L2)=0.
          ELSE
          HIST2(K,zj_L1,zj_L2)=X*HIST2(I,zj_L1,zj_L2) / (Y*HIST2(J,zj_L1,zj_L2))
        ENDIF
      ELSEIF(OPER.EQ.'F') THEN
      HIST2(K,zj_L1,zj_L2)=X*HIST2(I,zj_L1,zj_L2)
      ELSEIF(OPER.EQ.'R') THEN
        IF(HIST2(I,zj_L1,zj_L2).GT.0.) THEN
        HIST2(K,zj_L1,zj_L2)=X*SQRT(HIST2(I,zj_L1,zj_L2))
        ELSE
        HIST2(K,zj_L1,zj_L2)=0.
        ENDIF
      ELSEIF(OPER.EQ.'S') THEN
      HIST2(K,zj_L1,zj_L2)=X*HIST2(I,zj_L1,zj_L2)**2
      ELSEIF(OPER.EQ.'L') THEN
        IF(HIST2(I,zj_L1,zj_L2).EQ.0..OR.J.EQ.0.) THEN
             HIST2(K,zj_L1,zj_L2)=0.
             ELSE
             HIST2(K,zj_L1,zj_L2)=X*LOG10(Y*HIST2(I,zj_L1,zj_L2))
        ENDIF
      ELSE
      WRITE(98,5) OPER
   5  FORMAT(' ****** OPERATION ="',A1,'" UNKNOWN ********'/)
...
C-- projection on the y-axis
  ...
  10  BOOK2(N)=' NO'
      ...
      SUBROUTINE zj_PWHGPROHIS(N,M,IAX)
...
c$$$      include 'src/Lib/zj/pwhg_book.h'
c$$$      CHARACTER CTIME*8
...
c$$$      DO ND=1,NDOTS
c$$$      X=XHIS2(N,I)+(2.*zj_random()-1.)*HDEL2(1,N)
c$$$      Y=YHIS2(N,J)+(2.*zj_random()-1.)*HDEL2(2,N)
c$$$      WRITE(99,*) X,Y
c$$$      ENDDO
c$$$      NTOT=NTOT+NDOTS
c$$$  3   CONTINUE       
...
c$$$      END
      ...
      SUBROUTINE zj_PWHGMULTITOP(NH,NE,N,M,BTIT,LTIT,SCA,INI)
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      CHARACTER CTIME*8,zj_scale*3
...
20    CONTINUE
	...
	XMX=MAX(XMX,ABS(X))
      ...
      zj_scale=SCA
50    IF(zj_scale.EQ.'LIN') THEN
	IF(FMN.GE.0.)	FMIN=0.
	IF(FMN.LT.0.)	FMIN=FMN*1.3
	IF(FMX.GT.0.)	FMAX=FMX*1.3
	IF(FMX.LT.0.)	FMAX=0.
      ELSEIF(zj_scale.EQ.'LOG') THEN 
        IF(FMN.LE.0.) THEN
	     	zj_scale='LIN'
		GO TO 50
	ENDIF
...
106   FORMAT(2X,'SET ORD X Y ',/,2(F8.4,1X),/,2(F8.4,1X),/,
     ...
     *       2X,'JOIN TEXT')                                    
      WRITE(99,107) BTIT,LTIT,zj_scale,HMIN(NH),HMAX(NH),FMIN,FMAX
107   FORMAT(                                           
     &' TITLE BOTTOM ','"',A,'"',/1X,
     &' TITLE LEFT ','"',A,'"',/1X,
     &' SET zj_scale Y ',A,/1X,
...
C
      ENTRY zj_PWHGTFILL(NH,NE,N,M,BTIT,LTIT,SCA)
      IP=IP+1                             
      IF(IP.GT.4) IP=1
...
202   FORMAT('   NEW PLOT',/,/)
203   RETURN     
      entry zj_pwhgresetmultitop
      iframe=0
      END                      

      SUBROUTINE zj_PWHGNEWPLOT
      WRITE(99,202) 
202   FORMAT('   NEW PLOT',/,/)
      CALL zj_PWHGMULTITOP(iDUM,iDUM,iDUM,iDUM,' ',' ','REF',0)
      END


c  added by SA and ER to use with MC@NLO
      SUBROUTINE zj_PWHGCOPY(NIN,NOUT)
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      integer nin,nout,i
...
C This may happen if zj_PWHGFINAL is called prior to zj_PWHGCOPY
      IF(BOOK(NIN)(1:1).EQ.'R'.AND.HMIN(NIN).EQ.HMAX(NIN)) RETURN
      IF(BOOK(NOUT)(1:1).NE.'Y'.AND.BOOK(NOUT)(1:1).NE.'R') 
     &  CALL zj_PWHGBOOK(NOUT,TITLE(NIN)(2:),HDEL(NIN),HMIN(NIN),HMAX(NIN))
      ...
      subroutine zj_pwhgbookup(n,string,zj_scale,del,xl,xu)
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      real * 8 del,xl,xu
      integer n
      character * (*) string
      character * 3 zj_scale
...
c     A call to zj_pwhgaccumup causes
c     the histogram n to be summed to the n+nmh, its square is
c     summed into n+nmh2. and n is zeroed.
c
c     When the subroutine zj_pwhgaddout is called, the statistical
c     zj_analysis of the n+nmh and n+nmh2 content is performed, the
c     average (with respect to the number of calls to zj_pwhgaccumup)
c     is added to the n+nmh3, and the error is summed in quadrature
c     to the n+nmh4; n+nmh and n+nmh2 are zeroed.
c
c     If instead zj_pwhgsetout is called, the statistical
c     zj_analysis of the n+nmh and n+nmh2 content is performed, the
c     average (with respect to the number of calls to zj_pwhgaccumup)
...
c.
      ...
      call zj_PWHGBOOK(n,  string,del,xl,xu)
      call zj_PWHGBOOK(n+nmh,'tmp ',del,xl,xu)
      call zj_PWHGBOOK(n+nmh2,'tmp square',del,xl,xu)
c this is the histogram that will be output
      call zj_PWHGBOOK(n+nmh3,string,del,xl,xu)
c the zj_scale for plotting (log or lin) is encoded in the title
      call zj_PWHGBOOK(n+nmh4,zj_scale//'error ',del,xl,xu)
      call zj_PWHGPUTTAG(n,'YST')
      return
      end

      

      subroutine zj_pwhgaccumup
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      integer j
      character * 3 tag
...
c     statistical zj_analysis, then empty the original histogram j.
c
      do j=1,nmh
         call zj_pwhgGETTAG(j,tag)
         if(tag.eq.'YST') then
            do l=1,nbin(j)
...
c
      do j=1,nmb
c     finish statistical zj_analysis
         call zj_pwhgGETTAG(j,tag)
         if(tag.eq.'YST') then
...
c     for the mean value is the number of calls to zj_pwhgaccumup, which in
...
c     histogram, as filled by pwhgaccumup.
     ...
     1             + xxx**2*(abs(xsumsq-xsum**2*xxx)))
      ...
      subroutine zj_pwhgtopout
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      character * 50 title0,zj_scale
      character * 3 tag
      integer i,ini
      ini=0
      do i=1,nmb
         call zj_pwhgGETTAG(i,tag)
         if(tag.eq.'YST') then
            call zj_PWHGFINAL(i+nmh3)
            call zj_PWHGFINAL(i+nmh4)
            call zj_pwhggettitle(i,title0)
            call zj_pwhggettitle(i+nmh4,zj_scale)
            call zj_PWHGMULTITOP(i+nmh3,i+nmh4,2,3,title0,' ',zj_scale(2:4)
     $           ,ini)
         endif
      enddo
      end            
      ...
      subroutine zj_pwhgrescale(zj_scale)
C********************************************************************
C The following function rescale the content of the histogram by the
C zj_scale factor.  IMPORTANT: must be called AFTER the statistical
C zj_analysis has been performed and before histograms are written to the
C TOPDRAWER file. For example : 
C    call zj_pwhgsetout ( or zj_pwhgaddout)
C    call zj_pwhgrescale(zj_scale)
C    call zj_pwhgtopout 
C**********************************************************************
      implicit none
      include 'src/Lib/zj/pwhg_book.h'
      real * 8 zj_scale,dummy_real
      integer i,dummy_int
      character * 3 tag
      do i=1,nmb
         call zj_pwhgGETTAG(i,tag)
         if(tag.eq.'YST') then
C zj_PWHGOPERA(I,'F',J,K,X,Y) multiplies hist I by the factor X, and puts
C the result in hist K;
            call zj_PWHGOPERA(i+nmh3,'F',dummy_int,i+nmh3,zj_scale,dummy_real)
C i+nmh3 contains the values 
            call zj_PWHGOPERA(i+nmh4,'F',dummy_int,i+nmh4,zj_scale,dummy_real)

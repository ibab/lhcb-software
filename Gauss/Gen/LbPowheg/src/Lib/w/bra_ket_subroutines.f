      function w_dotp(p1,p2)
      implicit none
      real * 8 w_dotp,p1(0:3),p2(0:3)
      w_dotp = (p1(0)*p2(0) - p1(3)*p2(3)) - p1(1)*p2(1) - p1(2)*p2(2)
      end

      function w_cdotp(p1,cp2)
      implicit none      
      complex * 16 w_cdotp,cp2(0:3)
      real * 8 p1(0:3)
      w_cdotp = (p1(0)*cp2(0) - p1(3)*cp2(3))
     #      - p1(1)*cp2(1) - p1(2)*cp2(2)
      end
      
      function w_ccdotp(cp1,cp2)
      implicit none      
      complex * 16 w_ccdotp,cp1(0:3),cp2(0:3)
      w_ccdotp = (cp1(0)*cp2(0) - cp1(3)*cp2(3))
...
C
C
C*********** polarisation vector (w_POLVEC) ************
C*********** entry w_HELVEC ****************************
C
...
C   bug in w_HELVEC for KT=0 corrected Nov.4, 1993
C
C   INPUT:
C
...
C                        -1,0,1) when entry w_HELVEC is used
C
C
C   OUTPUT:
...
C                  must be used!
C
      SUBROUTINE w_POLVEC(KBAR,LAMBDA,EPSCAR)
C
C   arguments:
C
      real*8  KBAR(0:4)
...
C
                  WRITE(*,*) 'Mass = 0 for Lambda = 3 in w_POLVEC'
                  EPSCAR(1) = 0.0d0
                  EPSCAR(2) = 0.0d0
                  EPSCAR(3) = 0.0d0
                  RETURN
...
C
            WRITE (*,*) 'Invalid Lambda in w_POLVEC: Lambda = ',LAMBDA
                  EPSCAR(1) = 0.0d0
                  EPSCAR(2) = 0.0d0
                  EPSCAR(3) = 0.0d0
            END IF
      RETURN
C
      ENTRY w_HELVEC(KBAR,SIGMAK,LAMBDA,EPSHEL)
C
C   code:
C
C
...
C                  Mass = 0 not allowed for longitudinal polarization
C
                  WRITE(*,*) 'Mass = 0 for Lambda = 0 in w_HELVEC'
                  EPSHEL(1) = 0.0d0
                  EPSHEL(2) = 0.0d0
                  EPSHEL(3) = 0.0d0
                  RETURN
...
C                  Unrecognised value of Lambda
C
            WRITE (*,*) 'Invalid Lambda in w_HELVEC: Lambda = ',LAMBDA
                  EPSHEL(1) = 0.0d0
                  EPSHEL(2) = 0.0d0
                  EPSHEL(3) = 0.0d0
            END IF
...
c gives w_ket_X(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine w_ket_X(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
...
c gives w_ket_X(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine w_ket_limit(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
      complex * 16 psi(1:2,-1:1)
...
c p = physical momentum  p[0] = E > 0
c ferm_type = fermion (+1) or antifermion (-1)
c gives w_bra_X(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine w_bra_X(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
      complex * 16 psi(1:2,-1:1)
...
c                           ( ris(1))
c gives p_slash |w_ket_X,hel> = (       )
c                           ( ris(2))
      subroutine w_slash_ket(p,w_ket_X,hel,ris)
      implicit none
      complex * 16 p(0:3)
      complex * 16 w_ket_X(1:2)
      integer hel
      complex * 16 ris(1:2)
      ...
      ris(1) = pslash(1,1)*w_ket_X(1)+pslash(1,2)*w_ket_X(2)
      ris(2) = pslash(2,1)*w_ket_X(1)+pslash(2,2)*w_ket_X(2)
      end


c gives <w_bra_X,hel | p_slash = ( ris(1), ris(2) )      

      subroutine w_slash_bra(p,w_bra_X,hel,ris)
      implicit none
      complex * 16 p(0:3)
      complex * 16 w_bra_X(1:2)
      integer hel
      complex * 16 ris(1:2)
      complex * 16 pslash(1:2,1:2)
      complex * 16 I
      ...
      pslash(2,1) = -hel*(p(1) + I*p(2))
      pslash(2,2) = p(0) + hel*p(3)
      ris(1) = pslash(1,1)*w_bra_X(1)+pslash(2,1)*w_bra_X(2)
      ris(2) = pslash(1,2)*w_bra_X(1)+pslash(2,2)*w_bra_X(2)
      end


c <w_bra_X, helbra| slash_p_1 slash_p_2 ... slash_p_n |w_ket_X, helket>
c where list_mom = [p_1, p_2,...., p_n]
c helket = -(-1)^n*helbra, otherwise the result is zero!!!
      subroutine w_bra_slash_ket(w_bra_X,w_ket_X,helket,list_mom,n,ris)
      implicit none
      complex * 16 w_bra_X(2,-1:1),w_ket_X(2,-1:1)
      integer helket
      integer n
...
c      dimension list_mom(0:3,n)
      ...
      helbra = -(-1)**n*helket
      hel = helket
      b(1) = w_bra_X(1,helbra)
      b(2) = w_bra_X(2,helbra)
      k(1) = w_ket_X(1,helket)
      k(2) = w_ket_X(2,helket)
      
      do ii=n,1,-1
         call w_slash_ket(list_mom(0,ii),k,hel,new_ket)
         k(1) = new_ket(1)
         k(2) = new_ket(2)
         hel = -hel
      enddo
...
c <w_bra_X, helbra| slash_curr_vertex_1, slash_p1, slash_curr_vertex_2|w_ket_X, helket>
...
c is emitted 
      subroutine w_bra_slash_ket_gluon
     #     (w_bra_X,w_ket_X,helket,pbra,pket,pg,eps,list_mom,ris)
      implicit none
      complex * 16 w_bra_X(2,-1:1),w_ket_X(2,-1:1)
      integer helket
...
c      parameter (I=(0,1))
      ...
      integer mu
      complex * 16 w_ccdotp
c emissione dalla gamba esterna w_bra_X
      do mu=0,3
      ...
      enddo
      call w_bra_slash_ket(w_bra_X,w_ket_X,helket,local_list,5,ris1)

      ris1 = ris1 / w_ccdotp(local_list(0,2),local_list(0,2))/
     #     w_ccdotp(local_list(0,4),local_list(0,4))
...
c emissione dal propagatore interno
      do mu=0,3
         ...
         local_list(mu,5) = list_mom(mu,3)
      enddo
      call w_bra_slash_ket(w_bra_X,w_ket_X,helket,local_list,5,ris2)
      ris2 = ris2 / w_ccdotp(local_list(0,2),local_list(0,2)) /
     #     w_ccdotp(local_list(0,4),local_list(0,4))


c emissione dalla gamba esterna w_ket_X
      do mu=0,3
         local_list(mu,1) = list_mom(mu,1)
         local_list(mu,2) = list_mom(mu,2)-pg(mu)
      ...
      enddo
      call w_bra_slash_ket(w_bra_X,w_ket_X,helket,local_list,5,ris3)
      ris3 = ris3 / w_ccdotp(local_list(0,2),local_list(0,2))/
     #     w_ccdotp(local_list(0,4),local_list(0,4))
...
C NO TRIPLE GLUON VERTEX HERE!!!
      subroutine w_bra_slash_ket_g1_g2
     #     (w_bra_X,w_ket_X,helket,pbra,pket,pg1,eps1,pg2,eps2,jcurr,ris)
      implicit none
      complex * 16 w_bra_X(2,-1:1),w_ket_X(2,-1:1)
      integer helket
...
c emission  <w_bra_X| 1,2,j |w_ket_X>
      do mu=0,3
...
c emission <w_bra_X|1,j,2|w_ket_X> 
      do mu=0,3
         local_list(mu,1) = eps1(mu)
         local_list(mu,2) = pg1(mu)+pbra(mu)
...
c emission <w_bra_X|j,1,2|w_ket_X>
...
c where list_mom = [p_1, p_2,...., p_n]
c helket = -(-1)^n*helbra, otherwise the result is zero!!!
      subroutine w_bra_slash_ket1(w_bra_X,w_ket_X,helbra,list_mom,n,ris)
      implicit none
      complex * 16 w_bra_X(2,-1:1),w_ket_X(2,-1:1)
      integer helket
      integer n
...
c      dimension list_mom(0:3,n)
      ...
      do ii=1,n
         call w_slash_bra(list_mom(0,ii),b,hel,new_bra)
         b(1) = new_bra(1)
         b(2) = new_bra(2)
         hel = -hel
      enddo
...
c the helicity of the w_bra_X and the w_ket_X are the same, otherwise this is zero!
c sig^mu = <w_bra_X, helbra| gamma^mu |w_ket_X, helket>
      subroutine w_bra_gamma_ket(w_bra_X,w_ket_X,hel,sig)
      implicit none
      complex * 16 w_bra_X(2,-1:1),w_ket_X(2,-1:1)
      complex * 16 b(2),k(2)
      integer hel
...
c the helicity of the w_bra_X and the w_ket_X are the same, otherwise this is zero!
c sig^mu = 1/w_dotp(pbra+pglu,pbra+pglu) * 
c   <w_bra_X, pbra, hel| eps_slash, (pbra+pglu)_slash, gamma^mu |w_ket_X, pket, hel>+
c          1/w_dotp(pket-pglu,pket-pglu) * 
c   <w_bra_X, pbra, hel| gamma^mu, (pket-pglu)_slash, eps_slash |w_ket_X, pket, hel>

      subroutine w_bra_gamma_ket_gluon(w_bra_X,w_ket_X,hel,pbra,pket,pglu,eps,sig)
      implicit none
      complex * 16 w_bra_X(2,-1:1),w_ket_X(2,-1:1)
      integer hel
      ...
      parameter (I=(0,1))
      complex*16 list_mom(0:3,3)
      real * 8 gamma(0:3),prop1,prop2,w_dotp
      integer mu,nu
      ...
      do mu=0,3
         ...
         enddo 
         call w_bra_slash_ket(w_bra_X,w_ket_X,hel,list_mom,3,J1(mu))
      enddo
      ...
      do mu=0,3
         ...
         enddo 
         call w_bra_slash_ket(w_bra_X,w_ket_X,hel,list_mom,3,J2(mu))
      enddo


      prop1 = 2*w_dotp(pglu,pbra)
      prop2 = -2*w_dotp(pglu,pket)
...
c with a single vertex inserted
      subroutine w_bra_gamma_ket_3gv(w_bra_X,w_ket_X,hel,pbra,pket,p1,eps1,
     #                  p2,eps2,sig)
      implicit none
      complex * 16 w_bra_X(2,-1:1),w_ket_X(2,-1:1)
      integer hel
     ...
     #            eps1(0:3),eps2(0:3)
      ...
      coeff1=2d0*w_dotp(p1,eps2)+w_dotp(p2,eps2)
      coeff2=-2d0*w_dotp(p2,eps1)-w_dotp(p1,eps1)
      coeff3=w_dotp(eps1,eps2)
     ...
     #             coeff3*(p2(mu)-p1(mu))
      ...
      prop1 = 2*w_dotp(p1,p2)*(w_dotp(ptemp,ptemp)+2*w_dotp(pbra,ptemp))
      prop2 = 2*w_dotp(p1,p2)*(w_dotp(ptemp,ptemp)-2*w_dotp(ptemp,pket))
...
c all the fermionic line  (NO COLOR STRUCTURE!!)
c the helicity of the w_bra_X and the w_ket_X are the same, otherwise this is zero!
c sig^mu = 1/w_dotp(pbra+pglu,pbra+pcurr) * 
c   <w_bra_X, pbra, hel| j_slash, (pbra+pcurr)_slash, gamma^mu |w_ket_X, pket, hel>+
c          1/w_dotp(pket-pcurr,pket-pcurr) * 
c   <w_bra_X, pbra, hel| gamma^mu, (pket-pcurr)_slash, j_slash |w_ket_X, pket, hel>

      subroutine w_bra_gamma_ket_curr(w_bra_X,w_ket_X,hel,pbra,
     # pket,pcurr,jcurr,sig)
      ...
      prop1 = w_dotp(pcurr,pcurr)*(w_dotp(pcurr,pcurr)+2*w_dotp(pcurr,pbra))
      prop2 = w_dotp(pcurr,pcurr)*(w_dotp(pcurr,pcurr)-2*w_dotp(pcurr,pket))
      ...
      subroutine w_exchange_momenta(p1,p2)
      implicit none
...
c return a w_random number between 0 and 1
      function w_rand_num()
      implicit none
      real * 8 w_rand_num
      real * 8 w_random2
      integer num,num2
      COMMON/w_SEED/num,num2
c set w_random seeds
      data NUM/12345/,NUM2/67890/
      w_rand_num = w_random2(num,num2)
      end
      
      subroutine w_exchange_mom(p,i,j,dim,pnew)
      implicit none
...
C-----------------------------------------------------------------------
      FUNCTION w_random2(iseed1,iseed2)
C     MAIN w_random NUMBER GENERATOR
C     USES METHOD OF l'Ecuyer, (VIA F.JAMES, COMP PHYS COMM 60(1990)329)
C-----------------------------------------------------------------------
      IMPLICIT NONE
      DOUBLE PRECISION w_random2
      ...
      w_random2=DBLE(IZ)/2147483589
c      w_random2=DBLE(IZ)*4.656613001013252D-10

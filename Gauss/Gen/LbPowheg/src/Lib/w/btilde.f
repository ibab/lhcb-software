      function w_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 w_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call w_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call w_btildevirt(resvirt)
            call w_btildecoll(xrad,rescoll,www)
            call w_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         w_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            w_btilde=w_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to w_btildeborn and w_btildevirt can be
...
c     to the NLO w_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c w_btildecoll and w_btildereal take care themselves to invoke the NLO
c w_analysis if required.
            call w_btildecoll(xrad,rescoll,www)
            call w_btildereal(xrad,resreal,www)
         endif
         w_btilde=0
...
c output Born
            ...
            call w_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            w_btilde=w_btilde+results(j)
c     Transfer all flavour components of w_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in w_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine w_adduptotals(results,n)
      implicit none
      include 'src/Lib/w/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/w_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(w_powheginput('#ubsigmadetails').eq.1) then
         call w_newunit(iun)

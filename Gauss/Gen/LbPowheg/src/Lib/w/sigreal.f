      subroutine w_btildereal(xrad,resreal,www)
      implicit none
...
c output values for w_analysis_driver
         out0=0
         out1=0
...
c     final state radiation
               call w_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call w_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call w_collfsr(rc)
c     w_soft subtraction
                  call w_soft(r0s)
                  call w_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call w_collisrp(rp)
                     call w_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call w_collisrm(rm)
                     call w_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call w_analysis_driver(out0,0)
            if(out1.ne.0d0) call w_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine w_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/w/nlegborn.h'
      include 'src/Lib/w/pwhg_flst.h'
      ...
      external w_sigreal_btl,w_soft,w_collfsr,w_softcollfsr, w_collisrp,
     $     w_softcollisrp,w_collisrm,w_softcollisrm
      call w_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  w_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call w_checksoft(w_sigreal_btl,w_soft,' w_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call w_checkcoll(w_sigreal_btl,w_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         do kn_emitter=0,2
c            call w_randomsave
            if(kn_emitter.ne.2) call w_checkcoll(w_sigreal_btl,w_collisrp,1
     $           ,' coll-plus',iun)
c            call w_randomrestore
            if(kn_emitter.ne.1) call w_checkcoll(w_sigreal_btl,w_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  w_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call w_checksoft(w_collfsr,w_softcollfsr,' w_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  w_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call w_checksoft(w_collisrp,w_softcollisrp,
     $           ' w_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call w_checksoft(w_collisrm,w_softcollisrm,
     $           ' w_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-w_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call w_checkcoll(w_soft,w_softcollfsr,1,' coll-w_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-w_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call w_checkcoll(w_soft,w_softcollisrp,1
     $           ,' coll-plus-w_soft',iun)
            if(kn_emitter.ne.1) call w_checkcoll(w_soft,w_softcollisrm,-1
     $           ,' coll-minus-w_soft',iun)
         enddo
      endif
      call w_randomrestore
      end

      subroutine w_checkborn(iun)
...
c are consistent with total Born
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine w_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/w/pwhg_dbg.h'
      include 'src/Lib/w/nlegborn.h'
      include 'src/Lib/w/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 w_random,w_dotp
      external w_random,w_dotp
      logical w_valid_emitter,iszero,isnonzero,isequal
      external w_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=w_random()
      enddo
      ...
      call w_allborn
      call w_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*w_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=w_random()
      enddo
c Check w_soft limits
      if(w_valid_emitter(kn_emitter)) then
         write(iun,*) ' w_random Born variables ====> ',xborn
          write(iun,*) ' w_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call w_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call w_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check w_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine w_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/w/pwhg_dbg.h'
      include 'src/Lib/w/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' w_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
...
c     generate "nmomset" w_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call w_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call w_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call w_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine w_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call w_sigreal_btl0(r0,0)
      end

c Real cross section, required by w_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine w_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/w/nlegborn.h'
      include 'src/Lib/w/pwhg_flst.h'
      include 'src/Lib/w/pwhg_kn.h'
...
c     generate "nmomset" w_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call w_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c First mark as being computed
               markused(alr)=1
               call w_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' w_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: w_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine w_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=w_random()
            enddo
         enddo
         preal(3,nlegreal,j)=w_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine w_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/w/nlegborn.h'
      include 'src/Lib/w/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from w_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine w_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/w/nlegborn.h'
      include 'src/Lib/w/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call w_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'w_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine w_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/w/nlegborn.h'
      include 'src/Lib/w/pwhg_flst.h'
      include 'src/Lib/w/pwhg_math.h'
      ...
      call w_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.w_pwhg_isfinite(amp2)) amp2=0d0

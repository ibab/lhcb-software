...
c...writes initialization information to a les houches events file on unit nlf. 
      subroutine wbb_lhefwritehdr(nlf)
      implicit none
     ...
     #     version
      ...
      call wbb_wrtpowheginput(nlf)
      write(nlf,'(a)') 'End of powheg.input content'
      call wbb_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) 'wbb_random number generator initialized with: ',
     # iran,' ',n1ran,' ',n2ran
      write(nlf,'(a)') '-->'
      write(nlf,'(a)') '<init>'
 ...
 120  format(1p,3(1x,e12.5),1x,i6)
      ...
      subroutine wbb_printleshouches
c useful for debugging
      call wbb_lhefwritev(6)
      end

c...lhefeader(nlf)
c...writes event information to a les houches events file on unit nlf. 
      subroutine wbb_lhefwritev(nlf)
      implicit none
      integer nlf
      include 'src/Lib/wbb/LesHouches.h'
      include 'src/Lib/wbb/pwhg_flg.h'
     ...
     & vtimup(i),spinup(i)
 200  continue
      if(flg_pdfreweight) call wbb_lhefwritepdfrw(nlf)
      if(flg_debug) call wbb_lhefwritextra(nlf)
      write(nlf,'(a)')'</event>'      
...
c...writes trailer to a les houches events file on unit nlf. 
      subroutine wbb_lhefwritetrailer(nlf)
      implicit none
      integer nlf,iran,n1ran,n2ran
      write(nlf,'(a)') '</LesHouchesEvents>'
c     save last wbb_random number
      call wbb_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) '#wbb_random number generator exit values: ',
     # iran,' ',n1ran,' ',n2ran
      end

      subroutine wbb_lhefwritextra(nlf)
      implicit none
      include 'src/Lib/wbb/LesHouches.h'
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
...
c      write(nlf,*) '# ',rad_kinreg,'       rad_kinreg'
      write(nlf,*) '# ',rad_type,'         rad_type'
c      call wbb_getcurrentrandom(iran,n1ran,n2ran)
c      write(nlf,*) '# ', iran,' ',n1ran,' ',n2ran,
c     #     "    previous event's wbb_random seeds "
      write(nlf,'(a)') '# End extra-info-previous-event'
      end

      subroutine wbb_lhefwritepdfrw(nlf)
      implicit none
      integer nlf
      integer id1,id2
      real * 8 x1,x2,xf1,xf2,xmufact
      call wbb_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)

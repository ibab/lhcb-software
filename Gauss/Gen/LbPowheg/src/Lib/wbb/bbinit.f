     ...
     #                  ,xmmm(0:50,ndiminteg),xmmmrm(0:50,ndiminteg)
      ...
      common/wbb_cgengrids/xgrid,ymax,xmmm,xgridrm,ymaxrm,xmmmrm,
     #                ifold,ifoldrm
      character * 20 pwgprefix
      integer lprefix
      common/wbb_cpwgprefix/pwgprefix,lprefix
      integer j,k,mcalls,icalls,imode,iunstat
      real * 8 wbb_powheginput
      external wbb_btilde,wbb_sigremnant,wbb_powheginput
      do j=1,ndiminteg
         do k=0,50
            xgrid(k,j)=0
            xgridrm(k,j)=0
     ...
     1        rnd_cwhichseed//'.dat',status='unknown')
      ...
      ncall1=wbb_powheginput('ncall1')
      ncall2=wbb_powheginput('ncall2')
      itmx1=wbb_powheginput('itmx1')
      itmx2=wbb_powheginput('itmx2')
      iret1=0
      iret2=0
      call wbb_loadgrids(iret1,xgrid,ymax,xgridrm,ymaxrm,
     2     ifold,ifoldrm)
      if(iret1.ne.0) call wbb_loadxgrid(iret2,xgrid,xint,xgridrm,xintrm)
      if(iret2.ne.0) then
         write(*,*)
     ...
     #        status='unknown')
         ...
         flg_nlotest=.false.
         imode=0
         call wbb_mint(wbb_btilde,ndiminteg,ncall1,itmx1,ifold,imode,iun,
     ...
     #        status='unknown')
            imode=0
            call wbb_mint(wbb_sigremnant,ndiminteg,ncall1,itmx1,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
            close(iun)
         endif
         call wbb_storexgrid(xgrid,xint,xgridrm,xintrm)
...
c importance sampling grids have been initialized with default wbb_SEED;
c they must all be the same. Now set current wbb_SEED, if required
      if(rnd_cwhichseed.ne.'none')
     1     call wbb_setrandom(rnd_initialseed,rnd_i1,rnd_i2)
      if(iret1.ne.0) then
         if (wbb_powheginput('#testplots').eq.1d0) call wbb_init_hist
c set  up the folding here, if required
         ifold(ndiminteg-2) = wbb_powheginput("foldcsi")
         ifold(ndiminteg-1) = wbb_powheginput("foldy")
         ifold(ndiminteg)   = wbb_powheginput("foldphi")
...
c Results in rad_tot???btl do not depend upon flg_withnegweights
         call wbb_resettotals
         call wbb_mint(wbb_btilde,ndiminteg,ncall2,itmx2,ifold,imode,iun,
     #        xgrid,xint,ymax,sigbtl,errbtl)
         call wbb_finaltotals
c finalize wbb_btilde output in histograms
         call wbb_pwhgaddout
         flg_nlotest=.false.
         write(*,*) 'wbb_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(*,*) 'wbb_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(*,*) 'wbb_btilde total (pos.-|neg.|):', rad_totbtl,' +-',
     1        rad_etotbtl
         write(iunstat,*) 'wbb_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(iunstat,*) 'wbb_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(iunstat,*) 'wbb_btilde Total (pos.-|neg.|):', rad_totbtl,
     1        ' +-',rad_etotbtl
c Now compute the remnant contributions
     ...
     #                ' and/or regular remnants'
            flg_nlotest=.true.
            imode=1
            call wbb_mint(wbb_sigremnant,ndiminteg,ncall2,itmx2,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
...
c wbb_btilde and remnant event are chosen in proportion to
...
c        
         call wbb_storegrids(xgrid,ymax,xgridrm,ymaxrm,ifold,ifoldrm,
     1        ncall2,itmx2)
c Output NLO histograms
         if (wbb_powheginput('#testplots').eq.1d0) then
            if(rnd_cwhichseed.eq.'none') then
     ...
     1           rad_totrm,'+-',rad_etotrm
         ...
         if (flg_weightedev) then
            write(iunstat,*) 
     1 ' total (wbb_btilde+remnants) cross section times '
            write(iunstat,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
         else
            write(iunstat,*) 
     1 ' total (wbb_btilde+remnants) cross section in pb',
     2     rad_tot,'+-',rad_etot
         endif
     ...
     1        rad_totrm,'+-',rad_etotrm
      ...
      if (flg_weightedev) then
         write(*,*) ' total (wbb_btilde+remnants) cross section times '
         write(*,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
      else
         write(*,*) ' total (wbb_btilde+remnants) cross section in pb',
     1     rad_tot,'+-',rad_etot
      endif
     ...
     1     rad_totnegbtl/(2*rad_totnegbtl+rad_tot)
      close(iunstat)
      call flush
c wbb_initialize wbb_gen; the array xmmm is set up at this stage.
      call wbb_gen(wbb_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,0,
     #    mcalls,icalls,xx)
c compute normalization of upper bounding function for radiation
      call wbb_do_maxrat(mcalls,icalls)
c print statistics
      call wbb_gen(wbb_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,3,
     #    mcalls,icalls,xx)
      if(xx(1).gt.0) write(*,*) 'POWHEG: efficiency in the generation'
     # //' of the Born variables =',xx(1)
      if((flg_withreg.or.flg_withdamp).and..not.flg_bornonly) then
c wbb_initialize wbb_gen for remnants
         call wbb_gen(wbb_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            0,mcalls,icalls,xx)
c     save wbb_random number seeds
         call wbb_randomsave
c generate few events from remnants, just to determine the generation efficiency
         do j=1,int(min(wbb_powheginput('nubound'),10d0))
            call wbb_gen(wbb_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            1,mcalls,icalls,xx)
         enddo
c     restore  wbb_random number seeds
...
c     print statistics
     ...
     #                   //' of the remnant variables =',xx(1)
      ...
      subroutine wbb_gen_btilde(mcalls,icalls)
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
     ...
     #                ifold,ifoldrm
      ...
      real * 8 wbb_btilde
      external wbb_btilde
      call wbb_gen(wbb_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)
      end

      subroutine wbb_gen_sigremnant
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
      real * 8 xgrid(0:50,ndiminteg),ymax(50,ndiminteg)
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)
      integer mcalls,icalls
      real * 8 wbb_sigremnant
      external wbb_sigremnant
      call wbb_gen(wbb_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,1,
     #    mcalls,icalls,xx)
      end


      subroutine wbb_storegrids(xgrid,ymax,xgridrm,ymaxrm,
     ...
     8     rad_tot,rad_etot
      ...
      subroutine wbb_loadgrids(iret,xgrid,ymax,xgridrm,ymaxrm,
     #           ifold,ifoldrm)
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
...
c
      ...
      if(wbb_powheginput('use-old-grid').eq.0) then
         iret=1
         return
      endif
      iret=0
...
c wbb_random seeds
     ...
     1              chseed//'.dat'
         ...
         if(wbb_powheginput("#ncallfrominput").eq.1) then
            ncall2=wbb_powheginput("ncall2")
            itmx2=wbb_powheginput("itmx2")
            ncall2=ncall2*itmx2
         endif
 ...
 998  continue
      ...
      subroutine wbb_storexgrid(xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
      include 'src/Lib/wbb/pwhg_kn.h'
     ...
     #     form='unformatted',status='unknown')
      ...
      subroutine wbb_loadxgrid(iret,xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
      include 'src/Lib/wbb/pwhg_kn.h'

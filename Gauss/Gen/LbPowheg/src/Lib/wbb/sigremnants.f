c Function to be integrated with wbb_mint, used to generate
c non-singular contributions using standard wbb_mint-wbb_gen method
...
c damping
      function wbb_sigremnant(xx,ww,ifirst)
      implicit none
      ...
      real * 8 wbb_sigremnant,xx(ndiminteg),ww
      integer ifirst
      real * 8 xrad(3)
     ...
     #         jac_over_csi_s,jac_over_csi_coll
      ...
      if(ifirst.eq.2) then
         wbb_sigremnant=rad_reg_tot+rad_damp_rem_tot
         if(flg_nlotest) call wbb_pwhgaccumup
         return
      endif
      do j=1,ndiminteg-3
         xborn(j)=xx(j)
...
c the following is needed to compute wbb_soft and collinear limits
      call wbb_allborn
      call wbb_born_suppression(suppfact)
      if(flg_withreg.or.(flg_withdamp.and.(
     # wbb_valid_emitter(0).or.wbb_valid_emitter(1).or.wbb_valid_emitter(2)))) then
         call wbb_gen_real_phsp_isr(xrad,
     #    jac_over_csi,jac_over_csi_p,jac_over_csi_m,jac_over_csi_s)
         xjac=jac_over_csi*kn_csi*kn_csimax*kn_jacborn*suppfact*ww*hc2
      endif
      if(flg_withreg) then
         call wbb_sigreal_reg(xjac,rad_reg_tot,rad_reg_arr)
         if(flg_nlotest) then
            call wbb_analysis_driver(rad_reg_tot/suppfact,1)
         endif
      else
         rad_reg_tot=0
      endif
...
c     No need to generate phase space; it is already available
                  call wbb_sigreal_damp_rem(xjac,ttt,rad_damp_rem_arr)
                  if(flg_nlotest) then
                     call wbb_analysis_driver(ttt/suppfact,1)
                  endif
                  rad_damp_rem_tot=rad_damp_rem_tot+ttt
               else
                  call wbb_gen_real_phsp_fsr(xrad,
     ...
     #                *kn_jacborn*suppfact*ww*hc2
      ...
      subroutine wbb_sigreal_reg(xjac,sig,r0)
...
c     generate "nmomset" wbb_random real-phase space configurations
     ...
     1                 flst_regular(1,lreg),preal(0,1,j),res(j,lreg))
               enddo
               call wbb_compare_vecs_reg(nmomset,lreg,res,lregpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(lreg)=lregpr
                  equivcoef(lreg)=1
...
c ----------------
         if(equivto(lreg).lt.0) then
            call wbb_realgr(flst_regular(1,lreg),kn_cmpreal,r0(lreg))
         else
     ...
     #      pdf1(flst_regular(1,lreg))*pdf2(flst_regular(2,lreg))
      ...
      subroutine wbb_compare_vecs_reg(nmomset,lreg,res,lregpr,cprop,iret)
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
      real * 8 ep
 ...
 10      continue
      ...
      subroutine wbb_sigreal_damp_rem(xjac,sig,r1)
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
      include 'src/Lib/wbb/pwhg_kn.h'

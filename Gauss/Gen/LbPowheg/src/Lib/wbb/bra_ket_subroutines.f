      function wbb_dotp(p1,p2)
      implicit none
      real * 8 wbb_dotp,p1(0:3),p2(0:3)
      wbb_dotp = (p1(0)*p2(0) - p1(3)*p2(3)) - p1(1)*p2(1) - p1(2)*p2(2)
      end

      function wbb_cdotp(p1,cp2)
      implicit none      
      complex * 16 wbb_cdotp,cp2(0:3)
      real * 8 p1(0:3)
      wbb_cdotp = (p1(0)*cp2(0) - p1(3)*cp2(3))
     #      - p1(1)*cp2(1) - p1(2)*cp2(2)
      end
      
      function wbb_ccdotp(cp1,cp2)
      implicit none      
      complex * 16 wbb_ccdotp,cp1(0:3),cp2(0:3)
      wbb_ccdotp = (cp1(0)*cp2(0) - cp1(3)*cp2(3))
...
C
C
C*********** polarisation vector (wbb_POLVEC) ************
C*********** entry wbb_HELVEC ****************************
C
...
C   bug in wbb_HELVEC for KT=0 corrected Nov.4, 1993
C
C   INPUT:
C
...
C                        -1,0,1) when entry wbb_HELVEC is used
C
C
C   OUTPUT:
...
C                  must be used!
C
      SUBROUTINE wbb_POLVEC(KBAR,LAMBDA,EPSCAR)
C
C   arguments:
C
      real*8  KBAR(0:4)
...
C
                  WRITE(*,*) 'Mass = 0 for Lambda = 3 in wbb_POLVEC'
                  EPSCAR(1) = 0.0d0
                  EPSCAR(2) = 0.0d0
                  EPSCAR(3) = 0.0d0
                  RETURN
...
C
            WRITE (*,*) 'Invalid Lambda in wbb_POLVEC: Lambda = ',LAMBDA
                  EPSCAR(1) = 0.0d0
                  EPSCAR(2) = 0.0d0
                  EPSCAR(3) = 0.0d0
            END IF
      RETURN
C
      ENTRY wbb_HELVEC(KBAR,SIGMAK,LAMBDA,EPSHEL)
C
C   code:
C
C
...
C                  Mass = 0 not allowed for longitudinal polarization
C
                  WRITE(*,*) 'Mass = 0 for Lambda = 0 in wbb_HELVEC'
                  EPSHEL(1) = 0.0d0
                  EPSHEL(2) = 0.0d0
                  EPSHEL(3) = 0.0d0
                  RETURN
...
C                  Unrecognised value of Lambda
C
            WRITE (*,*) 'Invalid Lambda in wbb_HELVEC: Lambda = ',LAMBDA
                  EPSHEL(1) = 0.0d0
                  EPSHEL(2) = 0.0d0
                  EPSHEL(3) = 0.0d0
            END IF
...
c gives wbb_ket(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine wbb_ket(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
...
c gives wbb_ket(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine wbb_ket_limit(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
      complex * 16 psi(1:2,-1:1)
...
c p = physical momentum  p[0] = E > 0
c ferm_type = fermion (+1) or antifermion (-1)
c gives wbb_bra(1..2,-1:1) for massless spinors
c  -1 : 1 helicity
      subroutine wbb_bra(p,ferm_type,psi)
      implicit none
      real * 8 p(0:3)
      integer ferm_type
      complex * 16 psi(1:2,-1:1)
...
c                           ( ris(1))
c gives p_slash |wbb_ket,hel> = (       )
c                           ( ris(2))
      subroutine wbb_slash_ket(p,wbb_ket,hel,ris)
      implicit none
      complex * 16 p(0:3)
      complex * 16 wbb_ket(1:2)
      integer hel
      complex * 16 ris(1:2)
      ...
      ris(1) = pslash(1,1)*wbb_ket(1)+pslash(1,2)*wbb_ket(2)
      ris(2) = pslash(2,1)*wbb_ket(1)+pslash(2,2)*wbb_ket(2)
      end


c gives <wbb_bra,hel | p_slash = ( ris(1), ris(2) )      

      subroutine wbb_slash_bra(p,wbb_bra,hel,ris)
      implicit none
      complex * 16 p(0:3)
      complex * 16 wbb_bra(1:2)
      integer hel
      complex * 16 ris(1:2)
      complex * 16 pslash(1:2,1:2)
      complex * 16 I
      ...
      pslash(2,1) = -hel*(p(1) + I*p(2))
      pslash(2,2) = p(0) + hel*p(3)
      ris(1) = pslash(1,1)*wbb_bra(1)+pslash(2,1)*wbb_bra(2)
      ris(2) = pslash(1,2)*wbb_bra(1)+pslash(2,2)*wbb_bra(2)
      end


c <wbb_bra, helbra| slash_p_1 slash_p_2 ... slash_p_n |wbb_ket, helket>
c where list_mom = [p_1, p_2,...., p_n]
c helket = -(-1)^n*helbra, otherwise the result is zero!!!
      subroutine wbb_bra_slash_ket(wbb_bra,wbb_ket,helket,list_mom,n,ris)
      implicit none
      complex * 16 wbb_bra(2,-1:1),wbb_ket(2,-1:1)
      integer helket
      integer n
...
c      dimension list_mom(0:3,n)
      ...
      helbra = -(-1)**n*helket
      hel = helket
      b(1) = wbb_bra(1,helbra)
      b(2) = wbb_bra(2,helbra)
      k(1) = wbb_ket(1,helket)
      k(2) = wbb_ket(2,helket)
      
      do ii=n,1,-1
         call wbb_slash_ket(list_mom(0,ii),k,hel,new_ket)
         k(1) = new_ket(1)
         k(2) = new_ket(2)
         hel = -hel
      enddo
...
c <wbb_bra, helbra| slash_curr_vertex_1, slash_p1, slash_curr_vertex_2|wbb_ket, helket>
...
c is emitted 
      subroutine wbb_bra_slash_ket_gluon
     #     (wbb_bra,wbb_ket,helket,pbra,pket,pg,eps,list_mom,ris)
      implicit none
      complex * 16 wbb_bra(2,-1:1),wbb_ket(2,-1:1)
      integer helket
...
c      parameter (I=(0,1))
      ...
      integer mu
      complex * 16 wbb_ccdotp
c emissione dalla gamba esterna wbb_bra
      do mu=0,3
      ...
      enddo
      call wbb_bra_slash_ket(wbb_bra,wbb_ket,helket,local_list,5,ris1)

      ris1 = ris1 / wbb_ccdotp(local_list(0,2),local_list(0,2))/
     #     wbb_ccdotp(local_list(0,4),local_list(0,4))
...
c emissione dal propagatore interno
      do mu=0,3
         ...
         local_list(mu,5) = list_mom(mu,3)
      enddo
      call wbb_bra_slash_ket(wbb_bra,wbb_ket,helket,local_list,5,ris2)
      ris2 = ris2 / wbb_ccdotp(local_list(0,2),local_list(0,2)) /
     #     wbb_ccdotp(local_list(0,4),local_list(0,4))


c emissione dalla gamba esterna wbb_ket
      do mu=0,3
         local_list(mu,1) = list_mom(mu,1)
         local_list(mu,2) = list_mom(mu,2)-pg(mu)
      ...
      enddo
      call wbb_bra_slash_ket(wbb_bra,wbb_ket,helket,local_list,5,ris3)
      ris3 = ris3 / wbb_ccdotp(local_list(0,2),local_list(0,2))/
     #     wbb_ccdotp(local_list(0,4),local_list(0,4))
...
C NO TRIPLE GLUON VERTEX HERE!!!
      subroutine wbb_bra_slash_ket_g1_g2
     #     (wbb_bra,wbb_ket,helket,pbra,pket,pg1,eps1,pg2,eps2,jcurr,ris)
      implicit none
      complex * 16 wbb_bra(2,-1:1),wbb_ket(2,-1:1)
      integer helket
...
c emission  <wbb_bra| 1,2,j |wbb_ket>
      do mu=0,3
...
c emission <wbb_bra|1,j,2|wbb_ket> 
      do mu=0,3
         local_list(mu,1) = eps1(mu)
         local_list(mu,2) = pg1(mu)+pbra(mu)
...
c emission <wbb_bra|j,1,2|wbb_ket>
...
c where list_mom = [p_1, p_2,...., p_n]
c helket = -(-1)^n*helbra, otherwise the result is zero!!!
      subroutine wbb_bra_slash_ket1(wbb_bra,wbb_ket,helbra,list_mom,n,ris)
      implicit none
      complex * 16 wbb_bra(2,-1:1),wbb_ket(2,-1:1)
      integer helket
      integer n
...
c      dimension list_mom(0:3,n)
      ...
      do ii=1,n
         call wbb_slash_bra(list_mom(0,ii),b,hel,new_bra)
         b(1) = new_bra(1)
         b(2) = new_bra(2)
         hel = -hel
      enddo
...
c the helicity of the wbb_bra and the wbb_ket are the same, otherwise this is zero!
c sig^mu = <wbb_bra, helbra| gamma^mu |wbb_ket, helket>
      subroutine wbb_bra_gamma_ket(wbb_bra,wbb_ket,hel,sig)
      implicit none
      complex * 16 wbb_bra(2,-1:1),wbb_ket(2,-1:1)
      complex * 16 b(2),k(2)
      integer hel
...
c in fact  slash_p = gamma_0*p_0 - vec{gamma} wbb_dot vec(p)
...
c the helicity of the wbb_bra and the wbb_ket are the same, otherwise this is zero!
c sig^mu = 1/wbb_dotp(pbra+pglu,pbra+pglu) * 
c   <wbb_bra, pbra, hel| eps_slash, (pbra+pglu)_slash, gamma^mu |wbb_ket, pket, hel>+
c          1/wbb_dotp(pket-pglu,pket-pglu) * 
c   <wbb_bra, pbra, hel| gamma^mu, (pket-pglu)_slash, eps_slash |wbb_ket, pket, hel>

      subroutine wbb_bra_gamma_ket_gluon(wbb_bra,wbb_ket,hel,pbra,pket,pglu,eps,sig)
      implicit none
      complex * 16 wbb_bra(2,-1:1),wbb_ket(2,-1:1)
      integer hel
      ...
      parameter (I=(0,1))
      complex*16 list_mom(0:3,3)
      real * 8 gamma(0:3),prop1,prop2,wbb_dotp
      integer mu,nu
      ...
      do mu=0,3
         ...
         enddo 
         call wbb_bra_slash_ket(wbb_bra,wbb_ket,hel,list_mom,3,J1(mu))
      enddo
      ...
      do mu=0,3
         ...
         enddo 
         call wbb_bra_slash_ket(wbb_bra,wbb_ket,hel,list_mom,3,J2(mu))
      enddo


      prop1 = 2*wbb_dotp(pglu,pbra)
      prop2 = -2*wbb_dotp(pglu,pket)
...
c with a single vertex inserted
      subroutine wbb_bra_gamma_ket_3gv(wbb_bra,wbb_ket,hel,pbra,pket,p1,eps1,
     #                  p2,eps2,sig)
      implicit none
      complex * 16 wbb_bra(2,-1:1),wbb_ket(2,-1:1)
      integer hel
     ...
     #            eps1(0:3),eps2(0:3)
      ...
      coeff1=2d0*wbb_dotp(p1,eps2)+wbb_dotp(p2,eps2)
      coeff2=-2d0*wbb_dotp(p2,eps1)-wbb_dotp(p1,eps1)
      coeff3=wbb_dotp(eps1,eps2)
     ...
     #             coeff3*(p2(mu)-p1(mu))
      ...
      prop1 = 2*wbb_dotp(p1,p2)*(wbb_dotp(ptemp,ptemp)+2*wbb_dotp(pbra,ptemp))
      prop2 = 2*wbb_dotp(p1,p2)*(wbb_dotp(ptemp,ptemp)-2*wbb_dotp(ptemp,pket))
...
c all the fermionic line  (NO COLOR STRUCTURE!!)
c the helicity of the wbb_bra and the wbb_ket are the same, otherwise this is zero!
c sig^mu = 1/wbb_dotp(pbra+pglu,pbra+pcurr) * 
c   <wbb_bra, pbra, hel| j_slash, (pbra+pcurr)_slash, gamma^mu |wbb_ket, pket, hel>+
c          1/wbb_dotp(pket-pcurr,pket-pcurr) * 
c   <wbb_bra, pbra, hel| gamma^mu, (pket-pcurr)_slash, j_slash |wbb_ket, pket, hel>

      subroutine wbb_bra_gamma_ket_curr(wbb_bra,wbb_ket,hel,pbra,
     # pket,pcurr,jcurr,sig)
      ...
      prop1 = wbb_dotp(pcurr,pcurr)*(wbb_dotp(pcurr,pcurr)+2*wbb_dotp(pcurr,pbra))
      prop2 = wbb_dotp(pcurr,pcurr)*(wbb_dotp(pcurr,pcurr)-2*wbb_dotp(pcurr,pket))
      ...
      subroutine wbb_exchange_momenta(p1,p2)
      implicit none
...
c return a wbb_random number between 0 and 1
      function wbb_rand_num()
      implicit none
      real * 8 wbb_rand_num
      real * 8 wbb_random2
      integer num,num2
      COMMON/wbb_SEED/num,num2
c set wbb_random seeds
      data NUM/12345/,NUM2/67890/
      wbb_rand_num = wbb_random2(num,num2)
      end
      
      subroutine wbb_exchange_mom(p,i,j,dim,pnew)
      implicit none
...
C-----------------------------------------------------------------------
      FUNCTION wbb_random2(iseed1,iseed2)
C     MAIN wbb_random NUMBER GENERATOR
C     USES METHOD OF l'Ecuyer, (VIA F.JAMES, COMP PHYS COMM 60(1990)329)
C-----------------------------------------------------------------------
      IMPLICIT NONE
      DOUBLE PRECISION wbb_random2
      ...
      wbb_random2=DBLE(IZ)/2147483589
c      wbb_random2=DBLE(IZ)*4.656613001013252D-10

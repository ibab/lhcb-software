      subroutine wbb_analysis_driver(dsig0,ikin)
      implicit none
      real * 8 dsig0
      integer ikin
      integer jpart,mu
      ...
      include 'src/Lib/wbb/hepevt.h'
      real * 8 wbb_powheginput
      
      if (wbb_powheginput('#testplots').ne.1d0) return
      if(ikin.eq.0) then
         nhep=nlegborn
         do jpart=1,nhep
            do mu=1,3
...
c     We can then use the flst_born(jpart,1) of the FIRST Born
      ...
      enddo
c     call wbb_analysis routine
      call wbb_analysis(dsig0)
      end

      subroutine wbb_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 wbb_powheginput,wbb_random
      external wbb_powheginput,wbb_random      
      call wbb_zerohistnorms
      loaded=.false.
      nubound=wbb_powheginput('nubound')
      rad_nynorms=wbb_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=wbb_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=wbb_powheginput('#xupbound')
      ...
      if(wbb_powheginput('use-old-ubound').eq.1) then
         call wbb_newunit(iun)
...
c      call maxratident
      ...
      common/wbb_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical wbb_refuse_pdf
      real * 8 wbb_pwhg_pt2,wbb_pwhg_upperb_rad
      external wbb_pwhg_pt2,wbb_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         call wbb_set_rad_scales(ptsq)
         if(.not.wbb_refuse_pdf()) then
            ...
            if(born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' wbb_inc_norms: Warning, born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' wbb_inc_norms: x1,x2', kn_xb1,kn_xb2
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' wbb_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' wbb_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/born/wbb_pwhg_upperb_rad()
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
     ...
     1              efficiencies(ireg,iborn)/wbb_powheginput('nubound')/
...
c     1           wbb_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine wbb_zerohistnorms
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
      include 'src/Lib/wbb/pwhg_rad.h'
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call wbb_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call wbb_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine wbb_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/wbb/pwhg_math.h'
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
...
c
      ...
      subroutine wbb_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      include 'src/Lib/wbb/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 wbb_btilde,wbb_powheginput
      external wbb_btilde,wbb_powheginput
...
c will not be used;
         ...
         call wbb_mint(wbb_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=wbb_powheginput('ncall2')
         itmx2=wbb_powheginput('itmx2')
         call wbb_mint(wbb_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call wbb_gen(wbb_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn't used the ifold array may need reset to 1 here.
      call wbb_gen(wbb_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

      function wbb_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 wbb_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call wbb_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call wbb_btildevirt(resvirt)
            call wbb_btildecoll(xrad,rescoll,www)
            call wbb_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         wbb_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            wbb_btilde=wbb_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to wbb_btildeborn and wbb_btildevirt can be
...
c     to the NLO wbb_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c wbb_btildecoll and wbb_btildereal take care themselves to invoke the NLO
c wbb_analysis if required.
            call wbb_btildecoll(xrad,rescoll,www)
            call wbb_btildereal(xrad,resreal,www)
         endif
         wbb_btilde=0
...
c output Born
            ...
            call wbb_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
               ...
               call wbb_analysis_driver(totvirt,0)
            endif
c closing call to end a sequence of correlated events in the
c wbb_analysis routines.
            call wbb_pwhgaccumup
         endif
c     closing call: accumulate values with correct signs
         wbb_btilde=0
         call wbb_adduptotals(results,flst_nborn)
         do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            wbb_btilde=wbb_btilde+results(j)
c     Transfer all flavour components of wbb_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in wbb_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine wbb_adduptotals(results,n)
      implicit none
      include 'src/Lib/wbb/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/wbb_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(wbb_powheginput('#ubsigmadetails').eq.1) then

      double complex function wz_BigT(j1,j2,j3,j4,j5,j6,za,zb)
      implicit none
...
c----first line
      wz_BigT=1.5d0*s(j1,j2)*del12*(t134-t234)
...
c----third line
      wz_BigT=wz_BigT+t134/(z2(j2,j5,j6,j1)**2*Del3)
     . *(zb(j1,j4)*za(j2,j6)
...
c----fourth line
      wz_BigT=wz_BigT+za(j3,j6)*zb(j4,j5)*s(j1,j2)*t134/(z2(j2,j5,j6,j1)*Del3)
...
c---"fourth" line from the bottom
      wz_BigT=wz_BigT+2d0*z2(j6,j2,j5,j4)/(z2(j2,j5,j6,j1)*Del3)
...
c---"third" line from the bottom
      wz_BigT=wz_BigT-zb(j1,j4)*za(j2,j6)*z2(j3,j2,j6,j5)/z2(j2,j5,j6,j1)**2
...
c---last line
     ...
     . -(za(j1,j3)*zb(j2,j5))**2/(za(j3,j4)*zb(j5,j6)))/z2(j1,j5,j6,j2)
      wz_BigT=wz_BigT-0.5d0*(zb(j1,j4)*za(j2,j6))**2

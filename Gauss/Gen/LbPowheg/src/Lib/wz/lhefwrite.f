...
c...writes initialization information to a les houches events file on unit nlf. 
      subroutine wz_lhefwritehdr(nlf)
      implicit none
     ...
     #     version
      ...
      call wz_wrtpowheginput(nlf)
      write(nlf,'(a)') 'End of powheg.input content'
      call wz_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) 'wz_random number generator initialized with: ',
     # iran,' ',n1ran,' ',n2ran
      write(nlf,'(a)') '-->'
      write(nlf,'(a)') '<init>'
 ...
 120  format(1p,3(1x,e12.5),1x,i6)
      ...
      subroutine wz_printleshouches
c useful for debugging
      call wz_lhefwritev(6)
      end

c...lhefeader(nlf)
c...writes event information to a les houches events file on unit nlf. 
      subroutine wz_lhefwritev(nlf)
      implicit none
      integer nlf
      include 'src/Lib/wz/LesHouches.h'
      include 'src/Lib/wz/pwhg_flg.h'
     ...
     & vtimup(i),spinup(i)
 200  continue
      if(flg_pdfreweight) call wz_lhefwritepdfrw(nlf)
      if(flg_debug) call wz_lhefwritextra(nlf)
      write(nlf,'(a)')'</event>'      
...
c...writes trailer to a les houches events file on unit nlf. 
      subroutine wz_lhefwritetrailer(nlf)
      implicit none
      integer nlf,iran,n1ran,n2ran
      write(nlf,'(a)') '</LesHouchesEvents>'
c     save last wz_random number
      call wz_RM48UT(iran,n1ran,n2ran)
      write(nlf,*) '#wz_random number generator exit values: ',
     # iran,' ',n1ran,' ',n2ran
      end

      subroutine wz_lhefwritextra(nlf)
      implicit none
      include 'src/Lib/wz/LesHouches.h'
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      ...
      call wz_getcurrentrandom(iran,n1ran,n2ran)
      write(nlf,*) '# ', iran,' ',n1ran,' ',n2ran,
     #     "    previous event's wz_random seeds "
      write(nlf,'(a)') '# End extra-info-previous-event'
      end

      subroutine wz_lhefwritepdfrw(nlf)
      implicit none
      integer nlf
      integer id1,id2
      real * 8 x1,x2,xf1,xf2,xmufact
      call wz_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)

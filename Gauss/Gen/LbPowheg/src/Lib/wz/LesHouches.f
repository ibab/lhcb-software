...
c first assign flavours and colour to the underlying Born process
         call wz_momenta_lh(kn_pborn,nlegborn)
         call wz_born_lh
         scalup=sqrt(rad_ptsqmin)
      else
c it is an event with radiation
         call wz_momenta_lh(kn_preal,nlegreal)
         call wz_born_lh
         ...
         if(em.le.2) then
c wz_setcolour_rad works with all incoming (or all outgoing) flavours and colours;
...
c and conjugate its colour in output, to make it incoming.
            call wz_setcolour_rad(coluborn,-flem,flrad,
     #                     icolup(1,em),icolup(1,rad))
            call wz_colour_conj(icolup(1,em))
         else
...
c conjugate their colours in the output, to make them outgoing.
            call wz_setcolour_rad(coluborn,-flem,-flrad,
     #                     icolup(1,em),icolup(1,rad))
            call wz_colour_conj(icolup(1,rad))
            call wz_colour_conj(icolup(1,em))
         endif
...
c     i1<i2
      subroutine wz_add_resonance(idpdg,i1,i2)
      implicit none
      include 'src/Lib/wz/LesHouches.h'
     ...
     #     spinuptmp(maxnup)
      ...
      if (i1.ge.i2) then
         write(*,*) 'wrong sequence in wz_add_resonance'
         stop
      endif
      do i=i1,nup
         iduptmp(i) = idup(i)
...
c     change mothers of decaying particles
      ...
      subroutine wz_colour_conj(icol)
      implicit none
      integer icol(2)
      integer itmp
      itmp=icol(2)
     ...
     #       -p(2,k)**2-p(3,k)**2))
      ...
      include 'src/Lib/wz/pwhg_rad.h'
      include 'src/Lib/wz/LesHouches.h'
c      include 'src/Lib/wz/PhysPars.h'
...
c gluons are numbered 21 in pdg
         ...
         vtimup(k)=0
      enddo
      call wz_borncolour_lh
      end


      subroutine wz_getnewcolor(newcolor)
      implicit none
      integer newcolor
      include 'src/Lib/wz/LesHouches.h'
      integer j,k
...
c the one where both fl2 and fl3 are gluons, where a 50% wz_random choice is made
c over the two possible colour connections.
      subroutine wz_setcolour_rad(colin,fl2,fl3,colout2,colout3)
      implicit none
      integer colin(2),fl2,fl3,colout2(2),colout3(2)
      integer newcolor
      real * 8 wz_random
      external wz_random
      call wz_getnewcolor(newcolor)
      if(colin(1).ne.0.and.colin(2).ne.0) then
         if(fl2.eq.0.and.fl3.eq.0) then
            if(wz_random().gt.0.5d0) then
               colout2(1)=colin(2)
               colout2(2)=newcolor
               colout3(1)=newcolor
               colout3(2)=colin(1)
 ...
 998  continue
      write(*,*)
     #     ' Error in wz_setcolour_rad: inconsistent colour flavour input'
...
c first conjugate incoming colors
      call wz_colour_conj(icolup(1,1))
      call wz_colour_conj(icolup(1,2))
      do j=1,nup
         mark(j)=1
      enddo
      imarked=0
 ...
 22   continue
      ...
      subroutine wz_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)
      implicit none
      integer id1,id2
...
c wz_btilde event: pass x1 and x2, id1, id2 etc. of the current underlying Born
      ...
      call wz_pdfcall(1,x1,pdf)
      xf1=x1*pdf(id1)
      call wz_pdfcall(2,x2,pdf)
      xf2=x2*pdf(id2)
      if(id1.eq.0) id1=21
      if(id2.eq.0) id2=21
      end

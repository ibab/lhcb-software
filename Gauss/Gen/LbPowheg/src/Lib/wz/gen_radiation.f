      ...
      integer iret,iun
      real * 8 suppfact
      real * 8 wz_random,wz_powheginput
      external wz_random,wz_powheginput
      integer mcalls,icalls
      real * 8 wz_pwhg_pt2,wz_pt2max_regular
      external wz_pwhg_pt2,wz_pt2max_regular
      integer i
      if(idwtup.eq.3) then
         xwgtup=1
...
c     store current wz_random seeds. To be used to restart at problematic events
      call wz_savecurrentrandom
      if(wz_random().gt.rad_totrm/rad_totgen) then
c     generate underlying Born kinematics
         call wz_gen_btilde(mcalls,icalls)
c     generate underlying Born flavour
         call wz_gen_uborn_idx
c
         if(wz_powheginput('#wz_testsuda').eq.1) then
            call wz_testsuda
         endif
c generate radiation
         call wz_gen_radiation
c add a wz_random azimuthal rotation around beam axis
         call wz_add_azimuth
c --- set up les houches interface
         rad_pt2max=wz_pwhg_pt2()
         call wz_gen_leshouches
c if negative weight, flip the sign of xwgtup
         if(rad_btilde_sign(rad_ubornidx).eq.-1) then
            xwgtup=-xwgtup
         endif
c rad_type=1 for wz_btilde events (used only for debugging purposes)
         rad_type=1
         if(flg_weightedev) then
            call wz_born_suppression(suppfact)
...
c iret=2: reg contribution (real graphs without singular regions)
         call wz_gen_remnant(iret)
c         if (wz_pwhg_pt2().lt.rad_ptsqmin) then
c            write(*,*) '****************************************'
c            write(*,*) 'WARNING in wz_gen_remnant'
c            write(*,*) 'wz_pwhg_pt2 < rad_ptsqmin ',
c     #           wz_pwhg_pt2(),' < ',rad_ptsqmin
c            write(*,*) (flst_alr(i,rad_realalr),i=1,nlegreal)
c            write(*,*) 'To generate this event, use the following seeds'
c            call wz_printcurrentrandom
c            write(*,*) '****************************************'
c         endif
...
c     set st_muren2 equal to pt2 for scalup value
            rad_pt2max=max(rad_ptsqmin,wz_pwhg_pt2())
            call wz_set_rad_scales(rad_pt2max)
            call wz_gen_leshouches
            call wz_born_suppression(suppfact)
            if(suppfact.eq.0) then
               write(*,*) ' 0 suppression factor in event generation'
...
c     set st_muren2 for scalup value for regular contributions
            rad_pt2max=max(rad_ptsqmin,wz_pt2max_regular())
            call wz_set_rad_scales(rad_pt2max)
            call wz_gen_leshouches_reg
c rad_type=3 for regular contributions
      ...
      include 'src/Lib/wz/pwhg_kn.h'
      include 'src/Lib/wz/pwhg_rad.h'
      real * 8 wz_t,csi,y,azi,sig
      real * 8 tmax
      common/wz_ctmax/tmax
      integer kinreg,firstreg,lastreg
      logical ini
      data ini/.true./
      real * 8 wz_pwhg_pt2,wz_powheginput
      external wz_pwhg_pt2,wz_powheginput
      save ini,firstreg,lastreg
      if(ini) then
         firstreg=wz_powheginput("#radregion")
...
c     initial state radiation
               call wz_gen_rad_isr(wz_t)
            else
c     final state radiation
               call wz_gen_rad_fsr(wz_t)
            endif
            if(wz_t.gt.tmax) then
               tmax=wz_t
               kinreg=rad_kinreg
               csi=kn_csi
               y=kn_y
...
c Generate a Born like event
         ...
         wz_t=tmax
         if(rad_kinreg.eq.1) then
            call wz_gen_real_phsp_isr_rad
         else
            call wz_gen_real_phsp_fsr_rad
         endif
         call wz_set_rad_scales(wz_t)
         call wz_sigreal_rad(sig)
         call wz_gen_real_idx
      endif
      end



      function wz_pwhg_pt2()
      implicit none
      real * 8 wz_pwhg_pt2
      include 'src/Lib/wz/pwhg_math.h'
      include 'src/Lib/wz/nlegborn.h'
      ...
      include 'src/Lib/wz/pwhg_rad.h'
      if(rad_kinreg.eq.1) then
         wz_pwhg_pt2=(kn_sreal/4)*(1-kn_y**2)*kn_csi**2
      else
         wz_pwhg_pt2=(kn_sreal/2)*(1-kn_y)*kn_csi**2
      endif
      end



      function wz_pwhg_upperb_rad()
      implicit none
      real * 8 wz_pwhg_upperb_rad
      include 'src/Lib/wz/pwhg_math.h'
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
      ...
      if(rad_kinreg.eq.1) then
         if(rad_iupperisr.eq.1) then
            wz_pwhg_upperb_rad = st_alpha/((1-x)*(1-y**2))
c Possible alternatives:
c rad_iupper=2   wz_pwhg_upperb_rad = st_alpha/(x*(1-x)*(1-y**2))
c 
c rad_iupper=3:  wz_pwhg_upperb_rad = st_alpha/(x**2*(1-x)*(1-y**2))
         else
...
c for now use the same
         if(rad_iupperfsr.eq.1) then
            wz_pwhg_upperb_rad = st_alpha/(csi*(1-y))
         elseif(rad_iupperfsr.eq.2) then
            wz_pwhg_upperb_rad = st_alpha/(csi**2*(1-y)*(1-csi/2*(1-y))**2)
     2          *csi
...
c We use it to find its zero in pt2.
      ...
      include 'src/Lib/wz/pwhg_rad.h'
      include 'src/Lib/wz/pwhg_math.h'
      real * 8 wz_pt2solve,pt2
c i set by wz_DZERO: 1 for first call, 2 for subsequent calls, 3 for last call
c before a normal exit; not used here
      integer i
      real * 8 xlr,q2,xlam2c,kt2max,cunorm
      integer nlc
      common/wz_cpt2solve/xlr,q2,kt2max,xlam2c,cunorm,nlc
...
c see Notes/upperbounding-isr.pdf
     ...
     #           + xlr
               ...
               wz_pt2solve=cunorm*pi/b0*(log(2d0)
...
c final state radiation
     ...
     2   /(p*xm)/2.d+0) + xlr
      ...
      subroutine wz_gen_rad_isr(wz_t)
c Generates hard radiation kinematics according to
c appendix D in ZZ paper.
c
c  common/wz_cptmin/ptminsq: minimum pt^2 accepted
c 
      implicit none
      include 'src/Lib/wz/pwhg_math.h'
      include 'src/Lib/wz/nlegborn.h'
      ...
      integer nlc
      common/wz_cpt2solve/xlr,q2,kt2max,xlam2c,unorm,nlc
      real * 8 xmin,rv,xp,xm,chi,tk,uk,ubound,ufct,
     #   sborn,value,err,tmp1,tmp2,tmp,rvalue,born,sig
      common/wz_cdfxmin/xmin
      real * 8 tmax
      common/wz_ctmax/tmax
      real * 8 wz_random,wz_pt2solve,dfxmin,wz_pwhg_alphas0,wz_pwhg_upperb_rad
      external wz_random,wz_pt2solve,dfxmin,wz_pwhg_alphas0,wz_pwhg_upperb_rad
...
c see section 4 in ZZ paper, last paragraph
      ...
      xlr=0
 1    continue
      xlr=xlr+log(wz_random())
c CERNLIB voodoo:
      call wz_KERSET('C205. ',0,0,101)
c solve for zero of wz_pt2solve
c wz_DZERO(xmin,xmax,x,err,eps,maxcalls,function)
...
c eps: required accuracy
      call wz_DZERO(rad_ptsqmin,kt2max,wz_t,err,1d-8,1000000,wz_pt2solve)
c error conditions
      if(wz_t.eq.0.and.err.lt.0d0
     # .and.err.gt.rad_ptsqmin-kt2max) then
         write(*,*) 'wz_DZERO fails'
         write(*,*) ' number of calls exceeded'
         call exit(1)
      endif
 3    if(wz_t.lt.rad_ptsqmin.or.wz_t.lt.tmax) then
...
c vetoes:
      rv=wz_random()
      xp=(sqrt(1+wz_t/sborn)+sqrt(wz_t/sborn))**2
      xm=(sqrt(1+wz_t/sborn)-sqrt(wz_t/sborn))**2
c tmp1: V(wz_t)/tilde{V}(wz_t) in appendix D of ZZ paper;
c (typo: in D.13, log log -> log
      xmin=min(x1b,x2b)/(2*sqrt(1+wz_t/sborn))
      if(rad_iupperisr.eq.1) then
         tmp1=log((sqrt(xp-xmin)+sqrt(xm-xmin))
     #       /(sqrt(xp-xmin)-sqrt(xm-xmin)))
         if(wz_t.lt.sborn) then
            tmp1=tmp1/(log(2*sborn/wz_t)/2)
         else
            tmp1=tmp1/(log(2d0)/2)
         endif
      elseif(rad_iupperisr.eq.2) then
         tmp1=log(2/xmin*(sqrt((xp-xmin)*(xm-xmin))
     # +1-xmin/2*(xp+xm))/(xp-xm)) /(log(q2/wz_t)/2)
      endif
c compare with D.11-D.12
c to set xmuren2:
      call wz_set_rad_scales(wz_t)
      tmp2=st_alpha / wz_pwhg_alphas0(wz_t,rad_lamll,nlc)
      tmp=tmp1*tmp2
      if(tmp.gt.1) then
...
c using       d sqrt(xm-x) /sqrt(xp-xm) (rad_iupper=1) or d sqrt(xm-x) /(xmin sqrt(xp-xm)) (rad_iupper=2) as upper bound using hit and miss
 2    chi=sqrt(xm-xmin)*wz_random()
      x=xm-chi**2
      if(rad_iupperisr.eq.1) then
         if(wz_random().gt.sqrt(xp-xm)/sqrt(xp-x)) goto 2
      elseif(rad_iupperisr.eq.2) then
         if(wz_random().gt.(xmin*sqrt(xp-xm))/(x*sqrt(xp-x))) goto 2
      endif
c get y (abs to avoid tiny negative values)
      y=sqrt(abs(1-4*x/(1-x)**2*wz_t/sborn))
      if(wz_random().gt.0.5d0) y=-y
...
c that some pdf vanish (typically heavy flavour pdf's)
      ...
      kn_y=y
      kn_csi=1-x
      kn_azi=2*pi*wz_random()
      ubound=born*wz_pwhg_upperb_rad()*unorm*ufct
      call wz_gen_real_phsp_isr_rad
      call wz_sigreal_rad(sig)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      rvalue=wz_random()*ubound
      if(rvalue.gt.value) then
         call wz_increasecnt('vetoed radiation')
         goto 1
      endif
      end


      subroutine wz_gen_rad_fsr(wz_t)
...
c 
     ...
     #   s,value,err,tmp,rvalue,born,sig
      ...
      real * 8 tmax
      common/wz_ctmax/tmax
      real * 8 wz_random,wz_pt2solve,wz_pwhg_alphas0,wz_pwhg_upperb_rad
      external wz_random,wz_pt2solve,wz_pwhg_alphas0,wz_pwhg_upperb_rad
      unorm=rad_norms(rad_kinreg,rad_ubornidx)
c kn_sborn=kn_sreal:
      s=kn_sborn
...
c vetoes:
      ...
      call wz_set_rad_scales(wz_t)
      if(rad_iupperfsr.eq.1) then
         tmp=st_alpha / wz_pwhg_alphas0(wz_t,rad_lamll,nlc)
      elseif(rad_iupperfsr.eq.2) then
         tmp=st_alpha
      elseif(rad_iupperfsr.eq.3) then
         tmp=st_alpha
...
c in the range wz_t/s < csi^2 < csimax^2
         rv=wz_random()
         csi=exp(rv*log(wz_t/s)/2+(1-rv)*log(kn_csimax))
c get y
         y=1-2*wz_t/(s*csi**2)
c At this point a csi-y pair is generated according to the
c distribution upper(). It is automatically within range.
      elseif(rad_iupperfsr.eq.2) then
c     csi distributed uniformly in 1/(csi-wz_t/s)
         rv=wz_random()
         csi=1/(rv/(sqrt(wz_t/s)-wz_t/s)+(1-rv)/(kn_csimax-wz_t/s))+wz_t/s
c extra csi dependent factor
         if(wz_random().gt.csi) goto 1
c get y
         y=1-2*wz_t/(s*csi**2)
c At this point a csi-y pair is generated according to the
c distribution upper(). It is automatically within range.
...
c get y
         y=1-2*wz_t/(s*csi**2)
         if(wz_random().gt.(csi-wz_t/s)) goto 1
      else
         write(*,*) ' wz_gen_rad_fsr:  rad_iupper=',rad_iupperfsr,
     1        ' invalid'
      endif
c
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call wz_uboundfct(ufct,csi,y)
      if(wz_random().gt.ufct) goto 1
c Veto from upper bound to real value. Count how many vetoes,
c since these may be expensive.
...
c that some pdf vanish (typically heavy flavour pdf's)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      real * 8 wz_random
      external wz_random
      azi=2d0*pi*wz_random()
      sazi = sin(azi)
      cazi = cos(azi)
      if (kn_csi.eq.0d0) then
         do ileg=1, nlegborn
            call wz_mrotate(dir,sazi,cazi,kn_pborn(1,ileg))
         enddo
      else
         do ileg=1, nlegreal
            call wz_mrotate(dir,sazi,cazi,kn_preal(1,ileg))
         enddo
      endif
      end

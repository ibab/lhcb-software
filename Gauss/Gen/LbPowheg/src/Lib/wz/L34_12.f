      double complex function wz_L34_12(j1,j2,j3,j4,j5,j6,za,zb)
      implicit none
!      include 'src/Lib/wz/constants.h'
      ...
      double precision wz_t,t134,t234,del3,del12,del56
...
c-----first line
      wz_L34_12=1.5d0*del56*(t134-t234)*z2(j3,j1,j2,j4)*z2(j6,j1,j2,j5)
     . /(z2(j2,j5,j6,j1)*Del3**2)
      wz_L34_12=wz_L34_12+1.5d0*za(j3,j6)*bit1/(z2(j2,j5,j6,j1)*Del3)

c----second line
      wz_L34_12=wz_L34_12+0.5d0*za(j3,j4)*zb(j4,j5)*bit2
     . /(zb(j5,j6)*z2(j2,j5,j6,j1)*del3)
      wz_L34_12=wz_L34_12+zb(j1,j4)*za(j2,j6)*t134
     . *(za(j3,j6)*del12-2d0*za(j3,j4)*zb(j4,j5)*za(j5,j6))
     . /(za(j5,j6)*z2(j2,j5,j6,j1)**2*del3)

c-----third line
      wz_L34_12=wz_L34_12+0.5d0*t134/(z2(j2,j5,j6,j1)*del3)

      subroutine wz_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 wz_powheginput,wz_random
      external wz_powheginput,wz_random      
      call wz_zerohistnorms
      loaded=.false.
      nubound=wz_powheginput('nubound')
      rad_nynorms=wz_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=wz_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=wz_powheginput('#xupbound')
      ...
      call wz_newunit(iun)
      if(wz_powheginput('use-old-ubound').eq.1) then
         call wz_newunit(iun)
...
c      call maxratident
      ...
      common/wz_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical wz_refuse_pdf
      real * 8 wz_pwhg_pt2,wz_pwhg_upperb_rad
      external wz_pwhg_pt2,wz_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         call wz_set_rad_scales(ptsq)
         if(.not.wz_refuse_pdf()) then
            ...
            if(born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' wz_inc_norms: Warning, born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' wz_inc_norms: x1,x2', kn_xb1,kn_xb2
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' wz_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' wz_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/born/wz_pwhg_upperb_rad()
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
     ...
     1              efficiencies(ireg,iborn)/wz_powheginput('nubound')/
...
c     1           wz_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine wz_zerohistnorms
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_rad.h'
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call wz_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call wz_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine wz_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/wz/pwhg_math.h'
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
...
c
      ...
      subroutine wz_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 wz_btilde,wz_powheginput
      external wz_btilde,wz_powheginput
...
c will not be used;
         call wz_resettotals
         call wz_mint(wz_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=wz_powheginput('ncall2')
         itmx2=wz_powheginput('itmx2')
         call wz_mint(wz_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call wz_gen(wz_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn'wz_t used the ifold array may need reset to 1 here.
      call wz_gen(wz_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

      ...
      external wz_whichpdfpk,wz_powheginput
...
c End initialization of common block defaults.
      pdf_ih1=wz_powheginput('ih1')
      pdf_ih2=wz_powheginput('ih2')
      if(wz_whichpdfpk().eq.'lha') then
         pdf_ndns1=wz_powheginput('lhans1')
         pdf_ndns2=wz_powheginput('lhans2')
      elseif(wz_whichpdfpk().eq.'mlm') then
         pdf_ndns1=wz_powheginput('ndns1')
         pdf_ndns2=wz_powheginput('ndns2')
      else
         write(*,*) ' unimplemented pdf package',wz_whichpdfpk()
         stop
      endif
      if(pdf_ndns1.ne.pdf_ndns2) then
         st_lambda5MSB=wz_powheginput('QCDLambda5')
      else
         call wz_genericpdfpar(pdf_ndns1,pdf_ih1,st_lambda5MSB,
     1                      scheme,iorder,iret)
      ...
      kn_beams(0,1)=wz_powheginput('ebeam1')
      kn_beams(0,2)=wz_powheginput('ebeam2')
      kn_beams(1,1)=0
      kn_beams(1,2)=0
      kn_beams(2,1)=0
      kn_beams(2,2)=0
...
c generation cut: see wz_gen_born_phsp.f
      kn_ktmin=wz_powheginput("#bornktmin")
...
c thresholds 
      rad_ptsqmin=wz_powheginput('#ptsqmin')
      if(rad_ptsqmin.lt.0) rad_ptsqmin=0.8d0
      rad_charmthr2=wz_powheginput('#charmthr')
      if(rad_charmthr2.lt.0) rad_charmthr2=1.5d0
      rad_charmthr2=rad_charmthr2**2
      rad_bottomthr2=wz_powheginput('#bottomthr')
      if(rad_bottomthr2.lt.0) rad_bottomthr2=5d0
      rad_bottomthr2=rad_bottomthr2**2
c scale factors
      st_renfact=wz_powheginput('#renscfact')
      st_facfact=wz_powheginput('#facscfact')
...
c initialize Lambda values for radiation
      call wz_init_rad_lambda
c
      call wz_init_processes

      call wz_init_couplings

c initialize number of singular regions
      rad_nkinreg=1+(nlegborn-flst_lightpart+1)
      call wz_genflavreglist


      dbg_softtest=.true.
      dbg_colltest=.true.
      if(flg_withdamp) then
         write(*,*) ' no wz_soft tests if withdamp is set'
         dbg_softtest=.false.
      endif
      if(flg_bornonly) then
         write(*,*) ' no wz_soft and coll. tests if bornonly is set'
         dbg_softtest=.false.
         dbg_colltest=.false.
      endif

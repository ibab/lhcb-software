c subroutines wz_soft, wz_collfsr, wz_softcollfsr, wz_collisrp, wz_collisrm,
c             wz_softcollisrp, wz_softcollisrm 
c are called by wz_btildereal; they return (1-y) csi^2 R_alr (if kn_emitter
c is >2, i.e. final state) or (1-y^2) csi^2 R (if kn_emitter <=2) in the
c collinear final state                      (wz_collfsr)
c wz_soft-collinear final state                 (wz_softcollfsr)
c collinear initial state in the + direction (wz_collisrp)
c collinear initial state in the - direction (wz_collisrm)
c wz_soft-collinear initial state +             (wz_softcollisrp)
c wz_soft-collinear initial state -             (wz_softcollisrm)
c The wz_soft limit is taken keeping y (i.e. kn_y) fixed
...
c
c
c Functions: wz_collbtl and wz_softbtl are required to compute the damping
c            factor in wz_bornzerodamp. They are called by sigreal, and
c            sigreal is used by wz_btilde (hence the btl suffix)

c wz_softbtl: computes the same quantity as wz_soft, but does not include 
c          the luminosity
c
c wz_collbtl: if kn_emitter is in the final state, computes the same quantity as
c          wz_collfsr, but does not include the luminosity
c          if kn_emitter is 1 or 2  computes the same quantity as
c          wz_collisrp or wz_collisrm, without luminosity
c          if kn_emitter is 0 computes the average of the return values
c          of wz_collisrp and wz_collisrm, weighted with the factors 
c          (1+y)/2 and (1-y)/2.
c
c 
c
c Functions: wz_collrad, wz_softrad  are required to compute the damping
c            factor in wz_bornzerodamp. They are called by wz_sigreal_rad,
c            i.e. they are used in the generation of radiation.
c            They compute the same quantities as wz_collbtl and wz_softbtl,
...
c for fsr csi is y independent
      ...
      phi=kn_azi
      x=csi*xocsi
      call wz_buildkperp(em,phi,kperp,kperp2)
      do alr=1,flst_nalr
         if(em.eq.flst_emitter(alr)) then
            call wz_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
      ...
      subroutine wz_buildkperp(em,phi,kperp,kperp2)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
...
c     Then rotate it counterclockwise around the em direction
     ...
     #/kn_cmpborn(3,em)
      ...
      call wz_mrotate(dir,sin(phi),cos(phi),kperp(1))
      kperp(0)=0
      end
      

      subroutine wz_softcollfsr(rc)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
      ...
      call wz_collfsrnopdf(rc)
      kn_csi=tmp
      call wz_pdfcall(1,kn_xb1,pdf1)
      call wz_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end
      ...
      subroutine wz_collisrp(rc)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      real * 8 rc(maxalr)
      call wz_collisr(1,rc)
      end

      subroutine wz_collisrm(rc)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      real * 8 rc(maxalr)
      call wz_collisr(2,rc)
      end

      subroutine wz_softcollisrp(rc)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
      ...
      call wz_pdfcall(1,x1,pdf1)
      call wz_pdfcall(2,x2,pdf2)
      do alr=1,flst_nalr
         rc(alr)=rc(alr)*pdf1(flst_alr(1,alr))*pdf2(flst_alr(2,alr))
      enddo         
      end

      subroutine wz_collisrnopdf(i,rc)
      implicit none
      integer i
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
...
c Construct kperp
      ...
      do alr=1,flst_nalr
         if(flst_emitter(alr).eq.kn_emitter) then
            call wz_collisralr(alr,i,csi,kperp,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
...
c softvec(0:3): real * 8, 4-vector of wz_soft gluon normalized to softvec(0)=1
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
      ...
      real * 8 y
      real * 8 pdf1(-6:6),pdf2(-6:6)
c wz_boost Born momenta to their rest frame
c find wz_boost velocity
      em=kn_emitter
      y=kn_y
      do alr=1,flst_nalr
         call wz_softalr(alr,em,y,r0(alr))
      enddo
      call wz_pdfcall(1,kn_xb1,pdf1)
      call wz_pdfcall(2,kn_xb2,pdf2)
      do alr=1,flst_nalr
      ...
      subroutine wz_collfsralr(alr,csi,xocsi,x,q0,kperp,kperp2,res)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
...
c provide multiplicity of emitter in underlyng Born
      ...
      subroutine wz_collisralr(alr,i,csi,kperp,res)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
...
c     the last expression being wz_boost invariant.
...
c     The remaining csi=1-x factor has been applied earlier
      ...
      subroutine wz_softalr(alr,em,y,res)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
               ...
               if(abs(flst_born(j,iub)).le.6) then
                  do k=j+1,nlegborn
                     if(abs(flst_born(k,iub)).le.6) then
                        result=result+
     #wz_dotp(kn_cmpborn(0,j),kn_cmpborn(0,k))
     #/(wz_dotp(kn_cmpborn(0,j),kn_softvec)*
     #wz_dotp(kn_cmpborn(0,k),kn_softvec)) * br_bornjk(j,k,iub)
                     endif
                  enddo
               endif
            enddo
...
c     symmetric, multiply by 2
     ...
     #kn_cmpborn(2,j)**2-kn_cmpborn(3,j)**2
                  result=result-pjsq/wz_dotp(kn_cmpborn(0,j),
     #kn_softvec)**2*br_born(iub)*cf
               endif
            enddo
c     having chosen a wz_soft four momentum of energy 1, supply
...
c     Multiply wz_soft result by (wz_soft limit Sij appropriate factors)
      if(res.ne.0) then
         sumdijinv=0
...
c cross section used in wz_btilde
      subroutine wz_collbtl(rc)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      include 'src/Lib/wz/pwhg_flst.h'
      include 'src/Lib/wz/pwhg_kn.h'
         ...
         do alr=1,flst_nalr
            if(flst_emitter(alr).eq.em) then
               if(em.ne.2) then
                  ...
                  call wz_collisralr(alr,i,csi,kperp,r1)
               endif
               if(em.ne.1) then
                  i=2
                  csi=kn_csi*kn_csimaxm/kn_csimax
                  call wz_collisralr(alr,i,csi,kperp,r2)
               endif
               if(em.eq.0) then
...
c     Construct kperp
            ...
            if(em.ne.2) then
               csi=kn_csi*kn_csimaxp/kn_csimax
               call wz_collisralr(alr,1,csi,kperp,r1)
            endif
            if(em.ne.1) then
               csi=kn_csi*kn_csimaxm/kn_csimax
               call wz_collisralr(alr,2,csi,kperp,r2)
            endif
            if(em.eq.0) then
               rc(alr)=(r1*(1+kn_y)+r2*(1-kn_y))/2
            elseif(em.eq.1) then

      function wz_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 wz_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call wz_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call wz_btildevirt(resvirt)
            call wz_btildecoll(xrad,rescoll,www)
            call wz_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         wz_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            wz_btilde=wz_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to wz_btildeborn and wz_btildevirt can be
...
c     to the NLO wz_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c wz_btildecoll and wz_btildereal take care themselves to invoke the NLO
c wz_analysis if required.
            call wz_btildecoll(xrad,rescoll,www)
            call wz_btildereal(xrad,resreal,www)
         endif
         wz_btilde=0
...
c output Born
            ...
            call wz_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            wz_btilde=wz_btilde+results(j)
c     Transfer all flavour components of wz_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in wz_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine wz_adduptotals(results,n)
      implicit none
      include 'src/Lib/wz/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/wz_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(wz_powheginput('#ubsigmadetails').eq.1) then
         call wz_newunit(iun)

      subroutine dijet_btildevirt(resvirt)
      implicit none
     ...
     #     loglog
      ...
      logical ini
      data ini/.true./
      real * 8 dijet_DDILOG,dijet_dotp
      external dijet_DDILOG,dijet_dotp
      save c,gamma,gammap,ini
      real * 8 tiny
      parameter (tiny=1d-14)
      real * 8 ll
      real * 8 dijet_Intm_ep,dijet_Int0m_0,dijet_Int0m_ep,dijet_Intmm_0,dijet_Intmm_ep
      external dijet_Intm_ep,dijet_Int0m_0,dijet_Int0m_ep,dijet_Intmm_0,dijet_Intmm_ep
...
c get pdfs
      call dijet_pdfcall(1,kn_xb1,pdfb1)
      call dijet_pdfcall(2,kn_xb2,pdfb2)      

      call dijet_sigvirtual(virt_arr)
...
c     loop on final-state massive partons
     ...
     #              (ll-0.5*dijet_Intm_ep(kn_cmpborn(0,leg)))               
            endif
         enddo
         Q=Q*br_born(jb)
         I=0
...
c     massless-massless case
            if (kn_masses(legi).eq.0.and.kn_masses(legj).eq.0) then
               kij=dijet_dotp(kn_cmpborn(0,legi),kn_cmpborn(0,legj))
               ...
               I=I+(1d0/2*ll**2
     #              +ll*log(kij/(2*eij))
     #              -dijet_DDILOG(kij/(2*eij))+1d0/2*log(kij/(2*eij))**2
     #              -loglog)*
...
c     massless-massive case
     ...
     #            +0.5*dijet_Int0m_0(kn_cmpborn(0,legi),kn_cmpborn(0,legj))*ll
     #            -0.5*dijet_Int0m_ep(kn_cmpborn(0,legi),kn_cmpborn(0,legj)))*
     #              br_bornjk(legi,legj,jb)
            endif
c     massive-massless case
            if (kn_masses(legi).gt.0.and.kn_masses(legj).eq.0) then
               I = I + (0.5*(0.5*ll**2 - pi**2/6)
     #            +0.5*dijet_Int0m_0(kn_cmpborn(0,legj),kn_cmpborn(0,legi))*ll
     #            -0.5*dijet_Int0m_ep(kn_cmpborn(0,legj),kn_cmpborn(0,legi)))*
     #              br_bornjk(legj,legi,jb)
            endif
c     massive-massive case
            if (kn_masses(legi).gt.0.and.kn_masses(legj).gt.0) then
               I = I + (0.5*ll*
     #            dijet_Intmm_0(kn_cmpborn(0,legi),kn_cmpborn(0,legj))
     #            -0.5*dijet_Intmm_ep(kn_cmpborn(0,legi),kn_cmpborn(0,legj)))
     #            *br_bornjk(legi,legj,jb)
...
c we only summed over j>i, multiply by 2
     ...
     #       *pdfb1(fl1)*pdfb2(fl2)*kn_jacborn
         tot=tot+resvirt(jb)
      enddo
      if (.not.dijet_pwhg_isfinite(tot)) then
         do jb=1,flst_nborn
            resvirt(jb)=0d0
         enddo
      endif
...
c I_ep(k,m)
      function dijet_Int0m_ep(k,m)
c               /                                           [                  ]
c               |           d phi                           [   k.m       k0   ]
c dijet_Int0m_ep = -2 | d cos th  ----- log(sin th sin phi)  l0^2 [ ------- - ------ ]
c               |             pi                            [ k.l m.l   k.l l0 ]
c               /                                           [                  ]
c the range in phi is 0<phi<pi.
      implicit none
      real * 8 dijet_Int0m_ep, k(0:3),m(0:3)
      ...
      do j=0,3
         ...
         mh(j)=m(j)/m(0)
      enddo
      b=sqrt(1-dijet_dotp(mh,mh))
      khmh=dijet_dotp(kh,mh)
      dijet_Int0m_ep=-2*(log((1-b)/(1+b))**2/4+log(khmh/(1+b))*log(khmh/(1-b))
     # +dijet_DDILOG(1-khmh/(1+b))+dijet_DDILOG(1-khmh/(1-b)))
      end
...
c            /                       [                  ]
c the range in phi is 0<phi<pi.
      function dijet_Int0m_0(k,m)
      implicit none
      real * 8 dijet_Int0m_0, k(0:3),m(0:3)
      ...
      do j=0,3
         ...
         mh(j)=m(j)/m(0)
      enddo
      dijet_Int0m_0=log( dijet_dotp(kh,mh)**2/dijet_dotp(mh,mh) )
      end



      function dijet_Intm_ep(p)
c               /                                              2
c               |           d phi                             p  
c  dijet_Intm_ep = -2 | d cos th  ----- log(sin th sin phi) l0^2  -------
c               |             pi                            (p.l)^2
c               /
c the range in phi is 0<phi<pi.
      implicit none
      real * 8 p(0:3),dijet_Intm_ep
      real * 8 beta2,beta
      integer j
      beta2=0
      do j=1,3
      ...
      enddo
      beta=sqrt(beta2)
      dijet_Intm_ep=2*log((1+beta)/(1-beta))/beta
      end


      function dijet_Intmm_ep(p1,p2)
c               / 
c               |           d phi                             p1.p2  
c dijet_Intmm_ep= -2 | d cos th  ----- log(sin th sin phi) k0^2 ----------
c               |             pi                           p1.k  p2.k
c               /
c p1^2>0, p2^2>0.
c The range in phi is 0<phi<pi.
      implicit none
      real * 8 p1(0:3),p2(0:3), softintmm3,dijet_Intmm_ep
      real * 8 i,z,zm,zmob,zp,z1,z1ob,z2,a,b,x1,x2,b1(3),b2(3),
     1 bb1,bb2,b1b2
      integer j
      real * 8 dijet_DDILOG
      external dijet_DDILOG
      i(z)=-1d0/2*log((z-zm)*(zp-z)/((zp+z)*(z+zm)))**2
     # -2*dijet_DDILOG(2*zm/(zp-zm)*(zp-z)/(zm+z))
     # -2*dijet_DDILOG(-2*zp/(zp-zm)*(zm+z)/(zp-z))
      do j=1,3
         b1(j)=p1(j)/p1(0)
         b2(j)=p2(j)/p2(0)
      enddo
...
c z1 over b (same as before
     ...
     2 +2*dijet_DDILOG(2*zmob/(zp-zm)*(zp-z1)/(zmob+z1ob))
     3 +2*dijet_DDILOG(-2*zp/(zp-zm)*(zm+z1)/(zp-z1)) )
     4  *(1-b1b2)/sqrt(a-b)
      end

      function dijet_Intmm_0(p1,p2)
c            / 
c            |           d phi        p1.p2  
c  dijet_Intmm_0 = | d cos th  ----- k0^2 ----------
c            |             pi       p1.k  p2.k
c            /
c the range in phi is 0<phi<pi.
      implicit none
      real * 8 p1(0:3),p2(0:3), dijet_Intmm_0
      ...
      dijet_Intmm_0=log((1+beta)/(1-beta))/beta
      end

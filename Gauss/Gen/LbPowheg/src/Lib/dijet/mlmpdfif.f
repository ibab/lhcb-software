      subroutine dijet_genericpdf(ndns,ih,xmu2,x,fx)
c Interface to dijet_MLMPDF package.
      implicit none
      integer ndns,ih
      ...
      sx=x
      sxmu2=xmu2
      call dijet_MLMPDF(ndns,ih,sxmu2,sx,sfx,5)
      do j=-5,5
         fx(j)=sfx(j)
      enddo
      fx(1)=sfx(2)
...
c link errors. It is never invoked in the present case.
      subroutine dijet_getq2min()
      end

      subroutine dijet_genericpdfpar(ndns,ih,xlam,scheme,iorder,iret)
      implicit none
      integer ndns,ih
      real * 8 xlam
      character * 2 scheme
      integer iret,iorder
      call dijet_PDFPAR(ndns,ih,xlam,scheme,iret)
c not yet implemented
      iorder=-1
      end

      function dijet_whichpdfpk()
      character * 3 dijet_whichpdfpk
      dijet_whichpdfpk='mlm'
      end

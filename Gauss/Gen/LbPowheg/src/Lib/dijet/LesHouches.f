      subroutine dijet_gen_leshouches
      implicit none
...
c first assign flavours and colour to the underlying Born process
         call dijet_momenta_lh(kn_pborn,nlegborn)
         call dijet_born_lh
         scalup=sqrt(rad_ptsqmin)
      else
c it is an event with radiation
         call dijet_momenta_lh(kn_preal,nlegreal)
         call dijet_born_lh
         ...
         if(em.le.2) then
c dijet_setcolour_rad works with all incoming (or all outgoing) flavours and colours;
...
c and conjugate its colour in output, to make it incoming.
            call dijet_setcolour_rad(coluborn,-flem,flrad,
     #                     icolup(1,em),icolup(1,rad))
            call dijet_colour_conj(icolup(1,em))
         else
...
c conjugate their colours in the output, to make them outgoing.
            call dijet_setcolour_rad(coluborn,-flem,-flrad,
     #                     icolup(1,em),icolup(1,rad))
            call dijet_colour_conj(icolup(1,rad))
            call dijet_colour_conj(icolup(1,em))
         endif
...
c     i1<i2
      subroutine dijet_add_resonance(idpdg,i1,i2)
      implicit none
      include 'src/Lib/dijet/LesHouches.h'
     ...
     #     spinuptmp(maxnup)
      ...
      if (i1.ge.i2) then
         write(*,*) 'wrong sequence in dijet_add_resonance'
         stop
      endif
      do i=i1,nup
         iduptmp(i) = idup(i)
...
c     change mothers of decaying particles
      ...
      subroutine dijet_colour_conj(icol)
      implicit none
      integer icol(2)
      integer itmp
      itmp=icol(2)
      ...
      subroutine dijet_momenta_lh(p,n)
      implicit none
      integer n
      real * 8 p(0:3,n)
      integer k,mu
     ...
     #       -p(2,k)**2-p(3,k)**2))
      ...
      include 'src/Lib/dijet/pwhg_rad.h'
      include 'src/Lib/dijet/LesHouches.h'
c      include 'src/Lib/dijet/PhysPars.h'
...
c gluons are numbered 21 in pdg
         ...
         vtimup(k)=0
      enddo
      call dijet_borncolour_lh
      end


      subroutine dijet_getnewcolor(newcolor)
      implicit none
      integer newcolor
      include 'src/Lib/dijet/LesHouches.h'
      integer j,k
...
c the one where both fl2 and fl3 are gluons, where a 50% dijet_random choice is made
c over the two possible colour connections.
      subroutine dijet_setcolour_rad(colin,fl2,fl3,colout2,colout3)
      implicit none
 ...
 998  continue
      write(*,*)
     #     ' Error in dijet_setcolour_rad: inconsistent colour flavour input'
      ...
      subroutine dijet_displeshouches
      include 'src/Lib/dijet/LesHouches.h'
...
c first conjugate incoming colors
 ...
 22   continue
      ...
      subroutine dijet_pdfreweightinfo(id1,id2,x1,x2,xmufact,xf1,xf2)
      implicit none
      integer id1,id2
...
c dijet_btilde event: pass x1 and x2, id1, id2 etc. of the current underlying Born
      ...
      call dijet_pdfcall(1,x1,pdf)
      xf1=x1*pdf(id1)
      call dijet_pdfcall(2,x2,pdf)
      xf2=x2*pdf(id2)
      if(id1.eq.0) id1=21
      if(id2.eq.0) id2=21
      end

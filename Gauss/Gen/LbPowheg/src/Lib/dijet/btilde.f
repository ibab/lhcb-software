      function dijet_btilde(xx,www0,ifirst)
      implicit none
...
c     for x_1 and x_2, and take away an overall azimuth
      real * 8 dijet_btilde,xx(ndiminteg),www0
      real * 8 xrad(3)
...
c     sets xscaled, y, phi in kinematics common block
         call dijet_btildeborn(resborn)
         if (.not.flg_bornonly) then
            call dijet_btildevirt(resvirt)
            call dijet_btildecoll(xrad,rescoll,www)
            call dijet_btildereal(xrad,resreal,www)
         endif
c     accumulate values
         dijet_btilde=0
         do j=1,flst_nborn
...
c     initial value in results
            ...
            dijet_btilde=dijet_btilde+tmp*www*suppfact
         enddo
      elseif(ifirst.eq.1) then
c     subsequent calls:
c     In case of folding the call to dijet_btildeborn and dijet_btildevirt can be
...
c     to the NLO dijet_analysis routine.
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
c dijet_btildecoll and dijet_btildereal take care themselves to invoke the NLO
c dijet_analysis if required.
            call dijet_btildecoll(xrad,rescoll,www)
            call dijet_btildereal(xrad,resreal,www)
         endif
         dijet_btilde=0
...
c output Born
            ...
            call dijet_analysis_driver(totborn,0)
            if(.not.flg_bornonly) then
c output virtual
               totvirt=0d0
               do j=1,flst_nborn
               ...
               call dijet_analysis_driver(totvirt,0)
            endif
c closing call to end a sequence of correlated events in the
c dijet_analysis routines.
            call dijet_pwhgaccumup
         endif
c     closing call: accumulate values with correct signs
         dijet_btilde=0
         call dijet_adduptotals(results,flst_nborn)
         do j=1,flst_nborn
...
c interface will simply output this sign for the event.)
            ...
            dijet_btilde=dijet_btilde+results(j)
c     Transfer all flavour components of dijet_btilde to the array
c     in common block; will be used to decide the underlying
c     flavour of the event
            rad_btilde_arr(j)=results(j)
         enddo
      else
         write(*,*) 'wrong value of ifirst in dijet_btilde => ',ifirst
         stop
      endif
      end
      

      subroutine dijet_adduptotals(results,n)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      integer n
      real * 8 results(n)
     ...
     3     etotposj(maxprocborn),etotnegj(maxprocborn)
      integer nentries
      common/dijet_cadduptotals/tot,totabs,totpos,totneg,etot,etotabs,
...
c
      if(dijet_powheginput('#ubsigmadetails').eq.1) then

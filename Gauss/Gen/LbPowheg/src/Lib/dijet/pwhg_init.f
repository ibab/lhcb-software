      ...
      integer i1,n1,n2
      call dijet_init_flsttag
      flg_debug=.false.
      if(dijet_powheginput("#flg_debug").eq.1) flg_debug=.true.
c whether to output negative weights or not
      flg_withnegweights=.false.
      if(dijet_powheginput("#withnegweights").eq.1)flg_withnegweights=.true.
c See if we have weighted events
      flg_weightedev=.false.
      if(dijet_powheginput("#bornsuppfact").gt.0) flg_weightedev=.true.
      if(dijet_powheginput("#ptsupp").gt.0) then
...
c     amplitudes, for Born, real and virtual contributions
      flg_smartsig=.true.
      if(dijet_powheginput("#smartsig").eq.0) flg_smartsig=.false.
c     if true also counterterm are output in NLO tests (default)
      flg_withsubtr=.true.
      if(dijet_powheginput("#withsubtr").eq.0) flg_withsubtr=.false.
...
c     is required
      flg_withdamp=.false.
      if((dijet_powheginput("#withdamp").eq.1).or.
     $  (dijet_powheginput("#hfact").gt.0d0))   flg_withdamp=.true.
c     If set do only the Born term
      flg_bornonly=.false.
      if (dijet_powheginput("#bornonly").eq.1) flg_bornonly=.true.
c     initialize dijet_random number sequence
      i1=dijet_powheginput('#iseed')
      if (i1.lt.0) i1=0
      n1=dijet_powheginput('#rand1')
      if (n1.lt.0) n1=0
      n2=dijet_powheginput('#rand2')
      if (n2.lt.0) n2=0
c select which upper bounding function form
      rad_iupperisr=dijet_powheginput("#iupperisr")
...
c info on pdf for each event
      ...
      if(dijet_powheginput("#pdfreweight").eq.1)flg_pdfreweight=.true.
c
      call dijet_setrandom(i1,n1,n2)
...
c     inside the user-defined subroutine dijet_init_processes
      lprup(1)=10001
c     initialize physical parameters
      call dijet_init_phys

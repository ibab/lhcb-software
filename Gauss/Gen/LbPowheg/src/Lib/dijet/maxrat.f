      subroutine dijet_do_maxrat(mcalls,icalls)
      implicit none
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      real * 8 dijet_powheginput,dijet_random
      external dijet_powheginput,dijet_random      
      call dijet_zerohistnorms
      loaded=.false.
      nubound=dijet_powheginput('nubound')
      rad_nynorms=dijet_powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
      rad_ncsinorms=dijet_powheginput('#icsimax')
     ...
     #  ' error in poweg.input: rad_nynorms or rad_ncsinorms too large'
      ...
      rad_normfact=dijet_powheginput('#xupbound')
      ...
      if(dijet_powheginput('use-old-ubound').eq.1) then
         call dijet_newunit(iun)
...
c Compute and store normalization grid
 111  continue
     ...
     # ' bounding function for radiation'
      ...
      enddo
      do j=1,nubound
         call dijet_gen_born(mcalls,icalls)
         call dijet_gen_uborn_idx
         kn_csitilde=dijet_random()
         kn_y=2*dijet_random()-1
         kn_azi=2*pi*dijet_random()
         do rad_kinreg=1,rad_nkinreg
            if(rad_kinreg.eq.1.and.rad_kinreg_on(1)) then
c     initial state radiation
               call dijet_gen_real_phsp_isr_rad0
               call dijet_inc_norms
            elseif(rad_kinreg_on(rad_kinreg)) then
c     final state radiation
               call dijet_gen_real_phsp_fsr_rad0
               call dijet_inc_norms
            endif
         enddo    
      enddo
...
c      call maxratident
      ...
      common/dijet_chistnorms/histnorms,efficiencies
      integer icsi,iy,ind,k
      logical dijet_refuse_pdf
      real * 8 dijet_pwhg_pt2,dijet_pwhg_upperb_rad
      external dijet_pwhg_pt2,dijet_pwhg_upperb_rad
      logical debug
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.rad_ptsqmin) then
         call dijet_set_rad_scales(ptsq)
         if(.not.dijet_refuse_pdf()) then
            ...
            if(born.le.0) then
               if(verbose) then
               write(*,*) ' *****************************'
               write(*,*) ' dijet_inc_norms: Warning, born zero'
               write(*,*) ' RAD REG: ', rad_kinreg
               write(*,*) ' dijet_inc_norms: x1,x2', kn_xb1,kn_xb2
     ...
     $              ,rad_ubornidx)).ge.4)) then
                  ...
                  write(*,*) ' dijet_inc_norms: hvq in initial state!'
               else
                  write(*,*) ' dijet_inc_norms: more serious problem!'
               endif
               write(*,*) ' *****************************'
               endif
               return
            endif
            xnorm=sig/born/dijet_pwhg_upperb_rad()
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
      ...
      subroutine dijet_printhistnorms
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      include 'src/Lib/dijet/pwhg_rad.h'
     ...
     1              efficiencies(ireg,iborn)/dijet_powheginput('nubound')/
...
c     1           dijet_powheginput('nubound')/rad_norms(ireg,iborn)
c         enddo
c      enddo
      close(iun)
      end

      subroutine dijet_zerohistnorms
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      include 'src/Lib/dijet/pwhg_rad.h'
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      do k=1,2
         if(k.eq.1) then
            ...
            x=kn_xb1
            call dijet_pdfcall(1,x,pdf)
         else
            fl=flst_born(2,rad_ubornidx)
            x=kn_xb2
            call dijet_pdfcall(2,x,pdf)
         endif
         sn=0
         do j=1,5
            sn=sn+pdf(j)+pdf(-j)
      ...
      subroutine dijet_uboundfct(fct,csi,y)
      implicit none
      include 'src/Lib/dijet/pwhg_math.h'
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
...
c
      ...
      subroutine dijet_gen_born(mcalls,icalls)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
     ...
     1     itmx1,itmx2,imode,j,k,iun
      ...
      real * 8 dijet_btilde,dijet_powheginput
      external dijet_btilde,dijet_powheginput
...
c will not be used;
         ...
         call dijet_mint(dijet_btilde,ndimbornint,ncall1,itmx1,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         close(iun)
         imode=1
         ncall2=dijet_powheginput('ncall2')
         itmx2=dijet_powheginput('itmx2')
         call dijet_mint(dijet_btilde,ndimbornint,ncall2,itmx2,ifold,imode,iun,
     1        xgrid,xint,ymax,sigbtl,errbtl)
         call dijet_gen(dijet_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,0,
     1        mcalls,icalls,xx)
         ini=.false.
      endif
C - N.B. if -fno-automatic isn't used the ifold array may need reset to 1 here.
      call dijet_gen(dijet_btilde,ndimbornint,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)

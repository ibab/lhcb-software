      subroutine dijet_bornzerodamp(alr,r0,rc,rs,dampfac)
...
c the real contribution to implement Born zero suppression
      ...
      integer alr
      real * 8 r0,rc,rs,dampfac,h,dijet_pwhg_pt2,pt2,dijet_powheginput
      logical ini
      data ini/.true./
      save ini,h
      external dijet_pwhg_pt2,dijet_powheginput
      if(ini) then
         h=dijet_powheginput("#hfact")
         if(h.gt.0) then
            write(*,*)'***************************************'
...
c      write(*,*) ' dijet_bornzerodamp: r0=',r0,'  rc=',rc,'  rs=',rs,
c     # ' dampfac=',dampfac
      end

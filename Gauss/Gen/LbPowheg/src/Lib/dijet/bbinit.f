     ...
     #                  ,xmmm(0:50,ndiminteg),xmmmrm(0:50,ndiminteg)
      ...
      common/dijet_cgengrids/xgrid,ymax,xmmm,xgridrm,ymaxrm,xmmmrm,
     #                ifold,ifoldrm
      ...
      external dijet_btilde,dijet_sigremnant,dijet_powheginput
      do j=1,ndiminteg
         do k=0,50
            xgrid(k,j)=0
            xgridrm(k,j)=0
     ...
     1        rnd_cwhichseed//'.dat',status='unknown')
      ...
      ncall1=dijet_powheginput('ncall1')
      ncall2=dijet_powheginput('ncall2')
      itmx1=dijet_powheginput('itmx1')
      itmx2=dijet_powheginput('itmx2')
      iret1=0
      iret2=0
      call dijet_loadgrids(iret1,xgrid,ymax,xgridrm,ymaxrm,
     2     ifold,ifoldrm)
      if(iret1.ne.0) call dijet_loadxgrid(iret2,xgrid,xint,xgridrm,xintrm)
      if(iret2.ne.0) then
         write(*,*)
     ...
     #        status='unknown')
         ...
         flg_nlotest=.false.
         imode=0
         call dijet_mint(dijet_btilde,ndiminteg,ncall1,itmx1,ifold,imode,iun,
     ...
     #        status='unknown')
            imode=0
            call dijet_mint(dijet_sigremnant,ndiminteg,ncall1,itmx1,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
            close(iun)
         endif
         call dijet_storexgrid(xgrid,xint,xgridrm,xintrm)
...
c importance sampling grids have been initialized with default dijet_SEED;
c they must all be the same. Now set current dijet_SEED, if required
      if(rnd_cwhichseed.ne.'none')
     1     call dijet_setrandom(rnd_initialseed,rnd_i1,rnd_i2)
      if(iret1.ne.0) then
         if (dijet_powheginput('#testplots').eq.1d0) call dijet_init_hist
c set  up the folding here, if required
         ifold(ndiminteg-2) = dijet_powheginput("foldcsi")
         ifold(ndiminteg-1) = dijet_powheginput("foldy")
         ifold(ndiminteg)   = dijet_powheginput("foldphi")
...
c Results in rad_tot???btl do not depend upon flg_withnegweights
         call dijet_resettotals
         call dijet_mint(dijet_btilde,ndiminteg,ncall2,itmx2,ifold,imode,iun,
     #        xgrid,xint,ymax,sigbtl,errbtl)
         call dijet_finaltotals
c finalize dijet_btilde output in histograms
         call dijet_pwhgaddout
         flg_nlotest=.false.
         write(*,*) 'dijet_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(*,*) 'dijet_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(*,*) 'dijet_btilde total (pos.-|neg.|):', rad_totbtl,' +-',
     1        rad_etotbtl
         write(iunstat,*) 'dijet_btilde pos.   weights:', rad_totposbtl,' +-',
     1        rad_etotposbtl
         write(iunstat,*) 'dijet_btilde |neg.| weights:', rad_totnegbtl,' +-',
     1        rad_etotnegbtl
         write(iunstat,*) 'dijet_btilde Total (pos.-|neg.|):', rad_totbtl,
     1        ' +-',rad_etotbtl
c Now compute the remnant contributions
     ...
     #                ' and/or regular remnants'
            flg_nlotest=.true.
            imode=1
            call dijet_mint(dijet_sigremnant,ndiminteg,ncall2,itmx2,ifoldrm,imode,
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
...
c dijet_btilde and remnant event are chosen in proportion to
...
c        
         call dijet_storegrids(xgrid,ymax,xgridrm,ymaxrm,ifold,ifoldrm,
     1        ncall2,itmx2)
c Output NLO histograms
         if (dijet_powheginput('#testplots').eq.1d0) then
            if(rnd_cwhichseed.eq.'none') then
     ...
     1           rad_totrm,'+-',rad_etotrm
         ...
         if (flg_weightedev) then
            write(iunstat,*) 
     1 ' total (dijet_btilde+remnants) cross section times '
            write(iunstat,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
         else
            write(iunstat,*) 
     1 ' total (dijet_btilde+remnants) cross section in pb',
     2     rad_tot,'+-',rad_etot
         endif
     ...
     1        rad_totrm,'+-',rad_etotrm
      ...
      if (flg_weightedev) then
         write(*,*) ' total (dijet_btilde+remnants) cross section times '
         write(*,*) ' suppression factor in pb',
     1        rad_tot,'+-',rad_etot         
      else
         write(*,*) ' total (dijet_btilde+remnants) cross section in pb',
     1     rad_tot,'+-',rad_etot
      endif
     ...
     1     rad_totnegbtl/(2*rad_totnegbtl+rad_tot)
      close(iunstat)
      call flush
c initialize dijet_gen; the array xmmm is set up at this stage.
      call dijet_gen(dijet_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,0,
     #    mcalls,icalls,xx)
c compute normalization of upper bounding function for radiation
      call dijet_do_maxrat(mcalls,icalls)
c print statistics
      call dijet_gen(dijet_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,3,
     #    mcalls,icalls,xx)
      if(xx(1).gt.0) write(*,*) 'POWHEG: efficiency in the generation'
     # //' of the Born variables =',xx(1)
      if((flg_withreg.or.flg_withdamp).and..not.flg_bornonly) then
c initialize dijet_gen for remnants
         call dijet_gen(dijet_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            0,mcalls,icalls,xx)
c     save dijet_random number seeds
         call dijet_randomsave
c generate few events from remnants, just to determine the generation efficiency
         do j=1,int(min(dijet_powheginput('nubound'),10d0))
            call dijet_gen(dijet_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,
     #            1,mcalls,icalls,xx)
         enddo
c     restore  dijet_random number seeds
...
c     print statistics
     ...
     #                   //' of the remnant variables =',xx(1)
      ...
      subroutine dijet_gen_btilde(mcalls,icalls)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)      
      real * 8 dijet_btilde
      external dijet_btilde
      call dijet_gen(dijet_btilde,ndiminteg,xgrid,ymax,xmmm,ifold,1,
     #    mcalls,icalls,xx)
      end

      subroutine dijet_gen_sigremnant
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      real * 8 xgrid(0:50,ndiminteg),ymax(50,ndiminteg)
     ...
     #                ifold,ifoldrm
      real * 8 xx(ndiminteg)
      integer mcalls,icalls
      real * 8 dijet_sigremnant
      external dijet_sigremnant
      call dijet_gen(dijet_sigremnant,ndiminteg,xgridrm,ymaxrm,xmmmrm,ifoldrm,1,
     #    mcalls,icalls,xx)
      end


      subroutine dijet_storegrids(xgrid,ymax,xgridrm,ymaxrm,
     ...
     8     rad_tot,rad_etot
      ...
      subroutine dijet_loadgrids(iret,xgrid,ymax,xgridrm,ymaxrm,
     #           ifold,ifoldrm)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
...
c
      ...
      if(dijet_powheginput('use-old-grid').eq.0) then
         iret=1
         return
      endif
      iret=0
...
c dijet_random seeds
     ...
     1              chseed//'.dat'
         ...
         if(dijet_powheginput("#ncallfrominput").eq.1) then
            ncall2=dijet_powheginput("ncall2")
            itmx2=dijet_powheginput("itmx2")
            ncall2=ncall2*itmx2
         endif
 ...
 998  continue
      ...
      subroutine dijet_storexgrid(xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      include 'src/Lib/dijet/pwhg_kn.h'
     ...
     #     form='unformatted',status='unknown')
      ...
      subroutine dijet_loadxgrid(iret,xgrid,xint,xgridrm,xintrm)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      include 'src/Lib/dijet/pwhg_kn.h'

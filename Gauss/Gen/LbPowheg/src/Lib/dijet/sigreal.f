      subroutine dijet_btildereal(xrad,resreal,www)
      implicit none
...
c output values for dijet_analysis_driver
         out0=0
         out1=0
c check that emitter is valid
         if(dijet_valid_emitter(kn_emitter)) then
            if(kn_emitter.gt.2) then
c     final state radiation
               call dijet_gen_real_phsp_fsr(xrad,jac_over_csi,
...
c to zero. 
               call dijet_sigreal_btl(r0)
               if(flg_withsubtr) then
                  call dijet_collfsr(rc)
c     dijet_soft subtraction
                  call dijet_soft(r0s)
                  call dijet_softcollfsr(rcs)
     ...
     #jac_over_csi_soft)
               ...
               if(flg_withsubtr) then
                  ...
                  if(kn_emitter.ne.2) then
                     call dijet_collisrp(rp)
                     call dijet_softcollisrp(rps)
                  endif
                  if(kn_emitter.ne.1) then
                     call dijet_collisrm(rm)
                     call dijet_softcollisrm(rms)
                  endif
...
c     to avoid divergent integral (25 is an ad hoc value)
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
         ...
         if(flg_nlotest) then
            out0=out0*www
            out1=out1*www
            if(out0.ne.0d0) call dijet_analysis_driver(out0,0)
            if(out1.ne.0d0) call dijet_analysis_driver(out1,1)
         endif
      enddo
      end

      subroutine dijet_checklims(iun)
      implicit none
      integer iun
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      ...
      external dijet_sigreal_btl,dijet_soft,dijet_collfsr,dijet_softcollfsr, dijet_collisrp,
     $     dijet_softcollisrp,dijet_collisrm,dijet_softcollisrm
      call dijet_randomsave
      if(dbg_softtest) then
         write(iun,*) '******************************************'   
         write(iun,*) '           CHECK  dijet_soft LIMITS             '     
         write(iun,*)
         do kn_emitter=0,nlegborn
            call dijet_checksoft(dijet_sigreal_btl,dijet_soft,' dijet_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
      endif
      ...
      if(dbg_colltest) then
         ...
         do kn_emitter=3,nlegborn
            call dijet_checkcoll(dijet_sigreal_btl,dijet_collfsr,1,' coll',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
c            call dijet_randomsave
            if(kn_emitter.ne.2) call dijet_checkcoll(dijet_sigreal_btl,dijet_collisrp,1
     $           ,' coll-plus',iun)
c            call dijet_randomrestore
            if(kn_emitter.ne.1) call dijet_checkcoll(dijet_sigreal_btl,dijet_collisrm,-1
     $           ,' coll-minus',iun)
         enddo
         write(iun,*) '******************************************'   
         write(iun,*)
      ...
      if(dbg_softtest.and.dbg_colltest) then  
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  dijet_soft-COLL. LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call dijet_checksoft(dijet_collfsr,dijet_softcollfsr,' dijet_soft-coll',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  dijet_soft-COLL. LIMITS FOR ISR +   '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2)call dijet_checksoft(dijet_collisrp,dijet_softcollisrp,
     $           ' dijet_soft-coll-plus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         ...
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.1)call dijet_checksoft(dijet_collisrm,dijet_softcollisrm,
     $           ' dijet_soft-coll-minus',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         ...
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-dijet_soft LIMITS FOR FSR     '         
         write(iun,*)
         do kn_emitter=3,nlegborn
            call dijet_checkcoll(dijet_soft,dijet_softcollfsr,1,' coll-dijet_soft',iun)
         enddo
         write(iun,*) '******************************************'
         write(iun,*)
         write(iun,*) '******************************************'   
         write(iun,*) '   CHECK  COLL.-dijet_soft LIMITS FOR ISR     '         
         write(iun,*)
         do kn_emitter=0,2
            if(kn_emitter.ne.2) call dijet_checkcoll(dijet_soft,dijet_softcollisrp,1
     $           ,' coll-plus-dijet_soft',iun)
            if(kn_emitter.ne.1) call dijet_checkcoll(dijet_soft,dijet_softcollisrm,-1
     $           ,' coll-minus-dijet_soft',iun)
         enddo
      endif
      call dijet_randomrestore
      end

      subroutine dijet_checkborn(iun)
...
c are consistent with total Born
     ...
     3                 (flst_born(k,iborn),k=1,nlegborn)
      ...
      subroutine dijet_checksoft(sig,sigs,label,iun)
      implicit none
      include 'src/Lib/dijet/pwhg_dbg.h'
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
     ...
     #r0s(maxalr,nexp),jac_over_csi_p,jac_over_csi_m
      ...
      real * 8 dijet_random,dijet_dotp
      external dijet_random,dijet_dotp
      logical dijet_valid_emitter,iszero,isnonzero,isequal
      external dijet_valid_emitter
      do j=1,ndiminteg-3
         xborn(j)=dijet_random()
      enddo
      call dijet_gen_born_phsp(xborn)
      call dijet_setscalesbtilde
      call dijet_allborn
      call dijet_checkborn(iun)
c      write(iun,*)' mass',sqrt(2*dijet_dotp(kn_pborn(0,3),kn_pborn(0,4)))
      do j=1,3
         xrad(j)=dijet_random()
      enddo
c Check dijet_soft limits
      if(dijet_valid_emitter(kn_emitter)) then
         write(iun,*) ' dijet_random Born variables ====> ',xborn
          write(iun,*) ' dijet_random radiation variables ====> ',xrad
          do jexp=1,nexp
            xrad(1)=10d0**(-jexp)
            if(kn_emitter.gt.2) then
               call dijet_gen_real_phsp_fsr(xrad,jac_over_csi,
     $              jac_over_csi_coll,jac_over_csi_soft)
               ...
               call dijet_gen_real_phsp_isr (xrad,jac_over_csi,jac_over_csi_p,
     $              jac_over_csi_m,jac_over_csi_soft)
            endif
            write(iun,*) '### Check dijet_soft',xrad(1)
            call sig(r0(1,jexp))
...
c               if(r0s(alr,jexp).eq.0) iszero=.true.
     ...
     $             r0s(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine dijet_checkcoll(sig,sigc,idir,label,iun)
      implicit none
      integer iun
      include 'src/Lib/dijet/pwhg_dbg.h'
      include 'src/Lib/dijet/nlegborn.h'
         ...
         do alr=1,flst_nalr
            do jexp=1,nexp
               if(r0c(alr,jexp).ne.r0c(alr,1)) then
                  write(iun,*)
     #' dijet_checklims error : coll lim depends upon coll variable'
               endif
            enddo
         enddo
         do alr=1,flst_nalr
...
c     Added this 'if' to be sure that no division by zero occurs
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      subroutine dijet_sigreal_rad(sig)
      implicit none
      real * 8 sig
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      ...
      real * 8 ptsq,dijet_pwhg_pt2
      logical computed(maxalr)
      logical condition
      logical ini
      data ini/.true./
      save ini,equivto,equivcoef
      external dijet_pwhg_pt2
      if(ini) then
         do alr=1,flst_nalr
            equivto(alr)=-1
         enddo
         if(flg_smartsig) then
            call dijet_randomsave
c     generate "nmomset" dijet_random real-phase space configurations
            call dijet_fillmomenta(nlegreal,nmomset,kn_masses,preal)
            do alr=1,flst_nalr
               do j=1,nmomset
                  call dijet_realgr(
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call dijet_compare_vecs(nmomset,alr,res,1,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      ...
      if(flg_withdamp) then
         call dijet_collrad(rc)
         call dijet_softrad(rs)
      endif
      do alr=1,flst_nalr
         computed(alr)=.false.
      enddo
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               call dijet_realgr(flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  r=r0(alr)
                  call dijet_bornzerodamp(alr,r,rc(alr),rs(alr),dampfac)
 ...
 995     continue
      ...
      subroutine dijet_sigreal_btl(r0)
      implicit none
      real * 8 r0(*)
      call dijet_sigreal_btl0(r0,0)
      end

c Real cross section, required by dijet_btilde;
...
c    csi^2 (1-y)   for FSR regions
      subroutine dijet_sigreal_btl0(r0,imode)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      include 'src/Lib/dijet/pwhg_kn.h'
...
c     generate "nmomset" dijet_random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
               enddo
               call dijet_compare_vecs(nmomset,alr,res,0,alrpr,cprop,iret)
               if(iret.eq.0) then
c     they are equal:
                  equivto(alr)=alrpr
                  equivcoef(alr)=1
...
c End initialization phase; compute graphs
      ...
      enddo
      if(flg_withdamp) then
         call dijet_collbtl(rc)
         call dijet_softbtl(rs)
      endif
      do alr=1,flst_nalr
...
c First mark as being computed
               markused(alr)=1
               call dijet_realgr(flst_alr(1,alr),kn_preal,r0(alr))
...
c supply Born zero damping factor, if required
               if(flg_withdamp) then
                  ...
                  elseif(imode.eq.1) then
                     ...
                     write(*,*) ' dijet_sigreal_btl0: improper call'
                  endif
               endif
            else
               if(markused(equivto(alr)).ne.1) then
                  write(*,*) ' error: dijet_sigreal_btl flg_smartsig bug'
                  call exit(1)
               endif
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
            endif
...
c include pdf's
      ...
      subroutine dijet_fillmomenta(nlegreal,nmomset,kn_masses,preal)
      implicit none
      ...
      do j=1,nmomset
         do k=3,nlegreal-1
            do mu=1,3
               preal(mu,k,j)=dijet_random()
            enddo
         enddo
         preal(3,nlegreal,j)=dijet_random()
...
c We should have the masses as a function of flavour id, somewhere!
     ...
     #         preal(1,k,j)**2+preal(2,k,j)**2+preal(3,k,j)**2)
      ...
      subroutine dijet_compare_vecs(nmomset,alr,res,imode,alrpr,cprop,iret)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      real * 8 ep
...
c imode=0 when called from dijet_btilde,
...
c are not considered.
 ...
 10      continue
      ...
      subroutine dijet_realgr(rflav,p,res)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      integer rflav(nlegreal)
      real * 8 p(0:3,nlegreal),res
      call dijet_real_ampsq(p,rflav,res)
c flux factor
      res=res/(8*p(0,1)*p(0,2))
      if(res.eq.0) then
c         write(*,*) 'dijet_realgr:', rflav
         continue
      endif
      end
      ...
      subroutine dijet_real_ampsq(p,rflav,amp2)
      implicit none
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      include 'src/Lib/dijet/pwhg_math.h'
      ...
      integer rflav(nlegreal)
      real * 8 amp2 
      logical dijet_pwhg_isfinite
      external dijet_pwhg_isfinite
      call dijet_setreal(p,rflav,amp2)
c     check if amp2 is finite
      if (.not.dijet_pwhg_isfinite(amp2)) amp2=0d0

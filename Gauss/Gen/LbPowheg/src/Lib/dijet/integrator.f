...
c Documented in arXiv:0709.2085: dijet_mint: A Computer program for adaptive Monte Carlo
...
c     state variables must be passed as arguments). In this way, one can use dijet_mint
c     and dijet_gen for the integration-generation of different functions, provided
c     the arguments are kept different.
c

      subroutine dijet_mint(fun,ndim,ncalls0,nitmax,ifold,imode,iun,
...
c     of each other these 5 groups. Suppose, for example, that we choose a dijet_random point
...
c     integration grid; initialized and updated with the call to dijet_mint with imode=0
c 
c xint: real
...
c     integrand upper bounds, set up by dijet_mint when called with imode=1, to be used
c     by the subroutine dijet_gen for the generation of unweighted events
c
c ans: real
...
c
     ...
     #  ans,err
      ...
      real * 8 dijet_random
      external dijet_random,fun
      if(ndim.gt.ndiminteg) then
         write(*,*) 'dijet_mint: at most ',ndiminteg,' dimensions'
         write(*,*) 'Got ',ndim
         call exit(-1)
      endif
      if(imode.eq.0) then
      ...
      elseif(imode.eq.1) then
         do kdim=1,ndim
            ...
            if(nintcurr*ifold(kdim).ne.nintervals) then
               write(*,*)
     # 'dijet_mint: the values in the ifold array shoud be divisors of',
     #  nintervals
               stop
            endif
            do kint=1,nintcurr
 ...
 10   continue
      ...
      etot=0
      do kpoint=1,ncalls
c find dijet_random x, and its dijet_random cell
         do kdim=1,ndim
            kfold(kdim)=1
            ncell(kdim)=nintervals/ifold(kdim)*dijet_random()+1
            rand(kdim)=dijet_random()
         enddo
         f=0
         ifirst=0
 1       continue
...
c this accumulated value will not be used
            ...
            ifirst=1
            call dijet_nextlexi(ndim,ifold,kfold,iret)
            if(iret.eq.0) goto 1
...
c iteration is finished; now rearrange the grid
         do kdim=1,ndim
            call dijet_regrid(xacc(0,kdim),xgrid(0,kdim),
...
c nitmax*ncalls calls.
     ...
     1     +(nit-1)*(ans-vtot)**2/(ncalls*nit**3))
      ...
      subroutine dijet_regrid(xacc,xgrid,nhits,kdim,nint,nit,iun)
      implicit none
...
c 1    continue
c      write(*,*) (kkk(j),j=1,3)
c      call dijet_nextlexi(ndim,iii,kkk,iret)
c      if(iret.eq.0) then
c         goto 1
c      endif
c      end

      subroutine dijet_nextlexi(ndim,iii,kkk,iret)
...
c subsequent calls to dijet_nextlexi return
...
c     the returned coordinate vector of the generated point
     ...
     #         ymax(nintervals,ndim),x(ndim)
      ...
      integer icell(ndimmax),ncell(ndimmax)
      integer ifold(ndimmax),kfold(ndimmax)
      real * 8 r,f,ubound,vol,dijet_random,xmmm(nintervals,ndimmax)
      real * 8 rand(ndimmax)
      external fun,dijet_random
...
c get final value (x and vol not used in this call)
      f=fun(x,vol,2)
      if(f.lt.0) then
         write(*,*) 'dijet_gen: non positive function',f
         f=fun(x,vol,2)
c         stop
      endif
      if(f.gt.ubound) then

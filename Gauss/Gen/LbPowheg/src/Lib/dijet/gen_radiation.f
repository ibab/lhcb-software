      ...
      integer iret,iun
      real * 8 suppfact
      real * 8 dijet_random,dijet_powheginput
      external dijet_random,dijet_powheginput
      integer mcalls,icalls
      real * 8 dijet_pwhg_pt2,dijet_pt2max_regular
      external dijet_pwhg_pt2,dijet_pt2max_regular
      integer i
      if(idwtup.eq.3) then
         xwgtup=1
...
c     store current dijet_random seeds. To be used to restart at problematic events
      call dijet_savecurrentrandom
      if(dijet_random().gt.rad_totrm/rad_totgen) then
c     generate underlying Born kinematics
         call dijet_gen_btilde(mcalls,icalls)
c     generate underlying Born flavour
         call dijet_gen_uborn_idx
c
         if(dijet_powheginput('#dijet_testsuda').eq.1) then
            call dijet_testsuda
         endif
c generate radiation
         call dijet_gen_radiation
c add a dijet_random azimuthal rotation around beam axis
         call dijet_add_azimuth
c --- set up les houches interface
         rad_pt2max=dijet_pwhg_pt2()
         call dijet_gen_leshouches
c if negative weight, flip the sign of xwgtup
         if(rad_btilde_sign(rad_ubornidx).eq.-1) then
            xwgtup=-xwgtup
         endif
c rad_type=1 for dijet_btilde events (used only for debugging purposes)
         rad_type=1
         if(flg_weightedev) then
            call dijet_born_suppression(suppfact)
...
c iret=2: reg contribution (real graphs without singular regions)
         call dijet_gen_remnant(iret)
c         if (dijet_pwhg_pt2().lt.rad_ptsqmin) then
c            write(*,*) '****************************************'
c            write(*,*) 'WARNING in dijet_gen_remnant'
c            write(*,*) 'dijet_pwhg_pt2 < rad_ptsqmin ',
c     #           dijet_pwhg_pt2(),' < ',rad_ptsqmin
c            write(*,*) (flst_alr(i,rad_realalr),i=1,nlegreal)
c            write(*,*) 'To generate this event, use the following seeds'
c            call dijet_printcurrentrandom
c            write(*,*) '****************************************'
c         endif
...
c     set st_muren2 equal to pt2 for scalup value
            rad_pt2max=max(rad_ptsqmin,dijet_pwhg_pt2())
            call dijet_set_rad_scales(rad_pt2max)
            call dijet_gen_leshouches
            call dijet_born_suppression(suppfact)
            if(suppfact.eq.0) then
               write(*,*) ' 0 suppression factor in event generation'
...
c     set st_muren2 for scalup value for regular contributions
            rad_pt2max=max(rad_ptsqmin,dijet_pt2max_regular())
            call dijet_set_rad_scales(rad_pt2max)
            call dijet_gen_leshouches_reg
c rad_type=3 for regular contributions
      ...
      subroutine dijet_gen_radiation
      implicit none
      include 'src/Lib/dijet/pwhg_math.h'
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      ...
      real * 8 tmax
      common/dijet_ctmax/tmax
      integer kinreg,firstreg,lastreg
      logical ini
      data ini/.true./
      real * 8 dijet_pwhg_pt2,dijet_powheginput
      external dijet_pwhg_pt2,dijet_powheginput
      save ini,firstreg,lastreg
      if(ini) then
         firstreg=dijet_powheginput("#radregion")
...
c     initial state radiation
               call dijet_gen_rad_isr(t)
            else
c     final state radiation
               call dijet_gen_rad_fsr(t)
            endif
            if(t.gt.tmax) then
               tmax=t
               kinreg=rad_kinreg
...
c Generate a Born like event
         ...
         t=tmax
         if(rad_kinreg.eq.1) then
            call dijet_gen_real_phsp_isr_rad
         else
            call dijet_gen_real_phsp_fsr_rad
         endif
         call dijet_set_rad_scales(t)
         call dijet_sigreal_rad(sig)
         call dijet_gen_real_idx
      endif
      end



      function dijet_pwhg_pt2()
      implicit none
      real * 8 dijet_pwhg_pt2
      include 'src/Lib/dijet/pwhg_math.h'
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      include 'src/Lib/dijet/pwhg_kn.h'
      include 'src/Lib/dijet/pwhg_rad.h'
      if(rad_kinreg.eq.1) then
         dijet_pwhg_pt2=(kn_sreal/4)*(1-kn_y**2)*kn_csi**2
      else
         dijet_pwhg_pt2=(kn_sreal/2)*(1-kn_y)*kn_csi**2
      endif
      end



      function dijet_pwhg_upperb_rad()
      implicit none
      real * 8 dijet_pwhg_upperb_rad
      include 'src/Lib/dijet/pwhg_math.h'
      include 'src/Lib/dijet/nlegborn.h'
      include 'src/Lib/dijet/pwhg_flst.h'
      include 'src/Lib/dijet/pwhg_kn.h'
      ...
      if(rad_kinreg.eq.1) then
         if(rad_iupperisr.eq.1) then
            dijet_pwhg_upperb_rad = st_alpha/((1-x)*(1-y**2))
c Possible alternatives:
c rad_iupper=2   dijet_pwhg_upperb_rad = st_alpha/(x*(1-x)*(1-y**2))
c 
c rad_iupper=3:  dijet_pwhg_upperb_rad = st_alpha/(x**2*(1-x)*(1-y**2))
         else
...
c for now use the same
         if(rad_iupperfsr.eq.1) then
            dijet_pwhg_upperb_rad = st_alpha/(csi*(1-y))
         elseif(rad_iupperfsr.eq.2) then
            dijet_pwhg_upperb_rad = st_alpha/(csi**2*(1-y)*(1-csi/2*(1-y))**2)
     2          *csi
...
c We use it to find its zero in pt2.
      ...
      include 'src/Lib/dijet/pwhg_rad.h'
      include 'src/Lib/dijet/pwhg_math.h'
      real * 8 dijet_pt2solve,pt2
c i set by dijet_DZERO: 1 for first call, 2 for subsequent calls, 3 for last call
c before a normal exit; not used here
      integer i
      real * 8 xlr,q2,xlam2c,kt2max,cunorm
      integer nlc
      common/dijet_cpt2solve/xlr,q2,kt2max,xlam2c,cunorm,nlc
...
c see Notes/upperbounding-isr.pdf
     ...
     #           + xlr
               ...
               dijet_pt2solve=cunorm*pi/b0*(log(2d0)
...
c         dijet_pt2solve=cunorm*pi/b0/2
...
c final state radiation
     ...
     2   /(p*xm)/2.d+0) + xlr
      ...
      subroutine dijet_gen_rad_isr(t)
c Generates hard radiation kinematics according to
c appendix D in ZZ paper.
c
c  common/dijet_cptmin/ptminsq: minimum pt^2 accepted
c 
      implicit none
      include 'src/Lib/dijet/pwhg_math.h'
      include 'src/Lib/dijet/nlegborn.h'
      ...
      integer nlc
      common/dijet_cpt2solve/xlr,q2,kt2max,xlam2c,unorm,nlc
      real * 8 xmin,rv,xp,xm,chi,tk,uk,ubound,ufct,
     #   sborn,value,err,tmp1,tmp2,tmp,rvalue,born,sig
      common/dijet_cdfxmin/xmin
      real * 8 tmax
      common/dijet_ctmax/tmax
      real * 8 dijet_random,dijet_pt2solve,dfxmin,dijet_pwhg_alphas0,dijet_pwhg_upperb_rad
      external dijet_random,dijet_pt2solve,dfxmin,dijet_pwhg_alphas0,dijet_pwhg_upperb_rad
...
c see section 4 in ZZ paper, last paragraph
      ...
      xlr=0
 1    continue
      xlr=xlr+log(dijet_random())
c CERNLIB voodoo:
      call dijet_KERSET('C205. ',0,0,101)
c solve for zero of dijet_pt2solve
c dijet_DZERO(xmin,xmax,x,err,eps,maxcalls,function)
...
c eps: required accuracy
      call dijet_DZERO(rad_ptsqmin,kt2max,t,err,1d-8,1000000,dijet_pt2solve)
c error conditions
      if(t.eq.0.and.err.lt.0d0
     # .and.err.gt.rad_ptsqmin-kt2max) then
         write(*,*) 'dijet_DZERO fails'
...
c to set xmuren2:
      call dijet_set_rad_scales(t)
      tmp2=st_alpha / dijet_pwhg_alphas0(t,rad_lamll,nlc)
      tmp=tmp1*tmp2
      if(tmp.gt.1) then
...
c using       d sqrt(xm-x) /sqrt(xp-xm) (rad_iupper=1) or d sqrt(xm-x) /(xmin sqrt(xp-xm)) (rad_iupper=2) as upper bound using hit and miss
 2    chi=sqrt(xm-xmin)*dijet_random()
      x=xm-chi**2
      if(rad_iupperisr.eq.1) then
         if(dijet_random().gt.sqrt(xp-xm)/sqrt(xp-x)) goto 2
      elseif(rad_iupperisr.eq.2) then
         if(dijet_random().gt.(xmin*sqrt(xp-xm))/(x*sqrt(xp-x))) goto 2
      endif
c get y (abs to avoid tiny negative values)
      y=sqrt(abs(1-4*x/(1-x)**2*t/sborn))
      if(dijet_random().gt.0.5d0) y=-y
...
c that some pdf vanish (typically heavy flavour pdf's)
      ...
      kn_y=y
      kn_csi=1-x
      kn_azi=2*pi*dijet_random()
      ubound=born*dijet_pwhg_upperb_rad()*unorm*ufct
      call dijet_gen_real_phsp_isr_rad
      call dijet_sigreal_rad(sig)
      value=sig*kn_jacreal
      if(value.gt.ubound) then
         call dijet_increasecnt(
     # 'upper bound failures in generation of radiation')
      endif
      rvalue=dijet_random()*ubound
      if(rvalue.gt.value) then
         call dijet_increasecnt('vetoed radiation')
         goto 1
      endif
      end


      subroutine dijet_gen_rad_fsr(t)
...
c 
     ...
     #   s,value,err,tmp,rvalue,born,sig
      ...
      real * 8 tmax
      common/dijet_ctmax/tmax
      real * 8 dijet_random,dijet_pt2solve,dijet_pwhg_alphas0,dijet_pwhg_upperb_rad
      external dijet_random,dijet_pt2solve,dijet_pwhg_alphas0,dijet_pwhg_upperb_rad
      unorm=rad_norms(rad_kinreg,rad_ubornidx)
c kn_sborn=kn_sreal:
      s=kn_sborn
...
c vetoes:
      ...
      call dijet_set_rad_scales(t)
      if(rad_iupperfsr.eq.1) then
         tmp=st_alpha / dijet_pwhg_alphas0(t,rad_lamll,nlc)
      elseif(rad_iupperfsr.eq.2) then
         tmp=st_alpha
      elseif(rad_iupperfsr.eq.3) then
         tmp=st_alpha
...
c get y
         y=1-2*t/(s*csi**2)
         if(dijet_random().gt.(csi-t/s)) goto 1
      else
         write(*,*) ' dijet_gen_rad_fsr:  rad_iupper=',rad_iupperfsr,
     1        ' invalid'
      endif
c
c extra suppression factor of upper bounding function (may depend upon radiation variables)
      call dijet_uboundfct(ufct,csi,y)
      if(dijet_random().gt.ufct) goto 1
c Veto from upper bound to real value. Count how many vetoes,
c since these may be expensive.
...
c that some pdf vanish (typically heavy flavour pdf's)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      real * 8 dijet_random
      external dijet_random
      azi=2d0*pi*dijet_random()
      sazi = sin(azi)
      cazi = cos(azi)
      if (kn_csi.eq.0d0) then
         do ileg=1, nlegborn
            call dijet_mrotate(dir,sazi,cazi,kn_pborn(1,ileg))
         enddo
      else
         do ileg=1, nlegreal
            call dijet_mrotate(dir,sazi,cazi,kn_preal(1,ileg))
         enddo
      endif
      end

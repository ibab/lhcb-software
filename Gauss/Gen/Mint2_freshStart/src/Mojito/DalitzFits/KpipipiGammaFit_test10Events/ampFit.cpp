...
protected:
  ...
  bool _firstCallToGetNorm; // debug june 2011

  long int _numCalls;
public:
  virtual double un_normalised_noPs(){
    if(0 == getEvent()) return 0;
    double ampSq =  _amps->RealVal();
    cout << "call " << ++_numCalls << " \t pz=" << getEvent()->p(1).Z()
	 << ", \t" << _amps->RealVal()
	 << endl;
    getEvent()->print();
    cout << "\n---- " << endl;
  ...
  AmpsPdf( const counted_ptr<FitAmpSum>& amps
    ...
    , _firstCallToGetNorm(true) // debug june 2011
    , _numCalls(0)
 {
  ...
  virtual bool getNorm(){   // debug june 2011
    bool sc = DalitzPdfBaseFastInteg::getNorm();
    cout << " norm = \t" << _norm << endl;
    if(_firstCallToGetNorm){
      if(0 != _fileGen) _fileGen->closeFile();
      _firstCallToGetNorm = false;
    }
    return sc;
  }
  ~AmpsPdf(){
...
int ampFit(){
  ...
  for(int j_ex = 0; j_ex < Nexperiments; j_ex++){
    ...
    // DalitzEventList BpEventList, BmEventList; // debug june 2011

    bool readFromFile= (0 != j_ex);
    ...
    if(0 == j_ex){ // debug june 2011
      if(readFromFile){
	BmEventList.fromFile("BminusEvents.root");
      }else{
    ...
    if(0 == j_ex){ // debug june 2011
      if(readFromFile){
	BpEventList.fromFile("BplusEvents.root");
      }else{
    ...
    */
    ...
    if(do2ndTest){
      //BmPdf.saveIntegrator("smIntegrator");
      //BpPdf.saveIntegrator("spIntegrator");
      AmpsPdf BmPdf_copy(BmAmps, &BmEventList, integPrecision, integMethod
			 , "second_" + (string) BmIntegFileName);
      AmpsPdf BpPdf_copy(BpAmps, &BpEventList, integPrecision, integMethod
			 , "second_" + (string) BpIntegFileName);
      
      //    BmPdf_copy.setIntegrationPrecision(0.9);
      //    BpPdf_copy.setIntegrationPrecision(0.9);
      // above prevents them from generating extra events.
      
      //    BmPdf_copy.setIntegratorFileName("smIntegrator");
      //    BpPdf_copy.setIntegratorFileName("spIntegrator");
      
      Neg2LL<IDalitzEvent> fcnM_copy(&BmPdf_copy, &BmEventList);
      Neg2LL<IDalitzEvent> fcnP_copy(&BpPdf_copy, &BpEventList);
      Neg2LLSum fcn_copy(&fcnM_copy, &fcnP_copy);
      
      Minimiser mini_copy(&fcn_copy);
      //Minimiser mini(&fcnP);
      mini_copy.doFit();    
      cout << "2nd time around, same events " << endl;
      mini_copy.printResultVsInput();
      cout << "-----------------------------------" << endl;
      BmPdf_copy.saveIntegrator("smIntegrator");
      BpPdf_copy.saveIntegrator("spIntegrator");
      
      bool doCopyTest = false;
      if(doCopyTest){
	IDalitzEvent* evtP = &(*BmEventList.begin());
	cout << "============= copy test ==============" << endl;
	cout << "Bm vs Bm copy " << BmPdf.getNewVal(evtP) << ", " << BmPdf_copy.getNewVal(evtP) << endl;
	cout << "Bp vs Bp copy " << BpPdf.getNewVal(evtP) << ", " << BpPdf_copy.getNewVal(evtP) << endl;
	cout << "============= end copy test ==========" << endl;
	
	BpPdf.setEvent(evtP); BpPdf_copy.setEvent(evtP);
	BmPdf.setEvent(evtP); BmPdf_copy.setEvent(evtP);
	
	double oldGamma = (double) gamma;
	gamma.setCurrentFitVal(0);
	cout << "--- for gamma = " << (double) gamma << ", and delta = " << (double) delta << " ---" << endl;
	cout << "Bp vs Bp copy " << BpPdf.getNewVal() << ", " << BpPdf_copy.getNewVal() << endl;
	cout << "Bm vs Bm copy " << BmPdf.getNewVal() << ", " << BmPdf_copy.getNewVal() << endl;
	cout << "--" << endl;
	cout << "Bp vs Bp copy " << BpPdf.un_normalised_noPs() << "/" << BpPdf.getIntegralValue() 
	     << ", " <<  BpPdf_copy.un_normalised_noPs() << "/" << BpPdf_copy.getIntegralValue() << endl; 
	cout << "Bm vs Bm copy " << BmPdf.un_normalised_noPs() << "/" << BmPdf.getIntegralValue() 
	     << ", " <<  BmPdf_copy.un_normalised_noPs() << "/" << BmPdf_copy.getIntegralValue() << endl; 
	cout << "fcn vs fcn_copy " << fcn.getNewVal() << ", " << fcn_copy.getNewVal() << endl;
	cout << "------------" << endl;
	
	gamma.setCurrentFitVal(oldGamma);
	cout << "--- for gamma = " << (double) gamma << ", and delta = " << (double) delta << " ---" << endl;
	cout << "Bp vs Bp copy " << BpPdf.getNewVal() << ", " << BpPdf_copy.getNewVal() << endl;
	cout << "Bm vs Bm copy " << BmPdf.getNewVal() << ", " << BmPdf_copy.getNewVal() << endl;
	cout << "Bp vs Bp copy " << BpPdf.un_normalised_noPs() << "/" << BpPdf.getIntegralValue() 
	...
	double oldDelta = (double) delta;
	delta.setCurrentFitVal(1.5);
	cout << "--- for gamma = " << (double) gamma << ", and delta = " << (double) delta << " ---" << endl;
	cout << "Bp vs Bp copy " << BpPdf.getNewVal() << ", " << BpPdf_copy.getNewVal() << endl;
	cout << "Bm vs Bm copy " << BmPdf.getNewVal() << ", " << BmPdf_copy.getNewVal() << endl;
	cout << "Bp vs Bp copy " << BpPdf.un_normalised_noPs() << "/" << BpPdf.getIntegralValue() 
	...
	gamma.setCurrentFitVal(oldGamma);
	delta.setCurrentFitVal(oldDelta);
	cout << "--- for gamma = " << (double) gamma << ", and delta = " << (double) delta << " ---" << endl;
	cout << "Bp vs Bp copy " << BpPdf.getNewVal() << ", " << BpPdf_copy.getNewVal() << endl;
	cout << "Bm vs Bm copy " << BmPdf.getNewVal() << ", " << BmPdf_copy.getNewVal() << endl;
	cout << "again Bp vs Bp copy " << BpPdf.getNewVal() << ", " << BpPdf_copy.getNewVal() << endl;
	cout << "again Bm vs Bm copy " << BmPdf.getNewVal() << ", " << BmPdf_copy.getNewVal() << endl;
	cout << "Bp vs Bp copy " << BpPdf.un_normalised_noPs() << "/" << BpPdf.getIntegralValue() 
	     << ", " <<  BpPdf_copy.un_normalised_noPs() << "/" << BpPdf_copy.getIntegralValue() << endl; 
	cout << "Bm vs Bm copy " << BmPdf.un_normalised_noPs() << "/" << BmPdf.getIntegralValue() 
	     << ", " <<  BmPdf_copy.un_normalised_noPs() << "/" << BmPdf_copy.getIntegralValue() << endl; 
	...
	BpPdf.resetEventRecord(); BpPdf_copy.resetEventRecord();
	BmPdf.resetEventRecord(); BmPdf_copy.resetEventRecord();
	cout << "fcn vs fcn_copy " << fcn.getNewVal() << ", " << fcn_copy.getNewVal() << endl;
      }
      //    return 0;
	...
	for(int i=0; i < scanBins; i++){
	  ...
	  cout << " === doing gamma = " << (double) gamma << endl;

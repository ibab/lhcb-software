...
{
  if(0 == _a1 || 0 == _a2){
    cout << "ERROR in FitAmpPair::FitAmpPair !"
	 << "\n\t One of the amplitude pointers is zero:"
	 << "\n\t _a1 = " << _a1 << ", _a2 = " << _a2
	 << "\n\t ... will crash now" << endl;
    throw "zero amp pointer in FitAmpPair constructor";
  }
  std::string name_1 = _a1->name();
  std::string name_2 = _a2->name();
  if(name_1 > name_2) swap(_a1, _a2);
...
const std::string& FitAmpPair::makeName(){
  if(0 == amp1() || 0 == amp2()){
    cout << "ERROR in FitAmpPair::makeName():"
...
double FitAmpPair::add(IDalitzEvent* evtPtr
  ...
  complex<double> c=ampValue(evtPtr) * w;
  _lastEntry = c;
  _sum   += c;
...
complex<double> FitAmpPair::fitParValue()const{
  ...
  complex<double> c1 = _a1->AmpPhase();
  complex<double> c2 = _a2->AmpPhase();
  complex<double> c2star = conj(c2);

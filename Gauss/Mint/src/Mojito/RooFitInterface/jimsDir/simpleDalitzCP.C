...
void simpleDalitzCP(){
  ...
  double minx=0, maxx=maxM12sq*1.05;
  int nbx = 200;
  double bwx = (maxx-minx)/((double)nbx);
  double miny=0, maxy=maxM23sq*1.05;
  int nby = 200;
  double bwy = (maxy-miny)/((double)nby);


  TH1F HDpipi("HDpipi","",200,0.0,2.0);
  TH1F HDpipi2("HDpipi2","",200,0.0,2.0);
  
  TH2F HD("HD", "HD", nbx, minx, maxx, nby, miny, maxy);
  TH2F HD2("HD2", "HD2", nbx, minx, maxx, nby, miny, maxy);
  ...
  //  double x = 0.02*ix - 0.01;
  //  double mother = 0.5*(pow(1.8645,2)+2.0*pow(0.1395,2)+pow(0.4976,2)-x);
  //  m12sq.setVal(mother);
  //  m23sq.setVal(mother);
  //  HDpipi.Fill(m13sq.getVal(),D.getVal());
  //  HDpipi2.Fill(m13sq.getVal(),arg(D.getCVal()));
  ...
  for(int ix=1; ix<=nbx; ix++){
    ...
    for(int iy=1; iy<=nby; iy++){
      ...
      double mpipi = (pow(1.8645,2)+2.0*pow(0.1395,2)+pow(0.4976,2)-x-y);
      ...
      if(phaseSpace.getVal() > 0){
        ...
        HDpipi.Fill(mpipi,D.getVal()*phaseSpace.getVal()/10.0);
  ...
  RooDataSet* dataPlus  = phaseSpace.generate(mset, 100*Nevents);

...
## are stored here
#from PyKaliOnline.Paths import Paths, PassIt, MaxIt
...
parser.add_argument("--input-file", type = str, dest = "input_file", default = None)
...
def GetOutput():
    ...
    histofile = ROOT.TFile(os.path.join(pt.store_location(),"OutputHistograms.root"),'recreate')
    ...
    filtxt = open(os.path.join(pt.store_location(),'CalibCoefficients.txt'),'w')
    for ic in lams.keys():
    ...
    fillam = ROOT.TFile(os.path.join(pt.store_location(),"OutputCoefficients.root"),'recreate')
    It2distr        .Write()
    It2distrInner   .Write()

...
//=============================================================================
OTt0OnlineClbr::OTt0OnlineClbr( const std::string& name, ISvcLocator* pSvcLocator)
: AnalysisTask ( name , pSvcLocator ), m_pPublishSvc(0)
{
...
StatusCode OTt0OnlineClbr::start() {
   ...
   ::sprintf(crn, ("Run" +m_xmlFilePath+"v"+ std::to_string(0) + "/"+m_CondStructure +m_xmlFileName).c_str());
   m_PubString = crn;
   if (m_pPublishSvc) m_pPublishSvc->updateItem("OTT0Calibration");
...
StatusCode OTt0OnlineClbr::write_Globalt0_XML(double global_t0)
{
   ...
   //Rn++;
   //::sprintf(crn,"Run %d /group/online/calibrations/RICH_Alignement_%d.xml",Rn,Rn);
   ::sprintf(crn, ("Run" +m_xmlFilePath+"v"+ std::to_string(m_version) + "/"+m_CondStructure +m_xmlFileName).c_str());
   m_PubString = crn;
   if (m_pPublishSvc) m_pPublishSvc->updateItem("OTT0Calibration");

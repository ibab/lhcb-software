    ...
    if (associatedMCParticles.size() >0 ) {
      ...
      for (iMCPart = iMCPartBegin; iMCPart != iMCPartEnd; iMCPart++) {
        verbose() << "  consider next associated MC particle : rec PID " 
                  ...
                  << endmsg;
        TestDST::PrintMCParticle(*iMCPart);        
      } // for iMCPart
    }// if # >0
...
//=============================================================================
StatusCode TestDST::PrintMCParticle(const LHCb::MCParticle* mcParticle) {

  verbose() << "print information about true particle with PID " << mcParticle->particleID().pid()
            << endmsg;

  verbose() << "   pt " << mcParticle->pt() << " p " << mcParticle->p() << endmsg;
  ...
  if (mcParticle->mother() ) {
    verbose() << "   true mother PID " << mcParticle->mother()->particleID().pid() << endmsg;
    TestDST::PrintMCParticle(mcParticle->mother());
  } // if mother

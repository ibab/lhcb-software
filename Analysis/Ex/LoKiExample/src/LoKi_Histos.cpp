// $Id: LoKi_Histos.cpp,v 1.1.1.1 2003-07-24 16:43:50 ibelyaev Exp $
// ============================================================================
...
LOKI_ALGORITHM( LoKi_Histos )
{
  ...
  plot( " Multiplicity of charged kaons " , kaons.size() , 0 , 100 , 50 );
  ...
  for( Range::iterator iKaon = kaons.begin() ; kaons.end() != iKaon ; ++iKaon ) 
    {
      ...
      plot( "Kaon momentum" , P( kaon ) / GeV , 0 , 100 , 100 ) ;
      
      // plot the transverse momentum of charged kaons  
      plot( "Kaon transverse momentum" , PT( kaon ) / GeV , 0 , 5 , 100 ) ;
      
      // evaluate the beta factor for kaon 
      double beta  = P( kaon ) / E( kaon ) ;
      // plot it 
      plot( "Beta factor for charged kaons" , beta , 0.9 , 1.01 , 101 );
      
    };
  
  // fill the histo through the implicit loop over all charged kaons
  plot( kaons , "Confidence Level for charged kaons" , CL , 0 , 1 , 100 );

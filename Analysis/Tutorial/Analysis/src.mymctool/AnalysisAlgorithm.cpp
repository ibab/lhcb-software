...
:	Algorithm(nam, pSvcLocator),
  ...
  declareProperty("MyToolToUse",   m_myToolName = "MCUtilityTool");
}
...
StatusCode AnalysisAlgorithm::initialize()   {
  ...
  // Retrieve private, specially configured tool
  sc = toolsvc->retrieveTool( m_myToolName, m_pMyUtilTool, this );
  if ( sc.isFailure() ) {
    log << MSG::FATAL << "Unable to retrieve requested private tool " 
        << m_toolName << endreq;
    return sc;
  }
  ...
  if ( parts )  {
    ...
    for ( ipart = parts->begin(); ipart != parts->end(); ipart++ )  {
      if ( (*ipart)->particleID().id() == m_partID )   {
        ...
        bool result = m_pMyUtilTool->matchDecayTree( *ipart, m_daugID );
        if ( result ) {

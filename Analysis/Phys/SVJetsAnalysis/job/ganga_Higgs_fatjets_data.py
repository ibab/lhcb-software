...
j.name = 'EWData_Strip17_DaVinci' + DaVinciVersion
...
j.splitter = DiracSplitter ( filesPerJob = 10, maxFiles = -1 , ignoremissing = True )
...
#j.outputdata    = ["Zjet_MC.root"]
j.outputdata    = ["ZJESAllMC.root","JEShistosMC.root"]
...
bkq.path  = "/LHCb/Collision11/Beam3500GeV-VeloClosed-MagDown/Real Data/Reco12/Stripping17/90000000/EW.DST"# ( Full stream )
bkqup = BKQuery( dqflag = "All" , path = "", type = "Path" )
bkqup.path  = "/LHCb/Collision11/Beam3500GeV-VeloClosed-MagUp/Real Data/Reco12/Stripping17/90000000/EW.DST"# ( Full stream )

dataset_All = LHCbDataset()
dataset_All = LHCbDataset(files = [ f for f in bkq.getDataset().files ] )
dataset_All.files.extend(bkqup.getDataset().files)

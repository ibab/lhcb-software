...
#define NN_bS new BDTNode_Bs_vt
   
#ifndef BDTNode_Bs_vt__def
#define BDTNode_Bs_vt__def
...
public:
   ...
   BDTNode_Bs_vt ( BDTNode_Bs_vt* left,BDTNode_Bs_vt* right,
   ...
   virtual ~BDTNode_Bs_vt();

   // test event if it decends the tree at this node to the right
   virtual bool GoesRight( const std::vector<double>& inputValues ) const;
   BDTNode_Bs_vt* GetRight( void )  {return fRight; };

   // test event if it decends the tree at this node to the left 
   virtual bool GoesLeft ( const std::vector<double>& inputValues ) const;
   BDTNode_Bs_vt* GetLeft( void ) { return fLeft; };   
...
private:
   ...
   BDTNode_Bs_vt*   fLeft;     // pointer to the left daughter node
   BDTNode_Bs_vt*   fRight;    // pointer to the right daughter node
...
//_______________________________________________________________________
bool BDTNode_Bs_vt::GoesRight( const std::vector<double>& inputValues ) const
{
   // test event if it decends the tree at this node to the right
   bool result;
...
//_______________________________________________________________________
bool BDTNode_Bs_vt::GoesLeft( const std::vector<double>& inputValues ) const
{
   // test event if it decends the tree at this node to the left
...
class ReadBDT_bS_vt : public IClassifierReader {

 public:

   // constructor
   ReadBDT_bS_vt( std::vector<std::string>& theInputVars ) 
      : IClassifierReader(),
        fClassName( "ReadBDT_bS_vt" ),
        fNvars( 6 ),
        fIsNormalised( false )
   {      
 ...
 private:
   ...
   std::vector<BDTNode_Bs_vt*> fForest;       // i.e. root nodes of decision trees
   std::vector<double>                fBoostWeights; // the weights applied in the individual boosts
};

double ReadBDT_bS_vt::GetMvaValue__( const std::vector<double>& inputValues ) const
{
   double myMVA = 0;
   double norm  = 0;
   for (unsigned int itree=0; itree<fForest.size(); itree++){
      BDTNode_Bs_vt *current = fForest[itree];
      while (current->GetNodeType() == 0) { //intermediate node
         if (current->GoesRight(inputValues)) current=(BDTNode_Bs_vt*)current->GetRight();
         else current=(BDTNode_Bs_vt*)current->GetLeft();
      }
...
// Clean up
inline void ReadBDT_bS_vt::Clear() 
{
   for (unsigned int itree=0; itree<fForest.size(); itree++) { 
      delete fForest[itree]; 
   }
}
   inline double ReadBDT_bS_vt::GetMvaValue( const std::vector<double>& inputValues ) const
   {

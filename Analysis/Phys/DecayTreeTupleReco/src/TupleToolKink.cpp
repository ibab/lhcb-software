...
//-----------------------------------------------------------------------------
// Implementation file for class : TupleToolKink
//
...
DECLARE_TOOL_FACTORY( TupleToolKink )


//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
TupleToolKink::TupleToolKink( const std::string& type,
...
StatusCode TupleToolKink::initialize(){
  StatusCode sc = m_trackfitter.retrieve() ;
...
// Destructor
//=============================================================================
TupleToolKink::~TupleToolKink() {} 
//=============================================================================
StatusCode TupleToolKink::fill( const LHCb::Particle*, 
...
{                                                         
  ...
  if(sc.isSuccess()){
    ...
    error() << "Unable to refit track, exiting"<< endreq;
...
//=============================================================================
StatusCode TupleToolKink::kinkChi2( const LHCb::Track& track, double &chi2kink, double &zkink) const
{
  chi2kink = 0;
  zkink = 0;
  ...
  if( fitresult ) {
    ...
    BOOST_FOREACH(  const LHCb::Node* node, nodes ) {
      if(!node){
        warning() << "Nodes not found, lets skip this entry" << endreq;
        return StatusCode::FAILURE;
    ...
    if(N<1){
      warning() << "No measurement nodes found " << endreq;
    ...
    for( int i=1; i<N; i++){
      chi2forward[i] = chi2forward[i-1]  + (measnodes[i] ?  measnodes[i]->deltaChi2Forward().chi2() : 0.);
    }
    ...
    double chi2tot = std::max( chi2forward[N-1], chi2backward[0] ) ;
    ...
    for( int i=minkinkstep-1; i<N-minkinkstep+1; ++i ) {
      double thischi2kink = (chi2tot - chi2forward[i] - chi2backward[i+1]) / ndof ;
    ...
    warning() << "NO  fit result found " << endreq;
...
//=============================================================================
StatusCode TupleToolKink::fitKink(const LHCb::Track &input_track, 
  ...
  if( fitresult ) {
    ...
    BOOST_FOREACH(  const LHCb::FitNode* node, measnodes ) {
      ...
      if(!DVDinv.Invert()){
        warning() << " No inverse matrix, no kink fit chi2 " << DVD <<  endreq;
        return StatusCode::FAILURE;
      }
      if (msgLevel(MSG::DEBUG)) 
      ...
      double deltaR = sqrt(delta(2)*delta(2) + delta(3)*delta(3));
      double deltaR_err = sqrt(pow(delta(2),2)*cov_2(2,2) + pow(delta(3),2)*cov_2(3,3)+
    ...
    warning() << " Fit result not found, skipping kinkFit calculation " << endreq;
    return StatusCode::FAILURE;
  }
  kink_chi2 = max_chi2;

                                       ...
                                       , Tuples::Tuple& tuple )
{
  ...
  if(checkPID){
    ...
    if (msgLevel(MSG::VERBOSE)) verbose() << "Filling TISTOS information for particle with pid  " << P->particleID().pid()<<endmsg;
                                         ...
                                         , Tuples::Tuple& tuple )
{
  ...
  if( m_verboseL0 )
	...
	tuple->column( prefix+name+"_Dec", decision);
	tuple->column( prefix+name+"_TIS", tis);
	tuple->column( prefix+name+"_TOS", tos);
      }
    }
...
std::string TupleToolTISTOS::getName(std::string mystr){

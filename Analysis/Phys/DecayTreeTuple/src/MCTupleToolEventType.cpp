...
StatusCode MCTupleToolEventType::fill( Tuples::Tuple& tuple )/*( const LHCb::Particle* 
  ...
  {
    if(msgLevel(MSG::DEBUG) || msgLevel(MSG::VERBOSE)) debug() << "Running event type finder on full event" << endmsg;
    //slow or fast method?
    ...
    if(m_fillPseudoFind && (msgLevel(MSG::DEBUG) || msgLevel(MSG::VERBOSE)) ) 
  ...
  if(m_fillWholeEvent)
    {
      if(msgLevel(MSG::DEBUG) || msgLevel(MSG::VERBOSE)) debug() << "filling info for whole event" << endmsg;
      if(m_fillSlowFind)
      {

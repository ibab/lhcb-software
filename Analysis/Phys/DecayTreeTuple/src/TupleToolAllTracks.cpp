...
DECLARE_TOOL_FACTORY( TupleToolAllTracks )
  ...
  , m_particleDescendants(0)
{
  ...
  declareProperty( "ANNPID"         ,m_ANNPIDCut      = -1.f                                , "Cut on ProbNN of added input particle type");
  declareProperty( "Theta"          ,m_theta          = -1.f                                , "Cut on theta angle of the added input particle");
  declareProperty( "DeltaPhi"       ,m_deltaphi       = -1.f                                , "Cut on the difference between phi angles of target and added particle");
  declareProperty( "GhostProb"      ,m_GhostProb      =  1.f                                , "Cut on the added input particle ghost probability");
  declareProperty( "DLLPID"         ,m_DLLPIDCut      = -9999.f                             , "Cut on DLL of added input particle type");                             //NB: PIDPi == -PIDK!
  declareProperty( "NewVertexChi2"  ,m_VertexChi2Cut  = -1.f                                , "Cut on Vertex chi2 with added particle");                              //NB: if -1, no cut is placed
  declareProperty( "ImprovedVertex" ,m_improvedVertex = -1.f                                , "Cut on Vertex chi2 w added particle - Vertex chi2 w/o added particle");//NB: if -1, no cut is placed
...
StatusCode TupleToolAllTracks::initialize() {
  ...
  for(auto location = m_InputParticles.begin(); location != m_InputParticles.end() ; location++ ){
    ...
    if (!exist<Particle::Range>(*location)){
      if (msgLevel(MSG::WARNING)) warning() << "No particles at " << *location << "! Skipping this TES location!" << endreq;
                                     ...
                                     , Tuples::Tuple& tuple )
{
  ...
  Vertex v;
  Particle::ConstVector parts2Vertex = Mother->daughtersVector();
  ...
  for(auto location : m_InputParticles){    
    ...
    if (msgLevel(MSG::DEBUG)) debug() << "Getting particles from " << location << " with " << (parts).size() << " particles" << endreq;
    for(auto *part : parts){
      ...
      bool proto_exits = part->isBasicParticle() && fabs(PID) != 22 && fabs(PID) != 111 && fabs(PID) != 221;

      //cut on track types includes Velo, VeloR, Long, Upstream and Downstream tracks
      if(proto_exits && ((part->proto()->track()->type() < Track::Types::Velo || part->proto()->track()->type() > Track::Types::Downstream) || isTrackInDecay(part,Mother)))continue;
      ...
      auto PIDVariables_underPIDHypothesis = [=] () -> std::pair<float,float> {
        if     (fabs(PID)==211.f) return {probNNpi,-pidK};
        else if(fabs(PID)==321.f) return {probNNk,pidK};
        else if(fabs(PID)==2212.f)return {probNNp,pidP};
        else if(fabs(PID)==13.f)  return {probNNmu,pidmu};
        else if(fabs(PID)==11.f)  return {probNNe,pide};
      ...
      double oldip, oldipchi2, newip, newipchi2, oldfd, oldfdchi2, newfd, newfdchi2;
      //Vertex chi2's
      auto newvertchi2 = vtxWithExtraTrack.chi2PerDoF();
      if(m_VertexChi2Cut != -1 && (newvertchi2 > m_VertexChi2Cut || newvertchi2 < 0)) continue;
      auto oldvertchi2 = v.chi2PerDoF();
      if(m_improvedVertex != -1 && (newvertchi2 > oldvertchi2 + m_improvedVertex || newvertchi2 < 0)) continue;
      //Vertex IP's
...
bool TupleToolAllTracks::isTrackInDecay(const Particle* particle, const Particle* mother){
  ...
  for(auto* it : stable_daughters){
    if(fabs(it->particleID().pid()) != 22 && fabs(it->particleID().pid()) != 111 && fabs(it->particleID().pid()) != 221){

...
StatusCode DecayTreeTupleBase::finalize()
{
  ...
  for ( std::vector<TupleToolDecay*>::iterator di = m_decays.begin();
        di != m_decays.end(); ++di )
  {
    (*di)->finalize();
    //(*di)->release();
...
{
  if ( !mit )
    Exception("DecayTreeTupleBase::matchSubDecays TupleToolDecay undefined");

  if ( !mit->decayFinder() )
    Exception("DecayTreeTupleBase::matchSubDecays TupleToolDecay DecayFinder undefined");
                                         ...
                                         const TupleToolDecay* mit)
{
  ...
  if ( !mit->mcDecayFinder() )
    Exception("DecayTreeTupleBase::matchSubDecays TupleToolDecay MCDecayFinder undefined");

...
Double_t RooRelBreitWigner::evaluate() const
{
  ...
  RooComplex T(x*x,0.0);
  RooComplex denom(mean*mean-x*x,-1*temp);
...
Double_t RooRelBreitWigner::getWidth() const
{
  ...
  if (q>0 && q0>0 && x>0 && mean>0) {
    result = width * getQterm(q,q0) * (mean*mean/(x*x)) * (getFF(q0) / getFF(q));
    //result = width * getQterm(q,q0) * (mean/x) * (getFF(q) / getFF(q0));
  }
  return (result);
}

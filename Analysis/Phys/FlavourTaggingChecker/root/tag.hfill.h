//Fill standard control plots
	...
	if(itag[3]==TrueTag) { 
	  ...
	  hr_k_weightedch->Fill(k_weightedch/k_weightedpt);
	  ...
	  hw_k_weightedch->Fill(k_weightedch/k_weightedpt);
      ...
      if(bkaonS || bpionS) { 
	int isame= ikaonS;
	if(bpionS)isame=ipionS; 
	if(bpionS)pS_weightedch=kS_weightedch; 
	if(bpionS)pS_weightedpt=kS_weightedpt; 
	if(itag[4]==TrueTag) { 
	  hr_kS_p->Fill(P[isame]); 
	  hr_kS_pt->Fill(Pt[isame]); 
	  hr_SS_weightedch->Fill(kS_weightedch/kS_weightedpt);
	  hr_kS_ips->Fill(fabs(ip[isame]/iperr[isame])); 
	  if(krec>1) hr_kS_ippu->Fill(ipPU[isame]);
	  hr_kS_lcs->Fill(lcs[isame]); 
	  hr_kS_pid->Fill(PIDk[isame]); 
	  hr_kS_tsal->Fill(tsal[isame]); 
	  hr_kS_mult->Fill(N); 
	  if(bkaonS)hr_kS_ncand->Fill(ncand_kS); 
	  if(bpionS)hr_kS_ncand->Fill(ncand_pS); 
	}
	if(itag[4]==-TrueTag) { 
	  hw_kS_p->Fill(P[isame]); 
	  hw_kS_pt->Fill(Pt[isame]);
	  hw_SS_weightedch->Fill(kS_weightedch/kS_weightedpt);
	  hw_kS_ips->Fill(fabs(ip[isame]/iperr[isame])); 
	  if(krec>1) hw_kS_ippu->Fill(ipPU[isame]);
	  hw_kS_lcs->Fill(lcs[isame]); 
	  hw_kS_pid->Fill(PIDk[isame]); 
	  hw_kS_tsal->Fill(tsal[isame]); 
	  hw_kS_mult->Fill(N); 
	  if(bkaonS)hw_kS_ncand->Fill(ncand_kS); 
	  if(bpionS)hw_kS_ncand->Fill(ncand_pS); 
	}
      }
...
//       }
	...
	//fill l0tistos
	h6u->Fill(var);
	if(tagdecision==TrueTag) h6r->Fill(var); else h6w->Fill(var);
	if (globall0tis) {if(tagdecision==TrueTag) h6rtis->Fill(var); else h6wtis->Fill(var);}
	if (globall0tos) {if(tagdecision==TrueTag) h6rtos->Fill(var); else h6wtos->Fill(var);}
        if (globall0tis && globall0tos) {if(tagdecision==TrueTag) h6rtistos->Fill(var); else h6wtistos->Fill(var);}
	if (globall0tis) h6tis->Fill(var); else h6notis->Fill(var);
	if (globall0tos) h6tos->Fill(var); else h6notos->Fill(var);
	if (globall0tis && globall0tos) h6tistos->Fill(var); else h6notistos->Fill(var);
      }

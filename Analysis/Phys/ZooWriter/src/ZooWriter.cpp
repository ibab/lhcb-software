...
ZooP* ZooWriter::GetSaved(const LHCb::Particle* p, const LHCb::RecVertex* recvtx) {
  ...
  if(!hasBestPV){
    const LHCb::RecVertex *newpv = (const LHCb::RecVertex*) this->bestPV(p);
    if(newpv){
      pv = *newpv;
      hasBestPV = true;
    }
  ...
  if (ch.empty()) {
    ...
    if (hasBestPV) {
      ...
      if (rpv) { //change of policy: NO AUTOMATIC REFITTING IF INTELLIGENCE IS SWITCHED OFF

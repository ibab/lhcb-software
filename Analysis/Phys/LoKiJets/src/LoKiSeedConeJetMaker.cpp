...
( const IJetMaker::Input& input_ ,  const LHCb::RecVertex& vtx_ , IJetMaker::Jets& jets_ ) const 
{
  ...
  if(m_uniquetrk){
    ...
    if(m_sort == 1) std::sort(outputP.begin(), outputP.end(),sortPt() );  
    if(m_sort == 2) std::sort(outputP.begin(), outputP.end(),sortE() );
    if(m_sort == 3) std::sort(outputP.begin(), outputP.end(),sortEta() );
    if(m_sort == 4) std::sort(outputP.begin(), outputP.end(),sortDauPt() );

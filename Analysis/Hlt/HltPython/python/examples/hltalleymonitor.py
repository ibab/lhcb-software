...
1) Rate
2) Time and integrated time
   Time per algorithm
3) Candidates of each selection
4) Quantities of the filters (before apply them)
   Best Quantity of a given filter
...
CHANNEL = arguments.argument("ch"," channel name ","Bd2KPi")
...
PLOT = arguments.argument("plot"," plot and save as ps histograms ", True)
...
EOPTS.append('HistogramPersistencySvc.OutputFile = "'+CHANNEL+'hltjob.root"')
...
def bookDecision():
    names = map(lambda x: "Hlt1"+x+"Decision",ALLEYS)
    names.append("OR")
    names.append("AND")
    n = len(names)+1

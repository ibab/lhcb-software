// $Header: /afs/cern.ch/project/cvs/reps/lhcb/Rec/Brunel/src/BrunelSicb/BrunelRecoTRAC.cpp,v 1.1.1.1 2001-03-14 16:06:47 cattaneb Exp $


// Include files
#include "BrunelRecoTRAC.h"
#include "GaudiKernel/MsgStream.h"
#include "GaudiKernel/AlgFactory.h"

//------------------------------------------------------------------------------
//
// Implementation of class :  BrunelRecoTRAC
//
// Calls SICB track finding (cheating) routine AXTFIT.
...
static const AlgFactory<BrunelRecoTRAC>  s_factory;
const IAlgFactory& BrunelRecoTRACFactory = s_factory;


extern "C" {
#ifdef WIN32
  void __stdcall DUMINIT( void );
  void __stdcall AXTFIT( void );
...
BrunelRecoTRAC::BrunelRecoTRAC( const std::string& name, 
...
IAlgorithm* BrunelRecoTRAC::create( const std::string& name, ISvcLocator* loc )
{
  IAlgorithm *alg = new BrunelRecoTRAC( name, loc );
  return alg;
}
...
StatusCode BrunelRecoTRAC::initialize() 
{
  DUMINIT();
...
StatusCode BrunelRecoTRAC::execute()
{
  AXTFIT();
  MsgStream log( messageService(), name() );
...
StatusCode BrunelRecoTRAC::finalize()
{
  DUMLAST();
  MsgStream log( messageService(), name() );

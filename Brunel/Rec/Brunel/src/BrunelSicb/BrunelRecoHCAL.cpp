// $Header: /afs/cern.ch/project/cvs/reps/lhcb/Rec/Brunel/src/BrunelSicb/BrunelRecoHCAL.cpp,v 1.1.1.1 2001-03-14 16:06:46 cattaneb Exp $


// Include files
#include "BrunelRecoHCAL.h"
#include "GaudiKernel/MsgStream.h"
#include "GaudiKernel/AlgFactory.h"

//------------------------------------------------------------------------------
//
// Implementation of class :  BrunelRecoHCAL
//
// Calls SICB HCAL reconstruction steering routine HCRECO.
...
static const AlgFactory<BrunelRecoHCAL>  s_factory;
const IAlgFactory& BrunelRecoHCALFactory = s_factory;


extern "C" {
#ifdef WIN32
  void __stdcall DUMINIT( void );
  void __stdcall HCRECO( void );
...
BrunelRecoHCAL::BrunelRecoHCAL( const std::string& name, 
...
IAlgorithm* BrunelRecoHCAL::create( const std::string& name, ISvcLocator* loc )
{
  IAlgorithm *alg = new BrunelRecoHCAL( name, loc );
  return alg;
}
...
StatusCode BrunelRecoHCAL::initialize() 
{
  DUMINIT();
...
StatusCode BrunelRecoHCAL::execute()
{
  HCRECO();
  MsgStream log( messageService(), name() );
...
StatusCode BrunelRecoHCAL::finalize()
{
  DUMLAST();
  MsgStream log( messageService(), name() );

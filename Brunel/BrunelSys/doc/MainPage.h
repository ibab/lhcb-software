/** \mainpage notitle
 *  \anchor bruneldoxygenmain
 *
 * This is the code reference manual for the Brunel reconstruction classes.

 * These pages have been generated directly from the code and reflect the exact 
 * state of the software for this version of the BrunelSys packages. More
 * information is available from the 
 * <a href="http://cern.ch/lhcb-comp/Reconstruction/">web pages</a>
 * of the Brunel project
 *
 * \sa
 * \li \ref lhcbdoxygenmain  "LHCbSys documentation"
 * \li \ref gaudidoxygenmain "Gaudi documentation"
 * \li \ref externaldocs     "Related external libraries"

 */
